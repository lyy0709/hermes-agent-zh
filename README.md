...
**由 [Nous Research](https://nousresearch.com) 构建的自我改进 AI Agent。** 它是唯一内置学习循环的 Agent —— 它从经验中创建技能，在使用中改进它们，推动自己持久化知识，搜索自己过去的对话，并在跨会话中构建关于你的日益深入的模型。在 5 美元的 VPS、GPU 集群或空闲时成本几乎为零的无服务器基础设施上运行它。它不局限于你的笔记本电脑 —— 当它在云 VM 上工作时，你可以通过 Telegram 与它交谈。
...
<tr><td><b>闭环学习</b></td><td>Agent 管理的记忆，带有定期提醒。复杂任务后自主创建技能。技能在使用过程中自我改进。使用 FTS5 会话搜索和 LLM 摘要实现跨会话回忆。<a href="https://github.com/plastic-labs/honcho">Honcho</a> 辩证用户建模。兼容 <a href="https://agentskills.io">agentskills.io</a> 开放标准。</td></tr>
...
<tr><td><b>随处运行，不限于你的笔记本电脑</b></td><td>六个终端后端 —— 本地、Docker、SSH、Daytona、Singularity 和 Modal。Daytona 和 Modal 提供无服务器持久性 —— 你的 Agent 的执行环境在空闲时休眠，按需唤醒，会话间成本几乎为零。在 5 美元的 VPS 或 GPU 集群上运行它。</td></tr>
...
适用于 Linux、macOS、WSL2 和通过 Termux 的 Android。安装程序会为你处理特定平台的设置。
...
Hermes 有两个入口点：使用 `hermes` 启动终端 UI，或者运行消息网关并通过 Telegram、Discord、Slack、WhatsApp、Signal 或 Email 与它交谈。一旦进入对话，许多斜杠命令在两个界面中是共享的。
...
| 章节 | 涵盖内容 |
|---------|---------------|
| [快速开始](https://hermes-agent.nousresearch.com/docs/getting-started/quickstart) | 安装 → 设置 → 2 分钟内首次对话 |
...
查看 `hermes claw migrate --help` 获取所有选项，或使用 `openclaw-migration` 技能进行交互式 Agent 引导的迁移，并带有干运行预览。
...
贡献者快速开始 —— 使用 `setup-hermes.sh` 克隆并开始：

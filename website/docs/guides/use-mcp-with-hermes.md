...
最后一点很重要。良好的 MCP 使用不仅仅是“连接一切”，而是“连接正确的东西，使用最小的有用接口”。
...
```text
检查本地项目文件，然后创建一个 GitHub 问题来总结你发现的错误。
```

...
Hermes Agent 包含一个基于 **Tinker-Atropos** 构建的集成式 RL（强化学习）训练流水线。这使得能够通过 Agent 的工具接口完全编排，使用 GRPO（组相对策略优化）和 LoRA 适配器，在特定环境任务上训练语言模型。
...
-   `tinker-atropos` 子模块（位于相对于 Hermes 根目录的 `tinker-atropos/` 路径下）

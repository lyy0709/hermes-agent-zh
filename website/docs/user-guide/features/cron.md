...
sidebar_position: 5
title: "定时任务 (Cron)"
description: "使用自然语言安排自动化任务，用一个 cron 工具管理它们，并附加一个或多个技能"
---
...
cronjob(
    ...
    prompt="寻找新的本地事件和附近有趣的地方，然后将它们合并成一个简短的简报。",
    schedule="every 6h",
    name="本地简报",
)
```
...
Cron 任务现在拥有比 create/remove 更完整的生命周期。
...
Cron 任务继承您配置的备用提供商和凭据池轮换。如果主要 API 密钥受到速率限制或提供商返回错误，cron Agent 可以：
...
:::warning 重要
Cron 任务在完全全新的 Agent 会话中运行。提示词必须包含 Agent 需要的所有内容，这些内容不是由附加技能已经提供的。
:::

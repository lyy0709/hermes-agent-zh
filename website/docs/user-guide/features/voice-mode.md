...
:::tip
`~/.hermes/` 目录和默认的 `config.yaml` 会在您首次运行 `hermes` 时自动创建。您只需要为 API 密钥手动创建 `~/.hermes/.env` 文件。
:::
...
启用 TTS 后，Agent 会在生成文本时**逐句**朗读其回复 — 您无需等待完整响应：
...
Whisper 有时会从静默或背景噪音中生成幻听文本（"Thank you for watching"、"Subscribe" 等）。Agent 使用一组 26 个已知的跨多种语言的幻觉短语，以及一个捕获重复变体的正则表达式模式，来过滤掉这些内容。
---

## 消息网关语音回复（Telegram 和 Discord）

如果你尚未设置你的消息机器人，请参阅平台特定的指南：
...
**私信（个人使用推荐）：** 只需打开与机器人的私信并输入内容——无需 @提及。语音回复和所有命令的工作方式与在频道中相同。

**服务器频道：** 机器人仅在你 @提及它时才会响应（例如 `@hermesbyt4 你好`）。请确保你从提及弹出窗口中选择**机器人用户**，而不是同名的角色。
...
最具沉浸感的语音功能：机器人加入 Discord 语音频道，监听用户说话，转录他们的语音，通过 Agent 处理，并在语音频道中语音回复。
...
:::warning
将机器人重新邀请到它已在的服务器将更新其权限，而不会移除它。你不会丢失任何数据或配置。
:::

#### 2. 特权网关意图

在 [开发者门户](https://discord.com/developers/applications) → 你的应用 → **机器人** → **特权网关意图** 中，启用所有三项：

| 意图 | 用途 |
|--------|---------|
| **在线状态意图** | 检测用户在线/离线状态 |
| **服务器成员意图** | 将语音 SSRC 标识符映射到 Discord 用户 ID |
| **消息内容意图** | 读取频道中的文本消息内容 |

完整的语音频道功能需要所有三项。**服务器成员意图** 尤其关键——没有它，机器人无法识别谁在语音频道中说话。
...
# GROQ_API_KEY=你的密钥            # 替代方案：基于云端、快速、免费层
...
:::info
你必须在运行 `/voice join` 之前处于一个语音频道中。机器人会加入你所在的同一个语音频道。
:::
...
1.  **独立监听** 每个用户的音频流
2.  **检测静音** — 在至少 0.5 秒的语音后，若出现 1.5 秒的静音，则触发处理
...
-   文本频道是指执行 `/voice join` 命令的那个频道

### 回声预防

机器人在播放 TTS 回复时，会自动暂停其音频监听器，防止它听到并重新处理自己的输出。
...
voice:
  ...
  beep_enabled: true               # 播放录音开始/结束的提示音
  silence_threshold: 200           # RMS 水平 (0-32767)，低于此值视为静音

...
2.  **预取相关记忆**在每次对话轮次之前（后台、非阻塞）
3.  **同步对话轮次**到提供商，在每次响应之后
4.  **在会话结束时提取记忆**（对于支持此功能的提供商）
5.  **镜像内置记忆写入**到外部提供商
6.  **添加提供商特定工具**，使 Agent 能够搜索、存储和管理记忆
...
**架构:** 两层上下文注入 —— 基础层（会话摘要 + 表示 + 同伴卡片，按 `contextCadence` 刷新）加上辩证补充层（LLM 推理，按 `dialecticCadence` 刷新）。辩证层根据是否存在基础上下文，自动选择冷启动提示词（通用用户事实）与热启动提示词（会话范围上下文）。
...
Honcho 将会话建模为对等体交换消息——每个 Hermes 配置文件对应一个用户对等体加一个 AI 对等体，所有对等体共享一个工作区。工作区是共享环境：用户对等体在所有配置文件中是全局的，每个 AI 对等体都有自己的身份。每个 AI 对等体都根据自己的观察构建独立的表示/卡片，因此 `coder` 配置文件保持代码导向，而 `writer` 配置文件在同一个用户面前保持编辑导向。
...
由 Volcengine（字节跳动）开发的上下文数据库，具有类文件系统的知识层次结构、分层检索功能，并能自动将记忆提取为 6 个类别。
...
具备知识图谱、实体解析和多策略检索的长期记忆。`hindsight_reflect` 工具提供了其他提供商不具备的跨记忆综合能力。自动保留完整的对话轮次（包括工具调用），并具有会话级别的文档跟踪。
...
本地 SQLite 事实存储，具备 FTS5 全文搜索、信任评分和用于组合代数查询的 HRR（全息简化表示）。

| | |
|---|---|
| **最适合** | 仅限本地、具备高级检索功能的记忆，无外部依赖 |
| **要求** | 无 (SQLite 始终可用)。NumPy 为 HRR 代数运算的可选依赖。 |
| **数据存储** | 本地 SQLite |
| **成本** | 免费 |
...
通过 `brv` CLI 实现的持久化记忆 — 具备分层检索（模糊文本 → LLM 驱动搜索）的层次化知识树。本地优先，可选云端同步。
...
- **画像作用域的容器** — 在 `container_tag` 中使用 `{identity}` (例如 `hermes-{identity}` → `hermes-coder`) 以隔离每个 Hermes 画像的记忆
- **多容器模式** — 启用 `enable_custom_container_tags` 并设置 `custom_containers` 列表，允许 Agent 跨命名容器进行读写。自动操作 (同步、预取) 保持在主容器上。
...
每个提供商的数据都按[画像](/docs/user-guide/profiles)进行隔离：

- **本地存储提供商** (Holographic, ByteRover) 使用 `$HERMES_HOME/` 路径，该路径因画像而异
- **配置文件提供商** (Honcho, Mem0, Hindsight, Supermemory) 将配置存储在 `$HERMES_HOME/` 中，因此每个画像都有自己的凭据
- **云端提供商** (RetainDB) 自动派生画像作用域的项目名称

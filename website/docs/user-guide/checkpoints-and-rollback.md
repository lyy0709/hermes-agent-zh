...
Hermes Agent 会在**破坏性操作**之前自动为您的项目创建快照，并允许您通过一条命令恢复。检查点**默认启用**——当没有文件修改工具触发时，成本为零。
...
禁用时，检查点管理器将不执行任何操作，且从不尝试 Git 操作。
...
1. 验证目标提交存在于影子仓库中。
2. 为当前状态创建一个**回滚前快照**，以便您稍后可以“撤销回滚”。
3. 恢复您工作目录中受跟踪的文件。
...
- **非致命错误** — 检查点管理器内的所有错误都记录在调试级别；您的工具会继续运行。
...
每个 `<hash>` 派生自工作目录的绝对路径。在每个影子仓库内部，您会发现：

- 标准的 Git 内部结构 (`HEAD`、`refs/`、`objects/`)
- 一个 `info/exclude` 文件，包含精心策划的忽略列表
- 一个 `HERMES_WORKDIR` 文件，指回原始的项目根目录

您通常永远不需要手动操作这些。

## 最佳实践

- **保持检查点启用** — 它们默认开启，并且在未修改文件时成本为零。

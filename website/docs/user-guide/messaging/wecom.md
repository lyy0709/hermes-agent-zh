...
将 Hermes 连接到 [企业微信](https://work.weixin.qq.com/)，这是腾讯的企业级通讯平台。该适配器使用企业微信的 AI Bot WebSocket 消息网关进行实时双向通信——无需公共端点或 Webhook。
...
为了进行精细控制，你可以限制特定群组内哪些用户被允许与机器人交互。这需要在 `config.yaml` 中配置：
...
4.  白名单条目支持 `*` 通配符以允许所有用户，并且条目不区分大小写。
5.  条目可以选择使用 `wecom:user:` 或 `wecom:group:` 前缀格式——前缀会被自动去除。

如果未为某个群组配置 `allow_from`，则该群组中的所有用户都被允许（假设该群组本身通过了顶级策略检查）。
...
适配器与企业微信的网关 `wss://openws.work.weixin.qq.com` 保持持久的 WebSocket 连接。

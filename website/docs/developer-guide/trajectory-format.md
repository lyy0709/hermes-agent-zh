...
批量运行器 (`batch_runner.py`) 为每个批次写入一个自定义输出文件（例如 `batch_001_output.jsonl`），并包含额外的元数据字段。
...
`tool_stats` 和 `tool_error_counts` 字典经过规范化，包含所有可能的工具（来自 `model_tools.TOOL_TO_TOOLSET_MAP`），并带有零默认值，确保条目间模式一致，以便 HuggingFace 数据集加载。
...
轨迹转换器将所有推理内容规范化为 `<think>` 标签，无论模型最初如何生成：

1.  **原生思考 Token**（来自 Anthropic、OpenAI o-series 等提供商的 `msg["reasoning"]` 字段）：包装为 `<think>\n{reasoning}\n</think>\n` 并前置到内容之前。
2.  **REASONING_SCRATCHPAD XML**（当原生思考被禁用且模型通过系统提示词指示的 XML 进行推理时）：`<REASONING_SCRATCHPAD>` 标签通过 `convert_scratchpad_to_think()` 转换为 `<think>`。
...
助手消息之后的所有工具结果被分组到单个 `tool` 回合中，并带有 XML 包装的 JSON 响应：
...
- 如果工具内容看起来像 JSON（以 `{` 或 `[` 开头），则将其解析，使 content 字段包含 JSON 对象/数组而不是字符串
- 多个工具结果在一个消息中用换行符连接
...
或通过 `--save-trajectories` 标志。当 Agent 以 `save_trajectories=True` 初始化时，`_save_trajectory()` 方法在每个对话回合结束时被调用。

批量运行器始终保存轨迹（这是其主要目的）。

批量运行器会自动丢弃所有回合中零推理的样本，以避免非推理示例污染训练数据。
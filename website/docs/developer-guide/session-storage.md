...
关键设计决策：
- **WAL 模式**：支持并发读取 + 一个写入器（网关多平台）
- **FTS5 虚拟表**：用于在所有会话消息中进行快速文本搜索
- **会话谱系**：通过 `parent_session_id` 链实现（由上下文压缩触发的会话拆分）
...
每次迁移都使用 `ALTER TABLE ADD COLUMN` 包装在 try/except 中，以处理列已存在的情况（幂等性）。每次成功的迁移块执行后，版本号会递增。
...
- **BEGIN IMMEDIATE** 事务，以便在事务开始时暴露锁竞争
- **定期 WAL 检查点**，每 50 次成功写入执行一次（PASSIVE 模式）

这避免了“车队效应”，即 SQLite 确定性的内部退避机制导致所有竞争写入器在同一时间间隔重试。
...
会话可以通过 `parent_session_id` 形成链。当上下文压缩在网关中触发会话拆分时，就会发生这种情况。

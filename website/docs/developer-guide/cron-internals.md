...
| `gateway/run.py` | 消息网关集成——在长期运行循环中触发定时任务 |
...
| 格式 | 示例 | 行为 |
|--------|---------|----------|
| **相对延迟** | `30m`, `2h`, `1d` | 一次性任务，在指定持续时间后触发 |
| **时间间隔** | `every 2h`, `every 30m` | 周期性任务，按固定间隔触发 |
| **Cron 表达式** | `0 9 * * *` | 标准的 5 字段 cron 语法（分钟、小时、日、月、星期几） |
| **ISO 时间戳** | `2025-01-15T09:00:00` | 一次性任务，在精确时间触发 |

面向模型的接口是一个单一的 `cronjob` 工具，具有动作式操作：`create`、`list`、`update`、`pause`、`resume`、`run`、`remove`。
...
旧版任务可能只有一个 `skill` 字段，而不是 `skills` 数组。调度器在加载时会将其规范化——单个 `skill` 会被提升为 `skills: [skill]`。
...
在网关模式下，调度器滴答周期被集成到网关的主事件循环中。网关在其定期维护周期中调用 `scheduler.tick()`，该周期与消息处理并行运行。
...
-   **备用提供商** — 从 `config.yaml` 读取 `fallback_providers`（列表）或 `fallback_model`（旧版字典），匹配网关的 `_load_fallback_model()` 模式。作为 `fallback_model=` 传递给 `AIAgent.__init__`，后者将两种格式都规范化为备用链。
-   **凭据池** — 使用解析出的运行时提供商名称，通过 `agent.credential_pool` 中的 `load_pool(provider)` 加载。仅在池中有凭据时传递（`pool.has_credentials()`）。支持在 429/速率限制错误时进行同提供商密钥轮换。

这反映了网关的行为——没有它，定时任务 Agent 在遇到速率限制时将无法尝试恢复。
...
| WhatsApp | `whatsapp` | 交付到 WhatsApp |
| Signal | `signal` | 交付到 Signal |
| Matrix | `matrix` | 交付到 Matrix 主房间 |
| Mattermost | `mattermost` | 交付到 Mattermost |
...
| QQ 机器人 | `qqbot` | 通过官方 API v2 交付到 QQ |
...
定时任务交付**不会**镜像到网关会话的对话历史中。它们仅存在于定时任务自身的会话中。这防止了目标聊天对话中出现消息交替违规。
...
调度器使用基于文件的锁定，以防止重叠的滴答周期两次执行同一批到期任务。这在网关模式下很重要，如果前一个滴答周期耗时超过滴答间隔，多个维护周期可能会重叠。

...
TIPS = [
    ...
    "设置 security.website_blocklist.enabled: true 以阻止来自 Web 工具的特定域名。",
    ...
    "上下文文件上限为 20,000 字符，带有智能头/尾截断。",

    # --- 浏览器 ---
    "五个浏览器提供商：本地 Chromium、Browserbase、Browser Use、Camofox 和 Firecrawl。",
    "Camofox 是一个反检测浏览器 — 带有 C++ 指纹欺骗的 Firefox 分支。",
    ...
    "检查点使用 ~/.hermes/checkpoints/ 下的影子仓库 — 您项目的 .git 永远不会被触及。",
    ...
    "定时任务脚本位于 ~/.hermes/scripts/ 下，在 Agent 之前运行 — 非常适合数据收集流水线。",
    ...
    "/compress <焦点主题> 将 60-70% 的总结预算分配给您的主题，并积极修剪其余部分。",
    "在第二次及以后的压缩中，压缩器会更新先前的总结，而不是从头开始。",
    ...
    "BROWSERBASE_ADVANCED_STEALTH=true 启用高级反检测与自定义 Chromium (Scale 计划)。",
    ...
    "压缩器进行廉价预传递：超过 200 字符的工具输出在 LLM 运行前被替换为占位符。",

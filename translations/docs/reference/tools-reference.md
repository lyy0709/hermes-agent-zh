...
**快速统计：** 10 个浏览器工具（核心）+ 2 个浏览器 CDP 工具，4 个文件工具，10 个 RL 工具，4 个 Home Assistant 工具，2 个终端工具，2 个网页工具，5 个飞书工具，7 个 Spotify 工具，5 个元宝工具，2 个 Discord 工具，以及分布在其他工具集中的 15 个独立工具。

:::tip MCP 工具
除了内置工具，Hermes 还可以从 MCP 服务器动态加载工具。MCP 工具会带有服务器名称前缀（例如，`github` MCP 服务器的 `github_create_issue`）。配置方法请参阅 [MCP 集成](/docs/user-guide/features/mcp)。
:::
...
| `browser_press` | 按下一个键盘按键。用于提交表单（Enter）、导航（Tab）或键盘快捷键。需要先调用 `browser_navigate`。 | — |
...
| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `browser_cdp` | 发送原始的 Chrome DevTools Protocol 命令。用于处理高级 `browser_*` 工具未涵盖的浏览器操作的逃生舱口。参见 https://chromedevtools.github.io/devtools-protocol/ | CDP 端点 |
...
| `read_file` | 读取带有行号和分页的文本文件。在终端中使用此工具代替 cat/head/tail。输出格式：'行号\|内容'。如果未找到文件，会建议相似的文件名。对大文件使用偏移量和限制。注意：无法读取图像… | — |
| `search_files` | 搜索文件内容或按名称查找文件。在终端中使用此工具代替 grep/rg/find/ls。基于 Ripgrep，比 shell 等效命令更快。内容搜索（target='content'）：在文件内部进行正则表达式搜索。输出模式：完整匹配行… | — |
...
| `rl_edit_config` | 更新配置字段。首先使用 rl_get_current_config() 查看所选环境的所有可用字段。每个环境都有不同的可配置选项。基础设施设置（tokenizer, URLs, lora_rank, learning_rate…）是固定的。 | TINKER_API_KEY, WANDB_API_KEY |
...
| `rl_stop_training` | 停止正在运行的训练任务。如果指标看起来不佳、训练停滞或您想尝试不同的设置，请使用此工具。 | TINKER_API_KEY, WANDB_API_KEY |
...
| 工具 | 描述 | 需要环境 |
|------|-------------|----------------------|
| `session_search` | 搜索你对过去对话的长期记忆。这是你的回忆功能——每个过去的会话都是可搜索的，此工具会总结发生了什么。在以下情况时主动使用此工具：- 用户说“我们之前做过这个”、“记得当时”、“上次…” | — |
...
| `skill_view` | 技能允许加载特定任务和工作流的信息，以及脚本和模板。加载技能的完整内容或访问其链接的文件（参考、模板、脚本）。首次调用返回 SKILL.md 内容加上… | — |
...
在 `hermes-discord` 平台工具集上注册（仅限消息网关）。使用与消息适配器相同的机器人 Token。
...
在 `hermes-discord` 平台工具集上注册。管理操作要求机器人持有匹配的 Discord 权限。
...
| `yb_query_group_members` | 查询群组的成员（用于 `@` 提及、按名称查找用户、列出机器人）。 | Yuanbao 凭据 |

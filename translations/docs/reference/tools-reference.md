...
:::tip MCP 工具
除了内置工具，Hermes 还可以从 MCP 服务器动态加载工具。MCP 工具带有前缀 `mcp_<server>_`（例如，对于 `github` MCP 服务器，工具名为 `mcp_github_create_issue`）。配置请参见 [MCP 集成](/docs/user-guide/features/mcp)。
:::
...
| `browser_navigate` | 在浏览器中导航到 URL。初始化会话并加载页面。必须在其他浏览器工具之前调用。对于简单的信息检索，建议使用 `web_search` 或 `web_extract`（更快、更便宜）。当您需要…时，请使用浏览器工具。 | — |
| `browser_press` | 按下键盘按键。用于提交表单（Enter）、导航（Tab）或键盘快捷键。需要先调用 `browser_navigate`。 | — |
| `browser_scroll` | 按方向滚动页面。使用此工具来显示当前视口上方或下方可能存在的更多内容。需要先调用 `browser_navigate`。 | — |
...
| `browser_type` | 将文本输入到由其引用 ID 标识的输入字段中。首先清除字段，然后输入新文本。需要先调用 `browser_navigate` 和 `browser_snapshot`。 | — |
| `browser_vision` | 截取当前页面的屏幕截图并使用视觉 AI 进行分析。当您需要从视觉上理解页面内容时使用此工具 — 对于验证码、视觉验证挑战、复杂布局或当文本快照…时特别有用。 | — |

## `browser` 工具集（CDP 门控工具）

这两个工具属于 `browser` 工具集，但仅在会话开始时可通过 Chrome DevTools Protocol 端点访问时才会注册 — 通过 `/browser connect`、`browser.cdp_url` 配置、Browserbase 会话或 Camofox。
...
| `browser_dialog` | 响应原生 JavaScript 对话框（alert / confirm / prompt / beforeunload）。首先调用 `browser_snapshot` — 待处理的对话框会出现在其 `pending_dialogs` 字段中。然后调用 `browser_dialog(action='accept'\|'dismiss')`。 | CDP 端点 |
...
| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `execute_code` | 运行一个可以编程方式调用 Hermes 工具的 Python 脚本。当您需要 3 个以上的工具调用并在它们之间有处理逻辑时，需要在工具输出进入您的上下文之前过滤/减少大型工具输出时，需要条件分支（…）时，请使用此工具。 | — |
...
限定于飞书文档评论智能回复处理器（`gateway/platforms/feishu_comment.py`）。不在 `hermes-cli` 或常规飞书聊天适配器中暴露。

| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `feishu_doc_read` | 根据给定的 `file_type` 和 `token` 读取飞书/Lark 文档（Docx、Doc 或 Sheet）的完整文本内容。 | 飞书应用凭证 |

## `feishu_drive` 工具集

限定于飞书文档评论处理器。驱动对网盘文件的评论读写操作。
...
| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `patch` | 在文件中进行针对性的查找和替换编辑。在终端中使用此工具代替 sed/awk。使用模糊匹配（9 种策略），因此微小的空格/缩进差异不会导致失败。返回统一差异。编辑后自动运行语法检查… | — |
...
| `ha_list_services` | 列出可用的 Home Assistant 服务（操作）以进行设备控制。显示每种设备类型可以执行的操作以及它们接受的参数。使用此工具来发现如何控制通过 ha_list_entities 找到的设备。 | — |
...
仅在 Agent 由看板调度器生成时注册（设置了 `HERMES_KANBAN_TASK` 环境变量）。让工作者能够通过结构化的交接负载标记任务完成、阻塞以等待人工输入、在长时间操作期间发送心跳、在评论线程上评论，以及（对于编排器）将任务分解为子任务。完整工作流请参阅[看板多 Agent](/docs/user-guide/features/kanban)。

| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `kanban_show` | 显示分配给此工作者的活动看板任务（标题、描述、评论、依赖项）。 | `HERMES_KANBAN_TASK` |
| `kanban_complete` | 使用结构化的交接负载（结果、工件、后续任务）将当前任务标记为完成。 | `HERMES_KANBAN_TASK` |
| `kanban_block` | 因向用户提问而阻塞当前任务——调度器会暂停、显示问题，并在人工回复后恢复。 | `HERMES_KANBAN_TASK` |
| `kanban_heartbeat` | 在长时间运行的操作期间发送进度心跳，以便调度器知道工作者仍然存活。 | `HERMES_KANBAN_TASK` |
| `kanban_comment` | 在不改变任务状态的情况下向任务线程添加评论——用于展示中间发现。 | `HERMES_KANBAN_TASK` |
| `kanban_create` | （仅限编排器）从当前任务分解出子任务。 | `HERMES_KANBAN_TASK` + 编排器角色 |
| `kanban_link` | （仅限编排器）将相关任务链接在一起（阻塞/被阻塞/相关）。 | `HERMES_KANBAN_TASK` + 编排器角色 |

## `memory` 工具集

| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `memory` | 将重要信息保存到持久化记忆中，该记忆在会话之间持续存在。你的记忆会在会话开始时出现在你的系统提示词中——这是你在对话之间记住关于用户和环境信息的方式。何时保存… | — |
## `messaging` 工具集
...
| 工具 | 描述 | 所需执行环境 |
|------|-------------|----------------------|
| `rl_check_status` | 获取训练运行的状态和指标。频率限制：对同一运行，两次检查之间强制间隔至少 30 分钟。返回 WandB 指标：step、state、reward_mean、loss、percent_correct。 | TINKER_API_KEY, WANDB_API_KEY |
...
| `rl_select_environment` | 为训练选择一个 RL 环境。加载环境的默认配置。选择后，使用 `rl_get_current_config()` 查看设置，使用 `rl_edit_config()` 修改它们。 | TINKER_API_KEY, WANDB_API_KEY |
| `rl_start_training` | 使用当前环境和配置启动一个新的 RL 训练运行。大多数训练参数（lora_rank、learning_rate 等）是固定的。在启动前使用 `rl_edit_config()` 设置 group_size、batch_size、wandb_project。警告：训练… | TINKER_API_KEY, WANDB_API_KEY |
| `rl_stop_training` | 停止正在运行的训练任务。如果指标看起来不佳、训练停滞或您想尝试不同的设置时使用。 | TINKER_API_KEY, WANDB_API_KEY |
| `rl_test_inference` | 针对任何环境的快速推理测试。使用 OpenRouter 运行几步推理 + 评分。默认：3 步 x 16 次完成 = 每个模型 48 次 rollout，测试 3 个模型 = 总共 144 次。测试环境加载、提示构建、推理… | TINKER_API_KEY, WANDB_API_KEY |

## `session_search` 工具集

| 工具 | 描述 | 所需执行环境 |
|------|-------------|----------------------|
| `session_search` | 搜索您过去对话的长期记忆。这是您的回忆 — 每个过去的会话都是可搜索的，此工具会总结发生了什么。在以下情况下主动使用此工具：- 用户说“我们之前做过这个”、“记得当…”、“上次… | — |

## `skills` 工具集

| 工具 | 描述 | 所需执行环境 |
|------|-------------|----------------------|
| `skill_manage` | 管理技能（创建、更新、删除）。技能是您的程序性记忆 — 针对重复任务类型的可重用方法。新技能保存到 `~/.hermes/skills/`；现有技能可以在其所在位置修改。操作：create（完整的 SKILL.m…） | — |
...
| `terminal` | 在 Linux 环境中执行 shell 命令。文件系统在调用之间持久化。对于长时间运行的服务器，设置 `background=true`。设置 `notify_on_complete=true`（与 `background=true` 一起）可在进程完成时自动收到通知 — 无需轮询。不要使用 cat/head/tail — 使用 read_file。不要使用 grep/rg/find — 使用 search_files。 | — |
...
可选工具集（未加载到默认的 `hermes-cli` 集合中）。通过 `--toolsets video` 添加或在您的 `toolsets:` 配置中包含 `video`。
...
| 工具 | 描述 | 所需环境变量 |
|------|-------------|----------------------|
| `web_search` | 在网络上搜索信息。默认返回最多 5 个结果，包含标题、URL 和描述。接受一个可选的 `limit` 参数（1-100，默认为 5）。查询会传递给配置的后端，因此当后端支持时，诸如 `site:domain`、`filetype:pdf`、`intitle:word`、`-term` 和 `"exact phrase"` 等操作符可能有效。 | EXA_API_KEY 或 PARALLEL_API_KEY 或 FIRECRAWL_API_KEY 或 TAVILY_API_KEY |
| `web_extract` | 从网页 URL 提取内容。以 Markdown 格式返回页面内容。也适用于 PDF URL —— 直接传递 PDF 链接，它会转换为 Markdown 文本。小于 5000 字符的页面返回完整的 Markdown；更大的页面则由 LLM 进行总结。 | EXA_API_KEY 或 PARALLEL_API_KEY 或 FIRECRAWL_API_KEY 或 TAVILY_API_KEY |

## `tts` 工具集

| 工具 | 描述 | 所需环境变量 |
|------|-------------|----------------------|
| `text_to_speech` | 将文本转换为语音音频。返回一个 MEDIA: 路径，平台会将其作为语音消息传递。在 Telegram 上，它会以语音气泡形式播放；在 Discord/WhatsApp 上，则作为音频附件。在 CLI 模式下，保存到 ~/voice-memos/。语音和提供商… | — |
...
| 工具 | 描述 | 所需环境变量 |
|------|-------------|----------------------|
| `yb_query_group_info` | 查询群组（在应用中称为"派/Pai"）的基本信息：名称、所有者、成员数量。 | Yuanbao 凭据 |

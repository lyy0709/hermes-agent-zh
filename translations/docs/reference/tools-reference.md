...
| `browser_snapshot` | 获取当前页面可访问性树的基于文本的快照。返回带有引用 ID（如 @e1, @e2）的交互式元素，供 `browser_click` 和 `browser_type` 使用。full=false（默认）：包含交互式元素的紧凑视图。full=true：完整… | — |
...
限定于飞书文档评论智能回复处理器（`gateway/platforms/feishu_comment.py`）。不在 `hermes-cli` 或常规的飞书聊天适配器中暴露。
...
| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `patch` | 在文件中进行有针对性的查找和替换编辑。在终端中使用此工具代替 sed/awk。使用模糊匹配（9 种策略），因此微小的空格/缩进差异不会导致失败。返回统一差异。编辑后自动运行语法检查… | — |
| `read_file` | 读取带有行号和分页的文本文件。在终端中使用此工具代替 cat/head/tail。输出格式：'LINE_NUM\|CONTENT'。如果未找到文件，会建议相似的文件名。对于大文件，使用 offset 和 limit 参数。注意：无法读取图像 o… | — |
| `search_files` | 搜索文件内容或按名称查找文件。在终端中使用此工具代替 grep/rg/find/ls。基于 Ripgrep，比 shell 等效命令更快。内容搜索（target='content'）：在文件内进行正则表达式搜索。输出模式：包含行…的完整匹配… | — |
| `write_file` | 将内容写入文件，完全替换现有内容。在终端中使用此工具代替 echo/cat heredoc。自动创建父目录。会**覆盖**整个文件 —— 进行有针对性的编辑请使用 'patch'。 | — |
...
| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `computer_use` | 通过 cua-driver 进行后台 macOS 桌面控制 —— 截图（SOM / vision / AX）、点击 / 拖拽 / 滚动 / 输入 / 按键 / 等待、list_apps、focus_app。**不会**窃取用户的鼠标光标或键盘焦点。适用于任何支持工具使用的模型。仅限 macOS。 | `cua-driver` 在 `$PATH` 中（通过 `hermes tools` 安装）。 |


:::note
**Honcho 工具**（`honcho_profile`、`honcho_search`、`honcho_context`、`honcho_reasoning`、`honcho_conclude`）不再是内置工具。它们可通过位于 `plugins/memory/honcho/` 的 Honcho 记忆提供商插件获得。有关安装和使用，请参阅[记忆提供商](../user-guide/features/memory-providers.md)。
:::
...
当 Agent 由看板调度器生成（设置了 `HERMES_KANBAN_TASK` 环境变量）或在明确启用了 `kanban` 工具集的配置文件中运行时注册。任务作用域的工作器使用生命周期工具来处理其分配的任务；编排器配置文件还会获得诸如 `kanban_list` 和 `kanban_unblock` 之类的看板路由工具。完整工作流请参阅[看板多 Agent](/user-guide/features/kanban)。
...
| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `memory` | 将重要信息保存到持久化记忆，这些记忆在会话之间保留。你的记忆会在会话开始时出现在系统提示词中——这是你在不同对话之间记住用户和环境信息的方式。何时保存… | — |

## `messaging` 工具集

| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `send_message` | 向已连接的消息平台发送消息，或列出可用目标。重要：当用户要求发送到特定频道或人员（不仅仅是平台名称）时，请先调用 `send_message(action='list')` 查看可用目标… | — |
...
| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `session_search` | 搜索存储在本地会话数据库中的过往会话，或在单个会话内滚动浏览。基于 FTS5 的检索；返回数据库中的实际消息（无需 LLM 调用）。三种模式：发现（传入 `query`）、滚动（传入 `session_id` + `around_message_id`）、浏览（无参数）。 | — |
...
| `skill_view` | 技能允许加载特定任务和工作流的信息，以及脚本和模板。加载技能的完整内容或访问其链接的文件（参考资料、模板、脚本）。首次调用返回 SKILL.md 内容加上… | — |
...
| `terminal` | 在 Linux 环境中执行 shell 命令。文件系统在调用之间保持持久。设置 `background=true` 用于长时间运行的服务器。设置 `notify_on_complete=true`（与 `background=true` 一起）可在进程完成时自动收到通知——无需轮询。不要使用 cat/head/tail——使用 `read_file`。不要使用 grep/rg/find——使用 `search_files`。 | — |
...
单一的 `video_generate` 工具涵盖两种模式——传入 `image_url` 为静态图像制作动画，省略它则仅从文本生成。活动后端会自动路由到正确的端点。该工具的描述在会话开始时重建，以反映活动后端的实际能力（模式、宽高比、分辨率、时长范围、最大参考图像数、音频支持）。后端开发请参阅[视频生成提供商插件](/developer-guide/video-gen-provider-plugin)。

| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `video_generate` | 使用用户配置的视频生成后端，从文本提示词（文生视频）生成视频或为静态图像制作动画（图生视频）。传入 `image_url` 为该图像制作动画；省略它则仅从文本生成。后端会自动路由到正确的端点。在 `video` 字段中返回 HTTP URL 或绝对文件路径。 | 活动的 `video_gen` 插件 + 其凭据（例如 `XAI_API_KEY`, `FAL_KEY`） |
...
| `web_extract` | 从网页 URL 提取内容。以 Markdown 格式返回页面内容。也适用于 PDF URL——直接传入 PDF 链接，它会转换为 Markdown 文本。小于 5000 字符的页面返回完整 Markdown；更大的页面由 LLM 进行摘要。 | EXA_API_KEY 或 PARALLEL_API_KEY 或 FIRECRAWL_API_KEY 或 TAVILY_API_KEY |
## `x_search` 工具集

| 工具 | 描述 | 所需执行环境 |
|------|-------------|----------------------|
| `x_search` | 使用 xAI 内置的 `x_search` Responses 工具搜索 X（Twitter）帖子、个人资料和主题。用于查找 X 上当前的讨论、反应或主张，而非一般网页。默认关闭——需通过 `hermes tools` → 🐦 X (Twitter) Search 选择启用。仅在配置了 xAI 凭据时注册模式（由检查函数控制）。 | XAI_API_KEY **或** xAI Grok OAuth（SuperGrok / Premium+）登录 |
...
| `yb_send_dm` | 向群组中的用户发送私聊/直接消息，可选附带媒体文件。 | Yuanbao 凭据 |
| `yb_search_sticker` | 通过关键词搜索内置的 Yuanbao 表情（TIM 表情）目录。 | Yuanbao 凭据 |
| `yb_send_sticker` | 向当前 Yuanbao 聊天发送内置表情。 | Yuanbao 凭据 |
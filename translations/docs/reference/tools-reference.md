...
| `browser_snapshot` | 获取当前页面无障碍树的基于文本的快照。返回带有引用 ID（如 @e1, @e2）的交互式元素，供 browser_click 和 browser_type 使用。full=false（默认）：包含交互式元素的紧凑视图。full=true：包含所有元素的完整无障碍树。需要先调用 browser_navigate。 | — |
| `browser_type` | 向由其引用 ID 标识的输入字段输入文本。先清除字段，然后输入新文本。需要先调用 browser_navigate 和 browser_snapshot。 | — |
| `browser_vision` | 截取当前页面的屏幕截图并用视觉 AI 进行分析。当需要视觉理解页面内容时使用此工具——对于验证码、视觉验证挑战、复杂布局或当文本快照不足时特别有用。需要先调用 browser_navigate。 | — |

## `clarify` 工具集

| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `clarify` | 当你在继续之前需要澄清、反馈或决策时，向用户提问。支持两种模式：1. **多项选择** — 提供最多 4 个选项。用户选择一个或通过第 5 个“其他”选项输入自己的答案。2. **开放式问题** — 提出一个开放式问题。用户输入自由格式的答案。 | — |

## `code_execution` 工具集

| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `execute_code` | 运行一个可以编程式调用 Hermes 工具的 Python 脚本。当你需要 3 个以上的工具调用且它们之间有处理逻辑时，需要在工具输出进入你的上下文之前过滤/减少大型工具输出时，需要条件分支（if/else）或循环时，或者需要将多个工具结果合并成结构化输出时，使用此工具。 | — |
...
| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `delegate_task` | 生成一个或多个子 Agent 在隔离的上下文中处理任务。每个子 Agent 都有自己的对话、终端会话和工具集。只返回最终摘要——中间的工具结果永远不会进入你的上下文窗口。两种模式：1. **并行委派** — 同时处理多个独立任务。2. **顺序委派** — 将一个任务分解成步骤，每个步骤由不同的子 Agent 处理。 | — |

## `file` 工具集

| 工具 | 描述 | 需要执行环境 |
|------|-------------|----------------------|
| `patch` | 在文件中进行有针对性的查找和替换编辑。在终端中使用此工具代替 sed/awk。使用模糊匹配（9 种策略），因此微小的空格/缩进差异不会导致失败。返回统一的差异。编辑后自动运行语法检查（如果适用）。 | — |
| `read_file` | 读取带有行号和分页的文本文件。在终端中使用此工具代替 cat/head/tail。输出格式：'LINE_NUM\|CONTENT'。如果未找到文件，会建议相似的文件名。对于大文件，使用 offset 和 limit。注意：无法读取图像或其他二进制文件。 | — |
...
| `ha_list_services` | 列出可用的 Home Assistant 服务（操作）以进行设备控制。显示可以对每种设备类型执行哪些操作以及它们接受哪些参数。使用此工具来发现如何控制通过 ha_list_entities 找到的设备。 | — |
:::note
...
| 工具 | 描述 | 需要环境变量 |
|------|-------------|----------------------|
| `memory` | 将重要信息保存到持久化记忆中，这些记忆在会话之间保留。你的记忆会在会话开始时出现在系统提示词中——这是你在不同对话之间记住用户和环境信息的方式。何时保存… | — |

## `messaging` 工具集

| 工具 | 描述 | 需要环境变量 |
|------|-------------|----------------------|
| `send_message` | 向已连接的消息平台发送消息，或列出可用目标。重要：当用户要求发送到特定频道或人员（不仅仅是平台名称）时，首先调用 send_message(action='list') 以查看可用目标… | — |

## `moa` 工具集

| 工具 | 描述 | 需要环境变量 |
|------|-------------|----------------------|
| `mixture_of_agents` | 通过多个前沿 LLM 协作路由解决难题。进行 5 次 API 调用（4 个参考模型 + 1 个聚合器），使用最大推理努力——请谨慎用于真正困难的问题。最适合：复杂数学、高级算法… | OPENROUTER_API_KEY |
...
| `rl_edit_config` | 更新配置字段。首先使用 rl_get_current_config() 查看所选环境的所有可用字段。每个环境都有不同的可配置选项。基础设施设置（tokenizer, URLs, lora_rank, learning_rate 等）是固定的。 | TINKER_API_KEY, WANDB_API_KEY |
...
| 工具 | 描述 | 需要环境变量 |
|------|-------------|----------------------|
| `session_search` | 搜索你过去对话的长期记忆。这是你的回忆——每个过去的会话都是可搜索的，此工具会总结发生了什么。在以下情况下主动使用此工具：- 用户说“我们之前做过这个”、“记得当时”、“上次…” | — |

## `skills` 工具集

| 工具 | 描述 | 需要环境变量 |
|------|-------------|----------------------|
| `skill_manage` | 管理技能（创建、更新、删除）。技能是你的程序性记忆——针对重复任务类型的可重用方法。新技能保存到 ~/.hermes/skills/；现有技能可以在其所在位置修改。操作：create（完整的 SKILL.md 内容），update（修改现有技能），delete（删除技能）。 | — |
...
| 工具 | 描述 | 需要环境变量 |
|------|-------------|----------------------|
| `process` | 管理使用 terminal(background=true) 启动的后台进程。操作：'list'（显示所有），'poll'（检查状态 + 新输出），'log'（带分页的完整输出），'wait'（阻塞直到完成或超时），'kill'（终止），'write'（发送输入）。 | — |
| `terminal` | 在 Linux 环境中执行 shell 命令。文件系统在调用之间持久化。设置 `background=true` 用于长时间运行的服务器。设置 `notify_on_complete=true`（与 `background=true` 一起）以便在进程完成时自动收到通知——无需轮询。不要使用 cat/head/tail——使用 read_file。不要使用 grep/rg/find——使用 search_files。 | — |
## `todo` 工具集

...
| `hermes web` | 启动 Web 仪表板以管理配置、API 密钥和会话。 |
...
| 子命令 | 描述 |
|------------|-------------|
| `subscribe` / `add` | 创建 webhook 路由。返回 URL 和 HMAC 密钥，以便在您的服务上配置。 |
...
| `update` | 当有可用时，重新安装具有上游更改的 hub 技能。 |
...
将 shell 自动补全脚本打印到标准输出。在你的 shell 配置文件中加载输出，即可对 Hermes 命令、子命令和配置文件名称进行 Tab 键补全。

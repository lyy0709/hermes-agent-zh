...
提供商和基础 URL 的更改会自动持久化到 `config.yaml`。当从自定义端点切换走时，旧的基础 URL 会被清除，以防止其泄漏到其他提供商。
...
| 选项 | 描述 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 操作时：作用于**每个配置文件的**消息网关，而不仅仅是活跃的 `HERMES_HOME`。如果你并行运行多个配置文件，并希望在 `hermes update` 后重启所有网关，这很有用。 |

:::tip WSL 用户
使用 `hermes gateway run` 而不是 `hermes gateway start` —— WSL 的 systemd 支持不可靠。可以将其包装在 tmux 中以实现持久化：`tmux new -s hermes 'hermes gateway run'`。详情请参阅 [WSL FAQ](/docs/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。
:::
...
**返回用户（已配置）：** 直接进入完整的重新配置向导 —— 每个提示都显示你当前的设置值作为默认值，按 Enter 键保留或输入新值。没有菜单。
...
| 选项 | 描述 |
|--------|-------------|
| `--quick` | 针对返回用户运行：仅提示缺失或未设置的项。跳过你已经配置好的项。 |
...
生成一个 Slack 应用清单，将 `COMMAND_REGISTRY` 中的每个消息网关命令（`/btw`、`/stop`、`/model` 等）注册为 Slack 原生的斜杠命令，以实现与 Discord 和 Telegram 的对等功能。将输出内容粘贴到你的 Slack 应用配置中，位置在 [https://api.slack.com/apps](https://api.slack.com/apps) → 你的应用 → **Features → App Manifest → Edit**，然后点击 **Save**。如果权限范围或斜杠命令发生更改，Slack 会提示重新安装。
...
| `--slashes-only` | 关闭 | 仅输出 `features.slash_commands` 数组，以便合并到手动维护的清单中。 |

在运行 `hermes update` 后，再次运行 `hermes slack manifest --write` 以获取任何新增的命令。
...
子命令：`add`、`list`、`remove`、`reset`。不带子命令调用时，会启动交互式管理向导。
...
多配置文件、多项目协作看板。每个安装实例可以托管多个看板（每个项目、代码库或领域一个）；每个看板都是一个独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装实例默认有一个名为 `default` 的看板，其数据库为 `~/.hermes/kanban.db` 以保持向后兼容；其他看板位于 `~/.hermes/kanban/boards/<slug>/kanban.db`。消息网关内嵌的调度器在每个周期都会扫描所有看板。
...
| `boards list` / `boards ls` | 列出所有看板及其任务数量。支持 `--json`、`--all`（包含已归档的）。 |
| `boards create <slug>` | 创建新看板。标志：`--name`、`--description`、`--icon`、`--color`、`--switch`（设为活动看板）。Slug 为 kebab-case，会自动转为小写。 |
...
| `archive <id>` | 从默认列表中隐藏任务。`gc` 将移除临时工作空间。 |
| `tail <id>` | 跟随任务的事件流。 |
...
| `--deliver-only` | 跳过 Agent——将渲染后的 `--prompt` 作为字面消息直接交付。零 LLM 成本，亚秒级交付。要求 `--deliver` 是一个真实目标（不能是 `log`）。 |
...
- `*.db-wal`、`*.db-shm`、`*.db-journal` — SQLite 的 WAL / 共享内存 / 日志附属文件。`*.db` 文件已通过 `sqlite3.backup()` 获得一致快照；将实时附属文件与其一起打包，可能会让恢复过程看到未完全提交的状态。
...
当 `--since` 处于活动状态时，包含无法解析时间戳的行（它们可能是多行日志条目的续行）。当 `--level` 处于活动状态时，包含无法检测级别的行。
...
curator 是一个辅助模型的背景任务，定期审查 Agent 创建的技能，清理过时的技能，合并重叠的技能，并归档废弃的技能。捆绑的和通过 hub 安装的技能永远不会被触及。归档的技能可以恢复；永远不会自动删除。
...
在全新安装后，第一次计划运行会推迟一个完整的 `interval_hours`（默认为 7 天）— 消息网关不会在 `hermes update` 后的第一次触发时立即进行整理。在此之前，可以使用 `hermes curator run --dry-run` 进行预览。
...
:::info 提供商特定的子命令
当外部记忆提供商处于活动状态时，它可能会注册自己的顶级 `hermes <provider>` 命令，用于提供商特定的管理（例如，当 Honcho 激活时，`hermes honcho`）。非活动状态的提供商不会暴露其子命令。运行 `hermes --help` 查看当前已连接的命令。
:::
## `hermes acp`
...
将 shell 自动补全脚本打印到标准输出。在您的 shell 配置文件中引入输出内容，即可获得 Hermes 命令、子命令和配置文件名称的 Tab 键自动补全功能。
...
拉取最新的 `hermes-agent` 代码并在您的虚拟环境中重新安装依赖项，然后重新运行安装后钩子（MCP 服务器、技能同步、自动补全安装）。可在已安装的实例上安全运行。

| 选项 | 描述 |
|--------|-------------|
| `--check` | 并排打印当前提交和最新的 `origin/main` 提交，如果同步则退出码为 0，如果落后则退出码为 1。不执行拉取、安装或重启任何操作。 |
| `--backup` | 在拉取之前，为 `HERMES_HOME`（配置、认证、会话、技能、配对数据）创建一个带标签的更新前快照。默认**关闭** —— 之前总是备份的行为在大型主目录上每次更新都会增加数分钟。可以通过 `config.yaml` 中的 `update.backup: true` 永久开启。 |
| `--restart-gateway` | 成功更新后，重启正在运行的消息网关服务。如果安装了多个配置文件，则隐含 `--all` 语义。 |

其他行为：

- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 之前为 `~/.hermes/pairing/` 和飞书评论规则创建一个轻量级快照。如果拉取操作覆盖了您正在编辑的文件，您可以使用 `hermes backup restore --state pre-update` 回滚。
...
- **退出码。** 成功时为 `0`，拉取/安装/安装后错误时为 `1`，工作树意外更改阻止 `git pull` 时为 `2`。
...
1. 按 `a` 添加备用提供商 → 选择一个提供商（基于 OAuth 的提供商会打开浏览器；API 密钥提供商会提示输入密钥），然后选择具体模型。
2. 使用 `↑`/`↓` 重新排序备用提供商（列表中的第一个会首先尝试）。
3. 按 `d` 删除一个。

所有更改都会持久化到 `config.yaml` 中的顶层 `fallback_providers:` 列表。与 [凭据池](/docs/user-guide/features/credential-pools) 交互：池在*同一*提供商内轮换密钥，备用则完全切换到*不同*的提供商。

有关行为详情以及与 `fallback_model`（遗留的单备用键）的交互，请参阅 [备用提供商](/docs/user-guide/features/fallback-providers)。

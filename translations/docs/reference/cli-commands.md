...
交互式提供商和模型选择器。**这是用于添加新提供商、设置 API 密钥和运行 OAuth 流程的命令。** 请从你的终端运行此命令——而不是在活跃的 Hermes 聊天会话内部。
...
**返回用户（已配置）：** 直接进入完整的重新配置向导——每个提示都显示当前值作为其默认值，按 Enter 键保留或输入新值。没有菜单。
...
生成一个 Slack 应用清单，将 `COMMAND_REGISTRY` 中的每个消息网关命令（`/btw`、`/stop`、`/model`、…）注册为一流的 Slack 斜杠命令——与 Discord 和 Telegram 功能保持一致。将输出粘贴到你的 Slack 应用配置中，位置在 [https://api.slack.com/apps](https://api.slack.com/apps) → 你的应用 → **功能 → 应用清单 → 编辑**，然后**保存**。如果范围或斜杠命令发生更改，Slack 会提示重新安装。
...
| `edit` | 更新作业的计划、提示词、名称、交付方式、重复次数或附加技能。支持 `--clear-skills`、`--add-skill` 和 `--remove-skill`。 |
| `pause` | 暂停作业而不删除它。 |
...
| `update` | 在有可用更新时重新安装有上游更改的 Hub 技能。 |
...
- **提供商插件** —— 用于记忆提供商和上下文引擎的单选配置。按 ENTER 键打开一个类别的单选选择器。
...
不带 `--summary` 选项时，此命令将启动交互式的、针对各平台的工具配置界面。
...
将你的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取，并写入到 `~/.hermes`。自动检测遗留目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件（`clawdbot.json`、`moltbot.json`）。

| 选项 | 描述 |
|--------|-------------|
| `--dry-run` | 预览将要迁移的内容，但不实际写入任何数据。 |
| `--preset <name>` | 迁移预设：`full`（所有兼容的设置）或 `user-data`（排除基础设施配置）。两种预设均**不**导入密钥——需显式传递 `--migrate-secrets`。 |
| `--overwrite` | 在冲突时覆盖现有的 Hermes 文件（默认：当计划存在冲突时拒绝应用）。 |
| `--migrate-secrets` | 在迁移中包含 API 密钥。即使在 `--preset full` 下也需要此选项。 |
| `--no-backup` | 跳过迁移前对 `~/.hermes/` 的 zip 快照（默认情况下，在应用迁移前会创建一个可恢复的存档到 `~/.hermes/backups/pre-migration-*.zip`；可使用 `hermes import` 恢复）。 |

...
相同的 Agent，相同的工具，相同的技能——只是去掉了所有交互式/装饰性的层。如果你也需要工具输出在记录中，请改用 `hermes chat -q`；`-z` 明确用于“我只想要最终答案”。
...
| `--no-supervise` | 在 `run` 操作时：在 s6-overlay Docker 镜像内部，选择退出自动监管并使用 pre-s6 前台语义——消息网关作为容器的主进程运行，没有自动重启。在 s6 镜像外部无效。相当于设置 `HERMES_GATEWAY_NO_SUPERVISE=1`。 |
...
管理语言服务器协议集成。LSP 在后台运行真实的语言服务器（pyright、gopls、rust-analyzer 等），并将其诊断信息输入到 `write_file` 和 `patch` 使用的写入后检查中。受 git 工作区检测限制——仅当当前工作目录或编辑的文件位于 git 工作树内时，LSP 才会运行。
...
**最简单路径：** `hermes setup --portal` —— 通过 OAuth 登录 Nous Portal 并一次性选择加入 [工具网关](../user-guide/features/tool-gateway.md)。

**首次运行：** 启动首次使用向导。

**返回用户（已配置）：** 直接进入完整的重新配置向导——每个提示都显示您当前的值作为其默认值，按 Enter 键保留或输入新值。没有菜单。
...
有关网关本身的配置，请参阅 [工具网关](../user-guide/features/tool-gateway.md)。有关一次性设置路径，请参阅上面的 `hermes setup --portal`。
...
如果既没有提供位置参数 `message` 也没有提供 `--file`，当 stdin 不是 TTY 时，`hermes send` 会从 stdin 读取。退出码：成功为 `0`，发送/后端失败为 `1`，使用错误为 `2`。
...
| 子命令 | 描述 |
|------------|-------------|
| `xai` | 扫描 `config.yaml` 中是否有对计划于 2026 年 5 月 15 日停用的 xAI 模型的引用，并（使用 `--apply`）根据 xAI 迁移指南将它们就地重写为官方替代品。默认为试运行。 |
...
运行一个本地的 OpenAI 兼容 HTTP 服务器，将请求转发到经过 OAuth 认证的上游提供商（例如 Nous Portal、xAI）。外部应用程序可以使用任何持有者令牌指向该代理；代理在转发时会附加您真实的 OAuth 凭据。完整指南请参阅：[订阅代理](../user-guide/features/subscription-proxy.md)。
...
| `edit` | 更新作业的调度、提示词、名称、交付方式、重复次数或附加的技能。支持 `--clear-skills`、`--add-skill` 和 `--remove-skill`。 |
| `pause` | 暂停作业而不删除它。 |
...
| `tail <id>` | 跟随任务的事件流。 |
| `dispatch` | 在活动看板上进行一次调度器轮询。标志：`--dry-run`、`--max N`、`--failure-limit N`、`--json`。 |
| `context <id>` | 打印工作线程将看到的完整上下文（标题 + 正文 + 父任务结果 + 评论）。 |
| `specify <id>` / `specify --all` | 通过辅助 LLM 将待办列中的任务具体化为详细规范（包含目标、方法、验收标准的标题和正文），然后将其提升到 `todo` 状态。标志：`--tenant`（将 `--all` 限定到一个租户）、`--author`、`--json`。在 `config.yaml` 的 `auxiliary.triage_specifier` 下配置模型。 |
| `decompose <id>` / `decompose --all` | 将待办列中的任务分解为由描述路由到专家配置文件的子任务图（编排器驱动的路径）。当 LLM 认为任务无法从分解中受益时，回退到 specify 风格的单任务提升。标志与 `specify` 相同。在 `config.yaml` 的 `auxiliary.kanban_decomposer` 下配置模型。当 `kanban.auto_decompose: true`（默认值）时，也会在每个调度器周期自动运行。请参阅[自动与手动编排](/user-guide/features/kanban#auto-vs-manual-orchestration)。 |
| `gc` | 移除已归档任务的临时工作空间。 |
示例：
...
| 选项 | 描述 |
|--------|-------------|
| `--prompt` | 提示词模板，支持 `{dot.notation}` 格式的负载引用。 |
...
| `--deliver-only` | 跳过 Agent——直接将渲染后的 `--prompt` 作为字面消息投递。零 LLM 成本，亚秒级投递。要求 `--deliver` 是一个真实目标（非 `log`）。 |
...
报告包括系统信息（操作系统、Python 版本、Hermes 版本）、最近的 Agent 和消息网关日志（每个文件限制 512 KB）以及脱敏的 API 密钥状态。密钥始终被脱敏——不会上传任何密钥。
...
- `*.db-wal`、`*.db-shm`、`*.db-journal` — SQLite 的 WAL / 共享内存 / 日志附属文件。`*.db` 文件已通过 `sqlite3.backup()` 获得一致快照；附带传输活动的附属文件会让恢复过程看到未完全提交的状态。
- `checkpoints/` — 每个会话的轨迹缓存。按哈希键存储，每个会话重新生成；无论如何也无法干净地移植到另一个安装。
...
| 子命令 | 描述 |
|------------|-------------|
| `status` (默认) | 显示总大小、项目数量和每个项目的细分。单独的 `hermes checkpoints` 命令等效于此。 |
| `list` | `status` 的别名。 |
...
当 `--since` 激活时，无法解析时间戳的行也会被包含（它们可能是多行日志条目的续行）。当 `--level` 激活时，无法检测级别的行也会被包含。
...
- 传递一个 `http(s)://…/*.md` URL 会直接安装一个单文件 SKILL.md。当 frontmatter 没有 `name:` 且 URL 段不是有效标识符时，交互式终端会提示输入名称；非交互式界面（TUI 内的 `/skills install`、消息网关平台）需要改用 `--name <x>`。
...
技能捆绑包将多个技能分组在一个 `/<捆绑包名称>` 斜杠命令下。调用捆绑包会将每个引用的技能加载到单个组合用户消息中。存储位置：`~/.hermes/skill-bundles/<slug>.yaml`。有关 YAML 模式和行为，请参阅[技能捆绑包](../user-guide/features/skills.md#skill-bundles)。

子命令：

| 子命令 | 描述 |
|------------|-------------|
| `list` | 列出已安装的捆绑包（未给出子命令时的默认操作） |
| `show <名称>` | 显示一个捆绑包的名称、描述、技能和文件路径 |
| `create <名称>` | 创建一个新的捆绑包。传递 `--skill <id>`（可重复）或省略以进行交互式输入。支持 `--description`、`--instruction`、`--force`。 |
| `delete <名称>` | 删除捆绑包文件 |
...
在聊天会话中，`/bundles` 列出已安装的捆绑包，`/<捆绑包名称>` 加载一个捆绑包。
...
| `backup` | 手动对 `~/.hermes/skills/` 创建 tar.gz 快照（策展人也会在每次实际运行前自动创建快照） |
| `rollback` | 从快照恢复 `~/.hermes/skills/`（默认为最新的） |
| `rollback --list` | 列出可用的快照 |
| `rollback --id <时间戳>` | 按 ID 恢复特定的快照 |
| `rollback -y` | 跳过确认提示 |
| `pause` | 暂停策展人直到恢复 |
| `resume` | 恢复已暂停的策展人 |
...
在全新安装后，首次定时执行会推迟一个完整的 `interval_hours`（默认为 7 天）—— 消息网关不会在 `hermes update` 后的第一次触发时立即进行内容精选。在此之前，可以使用 `hermes curator run --dry-run` 来预览将要发生的情况。

有关行为和配置，请参阅 [内容精选](../user-guide/features/curator.md)。
...
| `add` | 选择一个提供商 + 模型（与 `hermes model` 使用相同的选择器）并追加到链中 |
| `remove` (别名: `rm`) | 从链中选择一个条目进行删除 |
| `clear` | 移除所有备用条目 |
...
检查在 `~/.hermes/config.yaml` 中声明的 shell 脚本钩子，使用合成负载对其进行测试，并管理位于 `~/.hermes/shell-hooks-allowlist.json` 的首次使用同意允许列表。

| 子命令 | 描述 |
|------------|-------------|
| `list` (别名: `ls`) | 列出已配置的钩子及其匹配器、超时和同意状态 |
| `test <事件>` | 使用合成负载触发匹配 `<事件>` 的每个钩子 |
| `revoke` (别名: `remove`, `rm`) | 移除命令的允许列表条目（在下次重启后生效） |
| `doctor` | 检查每个已配置的钩子：执行权限、允许列表、修改时间偏移、JSON 有效性以及合成运行计时 |
...
:::info 提供商特定子命令
当外部记忆提供商处于活动状态时，它可能会注册自己的顶级 `hermes <提供商>` 命令，用于提供商特定的管理（例如，当 Honcho 激活时使用 `hermes honcho`）。非活动状态的提供商不会暴露其子命令。运行 `hermes --help` 查看当前已连接的命令。
:::
...
请参阅 [MCP 配置参考](./mcp-config-reference.md)、[与 Hermes 一起使用 MCP](../guides/use-mcp-with-hermes.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server)。
...
不带 `--summary` 选项时，此命令将启动交互式的按平台工具配置界面。
## `hermes computer-use`
...
| `install --upgrade` | 即使 cua-driver 已在 PATH 中，也重新运行安装程序。上游脚本总是拉取最新版本，因此这会执行原地升级。 |
| `status` | 打印 `cua-driver` 是否在 `$PATH` 中以及安装的版本。 |

`hermes computer-use install` 是用于安装 `computer_use` 工具集使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口点。它运行与 `hermes tools` 在你首次启用计算机使用时调用的相同上游安装程序，因此如果工具集切换未触发安装（例如，在返回用户设置中），可以安全地使用它来重新运行安装。

`hermes update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你希望立即获取的修复，而不想等待下一次 Hermes 更新时，请使用它。
...
将你的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hermes`。自动检测旧目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件（`clawdbot.json`、`moltbot.json`）。

| 选项 | 描述 |
|--------|-------------|
| `--dry-run` | 预览将要迁移的内容，不写入任何内容。 |
...
| `--no-backup` | 跳过迁移前对 `~/.hermes/` 的 zip 快照（默认情况下，在应用前会向 `~/.hermes/backups/pre-migration-*.zip` 写入一个可恢复的存档点；可通过 `hermes import` 恢复）。 |
...
| `--host` | `127.0.0.1` | 绑定地址 |
| `--no-open` | — | 不自动打开浏览器 |
| `--tui` | 关闭 | 通过在 PTY/WebSocket 桥接后运行 `hermes --tui` 来启用浏览器内聊天标签页。需要 `pip install 'hermes-agent[web,pty]'` 和 POSIX PTY 环境，例如 Linux、macOS 或 WSL2。 |
| `--insecure` | 关闭 | 允许绑定到非 localhost 主机。在网络中暴露仪表板凭据；仅在受信任的网络控制下使用。 |
...
| `export <名称> [-o 文件]` | 将配置文件导出到 `.tar.gz` 存档（本地备份）。 |
| `import <存档> [--name 名称]` | 从 `.tar.gz` 存档导入配置文件（本地恢复）。 |
| `install <源> [--name 名称] [--alias] [--force] [-y]` | 从 git URL 或本地目录安装配置文件发行版。 |
| `update <名称> [--force-config] [-y]` | 重新拉取发行版；保留用户数据（记忆、会话、认证信息）。 |
| `info <名称>` | 显示配置文件的发行版清单（版本、要求、源）。 |
...
将 shell 自动补全脚本打印到标准输出。在您的 shell 配置文件中加载输出，即可获得 Hermes 命令、子命令和配置文件名称的 Tab 键自动补全功能。
...
拉取最新的 `hermes-agent` 代码并在您的虚拟环境中重新安装依赖项，然后重新运行安装后钩子（MCP 服务器、技能同步、自动补全安装）。可以在运行中的安装上安全执行。
...
| 选项 | 描述 |
|--------|-------------|
| `--check` | 并排打印当前提交和最新的 `origin/main` 提交，如果同步则退出码为 0，如果落后则退出码为 1。不执行拉取、安装或重启任何操作。 |
| `--backup` | 在拉取之前，创建 `HERMES_HOME`（配置、认证信息、会话、技能、配对数据）的带标签的更新前快照。默认**关闭** — 之前总是备份的行为会给大型主目录的每次更新增加几分钟时间。可以通过在 `config.yaml` 中设置 `update.backup: true` 来永久开启。 |
| `--restart-gateway` | 成功更新后，重启正在运行的消息网关服务。如果安装了多个配置文件，则意味着 `--all` 语义。 |

其他行为：

- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 之前对 `~/.hermes/pairing/` 和飞书评论规则进行轻量级快照。如果拉取操作覆盖了您正在编辑的文件，您可以使用 `hermes backup restore --state pre-update` 回滚。
- **遗留 `hermes.service` 警告。** 如果 Hermes 检测到重命名前的 `hermes.service` systemd 单元（而不是当前的 `hermes-gateway.service`），它会打印一次迁移提示，以便您避免 flap-loop 问题。

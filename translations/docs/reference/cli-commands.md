...
每次运行的覆盖设置（不修改 `~/.hermes/config.yaml`）：
...
生成一个 Slack 应用清单，将 `COMMAND_REGISTRY` 中的每个消息网关命令（`/btw`、`/stop`、`/model`……）注册为 Slack 原生的斜杠命令——以实现与 Discord 和 Telegram 的对等功能。将输出内容粘贴到你的 Slack 应用配置中，位置在 [https://api.slack.com/apps](https://api.slack.com/apps) → 你的应用 → **Features → App Manifest → Edit**，然后 **Save**。如果权限范围或斜杠命令发生更改，Slack 会提示重新安装。
...
多配置文件协作看板。任务存储在 `~/.hermes/kanban.db`（WAL 模式 SQLite）中；每个配置文件读取和写入同一个看板。一个由 `cron` 驱动的调度器（`hermes kanban dispatch`）以原子方式认领就绪的任务，并以独立的工作空间作为独立进程启动分配到的配置文件。
...
| `tail <id>` | 跟随任务的事件流。 |
| `dispatch` | 执行一次调度器轮询。标志：`--dry-run`、`--max N`、`--json`。 |
| `context <id>` | 打印工作者将看到的完整上下文（标题 + 正文 + 父结果 + 评论）。 |
| `gc` | 为已归档的任务移除临时工作空间。 |

所有操作也可作为消息网关中的斜杠命令使用（`/kanban …`），具有相同的参数界面。
...
| `--events` | 逗号分隔的事件类型列表（例如 `issues,pull_request`）。为空表示接受所有事件。 |
| `--description` | 人类可读的描述。 |
| `--skills` | 逗号分隔的技能名称列表，用于加载到 Agent 运行中。 |
| `--deliver` | 交付目标：`log`（默认）、`telegram`、`discord`、`slack`、`github_comment`。 |
| `--deliver-chat-id` | 跨平台交付的目标聊天/频道 ID。 |
| `--secret` | 自定义 HMAC 密钥。如果省略则自动生成。 |

订阅信息会持久化到 `~/.hermes/webhook_subscriptions.json`，并且会被 webhook 适配器热重载，无需重启消息网关。
...
创建 Hermes 配置、技能、会话和数据的 zip 归档文件。备份会排除 hermes-agent 代码库本身。
...
- `*.db-wal`、`*.db-shm`、`*.db-journal` —— SQLite 的 WAL / 共享内存 / 日志附属文件。`*.db` 文件已经通过 `sqlite3.backup()` 获得了一个一致的快照；将活动的附属文件与它一起打包，可能会让恢复操作看到未完全提交的状态。
...
hermes skills install https://example.com/SKILL.md --name my-skill        # 当 frontmatter 中没有 name 时，覆盖名称
hermes skills check
...
在全新安装后，第一次定时执行会推迟一个完整的 `interval_hours`（默认为 7 天）—— 消息网关不会在 `hermes update` 后的第一次触发时立即进行策展。在此之前，可以使用 `hermes curator run --dry-run` 来预览。
...
| `revoke` (别名: `remove`, `rm`) | 移除某个命令的允许列表条目（在下次重启时生效） |
...
:::info 提供商特定的子命令
当外部记忆提供商处于活动状态时，它可能会注册自己的顶级 `hermes <provider>` 命令，用于提供商特定的管理（例如，当 Honcho 激活时使用 `hermes honcho`）。非活动状态的提供商不会暴露其子命令。运行 `hermes --help` 以查看当前已连接的命令。
:::
...
- **提供商插件** —— 用于记忆提供商和上下文引擎的单选配置。在某个类别上按 ENTER 键以打开单选选择器。
...
请参阅 [Plugins](../user-guide/features/plugins.md) 和 [Build a Hermes Plugin](../guides/build-a-hermes-plugin.md)。
...
| `--no-backup` | 跳过迁移前对 `~/.hermes/` 的 zip 快照（默认在应用前会创建一个恢复点存档到 `~/.hermes/backups/pre-migration-*.zip`；可通过 `hermes import` 恢复）。 |
...
将 shell 自动补全脚本打印到标准输出。在你的 shell 配置文件中加载输出，即可获得 Hermes 命令、子命令和配置文件名称的 Tab 键补全功能。
...
拉取最新的 `hermes-agent` 代码并在你的虚拟环境中重新安装依赖项，然后重新运行安装后钩子（MCP 服务器、技能同步、补全安装）。可在运行中的安装上安全执行。
| 选项 | 说明 |
|--------|-------------|
| `--check` | 并排打印当前提交和最新的 `origin/main` 提交，如果同步则退出 0，如果落后则退出 1。不执行拉取、安装或重启任何操作。 |
| `--backup` | 在拉取前创建 `HERMES_HOME`（配置、认证、会话、技能、配对数据）的带标签的更新前快照。默认**关闭**——之前总是备份的行为在大型主目录上会给每次更新增加数分钟。可通过 `config.yaml` 中的 `update.backup: true` 永久开启。 |
| `--restart-gateway` | 成功更新后，重启正在运行的消息网关服务。如果安装了多个配置文件，则隐含 `--all` 语义。 |

其他行为：

- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 之前对 `~/.hermes/pairing/` 和飞书评论规则进行轻量级快照。如果拉取操作重写了您正在编辑的文件，您可以使用 `hermes backup restore --state pre-update` 回滚它。
- **遗留 `hermes.service` 警告。** 如果 Hermes 检测到重命名前的 `hermes.service` systemd 单元（而非当前的 `hermes-gateway.service`），它会打印一次性迁移提示，以便您避免 flap-loop 问题。
...
管理后备提供商链（当您的主提供商达到速率限制或返回致命错误时使用），无需手动编辑 `config.yaml`。复用 `hermes model` 中的提供商选择器——相同的提供商列表、相同的凭据提示、相同的验证。

典型会话：

1. 按 `a` 添加后备 → 选择一个提供商（基于 OAuth 的提供商将打开浏览器；API 密钥提供商将提示输入密钥），然后选择特定模型。
2. 使用 `↑`/`↓` 重新排序后备（列表中的第一个会首先尝试）。
3. 按 `d` 删除一个。

所有更改都会持久化到 `config.yaml` 中 `model:` 下的 `fallback_providers:`。与[凭据池](/docs/user-guide/features/credential-pools)交互：池在提供商*内部*轮换密钥，而后备则完全切换到*不同*的提供商。

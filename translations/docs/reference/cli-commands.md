...
| `--ignore-rules` | 跳过 `AGENTS.md`、`SOUL.md`、`.cursorrules`、持久记忆和预加载技能的自动注入。与 `--ignore-user-config` 结合使用以实现完全隔离的运行。 |
...
对于程序化调用者（shell 脚本、CI、定时任务、父进程通过管道输入提示词），`hermes -z` 是最纯粹的单次调用入口点：**单条提示词输入，最终响应文本输出，stdout 或 stderr 上不输出任何其他内容。** 没有横幅、没有加载动画、没有工具预览、没有 `Session:` 行——只有 Agent 的最终回复作为纯文本。
...
相同的 Agent，相同的工具，相同的技能——只是去掉了所有交互式/装饰性的层。如果你也需要在记录中包含工具输出，请改用 `hermes chat -q`；`-z` 明确用于“我只想要最终答案”的场景。
...
| `restart` | 重启服务。 |
| `status` | 显示服务状态。 |
| `list` | 列出**所有配置文件**以及每个配置文件的消息网关当前是否正在运行（在可用时显示 PID）。当你并行运行多个配置文件并希望获得单一概览时很方便。 |
...
| 选项 | 描述 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：作用于**每个配置文件**的消息网关，而不仅仅是活跃的 `HERMES_HOME`。如果你并行运行多个配置文件，并希望在 `hermes update` 后重启所有网关时很有用。 |

:::tip WSL 用户
使用 `hermes gateway run` 而不是 `hermes gateway start`——WSL 的 systemd 支持不可靠。将其包装在 tmux 中以实现持久化：`tmux new -s hermes 'hermes gateway run'`。详情请参阅 [WSL 常见问题](/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。
:::
...
管理语言服务器协议集成。LSP 在后台运行真实的语言服务器（pyright、gopls、rust-analyzer 等），并将其诊断信息提供给 `write_file` 和 `patch` 使用的写入后检查。受 git 工作区检测限制——仅当当前工作目录或编辑的文件位于 git 工作树内时，LSP 才会运行。
...
**已配置用户（已配置）：** 直接进入完整重新配置向导——每个提示都会显示当前值作为默认值，按 Enter 键保留或输入新值。没有菜单。
...
| 选项 | 描述 |
|--------|-------------|
| `--quick` | 针对已配置用户运行：仅提示缺失或未设置的项。跳过已配置的项。 |
| `--non-interactive` | 使用默认值/环境变量，无需提示。 |
| `--reset` | 在设置前将配置重置为默认值。 |
| `--reconfigure` | 向后兼容的别名——在现有安装上运行 `hermes setup` 现在默认执行此操作。 |
| `--portal` | 一次性 Nous Portal 设置：通过 OAuth 登录，将 Nous 设置为推理提供商，并选择加入 [Tool Gateway](../user-guide/features/tool-gateway.md)。跳过向导的其余部分。 |
...
检查 Nous Portal 认证状态、Tool Gateway 路由，并访问订阅页面。不带子命令的调用将运行 `status`。

| 子命令 | 描述 |
|------------|-------------|
| `status` (默认) | Portal 认证状态 + 每个工具的 Tool Gateway 路由摘要。未提供子命令时也会显示。 |
| `open` | 在默认浏览器中打开 `portal.nousresearch.com/manage-subscription`。 |
| `tools` | 列出每个 Tool Gateway 合作伙伴（Firecrawl、FAL、OpenAI TTS、Browser Use、Modal）以及哪些通过 Nous 路由。 |

关于消息网关本身的配置，请参阅 [Tool Gateway](../user-guide/features/tool-gateway.md)。关于一次性设置路径，请参阅上面的 `hermes setup --portal`。
...
生成一个 Slack 应用清单，将 `COMMAND_REGISTRY` 中的每个消息网关命令（`/btw`、`/stop`、`/model`、…）注册为 Slack 原生斜杠命令——与 Discord 和 Telegram 功能对等。将输出粘贴到你的 Slack 应用配置中，位置在 [https://api.slack.com/apps](https://api.slack.com/apps) → 你的应用 → **Features → App Manifest → Edit**，然后 **Save**。如果权限范围或斜杠命令发生更改，Slack 会提示重新安装。

| 标志 | 默认值 | 用途 |
|------|---------|---------|
| `--write [PATH]` | stdout | 写入文件而不是 stdout。单独的 `--write` 写入 `$HERMES_HOME/slack-manifest.json`。 |
| `--name NAME` | `Hermes` | Slack 中显示的机器人名称。 |
...
|------------|-------------|
| `list` | 显示已调度的任务。 |
| `create` / `add` | 根据提示创建定时任务，可选地通过重复 `--skill` 附加一个或多个技能。 |
| `edit` | 更新任务的调度、提示、名称、交付方式、重复次数或附加的技能。支持 `--clear-skills`、`--add-skill` 和 `--remove-skill`。 |
| `pause` | 暂停任务而不删除它。 |
...
多配置文件、多项目协作看板。每个安装实例可以托管多个看板（每个项目、代码库或领域一个）；每个看板都是一个独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装实例开始时有一个名为 `default` 的看板，其数据库为 `~/.hermes/kanban.db`（用于向后兼容）；其他看板位于 `~/.hermes/kanban/boards/<slug>/kanban.db`。消息网关内嵌的调度器会在每个调度周期扫描所有看板。

**全局标志（适用于以下所有操作）：**

| 标志 | 用途 |
|------|---------|
| `--board <slug>` | 对指定看板进行操作。默认为当前看板（通过 `hermes kanban boards switch`、`HERMES_KANBAN_BOARD` 环境变量或 `default` 设置）。 |

**这是面向人工/脚本操作的界面。** 由调度器启动的 Agent 工作进程通过专用的 `kanban_*` [工具集](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器配置文件还拥有 `kanban_list` 和 `kanban_unblock`）来驱动看板，而不是通过 shell 调用 `hermes kanban`。工作进程的环境变量中固定设置了 `HERMES_KANBAN_BOARD`，因此它们物理上无法看到其他看板。

| 操作 | 用途 |
|--------|---------|
| `init` | 如果缺少 `kanban.db` 则创建。幂等操作。 |
...
| `boards rm <slug>` | 归档（默认）或硬删除一个看板。`--delete` 跳过归档步骤。已归档的看板移动到 `boards/_archived/<slug>-<ts>/`。对 `default` 看板拒绝此操作。 |
...
| `comment <id> "<text>"` | 追加评论。下一个认领该任务的工作进程会将其作为 `kanban_show()` 响应的一部分读取。 |
...
| `tail <id>` | 跟随任务的事件流。 |
| `dispatch` | 在活动看板上执行一次调度器扫描。标志：`--dry-run`、`--max N`、`--failure-limit N`、`--json`。 |
| `context <id>` | 打印工作进程将看到的完整上下文（标题 + 正文 + 父任务结果 + 评论）。 |
| `specify <id>` / `specify --all` | 通过辅助 LLM 将待办列中的任务具体化为详细规格（标题 + 包含目标、方法、验收标准的正文），然后将其提升到 `todo` 状态。标志：`--tenant`（将 `--all` 限制在一个租户内）、`--author`、`--json`。在 `config.yaml` 的 `auxiliary.triage_specifier` 下配置模型。 |
| `decompose <id>` / `decompose --all` | 将待办列中的任务分解为由描述路由到专家配置文件的子任务图（编排器驱动的路径）。当 LLM 认为任务不适合分解时，回退到 specify 风格的单任务提升。标志与 `specify` 相同。在 `config.yaml` 的 `auxiliary.kanban_decomposer` 下配置模型。当 `kanban.auto_decompose: true`（默认）时，也会在每个调度器周期自动运行。参见[自动与手动编排](/user-guide/features/kanban#auto-vs-manual-orchestration)。 |
| `gc` | 移除已归档任务的临时工作空间。 |

示例：

```bash
# 创建第二个看板并在其上放置任务，无需切换当前活动看板。
...
所有操作也可作为消息网关中的斜杠命令使用（`/kanban …`），具有相同的参数界面——包括 `boards` 子命令和 `--board` 标志。

关于完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的比较、八种协作模式、四个用户故事、并发正确性证明——请参阅代码库中的 `docs/hermes-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/user-guide/features/kanban)。
## `hermes webhook`
...
管理用于事件驱动 Agent 激活的动态 Webhook 订阅。需要在配置中启用 webhook 平台 —— 如果未配置，则会打印设置说明。
...
| `--deliver-only` | 跳过 Agent —— 将渲染后的 `--prompt` 作为字面消息投递。零 LLM 成本，亚秒级投递。要求 `--deliver` 是一个真实目标（非 `log`）。 |
...
创建你的 Hermes 配置、技能、会话和数据的 zip 归档。备份排除 hermes-agent 代码库本身。
...
- `*.db-wal`, `*.db-shm`, `*.db-journal` —— SQLite 的 WAL / 共享内存 / 日志附属文件。`*.db` 文件已通过 `sqlite3.backup()` 获得一致快照；将活动的附属文件与它一起打包会让恢复时看到未完全提交的状态。
- `checkpoints/` —— 每个会话的轨迹缓存。按哈希键控且每个会话重新生成；无论如何也无法干净地移植到另一个安装。
...
检查和管理位于 `~/.hermes/checkpoints/` 的影子 git 存储库——这是会话内 `/rollback` 命令背后的存储层。可随时安全运行；无需 Agent 正在运行。

| 子命令 | 描述 |
|------------|-------------|
| `status` (默认) | 显示总大小、项目数量及每个项目的细分情况。单独的 `hermes checkpoints` 命令效果相同。 |
| `list` | `status` 的别名。 |
| `prune` | 强制执行清理扫描——删除孤立和过期的项目，对存储库进行垃圾回收，强制执行大小上限。忽略 24 小时幂等性标记。 |
| `clear` | 删除整个检查点基础目录。不可逆；除非使用 `-f`，否则会要求确认。 |
| `clear-legacy` | 仅删除由 v1→v2 迁移产生的 `legacy-<timestamp>/` 归档文件。 |
...
| `--retention-days N` | `prune` | 删除 `last_touch` 时间早于 N 天的项目（默认 7）。 |
| `--max-size-mb N` | `prune` | 在孤立/过期项目清理后，按项目删除最旧的提交，直到总存储大小 ≤ N MB（默认 500）。 |
...
hermes checkpoints clear -f                         # 清空所有内容
```

完整架构和会话内命令请参阅 [检查点和 `/rollback`](../user-guide/checkpoints-and-rollback.md)。
...
将先前创建的 Hermes 备份恢复到您的 Hermes 主目录。归档中的所有文件将覆盖 Hermes 主目录中的现有文件；`--force` 仅用于跳过当目标目录已存在 Hermes 安装时触发的确认提示。
...
| `update` | 在有可用更新时，重新安装有上游变更的 hub 技能。 |
...
hermes skills install https://example.com/SKILL.md --name my-skill        # 当 frontmatter 中没有名称时，覆盖名称
hermes skills check
...
策展人是一个辅助模型的后台任务，定期审查 Agent 创建的技能，清理过时的技能，合并重叠的技能，并归档废弃的技能。捆绑的和通过 hub 安装的技能永远不会被触及。归档是可恢复的；永远不会自动删除。
...
在全新安装后，第一次计划运行会推迟一个完整的 `interval_hours`（默认为 7 天）— 消息网关不会在 `hermes update` 后的第一次触发时立即进行策展。在此之前，使用 `hermes curator run --dry-run` 进行预览。
...
| `add` | 选择一个提供商 + 模型（使用与 `hermes model` 相同的选择器）并追加到链中 |
| `remove` (别名: `rm`) | 从链中选择一个条目进行删除 |
| `clear` | 移除所有备用条目 |

参见 [备用提供商](../user-guide/features/fallback-providers.md)。
...
| `revoke` (别名: `remove`, `rm`) | 移除命令的允许列表条目（下次重启时生效） |
| `doctor` | 检查每个已配置的钩子：执行权限、允许列表、修改时间偏差、JSON 有效性以及合成运行计时 |

有关事件签名和负载结构，请参见 [钩子](../user-guide/features/hooks.md)。
...
:::info 提供商特定子命令
当外部记忆提供商处于活动状态时，它可能会注册自己的顶级 `hermes <提供商>` 命令，用于提供商特定的管理（例如，当 Honcho 激活时使用 `hermes honcho`）。非活动提供商不会暴露其子命令。运行 `hermes --help` 查看当前已连接的命令。
:::
...
| `add <名称> [--url URL] [--command CMD] [--args ...] [--auth oauth\|header]` | 添加自定义 MCP 服务器并自动发现工具。 |
...
参见 [MCP 配置参考](./mcp-config-reference.md)、[与 Hermes 一起使用 MCP](../guides/use-mcp-with-hermes.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server)。
...
统一的插件管理 — 将通用插件、记忆提供商和上下文引擎集中在一处。不带子命令运行 `hermes plugins` 将打开一个复合交互式界面，包含两个部分：
...
不带 `--summary` 时，此命令将启动交互式的按平台工具配置 UI。
...
| `install --upgrade` | 即使 cua-driver 已在 PATH 上，也重新运行安装程序。上游脚本总是拉取最新版本，因此这将执行原地升级。 |
| `status` | 打印 `cua-driver` 是否在 `$PATH` 上以及安装了哪个版本。 |
`hermes computer-use install` 是安装 `computer_use` 工具集使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口点。它运行与首次启用 Computer Use 时 `hermes tools` 调用的相同上游安装程序，因此如果工具集开关未触发安装（例如，在返回用户设置中），可以安全地使用它来重新运行安装。

`hermes update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了您希望立即获取的修复程序，而不想等待下一次 Hermes 更新时，请使用它。
...
将您的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hermes`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件（`clawdbot.json`、`moltbot.json`）。
...
**直接导入：** SOUL.md、MEMORY.md、USER.md、AGENTS.md、技能（4 个源目录）、默认模型、自定义提供商、MCP 服务器、消息平台令牌和允许列表（Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Mattermost）、Agent 默认值（推理强度、压缩、人工延迟、时区、沙盒）、会话重置策略、审批规则、TTS 配置、浏览器设置、工具设置、执行超时、命令允许列表、消息网关配置，以及来自 3 个来源的 API 密钥。
...
| `--host` | `127.0.0.1` | 绑定地址 |
| `--no-open` | — | 不自动打开浏览器 |
| `--tui` | 关闭 | 通过在 PTY/WebSocket 桥后运行 `hermes --tui` 来启用浏览器内的聊天标签页。需要 `pip install 'hermes-agent[web,pty]'` 和 POSIX PTY 环境，例如 Linux、macOS 或 WSL2。 |
...
| `create <name> [--clone] [--clone-all] [--clone-from <source>] [--no-alias]` | 创建一个新的配置文件。`--clone` 从当前活动配置文件复制配置、`.env` 和 `SOUL.md`。`--clone-all` 复制所有状态。`--clone-from` 指定一个源配置文件。 |
...
| `install <source> [--name N] [--alias] [--force] [-y]` | 从 git URL 或本地目录安装一个配置文件发行版。 |
...
将 shell 自动补全脚本打印到标准输出。在你的 shell 配置文件中加载此输出，即可获得 Hermes 命令、子命令和配置文件名称的 Tab 键自动补全功能。
...
拉取最新的 `hermes-agent` 代码并在你的虚拟环境中重新安装依赖项，然后重新运行安装后钩子（MCP 服务器、技能同步、自动补全安装）。可以在已安装的实例上安全运行。
...
| `--backup` | 在拉取之前，为 `HERMES_HOME`（配置、认证信息、会话、技能、配对数据）创建一个带标签的更新前快照。默认是**关闭**的 — 之前总是备份的行为在大型主目录上每次更新都会增加几分钟时间。可以通过在 `config.yaml` 中设置 `update.backup: true` 来永久开启此功能。 |
...
- **遗留 `hermes.service` 警告。** 如果 Hermes 检测到重命名前的 `hermes.service` systemd 单元（而不是当前的 `hermes-gateway.service`），它会打印一次性迁移提示，以便你可以避免 flap-loop 问题。

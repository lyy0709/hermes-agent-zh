...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择此次调用使用的 Hermes 配置文件。覆盖由 `hermes profile use` 设置的粘性默认值。 |
...
对于程序化调用者（shell 脚本、CI、定时任务、父进程通过管道输入提示词），`hermes -z` 是最纯粹的单次调用入口点：**单条提示词输入，最终响应文本输出，标准输出和标准错误上无其他内容。** 没有横幅，没有加载动画，没有工具预览，没有 `Session:` 行——只有 Agent 的最终回复，纯文本。
...
相同的 Agent，相同的工具，相同的技能——只是去掉了所有交互层/装饰层。如果你也需要在记录中包含工具输出，请改用 `hermes chat -q`；`-z` 明确用于“我只想要最终答案”的场景。
...
**如果你需要添加新提供商：** 先退出你的 Hermes 会话（`Ctrl+C` 或 `/quit`），然后从你的终端提示符运行 `hermes model`。
:::
...
:::tip WSL 用户
使用 `hermes gateway run` 而不是 `hermes gateway start`——WSL 的 systemd 支持不可靠。将其包装在 tmux 中以保持持久性：`tmux new -s hermes 'hermes gateway run'`。详情请参阅 [WSL FAQ](/docs/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。
:::
...
管理语言服务器协议集成。LSP 在后台运行真实的语言服务器（pyright、gopls、rust-analyzer 等），并将其诊断信息提供给 `write_file` 和 `patch` 使用的写入后检查。此功能受 Git 工作区检测限制——仅当当前工作目录或编辑的文件位于 Git 工作树内时，LSP 才会运行。
...
管理同提供商密钥轮换的凭据池。完整文档请参阅[凭据池](/docs/user-guide/features/credential-pools)。
...
|------------|-------------|
| `list` | 显示计划任务。 |
| `create` / `add` | 根据提示创建计划任务，可选地通过重复的 `--skill` 附加一个或多个技能。 |
...
多配置文件、多项目协作看板。每个安装可以托管多个看板（每个项目、仓库或领域一个）；每个看板都是一个独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装开始时有一个名为 `default` 的看板，其数据库为 `~/.hermes/kanban.db`（用于向后兼容）；其他看板位于 `~/.hermes/kanban/boards/<slug>/kanban.db`。消息网关内嵌的调度器在每个周期扫描每个看板。
...
**这是人工/脚本操作界面。** 由调度器生成的 Agent 工作线程通过专用的 `kanban_*` [工具集](/docs/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器配置文件还拥有 `kanban_list` 和 `kanban_unblock`）来驱动看板，而不是通过 shell 调用 `hermes kanban`。工作线程的环境变量中固定设置了 `HERMES_KANBAN_BOARD`，因此它们物理上无法看到其他看板。
| 操作 | 目的 |
|--------|---------|
| `init` | 如果缺少 `kanban.db` 则创建。幂等操作。 |
| `boards list` / `boards ls` | 列出所有看板及其任务数量。`--json`，`--all`（包含已归档的）。 |
| `boards create <slug>` | 创建一个新看板。标志：`--name`，`--description`，`--icon`，`--color`，`--switch`（设为活动看板）。Slug 为 kebab-case，自动转为小写。 |
...
| `comment <id> "<text>"` | 追加一条评论。下一个认领该任务的 worker 会将其作为 `kanban_show()` 响应的一部分读取。 |
| `complete <id>` | 标记任务完成。标志：`--result`，`--summary`，`--metadata`。 |
| `block <id> "<reason>"` | 标记任务因需要人工输入而阻塞。同时将原因作为评论追加。 |
| `schedule <id> "<reason>"` | 将延迟/后续工作放入 `scheduled` 状态，使其不显示为人工阻塞项。 |
| `unblock <id>` | 将阻塞或计划中的任务返回到就绪状态（如果依赖项仍未完成，则返回到 `todo` 状态）。 |
| `archive <id>` | 从默认列表中隐藏。`gc` 将移除临时工作空间。 |
| `tail <id>` | 跟随任务的事件流。 |
| `dispatch` | 在活动看板上执行一次调度器轮询。标志：`--dry-run`，`--max N`，`--failure-limit N`，`--json`。 |
| `context <id>` | 打印 worker 将看到的完整上下文（标题 + 正文 + 父任务结果 + 评论）。 |
| `specify <id>` / `specify --all` | 通过辅助 LLM，将待办列中的一个任务具体化为一个详细的规范（标题 + 包含目标、方法、验收标准的正文），然后将其提升到 `todo` 状态。标志：`--tenant`（将 `--all` 限定在一个租户内），`--author`，`--json`。在 `config.yaml` 的 `auxiliary.triage_specifier` 下配置模型。 |
| `decompose <id>` / `decompose --all` | 将待办列中的一个任务分解为由描述路由到专家配置文件的子任务图（编排器驱动路径）。当 LLM 认为任务不适合分解时，回退到 specify 风格的单任务提升。标志与 `specify` 相同。在 `config.yaml` 的 `auxiliary.kanban_decomposer` 下配置模型。当 `kanban.auto_decompose: true`（默认）时，每次调度器轮询也会自动运行。参见[自动与手动编排](/docs/user-guide/features/kanban#auto-vs-manual-orchestration)。 |
| `gc` | 移除已归档任务的临时工作空间。 |

示例：

```bash
# 创建第二个看板并在其上放置一个任务，而不切换当前活动看板。
...
所有操作也都可以作为消息网关中的斜杠命令使用（`/kanban …`），具有相同的参数界面——包括 `boards` 子命令和 `--board` 标志。

完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的比较、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/hermes-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/docs/user-guide/features/kanban)。
...
| `--events` | 要接受的事件类型，逗号分隔（例如 `issues,pull_request`）。空 = 全部。 |
| `--description` | 人类可读的描述。 |
...
订阅信息持久化到 `~/.hermes/webhook_subscriptions.json`，并由 webhook 适配器热重载，无需重启消息网关。
...
将调试报告（系统信息 + 近期日志）上传到粘贴服务并获取可分享的 URL。对于快速支持请求非常有用——包含了帮助者诊断问题所需的一切信息。
...
报告包含系统信息（操作系统、Python 版本、Hermes 版本）、最近的 Agent 和消息网关日志（每个文件限制 512 KB）以及脱敏的 API 密钥状态。密钥始终被脱敏——不会上传任何秘密。
...
- `checkpoints/` — 每个会话的轨迹缓存。基于哈希键生成，每个会话都会重新生成；无论如何也无法干净地移植到另一个安装中。
...
| 子命令 | 描述 |
|------------|-------------|
| `status` (默认) | 显示总大小、项目数量和每个项目的细分。裸命令 `hermes checkpoints` 与此等效。 |
| `list` | `status` 的别名。 |
...
| `--max-size-mb N` | `prune` | 在孤儿/陈旧项目清理后，按项目删除最旧的提交，直到存储总大小 ≤ N MB（默认 500）。 |
...
hermes checkpoints prune --max-size-mb 200          # 收紧一次大小限制
...
将先前创建的 Hermes 备份恢复到你的 Hermes 主目录。存档中的所有文件将覆盖 Hermes 主目录中的现有文件；`--force` 仅跳过在目标目录已存在 Hermes 安装时触发的确认提示。
...
- 传递一个 `http(s)://…/*.md` URL 会直接安装一个单文件 SKILL.md。当 frontmatter 中没有 `name:` 且 URL 段不是有效标识符时，交互式终端会提示输入名称；非交互式界面（TUI 内的 `/skills install`、消息网关平台）则需要 `--name <x>`。
...
请参阅 [Hooks](../user-guide/features/hooks.md) 了解事件签名和负载结构。
...
| 子命令 | 描述 |
|------------|-------------|
| `serve [-v\|--verbose]` | 将 Hermes 作为 MCP 服务器运行 —— 向其他 Agent 公开对话。 |
...
- **提供商插件** —— 用于记忆提供商和上下文引擎的单选配置。按 ENTER 键选择一个类别以打开单选选择器。
...
`hermes computer-use install` 是用于安装 `computer_use` 工具集使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口点。它运行与 `hermes tools` 在你首次启用计算机使用时调用的相同上游安装程序，因此，如果工具集开关没有触发安装（例如，在返回用户的设置中），重新运行安装是安全的。

`hermes update` 在更新结束时，如果 cua-driver 在 PATH 上，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你希望立即获取的修复程序，而不想等待下一次 Hermes 更新时，请使用它。
...
将你的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hermes`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件（`clawdbot.json`、`moltbot.json`）。
...
| `--no-backup` | 跳过迁移前对 `~/.hermes/` 的压缩快照（默认情况下，在应用前会创建一个可恢复的存档到 `~/.hermes/backups/pre-migration-*.zip`；可使用 `hermes import` 恢复）。 |
...
迁移涵盖 30 多个类别，包括人格、记忆、技能、模型提供商、消息平台、Agent 行为、会话策略、MCP 服务器、TTS 等。项目要么**直接导入**到 Hermes 的等效项，要么**存档**以供手动审查。
...
| `alias <name> [--remove] [--name NAME]` | 管理包装器脚本以实现快速配置文件访问。 |
...
将 shell 自动补全脚本打印到标准输出。在你的 shell 配置文件中引入此输出，即可获得 Hermes 命令、子命令和配置文件的 Tab 自动补全功能。
...
拉取最新的 `hermes-agent` 代码并在你的虚拟环境中重新安装依赖，然后重新运行安装后钩子（MCP 服务器、技能同步、补全安装）。可在已安装并运行的系统上安全执行。

**pip 安装：** `hermes update` 会自动检测基于 pip 的安装——它会查询 PyPI 获取最新版本，并运行 `pip install --upgrade hermes-agent` 而不是 `git pull`。PyPI 发布跟踪的是带标签的版本（主要/次要版本），而不是 `main` 分支上的每一次提交。使用 `--check` 可以查看是否有更新的 PyPI 版本可用，而无需安装。

| 选项 | 描述 |
|--------|-------------|
| `--check` | 并排打印当前提交和最新的 `origin/main` 提交，如果同步则退出码为 0，如果落后则退出码为 1。不执行拉取、安装或重启任何操作。 |
| `--backup` | 在拉取之前，为 `HERMES_HOME`（配置、认证、会话、技能、配对数据）创建一个带标签的更新前快照。默认**关闭**——之前总是备份的行为会在大型主目录上为每次更新增加数分钟时间。可以通过在 `config.yaml` 中设置 `update.backup: true` 来永久开启此功能。 |

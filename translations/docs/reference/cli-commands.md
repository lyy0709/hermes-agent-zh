...
对于程序化调用者（shell 脚本、CI、定时任务、父进程通过管道输入提示词），`hermes -z` 是最纯粹的单次调用入口点：**单条提示词输入，最终响应文本输出，标准输出或标准错误上无其他内容。** 没有横幅，没有加载动画，没有工具预览，没有 `Session:` 行——只有 Agent 的最终回复，纯文本。
...
每次运行的覆盖项（不会修改 `~/.hermes/config.yaml`）：
...
相同的 Agent，相同的工具，相同的技能——只是去掉了所有交互/装饰层。如果你也需要工具输出在记录中，请改用 `hermes chat -q`；`-z` 明确用于“我只需要最终答案”。

## `hermes model`

交互式提供商 + 模型选择器。**此命令用于添加新提供商、设置 API 密钥和运行 OAuth 流程。** 从你的终端运行它——而不是在活跃的 Hermes 聊天会话内部。
...
| 选项 | 描述 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：作用于**每个配置文件的**消息网关，而不仅仅是活跃的 `HERMES_HOME`。如果你并行运行多个配置文件，并想在 `hermes update` 后重启所有网关，这很有用。 |
...
管理语言服务器协议集成。LSP 在后台运行真实的语言服务器（pyright、gopls、rust-analyzer 等），并将其诊断信息提供给 `write_file` 和 `patch` 使用的写入后检查。此功能受 git 工作区检测限制——仅当当前工作目录或编辑的文件在 git 工作树内时，LSP 才会运行。
...
**已配置用户（已配置）：** 直接进入完整的重新配置向导 —— 每个提示都会显示当前值作为默认值，按 Enter 键保留或输入新值。没有菜单。
...
| 选项 | 描述 |
|--------|-------------|
| `--quick` | 在已配置用户运行时：仅提示缺失或未设置的项。跳过已配置的项。 |
...
生成一个 Slack 应用清单，将 `COMMAND_REGISTRY` 中的每个消息网关命令（`/btw`、`/stop`、`/model`、…）注册为 Slack 的一级斜杠命令 —— 以实现与 Discord 和 Telegram 的对等功能。将输出粘贴到你的 Slack 应用配置中，位置在 [https://api.slack.com/apps](https://api.slack.com/apps) → 你的应用 → **Features → App Manifest → Edit**，然后 **Save**。如果权限范围或斜杠命令发生更改，Slack 会提示重新安装。
...
| `comment <id> "<text>"` | 追加评论。下一个认领该任务的 worker 会将其作为 `kanban_show()` 响应的一部分读取。 |
...
| `archive <id>` | 从默认列表中隐藏。`gc` 将删除临时工作空间。 |
| `tail <id>` | 跟随任务的事件流。 |
...
| `decompose <id>` / `decompose --all` | 将待办列中的任务分解为由专家角色根据描述路由的子任务图（编排器驱动路径）。当 LLM 认为任务不适合分解时，回退到 specify 式的单任务提升。标志与 `specify` 相同。在 `config.yaml` 的 `auxiliary.kanban_decomposer` 下配置模型。当 `kanban.auto_decompose: true`（默认）时，每次调度器轮询也会自动运行。参见[自动与手动编排](/docs/user-guide/features/kanban#auto-vs-manual-orchestration)。 |
| `gc` | 删除已归档任务的临时工作空间。 |

示例：

```bash
# 创建第二个看板并在其上放置任务，而不切换当前活动看板。
...
- `*.db-wal`, `*.db-shm`, `*.db-journal` — SQLite 的 WAL / 共享内存 / 日志附属文件。`*.db` 文件已通过 `sqlite3.backup()` 获得一致快照；将实时附属文件与它一起打包会让恢复操作看到未完全提交的状态。
...
| 子命令 | 描述 |
|------------|-------------|
| `list` (别名: `ls`) | 显示当前的备用链（未提供子命令时的默认行为） |
...
检查在 `~/.hermes/config.yaml` 中声明的 shell 脚本钩子，针对合成负载进行测试，并管理位于 `~/.hermes/shell-hooks-allowlist.json` 的首次使用同意允许列表。
...
`hermes computer-use install` 是安装 `computer_use` 工具集使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口点。它运行与 `hermes tools` 在你首次启用计算机使用时调用的相同上游安装程序，因此如果工具集开关没有触发安装（例如，在返回用户设置中），重新运行安装是安全的。
...
将你的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取，并写入到 `~/.hermes`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。
| 选项 | 说明 |
|--------|-------------|
| `--dry-run` | 预览将要迁移的内容，但不实际写入任何内容。 |
| `--preset <name>` | 迁移预设：`full`（所有兼容的设置）或 `user-data`（排除基础设施配置）。两个预设均不导入密钥 — 需显式传递 `--migrate-secrets`。 |
...
启动 Web 仪表板 — 一个基于浏览器的 UI，用于管理配置、API 密钥和监控会话。需要 `pip install hermes-agent[web]`（FastAPI + Uvicorn）。嵌入式浏览器聊天标签页需要 `--tui` 加上 `pty` 额外依赖项。完整文档请参阅 [Web 仪表板](/docs/user-guide/features/web-dashboard)。
...
将 shell 自动补全脚本打印到标准输出。在您的 shell 配置文件中引入此输出，即可为 Hermes 命令、子命令和配置文件名称启用 Tab 键自动补全功能。
...
拉取最新的 `hermes-agent` 代码，并在您的虚拟环境中重新安装依赖项，然后重新运行安装后钩子（MCP 服务器、技能同步、自动补全安装）。可以在已安装并运行的系统上安全执行。

**pip 安装：** `hermes update` 会自动检测基于 pip 的安装——它会查询 PyPI 获取最新版本，并运行 `pip install --upgrade hermes-agent` 而不是 `git pull`。PyPI 发布版本跟踪的是带标签的版本（主要/次要版本），而不是 `main` 分支上的每次提交。使用 `--check` 选项可以查看是否有更新的 PyPI 版本可用，而无需实际安装。

| 选项 | 描述 |
|--------|-------------|
| `--check` | 并排打印当前提交和最新的 `origin/main` 提交，如果同步则退出码为 0，如果落后则退出码为 1。不执行拉取、安装或重启任何操作。 |
| `--backup` | 在拉取更新前，为 `HERMES_HOME`（配置、认证、会话、技能、配对数据）创建一个带标签的更新前快照。默认**关闭**——之前总是备份的行为在大型主目录上会给每次更新增加数分钟时间。可以通过在 `config.yaml` 中设置 `update.backup: true` 来永久开启此功能。 |
| `--restart-gateway` | 在成功更新后，重启正在运行的消息网关服务。如果安装了多个配置文件，则隐含 `--all` 语义。 |

其他行为：

- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 之前为 `~/.hermes/pairing/` 和飞书评论规则创建一个轻量级快照。如果拉取操作覆盖了您正在编辑的文件，您可以使用 `hermes backup restore --state pre-update` 回滚到该快照。
- **遗留 `hermes.service` 警告。** 如果 Hermes 检测到重命名前的 `hermes.service` systemd 单元（而不是当前的 `hermes-gateway.service`），它会打印一次性的迁移提示，以便您避免服务反复启动/停止的问题。

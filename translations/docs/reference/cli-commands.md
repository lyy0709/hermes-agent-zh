...
| `hermes update` | 拉取最新代码并重新安装依赖项。`--check` 打印提交差异而不拉取；`--backup` 在拉取前创建 `HERMES_HOME` 快照。 |
...
对于程序化调用者（shell 脚本、CI、定时任务、父进程通过管道传入提示词），`hermes -z` 是最纯粹的单次调用入口点：**单条提示词输入，最终响应文本输出，stdout 或 stderr 上不输出任何其他内容。** 没有横幅、没有加载动画、没有工具预览、没有 `Session:` 行——只有 Agent 的最终回复，纯文本。
...
相同的 Agent，相同的工具，相同的技能——只是去掉了所有交互/装饰层。如果你也需要在记录中包含工具输出，请改用 `hermes chat -q`；`-z` 明确用于“我只想要最终答案”的场景。
...
| `--reset` | 在设置前将配置重置为默认值。 |
| `--reconfigure` | 向后兼容的别名——现有安装上运行不带参数的 `hermes setup` 现在默认执行此操作。 |
## `hermes whatsapp`
...
生成一个 Slack 应用清单，将 `COMMAND_REGISTRY` 中的每个消息网关命令（`/btw`、`/stop`、`/model` 等）注册为 Slack 原生的斜杠命令，以实现与 Discord 和 Telegram 的对等功能。将输出内容粘贴到你的 Slack 应用配置中（访问 [https://api.slack.com/apps](https://api.slack.com/apps) → 你的应用 → **Features → App Manifest → Edit**，然后 **Save**）。如果权限范围或斜杠命令发生更改，Slack 会提示重新安装。

| 标志 | 默认值 | 用途 |
|------|---------|---------|
| `--write [PATH]` | stdout | 写入文件而非标准输出。仅使用 `--write` 会写入 `$HERMES_HOME/slack-manifest.json`。 |
...
| `tail <id>` | 跟随任务的事件流。 |
| `dispatch` | 在活动看板上执行一次调度器遍历。标志：`--dry-run`、`--max N`、`--json`。 |
| `context <id>` | 打印工作线程将看到的完整上下文（标题 + 正文 + 父任务结果 + 评论）。 |
| `specify <id>` / `specify --all` | 通过辅助 LLM 将待办列中的任务具体化为详细规范（包含目标、方法、验收标准的标题和正文），然后将其提升为 `todo` 状态。标志：`--tenant`（将 `--all` 限定到一个租户）、`--author`、`--json`。在 `config.yaml` 的 `auxiliary.triage_specifier` 下配置模型。 |
| `gc` | 移除已归档任务的临时工作空间。 |
示例：
...
关于完整设计 —— 与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的比较、八种协作模式、四个用户故事、并发正确性证明 —— 请参阅仓库中的 `docs/hermes-kanban-v1-spec.pdf` 或 [看板用户指南](/docs/user-guide/features/kanban)。
...
| 选项 | 描述 |
|--------|-------------|
| `--prompt` | 提示词模板，包含 `{dot.notation}` 格式的有效载荷引用。 |
...
报告包含系统信息（操作系统、Python 版本、Hermes 版本）、近期 Agent 和消息网关日志（每个文件限制 512 KB）以及脱敏的 API 密钥状态。密钥始终被脱敏 —— 不会上传任何秘密信息。
...
- `*.db-wal`、`*.db-shm`、`*.db-journal` — SQLite 的 WAL / 共享内存 / 日志附属文件。`*.db` 文件已通过 `sqlite3.backup()` 获得一致快照；附带传输活动的附属文件会让恢复操作看到未完全提交的状态。
...
| `run` | 立即触发策展人审查（阻塞直到 LLM 阶段完成） |
| `run --background` | 在后台线程中启动 LLM 阶段并立即返回 |
...
在新安装后，第一次计划运行会推迟一个完整的 `interval_hours`（默认为 7 天）——消息网关不会在 `hermes update` 后的第一次触发时立即进行策展。在此之前，可以使用 `hermes curator run --dry-run` 进行预览。
...
| 子命令 | 描述 |
|------------|-------------|
| `list` (别名: `ls`) | 显示当前的备用链（无子命令时的默认操作） |
...
| `revoke` (别名: `remove`, `rm`) | 移除命令的允许列表条目（下次重启后生效） |
...
:::info 提供商特定子命令
当外部记忆提供商处于活动状态时，它可能会注册自己的顶级 `hermes <提供商>` 命令，用于提供商特定的管理（例如，当 Honcho 激活时，使用 `hermes honcho`）。非活动状态的提供商不会暴露其子命令。运行 `hermes --help` 查看当前已连接的命令。
:::
...
将您的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取，并写入 `~/.hermes`。自动检测旧目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件（`clawdbot.json`、`moltbot.json`）。
...
将 shell 自动补全脚本打印到标准输出。在你的 shell 配置文件中加载此输出，即可获得 Hermes 命令、子命令和配置文件名的 Tab 键自动补全功能。
...
| `--backup` | 在拉取之前，为 `HERMES_HOME`（配置、认证、会话、技能、配对数据）创建一个带标签的更新前快照。默认**关闭**——之前总是备份的行为会在大型主目录上为每次更新增加数分钟时间。可以通过在 `config.yaml` 中设置 `update.backup: true` 来永久开启此选项。 |
...
- **遗留 `hermes.service` 警告。** 如果 Hermes 检测到重命名前的 `hermes.service` systemd 单元（而非当前的 `hermes-gateway.service`），它会打印一次性的迁移提示，以避免服务反复启动/停止的问题。

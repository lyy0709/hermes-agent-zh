...
对于编程调用者（shell 脚本、CI、定时任务、父进程通过管道输入提示），`hermes -z` 是最纯粹的单次运行入口点：**单次提示输入，最终响应文本输出，stdout 或 stderr 上无其他内容。** 没有横幅，没有加载动画，没有工具预览，没有 `Session:` 行——只有 Agent 的最终回复作为纯文本。
...
管理语言服务器协议集成。LSP 在后台运行真正的语言服务器（pyright、gopls、rust-analyzer 等），并将其诊断信息输入到 `write_file` 和 `patch` 使用的写入后检查中。依赖于 git 工作区检测——仅当当前工作目录或编辑的文件位于 git 工作树内时，LSP 才会运行。
...
**返回用户（已配置）：** 直接进入完整的重新配置向导——每个提示都显示您当前的设置值作为默认值，按 Enter 键保留或输入新值。没有菜单。
...
| `--portal` | 一键 Nous Portal 设置：通过 OAuth 登录，将 Nous 设置为推理提供商，并启用 [工具网关](../user-guide/features/tool-gateway.md)。跳过向导的其余部分。 |
...
检查 Nous Portal 认证状态、工具网关路由，并访问订阅页面。不带子命令的调用将运行 `status`。
...
生成一个 Slack 应用清单，将 `COMMAND_REGISTRY` 中的每个网关命令（`/btw`、`/stop`、`/model` 等）注册为一流的 Slack 斜杠命令——与 Discord 和 Telegram 功能对等。将输出粘贴到您的 Slack 应用配置中，位置在 [https://api.slack.com/apps](https://api.slack.com/apps) → 您的应用 → **Features → App Manifest → Edit**，然后 **Save**。如果权限范围或斜杠命令发生更改，Slack 会提示重新安装。
...
运行一个本地 OpenAI 兼容的 HTTP 服务器，将请求转发到经过 OAuth 认证的上游提供商（例如 Nous Portal、xAI）。外部应用程序可以使用任何承载令牌指向该代理；代理在转发时会附加您真实的 OAuth 凭据。完整指南请参阅：[订阅代理](../user-guide/features/subscription-proxy.md)。
...
**这是人工/脚本操作界面。** 由调度器生成的 Agent 工作进程通过专用的 `kanban_*` [工具集](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器配置文件还拥有 `kanban_list` 和 `kanban_unblock`）来驱动看板，而不是通过 shell 调用 `hermes kanban`。工作进程的环境变量中固定设置了 `HERMES_KANBAN_BOARD`，因此它们实际上无法看到其他看板。
...
| `specify <id>` / `specify --all` | 通过辅助 LLM，将待处理列中的任务充实为具体规格（标题 + 包含目标、方法、验收标准的正文），然后将其提升到 `todo` 状态。标志：`--tenant`（将 `--all` 限定到一个租户）、`--author`、`--json`。在 `config.yaml` 中的 `auxiliary.triage_specifier` 下配置模型。 |
| `decompose <id>` / `decompose --all` | 将待处理列中的任务分解为由描述路由到专家配置文件的子任务图（编排器驱动的路径）。当 LLM 认为任务无法从分解中受益时，回退到 specify 风格的单任务提升。标志与 `specify` 相同。在 `config.yaml` 中的 `auxiliary.kanban_decomposer` 下配置模型。当 `kanban.auto_decompose: true`（默认值）时，也会在每个调度器周期自动运行。请参阅[自动与手动编排](/user-guide/features/kanban#auto-vs-manual-orchestration)。 |
| `gc` | 移除已归档任务的临时工作空间。 |
示例：
...
所有操作也可以通过消息网关中的斜杠命令（`/kanban …`）使用，具有相同的参数界面——包括 `boards` 子命令和 `--board` 标志。
...
- `*.db-wal`, `*.db-shm`, `*.db-journal` — SQLite 的 WAL / 共享内存 / 日志辅助文件。`*.db` 文件已通过 `sqlite3.backup()` 获得一致快照；附带这些活动的辅助文件会让恢复时看到未完全提交的状态。
...
当 `--since` 激活时，无法解析时间戳的日志行也会被包含（它们可能是多行日志条目的续行）。当 `--level` 激活时，无法检测到级别的日志行也会被包含。
...
| `create <名称>` | 创建一个新包。传递 `--skill <id>`（可重复）或省略以进行交互式输入。支持 `--description`、`--instruction`、`--force`。 |
| `delete <名称>` | 移除一个包文件 |
...
在全新安装后，首次定时执行会推迟一个完整的 `interval_hours`（默认为 7 天）—— 消息网关不会在 `hermes update` 后的第一次触发时立即进行内容管理。在此之前，可以使用 `hermes curator run --dry-run` 进行预览。

有关行为和配置，请参阅 [内容管理](../user-guide/features/curator.md)。
...
| `test <event>` | 使用合成负载触发每个匹配 `<event>` 的钩子 |
...
| `login <name>` | 强制重新验证基于 OAuth 的 MCP 服务器。 |
...
- **提供商插件** —— 用于记忆提供商和上下文引擎的单选配置。按 ENTER 键打开类别选择器。
...
`hermes computer-use install` 是用于安装 `computer_use` 工具集使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口点。它运行与首次启用计算机使用时 `hermes tools` 调用的相同上游安装程序，因此如果工具集切换未触发安装（例如，在返回用户设置中），重新运行安装是安全的。
...
| `export <输出文件> [--session-id ID]` | 将会话导出为 JSONL。 |
| `delete <会话ID>` | 删除一个会话。 |
| `prune` | 删除旧会话。 |
| `stats` | 显示会话存储统计信息。 |
...
将您的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hermes`。自动检测旧目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件（`clawdbot.json`、`moltbot.json`）。

| 选项 | 描述 |
|--------|-------------|
| `--dry-run` | 预览将要迁移的内容，不写入任何内容。 |
| `--preset <名称>` | 迁移预设：`full`（所有兼容设置）或 `user-data`（排除基础设施配置）。两种预设均不导入密钥 — 需要显式传递 `--migrate-secrets`。 |
| `--overwrite` | 冲突时覆盖现有的 Hermes 文件（默认：当计划有冲突时拒绝应用）。 |
...
| `--host` | `127.0.0.1` | 绑定地址 |
| `--no-open` | — | 不自动打开浏览器 |
| `--tui` | 关闭 | 通过在 PTY/WebSocket 桥接后运行 `hermes --tui` 来启用浏览器内的聊天标签页。需要 `pip install 'hermes-agent[web,pty]'` 和 POSIX PTY 环境，例如 Linux、macOS 或 WSL2。 |
| `--insecure` | 关闭 | 允许绑定到非 localhost 主机。在网络上暴露仪表板凭据；仅在受信任的网络控制后使用。 |
...
| `list` | 列出所有配置文件。 |
| `use <名称>` | 设置一个粘性默认配置文件。 |
| `create <名称> [--clone] [--clone-all] [--clone-from <源>] [--no-alias]` | 创建一个新的配置文件。`--clone` 从当前活动配置文件复制配置、`.env` 和 `SOUL.md`。`--clone-all` 复制所有状态。`--clone-from` 指定源配置文件。 |
| `delete <名称> [-y]` | 删除一个配置文件。 |
| `show <名称>` | 显示配置文件详情（主目录、配置等）。 |
| `alias <名称> [--remove] [--name 名称]` | 管理包装器脚本以快速访问配置文件。 |
...
| `update <名称> [--force-config] [-y]` | 重新拉取分发版；保留用户数据（记忆、会话、认证）。 |
| `info <名称>` | 显示配置文件的分发版清单（版本、要求、来源）。 |
...
拉取最新的 `hermes-agent` 代码并在您的虚拟环境中重新安装依赖项，然后重新运行安装后钩子（MCP 服务器、技能同步、自动补全安装）。可在正在运行的安装上安全执行。

**pip 安装：** `hermes update` 会自动检测基于 pip 的安装 — 它会查询 PyPI 获取最新版本并运行 `pip install --upgrade hermes-agent`，而不是 `git pull`。PyPI 发布跟踪的是带标签的版本（主要/次要版本），而不是 `main` 分支上的每次提交。使用 `--check` 可以查看是否有更新的 PyPI 版本可用，而无需安装。

| 选项 | 描述 |
|--------|-------------|
| `--gateway` | 消息网关 `/update` 命令使用的内部模式。使用基于文件的 IPC 进行提示词和进度流传输，而不是从终端标准输入读取。这不是消息网关重启标志。 |
| `--check` | 检查是否有更新可用，但不进行拉取、安装依赖项或重启任何操作。 |
| `--no-backup` | 跳过本次运行的更新前备份，即使 `config.yaml` 中启用了 `updates.pre_update_backup`。 |
| `--backup` | 在拉取之前创建 `HERMES_HOME`（配置、认证、会话、技能、配对数据）的带标签的更新前快照。默认是**关闭**的 — 之前总是备份的行为会给大型主目录的每次更新增加数分钟时间。可以通过在 `config.yaml` 中设置 `updates.pre_update_backup: true` 永久开启。 |
| `--yes`, `-y` | 对于交互式提示（如配置迁移和存储恢复）假设回答为“是”。跳过 API 密钥输入；请单独运行 `hermes config migrate` 来处理这些。 |

其他行为：

- **消息网关重启。** 成功更新后，Hermes 会尝试自动重启所有正在运行的消息网关配置文件，以便它们获取新代码。当您想在不应用更新的情况下重启消息网关时，请使用 `hermes gateway restart`。
- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 之前对 `~/.hermes/pairing/` 和飞书评论规则进行轻量级快照。如果拉取操作重写了您正在编辑的文件，您可以使用 `hermes backup restore --state pre-update` 回滚。
- **遗留 `hermes.service` 警告。** 如果 Hermes 检测到重命名前的 `hermes.service` systemd 单元（而不是当前的 `hermes-gateway.service`），它会打印一次性迁移提示，以便您可以避免 flap-loop 问题。
...
| `hermes postinstall` | 内部引导程序。在 `pip install hermes-agent`（或 pip 安装上的 `hermes update`）之后运行一次，以安装 pip 无法提供的非 Python 依赖项 — Node.js 运行时、无头浏览器、ripgrep、ffmpeg — 然后如果配置文件尚未配置，则触发 `hermes setup`。可以安全地重新运行，具有幂等性。 |

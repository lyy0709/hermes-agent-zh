...
对于程序化调用者（shell 脚本、CI、定时任务、父进程通过管道输入提示词），`hermes -z` 是最纯粹的单次调用入口点：**单条提示词输入，最终响应文本输出，标准输出或标准错误上不输出任何其他内容。** 没有横幅，没有加载动画，没有工具预览，没有 `Session:` 行 —— 只有 Agent 的最终回复作为纯文本。
...
交互式提供商 + 模型选择器。**这是用于添加新提供商、设置 API 密钥和运行 OAuth 流程的命令。** 从你的终端运行它 —— 而不是在活动的 Hermes 聊天会话内部。
...
**`/model`**（在活动的 Hermes 聊天会话中键入）只能**在你已设置的提供商和模型之间切换**。它不能添加新提供商、运行 OAuth 或提示输入 API 密钥。
...
| 选项 | 描述 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：作用于**每个配置文件的**消息网关，而不仅仅是活动的 `HERMES_HOME`。如果你并行运行多个配置文件，并想在 `hermes update` 后重启所有网关，这很有用。 |
...
| `archive <id>` | 从默认列表中隐藏。`gc` 将移除暂存工作空间。 |
| `tail <id>` | 跟随任务的事件流。 |
| `dispatch` | 在活动看板上进行一次调度器遍历。标志：`--dry-run`、`--max N`、`--json`。 |
| `context <id>` | 打印工作者将看到的完整上下文（标题 + 正文 + 父任务结果 + 评论）。 |
| `specify <id>` / `specify --all` | 通过辅助 LLM 将待办列中的任务细化为具体规格（包含目标、方法、验收标准的标题和正文），然后将其提升为 `todo`。标志：`--tenant`（将 `--all` 限定到一个租户）、`--author`、`--json`。在 `config.yaml` 的 `auxiliary.triage_specifier` 下配置模型。 |
| `gc` | 移除已归档任务的暂存工作空间。 |
示例：

```bash
# 创建第二个看板并在不切换的情况下添加任务。
...
策展人是一个辅助模型的背景任务，定期审查 Agent 创建的技能，清理过时的技能，合并重叠的技能，并归档废弃的技能。捆绑的和通过 Hub 安装的技能永远不会被触及。归档是可恢复的；永远不会自动删除。
...
在全新安装后，第一次计划运行会推迟一个完整的 `interval_hours`（默认为 7 天）——消息网关不会在 `hermes update` 后的第一次触发时立即进行策展。在此之前，可以使用 `hermes curator run --dry-run` 进行预览。
...
检查在 `~/.hermes/config.yaml` 中声明的 shell 脚本钩子，使用合成负载对其进行测试，并管理首次使用同意允许列表 `~/.hermes/shell-hooks-allowlist.json`。

| 子命令 | 描述 |
|------------|-------------|
| `list` (别名: `ls`) | 列出已配置的钩子，包含匹配器、超时和同意状态 |
...
将 shell 自动补全脚本打印到标准输出。在您的 shell 配置文件中加载输出，以获得 Hermes 命令、子命令和配置文件名称的 Tab 键补全功能。
...
管理备用提供商链（当主提供商触发速率限制或返回致命错误时使用），无需手动编辑 `config.yaml`。复用 `hermes model` 中的提供商选择器——相同的提供商列表、相同的凭证提示、相同的验证。

典型会话：

1. 按 `a` 添加备用提供商 → 选择提供商（基于 OAuth 的提供商将打开浏览器；API 密钥提供商将提示输入密钥），然后选择具体模型。
2. 使用 `↑`/`↓` 重新排序备用提供商（列表中的第一个将首先被尝试）。
3. 按 `d` 删除一个。

所有更改将持久化到 `config.yaml` 中的顶层 `fallback_providers:` 列表。与[凭证池](/docs/user-guide/features/credential-pools)交互：凭证池在*同一*提供商内轮换密钥，而备用提供商则完全切换到*不同*的提供商。

有关行为细节以及与 `fallback_model`（旧版单备用键）交互的详细信息，请参阅[备用提供商](/docs/user-guide/features/fallback-providers)。

...
管理 Honcho 跨会话记忆集成。此命令由 Honcho 记忆提供商插件提供，仅当配置中 `memory.provider` 设置为 `honcho` 时可用。
...
将 shell 自动补全脚本打印到标准输出。在你的 shell 配置文件中引用输出，以获得 Hermes 命令、子命令和配置文件名称的 Tab 键自动补全。

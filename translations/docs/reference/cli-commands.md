...
| `hermes logs` | 查看、跟踪和筛选 Agent/网关/错误日志文件。 |
...
| `hermes acp` | 将 Hermes 作为 ACP 服务器运行以用于编辑器集成。 |
...
| `--source <tag>` | 用于筛选的会话来源标签（默认：`cli`）。对于不应出现在用户会话列表中的第三方集成，请使用 `tool`。 |
...
:::tip WSL 用户
使用 `hermes gateway run` 而不是 `hermes gateway start`——WSL 的 systemd 支持不可靠。将其包装在 tmux 中以实现持久化：`tmux new -s hermes 'hermes gateway run'`。详见 [WSL 常见问题](/docs/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。
:::
...
**返回用户（已配置）：** 直接进入完整的重新配置向导——每个提示都显示你当前的值为其默认值，按 Enter 键保留或键入新值。没有菜单。
...
生成一个 Slack 应用清单，将 `COMMAND_REGISTRY` 中的每个消息网关命令（`/btw`、`/stop`、`/model`……）注册为 Slack 原生的斜杠命令，以实现与 Discord 和 Telegram 的对等性。将输出粘贴到你的 Slack 应用配置中：[https://api.slack.com/apps](https://api.slack.com/apps) → 你的应用 → **Features → App Manifest → Edit**，然后 **Save**。如果权限范围或斜杠命令发生更改，Slack 会提示重新安装。
...
多配置文件、多项目协作看板。每个安装可以托管多个看板（每个项目、仓库或领域一个）；每个看板都是一个独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装开始时有一个名为 `default` 的看板，其数据库为 `~/.hermes/kanban.db` 以保持向后兼容性；其他看板位于 `~/.hermes/kanban/boards/<slug>/kanban.db`。消息网关内嵌的调度器在每个周期都会扫描每个看板。
...
**这是人工/脚本操作界面。** 由调度器生成的 Agent 工作线程通过专用的 `kanban_*` [工具集](/docs/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`）来驱动看板，而不是通过 shell 调用 `hermes kanban`。工作线程的环境中固定设置了 `HERMES_KANBAN_BOARD`，因此它们实际上无法看到其他看板。
...
- `*.db-wal`, `*.db-shm`, `*.db-journal` — SQLite 的 WAL / 共享内存 / 日志附属文件。`*.db` 文件已通过 `sqlite3.backup()` 获得一致快照；将实时附属文件与它一起打包，会让恢复过程看到未完全提交的状态。
...
| 选项 | 描述 |
|--------|-------------|
| `日志名称` | 要查看的日志：`agent`（默认）、`errors`、`gateway`，或使用 `list` 显示可用文件及其大小。 |
| `-n`, `--lines <N>` | 要显示的行数（默认：50）。 |
| `-f`, `--follow` | 实时跟踪日志，类似 `tail -f`。按 Ctrl+C 停止。 |
| `--level <级别>` | 要显示的最低日志级别：`DEBUG`、`INFO`、`WARNING`、`ERROR`、`CRITICAL`。 |
| `--session <ID>` | 过滤包含会话 ID 子字符串的行。 |
| `--since <时间>` | 显示从相对时间之前开始的行：`30m`、`1h`、`2d` 等。支持 `s`（秒）、`m`（分钟）、`h`（小时）、`d`（天）。 |
...
策展人是一个辅助模型后台任务，定期审查 Agent 创建的技能、清理过时的技能、合并重叠的技能并归档废弃的技能。捆绑安装和从中心安装的技能永远不会被触及。归档可恢复；永远不会自动删除。
...
检查在 `~/.hermes/config.yaml` 中声明的 shell 脚本钩子，使用合成负载测试它们，并管理 `~/.hermes/shell-hooks-allowlist.json` 中的首次使用同意允许列表。
...
| 子命令 | 描述 |
|------------|-------------|
| `serve [-v\|--verbose]` | 将 Hermes 作为 MCP 服务器运行 — 向其他 Agent 公开会话。 |
...
- **提供商插件** — 记忆提供商和上下文引擎的单选配置。按 ENTER 键打开类别以选择单选按钮。
...
不带 `--summary` 参数时，此命令将启动交互式的按平台工具配置 UI。
...
| `--no-backup` | 跳过迁移前对 `~/.hermes/` 的 zip 快照（默认情况下，在应用前会向 `~/.hermes/backups/pre-migration-*.zip` 写入一个可恢复的存档；可使用 `hermes import` 恢复）。 |
...
| `export <名称> [-o 文件]` | 将配置文件导出为 `.tar.gz` 归档文件。 |
...
拉取最新的 `hermes-agent` 代码并在你的虚拟环境中重新安装依赖项，然后重新运行安装后钩子（MCP 服务器、技能同步、自动补全安装）。可在已安装的实例上安全运行。

| 选项 | 描述 |
|--------|-------------|
| `--check` | 并排打印当前提交和最新的 `origin/main` 提交，如果同步则退出码为 0，如果落后则退出码为 1。不执行拉取、安装或重启任何操作。 |
| `--backup` | 在拉取之前，为 `HERMES_HOME`（配置、认证、会话、技能、配对数据）创建一个带标签的更新前快照。默认**关闭** — 之前总是备份的行为会给大型主目录的每次更新增加数分钟时间。可以通过在 `config.yaml` 中设置 `update.backup: true` 来永久开启此功能。 |
| `--restart-gateway` | 成功更新后，重启正在运行的消息网关服务。如果安装了多个配置文件，则隐含 `--all` 语义。 |

其他行为：

- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 之前为 `~/.hermes/pairing/` 和飞书评论规则创建一个轻量级快照。如果拉取操作覆盖了你正在编辑的文件，你可以使用 `hermes backup restore --state pre-update` 回滚。
- **遗留 `hermes.service` 警告。** 如果 Hermes 检测到重命名前的 `hermes.service` systemd 单元（而不是当前的 `hermes-gateway.service`），它会打印一次性的迁移提示，以便你避免循环启动问题。
...
管理备用提供商链（当你的主提供商达到速率限制或返回致命错误时使用），无需手动编辑 `config.yaml`。复用 `hermes model` 中的提供商选择器 — 相同的提供商列表、相同的凭证提示、相同的验证。

典型会话：

1. 按 `a` 添加备用提供商 → 选择一个提供商（基于 OAuth 的提供商会打开浏览器；API 密钥提供商会提示输入密钥），然后选择具体的模型。
2. 使用 `↑`/`↓` 重新排序备用提供商（列表中的第一个会首先被尝试）。
3. 按 `d` 移除一个。

所有更改都会持久化到 `config.yaml` 中 `model:` 下的 `fallback_providers:`。与[凭证池](/docs/user-guide/features/credential-pools)交互：凭证池在*同一个*提供商内轮换密钥，而备用链则完全切换到*不同的*提供商。

...
| `--cli` | 强制使用经典的 prompt_toolkit REPL。使用此选项可覆盖单次调用的 `display.interface: tui` 设置。 |
...
| `--ignore-rules` | 跳过 `AGENTS.md`、`SOUL.md`、`.cursorrules`、持久化记忆和预加载技能的自动注入。与 `--ignore-user-config` 结合使用可实现完全隔离的运行。 |
...
对于编程调用者（shell 脚本、CI、定时任务、父进程通过管道输入提示词），`hermes -z` 是最纯粹的单次运行入口点：**单次提示词输入，最终响应文本输出，标准输出或标准错误上无其他内容。** 没有横幅，没有旋转器，没有工具预览，没有 `Session:` 行——只有 Agent 的最终回复作为纯文本。
...
| `restart` | 重启服务。 |
| `status` | 显示服务状态。 |
| `list` | 列出**所有配置文件**以及每个配置文件的消息网关当前是否正在运行（如果可用，会显示 PID）。当您并行运行多个配置文件并希望获得一个总览时非常方便。 |
...
管理语言服务器协议集成。LSP 在后台运行真正的语言服务器（pyright、gopls、rust-analyzer 等），并将其诊断信息输入到 `write_file` 和 `patch` 使用的写入后检查中。受 git 工作区检测限制——仅当当前工作目录或编辑的文件位于 git 工作树内时，LSP 才会运行。
...
运行一个本地的 OpenAI 兼容 HTTP 服务器，将请求转发到经过 OAuth 认证的上游提供商（例如 Nous Portal、xAI）。外部应用程序可以使用任何承载令牌指向该代理；代理在转发时附加您真实的 OAuth 凭据。完整指南请参阅：[订阅代理](../user-guide/features/subscription-proxy.md)。
...
多配置文件、多项目协作看板。每个安装可以托管多个看板（每个项目、仓库或领域一个）；每个看板都是一个独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装开始时有一个名为 `default` 的看板，其数据库为 `~/.hermes/kanban.db` 以保持向后兼容性；其他看板位于 `~/.hermes/kanban/boards/<slug>/kanban.db`。网关内嵌的调度器在每个周期扫描所有看板。
...
| `specify <id>` / `specify --all` | 通过辅助 LLM 将待处理列中的任务具体化为详细规范（标题 + 包含目标、方法、验收标准的正文），然后将其提升到 `todo` 状态。标志：`--tenant`（将 `--all` 限定到一个租户）、`--author`、`--json`。在 `config.yaml` 的 `auxiliary.triage_specifier` 下配置模型。 |
| `decompose <id>` / `decompose --all` | 将待处理列中的任务分解为由描述路由到专家配置文件的子任务图。当 LLM 认为任务不适合分解时，回退到 specify 式的单任务提升。标志与 `specify` 相同。在 `config.yaml` 的 `auxiliary.kanban_decomposer` 下配置分解器模型；`kanban.orchestrator_profile` 仅控制分解后根/编排任务的归属。当 `kanban.auto_decompose: true`（默认值）时，每个调度器周期也会自动运行。参见[自动与手动编排](/user-guide/features/kanban#auto-vs-manual-orchestration)。 |
| `gc` | 移除已归档任务的临时工作空间。 |
示例：
...
完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的比较、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/hermes-kanban-v1-spec.pdf` 或 [看板用户指南](/user-guide/features/kanban)。
...
| 子命令 | 描述 |
|------------|-------------|
| `subscribe` / `add` | 创建 Webhook 路由。返回 URL 和 HMAC 密钥，用于在您的服务上配置。 |
...
- `*.db-wal`、`*.db-shm`、`*.db-journal` — SQLite 的 WAL / 共享内存 / 日志附属文件。`*.db` 文件已通过 `sqlite3.backup()` 获得一致快照；附带活动的附属文件会让恢复看到未完全提交的状态。
- `checkpoints/` — 每个会话的轨迹缓存。按会话哈希键控和重新生成；无论如何也无法干净地移植到另一个安装。
...
| `--max-size-mb N` | `prune` | 在孤立/过期项目清理后，删除每个项目中最旧的提交，直到总存储大小 ≤ N MB（默认 500）。 |
...
将先前创建的 Hermes 备份恢复到您的 Hermes 主目录。归档中的所有文件都会覆盖 Hermes 主目录中的现有文件；`--force` 仅跳过在目标已存在 Hermes 安装时触发的确认提示。
...
Hermes 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转 — 请查找 `agent.log.1`、`agent.log.2` 等文件。`hermes logs list` 子命令会显示包括轮转文件在内的所有日志文件。
...
:::tip
技能索引和工具模式的大小随你启用的技能和工具数量而增加。要缩小提示词，请禁用未使用的工具集（`hermes tools`）或卸载不需要的技能（`hermes skills`）。当前目录中的上下文文件（AGENTS.md、.cursorrules）也会计入总量。
:::
...
| `opt-out` | 停止将捆绑技能植入到活动配置文件中。写入一个 `.no-bundled-skills` 标记，以便安装程序、`hermes update` 和任何同步操作都跳过捆绑技能的植入。默认安全 — 不触及磁盘上的任何内容。使用 `--remove` 时，还会删除已存在的**未修改**的捆绑技能（用户编辑的、通过 hub 安装的和手写的技能永远不会被移除；会先预览并确认，使用 `--yes` 跳过确认）。 |
...
技能捆绑包将多个技能分组在一个 `/<bundle-name>` 斜杠命令下。调用捆绑包会将每个引用的技能加载到单个组合用户消息中。存储位置：`~/.hermes/skill-bundles/<slug>.yaml`。有关 YAML 模式和行为，请参阅[技能捆绑包](../user-guide/features/skills.md#skill-bundles)。

子命令：

| 子命令 | 描述 |
|------------|-------------|
| `list` | 列出已安装的捆绑包（未指定子命令时的默认行为） |
| `show <name>` | 显示一个捆绑包的名称、描述、技能和文件路径 |
| `create <name>` | 创建一个新的捆绑包。传递 `--skill <id>`（可重复）或省略以进行交互式输入。支持 `--description`、`--instruction`、`--force`。 |
| `delete <name>` | 删除捆绑包文件 |
...
在聊天会话中，`/bundles` 列出已安装的捆绑包，`/<bundle-name>` 加载一个捆绑包。
...
策展人是一个辅助模型的后台任务，定期审查 Agent 创建的技能，清理过时的技能，合并重叠的技能，并归档废弃的技能。捆绑包和中心安装的技能永远不会被触及。归档的技能可以恢复；永远不会自动删除。
...
| `install <名称>` | 安装目录中的条目（例如 `hermes mcp install n8n`）。 |
...
`hermes computer-use install` 是用于安装 `computer_use` 工具集使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口点。它运行与 `hermes tools` 在你首次启用计算机使用时调用的相同上游安装程序，因此如果工具集开关没有触发安装（例如，在返回用户设置中），重新运行安装是安全的。
...
将你的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hermes`。自动检测旧版目录名称（`~/.clawdbot`、`~/.moltbot`）和配置文件名称（`clawdbot.json`、`moltbot.json`）。
...
| `--source <路径>` | 自定义 OpenClaw 目录（默认值：`~/.openclaw`）。 |
...
启动 Web 仪表盘——一个基于浏览器的 UI，用于管理配置、API 密钥和监控会话。需要 `pip install hermes-agent[web]`（FastAPI + Uvicorn）。嵌入式浏览器聊天标签始终可用，但额外需要 `pty` 扩展（`pip install 'hermes-agent[web,pty]'`）以及 POSIX PTY 环境，例如 Linux、macOS 或 WSL2。完整文档请参阅 [Web 仪表盘](/user-guide/features/web-dashboard)。
...
| `--host` | `127.0.0.1` | 绑定地址 |
| `--no-open` | — | 不自动打开浏览器 |
| `--insecure` | off | 允许绑定到非 localhost 主机。将仪表盘凭据暴露在网络上；仅在受信任的网络控制后使用。 |
...
| `update <name> [--force-config] [-y]` | 重新拉取分发版；保留用户数据（记忆、会话、认证信息）。 |
...
将 shell 自动补全脚本打印到标准输出。在你的 shell 配置文件中获取输出内容，即可获得 Hermes 命令、子命令和配置文件名称的 Tab 键自动补全功能。
...
拉取最新的 `hermes-agent` 代码并在你的虚拟环境中重新安装依赖项，然后重新运行安装后钩子（MCP 服务器、技能同步、自动补全安装）。可以在已安装的实时环境中安全运行。
...
**git 安装：** `hermes update` 会拉取配置的更新分支（默认：`main`）。如果你的检出在另一个分支上，Hermes 可能会在拉取之前检出更新分支。当你希望在更新自动存储流程之外保留分支工作时，请在更新前提交分支工作。
| 选项 | 描述 |
|--------|-------------|
| `--gateway` | 消息网关 `/update` 命令使用的内部模式。使用基于文件的 IPC 进行提示词和进度流式传输，而不是从终端 stdin 读取。这不是一个消息网关重启标志。 |
| `--check` | 检查是否有可用更新，但不拉取、不安装依赖、不重启任何服务。 |
| `--no-backup` | 跳过本次运行的更新前备份，即使 `config.yaml` 中启用了 `updates.pre_update_backup`。 |
| `--backup` | 在拉取更新前，为 `HERMES_HOME`（配置、认证、会话、技能、配对数据）创建一个带标签的快照。默认是**关闭的**——之前总是备份的行为在大型主目录上会给每次更新增加数分钟时间。可以通过在 `config.yaml` 中设置 `updates.pre_update_backup: true` 来永久开启。 |
| `--yes`, `-y` | 对交互式提示（如配置迁移和存储恢复）假定回答“是”。API 密钥输入会被跳过；如需处理这些，请单独运行 `hermes config migrate`。 |

其他行为：

- **消息网关重启。** 成功更新后，Hermes 会尝试自动重启所有正在运行的消息网关配置文件，以便它们加载新代码。当你想在不应用更新的情况下重启消息网关时，请使用 `hermes gateway restart`。
- **本地源码更改。** 对于 git 安装，在切换分支或拉取之前，会自动暂存已跟踪的脏文件和未跟踪的文件（`git stash push --include-untracked`）。交互式终端更新会在恢复暂存前询问。非交互式更新默认会恢复暂存；仅在受管理的安装上，当成功的拉取后应丢弃本地源码编辑时，才设置 `updates.non_interactive_local_changes: discard`。如果暂存恢复冲突或拉取失败，暂存会保留在原地供手动恢复。
- **npm 锁文件变动。** 在暂存或切换分支之前，Hermes 会尽力清理由 npm install/build 步骤产生的已跟踪 `package-lock.json` 差异。在运行 `hermes update` 之前，请提交或手动暂存有意的锁文件编辑。
...
- **遗留 `hermes.service` 警告。** 如果 Hermes 检测到重命名前的 `hermes.service` systemd 单元（而不是当前的 `hermes-gateway.service`），它会打印一次迁移提示，以便你避免 flap-loop 问题。
...
| `hermes uninstall [--full] [--gui] [--yes]` | 卸载 Hermes，可选择删除所有配置/数据。`--gui` 仅删除桌面聊天 GUI，保留 Agent 完整；`--full` 还会删除配置/数据；`--yes` 跳过提示。 |

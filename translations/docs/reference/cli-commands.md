...
| `hermes kanban` | 多配置文件协作看板（任务、链接、分发器）。 |
...
| `hermes update` | 拉取最新代码并重新安装依赖项。`--check` 打印提交差异而不拉取；`--backup` 在拉取前拍摄 `HERMES_HOME` 快照。 |
...
相同的 Agent，相同的工具，相同的技能——只是剥离了所有交互/装饰层。如果你也需要在记录中看到工具输出，请改用 `hermes chat -q`；`-z` 明确用于“我只想要最终答案”的场景。
...
管理语言服务器协议集成。LSP 在后台运行真实的语言服务器（pyright、gopls、rust-analyzer 等），并将其诊断信息馈送到 `write_file` 和 `patch` 使用的写入后检查中。受 git 工作区检测限制——仅当当前工作目录或编辑的文件位于 git 工作树内时，LSP 才会运行。
...
完整指南、支持的语言和配置选项请参阅 [LSP — 语义诊断](/docs/user-guide/features/lsp)。
...
多配置文件、多项目协作看板。每个安装可以托管多个看板（每个项目、仓库或领域一个）；每个看板都是一个独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装开始时有一个名为 `default` 的看板，其数据库为 `~/.hermes/kanban.db` 以保持向后兼容；其他看板位于 `~/.hermes/kanban/boards/<slug>/kanban.db`。消息网关内嵌的调度器在每个周期扫描每个看板。
...
| `archive <id>` | 从默认列表中隐藏任务。`gc` 将移除其沙盒工作空间。 |
| `tail <id>` | 跟随任务的事件流。 |
| `dispatch` | 在活动看板上执行一次调度器轮询。标志：`--dry-run`，`--max N`，`--json`。 |
| `context <id>` | 打印 worker 将看到的完整上下文（标题 + 正文 + 父任务结果 + 评论）。 |
| `specify <id>` / `specify --all` | 通过辅助 LLM 将待办列中的任务具体化为详细规格（标题 + 包含目标、方法、验收标准的正文），然后将其提升到 `todo` 列。标志：`--tenant`（将 `--all` 范围限定到一个租户），`--author`，`--json`。在 `config.yaml` 的 `auxiliary.triage_specifier` 下配置模型。 |
| `gc` | 移除已归档任务的沙盒工作空间。 |
...
| **Identity** | 当前激活的配置文件名、HERMES_HOME 路径 |
| **Model** | 配置的默认模型和提供商 |
...
报告内容包括系统信息（操作系统、Python 版本、Hermes 版本）、最近的 Agent 和消息网关日志（每个文件限制 512 KB）以及脱敏的 API 密钥状态。密钥始终会被脱敏 — 不会上传任何密钥。
...
- `*.db-wal`, `*.db-shm`, `*.db-journal` — SQLite 的 WAL / 共享内存 / 日志附属文件。`*.db` 文件已通过 `sqlite3.backup()` 获得一致快照；将实时附属文件与它一起打包，可能会让恢复过程看到未完全提交的状态。
...
将先前创建的 Hermes 备份恢复到您的 Hermes 主目录。归档文件中的所有文件将覆盖您 Hermes 主目录中的现有文件；`--force` 仅跳过在目标目录已存在 Hermes 安装时触发的确认提示。
...
过滤器可以组合使用。当多个过滤器激活时，日志行必须通过**所有**过滤器才能显示：
...
在全新安装后，第一次计划运行会推迟一个完整的 `interval_hours`（默认为 7 天）— 消息网关不会在 `hermes update` 后的第一次触发时立即进行 curator 操作。在此之前，可以使用 `hermes curator run --dry-run` 进行预览。
...
`hermes computer-use install` 是用于安装 `computer_use` 工具集使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口点。它运行与 `hermes tools` 在你首次启用计算机使用时调用的相同上游安装程序，因此如果工具集开关没有触发安装（例如，在返回用户的设置中），用它来重新运行安装是安全的。

如果 cua-driver 在 PATH 上，`hermes update` 会在更新结束时自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你希望立即获取的修复，而不想等待下一次 Hermes 更新时，请使用它。
...
将你的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取，并写入 `~/.hermes`。自动检测遗留目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件（`clawdbot.json`、`moltbot.json`）。
...
| `--backup` | 在拉取之前，为 `HERMES_HOME`（配置、认证信息、会话、技能、配对数据）创建一个带标签的更新前快照。默认是 **关闭** —— 以前总是备份的行为在大型主目录上每次更新都会增加几分钟。可以通过在 `config.yaml` 中设置 `update.backup: true` 来永久开启它。 |

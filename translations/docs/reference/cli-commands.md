...
| `--ignore-rules` | 跳过 `AGENTS.md`、`SOUL.md`、`.cursorrules`、持久性记忆和预加载技能的自动注入。与 `--ignore-user-config` 结合使用可实现完全隔离的运行。 |
...
交互式提供商 + 模型选择器。**此命令用于添加新提供商、设置 API 密钥和运行 OAuth 流程。** 请从终端运行——而不是在活跃的 Hermes 聊天会话中。
...
**如果需要添加新提供商：** 先退出 Hermes 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `hermes model`。
:::
...
**返回用户（已配置）：** 直接进入完整的重新配置向导——每个提示都将其当前值显示为默认值，按 Enter 键保留或键入新值。没有菜单。
...
生成一个 Slack 应用清单，将 `COMMAND_REGISTRY` 中的每个消息网关命令（`/btw`、`/stop`、`/model`、…）注册为一流的 Slack 斜杠命令——与 Discord 和 Telegram 功能对等。将输出粘贴到 Slack 应用配置中，位置为 [https://api.slack.com/apps](https://api.slack.com/apps) → 你的应用 → **功能 → 应用清单 → 编辑**，然后**保存**。如果范围或斜杠命令发生更改，Slack 会提示重新安装。
...
上传调试报告（系统信息 + 近期日志）到粘贴服务并获取可分享的 URL。适用于快速寻求支持——包含帮助者诊断问题所需的一切信息。
...
报告包含系统信息（操作系统、Python 版本、Hermes 版本）、近期 Agent 和消息网关日志（每个文件限制 512 KB）以及脱敏后的 API 密钥状态。密钥始终会被脱敏——不会上传任何机密信息。
...
hermes skills install https://example.com/SKILL.md --name my-skill        # 当 frontmatter 中没有 name 时覆盖名称
hermes skills check
...
- 传递一个 `http(s)://…/*.md` URL 会直接安装一个单文件的 SKILL.md。当 frontmatter 中没有 `name:` 且 URL 的 slug 不是有效的标识符时，交互式终端会提示输入名称；非交互式界面（TUI 内的 `/skills install`，消息网关平台）则需要使用 `--name <x>`。
...
不使用 `--summary` 时，此命令将启动交互式的、针对每个平台的工具配置界面。
...
将您的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hermes`。自动检测遗留目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件（`clawdbot.json`、`moltbot.json`）。
...
**直接导入：** SOUL.md、MEMORY.md、USER.md、AGENTS.md、技能（4 个源目录）、默认模型、自定义提供商、MCP 服务器、消息平台令牌和允许列表（Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Mattermost）、Agent 默认值（推理努力程度、压缩、人工延迟、时区、沙盒）、会话重置策略、审批规则、TTS 配置、浏览器设置、工具设置、执行超时、命令允许列表、消息网关配置以及来自 3 个来源的 API 密钥。
...
将 shell 自动补全脚本输出到标准输出。在您的 shell 配置文件中引入此输出，即可为 Hermes 的命令、子命令和配置文件名称启用 Tab 键自动补全功能。

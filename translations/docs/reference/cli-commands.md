...
交互式提供商 + 模型选择器。**这是用于添加新提供商、设置 API 密钥和运行 OAuth 流程的命令。** 从你的终端运行它——不要从活跃的 Hermes 聊天会话内部运行。
...
|------------|-------------|
| `list` | 显示已调度的任务。 |
| `create` / `add` | 根据提示词创建定时任务，可选地通过重复使用 `--skill` 附加一个或多个技能。 |
...
多配置文件、多项目协作看板。每个安装实例可以托管多个看板（每个项目、代码库或领域一个）；每个看板都是一个独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装实例默认有一个名为 `default` 的看板，其数据库位于 `~/.hermes/kanban.db` 以保持向后兼容性；其他看板位于 `~/.hermes/kanban/boards/<slug>/kanban.db`。消息网关内嵌的调度器在每个周期都会扫描每个看板。
...
| `specify <id>` / `specify --all` | 通过辅助 LLM 将待办列中的任务具体化为详细规范（包含目标、方法、验收标准的标题和正文），然后将其提升为 `todo` 状态。标志：`--tenant`（将 `--all` 限制在一个租户内）、`--author`、`--json`。在 `config.yaml` 的 `auxiliary.triage_specifier` 下配置模型。 |
| `gc` | 移除已归档任务的临时工作空间。 |
示例：
...
所有操作也可作为斜杠命令在消息网关中使用（`/kanban …`），具有相同的参数界面——包括 `boards` 子命令和 `--board` 标志。
...
过滤器可以组合使用。当多个过滤器同时激活时，日志行必须通过**所有**过滤器才会被显示：
...
:::info 提供商特定子命令
当外部记忆提供商处于活动状态时，它可能会注册自己的顶级 `hermes <provider>` 命令，用于提供商特定的管理（例如，当 Honcho 激活时，使用 `hermes honcho`）。非活动状态的提供商不会暴露其子命令。运行 `hermes --help` 以查看当前已连接的命令。
:::
...
| `status` | 打印 `cua-driver` 是否在 `$PATH` 中。 |

`hermes computer-use install` 是用于安装 `computer_use` 工具集使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口点。它运行与 `hermes tools` 在你首次启用计算机使用时调用的相同上游安装程序，因此，如果工具集开关没有触发安装（例如，在返回用户设置中），可以安全地使用此命令重新运行安装。
...
将你的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hermes`。自动检测遗留目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件（`clawdbot.json`、`moltbot.json`）。
...
| `--no-backup` | 跳过迁移前对 `~/.hermes/` 的压缩快照（默认情况下，在应用前会将单个还原点存档写入 `~/.hermes/backups/pre-migration-*.zip`；可使用 `hermes import` 恢复）。 |
...
将 shell 自动补全脚本打印到标准输出。在你的 shell 配置文件中加载输出，即可获得 Hermes 命令、子命令和配置文件名称的 Tab 键自动补全功能。
示例：
...
拉取最新的 `hermes-agent` 代码并在你的虚拟环境中重新安装依赖，然后重新运行安装后钩子（MCP 服务器、技能同步、补全安装）。可以在已安装的实例上安全运行。

| 选项 | 描述 |
|--------|-------------|
| `--check` | 并排打印当前提交和最新的 `origin/main` 提交，如果同步则退出码为 0，如果落后则退出码为 1。不执行拉取、安装或重启任何操作。 |
| `--backup` | 在拉取之前，为 `HERMES_HOME`（配置、认证、会话、技能、配对数据）创建一个带标签的更新前快照。默认是**关闭**的——之前总是备份的行为在大型主目录上会给每次更新增加数分钟。可以通过在 `config.yaml` 中设置 `update.backup: true` 来永久开启。 |

...
Hermes 会将端点、提供商和基础 URL 持久化保存在 `config.yaml` 中，因此重启后设置依然有效。如果您的本地服务器恰好加载了一个模型，`/model custom` 会自动检测到它。您也可以在 config.yaml 中设置 `provider: custom` — 它是一个一流的提供商，而不是其他任何东西的别名。

这适用于 Ollama、vLLM、llama.cpp 服务器、SGLang、LocalAI 等。详情请参阅[配置指南](../user-guide/configuration.md)。

:::tip Ollama 用户
如果您在 Ollama 中设置了自定义的 `num_ctx`（例如，`ollama run --num_ctx 64000`），请确保在 Hermes 中设置匹配的上下文长度 — Ollama 的 `/api/show` 报告的是模型的*最大*上下文，而不是您配置的有效 `num_ctx`。
:::
...
Hermes Agent 本身是**免费且开源的**（MIT 许可证）。您只需为您选择的提供商的 LLM API 使用量付费。本地模型完全免费运行。
...
两者在会话间持久保存。详情请参阅[记忆](../user-guide/features/memory.md)和[技能](../user-guide/features/skills.md)。
...
**原因：** Hermes 通过在启动时运行一次 `bash -l` 来构建每个会话的环境快照。Bash 登录 shell 会读取 `/etc/profile`、`~/.bash_profile` 和 `~/.profile`，但**不会 source `~/.bashrc`**——因此安装在那里的工具（`nvm`、`asdf`、`pyenv`、`cargo`、自定义的 `PATH` 导出）对快照不可见。这最常发生在 Hermes 在 systemd 下运行或在最小化 shell 中运行，且没有预先加载交互式 shell 配置文件的情况下。
...
**解决方案：** 退出你的会话，并在终端中使用 `hermes model` 来添加新的提供商：
...
**原因：** 对话内容过长，超出了模型的上下文窗口，或者 Hermes 为你的模型检测到了错误的上下文长度。
**解决方案：**
```bash
# 压缩当前会话
/compress
...
如果这种情况发生在第一次长对话时，Hermes 可能为你的模型检测到了错误的上下文长度。检查它检测到了什么：
...
为了可靠的自动启动，使用 Windows 任务计划程序在登录时启动 WSL + 消息网关：
...
**解决方案：** 当你运行 `hermes gateway install` 时，消息网关会捕获你的 shell PATH。如果你在设置消息网关之后安装了工具，请重新运行安装以捕获更新后的 PATH：
...
**原因：** 大型模型、远程 API 服务器，或者带有许多工具的系统提示词过重。
...
配置文件是构建在 `HERMES_HOME` 之上的一个管理层。你*可以*在每次命令前手动设置 `HERMES_HOME=/some/path`，但配置文件为你处理了所有底层工作：创建目录结构、生成 shell 别名（`hermes-work`）、在 `~/.hermes/active_profile` 中跟踪活动配置文件，以及自动在所有配置文件间同步技能更新。它们还与标签页补全集成，因此你无需记住路径。
...
不共享。每个配置文件都有自己的记忆存储、会话数据库和技能目录。它们是完全隔离的。如果你想用现有的记忆和会话创建一个新的配置文件，请使用 `hermes profile create newname --clone-all` 从当前配置文件复制所有内容。
...
2.  **使用与单个用户的群聊。** 如果指定一个人作为“操作员”来转达问题，会话将保持统一。其他人可以阅读。
...
2.  在**源机器**上，创建完整备份：
    ...
    hermes backup
    ```
    这会创建你整个 `~/.hermes/` 目录的 zip 文件——包括配置、API 密钥、记忆、技能、会话和配置文件——保存到你的主目录，文件名为 `~/hermes-backup-<时间戳>.zip`。

...
- **[Nous Portal](/integrations/nous-portal)** — Nous Research 的订阅网关 — 通过一次 OAuth 登录即可访问 300+ 模型以及网页/图像/TTS/浏览器功能（推荐给新手）
...
# API 密钥：ollama
# 模型名称：qwen3.5:27b
# 上下文长度：64000   ← Hermes 最小值；请根据您服务器的实际上下文窗口设置此值
```
...
Hermes 会将端点、提供商和基础 URL 持久化保存在 `config.yaml` 中，因此重启后仍然有效。如果您的本地服务器只加载了一个模型，`/model custom` 会自动检测它。您也可以在 config.yaml 中设置 `provider: custom` — 它是一个一级提供商，而不是其他任何东西的别名。
...
terminal:
  ...
  # 设置此列表时，默认的 ~/.bashrc 自动 source 行为**不会**被添加——
  # 如果你想要两者，请显式包含它：
  #   - ~/.bashrc
  #   - ~/.zshrc
```
...
通过 `hermes model` 添加新提供商后，启动一个新的聊天会话——现在 `/model` 将显示你所有已配置的提供商。
...
:::tip
这是预期行为——Hermes 永远不会静默执行破坏性命令。批准提示会向你显示将要执行的确切内容。
:::
...
不可以。每个消息平台（Telegram、Discord 等）都需要独占访问一个机器人 Token。如果两个配置文件同时尝试使用相同的 Token，第二个消息网关将无法连接。为每个配置文件创建一个独立的机器人——对于 Telegram，请与 [@BotFather](https://t.me/BotFather) 对话以创建额外的机器人。
...
3.  **使用独立的号码。** 如果你需要真正独立的 Agent，可以为每个配置文件配备自己的 WhatsApp 号码。来自 Google Voice 等服务的虚拟号码可用于此目的。

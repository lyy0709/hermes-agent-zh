...
sidebar_position: 5
title: "内置技能目录"
description: "Hermes Agent 内置的技能目录"
---

# 内置技能目录

Hermes 内置了一个大型技能库，安装时会复制到 `~/.hermes/skills/` 目录。下面的每个技能都链接到一个专用页面，其中包含其完整定义、设置和使用说明。
...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code) | 将编码任务委派给 Claude Code CLI（功能开发、PR）。 | `autonomous-ai-agents/claude-code` |
| [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex) | 将编码任务委派给 OpenAI Codex CLI（功能开发、PR）。 | `autonomous-ai-agents/codex` |
...
| [`opencode`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-opencode) | 将编码任务委派给 OpenCode CLI（功能开发、代码审查）。 | `autonomous-ai-agents/opencode` |

## creative

| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`architecture-diagram`](/docs/user-guide/skills/bundled/creative/creative-architecture-diagram) | 生成暗色主题的 SVG 架构/云/基础设施图，输出为 HTML。 | `creative/architecture-diagram` |
...
| [`baoyu-comic`](/docs/user-guide/skills/bundled/creative/creative-baoyu-comic) | 知识漫画：教育、传记、教程类漫画。 | `creative/baoyu-comic` |
...
| [`comfyui`](/docs/user-guide/skills/bundled/creative/creative-comfyui) | 使用 ComfyUI 生成图像、视频和音频 — 安装、启动、管理节点/模型、通过参数注入运行工作流。使用官方的 comfy-cli 进行生命周期管理，并使用直接的 REST/WebSocket API 进行执行。 | `creative/comfyui` |
...
| [`manim-video`](/docs/user-guide/skills/bundled/creative/creative-manim-video) | Manim CE 动画：生成类似 3Blue1Brown 的数学/算法视频。 | `creative/manim-video` |
...
| [`pretext`](/docs/user-guide/skills/bundled/creative/creative-pretext) | 在使用 @chenglou/pretext 构建创意浏览器演示时使用 — 无 DOM 的文本布局，用于 ASCII 艺术、围绕障碍物的排版流、文本作为几何图形的游戏、动态排版以及文本驱动的生成艺术。生成单文件 HT... | `creative/pretext` |
...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`kanban-orchestrator`](/docs/user-guide/skills/bundled/devops/devops-kanban-orchestrator) | 看板协调器的分解剧本 + 专家名册约定 + 抗诱惑规则，用于通过看板路由工作。"不要自己动手"规则和基本生命周期会自动注入到每个看板工... | `devops/kanban-orchestrator` |
...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`dogfood`](/docs/user-guide/skills/bundled/dogfood/dogfood-dogfood) | Web 应用的探索性 QA：查找错误、证据、报告。 | `dogfood` |

## email

| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`himalaya`](/docs/user-guide/skills/bundled/email/email-himalaya) | Himalaya CLI：在终端中使用 IMAP/SMTP 处理电子邮件。 | `email/himalaya` |
...
| [`pokemon-player`](/docs/user-guide/skills/bundled/gaming/gaming-pokemon-player) | 通过无头模拟器 + RAM 读取来玩 Pokemon。 | `gaming/pokemon-player` |
...
| [`github-code-review`](/docs/user-guide/skills/bundled/github/github-github-code-review) | 审查 PR：通过 gh 或 REST API 处理差异、行内评论。 | `github/github-code-review` |
...
| [`youtube-content`](/docs/user-guide/skills/bundled/media/media-youtube-content) | 将 YouTube 转录内容转换为摘要、主题帖、博客。 | `media/youtube-content` |
...
| [`axolotl`](/docs/user-guide/skills/bundled/mlops/mlops-training-axolotl) | Axolotl：YAML 格式的 LLM 微调（LoRA, DPO, GRPO）。 | `mlops/training/axolotl` |
...
| [`obliteratus`](/docs/user-guide/skills/bundled/mlops/mlops-inference-obliteratus) | OBLITERATUS：消除 LLM 拒绝（diff-in-means）。 | `mlops/inference/obliteratus` |
...
| [`research-paper-writing`](/docs/user-guide/skills/bundled/research/research-research-paper-writing) | 撰写 NeurIPS/ICML/ICLR 的机器学习论文：从设计到提交。 | `research/research-paper-writing` |
...
| [`test-driven-development`](/docs/user-guide/skills/bundled/software-development/software-development-test-driven-development) | TDD：强制执行 RED-GREEN-REFACTOR，先写测试后写代码。 | `software-development/test-driven-development` |

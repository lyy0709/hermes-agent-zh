...
| `/steer <prompt>` | 注入一个在运行中的备注，该备注在**下一次工具调用之后**到达 Agent — 不中断，不创建新的用户轮次。文本在当前工具完成后附加到上一个工具结果的内容中，为 Agent 提供新的上下文而不破坏当前的工具调用循环。用于在任务中途调整方向（例如，在 Agent 运行测试时说“专注于认证模块”）。 |
| `/goal <text>` | 设置一个 Hermes 在多个轮次中持续努力实现的目标 — 我们对 Ralph 循环的实现。每轮之后，一个辅助判断模型会决定目标是否完成；如果没有，Hermes 会自动继续。子命令：`/goal status`、`/goal pause`、`/goal resume`、`/goal clear`。预算默认为 20 轮（`goals.max_turns`）；任何真实的用户消息都会抢占继续循环，并且状态在 `/resume` 后保留。完整指南请参见[持久目标](/docs/user-guide/features/goals)。 |
...
| `/background <prompt>` (别名: `/bg`, `/btw`) | 在单独的后台会话中运行一个提示。Agent 独立处理你的提示 — 你的当前会话保持空闲以进行其他工作。任务完成后，结果会以面板形式显示。请参见[CLI 后台会话](/docs/user-guide/cli#background-sessions)。 |
| `/branch [name]` (别名: `/fork`) | 分支当前会话（探索不同的路径） |
| `/handoff <platform>` | **仅限 CLI。** 将当前会话移交给消息平台（Telegram、Discord、Slack、WhatsApp、Signal、Matrix）。消息网关会立即接收它，在支持线程的平台上创建一个新线程（Telegram 主题、Discord 文本频道线程、Slack 消息锚定线程），将目的地重新绑定到你的 CLI session_id 以便完整回放具有角色意识的转录，并伪造一个合成的用户轮次，以便 Agent 确认它正在新位置工作。成功后你的 CLI 会干净退出，并给出 `/resume` 提示；随时可以使用 `/resume <title>` 在本地恢复。在运行中拒绝。需要消息网关正在运行，并且为目标平台配置了主频道（从目标聊天中使用 `/sethome`）。请参见[跨平台移交](/docs/user-guide/sessions#cross-platform-handoff)。 |
...
| `/codex-runtime [auto\|codex_app_server\|on\|off]` | 切换可选的 [Codex 应用服务器运行时](../user-guide/features/codex-app-server-runtime) 用于 OpenAI/Codex 模型。`auto`（默认）使用 Hermes 的标准聊天补全；`codex_app_server` 将轮次交给 `codex app-server` 子进程，用于原生 shell、apply_patch、ChatGPT 订阅认证以及迁移的 Codex 插件。在下一次会话生效。 |
| `/personality` | 设置预定义的人格 |
| `/verbose` | 循环切换工具进度显示：关闭 → 新 → 全部 → 详细。可以通过配置[为消息平台启用](#notes)。 |
...
| `/busy [queue\|steer\|interrupt\|status]` | 仅限 CLI：控制当 Hermes 正在工作时按 Enter 键的作用 — 将新消息排队、在运行中引导或立即中断。 |
...
| 命令 | 描述 |
|---------|-------------|
| `/quit` | 退出 CLI（也可用：`/exit`）。关于 `/q` 的说明请参见上文的 `/queue`。传递 `--delete`（或 `-d`）— 例如 `/exit --delete` — 在退出前还会永久删除当前会话的 SQLite 历史记录和磁盘上的转录文件。适用于对隐私敏感或一次性任务。 |
...
命令支持前缀匹配：输入 `/h` 会解析为 `/help`，`/mod` 会解析为 `/model`。当前缀存在歧义（匹配多个命令）时，注册顺序中的第一个匹配项胜出。完整的命令名称和已注册的别名始终优先于前缀匹配。
...
| `/background <提示词>` | 在单独的后台会话中运行提示词。任务完成后，结果将发送回同一聊天。参见 [消息平台后台会话](/docs/user-guide/messaging/#background-sessions)。 |
| `/queue <提示词>` (别名：`/q`) | 为下一轮排队一个提示词，而不中断当前轮次。 |
| `/steer <提示词>` | 在下一次工具调用后注入一条消息而不中断 — 模型在其下一次迭代中获取它，而不是作为新的一轮。 |
| `/goal <文本>` | 设置一个 Hermes 在多轮中持续努力实现的目标 — 我们对 Ralph 循环的实现。一个评判模型在每轮后检查；如果未完成，Hermes 会自动继续，直到完成、你暂停/清除它，或者达到轮次预算（默认 20）。子命令：`/goal status`、`/goal pause`、`/goal resume`、`/goal clear`。可以在 Agent 运行中途安全地运行 status/pause/clear；设置新目标需要先执行 `/stop`。参见 [持久目标](/docs/user-guide/features/goals)。 |
| `/footer [on\|off\|status]` | 切换最终回复上的运行时元数据页脚（显示模型、工具计数、计时）。 |
| `/curator [status\|run\|pin\|archive]` | 后台技能维护控制。 |
| `/kanban <动作>` | 从聊天中驱动多配置文件、多项目协作看板 — 参数界面与 CLI 完全相同。绕过正在运行的 Agent 保护，因此 `/kanban unblock t_abc`、`/kanban comment t_abc "…"`、`/kanban list --mine`、`/kanban boards switch <slug>` 等可以在轮次中途工作。`/kanban create …` 会自动将发起聊天的频道订阅到新任务的终端事件。参见 [Kanban 斜杠命令](/docs/user-guide/features/kanban#kanban-slash-command)。 |

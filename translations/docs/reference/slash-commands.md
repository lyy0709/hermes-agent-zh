...
| `/steer <prompt>` | 注入一个在运行中的提示，该提示将在**下一次工具调用之后**到达 Agent — 不会中断，也不会创建新的用户轮次。文本会在当前工具完成后附加到上一个工具结果的内容中，为 Agent 提供新的上下文，而不会破坏当前的工具调用循环。使用此命令可以在任务中途调整方向（例如，在 Agent 运行测试时说“专注于认证模块”）。 |
| `/goal <text>` | 设置一个 Hermes 在多个轮次中持续努力实现的长期目标 — 这是我们对 Ralph 循环的实现。每轮之后，一个辅助判断模型会决定目标是否完成；如果未完成，Hermes 会自动继续。子命令：`/goal status`、`/goal pause`、`/goal resume`、`/goal clear`。预算默认为 20 轮 (`goals.max_turns`)；任何真实的用户消息都会抢占继续循环，并且状态在 `/resume` 后仍然保留。完整指南请参见[持久性目标](/docs/user-guide/features/goals)。 |
...
| `/background <prompt>` (别名: `/bg`, `/btw`) | 在单独的后台会话中运行一个提示词。Agent 独立处理你的提示词 — 你的当前会话可以自由处理其他工作。任务完成后，结果会以面板形式显示。请参见[CLI 后台会话](/docs/user-guide/cli#background-sessions)。 |
| `/branch [name]` (别名: `/fork`) | 分支当前会话（探索不同的路径） |
| `/handoff <platform>` | **仅限 CLI。** 将当前会话移交给消息平台（Telegram、Discord、Slack、WhatsApp、Signal、Matrix）。消息网关会立即接管，在支持线程的平台上创建一个新线程（Telegram 主题、Discord 文本频道线程、Slack 消息锚定线程），将目标重新绑定到你的 CLI session_id 以便完整回放具有角色意识的对话记录，并伪造一个合成的用户轮次，以便 Agent 确认它正在新位置工作。成功后，你的 CLI 会干净退出，并给出 `/resume` 提示；随时可以使用 `/resume <title>` 在本地恢复。在轮次进行中会被拒绝。需要消息网关正在运行，并且为目标平台配置了主频道（在目标聊天中使用 `/sethome` 设置）。请参见[跨平台移交](/docs/user-guide/sessions#cross-platform-handoff)。 |
...
| `/codex-runtime [auto\|codex_app_server\|on\|off]` | 切换可选的 [Codex 应用服务器运行时](../user-guide/features/codex-app-server-runtime) 用于 OpenAI/Codex 模型。`auto`（默认）使用 Hermes 的标准聊天补全；`codex_app_server` 将轮次交给 `codex app-server` 子进程，以使用原生 shell、apply_patch、ChatGPT 订阅认证和迁移的 Codex 插件。在下一次会话生效。 |
| `/personality` | 设置预定义的人格 |
| `/verbose` | 循环切换工具进度显示：关闭 → 仅新 → 全部 → 详细。可以通过配置[为消息启用](#notes)。 |
...
| `/busy [queue\|steer\|interrupt\|status]` | 仅限 CLI：控制当 Hermes 正在工作时按 Enter 键的行为 — 将新消息排队、在轮次中途引导或立即中断。 |
...
| `/paste` | 附加剪贴板中的图像 |
| `/copy [number]` | 将最后一条助手回复复制到剪贴板（或使用数字指定倒数第 N 条回复）。仅限 CLI。 |
| `/image <path>` | 为你的下一个提示词附加本地图像文件。 |
| `/debug` | 上传调试报告（系统信息 + 日志）并获取可分享的链接。在消息平台中也可用。 |
...
为你经常使用的模型定义自己的短名称，然后在 CLI 或任何消息平台中使用 `/model <alias>` 来调用它们。别名在两者中的工作方式相同，适用于仅会话（默认）和 `--global` 切换。
...
用户定义的别名优先于内置的短名称，因此命名一个别名为 `sonnet`、`kimi`、`opus` 等将覆盖内置别名。别名名称不区分大小写。
...
| `/codex-runtime [auto\|codex_app_server\|on\|off]` | 切换可选的 [Codex 应用服务器运行时](../user-guide/features/codex-app-server-runtime)。将更改持久化到 config.yaml 中的 `model.openai_runtime`，并清除缓存的 Agent，以便下一条消息使用新的运行时。在下一次会话中生效。 |
...
| `/usage` | 显示 Token 使用情况、估计的成本明细（输入/输出）、上下文窗口状态、会话时长，以及 — 当活跃提供商支持时 — 一个 **账户限制** 部分，其中包含从提供商 API 实时拉取的剩余配额/积分。 |
...
| `/background <prompt>` | 在单独的后台会话中运行提示词。任务完成后，结果将发送回同一个聊天。参见 [消息平台后台会话](/docs/user-guide/messaging/#background-sessions)。 |
| `/queue <prompt>` (别名：`/q`) | 将提示词排队等待下一轮，而不中断当前轮次。 |
| `/steer <prompt>` | 在下一次工具调用后注入一条消息而不中断 — 模型在其下一次迭代中接收它，而不是作为新的一轮。 |
| `/goal <text>` | 设置一个 Hermes 在多轮中持续努力实现的目标 — 我们对 Ralph 循环的实现。一个评判模型在每轮后检查；如果未完成，Hermes 会自动继续，直到完成、你暂停/清除它，或者达到轮次预算（默认 20）。子命令：`/goal status`、`/goal pause`、`/goal resume`、`/goal clear`。可以在 Agent 运行中途安全地运行以查看状态/暂停/清除；设置新目标需要先执行 `/stop`。参见 [持久目标](/docs/user-guide/features/goals)。 |
| `/footer [on\|off\|status]` | 切换最终回复上的运行时元数据页脚（显示模型、工具计数、时间）。 |
| `/curator [status\|run\|pin\|archive]` | 后台技能维护控制。 |
| `/kanban <action>` | 从聊天中驱动多配置文件、多项目的协作看板 — 参数界面与 CLI 完全相同。绕过正在运行的 Agent 保护，因此 `/kanban unblock t_abc`、`/kanban comment t_abc "…"`、`/kanban list --mine`、`/kanban boards switch <slug>` 等可以在轮次中途工作。`/kanban create …` 会自动将发起聊天的频道订阅到新任务的终端事件。参见 [Kanban 斜杠命令](/docs/user-guide/features/kanban#kanban-slash-command)。 |
...
| `/restart` | 在排空活跃运行后，优雅地重启消息网关。当网关重新上线时，它会向请求者的聊天/线程发送确认信息。 |

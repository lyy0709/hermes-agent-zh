...
| `/steer <prompt>` | 注入一个在运行中的备注，该备注在**下一次工具调用之后**到达 Agent — 不中断，不产生新的用户轮次。文本在当前工具完成后附加到最后一次工具结果的内容中，为 Agent 提供新的上下文，而不会破坏当前的工具调用循环。使用此命令在任务中途微调方向（例如，在 Agent 运行测试时“专注于认证模块”）。 |
...
| `/handoff <platform>` | **仅限 CLI。** 将当前会话移交给消息平台（Telegram、Discord、Slack、WhatsApp、Signal、Matrix）。消息网关会立即接收它，在支持线程的平台上创建一个新线程（Telegram 主题、Discord 文本频道线程、Slack 消息锚定线程），将目的地重新绑定到你的 CLI session_id，以便完整回放具有角色意识的转录，并伪造一个合成的用户轮次，以便 Agent 确认它正在新位置工作。成功后，你的 CLI 会干净退出，并给出 `/resume` 提示；随时可以使用 `/resume <title>` 在本地恢复。在轮次中途拒绝。要求消息网关正在运行，并且为目标平台配置了主频道（从目标聊天中使用 `/sethome`）。请参阅[跨平台移交](/user-guide/sessions#cross-platform-handoff)。 |
### 配置
...
| `/verbose` | 循环切换工具进度显示：关闭 → 新工具 → 全部 → 详细。可通过配置[为消息传递启用](#notes)。 |
...
| `/busy [queue\|steer\|interrupt\|status]` | 仅限 CLI：控制 Hermes 工作时按 Enter 键的作用 — 将新消息排队、在回合中途转向或立即中断。 |
...
|---------|-------------|
| `/help` | 显示此帮助信息 |
| `/usage` | 显示 Token 使用情况、成本明细、会话时长，以及 — 当活动提供商支持时 — 一个**账户限制**部分，其中包含从提供商 API 实时拉取的剩余配额/积分/计划使用情况。 |
| `/insights` | 显示使用洞察和分析（最近 30 天） |
| `/platforms` (别名：`/gateway`) | 显示消息网关/消息传递平台状态（仅限 CLI 的摘要视图）。 |
| `/platform <list\|pause\|resume> [名称]` | 操作正在运行的消息网关平台。`/platform list` 列出每个适配器及其状态（运行中、因断路器暂停、手动暂停）；`/platform pause <名称>` 停止向该适配器分派新消息而不卸载它；`/platform resume <名称>` 重新启用它。当适配器的断路器因重复的可重试故障（网络/速率限制/5xx）而跳闸时，消息网关也会自动暂停该适配器 — 一旦上游恢复健康，使用 `/platform resume <名称>` 清除断路器。在消息网关可访问的任何地方（CLI 会话、Telegram、Discord、…）均可用。 |
| `/paste` | 附加剪贴板图像 |
| `/copy [数字]` | 将最后一个助手响应复制到剪贴板（或使用数字指定倒数第 N 个）。仅限 CLI。 |
...
| 命令 | 描述 |
|---------|-------------|
| `/quit` | 退出 CLI（同义词：`/exit`）。关于 `/q` 的说明请参见上方 `/queue` 部分。传递 `--delete`（或 `-d`）参数——例如 `/exit --delete`——可以在退出前同时永久删除当前会话的 SQLite 历史记录和磁盘上的转录文件。适用于对隐私敏感或一次性任务。 |
...
为您经常使用的模型定义自己的简称，然后在 CLI 或任何消息平台中使用 `/model <alias>` 来调用它们。别名在两者中的工作方式相同，适用于仅会话（默认）和 `--global` 开关。

支持两种配置格式：

**完整格式** — 固定一个确切的模型、提供商，以及可选的基 URL。将此内容放入 `~/.hermes/config.yaml`：
...
用户定义的别名优先于内置的简称，因此将别名命名为 `sonnet`、`kimi`、`opus` 等会覆盖内置名称。别名名称不区分大小写。

### 别名解析

命令支持前缀匹配：键入 `/h` 解析为 `/help`，键入 `/mod` 解析为 `/model`。当前缀存在歧义（匹配多个命令）时，注册顺序中的第一个匹配项胜出。完整的命令名称和已注册的别名始终优先于前缀匹配。
...
| 命令 | 描述 |
|---------|-------------|
| `/start` | 平台协议命令。许多聊天平台（Telegram、Discord 等）在用户首次打开机器人对话时会自动发送 `/start`。Hermes 会静默确认此 ping——不回复 Agent，不消耗会话轮次——因此首次接触握手不会浪费轮次。您也可以显式发送它以确认网关可达。 |
| `/new` | 开始新的对话。 |
| `/reset` | 重置对话历史。 |
...
| `/codex-runtime [auto\|codex_app_server\|on\|off]` | 切换可选的 [Codex 应用服务器运行时](../user-guide/features/codex-app-server-runtime)。持久化到 config.yaml 中的 `model.openai_runtime` 并清除缓存的 Agent，以便下一条消息获取新的运行时。在下一次会话生效。 |
...
| `/compress [here [N] \| focus topic]` | 手动压缩对话上下文。`/compress here [N]` 保留最近 N 次交互（默认为 2）的原文，并总结其余部分。焦点主题可以缩小完整摘要保留的范围。 |
...
| `/background <prompt>` | 在单独的后台会话中运行提示词。任务完成后，结果会发送回同一个聊天。参见[消息后台会话](/user-guide/messaging/#background-sessions)。 |
| `/queue <prompt>` (别名: `/q`) | 将提示词排队等待下一轮次，而不中断当前轮次。 |
| `/steer <prompt>` | 在下一次工具调用后注入一条消息而不中断——模型会在下一次迭代时处理它，而不是作为新轮次。 |
| `/goal <text>` | 设置一个 Hermes 在多个轮次中持续努力实现的目标——我们对 Ralph 循环的实现。一个评判模型在每次轮次后检查；如果未完成，Hermes 会自动继续，直到完成、您暂停/清除它，或达到轮次预算（默认为 20）。子命令：`/goal status`、`/goal pause`、`/goal resume`、`/goal clear`。在 Agent 运行期间可以安全地运行状态/暂停/清除；设置新目标需要先执行 `/stop`。参见[持久目标](/user-guide/features/goals)。 |
| `/footer [on\|off\|status]` | 切换最终回复上的运行时元数据页脚（显示模型、工具计数、时间）。 |
| `/curator [status\|run\|pin\|archive]` | 后台技能维护控制。 |
| `/kanban <action>` | 从聊天中驱动多配置文件、多项目的协作看板——参数界面与 CLI 完全相同。绕过运行中 Agent 的防护，因此 `/kanban unblock t_abc`、`/kanban comment t_abc "…"`、`/kanban list --mine`、`/kanban boards switch <slug>` 等可以在轮次中途工作。`/kanban create …` 会自动将发起聊天的频道订阅到新任务的终端事件。参见[看板斜杠命令](/user-guide/features/kanban#kanban-slash-command)。 |
| `/reload-mcp` (别名: `/reload_mcp`) | 从配置重新加载 MCP 服务器。 |
| `/yolo` | 切换 YOLO 模式——跳过所有危险命令的批准提示。 |
| `/commands [page]` | 浏览所有命令和技能（分页）。 |
| `/approve [session\|always]` | 批准并执行一个待处理的危险命令。`session` 仅批准当前会话；`always` 添加到永久允许列表。 |
| `/deny` | 拒绝一个待处理的危险命令。 |
...
对于这些命令，CLI 会打开一个三选一的模态框：**Approve Once**（本次执行）、**Always Approve**（执行并持久化 `approvals.destructive_slash_confirm: false`，以便未来的破坏性命令无需提示即可运行）或 **Cancel**。
...
在 `~/.hermes/config.yaml` 中设置 `approvals.destructive_slash_confirm: false` 以全局禁用提示；将其设置回 `true` 以重新启用。相关背景请参见 [安全 — 破坏性斜杠命令确认](../user-guide/security.md#dangerous-command-approval)。
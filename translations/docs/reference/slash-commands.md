...
| `/snapshot [create\|restore <id>\|prune]` (别名: `/snap`) | 创建或恢复 Hermes 配置/状态的状态快照。`create [label]` 保存快照，`restore <id>` 恢复到该快照，`prune [N]` 删除旧快照，或无参数时列出所有快照。 |
| `/stop` | 终止所有正在运行的后台进程 |
| `/queue <prompt>` (别名: `/q`) | 为下一轮排队一个提示（不中断当前 Agent 响应）。 |
| `/steer <prompt>` | 注入一个在运行中的备注，该备注**在下一次工具调用之后**到达 Agent — 不中断，不创建新的用户轮次。文本在当前工具完成后附加到上一个工具结果的内容中，为 Agent 提供新的上下文而不破坏当前的工具调用循环。使用此命令在任务中途微调方向（例如，在 Agent 运行测试时说“专注于认证模块”）。 |
| `/goal <text>` | 设置一个 Hermes 在多个轮次中持续努力实现的长期目标 — 我们对 Ralph 循环的实现。每轮之后，一个辅助判断模型会决定目标是否完成；如果未完成，Hermes 会自动继续。子命令：`/goal status`、`/goal pause`、`/goal resume`、`/goal clear`。预算默认为 20 轮（`goals.max_turns`）；任何真实的用户消息都会抢占继续循环，并且状态在 `/resume` 后保留。完整指南请参见[持久目标](/docs/user-guide/features/goals)。 |
...
| `/background <prompt>` (别名: `/bg`, `/btw`) | 在单独的后台会话中运行一个提示。Agent 独立处理你的提示 — 你的当前会话保持空闲以进行其他工作。任务完成后，结果会以面板形式显示。参见 [CLI 后台会话](/docs/user-guide/cli#background-sessions)。 |
| `/branch [name]` (别名: `/fork`) | 分支当前会话（探索不同的路径） |
| `/handoff <platform>` | **仅限 CLI。** 将当前会话移交给消息平台（Telegram、Discord、Slack、WhatsApp、Signal、Matrix）。消息网关会立即接管，在支持线程的平台上创建一个新线程（Telegram 主题、Discord 文本频道线程、Slack 消息锚定线程），将目标重新绑定到你的 CLI session_id 以便完整回放具有角色意识的对话记录，并伪造一个合成的用户轮次，以便 Agent 确认它正在新位置工作。成功后你的 CLI 会干净退出，并给出 `/resume` 提示；随时可以使用 `/resume <title>` 在本地恢复。在运行中拒绝。需要消息网关正在运行，并且为目标平台配置了主频道（从目标聊天中使用 `/sethome`）。参见[跨平台移交](/docs/user-guide/sessions#cross-platform-handoff)。 |
...
| `/codex-runtime [auto\|codex_app_server\|on\|off]` | 切换可选的 [Codex 应用服务器运行时](../user-guide/features/codex-app-server-runtime) 用于 OpenAI/Codex 模型。`auto`（默认）使用 Hermes 的标准聊天补全；`codex_app_server` 将轮次交给 `codex app-server` 子进程，用于原生 shell、apply_patch、ChatGPT 订阅认证和迁移的 Codex 插件。在下一次会话生效。 |
...
| `/busy [queue\|steer\|interrupt\|status]` | 仅限 CLI：控制当 Hermes 正在工作时按 Enter 键的作用 — 将新消息排队、在运行中微调或立即中断。 |
...
| `/paste` | 附加剪贴板中的图像 |
| `/copy [number]` | 将最后一条助手响应复制到剪贴板（或使用数字指定倒数第 N 条）。仅限 CLI。 |
...
| 命令 | 说明 |
|---------|-------------|
| `/quit` | 退出 CLI（也可用：`/exit`）。关于 `/q` 的说明请参见上文的 `/queue`。传递 `--delete`（或 `-d`）— 例如 `/exit --delete` — 可以在退出前同时永久删除当前会话的 SQLite 历史记录和磁盘上的转录文件。适用于对隐私敏感或一次性任务。 |
...
对于专业的 Telegram 机器人，设置 `telegram.command_menu: quick_commands_only` 以使 Telegram 的原生斜杠菜单仅显示配置文件定义的快捷命令。
在快捷命令上设置 `show_in_telegram_menu: false` 可以使其保持可调用状态，但将其从 Telegram 菜单中隐藏。

纯字符串的提示词快捷方式不支持作为快捷命令。将较长的可重用提示词放在技能中，或使用 `type: alias` 指向现有的斜杠命令。

### 自定义模型别名

为你经常使用的模型定义自己的短名称，然后在 CLI 或任何消息平台中使用 `/model <alias>` 来调用它们。别名在会话级别（默认）和 `--global` 切换时，在两者中的工作方式完全相同。
...
**简短格式** — 在一个字符串中指定 `provider/model`。无需编辑 YAML，从 shell 设置：
...
用户定义的别名优先于内置的短名称，因此命名一个别名为 `sonnet`、`kimi`、`opus` 等会覆盖内置的。别名名称不区分大小写。

### 别名解析

命令支持前缀匹配：输入 `/h` 解析为 `/help`，输入 `/mod` 解析为 `/model`。当前缀存在歧义（匹配多个命令）时，注册顺序中的第一个匹配项胜出。完整的命令名称和已注册的别名始终优先于前缀匹配。
## 消息斜杠命令
...
| `/model [provider:model]` | 显示或更改模型。支持提供商切换（`/model zai:glm-5`）、自定义端点（`/model custom:model`）、命名的自定义提供商（`/model custom:local:qwen`）、自动检测（`/model custom`）和用户定义的别名（`/model fav`，`/model grok` — 参见[自定义模型别名](#custom-model-aliases)）。使用 `--global` 将更改持久化到 config.yaml。**注意：** `/model` 只能在已配置的提供商之间切换。要添加新提供商或设置 API 密钥，请在终端（聊天会话之外）使用 `hermes model`。 |
| `/codex-runtime [auto\|codex_app_server\|on\|off]` | 切换可选的 [Codex 应用服务器运行时](../user-guide/features/codex-app-server-runtime)。将更改持久化到 config.yaml 中的 `model.openai_runtime`，并清除缓存的 Agent，以便下一条消息使用新的运行时。在下一个会话中生效。 |
| `/personality [name]` | 为会话设置一个灵魂（人格）覆盖层。 |
| `/fast [normal\|fast\|status]` | 切换快速模式 — OpenAI 优先处理 / Anthropic 快速模式。 |
| `/retry` | 重试上一条消息。 |
| `/undo` | 移除最后一次交互。 |
| `/sethome` (别名: `/set-home`) | 将当前聊天标记为平台的主频道，用于交付。 |
| `/compress [focus topic]` | 手动压缩会话上下文。可选的重点主题可以缩小摘要保留的范围。 |
...
| `/usage` | 显示 Token 使用情况、估计的成本明细（输入/输出）、上下文窗口状态、会话持续时间，以及 — 当活跃提供商支持时 — 一个**账户限制**部分，其中包含从提供商 API 实时获取的剩余配额/额度。 |
...
| `/background <prompt>` | 在单独的后台会话中运行一个提示词。任务完成后，结果将发送回同一个聊天。参见[消息后台会话](/docs/user-guide/messaging/#background-sessions)。 |
| `/queue <prompt>` (别名: `/q`) | 将提示词排队等待下一轮，而不中断当前轮次。 |
| `/steer <prompt>` | 在下一次工具调用后注入一条消息而不中断 — 模型会在下一次迭代中处理它，而不是作为一个新轮次。 |
| `/goal <text>` | 设置一个 Hermes 在多个轮次中持续努力实现的目标 — 这是我们对 Ralph 循环的实现。一个评判模型会在每轮之后检查；如果未完成，Hermes 会自动继续，直到完成、你暂停/清除它，或者达到轮次预算（默认 20）。子命令：`/goal status`，`/goal pause`，`/goal resume`，`/goal clear`。在 Agent 运行期间可以安全运行 status/pause/clear；设置新目标需要先执行 `/stop`。参见[持久目标](/docs/user-guide/features/goals)。 |
| `/footer [on\|off\|status]` | 切换最终回复上的运行时元数据页脚（显示模型、工具计数、计时）。 |
| `/curator [status\|run\|pin\|archive]` | 后台技能维护控制。 |
| `/kanban <action>` | 从聊天驱动多配置文件、多项目的协作看板 — 参数界面与 CLI 完全相同。绕过运行中 Agent 的防护，因此 `/kanban unblock t_abc`、`/kanban comment t_abc "…"`、`/kanban list --mine`、`/kanban boards switch <slug>` 等可以在轮次中间工作。`/kanban create …` 会自动将发起聊天的频道订阅到新任务的终端事件。参见[看板斜杠命令](/docs/user-guide/features/kanban#kanban-slash-command)。 |
| `/reload-mcp` (别名: `/reload_mcp`) | 从配置重新加载 MCP 服务器。 |
| `/yolo` | 切换 YOLO 模式 — 跳过所有危险命令的批准提示。 |
| `/commands [page]` | 浏览所有命令和技能（分页）。 |
| `/approve [session\|always]` | 批准并执行一个待处理的危险命令。`session` 仅批准此会话；`always` 添加到永久允许列表。 |
| `/deny` | 拒绝一个待处理的危险命令。 |
...
- `/status`、`/background`、`/queue`、`/steer`、`/voice`、`/reload-mcp`、`/reload-skills`、`/rollback`、`/debug`、`/fast`、`/footer`、`/curator`、`/kanban`、`/sessions` 和 `/yolo` 在 **CLI 和消息网关**中都有效。

...
| `/stop` | 终止所有正在运行的后台进程 |
| `/queue <prompt>` (别名: `/q`) | 为下一轮排队一个提示词（不中断当前 Agent 响应）。**注意：** `/q` 同时被 `/queue` 和 `/quit` 声明；最后注册的命令生效，因此实践中 `/q` 解析为 `/quit`。请明确使用 `/queue`。 |
| `/resume [name]` | 恢复一个先前命名的会话 |
| `/statusbar` (别名: `/sb`) | 切换上下文/模型状态栏的开关 |
| `/background <prompt>` (别名: `/bg`) | 在单独的后台会话中运行一个提示词。Agent 独立处理你的提示词——你当前的会话可以自由处理其他工作。任务完成后，结果会以面板形式显示。参见 [CLI 后台会话](/docs/user-guide/cli#background-sessions)。 |
...
| `/verbose` | 循环切换工具进度显示：关闭 → 仅新工具 → 所有工具 → 详细模式。可以通过配置[为消息平台启用](#notes)。 |
...
| `/resume [name]` | 恢复一个先前命名的会话。 |
| `/usage` | 显示 Token 使用情况、估计的成本细分（输入/输出）、上下文窗口状态和会话持续时间。 |

...
| `/stop` | 终止所有正在运行的后台进程 |
| `/queue <prompt>` (别名: `/q`) | 为下一轮排队一个提示词（不会中断当前的 Agent 响应）。 |
| `/steer <prompt>` | 注入一个运行中的备注，该备注**在下一次工具调用之后**到达 Agent — 不中断，不创建新的用户轮次。文本在当前工具完成后附加到最后一次工具结果的内容中，为 Agent 提供新的上下文，而不会中断当前的工具调用循环。使用此命令可以在任务中途调整方向（例如，当 Agent 正在运行测试时，“专注于认证模块”）。 |
| `/goal <text>` | 设置一个 Hermes 在多个轮次中持续努力实现的目标 — 我们对 Ralph 循环的实现。每轮之后，一个辅助判断模型会决定目标是否完成；如果没有，Hermes 会自动继续。子命令：`/goal status`、`/goal pause`、`/goal resume`、`/goal clear`。预算默认为 20 轮（`goals.max_turns`）；任何真实的用户消息都会抢占继续循环，并且状态在 `/resume` 后保留。完整指南请参阅[持久目标](/docs/user-guide/features/goals)。 |
| `/resume [name]` | 恢复一个先前命名的会话 |
| `/redraw` | 强制完全重绘 UI（在 tmux 调整大小、鼠标选择伪影等之后恢复终端显示） |
| `/status` | 显示会话信息 |
| `/agents` (别名: `/tasks`) | 显示当前会话中活跃的 Agent 和正在运行的任务。 |
| `/background <prompt>` (别名: `/bg`, `/btw`) | 在单独的后台会话中运行一个提示词。Agent 独立处理您的提示词 — 您的当前会话可以自由处理其他工作。任务完成后，结果会显示为一个面板。请参阅 [CLI 后台会话](/docs/user-guide/cli#background-sessions)。 |
...
| `/busy [queue\|steer\|interrupt\|status]` | 仅限 CLI：控制 Hermes 工作时按 Enter 键的行为 — 将新消息排队、在轮次中途转向或立即中断。 |
...
命令支持前缀匹配：输入 `/h` 会解析为 `/help`，输入 `/mod` 会解析为 `/model`。当前缀有歧义（匹配多个命令）时，注册表顺序中的第一个匹配项胜出。完整的命令名称和已注册的别名始终优先于前缀匹配。
...
| `/background <提示词>` | 在单独的后台会话中运行提示词。任务完成后，结果会传回同一聊天。参见[消息平台后台会话](/docs/user-guide/messaging/#background-sessions)。 |
| `/queue <提示词>` (别名: `/q`) | 将提示词排队等待下一轮，而不中断当前轮次。 |
| `/steer <提示词>` | 在下一次工具调用后注入一条消息而不中断 — 模型会在下一次迭代中处理它，而不是作为新的一轮。 |
| `/goal <文本>` | 设置一个 Hermes 在多个轮次中持续努力实现的目标 — 我们对 Ralph 循环的实现。一个评判模型在每轮之后检查；如果未完成，Hermes 会自动继续，直到完成、你暂停/清除它，或者达到轮次预算（默认 20）。子命令：`/goal status`、`/goal pause`、`/goal resume`、`/goal clear`。在 Agent 运行期间可以安全地运行状态/暂停/清除；设置新目标需要先执行 `/stop`。参见[持久化目标](/docs/user-guide/features/goals)。 |
| `/footer [on\|off\|status]` | 切换最终回复上的运行时元数据页脚（显示模型、工具计数、计时）。 |
| `/curator [status\|run\|pin\|archive]` | 后台技能维护控制。 |
| `/kanban <操作>` | 从聊天中驱动多配置、多项目的协作看板 — 参数界面与 CLI 相同。绕过运行中 Agent 的防护，因此 `/kanban unblock t_abc`、`/kanban comment t_abc "…"`、`/kanban list --mine`、`/kanban boards switch <slug>` 等可以在轮次中间工作。`/kanban create …` 会自动将发起聊天的频道订阅到新任务的终端事件。参见[看板斜杠命令](/docs/user-guide/features/kanban#kanban-slash-command)。 |
...
| `/restart` | 在排空正在运行的会话后，优雅地重启消息网关。当消息网关重新上线时，它会向请求者的聊天/线程发送确认信息。 |

...
| `/compress [focus topic]` | 手动压缩对话上下文（刷新记忆 + 摘要）。可选焦点主题可缩小摘要保留的范围。 |
...
| `/personality` | 设置预定义人格 |
| `/verbose` | 循环切换工具进度显示：关闭 → 新工具 → 所有工具 → 详细模式。可通过配置[为消息平台启用](#notes)。 |
...
用户定义的快捷命令将短斜杠命令映射到 shell 命令或其他斜杠命令。在 `~/.hermes/config.yaml` 中配置它们：
...
然后在 CLI 或消息平台中输入 `/status`、`/deploy` 或 `/inbox`。快捷命令在分发时解析，可能不会出现在每个内置的自动补全/帮助表中。

纯字符串的提示词快捷方式不支持作为快捷命令。将较长的可重用提示词放在技能中，或使用 `type: alias` 指向现有的斜杠命令。

### 别名解析

命令支持前缀匹配：输入 `/h` 解析为 `/help`，`/mod` 解析为 `/model`。当前缀不明确（匹配多个命令）时，注册表顺序中的第一个匹配项胜出。完整的命令名称和已注册的别名始终优先于前缀匹配。
...
| `/model [provider:model]` | 显示或更改模型。支持提供商切换（`/model zai:glm-5`）、自定义端点（`/model custom:model`）、命名的自定义提供商（`/model custom:local:qwen`）和自动检测（`/model custom`）。使用 `--global` 将更改持久化到 config.yaml。**注意：** `/model` 只能在已配置的提供商之间切换。要添加新提供商或设置 API 密钥，请从终端（在聊天会话之外）使用 `hermes model`。 |
...
| `/compress [focus topic]` | 手动压缩对话上下文。可选焦点主题可缩小摘要保留的范围。 |
| `/title [name]` | 设置或显示会话标题。 |
| `/resume [name]` | 恢复一个先前命名的会话。 |
| `/usage` | 显示 Token 使用情况、估计成本细分（输入/输出）、上下文窗口状态和会话持续时间。 |
...
| `/background <prompt>` | 在单独的后台会话中运行一个提示词。任务完成后，结果会发送回同一聊天。参见 [消息平台后台会话](/docs/user-guide/messaging/#background-sessions)。 |

...
安装后，该技能会出现在 Agent 的技能列表中，并可在检测到相关任务时自动加载。
...
| **concept-diagrams** | 生成扁平、极简、支持明暗模式的 SVG 图表作为独立的 HTML 文件，使用统一的教育视觉语言（9 种语义色彩渐变，自动深色模式）。最适合物理装置、化学反应机制、数学曲线、物理对象（飞机、涡轮机、智能手机）、平面图、横截面、生命周期/流程叙述以及中心辐射型系统图。附带 15 个示例图表。 |
| **meme-generation** | 通过选择模板并使用 Pillow 叠加文本来生成真实的梗图。生成实际的 `.png` 梗图文件。 |
| **touchdesigner-mcp** | 通过 twozero MCP 插件控制正在运行的 TouchDesigner 实例 — 创建操作器、设置参数、连接线路、执行 Python、构建实时音频响应视觉效果和 GLSL 网络。包含 36 个原生工具。 |

## 内部测试

| 技能 | 描述 |
|-------|-------------|
| **adversarial-ux-test** | 扮演产品中最难缠、最抗拒技术的用户角色 — 以该角色浏览、抱怨，然后通过 RED/YELLOW/WHITE/GREEN 实用主义层进行过滤，以便只有真正的用户体验摩擦才会成为工单。 |
...
| **mcporter** | `mcporter` CLI — 直接从终端列出、配置、认证和调用 MCP 服务器/工具（HTTP 或 stdio）。适用于临时 MCP 交互；对于常驻工具发现，请改用内置的 `native-mcp` 客户端。 |
...
| **pytorch-lightning** | 具有 Trainer 类的高级 PyTorch 框架，支持自动分布式训练（DDP/FSDP/DeepSpeed）、回调函数和最少的样板代码。 |
...
| **slime** | 使用 Megatron+SGLang 框架对 LLM 进行 RL 后训练。自定义数据生成工作流和紧密的 Megatron-LM 集成以实现 RL 扩展。 |
...
| **torchtitan** | 使用 4D 并行（FSDP2、TP、PP、CP）进行 PyTorch 原生分布式 LLM 预训练。从 8 个扩展到 512+ 个 GPU，支持 Float8 和 torch.compile。 |
| **whisper** | OpenAI 的通用语音识别。支持 99 种语言，提供转录、翻译成英语和语言识别功能。六种模型大小，从 tiny（3900 万）到 large（15.5 亿）。最适合稳健的多语言 ASR。 |
## 生产力
...
| **qmd** | 使用 qmd 在本地搜索个人知识库、笔记、文档和会议记录 — 一个结合了 BM25、向量搜索和 LLM 重排序的混合检索引擎。 |
| **scrapling** | 使用 Scrapling 进行网络爬取 — HTTP 抓取、隐身浏览器自动化、Cloudflare 绕过，以及通过 CLI 和 Python 进行蜘蛛爬取。 |

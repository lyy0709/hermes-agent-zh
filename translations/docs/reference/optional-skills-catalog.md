...
| **concept-diagrams** | 生成扁平、极简、支持明暗模式的 SVG 图表，作为独立的 HTML 文件，使用统一的教育视觉语言（9 种语义色彩渐变，自动深色模式）。最适合物理装置、化学机理、数学曲线、物理对象（飞机、涡轮机、智能手机）、平面图、横截面、生命周期/流程叙述以及中心辐射型系统图。附带 15 个示例图表。 |
| **meme-generation** | 通过选择模板并使用 Pillow 叠加文本来生成真实的梗图。生成实际的 `.png` 梗图文件。 |
| **touchdesigner-mcp** | 通过 twozero MCP 插件控制正在运行的 TouchDesigner 实例 — 创建操作器、设置参数、连接线路、执行 Python、构建实时音频响应式视觉效果和 GLSL 网络。包含 36 个原生工具。 |

## DevOps

| 技能 | 描述 |
|-------|-------------|
| **cli** | 通过 inference.sh CLI (infsh) 运行 150+ 个 AI 应用 — 图像生成、视频创作、LLM、搜索、3D 和社交自动化。 |
...
| 技能 | 描述 |
|-------|-------------|
| **fitness-nutrition** | 健身房锻炼计划制定和营养追踪器。通过 wger 按肌肉、设备或类别搜索 690+ 种练习。通过 USDA FoodData Central 查询 380,000+ 种食物的宏量营养素和卡路里。计算 BMI、TDEE、单次最大重复次数、宏量营养素分配和体脂率 — 纯 Python，无需 pip 安装。 |
...
| **mcporter** | `mcporter` CLI — 直接从终端列出、配置、授权和调用 MCP 服务器/工具（HTTP 或 stdio）。适用于临时 MCP 交互；对于常驻工具发现，请改用内置的 `native-mcp` 客户端。 |
...
| **hermes-atropos-environments** | 为 Atropos 训练构建、测试和调试 Hermes Agent RL 执行环境。涵盖 HermesAgentBaseEnv 接口、奖励函数、Agent 循环集成和评估。 |
...
| **nemo-curator** | 用于 LLM 训练的 GPU 加速数据整理。模糊去重（快 16 倍）、质量过滤（30+ 种启发式方法）、语义去重、PII 编辑。通过 RAPIDS 扩展。 |
| **peft-fine-tuning** | 使用 LoRA、QLoRA 和 25+ 种方法对 LLM 进行参数高效微调。在有限的 GPU 内存上，以最小的精度损失训练 7B–70B 模型中 <1% 的参数。HuggingFace 官方 PEFT 库。 |
...
| **slime** | 使用 Megatron+SGLang 框架通过 RL 对 LLM 进行后训练。自定义数据生成工作流和紧密的 Megatron-LM 集成，用于 RL 扩展。 |
...
| **torchtitan** | 具有 4D 并行性（FSDP2、TP、PP、CP）的 PyTorch 原生分布式 LLM 预训练。从 8 个扩展到 512+ 个 GPU，支持 Float8 和 torch.compile。 |
| **whisper** | OpenAI 的通用语音识别。支持 99 种语言，包括转录、翻译成英语和语言识别。六种模型大小，从 tiny（3900 万参数）到 large（15.5 亿参数）。最适合稳健的多语言 ASR。 |
## 生产力
...
| **parallel-cli** | Parallel CLI 的供应商技能 — 面向 Agent 的原生网络搜索、提取、深度研究、丰富和监控。 |
| **qmd** | 使用 qmd 在本地搜索个人知识库、笔记、文档和会议记录 — 一个结合了 BM25、向量搜索和 LLM 重排的混合检索引擎。 |
| **scrapling** | 使用 Scrapling 进行网络爬取 — HTTP 获取、隐身浏览器自动化、Cloudflare 绕过，以及通过 CLI 和 Python 进行蜘蛛爬取。 |

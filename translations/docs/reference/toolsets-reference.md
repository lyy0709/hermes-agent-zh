...
| `debugging` | 组合工具集 (`file` + `terminal` + `web`) | 调试捆绑包 — 文件、进程/终端、网络提取/搜索。 |
| `delegation` | `delegate_task` | 生成隔离的子 Agent 实例以进行并行工作。 |
| `discord` | `discord` | 核心 Discord 文本/嵌入/私信操作（仅限消息网关）。在 `hermes-discord` 工具集上激活。 |
| `discord_admin` | `discord_admin` | Discord 管理（封禁、角色变更、频道管理）。在 `hermes-discord` 工具集上激活；要求机器人持有相关的 Discord 权限。 |
...
| `context_engine` | (可变) | 由活动的上下文引擎插件暴露的运行时工具（在插件填充之前为空）。 |
| `image_gen` | `image_generate` | 通过 FAL.ai 进行文本到图像生成（可选择 OpenAI / xAI 后端）。 |
| `video_gen` | `video_generate` | 通过插件注册的后端进行文本到视频和图像到视频生成（xAI Grok-Imagine、FAL.ai Veo 3.1 / Pixverse v6 / Kling O3）。传递 `image_url` 以动画化图像；省略则进行文本到视频生成。 |
| `kanban` | `kanban_block`, `kanban_comment`, `kanban_complete`, `kanban_create`, `kanban_heartbeat`, `kanban_link`, `kanban_list`, `kanban_show`, `kanban_unblock` | 多 Agent 协调工具。为调度器生成的任务工作器 (`HERMES_KANBAN_TASK`) 以及显式按名称列出 `kanban` 工具集的配置文件注册（`all`/`*` 通配符**不会**启用它）。工作器标记任务完成、阻塞、心跳、评论以及创建/链接后续任务；编排器配置文件额外获得诸如列表/解除阻塞等看板路由工具。 |
...
| `video` | `video_analyze` | 视频分析和理解工具（需手动启用，不在默认工具集中 — 通过 `--toolsets` 显式添加）。 |
| `web` | `web_extract`, `web_search` | 网络搜索和页面内容提取。 |
| `x_search` | `x_search` | 通过 xAI 内置的 `x_search` Responses 工具搜索 X（Twitter）帖子和线程。默认关闭；通过 `hermes tools` 手动启用。仅在配置了 xAI 凭据（SuperGrok OAuth 或 `XAI_API_KEY`）时注册模式。 |
...
|---------|-------------------------------|
| `hermes-cli` | 完整工具集 — 交互式 CLI 会话的默认配置。包含文件、终端、网络、浏览器、记忆、技能、视觉、图像生成、待办事项、文本转语音、委派、代码执行、定时任务、会话搜索、澄清以及 `safe`（只读）捆绑包，外加标准消息工具。 |
...
每个配置的 MCP 服务器在运行时都会生成一个 `mcp-<服务器>` 工具集。例如，如果配置了一个 `github` MCP 服务器，则会创建一个包含该服务器暴露的所有工具的 `mcp-github` 工具集。
...
少数工具在工具集成员资格之上还有额外的可用性检查，并且**不会**仅因 `all`/`*` 而启用：

- **能力受控**的工具（浏览器、`computer_use`、`code_execution`、飞书、Home Assistant、定时任务）仅在其后端/凭据先决条件配置完成时出现。
- **工作流受控**的工具 — `kanban` 工具集 — 是刻意选择启用的。`all`/`*` **不会**启用看板；您必须显式列出 `kanban`（或者是设置了 `HERMES_KANBAN_TASK` 的调度器生成的工作器）。看板工具会改变共享看板状态，因此即使在 `all` 下也默认保持关闭。

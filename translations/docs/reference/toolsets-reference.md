...
| `debugging` | 组合工具集 (`file` + `terminal` + `web`) | 调试捆绑包 — 文件、进程/终端、网页提取/搜索。 |
...
| `kanban` | `kanban_block`, `kanban_comment`, `kanban_complete`, `kanban_create`, `kanban_heartbeat`, `kanban_link`, `kanban_list`, `kanban_show`, `kanban_unblock` | 多 Agent 协调工具。为调度器生成的任务工作器（`HERMES_KANBAN_TASK`）以及显式按名称列出 `kanban` 工具集的配置文件注册（`all`/`*` 通配符**不会**启用它）。工作器标记任务完成、阻塞、心跳、评论以及创建/链接后续任务；编排器配置文件额外获得列表/解除阻塞等看板路由工具。 |
...
少数工具在工具集成员资格之外还有额外的可用性检查，并且**不会**仅因 `all`/`*` 而启用：

- **能力受控**的工具（浏览器、`computer_use`、`code_execution`、飞书、Home Assistant、定时任务）仅在其后端/凭据先决条件配置完成时出现。
- **工作流受控**的工具 — `kanban` 工具集 — 是特意选择加入的。`all`/`*` **不会**启用看板；您必须显式列出 `kanban`（或者是设置了 `HERMES_KANBAN_TASK` 的调度器生成的工作器）。看板工具会改变共享看板状态，因此即使在 `all` 下也默认保持关闭。

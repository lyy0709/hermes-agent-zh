...
| `--no-alias` | 跳过包装脚本的创建。 |
| `--description "<文本>"` | 描述此配置文件擅长什么的一两句话。看板编排器使用此描述来根据角色而非仅根据配置文件名称来路由任务。可以跳过，稍后通过 `hermes profile describe` 添加。保存在 `<profile_dir>/profile.yaml` 中。 |
| `--no-skills` | 创建一个**空**配置文件，不启用任何捆绑技能。在配置文件中写入一个 `.no-skills` 标记，以便未来的 `hermes update` 运行不会重新填充捆绑集，并且拒绝与 `--clone` / `--clone-all` 结合使用（因为后者无论如何都会复制技能）。适用于不应继承完整技能目录的窄编排器配置文件或沙盒配置文件。 |

创建配置文件**不会**使该配置文件目录成为终端命令的默认项目/工作空间目录。如果您希望配置文件在特定项目中启动，请在该配置文件的 `config.yaml` 中设置 `terminal.cwd`。
...
读取或设置配置文件的描述。看板编排器使用此描述来根据每个配置文件擅长什么来路由任务，而不是仅根据配置文件名称猜测。保存在 `<profile_dir>/profile.yaml` 中，因此描述在重启后仍然存在，并与消息网关共享。
...
| `--auto` | 基于配置文件已安装的技能、配置的模型和名称，通过辅助 LLM 自动生成一两句描述。在 `config.yaml` 的 `auxiliary.profile_describer` 下配置模型。自动生成的描述标记为 `description_auto: true`，以便仪表板可以标记它们以供审查。 |
...
这里显示的是配置文件的 Hermes 主目录，而非终端工作目录。终端命令从 `terminal.cwd` 开始执行（或在本地后端使用 `cwd: "."` 时从启动目录开始）。
...
在 `~/.local/bin/<name>` 处重新生成 shell 别名脚本。如果别名被意外删除，或者在移动 Hermes 安装位置后需要更新，此命令很有用。
...
| `--name <name>` | 导入的配置文件的名称（默认：从归档文件中推断）。 |
...
:::tip
**初次接触分发？** 请从 [配置文件分发用户指南](../user-guide/profile-distributions.md) 开始——它涵盖了原因、时机和方法，并提供了完整的示例。下面的部分是当你明确知道要做什么时，一份简洁的 CLI 参考。
:::

分发将配置文件转换为可共享的、版本化的工件，并作为 **git 仓库** 发布。接收者只需一条命令即可安装分发，并且以后可以在不触及本地记忆、会话或凭据的情况下就地更新。
...
:::info
`hermes profile export` / `import` 仍然是用于 **本地备份和恢复** 你自己机器上的配置文件的正确命令。分发（`install` / `update` / `info`）是一个独立的概念：通过 git 交付配置文件，以便其他人可以安装它。
:::
...
`hermes profile list` 还会在 `Distribution` 列中显示发行版名称和版本，并且 `hermes profile show <名称>` / `delete <名称>` 会显示源 URL，以便你一眼就能看出哪些配置文件来自 git 仓库，哪些是在本地创建的。

### 私有发行版

私有 git 仓库可以作为发行版源，无需额外配置 —— 安装过程会调用你正常的 `git` 二进制文件，因此你的 shell 已设置的任何身份验证（SSH 密钥、`git credential` 助手、GitHub CLI 存储的 HTTPS 凭据）都会透明地应用。
...
如果在安装过程中克隆操作在终端中交互式地提示输入凭据，该提示会正常显示。请先按照你通常对同一仓库使用 `git clone` 的方式设置身份验证，然后再进行安装。

### 发行版清单 (`distribution.yaml`)

每个发行版在其仓库根目录都有一个 `distribution.yaml` 文件：
...
`distribution_owned` 是可选的。如果设置了，则更新时仅替换这些路径；配置文件中的其他任何内容都保持为用户所有。如果省略，则应用上述默认值。

...
创建配置文件**不会**使该配置文件目录成为终端命令的默认项目/工作空间目录。如果你希望配置文件从特定项目开始，请在该配置文件的 `config.yaml` 中设置 `terminal.cwd`。
...
这显示的是配置文件的 Hermes 主目录，而不是终端工作目录。终端命令从 `terminal.cwd`（或在本地后端启动时，当 `cwd: "."` 时从启动目录）开始。

...
| `info` | 显示配置文件的发行版元数据（来源 URL、提交、最后更新时间）。 |
...
| `--no-skills` | 创建一个**空**配置文件，不启用任何捆绑技能。在配置文件中写入一个 `.no-bundled-skills` 标记，以便未来的 `hermes update` 运行不会重新植入捆绑技能集，并且拒绝与 `--clone` / `--clone-all` 结合使用（因为后者无论如何都会复制技能）。适用于不应继承完整技能目录的窄编排器配置文件或沙盒配置文件。要在已创建的配置文件（包括默认的 `~/.hermes`）上切换此设置，请使用 `hermes skills opt-out` / `hermes skills opt-in`。 |

创建配置文件**不会**使该配置文件目录成为终端命令的默认项目/工作区目录。如果你希望配置文件在特定项目中启动，请在该配置文件的 `config.yaml` 中设置 `terminal.cwd`。
...
此处显示的是配置文件的 Hermes 主目录，而非终端工作目录。终端命令从 `terminal.cwd`（或在本地后端启动时，若 `cwd: "."` 则从启动目录）开始执行。
...
:::tip
**初次接触分发？** 请从 [配置文件分发用户指南](../user-guide/profile-distributions.md) 开始——它涵盖了原因、时机和方法，并提供了完整的示例。下面的部分是简洁的 CLI 参考，适用于您明确知道要做什么的情况。
:::

分发将配置文件转换为可共享的、版本化的工件，并作为 **git 仓库** 发布。接收者只需一条命令即可安装分发，并且以后可以在不触及本地记忆、会话或凭据的情况下就地更新它。

`auth.json` 和 `.env` 从不属于分发的一部分——它们保留在安装用户的机器上。

接收者的用户数据（记忆、会话、认证信息、他们对 `.env` 的编辑）在初始安装和后续更新过程中始终会被保留。

:::info
`hermes profile export` / `import` 仍然是用于 **本地备份和恢复** 您自己机器上的配置文件的正确命令。分发（`install` / `update` / `info`）是一个独立的概念：通过 git 分发配置文件，以便其他人可以安装它。
:::
...
安装程序会显示清单，列出所需的环境变量，并在请求确认前警告有关定时任务的信息。所需的环境变量会放入一个 `.env.EXAMPLE` 文件中，您需要将其复制为 `.env` 并填写。
...
默认情况下，`config.yaml` 会被保留以保持您的本地覆盖。传递 `--force-config` 以将其重置为分发附带的配置。
...
`hermes profile list` 还会在 `Distribution` 列中显示发行版的名称和版本，而 `hermes profile show <名称>` / `delete <名称>` 会显示来源 URL，这样你就能一目了然地分辨出哪些配置文件来自 git 仓库，哪些是在本地创建的。

### 私有发行版

私有 git 仓库作为发行版来源无需额外配置 —— 安装过程会调用你系统中正常的 `git` 二进制文件，因此你的 shell 已设置的任何身份验证（SSH 密钥、`git credential` 助手、GitHub CLI 存储的 HTTPS 凭据）都会透明地应用。
...
如果在安装过程中，克隆操作在你的终端中交互式地提示输入凭据，该提示会正常显示。请先按照你通常对同一仓库使用 `git clone` 的方式设置好身份验证，然后再进行安装。
...
`hermes_requires` 支持 `>=`、`<=`、`==`、`!=`、`>`、`<` 或一个裸版本号（视为 `>=`）。如果当前的 Hermes 版本不满足规范，安装会失败并显示清晰的错误信息。

`distribution_owned` 是可选的。如果设置了，则只有这些路径会在更新时被替换；配置文件中其他任何内容都保持为用户所有。如果省略，则应用上述默认值。

### 发布发行版

创作一个发行版只需进行 git push：

1.  在你的配置文件目录中，创建 `distribution.yaml`，至少包含 `name` 和 `version`。
2.  初始化一个 git 仓库（或使用现有仓库）并推送到 GitHub / GitLab / 任何 Hermes 可以克隆的托管平台。
3.  告诉接收者运行 `hermes profile install <你的仓库URL>`。

使用语义化版本标签进行版本化发布 —— 克隆 `HEAD` 的接收者会获得你的最新状态，并且你始终可以在清单中更新 `version:`。
...
全局标志，用于在特定配置文件下运行任何 Hermes 命令，而无需更改粘性默认值。这会在命令执行期间覆盖当前激活的配置文件。

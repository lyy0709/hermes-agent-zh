...
| `AZURE_ANTHROPIC_KEY` | 当 `provider: anthropic` 且 `base_url` 指向 Azure Foundry Claude 部署时使用的 Azure Anthropic API 密钥（当同时配置了 Anthropic 和 Azure Anthropic 时，作为 `ANTHROPIC_API_KEY` 的替代） |
...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 优先使用这些凭证，因为它们可以自动刷新。**针对 Anthropic 的 OAuth 需要 Claude Max 计划并购买额外的使用额度** —— Hermes 以 Claude Code 身份路由，这只会从 Max 计划的额外/超额额度中扣除，而不是基础 Max 额度，并且在 Claude Pro 上无效。如果没有 Max 计划 + 额外额度，请改用 API 密钥。环境变量如 `ANTHROPIC_TOKEN` 作为手动覆盖仍然有用，但它们不再是 Claude Max 登录的首选方式。
...
| `HERMES_DOCKER_BINARY` | 覆盖 Hermes 调用的容器二进制文件（例如 `podman`、`/usr/local/bin/docker`）。未设置时，Hermes 自动在 `PATH` 上发现 `docker` 或 `podman`。当两者都安装且你想使用非默认项，或二进制文件位于 `PATH` 之外时需要。 |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信**群聊 ID**（非成员用户 ID）。变量名是遗留的——它期望群组 ID。仅当 iLink 实际传递群组事件时生效；二维码登录的 iLink 机器人身份（`...@im.bot`）通常不会接收普通微信群聊消息。 |
...
| `BLUEBUBBLES_HOME_CHANNEL` | 定时任务/通知交付的电话/电子邮件 |
...
| `QQ_GROUP_ALLOWED_USERS` | 允许群组 @-消息访问的逗号分隔的 QQ 群组 ID |
...
| `MATRIX_DEVICE_ID` | 稳定的 Matrix 设备 ID，用于跨重启的 E2EE 持久化（例如 `HERMES_BOT`）。没有这个，E2EE 密钥每次启动都会轮换，历史房间解密会中断。 |
...
| `WEBHOOK_SECRET` | Webhook 签名验证的全局 HMAC 密钥（当路由未指定自己的密钥时用作后备） |
...
针对各平台的高级调节旋钮，用于节流出站消息批处理器。大多数用户永远不需要调整这些设置；默认值已设置为尊重各平台的速率限制，同时不会感到迟钝。
...
| `HERMES_STREAM_RETRIES` | 在瞬态网络错误时，流中重连尝试的次数（默认：`3`）。 |
...
| `HERMES_AGENT_NOTIFY_INTERVAL` | 消息网关：长时间运行的 Agent 回合之间，进度通知的间隔时间（秒）。 |
...
| `HERMES_WRITE_SAFE_ROOT` | 可选的目录前缀，用于限制 `write_file`/`patch` 的写入；此范围之外的路径需要批准。 |
...
| `HERMES_TUI_RESUME` | 启动时通过 ID 恢复特定的 TUI 会话。设置此变量后，`hermes --tui` 会跳过创建新会话，转而拾取指定名称的会话 —— 这在断开连接或终端崩溃后重新连接时非常有用。 |
| `HERMES_TUI_THEME` | 强制设置 TUI 颜色主题：`light`、`dark` 或原始的 6 字符背景十六进制值（例如 `ffffff` 或 `1a1a2e`）。未设置时，Hermes 使用 `COLORFGBG` 和终端背景查询自动检测；此变量会覆盖那些未设置 `COLORFGBG` 的终端（如 Ghostty、Warp、iTerm2 等）的检测结果。 |

...
| `AZURE_ANTHROPIC_KEY` | 当 `provider: anthropic` 且 `base_url` 指向 Azure Foundry Claude 部署时，用于 `AZURE_ANTHROPIC_KEY` 的 Azure Anthropic API 密钥（当同时配置了 Anthropic 和 Azure Anthropic 时，作为 `ANTHROPIC_API_KEY` 的替代方案） |
| `HF_TOKEN` | Hugging Face Inference Providers 的 Token ([huggingface.co/settings/tokens](https://huggingface.co/settings/tokens)) |
...
| `AWS_PROFILE` | Bedrock 身份验证的 AWS 命名配置文件（读取 `~/.aws/credentials`）。留空则使用默认的 boto3 凭据链。 |
...
| `CLAUDE_CODE_OAUTH_TOKEN` | 如果手动导出了 Claude Code Token，则显式覆盖 |
...
| `HERMES_LOCAL_STT_LANGUAGE` | 传递给 `HERMES_LOCAL_STT_COMMAND` 的默认语言，或自动检测的本地 `whisper` CLI 后备方案（默认：`en`） |
...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 会优先使用这些凭证，因为它们可以自动刷新。**针对 Anthropic 的 OAuth 需要 Claude Max 计划并已购买额外使用额度** —— Hermes 会以 Claude Code 的身份路由请求，这只会从 Max 计划的额外/超额额度中扣除，不会消耗基础 Max 额度，并且在 Claude Pro 计划上无效。如果没有 Max 计划及额外额度，请改用 API 密钥。像 `ANTHROPIC_TOKEN` 这样的环境变量仍然可以作为手动覆盖使用，但它们不再是 Claude Max 登录的首选方式。
...
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` / `LANGFUSE_BASE_URL` | 标准 Langfuse SDK 名称。当等效的 `HERMES_LANGFUSE_*` 变量未设置时，会作为后备方案接受。 |
### Nous 工具网关
...
| `TELEGRAM_WEBHOOK_SECRET` | Telegram 在每个更新中回显用于验证的密钥令牌。**只要设置了 `TELEGRAM_WEBHOOK_URL` 就必需**——没有它网关拒绝启动（GHSA-3vpc-7q5r-276h）。使用 `openssl rand -hex 32` 生成。 |
| `TELEGRAM_REACTIONS` | 在处理消息时启用表情符号反应（默认：`false`） |
| `TELEGRAM_REPLY_TO_MODE` | 回复引用行为：`off`、`first`（默认）或 `all`。匹配 Discord 模式。 |
| `TELEGRAM_IGNORED_THREADS` | 机器人从不响应的逗号分隔的 Telegram 论坛主题/线程 ID |
...
| `DISCORD_IGNORED_CHANNELS` | 机器人从不响应的逗号分隔的频道 ID |
...
| `GOOGLE_CHAT_HOME_CHANNEL` | 定时任务交付的默认空间（例如 `spaces/AAAA...`） |
...
| `GOOGLE_CHAT_BOOTSTRAP_SPACES` | 启动时解析机器人自身 `users/{id}` 时探测的额外空间 ID 的逗号分隔列表 |
...
| `FEISHU_HOME_CHANNEL` | 定时任务交付和通知的飞书聊天 ID |
...
| `WECOM_HOME_CHANNEL` | 定时任务交付和通知的企业微信聊天 ID |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信**群聊 ID**（非成员用户 ID）。变量名是遗留的——它期望群组 ID。仅当 iLink 实际传递群组事件时生效；二维码登录的 iLink 机器人身份（`...@im.bot`）通常不接收普通的微信群组消息。 |
| `WEIXIN_HOME_CHANNEL` | 定时任务交付和通知的微信聊天 ID |
...
| `BLUEBUBBLES_HOME_CHANNEL` | 定时任务/通知交付的电话/邮箱 |
...
| `QQBOT_HOME_CHANNEL` | 定时任务交付和通知的 QQ 用户/群组 openID |
...
| `MATRIX_DEVICE_ID` | 用于跨重启持久化 E2EE 的稳定 Matrix 设备 ID（例如 `HERMES_BOT`）。没有这个，E2EE 密钥每次启动都会轮换，历史房间解密会中断。 |
| `MATRIX_REACTIONS` | 在入站消息上启用处理生命周期表情符号反应（默认：`true`）。设置为 `false` 以禁用。 |
...
| `API_SERVER_MODEL_NAME` | 在 `/v1/models` 上公布的模型名称。默认为配置文件名称（或默认配置文件的 `hermes-agent`）。对于多用户设置很有用，其中像 Open WebUI 这样的前端需要每个连接不同的模型名称。 |
...
| `GATEWAY_PROXY_KEY` | 代理模式下用于远程 API 服务器认证的 Bearer 令牌。必须与远程主机上的 `API_SERVER_KEY` 匹配。 |
...
用于限制出站消息批处理程序的高级平台特定调节选项。大多数用户永远不需要调整这些；默认值设置为在尊重每个平台速率限制的同时保持流畅体验。
...
| `HERMES_TELEGRAM_FOLLOWUP_GRACE_SECONDS` | Agent 完成后发送跟进消息前的延迟，以避免与最后一个流块竞争。 |
...
| `HERMES_RESTART_DRAIN_TIMEOUT` | 消息网关：在 `/restart` 时等待活动运行排空的秒数，然后强制重启（默认：`900`）。 |
...
| `HERMES_API_CALL_STALE_TIMEOUT` | 非流式陈旧调用超时时间（秒）（默认：`300`）。留空时对本地提供商自动禁用。也可通过 `config.yaml` 中的 `providers.<id>.stale_timeout_seconds` 或 `providers.<id>.models.<model>.stale_timeout_seconds` 配置。 |
| `HERMES_STREAM_READ_TIMEOUT` | 流式套接字读取超时时间（秒）（默认：`120`）。对于本地提供商，自动增加到 `HERMES_API_TIMEOUT`。如果本地 LLM 在长代码生成期间超时，请增加此值。 |
| `HERMES_STREAM_STALE_TIMEOUT` | 陈旧流检测超时时间（秒）（默认：`180`）。对本地提供商自动禁用。如果在此时间窗口内没有数据块到达，则触发连接终止。 |
| `HERMES_STREAM_RETRIES` | 在临时网络错误时，流传输过程中重新连接的尝试次数（默认：`3`）。 |
| `HERMES_AGENT_TIMEOUT` | 运行中 Agent 的消息网关不活动超时时间（秒）（默认：`900`）。每次工具调用和流式 Token 时重置。设置为 `0` 以禁用。 |
...
| 变量 | 描述 |
|----------|-------------|
| `HERMES_TUI` | 设置为 `1` 时，启动 [TUI](../user-guide/tui.md) 而不是经典 CLI。等同于传递 `--tui`。 |
| `HERMES_TUI_DIR` | 指向预构建的 `ui-tui/` 目录的路径（必须包含 `dist/entry.js` 和已填充的 `node_modules`）。由发行版和 Nix 使用，以跳过首次启动时的 `npm install`。 |
| `HERMES_TUI_RESUME` | 启动时按 ID 恢复特定的 TUI 会话。设置后，`hermes --tui` 会跳过创建新会话，而是拾取指定的会话——在断开连接或终端崩溃后重新连接时很有用。 |
...
主模型备用链仅通过 `config.yaml` 配置——没有对应的环境变量。在顶层添加一个包含 `provider` 和 `model` 键的 `fallback_providers` 列表，以便在主模型遇到错误时启用自动故障转移。
...
为了向后兼容，旧的顶层 `fallback_model` 单提供商格式仍然会被读取，但新配置应使用 `fallback_providers`。

...
所有变量都放在 `~/.hermes/.env` 文件中。你也可以使用 `hermes config set VAR value` 来设置它们。
...
| `CLAUDE_CODE_OAUTH_TOKEN` | 如果手动导出，则显式覆盖 Claude Code Token |
...
| `TERMINAL_SSH_PERSISTENT` | 为 SSH 覆盖持久化 shell（默认：遵循 `TERMINAL_PERSISTENT_SHELL`） |
...
| `TERMINAL_SSH_PERSISTENT` | 为 SSH 后端覆盖持久化 shell（默认：遵循 `TERMINAL_PERSISTENT_SHELL`） |
...
| `DISCORD_ALLOWED_ROLES` | 允许使用机器人的 Discord 角色 ID，以逗号分隔（与 `DISCORD_ALLOWED_USERS` 为 OR 关系）。自动启用成员意图。在审核团队变动时很有用 —— 角色授权会自动传播。 |
...
| `DISCORD_NO_THREAD_CHANNELS` | 机器人响应但不自动创建线程的频道 ID，以逗号分隔 |
...
| `SIGNAL_ALLOWED_USERS` | E.164 格式的电话号码或 UUID，以逗号分隔 |
...
| `SMS_ALLOWED_USERS` | 允许聊天的 E.164 格式电话号码，以逗号分隔 |
...
| `EMAIL_ALLOWED_USERS` | 允许向机器人发送消息的电子邮件地址，以逗号分隔 |
...
| `WEIXIN_ALLOWED_USERS` | 允许与机器人私信的微信用户 ID，以逗号分隔 |
...
| `QQ_SANDBOX` | 将 QQ 机器人路由到沙盒消息网关以进行开发测试（`true`/`false`）。与来自 [q.qq.com](https://q.qq.com) 的沙盒应用凭证一起使用。 |
...
| `MATTERMOST_FREE_RESPONSE_CHANNELS` | 机器人响应无需 `@提及` 的频道 ID，以逗号分隔 |
...
| `MATRIX_FREE_RESPONSE_ROOMS` | 机器人响应无需 `@提及` 的房间 ID，以逗号分隔 |
...
| `API_SERVER_MODEL_NAME` | 在 `/v1/models` 上公布的模型名称。默认为配置文件名称（或默认配置文件的 `hermes-agent`）。对于多用户设置很有用，其中像 Open WebUI 这样的前端需要每个连接有独特的模型名称。 |
...
| `HERMES_STREAM_READ_TIMEOUT` | 流式传输套接字读取超时时间（秒）（默认：`120`）。对于本地提供商，会自动增加到 `HERMES_API_TIMEOUT`。如果本地 LLM 在生成长代码时超时，请增加此值。 |
| `HERMES_STREAM_STALE_TIMEOUT` | 流式传输陈旧检测超时时间（秒）（默认：`180`）。对于本地提供商，会自动禁用。如果在此时间窗口内没有收到数据块，则会触发连接终止。 |
...
| `HERMES_TUI_DIR` | 预构建的 `ui-tui/` 目录的路径（必须包含 `dist/entry.js` 和已填充的 `node_modules`）。发行版和 Nix 使用此变量来跳过首次启动时的 `npm install`。 |
...
| `HERMES_CRON_SCRIPT_TIMEOUT` | 定时任务附加的预运行脚本的超时时间（秒）（默认：`120`）。对于需要更长时间执行的脚本（例如，用于反机器人计时的随机延迟），可以覆盖此值。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 进行配置。 |

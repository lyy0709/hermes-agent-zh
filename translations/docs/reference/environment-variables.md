...
| `IDENTITY_ENDPOINT` / `MSI_ENDPOINT` | App Service、Functions 和 Container Apps 的托管标识端点；VM 通常使用 IMDS 且不设置这些 |
...
| `AWS_PROFILE` | AWS Bedrock 认证的命名配置文件（读取 `~/.aws/credentials`）。留空则使用默认的 boto3 凭证链。 |
...
| `CLAUDE_CODE_OAUTH_TOKEN` | 如果你手动导出了一个令牌，则显式覆盖 Claude Code 令牌 |
| `HERMES_MODEL` | 在进程级别覆盖模型名称（由定时任务调度器使用；正常使用更推荐 `config.yaml`） |
...
| `HERMES_KANBAN_DISPATCH_IN_GATEWAY` | `kanban.dispatch_in_gateway` 的运行时覆盖。设置为 `0`、`false`、`no` 或 `off` 以防止消息网关启动嵌入式看板调度器；任何其他非空值则启用它。当单独的调度器进程拥有看板时有用 |
## 提供商认证（OAuth）

对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 优先使用它们，因为这些凭证可以自动刷新。**针对 Anthropic 的 OAuth 需要一个 Claude Max 计划并购买了额外的使用额度** —— Hermes 以 Claude Code 的身份路由，这只会从 Max 计划的额外/超额额度中扣除，而不是基础 Max 额度，并且在 Claude Pro 上无效。如果没有 Max 计划 + 额外额度，请改用 API 密钥。环境变量如 `ANTHROPIC_TOKEN` 作为手动覆盖仍然有用，但它们不再是 Claude Max 登录的首选方式。
...
| `SEARXNG_URL` | SearXNG 实例 URL，用于免费自托管的网络搜索 —— 无需 API 密钥 ([searxng.github.io](https://searxng.github.io/searxng/)) |
...
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` / `LANGFUSE_BASE_URL` | 标准 Langfuse SDK 名称。当等效的 `HERMES_LANGFUSE_*` 变量未设置时，作为备用值被接受。 |
### Nous 工具网关
...
| `TELEGRAM_GROUP_ALLOWED_USERS` | 仅在群组/论坛中授权的逗号分隔的发送者用户 ID（**不**授予私聊访问权限）。为向后兼容 #17686 之前的配置，仍接受以 `-` 开头的聊天 ID 形值作为聊天 ID，但会显示弃用警告。 |
...
| `DISCORD_ALLOW_ANY_ATTACHMENT` | 为 `true` 时，接受任何文件类型的附件（不仅仅是内置的 PDF/文本/zip/办公软件白名单）。未知类型会被缓存，并作为本地路径提供给 Agent，以便通过 `terminal` / `read_file` / `ffprobe` 进行检查。默认 `false`。 |
...
| `GOOGLE_CHAT_BOOTSTRAP_SPACES` | 启动时解析机器人自身的 `users/{id}` 时要探测的额外逗号分隔的空间 ID |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信**群聊 ID**（非成员用户 ID）。变量名是遗留的——它期望群组 ID。仅当 iLink 实际传递群组事件时生效；二维码登录的 iLink 机器人身份（`...@im.bot`）通常不会接收普通的微信群聊消息。 |
...
| `GATEWAY_PROXY_URL` | 用于转发消息的远程 Hermes API 服务器 URL（[代理模式](/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)）。设置后，网关仅处理平台 I/O——所有 Agent 工作都委派给远程服务器。也可通过 `config.yaml` 中的 `gateway.proxy_url` 配置。 |
...
用于 [Web 仪表盘](/user-guide/features/web-dashboard) 的身份验证以及将 [Hermes Desktop 连接到远程后端](/user-guide/features/web-dashboard#connecting-hermes-desktop-to-a-remote-backend)。根据仅存储密钥的惯例，凭证应存放在 `~/.hermes/.env` 中；OAuth 的 `client_id`/`portal_url` 最好在 `config.yaml` 的 `dashboard.oauth` 下设置（如果同时设置了环境变量，则环境变量优先）。

为远程 Hermes Desktop 连接暴露仪表盘的推荐方式是使用内置的**用户名/密码**提供商：设置下面的 `HERMES_DASHBOARD_BASIC_AUTH_*` 环境变量并运行 `hermes dashboard --host 0.0.0.0`。绑定到非环回地址会启用身份验证网关，Desktop 使用用户名和密码登录。

| 变量 | 描述 |
|----------|-------------|
| `HERMES_DASHBOARD_BASIC_AUTH_USERNAME` | 内置用户名/密码仪表盘身份验证提供商 (`plugins/dashboard_auth/basic`) 的用户名。与密码一起设置时激活该提供商。覆盖 `dashboard.basic_auth.username`。 |
| `HERMES_DASHBOARD_BASIC_AUTH_PASSWORD` | 基础提供商的明文密码（加载时在内存中哈希处理）。优先于配置中的 `password_hash`，因此可以通过环境变量轮换。覆盖 `dashboard.basic_auth.password`。 |
| `HERMES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | 基础提供商的 scrypt 密码哈希（首选——静态时不存储明文）。使用 `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"` 计算。覆盖 `dashboard.basic_auth.password_hash`。 |
| `HERMES_DASHBOARD_BASIC_AUTH_SECRET` | 用于签署基础提供商无状态会话令牌的 HMAC 密钥（32+ 字节，base64/hex/raw）。显式设置以使会话在重启后保持/跨多个工作进程；留空 → 每个进程随机生成（每次重启都会登出）。覆盖 `dashboard.basic_auth.secret`。 |
| `HERMES_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | 基础提供商的访问令牌有效期（默认 12 小时）。覆盖 `dashboard.basic_auth.session_ttl_seconds`。 |
| `HERMES_DESKTOP_REMOTE_URL` | (Desktop 端) 远程后端的基础 URL，例如 `http://host:9119`。设置后，覆盖应用内的消息网关 URL；您仍需通过消息网关设置面板使用用户名和密码登录。 |
| `HERMES_DASHBOARD_OAUTH_CLIENT_ID` | 受保护/公开仪表盘的 OAuth 客户端 ID (`agent:{instance_id}`)。覆盖 `dashboard.oauth.client_id`。由 Nous Portal 为托管部署提供。 |
| `HERMES_DASHBOARD_PORTAL_URL` | OAuth 门户 URL (默认: `https://portal.nousresearch.com`)。仅在暂存/自定义部署时覆盖。 |
| `HERMES_DASHBOARD_PUBLIC_URL` | 仪表盘可访问的完整公开 URL，用于在反向代理后构建 OAuth 回调。覆盖 `dashboard.public_url`。 |

### Microsoft Graph (Teams 会议)

即将推出的 Teams 会议摘要流水线所使用的 Microsoft Graph REST 客户端的仅应用凭证。请参阅 [注册 Microsoft Graph 应用程序](/guides/microsoft-graph-app-registration) 了解 Azure 门户操作指南和所需的精确 API 权限。
...
| `MSGRAPH_SCOPE` | 客户端凭证令牌请求的 OAuth2 作用域 (默认: `https://graph.microsoft.com/.default`)。 |
| `MSGRAPH_AUTHORITY_URL` | Microsoft 身份平台授权机构 (默认: `https://login.microsoftonline.com`)。仅在国家/主权云中覆盖（例如，GCC High 使用 `https://login.microsoftonline.us`）。 |
...
| `MSGRAPH_WEBHOOK_ACCEPTED_RESOURCES` | Graph 资源路径/模式的逗号分隔允许列表（例如 `communications/onlineMeetings,chats/*/messages`）。尾随的 `*` 表示前缀匹配。空 = 接受所有。 |
| `MSGRAPH_WEBHOOK_ALLOWED_SOURCE_CIDRS` | 允许向监听器发送 POST 请求的逗号分隔 CIDR 范围（例如 `52.96.0.0/14,52.104.0.0/14`）。空 = 允许所有（默认）。在生产环境中限制为 Microsoft Graph 发布的出口范围。 |
...
| `TEAMS_GRAPH_ACCESS_TOKEN` | 预先获取的用于 Graph 投递的委派访问令牌。很少需要——未设置时，写入器会回退到 `MSGRAPH_*` 应用凭证。 |
...
由内置的 LINE 平台插件 (`plugins/platforms/line/`) 使用。完整设置请参阅 [消息网关 → LINE](/user-guide/messaging/line)。
| 变量 | 描述 |
|----------|-------------|
| `LINE_CHANNEL_ACCESS_TOKEN` | 来自 LINE Developers Console（Messaging API 标签页）的长期 Channel Access Token。必需。 |
| `LINE_CHANNEL_SECRET` | Channel Secret（Basic settings 标签页）；用于 HMAC-SHA256 Webhook 签名验证。必需。 |
...
| `LINE_HOME_CHANNEL` | 使用 `deliver: line` 的定时任务的默认投递目标。 |
| `LINE_SLOW_RESPONSE_THRESHOLD` | 触发慢速 LLM 模板按钮回传前的秒数（默认：`45`）。设置为 `0` 以禁用并始终使用推送回退。 |
| `LINE_PENDING_TEXT` | 与回传按钮一起显示的气泡文本。 |
| `LINE_BUTTON_LABEL` | 回传按钮标签（默认：`Get answer`）。 |
| `LINE_DELIVERED_TEXT` | 当再次点击已投递的回传按钮时的回复（默认：`Already replied ✅`）。 |
| `LINE_INTERRUPTED_TEXT` | 当点击因 `/stop` 而孤立的回传按钮时的回复（默认：`Run was interrupted before completion.`）。 |
...
| `NTFY_TOKEN` | 可选的认证 Token。Bearer Token（例如 `tk_xyz`）或用于 Basic 认证的 `user:pass`。 |
...
| `NTFY_HOME_CHANNEL` | 使用 `deliver: ntfy` 的定时任务的默认投递目标。 |
| `NTFY_HOME_CHANNEL_NAME` | 主通道的人工标签（默认为主题名称）。 |

在使用不受信任的主题部署之前，请参阅 [ntfy 消息指南](/user-guide/messaging/ntfy) — 特别是 **身份模型** 部分。

### 高级消息调优

用于节流出站消息批处理器的各平台高级旋钮。大多数用户永远不需要调整这些；默认值设置为尊重每个平台的速率限制，同时不感觉迟缓。
...
| `HERMES_MATRIX_TEXT_BATCH_DELAY_SECONDS` / `_SPLIT_DELAY_SECONDS` | Matrix 对应的批处理旋钮。 |
...
| `HERMES_GATEWAY_BOOTSTRAP_STATE` | 在 s6-overlay Docker 镜像内部，声明消息网关在全新卷上的**初始**监管状态。在空白卷上没有持久化的 `gateway_state.json`，因此引导协调器会注册 `gateway-default` 槽位但将其保持为**停止**状态（它只在最后记录的状态是 `running` 时才会自动启动）。将其设置为 `running`，首次启动设置钩子会在协调器运行*之前*植入 `gateway_state.json`，因此消息网关在首次启动时就会启动。只有字面值 `running` 会被识别。仅首次启动有效：现有的 `gateway_state.json` 永远不会被覆盖，因此有意停止的消息网关在重启后保持停止状态。在 s6 镜像外部无效。 |
| `HERMES_FILE_MUTATION_VERIFIER` | 启用每轮文件变更验证器页脚（默认：`true`）。启用后，Hermes 会附加一个提示性列表，列出本轮中任何失败且未被成功写入覆盖的 `write_file` / `patch` 调用。设置为 `0`、`false`、`no` 或 `off` 以抑制。镜像 `config.yaml` 中的 `display.file_mutation_verifier`；设置环境变量时，环境变量优先。 |
| `HERMES_CRON_TIMEOUT` | 定时任务 Agent 运行的不活动超时（秒）（默认：`600`）。Agent 在主动调用工具或接收流 Token 时可以无限期运行 — 此设置仅在空闲时触发。设置为 `0` 表示无限制。 |
| `HERMES_CRON_SCRIPT_TIMEOUT` | 定时任务附加的预运行脚本的超时时间（秒）（默认：`120`）。覆盖需要更长时间执行的脚本（例如，用于反机器人定时的随机延迟）。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
| `HERMES_CRON_MAX_PARALLEL` | 每次触发时并行运行的最大定时任务数（默认：`4`）。 |
## Agent 行为
...
| `CODEX_HOME` | 当启用 [Codex 应用服务器运行时](../user-guide/features/codex-app-server-runtime) 时，覆盖 Codex CLI 读取其配置和认证信息的目录（默认值：`~/.codex`）。Hermes 的迁移会将托管块写入 `<CODEX_HOME>/config.toml`。 |
...
| `HERMES_DISABLE_FILE_STATE_GUARD` | 设置为 `1` 以关闭 `patch`/`write_file` 上的“自读取后文件已更改”防护。 |
| `HERMES_CORE_TOOLS` | 用于覆盖规范核心工具列表的逗号分隔列表（高级；很少需要）。 |
| `HERMES_BUNDLED_SKILLS` | 用于覆盖启动时加载的捆绑技能列表的逗号分隔列表。 |
| `HERMES_OPTIONAL_SKILLS` | 可选技能名称的逗号分隔列表，用于在首次运行时自动安装。 |

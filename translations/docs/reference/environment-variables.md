...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 优先使用它们，因为这些凭证可以自动刷新。**针对 Anthropic 的 OAuth 需要一个 Claude Max 计划并购买了额外的使用额度** —— Hermes 以 Claude Code 的身份路由，这只会从 Max 计划的额外/超额额度中扣除，而不是基础 Max 额度，并且在 Claude Pro 上无效。如果没有 Max 计划 + 额外额度，请改用 API 密钥。环境变量如 `ANTHROPIC_TOKEN` 仍然可以作为手动覆盖使用，但它们不再是 Claude Max 登录的首选方式。
...
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` / `LANGFUSE_BASE_URL` | 标准 Langfuse SDK 名称。当对应的 `HERMES_LANGFUSE_*` 变量未设置时，作为备用值接受。 |
...
| `TELEGRAM_GROUP_ALLOWED_USERS` | 仅在群组/论坛中授权的逗号分隔的发送者用户 ID (**不**授予私聊访问权限)。为向后兼容 #17686 之前的配置，以 `-` 开头的聊天 ID 格式的值仍被识别为聊天 ID，但会显示弃用警告。 |
...
| `DISCORD_NO_THREAD_CHANNELS` | 机器人响应但不自动创建线程的逗号分隔的频道 ID |
...
| `GOOGLE_CHAT_BOOTSTRAP_SPACES` | 启动时解析机器人自身的 `users/{id}` 时探测的逗号分隔的额外空间 ID |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信**群聊 ID** (非成员用户 ID)。变量名是历史遗留——它期望群组 ID。仅当 iLink 实际传递群组事件时生效；扫码登录的 iLink 机器人身份 (`...@im.bot`) 通常不会接收普通微信群的群消息。 |
...
| `MATRIX_DEVICE_ID` | 稳定的 Matrix 设备 ID，用于跨重启的 E2EE 持久化 (例如 `HERMES_BOT`)。没有此设置，E2EE 密钥每次启动都会轮换，历史房间解密会中断。 |
| `MATRIX_REACTIONS` | 对入站消息启用处理周期表情符号反应 (默认: `true`)。设置为 `false` 以禁用。 |
...
| `WEBHOOK_SECRET` | Webhook 签名验证的全局 HMAC 密钥 (当路由未指定自己的密钥时用作备用) |
...
用于即将推出的 Teams 会议摘要流水线的 Microsoft Graph REST 客户端的仅应用凭据。有关 Azure 门户操作指南和所需的确切 API 权限，请参阅[注册 Microsoft Graph 应用程序](/docs/guides/microsoft-graph-app-registration)。
...
| `HERMES_FILE_MUTATION_VERIFIER` | 启用每轮文件变更验证器页脚（默认：`true`）。启用后，Hermes 会附加一个提示列表，列出该轮中任何失败且未被成功写入覆盖的 `write_file` / `patch` 调用。设置为 `0`、`false`、`no` 或 `off` 以抑制。镜像 `config.yaml` 中的 `display.file_mutation_verifier`；设置环境变量时，环境变量优先。 |
| `HERMES_CRON_TIMEOUT` | 定时任务 Agent 运行的不活动超时（秒）（默认：`600`）。Agent 在主动调用工具或接收流 Token 时可以无限期运行 —— 此设置仅在空闲时触发。设置为 `0` 表示无限制。 |
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到定时任务的预运行脚本的超时时间（秒）（默认：`120`）。为需要更长时间执行的脚本覆盖此设置（例如，用于反机器人定时的随机延迟）。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
| `HERMES_KANBAN_TASK` | 看板调度器在创建工作进程时设置（任务 UUID）。工作进程和衍生的 `hermes-tools` MCP 子进程会继承它，以便看板工具正确进行门控。请勿手动设置。 |
...
| `HERMES_TUI_RESUME` | 启动时通过 ID 恢复特定的 TUI 会话。设置后，`hermes --tui` 会跳过创建新会话，转而拾取指定名称的会话 —— 适用于在断开连接或终端崩溃后重新连接。 |
...
| `HERMES_INFERENCE_MODEL` | 强制指定 `hermes -z` / `hermes chat` 使用的模型，而无需修改 `config.yaml`。与 `HERMES_INFERENCE_PROVIDER` 配对使用。适用于需要每次运行覆盖默认模型的脚本化调用者（sweeper、CI、批量运行器）。 |
...
| `SESSION_RESET_HOUR` | 每日重置的小时数，24 小时制（默认：4 = 凌晨 4 点） |

...
针对各平台的高级调节旋钮，用于节流出站消息批处理器。大多数用户永远不需要调整这些；默认值已设置为尊重各平台的速率限制，同时不会感觉迟钝。
...
| `HERMES_DISABLE_FILE_STATE_GUARD` | 设置为 `1` 以关闭 `patch`/`write_file` 上的“文件自您读取后已更改”防护。 |
...
| `HERMES_TUI_RESUME` | 启动时按 ID 恢复特定的 TUI 会话。设置后，`hermes --tui` 会跳过创建新会话，转而拾取指定会话 —— 适用于在断开连接或终端崩溃后重新连接。 |
...
| `HERMES_INFERENCE_MODEL` | 强制指定 `hermes -z` / `hermes chat` 使用的模型，而无需修改 `config.yaml`。与 `HERMES_INFERENCE_PROVIDER` 配对使用。适用于需要每次运行覆盖默认模型的脚本化调用程序（sweeper、CI、批量运行器）。 |

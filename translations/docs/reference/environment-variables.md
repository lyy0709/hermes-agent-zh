...
| `NVIDIA_BASE_URL` | 覆盖 NVIDIA 基础 URL（默认：`https://integrate.api.nvidia.com/v1`；对于本地 NIM 端点，可设置为 `http://localhost:8000/v1`） |
...
| `HERMES_MODEL` | 在进程级别覆盖模型名称（由定时任务调度器使用；常规使用建议使用 `config.yaml`） |
...
| 变量 | 描述 |
|----------|-------------|
| `HERMES_INFERENCE_PROVIDER` | 覆盖提供商选择：`auto`、`openrouter`、`nous`、`openai-codex`、`copilot`、`copilot-acp`、`anthropic`、`huggingface`、`zai`、`kimi-coding`、`kimi-coding-cn`、`minimax`、`minimax-cn`、`kilocode`、`xiaomi`、`arcee`、`alibaba`、`deepseek`、`nvidia`、`ollama-cloud`、`xai`（别名 `grok`）、`google-gemini-cli`、`opencode-zen`、`opencode-go`、`ai-gateway`（默认：`auto`） |
...
| `MATTERMOST_FREE_RESPONSE_CHANNELS` | 机器人响应无需 `@提及` 的频道 ID，逗号分隔 |
...
| `MATRIX_FREE_RESPONSE_ROOMS` | 机器人响应无需 `@提及` 的房间 ID，逗号分隔 |
...
| `HERMES_CRON_SCRIPT_TIMEOUT` | 定时任务附加的预运行脚本的超时时间（秒）（默认：`120`）。为需要更长时间执行的脚本（例如，用于反机器人计时的随机延迟）覆盖此值。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |

...
| `ANTHROPIC_TOKEN` | 手动或旧版 Anthropic OAuth/设置 Token 覆盖 |
...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 优先使用它们，因为这些凭证可以自动刷新。**针对 Anthropic 的 OAuth 需要 Claude Max 计划并购买额外的使用额度** —— Hermes 以 Claude Code 身份路由，这只会消耗 Max 计划的额外/超额额度，而非基础 Max 额度，并且在 Claude Pro 上无效。如果没有 Max 计划 + 额外额度，请改用 API 密钥。环境变量如 `ANTHROPIC_TOKEN` 作为手动覆盖仍然有用，但它们不再是 Claude Max 登录的首选方式。
...
| `HERMES_NOUS_MIN_KEY_TTL_SECONDS` | 重新签发前的最小 Agent 密钥 TTL（默认：1800 = 30分钟） |
...
用于捆绑的 [`observability/langfuse`](/docs/user-guide/features/built-in-plugins#observabilitylangfuse) 插件的环境变量。通过 `hermes tools → Langfuse Observability` 或在 `~/.hermes/.env` 中手动设置这些变量。插件也必须启用（`hermes plugins enable observability/langfuse`），这些变量才能生效。
...
| `TELEGRAM_GROUP_ALLOWED_USERS` | 仅在群组/论坛中授权的逗号分隔的发送者用户 ID（**不**授予私聊访问权限）。为向后兼容 #17686 之前的配置，仍接受聊天 ID 格式的值（以 `-` 开头），但会显示弃用警告。 |
...
| `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON` | 服务帐户 JSON 的路径，或内联 JSON（回退到 `GOOGLE_APPLICATION_CREDENTIALS`） |
...
| `GATEWAY_PROXY_KEY` | 在代理模式下用于远程 API 服务器认证的 Bearer 令牌。必须与远程主机上的 `API_SERVER_KEY` 匹配。 |
...
| `MSGRAPH_AUTHORITY_URL` | Microsoft 标识平台颁发机构（默认值：`https://login.microsoftonline.com`）。仅在国家/主权云中需要覆盖（例如，GCC High 使用 `https://login.microsoftonline.us`）。 |
...
| `LINE_SLOW_RESPONSE_THRESHOLD` | 慢速 LLM 模板按钮回传触发前的秒数（默认值：`45`）。设置为 `0` 以禁用并始终回退到 Push。 |
...
| `HERMES_RESTART_DRAIN_TIMEOUT` | 消息网关：在 `/restart` 时等待活动运行排空然后强制重启的秒数（默认值：`900`）。 |
...
| `HERMES_CRON_TIMEOUT` | 定时任务 Agent 运行的不活动超时时间（秒）（默认值：`600`）。Agent 在主动调用工具或接收流 Token 时可以无限期运行——仅在空闲时触发。设置为 `0` 表示无限制。 |
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到定时任务的预运行脚本的超时时间（秒）（默认值：`120`）。为需要更长时间执行的脚本覆盖（例如，用于反机器人定时的随机延迟）。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
| `HERMES_STREAM_READ_TIMEOUT` | 流式套接字读取超时时间（秒）（默认：`120`）。对于本地提供商，会自动增加到 `HERMES_API_TIMEOUT`。如果本地 LLM 在长代码生成期间超时，请增加此值。 |
| `HERMES_STREAM_STALE_TIMEOUT` | 陈旧流检测超时时间（秒）（默认：`180`）。对于本地提供商会自动禁用。如果在此时间窗口内没有收到数据块，则触发连接终止。 |
...
:::info 旧版迁移
首次加载时，带有 `compression.summary_model`、`compression.summary_provider` 和 `compression.summary_base_url` 的旧配置会自动迁移到 `auxiliary.compression.*`。
:::

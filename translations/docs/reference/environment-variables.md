...
所有变量都应放在 `~/.hermes/.env` 中。你也可以使用 `hermes config set VAR value` 来设置它们。
...
| `MINIMAX_API_KEY` | MiniMax API 密钥 — 全球端点 ([minimax.io](https://www.minimax.io))。**`minimax-oauth` 不使用**（OAuth 路径使用浏览器登录）。 |
| `MINIMAX_BASE_URL` | 覆盖 MiniMax 基础 URL（默认：`https://api.minimax.io/anthropic` — Hermes 使用 MiniMax 的 Anthropic Messages 兼容端点）。**`minimax-oauth` 不使用**。 |
| `MINIMAX_CN_API_KEY` | MiniMax API 密钥 — 中国端点 ([minimaxi.com](https://www.minimaxi.com))。**`minimax-oauth` 不使用**（OAuth 路径使用浏览器登录）。 |
| `MINIMAX_CN_BASE_URL` | 覆盖 MiniMax 中国区基础 URL（默认：`https://api.minimaxi.com/anthropic`）。**`minimax-oauth` 不使用**。 |
...
| `HERMES_KANBAN_BOARD` | 为此进程固定活动看板。优先于 `~/.hermes/kanban/current`；调度器将其注入工作子进程环境，使工作进程在物理上无法看到其他看板上的任务。默认为 `default`。Slug 验证：小写字母数字 + 连字符 + 下划线，1-64 个字符 |
...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 优先使用这些凭证，因为它们可以自动刷新。**针对 Anthropic 的 OAuth 需要一个 Claude Max 计划并购买了额外的使用额度** —— Hermes 以 Claude Code 身份路由，这只会从 Max 计划的额外/超额额度中扣除，而不是基础 Max 额度，并且在 Claude Pro 上无效。如果没有 Max 计划 + 额外额度，请改用 API 密钥。环境变量如 `ANTHROPIC_TOKEN` 作为手动覆盖仍然有用，但它们不再是 Claude Max 登录的首选方式。
...
| `HINDSIGHT_TIMEOUT` | Hindsight 记忆提供者 API 调用的超时时间（秒）（默认：`60`）。如果在 `/sync` 或 `on_session_switch` 期间你的 Hindsight 实例响应缓慢，并且在 `errors.log` 中看到超时，请增加此值。 |
...
用于捆绑的 [`observability/langfuse`](/docs/user-guide/features/built-in-plugins#observabilitylangfuse) 插件的环境变量。通过 `hermes tools → Langfuse Observability` 或在 `~/.hermes/.env` 中手动设置这些变量。这些变量生效前，还必须启用该插件（`hermes plugins enable observability/langfuse`）。
...
这些变量为付费 Nous 订阅者或自托管网关部署配置[工具网关](/docs/user-guide/features/tool-gateway)。大多数用户无需设置这些——网关会通过 `hermes model` 或 `hermes tools` 自动配置。
...
| `HERMES_DOCKER_BINARY` | 覆盖 Hermes 调用的容器二进制文件（例如 `podman`、`/usr/local/bin/docker`）。未设置时，Hermes 会自动在 `PATH` 上发现 `docker` 或 `podman`。当两者都安装且你希望使用非默认项，或者二进制文件位于 `PATH` 之外时需要。 |
...
| `TELEGRAM_GROUP_ALLOWED_USERS` | 仅在群组/论坛中授权的逗号分隔的发送者用户 ID（**不授予**私聊访问权限）。为向后兼容 #17686 之前的配置，仍接受以 `-` 开头的聊天 ID 形状的值作为聊天 ID，但会显示弃用警告。 |
...
| `TELEGRAM_WEBHOOK_SECRET` | Telegram 在每个更新中回显用于验证的密钥令牌。**只要设置了 `TELEGRAM_WEBHOOK_URL` 就必需**——没有它网关将拒绝启动（GHSA-3vpc-7q5r-276h）。使用 `openssl rand -hex 32` 生成。 |
...
| `DISCORD_ALLOWED_ROLES` | 允许使用机器人的逗号分隔的 Discord 角色 ID（与 `DISCORD_ALLOWED_USERS` 进行 OR 运算）。自动启用 Members 意图。在审核团队变动时很有用——角色授权会自动传播。 |
...
| `GOOGLE_CHAT_BOOTSTRAP_SPACES` | 启动时解析机器人自身 `users/{id}` 时要探测的额外空间 ID 的逗号分隔列表 |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信**群聊 ID**（非成员用户 ID）。变量名是旧版——它期望群组 ID。仅当 iLink 实际传递群组事件时才生效；二维码登录的 iLink 机器人身份（`...@im.bot`）通常不会接收普通的微信群组消息。 |
...
| `API_SERVER_KEY` | API 服务器认证的 Bearer 令牌。对于非环回绑定强制执行。 |
...
| `GATEWAY_PROXY_KEY` | 在代理模式下用于认证远程 API 服务器的 Bearer 令牌。必须与远程主机上的 `API_SERVER_KEY` 匹配。 |
...
| `MSGRAPH_AUTHORITY_URL` | Microsoft 身份平台授权机构（默认：`https://login.microsoftonline.com`）。仅在国家/主权云中需要覆盖（例如，GCC High 使用 `https://login.microsoftonline.us`）。 |
...
用于限制出站消息批处理程序的高级平台特定旋钮。大多数用户永远不需要调整这些；默认值设置为尊重每个平台的速率限制，同时不会感觉迟钝。
...
| `HERMES_TELEGRAM_FOLLOWUP_GRACE_SECONDS` | Agent 完成后发送跟进消息之前的延迟，以避免与最后一个流块竞争。 |
...
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到定时任务的预运行脚本的超时时间（秒）（默认：`120`）。为需要更长时间执行的脚本覆盖（例如，用于反机器人计时的随机延迟）。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
| `HERMES_BACKGROUND_NOTIFICATIONS` | 消息网关中的后台进程通知模式：`all`（默认）、`result`、`error`、`off` |
...
| `DELEGATION_MAX_CONCURRENT_CHILDREN` | 每个 `delegate_task` 批次的最大并行子 Agent 数（默认：`3`，下限为 1，无上限）。也可以通过 `config.yaml` 中的 `delegation.max_concurrent_children` 配置——配置值优先级更高。 |
...
| `HERMES_TUI_RESUME` | 在启动时按 ID 恢复特定的 TUI 会话。设置后，`hermes --tui` 会跳过创建新会话，而是拾取指定的会话——适用于在断开连接或终端崩溃后重新连接。 |
...
| `HERMES_INFERENCE_MODEL` | 强制设置 `hermes -z` / `hermes chat` 的模型，而不修改 `config.yaml`。与 `HERMES_INFERENCE_PROVIDER` 配对使用。适用于需要每次运行覆盖默认模型的脚本化调用者（sweeper、CI、批量运行器）。 |
## 会话设置
...
:::info 旧版迁移
旧版配置中的 `compression.summary_model`、`compression.summary_provider` 和 `compression.summary_base_url` 在首次加载时会自动迁移到 `auxiliary.compression.*`。
:::
...
主模型备用链仅通过 `config.yaml` 配置——没有对应的环境变量。添加一个顶层的 `fallback_providers` 列表，其中包含 `provider` 和 `model` 键，以便在主模型遇到错误时启用自动故障转移。

...
| `AZURE_ANTHROPIC_KEY` | 用于 `provider: anthropic` 且 `base_url` 指向 Azure Foundry Claude 部署的 Azure Anthropic API 密钥（当同时配置了 Anthropic 和 Azure Anthropic 时，作为 `ANTHROPIC_API_KEY` 的替代方案） |
...
| `HERMES_GIT_BASH_PATH` | **仅限 Windows。** 覆盖终端工具对 `bash.exe` 的发现。指向任何 bash — 完整的 Git-for-Windows 安装、通过符号链接的 WSL bash、MSYS2、Cygwin。安装程序会自动将其设置为它提供的 PortableGit。参见 [Windows (原生) 指南](../user-guide/windows-native.md#how-hermes-runs-shell-commands-on-windows) |
| `HERMES_DISABLE_WINDOWS_UTF8` | **仅限 Windows。** 设置为 `1` 以禁用 UTF-8 标准输入输出垫片 (`configure_windows_stdio()`)，并回退到控制台的区域设置代码页。用于二分查找编码错误；在正常操作中很少是正确的设置 |
...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 会优先使用它们，因为这些凭证可以自动刷新。**针对 Anthropic 的 OAuth 需要 Claude Max 计划并购买额外的使用额度** —— Hermes 以 Claude Code 身份路由，这只会从 Max 计划的额外/超额额度中扣除，而不是基础 Max 额度，并且在 Claude Pro 上无效。如果没有 Max 计划 + 额外额度，请改用 API 密钥。环境变量如 `ANTHROPIC_TOKEN` 仍然可以作为手动覆盖使用，但它们不再是 Claude Max 登录的首选方式。
...
| `HINDSIGHT_TIMEOUT` | Hindsight 记忆提供商 API 调用的超时时间（秒）（默认：`60`）。如果你的 Hindsight 实例在 `/sync` 或 `on_session_switch` 期间响应缓慢，并且在 `errors.log` 中看到超时，请增加此值。 |
...
| `DISCORD_ALLOWED_ROLES` | 允许使用机器人的逗号分隔的 Discord 角色 ID（与 `DISCORD_ALLOWED_USERS` 为 OR 关系）。自动启用 Members 意图。当审核团队变动时很有用——角色授权会自动传播。 |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信**群聊 ID**（非成员用户 ID）。变量名是历史遗留——它期望群组 ID。仅当 iLink 实际传递群组事件时生效；二维码登录的 iLink 机器人身份（`...@im.bot`）通常不会接收普通的微信群组消息。 |
...
| `MATTERMOST_FREE_RESPONSE_CHANNELS` | 机器人响应无需 `@提及` 的逗号分隔的频道 ID |
...
| `MATRIX_DEVICE_ID` | 用于跨重启持久化 E2EE 的稳定 Matrix 设备 ID（例如 `HERMES_BOT`）。没有此设置，E2EE 密钥每次启动都会轮换，历史房间解密会中断。 |
| `MATRIX_REACTIONS` | 在入站消息上启用处理周期表情符号反应（默认：`true`）。设置为 `false` 以禁用。 |
| `MATRIX_REQUIRE_MENTION` | 在房间中需要 `@提及`（默认：`true`）。设置为 `false` 以响应所有消息。 |
| `MATRIX_FREE_RESPONSE_ROOMS` | 机器人响应无需 `@提及` 的逗号分隔的房间 ID |
...
| `API_SERVER_HOST` | API 服务器的主机/绑定地址（默认：`127.0.0.1`）。使用 `0.0.0.0` 用于网络访问——需要 `API_SERVER_KEY` 和狭窄的 `API_SERVER_CORS_ORIGINS` 允许列表。 |
...
即将推出的 Teams 会议摘要流水线所使用的 Microsoft Graph REST 客户端的仅应用凭据。有关 Azure 门户的分步指南和所需的确切 API 权限，请参阅[注册 Microsoft Graph 应用程序](/docs/guides/microsoft-graph-app-registration)。
...
| `MSGRAPH_AUTHORITY_URL` | Microsoft 标识平台授权机构（默认值：`https://login.microsoftonline.com`）。仅在国家/主权云（例如 GCC High 的 `https://login.microsoftonline.us`）中需要覆盖。 |
...
| `LINE_ALLOWED_USERS` | 允许与机器人进行私聊的逗号分隔用户 ID（以 `U` 为前缀）。 |
...
| `LINE_SLOW_RESPONSE_THRESHOLD` | 慢速 LLM 模板按钮回传触发前的秒数（默认值：`45`）。设置为 `0` 以禁用并始终回退到推送。 |
...
| `LINE_INTERRUPTED_TEXT` | 当点击被 `/stop` 中断的回传按钮时的回复（默认值：`Run was interrupted before completion.`）。 |

### 高级消息调优

用于限制出站消息批处理程序的高级平台特定调节选项。大多数用户永远不需要调整这些；默认值设置为在尊重每个平台速率限制的同时不感觉迟钝。

| 变量 | 描述 |
|----------|-------------|
| `HERMES_TELEGRAM_TEXT_BATCH_DELAY_SECONDS` | 刷新排队的 Telegram 文本块前的宽限窗口（默认值：`0.6`）。 |
| `HERMES_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS` | 当单个 Telegram 消息超过长度限制时，分割块之间的延迟（默认值：`2.0`）。 |
| `HERMES_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS` | 刷新排队的 Telegram 媒体前的宽限窗口（默认值：`0.6`）。 |
...
| `HERMES_DISCORD_TEXT_BATCH_DELAY_SECONDS` | 刷新排队的 Discord 文本块前的宽限窗口（默认值：`0.6`）。 |
...
| `HERMES_VISION_DOWNLOAD_TIMEOUT` | 将图像交给视觉模型前下载的超时时间（秒）（默认值：`30`）。 |
| `HERMES_RESTART_DRAIN_TIMEOUT` | 消息网关：在强制重启前等待活动运行排空的秒数（默认值：`900`）。 |
...
| `HERMES_FILE_MUTATION_VERIFIER` | 启用每轮文件变更验证器页脚（默认值：`true`）。启用后，Hermes 会附加一个提示列表，列出在当前轮次中失败且未被成功写入覆盖的任何 `write_file` / `patch` 调用。设置为 `0`、`false`、`no` 或 `off` 以抑制。镜像 `config.yaml` 中的 `display.file_mutation_verifier`；设置环境变量时优先。 |
| `HERMES_CRON_TIMEOUT` | 定时任务 Agent 运行的不活动超时时间（秒）（默认值：`600`）。Agent 在主动调用工具或接收流 Token 时可以无限期运行——此设置仅在空闲时触发。设置为 `0` 表示无限制。 |
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到定时任务的预运行脚本的超时时间（秒）（默认值：`120`）。为需要更长时间执行的脚本（例如，用于反机器人定时的随机延迟）覆盖。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
| `HERMES_API_CALL_STALE_TIMEOUT` | 非流式陈旧调用超时时间（秒）（默认：`300`）。对于本地提供商，如果未设置则自动禁用。也可通过 `config.yaml` 中的 `providers.<id>.stale_timeout_seconds` 或 `providers.<id>.models.<model>.stale_timeout_seconds` 配置。 |
...
| `HERMES_TUI_RESUME` | 启动时按 ID 恢复特定的 TUI 会话。设置后，`hermes --tui` 会跳过创建新会话，转而拾取指定名称的会话 —— 这在断开连接或终端崩溃后重新连接时很有用。 |

...
| `MINIMAX_CN_BASE_URL` | 覆盖 MiniMax 中国区基础 URL (默认: `https://api.minimaxi.com/anthropic`) |
...
| `HERMES_MODEL` | 在进程级别覆盖模型名称（由定时任务调度器使用；正常使用建议使用 `config.yaml`） |
...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 会优先使用它们，因为这些凭证可以自动刷新。像 `ANTHROPIC_TOKEN` 这样的环境变量作为手动覆盖仍然有用，但它们不再是 Claude Pro/Max 登录的首选方式。
...
对于云沙盒后端，持久性是面向文件系统的。`TERMINAL_LIFETIME_SECONDS` 控制 Hermes 何时清理空闲的终端会话，后续的恢复可能会重新创建沙盒，而不是保持相同的活动进程运行。
## SSH 后端
...
| `TELEGRAM_ALLOWED_USERS` | 允许使用机器人的用户 ID，以逗号分隔 |
...
| `TELEGRAM_IGNORED_THREADS` | 机器人永不响应的 Telegram 论坛主题/线程 ID，以逗号分隔 |
| `TELEGRAM_PROXY` | Telegram 连接的代理 URL —— 覆盖 `HTTPS_PROXY`。支持 `http://`、`https://`、`socks5://` |
| `DISCORD_BOT_TOKEN` | Discord 机器人 Token |
| `DISCORD_ALLOWED_USERS` | 允许使用机器人的 Discord 用户 ID，以逗号分隔 |
| `DISCORD_ALLOWED_ROLES` | 允许使用机器人的 Discord 角色 ID，以逗号分隔（与 `DISCORD_ALLOWED_USERS` 为 OR 关系）。自动启用 Members 意图。适用于审核团队变动时 —— 角色授权会自动传播。 |
| `DISCORD_ALLOWED_CHANNELS` | Discord 频道 ID，以逗号分隔。设置后，机器人仅在这些频道中响应（如果允许，还包括私信）。覆盖 `config.yaml` 中的 `discord.allowed_channels`。 |
...
| `DISCORD_FREE_RESPONSE_CHANNELS` | 不需要提及的频道 ID，以逗号分隔 |
| `DISCORD_AUTO_THREAD` | 在支持时自动为长回复创建线程 |
| `DISCORD_REACTIONS` | 在处理消息时启用表情符号反应（默认：`true`） |
| `DISCORD_IGNORED_CHANNELS` | 机器人永不响应的频道 ID，以逗号分隔 |
| `DISCORD_NO_THREAD_CHANNELS` | 机器人响应时不自动创建线程的频道 ID，以逗号分隔 |
...
| `SLACK_ALLOWED_USERS` | Slack 用户 ID，以逗号分隔 |
...
| `WHATSAPP_ALLOWED_USERS` | 允许的电话号码（带国家代码，无 `+`），以逗号分隔，或 `*` 允许所有发送者 |
...
| `SIGNAL_ALLOWED_USERS` | E.164 电话号码或 UUID，以逗号分隔 |
| `SIGNAL_GROUP_ALLOWED_USERS` | 群组 ID，以逗号分隔，或 `*` 表示所有群组 |
...
| `SMS_ALLOWED_USERS` | 允许聊天的 E.164 电话号码，以逗号分隔 |
...
| `EMAIL_ALLOWED_USERS` | 允许向机器人发送消息的邮箱地址，以逗号分隔 |
| `EMAIL_HOME_ADDRESS` | 主动电子邮件交付的默认收件人 |
| `EMAIL_HOME_ADDRESS_NAME` | 电子邮件主目标的显示名称 |
| `EMAIL_POLL_INTERVAL` | 电子邮件轮询间隔，单位秒 |
...
| `DINGTALK_ALLOWED_USERS` | 允许向机器人发送消息的钉钉用户 ID，以逗号分隔 |
...
| `FEISHU_ALLOWED_USERS` | 允许向机器人发送消息的飞书用户 ID，以逗号分隔 |
...
| `WECOM_ALLOWED_USERS` | 允许向机器人发送消息的企业微信用户 ID，以逗号分隔 |
...
| `WECOM_CALLBACK_ALLOWED_USERS` | 允许列表的用户 ID，以逗号分隔 |
...
| `WEIXIN_ALLOWED_USERS` | 允许私信机器人的微信用户 ID，以逗号分隔 |
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的微信群组 ID，以逗号分隔 |
...
| `BLUEBUBBLES_ALLOWED_USERS` | 授权用户，以逗号分隔 |
...
| `QQ_ALLOWED_USERS` | 允许向机器人发送消息的 QQ 用户 openID，以逗号分隔 |
| `QQ_GROUP_ALLOWED_USERS` | 用于群组 @-消息访问的 QQ 群组 ID，以逗号分隔 |
...
| `MATRIX_ALLOWED_USERS` | 允许向机器人发送消息的 Matrix 用户 ID，以逗号分隔（例如 `@alice:matrix.org`） |
...
| `MATRIX_FREE_RESPONSE_ROOMS` | 机器人无需 `@提及` 即可响应的房间 ID，以逗号分隔 |
...
| `API_SERVER_CORS_ORIGINS` | 允许直接调用 API 服务器的浏览器来源，以逗号分隔（例如 `http://localhost:3000,http://127.0.0.1:3000`）。默认：禁用。 |
...
| `API_SERVER_MODEL_NAME` | 在 `/v1/models` 上公布的模型名称。默认为配置文件名称（或默认配置文件的 `hermes-agent`）。适用于多用户设置，其中像 Open WebUI 这样的前端需要每个连接有不同的模型名称。 |
...
| `GATEWAY_ALLOWED_USERS` | 允许跨所有平台的用户 ID，以逗号分隔 |
...
| `HERMES_STREAM_READ_TIMEOUT` | 流式套接字读取超时时间（秒）（默认：`120`）。对于本地提供商，自动增加到 `HERMES_API_TIMEOUT`。如果本地 LLM 在生成长代码时超时，请增加此值。 |
| `HERMES_STREAM_STALE_TIMEOUT` | 陈旧流检测超时时间（秒）（默认：`180`）。对本地提供商自动禁用。如果在此时间窗口内没有收到数据块，则触发连接终止。 |
...
| `DELEGATION_MAX_CONCURRENT_CHILDREN` | 每个 `delegate_task` 批次的最大并行子 Agent 数（默认：`3`，下限为 1，无上限）。也可通过 `config.yaml` 中的 `delegation.max_concurrent_children` 配置 — 配置文件值优先。 |

## 界面

| 变量 | 描述 |
|----------|-------------|
| `HERMES_TUI` | 当设置为 `1` 时，启动 [TUI](../user-guide/tui.md) 而不是经典 CLI。等同于传递 `--tui` 参数。 |
| `HERMES_TUI_DIR` | 预构建的 `ui-tui/` 目录的路径（必须包含 `dist/entry.js` 和已填充的 `node_modules`）。由发行版和 Nix 使用，以跳过首次启动时的 `npm install`。 |
...
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到定时任务的预运行脚本的超时时间（秒）（默认：`120`）。对于需要更长时间执行的脚本（例如，用于反机器人计时的随机延迟）进行覆盖。也可通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
:::info 旧版迁移
包含 `compression.summary_model`、`compression.summary_provider` 和 `compression.summary_base_url` 的旧配置文件会在首次加载时自动迁移到 `auxiliary.compression.*`。
:::
...
有关完整详细信息，请参阅 [备用提供商](/docs/user-guide/features/fallback-providers)。
...
:::提示
使用 `hermes config set` 来设置环境变量——它会自动将变量保存到正确的文件中（`.env` 文件用于保存密钥，`config.yaml` 文件用于保存其他所有配置）。
:::
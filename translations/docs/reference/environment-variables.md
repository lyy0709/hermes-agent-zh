...
| `CLAUDE_CODE_OAUTH_TOKEN` | 如果你手动导出了一个 Token，用于显式覆盖 Claude Code Token |
...
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到定时任务的预运行脚本的超时时间（秒）（默认：`120`）。对于需要更长时间执行的脚本（例如，用于反机器人定时的随机延迟），可以覆盖此值。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 进行配置。 |

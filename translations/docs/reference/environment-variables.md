...
| `GH_TOKEN` | GitHub Token — Copilot 的第二优先级（也被 `gh` CLI 使用） |
...
| `DISCORD_FREE_RESPONSE_CHANNELS` | 不需要 @提及 的频道 ID，逗号分隔 |
...
| `WHATSAPP_ALLOWED_USERS` | 允许的电话号码（带国家代码，无 `+`），逗号分隔，或 `*` 允许所有发送者 |
...
| `QQ_GROUP_ALLOWED_USERS` | 用于群组 @-消息 访问的 QQ 群组 ID，逗号分隔 |
...
| `HERMES_API_CALL_STALE_TIMEOUT` | 非流式陈旧调用超时时间（秒）（默认：`300`）。未设置时，对于本地提供商会自动禁用。也可通过 `config.yaml` 中的 `providers.<id>.stale_timeout_seconds` 或 `providers.<id>.models.<model>.stale_timeout_seconds` 配置。 |
...
| 变量 | 描述 |
|----------|-------------|
| `HERMES_TUI` | 当设置为 `1` 时，启动 [TUI](../user-guide/tui.md) 而不是经典 CLI。等同于传递 `--tui`。 |
...
主模型备用配置仅通过 `config.yaml` 配置——没有对应的环境变量。添加一个包含 `provider` 和 `model` 键的 `fallback_model` 部分，以在主模型遇到错误时启用自动故障转移。

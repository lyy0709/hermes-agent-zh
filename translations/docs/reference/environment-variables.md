...
| `CLAUDE_CODE_OAUTH_TOKEN` | 如果你手动导出了一个令牌，用于显式覆盖 Claude Code token |
...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 会优先使用它们，因为这些凭证可以自动刷新。**针对 Anthropic 的 OAuth 需要 Claude Max 计划并购买额外的使用额度** —— Hermes 会以 Claude Code 的身份路由请求，这只会消耗 Max 计划的额外/超额额度，而不是基础 Max 额度，并且在 Claude Pro 计划上无效。如果没有 Max 计划 + 额外额度，请改用 API 密钥。像 `ANTHROPIC_TOKEN` 这样的环境变量仍然可以作为手动覆盖使用，但它们不再是 Claude Max 登录的首选方式。
...
用于捆绑的 [`observability/langfuse`](/docs/user-guide/features/built-in-plugins#observabilitylangfuse) 插件的环境变量。通过 `hermes tools → Langfuse Observability` 或在 `~/.hermes/.env` 中手动设置这些变量。在生效之前，还必须启用该插件（`hermes plugins enable observability/langfuse`）。
...
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` / `LANGFUSE_BASE_URL` | 标准 Langfuse SDK 名称。当等效的 `HERMES_LANGFUSE_*` 变量未设置时，作为备用选项接受。 |
### Nous 工具网关
...
| `DISCORD_NO_THREAD_CHANNELS` | 机器人响应但不自动线程化的逗号分隔的频道 ID |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信**群聊 ID**（非成员用户 ID）。变量名是历史遗留——它期望群组 ID。仅当 iLink 实际传递群组事件时才生效；二维码登录的 iLink 机器人身份（`...@im.bot`）通常不会接收普通的微信群消息。 |
...
| `MSGRAPH_SCOPE` | 客户端凭据 Token 请求的 OAuth2 范围（默认值：`https://graph.microsoft.com/.default`）。 |
...
用于限制出站消息批量处理程序的高级平台特定旋钮。大多数用户永远不需要调整这些；默认值设置为尊重每个平台的速率限制，同时不会感觉迟缓。
...
| `HERMES_MATRIX_TEXT_BATCH_DELAY_SECONDS` / `_SPLIT_DELAY_SECONDS` | Matrix 对应的批量处理旋钮。 |
| `HERMES_FEISHU_TEXT_BATCH_DELAY_SECONDS` / `_SPLIT_DELAY_SECONDS` / `_MAX_CHARS` / `_MAX_MESSAGES` | 飞书批量处理程序调优——延迟、分割延迟、每条消息最大字符数、每批最大消息数。 |
| `HERMES_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 飞书媒体刷新延迟。 |
| `HERMES_FEISHU_DEDUP_CACHE_SIZE` | 飞书 Webhook 去重缓存的大小（默认值：`1024`）。 |
| `HERMES_WECOM_TEXT_BATCH_DELAY_SECONDS` / `_SPLIT_DELAY_SECONDS` | 企业微信批量处理程序调优。 |
...
| `HERMES_API_CALL_STALE_TIMEOUT` | 非流式陈旧调用超时时间（秒）（默认：`300`）。如果未设置，对于本地提供商将自动禁用。也可通过 `config.yaml` 中的 `providers.<id>.stale_timeout_seconds` 或 `providers.<id>.models.<model>.stale_timeout_seconds` 配置。 |
...
| `HERMES_AGENT_NOTIFY_INTERVAL` | 消息网关：长时间运行的 Agent 回合之间进度通知的间隔（秒）。 |
...
| `HERMES_TUI_RESUME` | 启动时按 ID 恢复特定的 TUI 会话。设置后，`hermes --tui` 会跳过创建新会话，而是拾取指定会话 — 适用于在断开连接或终端崩溃后重新连接。 |

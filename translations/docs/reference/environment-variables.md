...
| `AZURE_ANTHROPIC_KEY` | 当 `provider: anthropic` 且 `base_url` 指向 Azure Foundry Claude 部署时，用于 Azure Anthropic 的 API 密钥（当同时配置了 Anthropic 和 Azure Anthropic 时，作为 `ANTHROPIC_API_KEY` 的替代） |
...
| `BEDROCK_BASE_URL` | 覆盖 Bedrock 运行时基础 URL（默认：`https://bedrock-runtime.us-east-1.amazonaws.com`；通常留空，使用 `AWS_REGION` 代替） |
...
| `CLAUDE_CODE_OAUTH_TOKEN` | 如果手动导出了令牌，用于显式覆盖 Claude Code 令牌 |
...
| `HERMES_NOUS_INFERENCE_BASE_URL` | 覆盖 Nous 推理 API URL |
...
| `HERMES_DOCKER_BINARY` | 覆盖 Hermes 调用的容器二进制文件（例如 `podman`、`/usr/local/bin/docker`）。未设置时，Hermes 自动在 `PATH` 上发现 `docker` 或 `podman`。当两者都安装且您想要非默认的二进制文件，或者二进制文件位于 `PATH` 之外时需要。 |
...
| `TELEGRAM_WEBHOOK_SECRET` | Telegram 在每个更新中回显用于验证的密钥 Token。**只要设置了 `TELEGRAM_WEBHOOK_URL` 就必需**——没有它网关拒绝启动（GHSA-3vpc-7q5r-276h）。使用 `openssl rand -hex 32` 生成。 |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信**群聊 ID**（非成员用户 ID）。变量名是遗留的——它期望群组 ID。仅当 iLink 实际传递群组事件时生效；扫码登录的 iLink 机器人身份（`...@im.bot`）通常不会接收普通微信群消息。 |
...
| `MATRIX_ACCESS_TOKEN` | 机器人认证的 Matrix 访问 Token |
...
| `HERMES_RESTART_DRAIN_TIMEOUT` | 消息网关：在 `/restart` 命令后等待活动运行任务排空的时间（秒），之后强制重启（默认：`900`）。 |
...
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到定时任务的预运行脚本的超时时间（秒）（默认：`120`）。为需要更长时间执行的脚本（例如，用于反机器人定时的随机延迟）覆盖此设置。也可通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
| `HERMES_AGENT_TIMEOUT` | 消息网关中正在运行的 Agent 的不活动超时时间（秒）（默认：`900`）。每次工具调用和流式 Token 都会重置。设置为 `0` 以禁用。 |
...
主模型备用配置仅通过 `config.yaml` 进行 —— 没有对应的环境变量。添加一个包含 `provider` 和 `model` 键的 `fallback_model` 部分，以在主模型遇到错误时启用自动故障转移。

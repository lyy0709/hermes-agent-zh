...
| `IDENTITY_ENDPOINT` / `MSI_ENDPOINT` | App Service、Functions 和 Container Apps 的托管身份端点；VM 通常使用 IMDS 且不设置这些 |
...
| `CLAUDE_CODE_OAUTH_TOKEN` | 如果你手动导出了一个，则显式覆盖 Claude Code 令牌 |
...
| `HERMES_DISABLE_WINDOWS_UTF8` | **仅限 Windows。** 设置为 `1` 以禁用 UTF-8 stdio 垫片 (`configure_windows_stdio()`) 并回退到控制台的区域设置代码页。用于二分查找编码错误；正常操作中很少是正确设置 |
...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 优先使用它们，因为这些凭证可以自动刷新。**针对 Anthropic 的 OAuth 需要 Claude Max 计划并购买额外的使用额度** —— Hermes 以 Claude Code 的身份路由，这只会从 Max 计划的额外/超额额度中扣除，而不是基础 Max 额度，并且在 Claude Pro 上无效。如果没有 Max 计划 + 额外额度，请改用 API 密钥。环境变量如 `ANTHROPIC_TOKEN` 作为手动覆盖仍然有用，但它们不再是 Claude Max 登录的首选方式。
...
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` / `LANGFUSE_BASE_URL` | 标准 Langfuse SDK 名称。当未设置等效的 `HERMES_LANGFUSE_*` 变量时，作为备用值接受。 |
### Nous 工具网关
...
| `TELEGRAM_WEBHOOK_SECRET` | Telegram 在每个更新中回显以进行验证的密钥 Token。**只要设置了 `TELEGRAM_WEBHOOK_URL` 就必须提供**——没有它网关将拒绝启动 (GHSA-3vpc-7q5r-276h)。使用 `openssl rand -hex 32` 生成。 |
...
| `TELEGRAM_EXCLUSIVE_BOT_MENTIONS` | 启用后，Telegram 群组中显式的 `@...bot` 提及在回复或唤醒词回退运行之前，仅路由到提及的机器人用户名。默认：`true`。等同于 `telegram.exclusive_bot_mentions`。 |
...
| `LINE_INTERRUPTED_TEXT` | 当点击被 `/stop` 中断的回发按钮时的回复（默认值：`Run was interrupted before completion.`）。 |
...
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到定时任务的预运行脚本的超时时间（秒）（默认：`120`）。为需要更长时间执行的脚本（例如，用于反机器人定时的随机延迟）覆盖此设置。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
| `HERMES_STREAM_RETRIES` | 在发生瞬态网络错误时，流中重新连接尝试的次数（默认：`3`）。 |
...
| `HERMES_AGENT_NOTIFY_INTERVAL` | 消息网关：长时间运行的 Agent 轮次之间，进度通知的间隔秒数。 |
...
| `HERMES_SESSION_ID` | **自动导出到 Hermes 生成的每个工具子进程**（`terminal`、`execute_code`、持久化 shell、Docker/Singularity 后端、委派的子 Agent 运行）。由 Agent 设置为当前会话 ID；从工具调用的用户脚本可以读取此变量，以将其输出、遥测数据或副作用与原始的 Hermes 会话关联起来。**您不应手动设置此变量** —— 从父 shell 覆盖它仅在 Agent 运行之外生效，并且在 Agent 启动会话时会被覆盖。 |

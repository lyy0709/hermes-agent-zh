...
| `IDENTITY_ENDPOINT` / `MSI_ENDPOINT` | App Service、Functions 和 Container Apps 的托管身份端点；VM 通常使用 IMDS 而不设置这些 |
...
| `AWS_PROFILE` | AWS 命名配置文件，用于 Bedrock 身份验证（读取 `~/.aws/credentials`）。留空以使用默认的 boto3 凭据链。 |
...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 优先使用它们，因为这些凭证可以自动刷新。**针对 Anthropic 的 OAuth 需要一个 Claude Max 计划并购买了额外的使用额度** —— Hermes 以 Claude Code 身份路由，这只会从 Max 计划的额外/超额额度中扣除，而不是基础 Max 额度，并且在 Claude Pro 上无效。如果没有 Max 计划 + 额外额度，请改用 API 密钥。环境变量如 `ANTHROPIC_TOKEN` 作为手动覆盖仍然有用，但它们不再是 Claude Max 登录的首选方式。
...
| `TAVILY_BASE_URL` | 覆盖 Tavily API 端点。适用于企业代理和自托管的 Tavily 兼容搜索后端。与 `GROQ_BASE_URL` 模式相同。 |
...
| `AGENT_BROWSER_ARGS` | 额外的 Chromium 启动标志（逗号或换行分隔）。当以 root 身份运行或在受 AppArmor 限制的无特权用户命名空间（Ubuntu 23.10+、DGX Spark、许多容器镜像）中运行时，Hermes 会自动注入 `--no-sandbox,--disable-dev-shm-usage`；仅在需要覆盖或添加其他标志时手动设置此变量。 |
...
用于捆绑的 [`observability/langfuse`](/user-guide/features/built-in-plugins#observabilitylangfuse) 插件的环境变量。在 `~/.hermes/.env` 中设置这些变量。在它们生效之前，还必须启用该插件（`hermes plugins enable observability/langfuse`，或在 `hermes plugins` 中勾选复选框）。
...
| `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` / `LANGFUSE_BASE_URL` | 标准 Langfuse SDK 名称。当等效的 `HERMES_LANGFUSE_*` 变量未设置时，作为备用选项被接受。 |
### Nous 工具网关
...
| `TERMINAL_DOCKER_MOUNT_CWD_TO_WORKSPACE` | 高级选择加入：将启动时的当前工作目录挂载到 Docker `/workspace`（`true`/`false`，默认：`false`） |
...
| `DISCORD_ALLOW_ANY_ATTACHMENT` | 为 `true` 时，接受任何文件类型的附件（不仅仅是内置的 PDF/文本/zip/办公软件白名单）。未知类型会被缓存并作为本地路径提供给 Agent，以便其通过 `terminal` / `read_file` / `ffprobe` 进行检查。默认 `false`。 |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信**群聊 ID**（非成员用户 ID）。变量名是历史遗留——它期望群组 ID。仅当 iLink 实际传递群组事件时生效；二维码登录的 iLink 机器人身份（`...@im.bot`）通常不会接收普通的微信群消息。 |
...
| `API_SERVER_KEY` | API 服务器认证的 Bearer Token。只要启用了 API 服务器就必需。 |
...
| `API_SERVER_HOST` | API 服务器的主机/绑定地址（默认：`127.0.0.1`）。回环地址上仍需要 `API_SERVER_KEY`；使用狭窄的 `API_SERVER_CORS_ORIGINS` 允许列表进行浏览器访问。 |
| `API_SERVER_MODEL_NAME` | 在 `/v1/models` 上公布的模型名称。默认为配置文件名称（或默认配置文件的 `hermes-agent`）。对于多用户设置很有用，其中像 Open WebUI 这样的前端每个连接需要不同的模型名称。 |
...
用于 [Web 仪表盘](/user-guide/features/web-dashboard) 身份验证以及 [将 Hermes Desktop 连接到远程后端](/user-guide/features/web-dashboard#connecting-hermes-desktop-to-a-remote-backend)。根据仅存储密钥的惯例，凭证应存放在 `~/.hermes/.env` 中；OAuth 的 `client_id` 更适合设置在 `config.yaml` 的 `dashboard.oauth` 下（若同时设置，环境变量优先）。

系统内置了三种仪表盘身份验证提供商。对于远程 Hermes Desktop 连接或任何面向互联网的仪表盘，推荐的提供商是 **OAuth (Nous Portal)** —— 设置 `HERMES_DASHBOARD_OAUTH_CLIENT_ID`（可通过 `hermes dashboard register` 命令获取）。捆绑的 **用户名/密码** 提供商 (`HERMES_DASHBOARD_BASIC_AUTH_*`) 是在受信任的局域网或 VPN 后端的后端上最快的选项，但不适合直接暴露在公共互联网上。若需对接到您自己的身份提供商，请使用 **自托管 OIDC** 提供商 (`HERMES_DASHBOARD_OIDC_*`)。无论哪种方式，非回环地址绑定 (`hermes dashboard --host 0.0.0.0`) 都会启用身份验证网关。完整说明请参阅 [Web 仪表盘 → 身份验证](/user-guide/features/web-dashboard#authentication-gated-mode)。
...
| `HERMES_DASHBOARD_BASIC_AUTH_SECRET` | 用于签署基本提供商无状态会话令牌的 HMAC 密钥（32+ 字节，base64/hex/raw）。显式设置以确保会话在重启后保留 / 跨多个工作进程；为空 → 每个进程随机生成（每次重启都会退出登录）。覆盖 `dashboard.basic_auth.secret`。 |
| `HERMES_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | 基本提供商的访问令牌有效期（默认 12 小时）。覆盖 `dashboard.basic_auth.session_ttl_seconds`。 |
| `HERMES_DASHBOARD_OAUTH_CLIENT_ID` | 受保护/公共仪表盘的 OAuth 客户端 ID (`agent:{instance_id}`)，用于激活 Nous (`plugins/dashboard_auth/nous`) 提供商。覆盖 `dashboard.oauth.client_id`。通过 `hermes dashboard register` 命令获取。 |
| `HERMES_DASHBOARD_PUBLIC_URL` | 仪表盘可访问的完整公共 URL，用于在反向代理后构建 OAuth 回调。覆盖 `dashboard.public_url`。 |
...
| `HERMES_DESKTOP_REMOTE_URL` | （Desktop 端）远程后端的基础 URL，例如 `http://host:9119`。设置后，覆盖应用内的消息网关 URL；您仍需从消息网关设置面板登录（OAuth 重定向或用户名/密码，取决于后端通告的方式）。 |

### Microsoft Graph (Teams 会议)

即将推出的 Teams 会议摘要流水线所使用的 Microsoft Graph REST 客户端的仅应用凭证。有关 Azure 门户操作指南和所需的确切 API 权限，请参阅 [注册 Microsoft Graph 应用程序](/guides/microsoft-graph-app-registration)。
...
| `MSGRAPH_AUTHORITY_URL` | Microsoft 身份平台颁发机构（默认：`https://login.microsoftonline.com`）。仅在国家/主权云中需要覆盖（例如，GCC High 使用 `https://login.microsoftonline.us`）。 |
...
| `MSGRAPH_WEBHOOK_ALLOWED_SOURCE_CIDRS` | 允许向监听器发送 POST 请求的逗号分隔的 CIDR 范围（例如 `52.96.0.0/14,52.104.0.0/14`）。为空 = 允许所有（默认）。在生产环境中应限制为 Microsoft Graph 发布的出口范围。 |

### Teams 会议摘要投递

仅在启用 [`teams_pipeline` 插件](/user-guide/messaging/msgraph-webhook) 时使用。这些设置也可以在 `config.yaml` 的 `platforms.teams.extra` 下配置——若两者都设置，环境变量优先。请参阅 [Microsoft Teams → 会议摘要投递](/user-guide/messaging/teams#meeting-summary-delivery-teams-meeting-pipeline)。
| 变量 | 描述 |
|----------|-------------|
...
由捆绑的 LINE 平台插件 (`plugins/platforms/line/`) 使用。完整设置请参阅 [消息网关 → LINE](/user-guide/messaging/line)。
...
| `LINE_ALLOWED_USERS` | 允许与机器人私聊的用户 ID 列表 (以 `U` 开头)，用逗号分隔。 |
| `LINE_ALLOWED_GROUPS` | 机器人将响应的群组 ID 列表 (以 `C` 开头)，用逗号分隔。 |
| `LINE_ALLOWED_ROOMS` | 机器人将响应的聊天室 ID 列表 (以 `R` 开头)，用逗号分隔。 |
| `LINE_ALLOW_ALL_USERS` | 仅用于开发的应急开关 —— 接受任何来源。默认: `false`。 |
| `LINE_HOME_CHANNEL` | 带有 `deliver: line` 的定时任务的默认交付目标。 |
| `LINE_SLOW_RESPONSE_THRESHOLD` | 触发慢速 LLM 模板按钮回传前的秒数 (默认: `45`)。设置为 `0` 以禁用并始终回退到 Push。 |
...
用于限制出站消息批处理器的各平台高级调节选项。大多数用户永远不需要调整这些；默认值设置为尊重每个平台的速率限制，同时不会感觉迟钝。
...
| `HERMES_TELEGRAM_FOLLOWUP_GRACE_SECONDS` | Agent 完成后发送后续消息之前的延迟，以避免与最后一个流块竞争。 |
...
| `HERMES_VISION_DOWNLOAD_TIMEOUT` | 在将图像交给视觉模型之前下载的超时时间 (秒) (默认: `30`)。 |
| `HERMES_RESTART_DRAIN_TIMEOUT` | 消息网关: 在 `/restart` 时等待活动运行排空的时间 (秒)，然后强制重启 (默认: `900`)。 |
...
| `HERMES_GATEWAY_BOOTSTRAP_STATE` | 在 s6-overlay Docker 镜像内部，声明消息网关在全新卷上的 **初始** 监管状态。在空白卷上没有持久化的 `gateway_state.json`，因此启动协调器会注册 `gateway-default` 槽位但将其保持为 **down** 状态 (只有当最后记录的状态是 `running` 时才会自动启动)。将其设置为 `running`，首次启动设置钩子会在协调器运行 *之前* 植入 `gateway_state.json`，因此消息网关在首次启动时就会启动。只有字面值 `running` 会被识别。仅首次启动有效：现有的 `gateway_state.json` 永远不会被覆盖，因此故意停止的消息网关在重启后会保持停止状态。在 s6 镜像外部无效。 |
| `HERMES_FILE_MUTATION_VERIFIER` | 启用每轮文件变更验证器页脚 (默认: `true`)。启用后，Hermes 会附加一个提示性列表，列出本轮中任何失败且未被成功写入覆盖的 `write_file` / `patch` 调用。设置为 `0`、`false`、`no` 或 `off` 以抑制。镜像 `config.yaml` 中的 `display.file_mutation_verifier`；当设置时，环境变量优先。 |
| `HERMES_CRON_TIMEOUT` | 定时任务 Agent 运行的不活动超时时间 (秒) (默认: `600`)。Agent 在主动调用工具或接收流 Token 时可以无限期运行 —— 此设置仅在空闲时触发。设置为 `0` 表示无限制。 |
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到定时任务的预运行脚本的超时时间 (秒) (默认: `120`)。覆盖需要更长时间执行的脚本 (例如，用于反机器人计时的随机延迟)。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
| `CODEX_HOME` | 当启用 [Codex 应用服务器运行时](../user-guide/features/codex-app-server-runtime) 时，覆盖 Codex CLI 读取其配置和认证的目录（默认：`~/.codex`）。Hermes 的迁移会将托管块写入 `<CODEX_HOME>/config.toml`。 |
...
| `HERMES_API_CALL_STALE_TIMEOUT` | 非流式陈旧调用超时时间（秒）（默认：`300`）。如果留空，对于本地提供商会自动禁用。也可以通过 `config.yaml` 中的 `providers.<id>.stale_timeout_seconds` 或 `providers.<id>.models.<model>.stale_timeout_seconds` 配置。 |
| `HERMES_STREAM_READ_TIMEOUT` | 流式套接字读取超时时间（秒）（默认：`120`）。对于本地提供商，会自动增加到 `HERMES_API_TIMEOUT`。如果本地 LLM 在生成长代码时超时，请增加此值。 |
| `HERMES_STREAM_STALE_TIMEOUT` | 陈旧流检测超时时间（秒）（默认：`180`）。对于本地提供商自动禁用。如果在此时间窗口内没有数据块到达，则触发连接终止。 |
| `HERMES_STREAM_RETRIES` | 在发生瞬态网络错误时，流中重新连接的尝试次数（默认：`3`）。 |
| `HERMES_AGENT_TIMEOUT` | 运行中 Agent 的消息网关不活动超时时间（秒）（默认：`900`）。每次工具调用和流式 Token 都会重置。设置为 `0` 以禁用。 |
...
| `HERMES_DISABLE_FILE_STATE_GUARD` | 设置为 `1` 以关闭 `patch`/`write_file` 上的“自你读取后文件已更改”防护。 |
| `HERMES_CORE_TOOLS` | 用于覆盖规范核心工具列表的逗号分隔列表（高级；很少需要）。 |
| `HERMES_BUNDLED_SKILLS` | 用于覆盖启动时加载的捆绑技能列表的逗号分隔列表。 |
| `HERMES_OPTIONAL_SKILLS` | 在首次运行时自动安装的可选技能名称的逗号分隔列表。 |
...
| `DELEGATION_MAX_CONCURRENT_CHILDREN` | 每个 `delegate_task` 批次的最大并行子 Agent 数（默认：`3`，下限为 1，无上限）。也可以通过 `config.yaml` 中的 `delegation.max_concurrent_children` 配置——配置值具有优先级。 |
## 界面
...
| `HERMES_INFERENCE_MODEL` | 在不修改 `config.yaml` 的情况下，强制指定 `hermes -z` / `hermes chat` 使用的模型。与 `--provider` 标志配对使用。对于需要每次运行覆盖默认模型的脚本化调用者（sweeper、CI、批量运行器）很有用。 |
...
| 变量 | 描述 |
|----------|-------------|
| `AUXILIARY_VISION_PROVIDER` | 覆盖视觉任务使用的提供商 |
...
| `AUXILIARY_WEB_EXTRACT_PROVIDER` | 覆盖网页提取/摘要任务使用的提供商 |
| `AUXILIARY_WEB_EXTRACT_MODEL` | 覆盖网页提取/摘要任务使用的模型 |
| `AUXILIARY_WEB_EXTRACT_BASE_URL` | 用于网页提取/摘要任务的直接 OpenAI 兼容端点 |
| `AUXILIARY_WEB_EXTRACT_API_KEY` | 与 `AUXILIARY_WEB_EXTRACT_BASE_URL` 配对的 API 密钥 |

对于特定任务的直接端点，Hermes 使用该任务配置的 API 密钥或 `OPENAI_API_KEY`。它不会为这些自定义端点重用 `OPENROUTER_API_KEY`。

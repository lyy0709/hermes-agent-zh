...
| `HERMES_GIT_BASH_PATH` | **仅限 Windows。** 覆盖终端工具的 `bash.exe` 发现路径。指向任何 bash — 完整的 Git-for-Windows 安装、通过符号链接的 WSL bash、MSYS2、Cygwin。安装程序会自动将其设置为其提供的 PortableGit。参见 [Windows（原生）指南](../user-guide/windows-native.md#how-hermes-runs-shell-commands-on-windows) |
...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 优先使用它们，因为这些凭证可以自动刷新。**针对 Anthropic 的 OAuth 需要一个购买了额外使用额度的 Claude Max 套餐** —— Hermes 以 Claude Code 的身份路由，这只会消耗 Max 套餐的额外/超额额度，而不是基础 Max 额度，并且在 Claude Pro 上无效。如果没有 Max 套餐 + 额外额度，请改用 API 密钥。环境变量如 `ANTHROPIC_TOKEN` 作为手动覆盖仍然有用，但它们不再是 Claude Max 登录的首选方式。
...
| `AGENT_BROWSER_ARGS` | 额外的 Chromium 启动标志（逗号或换行符分隔）。当以 root 身份运行或在受 AppArmor 限制的无特权用户命名空间（Ubuntu 23.10+、DGX Spark、许多容器镜像）中运行时，Hermes 会自动注入 `--no-sandbox,--disable-dev-shm-usage`；仅在需要覆盖或添加其他标志时手动设置此变量。 |
...
| `TELEGRAM_WEBHOOK_SECRET` | Telegram 在每个更新中回显用于验证的密钥令牌。**只要设置了 `TELEGRAM_WEBHOOK_URL` 就必须提供**——没有它网关将拒绝启动 (GHSA-3vpc-7q5r-276h)。使用 `openssl rand -hex 32` 生成。 |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信**群聊 ID**（非成员用户 ID）。变量名是旧版——它期望群组 ID。仅当 iLink 实际传递群组事件时才生效；扫码登录的 iLink 机器人身份（`...@im.bot`）通常不会接收普通的微信群消息。 |
...
| `API_SERVER_KEY` | API 服务器认证的 Bearer 令牌。只要启用了 API 服务器就必须提供。 |
...
| `LINE_SLOW_RESPONSE_THRESHOLD` | 触发慢速 LLM 模板按钮回传前的秒数（默认：`45`）。设置为 `0` 以禁用并始终回退到推送。 |
...
| `NTFY_ALLOW_ALL_USERS` | 仅用于开发的应急方案——仅在访问控制的私有主题上安全。默认：`false`。 |
| `NTFY_HOME_CHANNEL` | 带有 `deliver: ntfy` 的定时任务的默认投递目标。 |
| `NTFY_HOME_CHANNEL_NAME` | 主频道的人工标签（默认为主题名称）。 |
在使用不受信任的主题部署前，请参阅 [ntfy 消息指南](/user-guide/messaging/ntfy) —— 特别是 **身份模型** 部分。
...
| `HERMES_RESTART_DRAIN_TIMEOUT` | 消息网关：在 `/restart` 时等待活动运行任务排空的时间（秒），然后强制重启（默认：`900`）。 |
...
| `HERMES_GATEWAY_BOOTSTRAP_STATE` | 在 s6-overlay Docker 镜像内部，声明消息网关在全新卷上的**初始**监管状态。在空白卷上没有持久化的 `gateway_state.json`，因此启动协调器会注册 `gateway-default` 插槽但将其保持为 **down** 状态（仅当最后记录的状态是 `running` 时才会自动启动）。将其设置为 `running`，首次启动设置钩子会在协调器运行*之前*植入 `gateway_state.json`，这样消息网关在首次启动时就会启动。仅字面值 `running` 有效。仅首次启动：现有的 `gateway_state.json` 永远不会被覆盖，因此有意停止的消息网关在重启后保持停止状态。在 s6 镜像外部无效。 |
...
| `HERMES_AGENT_NOTIFY_INTERVAL` | 消息网关：长时间运行的 Agent 轮次之间进度通知的间隔（秒）。 |
...
上下文压缩仅通过 `config.yaml` 配置 —— 没有对应的环境变量。阈值设置在 `compression:` 块中，而摘要模型/提供商设置在 `auxiliary.compression:` 下。

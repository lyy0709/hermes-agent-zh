...
| `HERMES_HOME` | 覆盖 Hermes 配置目录（默认：`~/.hermes`）。同时限定消息网关 PID 文件和 systemd 服务名称，允许多个安装同时运行 |
...
| `HERMES_KANBAN_DISPATCH_IN_GATEWAY` | `kanban.dispatch_in_gateway` 的运行时覆盖。设置为 `0`、`false`、`no` 或 `off` 以阻止消息网关启动嵌入式看板调度器；任何其他非空值则启用它。当单独的调度器进程拥有看板时有用 |
## 提供商认证（OAuth）

对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 优先使用它们，因为这些凭证可以自动刷新。**针对 Anthropic 的 OAuth 需要 Claude Max 计划并购买额外的使用额度** —— Hermes 以 Claude Code 身份路由，这只会从 Max 计划的额外/超额额度中扣除，而不是基础 Max 额度，并且在 Claude Pro 上无效。如果没有 Max 计划 + 额外额度，请改用 API 密钥。环境变量如 `ANTHROPIC_TOKEN` 作为手动覆盖仍然有用，但它们不再是 Claude Max 登录的首选方式。
...
| `SUPERMEMORY_API_KEY` | 具有个人资料召回和会话摄取功能的语义长期记忆 ([supermemory.ai](https://supermemory.ai)) |
...
| `TERMINAL_CWD` | 已弃用，用于直接覆盖网关/cron 终端会话的工作目录。建议使用 `config.yaml` 中的 `terminal.cwd`；CLI 仍使用启动目录。 |
...
| `TELEGRAM_GROUP_ALLOWED_USERS` | 仅在群组/论坛中授权的逗号分隔的发送者用户 ID（**不**授予私聊访问权限）。为向后兼容 #17686 之前的配置，仍接受聊天 ID 格式的值（以 `-` 开头）作为聊天 ID，但会显示弃用警告。 |
...
| `GOOGLE_CHAT_BOOTSTRAP_SPACES` | 启动时解析机器人自身 `users/{id}` 时要探测的额外逗号分隔的空间 ID |
...
| `WEIXIN_GROUP_ALLOWED_USERS` | 允许与机器人交互的逗号分隔的微信**群聊 ID**（非成员用户 ID）。变量名是遗留的——它期望群组 ID。仅当 iLink 实际传递群组事件时生效；扫码登录的 iLink 机器人身份（`...@im.bot`）通常不会接收普通的微信群消息。 |
...
用于 [Web 仪表盘](/user-guide/features/web-dashboard) 身份验证以及 [将 Hermes 桌面版连接到远程后端](/user-guide/features/web-dashboard#connecting-hermes-desktop-to-a-a-remote-backend)。根据仅存储密钥的惯例，Token 应放在 `~/.hermes/.env` 中；OAuth 的 `client_id`/`portal_url` 最好在 `config.yaml` 的 `dashboard.oauth` 下设置（若同时设置，环境变量优先）。

| 变量 | 描述 |
|----------|-------------|
| `HERMES_DASHBOARD_SESSION_TOKEN` | 固定仪表盘会话 Token，而不是每次启动时随机生成。在后端设置此值（例如 `openssl rand -base64 32`），然后将相同的值粘贴到 Hermes 桌面版 → 设置 → 消息网关 → 远程网关 → 会话 Token。这是稳定远程桌面连接所必需的。 |
| `HERMES_DESKTOP_REMOTE_URL` | （桌面端）远程后端的基础 URL，例如 `http://host:9119`。设置后，将覆盖应用内的消息网关设置。必须与 `HERMES_DESKTOP_REMOTE_TOKEN` 配对使用。 |
| `HERMES_DESKTOP_REMOTE_TOKEN` | （桌面端）用于向远程后端进行身份验证的会话 Token —— 与后端的 `HERMES_DASHBOARD_SESSION_TOKEN` 值相同。 |
| `HERMES_DASHBOARD_TUI` | 设置为 `1` 将在浏览器中暴露聊天标签页（嵌入的 `hermes --tui`），与 `--tui` 标志效果相同。 |
| `HERMES_DASHBOARD_OAUTH_CLIENT_ID` | 用于受控/公开仪表盘的 OAuth 客户端 ID（格式为 `agent:{instance_id}`）。覆盖 `dashboard.oauth.client_id`。对于托管部署，由 Nous Portal 提供。 |
...
即将推出的 Teams 会议摘要流水线所使用的 Microsoft Graph REST 客户端的仅应用凭据。请参阅 [注册 Microsoft Graph 应用程序](/guides/microsoft-graph-app-registration) 了解 Azure 门户操作指南以及所需的确切 API 权限。
...
| `MSGRAPH_AUTHORITY_URL` | Microsoft 身份平台授权机构（默认：`https://login.microsoftonline.com`）。仅在国家/主权云（例如 GCC High 使用 `https://login.microsoftonline.us`）时覆盖。 |
...
| `MSGRAPH_WEBHOOK_ALLOWED_SOURCE_CIDRS` | 允许向监听器发送 POST 请求的逗号分隔 CIDR 范围（例如 `52.96.0.0/14,52.104.0.0/14`）。空值 = 允许所有（默认）。在生产环境中应限制为 Microsoft Graph 发布的出口范围。 |
...
| `LINE_PUBLIC_URL` | 公开 HTTPS 基础 URL（例如 `https://my-tunnel.example.com`）。发送图片/音频/视频时必需 —— LINE 只接受可通过 HTTPS 访问的 URL。 |
| `LINE_ALLOWED_USERS` | 允许与机器人私聊的逗号分隔用户 ID（以 `U` 为前缀）。 |
| `LINE_ALLOWED_GROUPS` | 机器人将响应的逗号分隔群组 ID（以 `C` 为前缀）。 |
| `LINE_ALLOWED_ROOMS` | 机器人将响应的逗号分隔房间 ID（以 `R` 为前缀）。 |
| `LINE_ALLOW_ALL_USERS` | 仅用于开发的应急开关 —— 接受任何来源。默认：`false`。 |
| `LINE_HOME_CHANNEL` | 带有 `deliver: line` 的定时任务的默认投递目标。 |
| `LINE_SLOW_RESPONSE_THRESHOLD` | 慢速 LLM 模板按钮回发触发前的秒数（默认：`45`）。设置为 `0` 以禁用并始终回退到推送。 |
...
| `LINE_INTERRUPTED_TEXT` | 当点击被 `/stop` 中断的孤立回发按钮时的回复（默认：`Run was interrupted before completion.`）。 |
### ntfy（推送通知）
...
用于限制出站消息批处理器的、针对每个平台的高级调节选项。大多数用户永远不需要接触这些；默认值设置为尊重每个平台的速率限制，同时不会感觉迟钝。
...
| `HERMES_GATEWAY_BOOTSTRAP_STATE` | 在 s6-overlay Docker 镜像内部，声明消息网关在全新卷上的**初始**监管状态。在空白卷上没有持久化的 `gateway_state.json`，因此启动协调器会注册 `gateway-default` 插槽但将其保持为**停止**状态（仅当最后记录的状态是 `running` 时才会自动启动）。将其设置为 `running`，首次启动设置钩子会在协调器运行*之前*植入 `gateway_state.json`，因此消息网关在首次启动时就会启动。仅字面值 `running` 有效。仅首次启动：现有的 `gateway_state.json` 永远不会被覆盖，因此故意停止的消息网关在重启后保持停止状态。在 s6 镜像外部无效。 |
| `HERMES_FILE_MUTATION_VERIFIER` | 启用每轮文件变更验证器页脚（默认：`true`）。启用后，Hermes 会附加一个咨询列表，列出本轮中任何失败且未被成功写入覆盖的 `write_file` / `patch` 调用。设置为 `0`、`false`、`no` 或 `off` 以抑制。镜像 `config.yaml` 中的 `display.file_mutation_verifier`；设置环境变量时，环境变量优先。 |
...
| `HERMES_CRON_MAX_PARALLEL` | 每个滴答中并行运行的最大定时任务数（默认：`4`）。 |
## Agent 行为
...
| `HERMES_API_CALL_STALE_TIMEOUT` | 非流式陈旧调用超时时间（秒）（默认：`300`）。如果未设置，对于本地提供商会自动禁用。也可以通过 `config.yaml` 中的 `providers.<id>.stale_timeout_seconds` 或 `providers.<id>.models.<model>.stale_timeout_seconds` 配置。 |
...
| `HERMES_ENABLE_PROJECT_PLUGINS` | 为 Agent 加载器和仪表板 Web 服务器启用从 `./.hermes/plugins/` 自动发现仓库本地插件。接受标准的真值集：`1` / `true` / `yes` / `on`（不区分大小写）。其他所有值——包括 `0`、`false`、`no`、`off` 和空字符串——都被视为**禁用**（默认）。注意：根据 GHSA-5qr3-c538-wm9j (#29156)，即使此变量启用，仪表板 Web 服务器也拒绝自动导入项目插件的 Python `api` 文件——项目插件可以通过静态 JS/CSS 扩展 UI，但其后端路由仅在移动到 `~/.hermes/plugins/` 下时才会加载。 |
...
| `HERMES_WRITE_SAFE_ROOT` | 可选的目录前缀，用于限制 `write_file`/`patch` 的写入；此路径外的写入需要批准。 |
...
| `DELEGATION_MAX_CONCURRENT_CHILDREN` | 每个 `delegate_task` 批处理的最大并行子 Agent 数（默认：`3`，下限为 1，无上限）。也可以通过 `config.yaml` 中的 `delegation.max_concurrent_children` 配置——配置值优先级更高。 |
## 界面
...
| `HERMES_INFERENCE_MODEL` | 强制指定 `hermes -z` / `hermes chat` 使用的模型，而不修改 `config.yaml`。与 `--provider` 标志配对使用。对于需要每次运行覆盖默认模型的脚本化调用者（sweeper、CI、批量运行器）很有用。 |

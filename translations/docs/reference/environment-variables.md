...
对于原生的 Anthropic 认证，当 Claude Code 自身的凭证文件存在时，Hermes 会优先使用它们，因为这些凭证可以自动刷新。**针对 Anthropic 的 OAuth 需要 Claude Max 计划并购买额外的使用额度** —— Hermes 会以 Claude Code 的身份路由，这只会消耗 Max 计划的额外/超额额度，而不是基础 Max 额度，并且在 Claude Pro 上无效。如果没有 Max 计划 + 额外额度，请改用 API 密钥。环境变量如 `ANTHROPIC_TOKEN` 仍然可以作为手动覆盖使用，但它们不再是 Claude Max 登录的首选方式。
...
| `MSGRAPH_WEBHOOK_ACCEPTED_RESOURCES` | Graph 资源路径/模式的逗号分隔允许列表（例如：`communications/onlineMeetings,chats/*/messages`）。尾随 `*` 表示前缀匹配。空值 = 接受所有。 |
...
| `HERMES_API_CALL_STALE_TIMEOUT` | 非流式陈旧调用超时时间（秒）（默认：`300`）。如果留空，对于本地提供商会自动禁用。也可通过 `config.yaml` 中的 `providers.<id>.stale_timeout_seconds` 或 `providers.<id>.models.<model>.stale_timeout_seconds` 配置。 |
...
主模型备用链仅通过 `config.yaml` 配置 —— 没有对应的环境变量。添加一个顶层的 `fallback_providers` 列表，其中包含 `provider` 和 `model` 键，以在主模型遇到错误时启用自动故障转移。

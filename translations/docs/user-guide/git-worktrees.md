...
Git **worktrees** 是为每个 Agent 提供其独立代码检出的最安全方式，无需复制整个仓库。
...
- 消息网关：`~/.hermes/config.yaml` 中 `terminal.cwd` 设置的目录
...
从您的主仓库（包含 `.git/`）中，为功能分支创建一个新的 worktree：
...
- 使用一个**独立的检查点历史记录**用于 `/rollback`，其范围限定在此 worktree 内。
...
- `git worktree remove` 会拒绝移除包含未提交更改的 worktree，除非您强制操作。
...
Hermes 内置了一个 `-w` 标志，可以**自动创建一个临时的 git worktree** 及其自己的分支。您无需手动设置 worktrees —— 只需 `cd` 进入您的仓库并运行：

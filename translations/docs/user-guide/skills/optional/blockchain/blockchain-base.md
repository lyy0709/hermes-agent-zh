...
title: "Base"
sidebar_label: "Base"
description: "查询 Base（以太坊 L2）区块链数据并附带美元计价——钱包余额、代币信息、交易详情、Gas 分析、合约检查、巨鲸检测..."
---
...
查询 Base（以太坊 L2）区块链数据并附带美元计价——钱包余额、代币信息、交易详情、Gas 分析、合约检查、巨鲸检测以及实时网络状态。使用 Base RPC + CoinGecko。无需 API 密钥。
...
通过 CoinGecko 查询 Base（以太坊 L2）链上数据并附带美元计价。
8 个命令：钱包资产组合、代币信息、交易、Gas 分析、合约检查、巨鲸检测、网络状态和价格查询。
...
- 用户询问 Base 钱包余额、代币持仓或资产组合价值时
- 用户想通过哈希值检查特定交易时
- 用户需要 ERC-20 代币元数据、价格、供应量或市值时
- 用户想了解 Base Gas 成本和 L1 数据费用时
- 用户想检查合约（ERC 类型检测、代理解析）时
- 用户想查找大额 ETH 转账（巨鲸检测）时
- 用户需要 Base 网络健康状况、Gas 价格或 ETH 价格时
- 用户询问“USDC/AERO/DEGEN/ETH 的价格是多少？”时
...
通过链上 `balanceOf` 调用检查约 15 种知名的 Base 代币（USDC, WETH, AERO, DEGEN 等）。
代币按价值排序，小额代币被过滤。
...
输出包括：ETH 余额 + 美元价值、按价值排序的代币列表及价格、小额代币数量、资产组合总价值（美元）。

注意：仅检查已知代币。未知的 ERC-20 代币不会被发现。
对于任何代币，请使用 `token` 命令并指定合约地址。

### 2. 交易详情

通过哈希值检查完整交易。显示转账的 ETH 价值、使用的 Gas、费用（ETH/USD）、状态以及解码后的 ERC-20/ERC-721 转账。
...
输出：哈希值、区块、发送方、接收方、价值（ETH + USD）、Gas 价格、使用的 Gas、费用、状态、合约创建地址（如有）、代币转账。
...
详细的 Gas 分析，包含常见操作的预估成本。
显示当前 Gas 价格、10 个区块内的基础费用趋势、区块利用率，以及 ETH 转账、ERC-20 转账和兑换的预估成本。

```bash
python3 ~/.hermes/skills/blockchain/base/scripts/base_client.py gas
```

输出：当前 Gas 价格、基础费用、区块利用率、10 个区块趋势、预估成本（ETH 和 USD）。

注意：Base 是 L2 —— 实际交易成本包括 L1 数据发布费，该费用取决于 calldata 大小和 L1 Gas 价格。显示的预估仅针对 L2 执行。

### 5. 合约检查

检查地址：确定是 EOA 还是合约，检测 ERC-20/ERC-721/ERC-1155 接口，解析 EIP-1967 代理实现地址。
...
Base 网络实时健康状况：最新区块、链 ID、Gas 价格、基础费用、区块利用率、交易数量和 ETH 价格。
...
已知符号：ETH, WETH, USDC, cbETH, AERO, DEGEN, TOSHI, BRETT, WELL, wstETH, rETH, cbBTC。
...
- **CoinGecko 速率限制** —— 免费层级允许每分钟约 10-30 次请求。
  价格查询每个代币使用 1 次请求。如需更快速度，请使用 `--no-prices`。
- **公共 RPC 速率限制** —— Base 的公共 RPC 限制请求。
  生产环境使用，请将 BASE_RPC_URL 设置为私有端点（Alchemy, QuickNode, Infura）。
- **钱包仅显示已知代币** —— 与 Solana 不同，EVM 链没有内置的“获取所有代币”RPC。钱包命令通过 `balanceOf` 检查约 15 种流行的 Base 代币。未知的 ERC-20 代币不会出现。对于任何特定合约，请使用 `token` 命令。
- **代币名称从合约读取** —— 如果合约未实现 `name()` 或 `symbol()`，这些字段可能为空。已知代币有硬编码的标签作为后备。
- **Gas 预估仅针对 L2** —— Base 交易成本包括 L1 数据发布费（取决于 calldata 大小和 L1 Gas 价格）。gas 命令仅预估 L2 执行成本。
- **巨鲸检测器仅扫描最新区块** —— 非历史数据。结果因查询时刻而异。默认阈值为 1.0 ETH。
- **代理检测** —— 仅检测 EIP-1967 代理。其他代理模式（EIP-1167 最小代理、自定义存储槽）未检查。
- **429 错误重试** —— RPC 和 CoinGecko 调用在遇到速率限制错误时，最多重试 2 次，并采用指数退避。

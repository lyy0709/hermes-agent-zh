...
description: "查询 Solana 区块链数据及美元计价信息 —— 钱包余额、带价值的代币投资组合、交易详情、NFT、巨鲸检测和实时网络状态..."
---
...
查询 Solana 区块链数据及美元计价信息 —— 钱包余额、带价值的代币投资组合、交易详情、NFT、巨鲸检测和实时网络状态。使用 Solana RPC + CoinGecko。无需 API 密钥。
...
通过 CoinGecko 查询带有美元计价信息的 Solana 链上数据。
...
获取 SOL 余额、带有美元价值的 SPL 代币持有量、NFT 数量和投资组合总值。代币按价值排序，过滤粉尘代币，已知代币按名称标记（BONK, JUP, USDC 等）。
...
- `--all` —— 显示所有代币，不过滤粉尘代币，无限制
- `--no-prices` —— 跳过 CoinGecko 价格查询（更快，仅使用 RPC）

输出包括：SOL 余额 + 美元价值、按价值排序的代币列表及价格、粉尘代币数量、NFT 摘要、以美元计的总投资组合价值。
...
实时 Solana 网络健康状况：当前区块、纪元、TPS、供应量、验证者版本、SOL 价格和市值。
...
- **CoinGecko 速率限制** —— 免费层级允许每分钟约 10-30 次请求。
  价格查询每个代币使用 1 次请求。持有大量代币的钱包可能无法获取所有代币的价格。如需速度，请使用 `--no-prices`。
- **公共 RPC 速率限制** —— Solana 主网公共 RPC 限制请求次数。
  生产环境使用，请将 SOLANA_RPC_URL 设置为私有端点（Helius, QuickNode, Triton）。
- **NFT 检测是启发式的** —— 数量=1 + 小数位数=0。压缩 NFT（cNFT）和 Token-2022 NFT 不会出现。
- **巨鲸检测器仅扫描最新区块** —— 非历史数据。结果因查询时刻而异。
- **交易历史** —— 公共 RPC 保留约 2 天。更早的交易可能无法获取。
- **代币名称** —— 约 25 个知名代币按名称标记。其他代币显示缩写的铸造地址。使用 `token` 命令获取完整信息。

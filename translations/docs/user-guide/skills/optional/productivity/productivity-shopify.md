...
通过 curl 使用 Shopify Admin 和 Storefront GraphQL API。产品、订单、客户、库存、元字段。
...
通过 `curl` 直接与 Shopify 商店交互：列出产品、管理库存、拉取订单、更新客户、读取元字段。无需 SDK，无需应用框架 — 只需 GraphQL 端点和自定义应用访问令牌。

REST Admin API 自 2024 年 4 月起已弃用，仅接收安全修复。**所有管理工作请使用 GraphQL Admin**。**Storefront GraphQL** 用于面向客户的只读查询（产品、系列、购物车）。

## 先决条件

1.  在 Shopify 管理后台：**设置 → 应用和销售渠道 → 开发应用 → 创建应用**。
2.  点击 **配置 Admin API 权限范围**，选择所需权限（下方有示例），保存。
3.  **安装应用** → Admin API 访问令牌**仅显示一次**。立即复制 — Shopify 将不再显示。令牌以 `shpat_` 开头。
4.  保存到 `~/.hermes/.env`：
    ```
...
- 订单：`read_orders`, `write_orders`（无 `read_all_orders` 权限时仅限最近 30 条）
...
- **HTTP 200 并不代表成功。** GraphQL 在顶层的 `errors` 数组和每个字段的 `userErrors` 中返回错误。务必检查两者。
- **ID 是 GID 字符串：** `gid://shopify/Product/10079467700516`, `gid://shopify/Variant/...`, `gid://shopify/Order/...`。请原样传递这些 ID — 不要去除前缀。
- **速率限制：** 通过查询成本（漏桶算法）计算。每个响应都包含 `extensions.cost`，其中有 `requestedQueryCost`, `actualQueryCost`, `throttleStatus.{currentlyAvailable, maximumAvailable, restoreRate}`。当 `currentlyAvailable` 低于你下一个查询的成本时，请退避。标准商店 = 100 点桶，50/秒恢复；Plus 商店 = 1000/100。
...
### 列出近期订单（默认情况下，若无 `read_all_orders` 权限，返回最近 30 条）
```bash
shop_gql '
{
...
用于处理超出速率限制允许范围的数据转储（例如完整的产品目录、一年的所有订单）：
...
```
每个 JSONL 行都是一个节点，嵌套的连接会以带有 `__parentId` 的独立行形式发出。如果需要，可以在客户端重新组装。
...
- **使用有效令牌却返回 403 = 缺少权限范围。** Shopify 会返回 `{"errors":[{"message":"Access denied for ..."}]}`。请在应用上重新配置 Admin API 权限范围，然后重新安装以重新生成令牌。
...
- **速率限制的意外情况。** 一个带有深度嵌套的 `products(first: 250)` 查询可能会消耗 1000+ 点数，并在标准套餐店铺上立即触发限流。开始时范围要小，读取 `extensions.cost`，然后进行调整。
- **分页顺序。** `products(first: N, reverse: true)` 是按 `id DESC` 排序，而不是 `created_at`。要按“最新优先”排序，请使用 `sortKey: CREATED_AT, reverse: true`。
- **`read_all_orders` 用于历史数据。** 没有这个权限范围，`orders(...)` 会静默地限制在 60 天的时间窗口内。你不会收到错误，只是得到的结果比预期的少。对于拥有大量订单的 Shopify Plus 商家，请通过应用的保护数据设置请求此权限范围。
- **货币是字符串。** 金额以 `"49.00"` 的形式返回，而不是 `49.0`。如果你关心零填充，不要盲目地使用 `jq tonumber`。
- **多币种货币字段** 包含 `shopMoney`（店铺货币）和 `presentmentMoney`（客户货币）。请始终如一地选择一种。

## 安全性

Shopify 中的变更操作是真实的——它们会创建产品、处理退款、取消订单、发货履行。在运行 `productDelete`、`orderCancel`、`refundCreate` 或任何批量变更操作之前：请明确说明变更内容、针对哪个店铺，并与用户确认。除非用户有单独的开发店铺，否则没有生产数据的暂存克隆。
...
自 2024 年 4 月起，REST Admin API 已过时，仅接收安全修复。**所有管理操作请使用 GraphQL Admin**。**Storefront GraphQL** 用于面向客户的只读查询（产品、系列、购物车）。

## 先决条件

1.  在 Shopify 管理后台：**设置 → 应用和销售渠道 → 开发应用 → 创建应用**。
2.  点击 **配置 Admin API 权限范围**，选择所需权限（见下文示例），保存。
...
- **速率限制：** 通过查询成本（漏桶算法）计算。每个响应都包含 `extensions.cost`，其中有 `requestedQueryCost`, `actualQueryCost`, `throttleStatus.{currentlyAvailable, maximumAvailable, restoreRate}`。当 `currentlyAvailable` 低于下一个查询的成本时，请退避。标准商店 = 100 点桶，50/秒恢复；Plus 商店 = 1000/100。
...
### 列出最近的订单（默认情况下，没有 `read_all_orders` 权限时返回最近 30 条）
```bash
shop_gql '
{
...
不同的端点，不同的 Token，用于面向客户的应用程序或类似 Hydrogen 的无头设置。请求头不同：
...
- **认证请求头（私有）：** `Shopify-Storefront-Private-Token: <private token>` — 仅限服务器端
...
用于转储超出速率限制允许范围的数据（完整产品目录、一年的所有订单）：
...
```
每个 JSONL 行都是一个节点，嵌套的连接会作为带有 `__parentId` 的独立行发出。如果需要，可以在客户端重新组装。
...
- **`userErrors` 为空 != 成功。** 同时检查 `data.<mutation>.<resource>` 是否为非空。有些失败情况两者都不会填充——请检查整个响应。
- **GID 与数字 ID。** 旧版 REST 使用数字 ID；GraphQL 需要完整的 GID 字符串。转换方法：`gid://shopify/Product/<numeric>`。
- **速率限制的意外情况。** 一个带有深度嵌套的 `products(first: 250)` 查询可能会消耗 1000+ 点数，并在标准套餐的商店上立即触发限流。开始时范围要小，读取 `extensions.cost`，然后进行调整。
- **分页顺序。** `products(first: N, reverse: true)` 是按 `id DESC` 排序，而不是 `created_at`。要“最新优先”，请使用 `sortKey: CREATED_AT, reverse: true`。
...
Shopify 中的变更操作是真实的——它们会创建产品、处理退款、取消订单、发货履行。在运行 `productDelete`、`orderCancel`、`refundCreate` 或任何批量变更操作之前：请明确说明变更内容、针对哪个商店，并与用户确认。除非用户有单独的开发商店，否则没有生产数据的暂存克隆。
...
> **注意：** 自 2026 年 1 月 1 日起，在 Shopify 管理后台创建的新的“旧版自定义应用”已不可用。新设置应使用 **Dev Dashboard** (`shopify.dev/docs/apps/build/dev-dashboard`)。现有的在管理后台创建的应用继续有效。如果用户的商店没有现有的自定义应用，并且时间在 2026-01-01 之后，请引导他们使用 Dev Dashboard 而非管理后台流程。
...
- **ID 是 GID 字符串：** `gid://shopify/Product/10079467700516`, `gid://shopify/Variant/...`, `gid://shopify/Order/...`。原样传递这些 — 不要去除前缀。
- **速率限制：** 通过查询成本（漏桶算法）计算。每个响应都有 `extensions.cost`，包含 `requestedQueryCost`, `actualQueryCost`, `throttleStatus.{currentlyAvailable, maximumAvailable, restoreRate}`。当 `currentlyAvailable` 低于你下一个查询的成本时，请退避。标准商店 = 100 点桶，50/秒恢复；Plus = 1000/100。
...
- **令牌格式检查。** 管理员令牌以 `shpat_` 开头。店面公共令牌以 `shpua_` 开头。如果你有一个令牌但使用了错误的请求头，每个请求都会返回 401，并且没有有用的错误正文。
...
- **速率限制的意外情况。** 一个带有深度嵌套的 `products(first: 250)` 查询可能会消耗 1000+ 点数，并在标准计划商店上立即触发限流。开始时范围要窄，读取 `extensions.cost`，然后进行调整。
- **分页顺序。** `products(first: N, reverse: true)` 是按 `id DESC` 排序，而不是 `created_at`。要按“最新优先”排序，请使用 `sortKey: CREATED_AT, reverse: true`。
- **`read_all_orders` 用于获取历史数据。** 没有这个权限范围，`orders(...)` 会静默地将数据限制在 60 天窗口内。你不会收到错误，只是得到的结果比预期的少。对于拥有大量订单的 Shopify Plus 商家，请通过应用的保护数据设置请求此权限范围。
- **货币是字符串。** 金额以 `"49.00"` 的形式返回，而不是 `49.0`。如果你关心零填充，不要盲目地使用 `jq tonumber`。
- **多币种货币字段** 包含 `shopMoney`（商店货币）和 `presentmentMoney`（客户货币）。请始终如一地选择一种。

## 安全性

Shopify 中的变更操作是真实的——它们会创建产品、处理退款、取消订单、发货履行。在运行 `productDelete`、`orderCancel`、`refundCreate` 或任何批量变更操作之前：请明确说明变更内容、针对哪个商店，并与用户确认。除非用户拥有独立的开发商店，否则没有生产数据的暂存克隆。
...
根据主题生成实际的梗图。选择模板、编写字幕，并通过文本叠加渲染出真实的 .png 文件。
...
该脚本支持 **imgflip 上约 100 个流行模板中的任何一个**（通过名称或 ID），外加 10 个经过手动调整文本定位的精选模板。
...
| `batman-slapping-robin` | Batman Slapping Robin | robin, batman | 否决糟糕想法 |
...
1.  解读用户的主题并识别核心动态（混乱、困境、偏好、讽刺等）
2.  选择最匹配的模板。参考“最佳用途”列，或使用 `--search` 搜索。
3.  为每个字段编写简短的字幕（每个字段最多 8-12 个单词，越短越好）。
4.  找到技能的脚本目录：
    ```
...
1.  首先编写字幕。
2.  使用 `image_generate` 创建一个符合梗图概念的场景。图像提示词中**不要**包含任何文本——文本将由脚本添加。只描述视觉场景。
...
4.  运行脚本并使用 `--image` 参数叠加文本，选择一种模式：
    ...
    - **条带**（图像上方/下方添加黑色条带，显示白色文本——更清晰，始终可读）：
      ...
      ```
    当图像内容繁杂/细节丰富，文本直接叠加难以阅读时，使用 `--bars`。
5.  **使用视觉模型验证**（如果 `vision_analyze` 可用）：检查结果是否良好：
    ...
    ```
    如果视觉模型指出问题（文本难以阅读、位置不佳等），尝试另一种模式（在叠加和条带之间切换）或重新生成场景。
6.  使用 `MEDIA:/tmp/meme.png` 返回图像

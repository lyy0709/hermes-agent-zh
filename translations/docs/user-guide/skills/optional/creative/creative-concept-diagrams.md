...
title: "概念图"
sidebar_label: "概念图"
description: "生成扁平、极简、支持明暗模式的SVG图表，作为独立的HTML文件，采用统一的教育视觉语言，包含9种语义色彩渐变、句子大小写排版和自动深色模式。最适合教育和非软件可视化——物理装置、化学机制、数学曲线、物理对象（飞机、涡轮机、智能手机、机械手表）、解剖学、平面图、剖面图、叙事旅程（X的生命周期、Y的过程）、中心辐射型系统集成（智慧城市、物联网）以及爆炸图层视图。如果存在针对该主题的更专业技能（专门的软件/云架构、手绘草图、动画解说等），请优先使用——否则此技能也可作为具有简洁教育外观的通用SVG图表备用方案。附带15个示例图表。"
---
...
生成扁平、极简、支持明暗模式的SVG图表，作为独立的HTML文件，采用统一的教育视觉语言，包含9种语义色彩渐变、句子大小写排版和自动深色模式。最适合教育和非软件可视化——物理装置、化学机制、数学曲线、物理对象（飞机、涡轮机、智能手机、机械手表）、解剖学、平面图、剖面图、叙事旅程（X的生命周期、Y的过程）、中心辐射型系统集成（智慧城市、物联网）以及爆炸图层视图。如果存在针对该主题的更专业技能（专门的软件/云架构、手绘草图、动画解说等），请优先使用——否则此技能也可作为具有简洁教育外观的通用SVG图表备用方案。附带15个示例图表。
...
使用统一的扁平、极简设计系统生成生产级SVG图表。输出为单个自包含的HTML文件，在任何现代浏览器中呈现效果一致，并支持自动明暗模式。
...
可选：如果用户想要浏览多个图表的画廊，请参见底部的"本地预览服务器"。
...
该模板嵌入了完整的CSS设计系统（`c-*` 颜色类、文本类、明暗模式变量、箭头标记样式）。你生成的SVG依赖于承载页面上存在的这些类。
...
- **支持深色模式**：所有颜色通过CSS类自动适配——无需针对每种模式单独处理SVG。
...
- **描边宽度**：所有节点边框为 0.5px。不是 1px，也不是 2px。
- **矩形圆角**：节点 `rx="8"`，内部容器 `rx="12"`，外部容器 `rx="16"` 到 `rx="20"`。
- **连接线路径**：**必须**有 `fill="none"`。否则 SVG 默认 `fill: black`。
...
- 色阶：`.c-purple`, `.c-teal`, `.c-coral`, `.c-pink`, `.c-gray`, `.c-blue`, `.c-green`, `.c-amber`, `.c-red`（均支持自动明暗模式）
...
2.  **结构/包含图** — 云基础设施嵌套、分层系统架构。带有内部区域的大型外部容器。虚线矩形用于逻辑分组。
3.  **API/端点映射图** — REST 路由、GraphQL 模式。从根开始的树形结构，分支到资源组，每个组包含端点节点。
4.  **微服务拓扑图** — 服务网格、事件驱动系统。服务作为节点，箭头表示通信模式，消息队列位于其间。
5.  **数据流图** — ETL 流水线、流式架构。从左到右的流，从源经过处理到接收器。
6.  **物理/结构图** — 车辆、建筑、硬件、解剖结构。使用与物理形态匹配的形状 — 曲线体用 `<path>`，锥形用 `<polygon>`，圆柱形部件用 `<ellipse>`/`<circle>`，隔间用嵌套的 `<rect>`。参见 `references/physical-shape-cookbook.md`。
7.  **基础设施/系统集成图** — 智慧城市、物联网网络、多领域系统。中心辐射布局，中央平台连接子系统。语义线样式（`.data-line`, `.power-line`, `.water-pipe`, `.road`）。参见 `references/infrastructure-patterns.md`。
8.  **UI/仪表板模拟图** — 管理面板、监控仪表板。屏幕框架内嵌套图表/仪表/指示器元素。参见 `references/dashboard-patterns.md`。
...
编写一个用户可以直接打开的单一 `.html` 文件。无需服务器，无依赖，可离线工作。模式如下：
...
**规则：**
- 仅绑定到 `127.0.0.1`。绝不使用 `0.0.0.0`。在共享网络上将所有图表暴露在所有网络接口上是安全风险。
- 选择一个空闲端口（不要硬编码端口号）并告诉用户所选的 URL。
...
如果用户坚持使用固定端口，请使用 `127.0.0.1:<端口>`——仍然绝不使用 `0.0.0.0`。记录如何停止服务器（`kill %1` 或 `pkill -f "http.server"`）。

---

## 示例参考

`examples/` 目录包含 15 个完整、经过测试的图表。在编写类似类型的新图表之前，请浏览它们以获取有效模式：

...
title: "概念图"
sidebar_label: "概念图"
description: "生成扁平、极简、支持明暗模式的SVG图表，作为独立的HTML文件，使用统一的教育视觉语言，包含9种语义色彩渐变、句子大小写排版和自动深色模式。最适合教育和非软件可视化——物理装置、化学机制、数学曲线、物理对象（飞机、涡轮机、智能手机、机械手表）、解剖学、平面图、剖面图、叙事旅程（X的生命周期、Y的过程）、中心辐射型系统集成（智慧城市、物联网）以及爆炸图层视图。如果存在针对该主题的更专业技能（专门的软件/云架构、手绘草图、动画解说等），请优先使用——否则此技能也可作为具有简洁教育外观的通用SVG图表备用方案。附带15个示例图表。"
---
...
生成扁平、极简、支持明暗模式的SVG图表，作为独立的HTML文件，使用统一的教育视觉语言，包含9种语义色彩渐变、句子大小写排版和自动深色模式。最适合教育和非软件可视化——物理装置、化学机制、数学曲线、物理对象（飞机、涡轮机、智能手机、机械手表）、解剖学、平面图、剖面图、叙事旅程（X的生命周期、Y的过程）、中心辐射型系统集成（智慧城市、物联网）以及爆炸图层视图。如果存在针对该主题的更专业技能（专门的软件/云架构、手绘草图、动画解说等），请优先使用——否则此技能也可作为具有简洁教育外观的通用SVG图表备用方案。附带15个示例图表。
...
如果存在针对该主题的更专业技能，请优先使用。如果没有合适的，此技能可作为通用SVG图表备用方案——输出将采用下文描述的简洁教育美学风格，这几乎是任何主题的合理默认选择。
...
- 每个 `<text>` 元素必须包含一个类（`t`、`ts` 或 `th`）。不能有无类的文本。
...
- **连接线路径**：必须设置 `fill="none"`。否则 SVG 默认 `fill: black`。
...
- 颜色梯度：`.c-purple`、`.c-teal`、`.c-coral`、`.c-pink`、`.c-gray`、`.c-blue`、`.c-green`、`.c-amber`、`.c-red`（全部自动适应明暗模式）
...
7.  **基础设施 / 系统集成图** — 智慧城市、物联网网络、多领域系统。中心辐射布局，中央平台连接子系统。语义线条样式（`.data-line`、`.power-line`、`.water-pipe`、`.road`）。参见 `references/infrastructure-patterns.md`。
8.  **UI / 仪表盘模拟图** — 管理面板、监控仪表盘。屏幕框架包含嵌套的图表/仪表/指示器元素。参见 `references/dashboard-patterns.md`。

对于物理、基础设施和仪表盘图表，在生成前加载相应的参考文件 — 每个文件都提供了现成的 CSS 类和形状基元。
---
...
**规则：**
- 仅绑定到 `127.0.0.1`。切勿使用 `0.0.0.0`。在共享网络上将所有网络接口暴露图表存在安全风险。
...
| `sn2-reaction-mechanism.md` | 化学 | 分子、曲线箭头、能量分布图 |

...
title: "概念图"
sidebar_label: "概念图"
description: "生成扁平、极简、支持明暗模式的SVG图表，输出为独立的HTML文件，采用统一的教育视觉语言，包含9种语义色彩渐变、句首字母大写的排版和自动深色模式。最适合教育和非软件类可视化——物理装置、化学机理、数学曲线、物理对象（飞机、涡轮机、智能手机、机械手表）、解剖图、平面图、剖面图、叙事旅程（X的生命周期、Y的流程）、中心辐射式系统集成（智慧城市、物联网）以及爆炸图层视图。如果存在针对该主题的更专业技能（专门的软件/云架构、手绘草图、动画解说等），请优先使用——否则，此技能也可作为具有简洁教育外观的通用SVG图表备用方案。附带15个示例图表。"
---
...
生成扁平、极简、支持明暗模式的SVG图表，输出为独立的HTML文件，采用统一的教育视觉语言，包含9种语义色彩渐变、句首字母大写的排版和自动深色模式。最适合教育和非软件类可视化——物理装置、化学机理、数学曲线、物理对象（飞机、涡轮机、智能手机、机械手表）、解剖图、平面图、剖面图、叙事旅程（X的生命周期、Y的流程）、中心辐射式系统集成（智慧城市、物联网）以及爆炸图层视图。如果存在针对该主题的更专业技能（专门的软件/云架构、手绘草图、动画解说等），请优先使用——否则，此技能也可作为具有简洁教育外观的通用SVG图表备用方案。附带15个示例图表。
...
使用统一的扁平、极简设计系统生成生产级SVG图表。输出为单个独立的HTML文件，在任何现代浏览器中渲染效果一致，并支持自动明暗模式。
...
**以下情况请优先考虑其他技能：**
- 具有深色科技美学的专门软件/云基础设施架构（如果可用，请考虑 `architecture-diagram`）
- 手绘白板草图（如果可用，请考虑 `excalidraw`）
- 动画解说或视频输出（请考虑动画技能）

如果存在针对该主题的更专业技能，请优先使用。如果没有合适的，此技能可作为通用SVG图表备用方案——输出将采用下述简洁的教育美学风格，这几乎是任何主题的合理默认选择。
...
该模板嵌入了完整的CSS设计系统（`c-*` 颜色类、文本类、明暗模式变量、箭头标记样式）。你生成的SVG依赖于宿主页面上存在的这些类。
...
- **极简化**：展示本质。框内无装饰性图标。
- **一致性**：所有图表使用相同的颜色、间距、排版和描边宽度。
- **支持深色模式**：所有颜色通过CSS类自动适配——无需针对每种模式单独制作SVG。

### 调色板

9种色彩渐变，每种有7个色阶。将类名放在 `<g>` 或形状元素上；模板CSS会处理两种模式。
...
- 每个 `<text>` 元素必须携带一个类（`t`、`ts` 或 `th`）。不允许存在未分类的文本。
...
- **箭头间隙**：箭头尖端与框边缘之间 10px。
- **单行框**：高度 44px。
- **双行框**：高度 56px，标题与副标题基线之间 18px。
- **容器内边距**：每个容器内部至少 20px。
- **最大嵌套深度**：2-3 层。更深层次在 680px 宽度下会变得难以阅读。

### 描边与形状

- **描边宽度**：所有节点边框使用 0.5px。不是 1px，也不是 2px。
- **矩形圆角**：节点使用 `rx="8"`，内部容器使用 `rx="12"`，外部容器使用 `rx="16"` 到 `rx="20"`。
- **连接线路径**：必须设置 `fill="none"`。否则 SVG 默认使用 `fill: black`。
...
在线上使用 `marker-end="url(#arrow)"`。箭头尖端通过 `context-stroke` 继承线条颜色。
...
- 色阶：`.c-purple`、`.c-teal`、`.c-coral`、`.c-pink`、`.c-gray`、`.c-blue`、`.c-green`、`.c-amber`、`.c-red`（全部自动适配明暗模式）
...
2.  **结构图 / 包含图** — 云基础设施嵌套、分层系统架构。大型外部容器包含内部区域。虚线矩形用于逻辑分组。
3.  **API / 端点映射图** — REST 路由、GraphQL 模式。从根开始的树状结构，分支到资源组，每个组包含端点节点。
4.  **微服务拓扑图** — 服务网格、事件驱动系统。服务作为节点，箭头表示通信模式，消息队列位于其间。
5.  **数据流图** — ETL 流水线、流式架构。从左到右的流动，从源经过处理到接收器。
6.  **物理 / 结构图** — 车辆、建筑、硬件、解剖结构。使用匹配物理形态的形状 — 使用 `<path>` 表示弯曲体，使用 `<polygon>` 表示锥形形状，使用 `<ellipse>`/`<circle>` 表示圆柱形部件，使用嵌套的 `<rect>` 表示隔间。参见 `references/physical-shape-cookbook.md`。
7.  **基础设施 / 系统集成图** — 智慧城市、物联网网络、多领域系统。中心辐射型布局，中央平台连接子系统。语义线型（`.data-line`、`.power-line`、`.water-pipe`、`.road`）。参见 `references/infrastructure-patterns.md`。
8.  **UI / 仪表盘线框图** — 管理面板、监控仪表盘。屏幕框架包含嵌套的图表/仪表/指示器元素。参见 `references/dashboard-patterns.md`。

对于物理、基础设施和仪表盘图表，请在生成前加载相应的参考文件 — 每个文件都提供了现成的 CSS 类和形状基元。
---
...
1.  每个 `<text>` 都有 `t`、`ts` 或 `th` 类。
2.  框内的每个 `<text>` 都有 `dominant-baseline="central"`。
3.  每个用作箭头的连接器 `<path>` 或 `<line>` 都有 `fill="none"`。
4.  没有箭头线穿过不相关的框。
...
**规则：**
- 仅绑定到 `127.0.0.1`。切勿使用 `0.0.0.0`。在共享网络上将所有图表暴露在所有网络接口上是安全风险。
- 选择一个空闲端口（不要硬编码端口号）并告知用户所选 URL。
...
| `smart-city-infrastructure.md` | 中心辐射型 | 每个系统的语义线型 |

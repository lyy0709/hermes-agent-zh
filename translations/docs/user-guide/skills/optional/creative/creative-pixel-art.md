...
使用时代调色板（NES、Game Boy、PICO-8）的像素艺术。
...
每个脚本都可导入或直接运行。预设会在您需要时代精确色彩（NES、Game Boy、PICO-8 等）时适配硬件调色板，或使用自适应 N 色量化来获得街机/SNES 风格的外观。
...
首先运行 `pixel_art()`；如果请求了动画，则链式调用 `pixel_art_video()` 处理结果。
...
4.  使用 Floyd-Steinberg 抖动进行量化 — 针对自适应 N 色调色板或命名的硬件调色板
5.  使用 `Image.NEAREST` 重新上采样

在下采样**之后**进行量化，可以使抖动与最终的像素网格对齐。在之前量化会浪费误差扩散在即将消失的细节上。

**视频叠加：**
- 每帧复制基础帧（静态背景）
- 叠加无状态逐帧粒子绘制（每个特效一个函数）
...
- 动画粒子数量针对约 640x480 的画布进行了调整。对于非常大的图像，您可能需要使用不同的种子进行第二次处理以获得合适的密度。
- `mono_green` / `mono_amber` 强制 `color=0.0`（去饱和）。如果覆盖此设置并保留色度，2 色调色板可能会在平滑区域产生条纹。
- `clarify` 循环：每轮最多调用两次（风格，然后场景）。不要用更多选择来打扰用户。
...
命名的硬件调色板和 `pixel_art_video.py` 中的程序化动画循环移植自 [pixel-art-studio](https://github.com/Synero/pixel-art-studio) (MIT)。有关详细信息，请参阅此技能目录中的 `ATTRIBUTION.md`。
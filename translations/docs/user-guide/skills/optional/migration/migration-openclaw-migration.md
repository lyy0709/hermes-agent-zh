...
将用户的 OpenClaw 自定义配置迁移到 Hermes Agent。从 `~/.openclaw` 导入与 Hermes 兼容的记忆、SOUL.md、命令白名单、用户技能和选定的工作空间资源，然后报告无法迁移的内容及其原因。
...
当用户希望以最少的清理工作将其 OpenClaw 设置迁移到 Hermes Agent 时，请使用此技能。
...
CLI 命令运行的是下文描述的相同迁移脚本。当您希望通过 Agent 进行交互式、引导式迁移，并包含预览和逐项冲突解决时，请使用此技能（通过 Agent）。
...
- 将 OpenClaw 的命令批准模式合并到 Hermes 的 `command_allowlist`
- 迁移与 Hermes 兼容的消息设置，如 `TELEGRAM_ALLOWED_USERS` 和 `MESSAGING_CWD`
- 将 OpenClaw 技能复制到 `~/.hermes/skills/openclaw-imports/`
- 可选地将 OpenClaw 工作空间说明文件复制到选定的 Hermes 工作空间
- 将兼容的工作空间资源（如 `workspace/tts/`）镜像到 `~/.hermes/tts/`
...
4. 调用终端工具时，不要传递 `workdir: "~"`。使用绝对目录（如用户的主目录），或者完全省略 `workdir`。

使用 `--migrate-secrets` 时，它还会导入一小部分允许的、与 Hermes 兼容的密钥，目前包括：
...
1.  首先进行试运行检查。
2.  简要总结可以迁移的内容、无法迁移的内容以及将要归档的内容。
3.  如果 `clarify` 工具可用，请使用它来进行用户决策，而不是请求自由格式的文本回复。
4.  如果试运行发现导入的技能目录存在冲突，请在执行前询问如何处理。
5.  在执行前，请用户在两个支持的迁移模式之间进行选择。
6.  仅当用户希望迁移工作空间说明文件时，才询问目标工作空间路径。
7.  使用匹配的预设和标志执行迁移。
8.  总结结果，特别是：
    - 已迁移的内容
    - 已归档以供手动审查的内容
...
- 切勿发出占位符或截断的选项，如 `...`
- 切勿使用额外的空白字符填充或样式化选项
- 切勿在问题中包含虚假的表单字段，如 `在此输入目录`、需要填写的空行或下划线 `_____`
- 对于开放式的路径问题，只询问简单的句子；用户在面板下方的普通 CLI 提示符中输入

如果 `clarify` 调用返回错误，请检查错误文本，更正有效载荷，并使用有效的 `question` 和干净的选项重试一次。

当 `clarify` 可用且试运行显示任何需要用户决策的内容时，您的**下一个操作必须是调用 `clarify` 工具**。
不要以普通的助手消息结束回合，例如：

- “让我展示选项”
- “您想做什么？”
- “以下是选项”
如果需要用户做出决定，请在生成更多文本前通过 `clarify` 收集。
如果存在多个未解决的决策，不要在它们之间插入解释性的助手消息。在收到一个 `clarify` 响应后，你的下一个动作通常应该是下一个所需的 `clarify` 调用。
...
1.  对于 `SOUL.md` 冲突，使用 `clarify`，并提供如下选择：
    ...
    - `review first`
2.  如果试运行显示一个或多个 `kind="skill"` 且 `status="conflict"` 的项目，使用 `clarify`，并提供如下选择：
...
8.  `full compatible migration` 意味着：迁移相同的兼容用户数据，并在存在时加上允许列表中的密钥。
...
-   `{"question":"您是否希望将 OpenClaw 工作区指令文件复制到 Hermes 工作区中？","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
在执行前，用通俗语言重述确切的命令计划，并确保其与用户的选择相符。
...
4.  除非 `kind="soul"` 的报告项目其 `status="migrated"`，否则不要说 `SOUL.md` 已被覆盖。
5.  如果 `report.summary.conflict > 0`，则包含一个冲突部分，而不是默默地暗示成功。
6.  如果计数与列出的项目不符，请在响应前修复列表以匹配报告。
7.  当报告中有 `output_dir` 路径时，包含该路径，以便用户可以检查 `report.json`、`summary.md`、备份和归档文件。
8.  对于记忆或用户配置文件溢出，除非报告明确显示归档路径，否则不要说条目已被归档。如果存在 `details.overflow_file`，则说明完整的溢出列表已导出到该处。
9.  如果一个技能被导入到重命名的文件夹下，报告最终目的地并提及 `details.renamed_from`。
10. 如果存在 `report.skill_conflict_mode`，则将其用作所选导入技能冲突策略的事实来源。
11. 如果一个项目的 `status="skipped"`，则不要将其描述为已覆盖、已备份、已迁移或已解决。
12. 如果 `kind="soul"` 的 `status="skipped"` 且原因为 `Target already matches source`，则说明其保持不变，并且不要提及备份。
13. 如果一个重命名的导入技能其 `details.backup` 为空，则不要暗示现有的 Hermes 技能已被重命名或备份。只需说明导入的副本被放置在新目的地，并引用 `details.renamed_from` 作为保留在原处的预先存在的文件夹。
## 迁移预设
...
默认情况下，不要使用 `$PWD` 或主目录作为工作区目标。首先请求一个明确的工作区路径。
...
5.  优先使用主要的 OpenClaw 工作区 (`~/.openclaw/workspace/`) 而非 `workspace.default/`。仅当主要文件缺失时，才使用默认工作区作为备用。
...
13. 试运行后，如果仍有未解决的决策，绝不要在总结后就停止。立即使用 `clarify` 处理最高优先级的阻塞决策。
14. 后续问题的优先级顺序：
    - `SOUL.md` 冲突
    - 导入的技能冲突
    - 迁移模式
    - 工作区指令目标
15. 不要承诺在同一消息中稍后呈现选择。通过实际调用 `clarify` 来呈现它们。
...
- 导入的 Hermes 人格状态
- 填充了已转换的 OpenClaw 知识的 Hermes 记忆文件

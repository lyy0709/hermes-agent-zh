...
description: "将 alibaba/page-agent 嵌入到你的 Web 应用程序中——这是一个纯 JavaScript 的页面内 GUI Agent，以单个 <script> 标签或 npm 包的形式提供，让网站最终用户能够使用自然语言驱动 UI（“点击登录，将用户名填写为 John”）。无需 Python，无需无头浏览器，无需浏览器扩展。当用户是希望为其 SaaS / 管理面板 / B2B 工具添加 AI 副驾驶的 Web 开发者，或希望通过自然语言访问遗留 Web 应用，或想针对本地（Ollama）或云端（Qwen / OpenAI / OpenRouter）LLM 评估 page-agent 时，请使用此技能。不适用于服务器端浏览器自动化——请将此类用户指向 Hermes 内置的浏览器工具。"
---
...
将 alibaba/page-agent 嵌入到您自己的 Web 应用程序中——这是一个纯 JavaScript 的页面内 GUI Agent，以单个 &lt;script> 标签或 npm 包的形式提供，让您网站的最终用户能够使用自然语言驱动 UI（“点击登录，将用户名填写为 John”）。无需 Python，无需无头浏览器，无需浏览器扩展。当用户是希望为其 SaaS / 管理面板 / B2B 工具添加 AI 副驾驶的 Web 开发者，或希望通过自然语言访问遗留 Web 应用，或想针对本地（Ollama）或云端（Qwen / OpenAI / OpenRouter）LLM 评估 page-agent 时，请使用此技能。不适用于服务器端浏览器自动化——请将此类用户指向 Hermes 内置的浏览器工具。
...
alibaba/page-agent (https://github.com/alibaba/page-agent, 17k+ stars, MIT) 是一个用 TypeScript 编写的页面内 GUI Agent。它存在于网页内部，将 DOM 读取为文本（无需截图，无需多模态 LLM），并针对当前页面执行自然语言指令，如“点击登录按钮，然后将用户名填写为 John”。纯客户端——宿主网站只需包含一个脚本并传递一个 OpenAI 兼容的 LLM 端点。

## 何时使用此技能

当用户希望实现以下目标时，加载此技能：

- **在自己的 Web 应用程序中部署 AI 副驾驶**（SaaS、管理面板、B2B 工具、ERP、CRM）——“我仪表板上的用户应该能够输入‘为 Acme Corp 创建发票并通过邮件发送’，而不是点击五个屏幕”
...
- **构建交互式培训/产品演示**——让 AI 在真实的 UI 中引导用户完成“如何提交费用报告”
...
- Node 22.13+ 或 24+，npm 10+（文档声称需要 11+，但 10.9 运行良好）
...
添加到任何 HTML 页面（或作为书签脚本粘贴到开发者工具控制台）：
...
书签脚本形式（拖到书签栏，在任何页面点击）：

```javascript
javascript:(function(){var s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/page-agent@1.8.0/dist/iife/page-agent.demo.js';document.head.appendChild(s);})();
```

## 路径 2 — 通过 npm 安装到您自己的 Web 应用程序（生产使用）
...
使用您自己的 LLM 端点进行配置——**切勿将演示 CDN 分发给真实用户**：
...
**安全性。** 对于真实部署，不要将您的 `apiKey` 放在客户端代码中——通过您的后端代理 LLM 调用，并将 `baseURL` 指向您的代理。演示 CDN 存在是因为阿里巴巴运行该代理用于评估。

## 路径 3 — 克隆源代码仓库（贡献或进行修改）

当用户想要修改 page-agent 本身、通过本地 IIFE 包在任意网站上测试，或开发浏览器扩展时使用此路径。
...
然后：运行 `npm run dev:demo`，在任何页面上点击书签脚本，本地构建就会注入。保存时自动重新构建。

**警告：** 您的 `.env` 文件中的 `LLM_API_KEY` 在开发构建期间会被内联到 IIFE 包中。不要分享该包。不要提交它。不要将 URL 粘贴到 Slack 中。（已验证：在公共开发包中 grep 会返回来自 `.env` 的字面值。）
...
2. 您应该看到一个浮动面板。如果没有，请检查控制台是否有错误（最常见：LLM 端点的 CORS 问题、错误的 `baseURL` 或无效的 API 密钥）
3. 输入一个与页面上可见内容匹配的简单指令（“点击登录链接”）
4. 观察网络选项卡——您应该能看到一个发往您的 `baseURL` 的请求
...
- **在生产环境中使用演示 CDN** — 不要这样做。它受速率限制，使用阿里巴巴的免费代理，并且其条款禁止生产使用。
- **API 密钥暴露** — 传递给 `new PageAgent({apiKey: ...})` 的任何密钥都会包含在您的 JS 包中。对于真实部署，始终通过您自己的后端进行代理。
- **非 OpenAI 兼容的端点** 会静默失败或出现隐晦错误。如果您的提供商需要原生 Anthropic/Gemini 格式，请在前面使用 OpenAI 兼容性代理（LiteLLM、OpenRouter）。
- **CSP 阻止** — 具有严格内容安全策略的网站可能会拒绝加载 CDN 脚本或禁止内联 eval。在这种情况下，请从您自己的源自行托管。
- **在路径 3 中编辑 `.env` 后重启开发服务器** — Vite 仅在启动时读取环境变量。
- **Node 版本** — 仓库声明需要 `^22.13.0 || >=24`。Node 20 将导致 `npm ci` 因引擎错误而失败。

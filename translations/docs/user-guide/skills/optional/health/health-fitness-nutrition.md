---
title: "健身营养 — 健身房训练计划制定与营养追踪"
sidebar_label: "健身营养"
description: "健身房训练计划制定与营养追踪"
---
...
健身房训练计划制定与营养追踪。通过 wger 按肌肉、器械或类别搜索 690+ 种训练动作。通过 USDA FoodData Central 查询 380,000+ 种食物的宏量营养素和卡路里。计算 BMI、TDEE、单次最大重量、宏量营养素分配和体脂率 — 纯 Python 实现，无需 pip 安装。专为追求增肌、减重或只是想吃得更好的人打造。
...
- **wger** (https://wger.de/api/v2/) — 开放训练动作数据库，690+ 个包含肌肉、器械、图片的动作。公共端点无需任何身份验证。
...
当用户询问以下内容时触发此技能：
- 训练动作、训练计划、健身房常规训练、肌肉群、训练分化
- 食物宏量营养素、卡路里、蛋白质含量、饮食计划、卡路里计算
...
所有 wger 公共端点都返回 JSON 且无需身份验证。始终在训练动作查询中添加 `format=json` 和 `language=2`（英语）。
...
查看 `references/FORMULAS.md` 了解每个公式背后的科学原理。

---

## 注意事项

- wger 训练动作端点默认返回**所有语言** — 始终添加 `language=2` 以获取英语内容
- wger 包含**未经验证的用户提交内容** — 添加 `status=2` 以仅获取已批准的训练动作
- USDA `DEMO_KEY` 有**30 次请求/小时**的限制 — 在批量请求之间添加 `sleep 2` 或获取免费密钥
- USDA 数据是**每 100 克**的 — 提醒用户根据实际份量进行换算
- BMI 无法区分肌肉和脂肪 — 肌肉发达的人 BMI 高不一定不健康
- 体脂率公式是**估算值**（±3-5%） — 建议使用 DEXA 扫描以获得精确结果
- 1RM 公式在超过 10 次重复次数时准确性下降 — 使用 3-5 次重复的组数以获得最佳估算
...
运行训练动作搜索后：确认结果包含训练动作名称、肌肉群和器械。
营养查询后：确认返回了每 100 克的宏量营养素，包括卡路里、蛋白质、脂肪、碳水化合物。
计算器计算后：对输出进行合理性检查（例如，大多数成年人的 TDEE 应在 1500-3500 之间）。
...
| 任务 | 来源 | 端点 |
|------|------|------|
| 按名称搜索训练动作 | wger | `GET /api/v2/exercise/search/?term=&language=english` |
| 训练动作详情 | wger | `GET /api/v2/exerciseinfo/{id}/` |

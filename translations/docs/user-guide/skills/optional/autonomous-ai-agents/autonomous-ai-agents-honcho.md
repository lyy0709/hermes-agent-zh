...
description: "配置和使用 Hermes 的 Honcho 记忆功能——跨会话用户建模、多配置文件对等体隔离、观察配置、辩证推理、会话摘要和上下文预算强制执行。在设置 Honcho、排查记忆问题、使用 Honcho 对等体管理配置文件或调整观察、回忆和辩证设置时使用。"
---
...
配置和使用 Hermes 的 Honcho 记忆功能——跨会话用户建模、多配置文件对等体隔离、观察配置、辩证推理、会话摘要和上下文预算强制执行。在设置 Honcho、排查记忆问题、使用 Honcho 对等体管理配置文件或调整观察、回忆和辩证设置时使用。
...
会话摘要在每个回合开始时由 Honcho 自动生成（当存在先前的会话时）。它让模型无需重放完整历史就能获得一个“热启动”。
...
| 条件 | 策略 | 发生的情况 |
|-----------|----------|--------------|
| 没有先前的会话或用户表征为空 | **冷启动** | 轻量级介绍提示词；跳过摘要注入；鼓励模型了解用户 |
| 存在用户表征和/或会话历史 | **热启动** | 完整的基础上下文注入（摘要 → 用户表征 → 卡片）；更丰富的系统提示词 |

您无需配置此选项——它会根据会话状态自动选择。
...
-   **AI 对等体** (`aiPeer`): 代表此 Hermes 实例。每个配置文件都有自己的 AI 对等体，因此 Agent 可以形成独立的视图。
...
| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `contextCadence` | `1` | 两次上下文 API 调用之间的最小轮次间隔 |
| `dialecticCadence` | `2` | 两次辩证推理 API 调用之间的最小轮次间隔。建议 1–5 |
| `injectionFrequency` | `every-turn` | 基础上下文注入频率：`every-turn` 或 `first-turn` |

较高的节奏值会减少辩证推理 LLM 的调用频率。`dialecticCadence: 2` 表示引擎每隔一轮调用一次。设置为 `1` 则每轮都调用。
...
这可以保持早期轮次成本低廉，同时在最终综合时使用完整深度。

**会话开始时的深度。** 会话开始的预热会在第 1 轮之前，在后台运行配置的完整 `dialecticDepth`。对冷启动的 Peer 进行单轮预热通常返回较少的输出 —— 多轮深度在用户开口之前就运行了审计/协调周期。第 1 轮直接使用预热结果；如果预热未能及时完成，第 1 轮会回退到具有有限超时的同步调用。
...
每个 Hermes 配置文件都有自己的 Honcho AI Peer，同时共享同一个工作区（用户上下文）。这意味着：

- 所有配置文件看到相同的用户表示
- 每个配置文件构建自己的 AI 身份和观察结果
- 一个配置文件写入的结论可以通过共享工作区对其他配置文件可见
...
| 工具 | 调用 LLM？ | 成本 | 使用场景 |
|------|-----------|------|----------|
| `honcho_profile` | 否 | minimal | 在对话开始时快速获取事实快照，或用于快速查找姓名/角色/偏好 |
| `honcho_search` | 否 | low | 获取特定的过去事实供自己推理 —— 原始摘录，无综合 |
| `honcho_context` | 否 | low | 完整的会话上下文快照：摘要、表示、卡片、最近消息 |
| `honcho_reasoning` | 是 | medium–high | 由 Honcho 的辩证推理引擎综合的自然语言问题 |
| `honcho_conclude` | 否 | minimal | 写入或删除持久性事实；传递 `peer: "ai"` 用于 AI 自我认知 |
...
### `honcho_context`
来自 Honcho 的完整会话上下文快照 —— 会话摘要、Peer 表示、Peer 卡片和最近消息。不调用 LLM。当你想一次性查看 Honcho 对当前会话和 Peer 了解的所有信息时使用。

### `honcho_reasoning`
由 Honcho 的辩证推理引擎回答的自然语言问题（在 Honcho 后端调用 LLM）。成本更高，质量更高。传递 `reasoning_level` 来控制深度：`minimal`（快速/廉价） → `low` → `medium` → `high` → `max`（彻底）。省略则使用配置的默认值（`low`）。用于对用户的模式、目标或当前状态进行综合理解。

### `honcho_conclude`
写入或删除关于 Peer 的持久性结论。传递 `conclusion: "..."` 以创建。传递 `delete_id: "..."` 以删除结论（用于 PII 移除 —— Honcho 会随时间自我修复不正确的结论，因此删除仅用于 PII）。你必须且只能传递这两个参数中的一个。
...
- `peer: "<explicit-id>"` —— 工作区中的任何 Peer ID
示例：
```
honcho_profile                        # 读取用户名片
honcho_profile peer="ai"              # 读取 AI 对等体名片
honcho_reasoning query="此用户最关心什么？"
...
**不要**在每一轮都调用 `honcho_reasoning`。自动注入已经处理了持续的上下文刷新。仅当你真正需要基础上下文未提供的综合洞察时，才使用推理工具。
...
差的结论："用户说了些关于 Rust 的事情"（太模糊）、"用户似乎懂技术"（已在表征中）
...
在 `hybrid` 和 `context` 模式下，基础上下文（用户表征 + 名片 + 会话摘要）会在每一轮之前自动注入。**不要**重新获取已注入的内容。仅在以下情况调用工具：
- 你需要注入的上下文所没有的内容
- 用户明确要求你回忆或检查记忆
- 你正在撰写关于新事物的结论
...
工具端的 `honcho_reasoning` 与自动注入的辩证推理共享相同的成本。在显式工具调用之后，自动注入节奏会重置 —— 避免对同一轮次重复计费。
...
### 消息被截断
超过 `messageMaxChars`（默认 25k）的消息会自动分块，并带有 `[continued]` 标记。如果经常遇到此情况，请检查是否是工具结果或技能内容增大了消息大小。
### 上下文注入过大
如果看到关于超出上下文预算的警告，请降低 `contextTokens` 或减少 `dialecticDepth`。当预算紧张时，会话摘要会首先被修剪。

### 会话摘要缺失
会话摘要要求当前 Honcho 会话中至少有一个先前的轮次。在冷启动（新会话，无历史记录）时，摘要会被省略，Honcho 转而使用冷启动提示词策略。
...
| `hermes honcho identity` | 植入 AI 对等方身份或显示两个对等方的表示形式 |

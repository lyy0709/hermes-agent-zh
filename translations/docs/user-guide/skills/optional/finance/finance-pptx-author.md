...
使用 `python-pptx` 在磁盘上生成 .pptx 文件。当您需要交付演示文稿文件作为产物，而不是驱动实时 PowerPoint 会话时使用。

改编自 Anthropic 在 [anthropics/financial-services](https://github.com/anthropics/financial-services) 中的 `pptx-author` 和 `pitch-deck` 技能。原始版本中的 MCP / Office-JS 分支已被移除——此技能假设使用无头 Python。

对于更广泛、已内置的 PowerPoint 创作技能（幻灯片、演讲者备注、嵌入、媒体），请参阅内置的 `powerpoint` 技能。此技能是一个更轻量级的模式，专为模型驱动的演示文稿（推介演示文稿、投资委员会备忘录、财报说明）调整，其中每个数字都必须能追溯到源工作簿。
...
### 每张幻灯片一个核心观点
标题陈述要点；正文提供支持。标题为“Q3 营收”的幻灯片较弱；“Q3 营收同比增长加速至 14%”则较强。
...
切勿凭记忆或摘要转录数字——打开工作簿，读取命名区域，并在可能的情况下以编程方式将演示文稿中的值绑定到该区域。
...
当保真度很重要时（模型的图表样式必须与演示文稿完全匹配），请从源工作簿将图表渲染为 PNG 并嵌入图像。原生的 `pptx.chart` 图表很脆弱，通常不符合公司惯例。
...
从您的 Excel 模型中读取命名区域或特定单元格，以确保演示文稿中的数字永不偏离。
...
请记住，在读取工作簿之前重新计算它——openpyxl 只有在工作表已被计算过时才能看到计算值。首先运行 `excel-author` 技能中的重新计算辅助函数，或通过真实的 Excel 会话打开/保存。

## 推介演示文稿的幻灯片类型清单

典型的投行推介演示文稿遵循此结构。非硬性规定，但可作为起始骨架：
...
- 用户处于实时 PowerPoint 会话中且 Office MCP 可用时——改为驱动其实时文档。

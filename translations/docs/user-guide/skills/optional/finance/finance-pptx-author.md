...
使用 python-pptx 无头构建 PowerPoint 演示文稿。可与 excel-author 技能搭配使用，创建每个数字都能追溯到工作簿单元格的模型驱动型演示文稿。适用于推介材料、投资委员会备忘录、财报说明等场景。
...
使用 `python-pptx` 在磁盘上生成 .pptx 文件。当您需要交付演示文稿作为文件产物，而非驱动实时 PowerPoint 会话时使用。

改编自 Anthropic 在 [anthropics/financial-services](https://github.com/anthropics/financial-services) 中的 `pptx-author` 和 `pitch-deck` 技能。原始版本中的 MCP / Office-JS 分支已被移除——此技能假设为无头 Python 环境。

对于更广泛、已内置的 PowerPoint 创作技能（幻灯片、演讲者备注、嵌入、媒体），请参阅内置的 `powerpoint` 技能。此技能是一个更轻量级的模式，专为模型驱动型演示文稿（推介材料、投资委员会备忘录、财报说明）调整，其中每个数字都必须追溯到源工作簿。
...
### 每个数字都追溯到模型
如果幻灯片上的数字来自 `./out/model.xlsx`，请注明工作表名称和单元格。

```
Revenue: $1,250M  (Source: model.xlsx, Inputs!C3)
```

切勿凭记忆或摘要转录数字——打开工作簿，读取命名范围，并在可能的情况下以编程方式将演示文稿中的值与之绑定。
...
从您的 Excel 模型中读取命名范围或特定单元格，以确保演示文稿中的数字永不偏离。
...
请记住，在读取工作簿之前重新计算它——openpyxl 只有在工作表已被计算过的情况下才能看到计算值。首先运行 `excel-author` 技能中的重新计算辅助函数，或通过真实的 Excel 会话打开/保存。

## 推介材料幻灯片类型清单

典型的投行推介材料遵循以下结构。非规定性，但可作为起始骨架：
...
- 非财务幻灯片（季度全员会议、营销材料）——使用更广泛的 `powerpoint` 技能。
- 包含大量动画、过渡或演讲者备注的演示文稿——使用更广泛的 `powerpoint` 技能。

## 归属

约定改编自 Anthropic 的 Claude for Financial Services 插件套件，Apache-2.0 许可。原始来源：https://github.com/anthropics/financial-services/tree/main/plugins/agent-plugins/pitch-agent/skills/pptx-author
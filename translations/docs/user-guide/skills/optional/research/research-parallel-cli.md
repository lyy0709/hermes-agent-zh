...
description: "Parallel CLI 的可选供应商技能 —— 专为 Agent 设计的网页搜索、提取、深度研究、数据增强、FindAll 和监控功能"
---
...
Parallel CLI 的可选供应商技能 —— 专为 Agent 设计的网页搜索、提取、深度研究、数据增强、FindAll 和监控功能。优先使用 JSON 输出和非交互式流程。
...
当用户明确要求使用 Parallel，或者当终端原生工作流能够受益于 Parallel 供应商特定的网页搜索、提取、深度研究、数据增强、实体发现或监控技术栈时，请使用 `parallel-cli`。
...
- 当用户特别提到 Parallel 或需要 Parallel 特有的功能（如数据增强、FindAll 或监控工作流）时，请优先使用此技能。
...
- 在一个 CLI 中实现搜索、提取、研究、数据增强、实体发现和监控
...
- 你需要可以稍后启动和轮询的异步深度研究任务
- 你需要结构化数据增强、FindAll 实体发现或监控功能

当用户没有特别要求 Parallel 时，对于快速的一次性查询，请优先使用 Hermes 原生的 `web_search` / `web_extract`。
...
如果你想要一个隔离的 Python 安装，也可以使用 `pipx`：
...
4.  仅引用 CLI 输出返回的 URL。
5.  当后续问题可能出现时，将大型 JSON 输出保存到临时文件。
6.  仅对真正长时间运行的工作流使用后台进程；否则在前台运行。
7.  除非用户特别要求 Parallel 或需要 Parallel 特有的工作流，否则优先使用 Hermes 原生工具。
...
当用户拥有 CSV/JSON/表格输入并希望通过网页研究推断出额外的列时使用。
...
当用户想要一个可发现的数据集而不是简短答案时，用于网络规模的实体发现。
...
当用户想要一个可以稍后审查、过滤或增强的可发现实体集合时，这比普通搜索更合适。
...
当用户希望对页面或来源进行周期性跟踪而不是一次性获取时，请使用此功能。
...
- 请记住，这是一个供应商工作流，通常需要账户身份验证，并且在免费层级之外通常需要付费使用。
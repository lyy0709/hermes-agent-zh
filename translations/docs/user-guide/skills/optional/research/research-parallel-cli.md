...
description: "Parallel CLI 的可选供应商技能 —— 面向 Agent 的网页搜索、提取、深度研究、信息增强、FindAll 和监控"
---
...
Parallel CLI 的可选供应商技能 —— 面向 Agent 的网页搜索、提取、深度研究、信息增强、FindAll 和监控。优先使用 JSON 输出和非交互式流程。
...
当用户明确需要 Parallel，或者当终端原生工作流能够受益于 Parallel 供应商特定的网页搜索、提取、深度研究、信息增强、实体发现或监控能力时，请使用 `parallel-cli`。
...
- 当用户明确提到 Parallel 或需要 Parallel 特有的功能（如信息增强、FindAll 或监控工作流）时，请优先使用此技能。
...
- 在一个 CLI 中集成搜索、提取、研究、信息增强、实体发现和监控
...
- 你需要结构化的信息增强、FindAll 实体发现或监控功能

当用户未明确要求 Parallel 时，对于快速的一次性查询，请优先使用 Hermes 原生的 `web_search` / `web_extract`。
...
当用户拥有 CSV/JSON/表格输入并希望基于网页研究推断出额外的列时使用。
...
当用户想要一个可以稍后审查、过滤或增强的已发现实体集合时，这比普通搜索更合适。

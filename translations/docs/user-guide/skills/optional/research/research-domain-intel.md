...
使用 Python 标准库进行被动域名侦察。包含子域名发现、SSL 证书检查、WHOIS 查询、DNS 记录、域名可用性检查以及批量多域名分析。无需 API 密钥。

## 技能元数据

| | |
|---|---|
| 来源 | Optional — 使用 `hermes skills install official/research/domain-intel` 安装 |
...
# SSL 证书检查（到期时间、密码套件、SANs、颁发者）
python3 SKILL_DIR/scripts/domain_intel.py ssl example.com

# WHOIS 查询（注册商、日期、名称服务器 — 支持 100+ 个 TLD）
...
在 Linux、macOS 和 Windows 上无需依赖即可完全相同地工作。

- **crt.sh 查询** 使用 HTTPS（端口 443）— 可在大多数防火墙后工作
- **WHOIS 查询** 使用 TCP 端口 43 — 在限制性网络上可能被阻止
- **DNS 查询** 使用 Google DoH (HTTPS) 进行 MX/NS/TXT 解析 — 防火墙友好
...
- WHOIS 查询使用 TCP 端口 43 — 在限制性网络上可能被阻止
- 某些 WHOIS 服务器会隐藏注册人信息（GDPR）— 向用户说明这一点
- crt.sh 对于非常流行的域名（数千个证书）可能很慢 — 设定合理的预期
- 可用性检查基于启发式方法（3 个被动信号）— 不像注册商 API 那样权威

...
title: "地图"
sidebar_label: "地图"
description: "位置智能——地理编码地点、反向地理编码坐标、查找附近地点（46个POI类别）、驾驶/步行/骑行距离+时间、逐向导航、时区查询、命名地点的边界框+面积，以及矩形区域内的POI搜索。使用OpenStreetMap + Overpass + OSRM。免费，无需API密钥。"
---
...
位置智能——地理编码地点、反向地理编码坐标、查找附近地点（46个POI类别）、驾驶/步行/骑行距离+时间、逐向导航、时区查询、命名地点的边界框+面积，以及矩形区域内的POI搜索。使用 OpenStreetMap + Overpass + OSRM。免费，无需 API 密钥。
...
使用免费、开放数据源的位置智能。8个命令，44个POI类别，零依赖（仅使用 Python 标准库），无需 API 密钥。

数据源：OpenStreetMap/Nominatim、Overpass API、OSRM、TimeAPI.io。

此技能取代了旧的 `find-nearby` 技能——`find-nearby` 的所有功能都由下面的 `nearby` 命令覆盖，并具有相同的 `--near "<place>"` 快捷方式和多类别支持。
...
`maps_url`（可点击的 Google 地图链接）, `directions_url`（从搜索点到该地点的 Google 地图导航链接），以及可用的推广标签——`cuisine`, `hours` (opening_hours), `phone`, `website`。
...
返回带编号的步骤，包含指令、距离、持续时间、道路名称和操作类型（转弯、出发、到达等）。
...
在地理矩形内查找 POI。首先使用 `area` 获取命名地点的边界框坐标。
...
```bash
# 用户在 36.17, -115.14 发送了一个图钉，并询问“查找附近的咖啡馆”
...
- 如果仅使用邮政编码在全球范围内产生歧义结果，请包含国家/州信息

...
通过 GraphQL API 管理 Linear 的问题、项目和团队。创建、更新、搜索和组织问题。使用 API 密钥认证（无需 OAuth）。所有操作均通过 curl 执行 — 无额外依赖。
...
使用 `curl` 直接通过 GraphQL API 管理 Linear 的问题、项目和团队。无需 MCP 服务器，无需 OAuth 流程，无需额外依赖。
...
- 如果创建问题时省略 `stateId`，Linear 将默认为第一个 backlog 状态

...
1.  在 https://airtable.com/create/tokens 创建一个**个人访问令牌**（以 `pat...` 开头）。
2.  授予以下最小范围：
    - `data.records:read` — 读取行
    - `data.records:write` — 创建 / 更新 / 删除行
    - `schema.bases:read` — 列出基础和工作表
...
> 注意：传统的 `key...` API 密钥已于 2024 年 2 月弃用。现在只有 PAT 和 OAuth 令牌有效。
...
- **对象 ID：** 基础 `app...`、表格 `tbl...`、记录 `rec...`、字段 `fld...`。ID 永不改变；名称可以改变。在自动化中优先使用 ID。
...
`-s` 抑制 curl 的进度条——为保持 Hermes 的工具输出整洁，请为每次调用都设置此选项。通过 `python3 -m json.tool`（始终存在）或 `jq`（如果已安装）管道传输以获得可读的 JSON。
...
列表端点每页最多返回 **100 条记录**。如果响应包含 `"offset": "..."`，则在下次调用时将其传回。循环直到该字段不存在：
...
2.  **找到基础。** 列出基础（上述步骤）**或者**如果令牌缺少 `schema.bases:read` 权限，直接向用户询问 `app...` ID。
3.  **检查模式。** `GET /v0/meta/bases/$BASE_ID/tables` — 在修改任何内容之前，在会话中本地缓存确切的字段名称和主字段名称。
4.  **先读后写。** 对于“更新满足 Y 条件的 X”，先使用 `filterByFormula` 来解析 `rec...` ID，然后使用 `PATCH /v0/$BASE_ID/$TABLE/$RECORD_ID`。切勿猜测记录 ID。
5.  **批量写入。** 将相关的创建操作合并到一个 10 条记录的 POST 请求中，以保持在 5 个请求/秒的预算内。
6.  **破坏性操作。** 删除操作无法通过 API 撤销。如果用户说“删除所有 X”，请回显筛选条件和记录数量，并在执行前进行确认。
...
- **PATCH 与 PUT。** `PATCH` 将提供的字段合并到记录中。`PUT` 会完全替换记录，并清除你未包含的任何字段。默认使用 `PATCH`。
- **单选选项必须存在。** 当 `Shipping` 不在字段的选项列表中时，写入 `"Status": "Shipping"` 会导致 `INVALID_MULTIPLE_CHOICE_OPTIONS` 错误，除非你传递 `"typecast": true`（这会自动创建该选项）。
- **按基础令牌范围限定。** 一个基础返回 `403` 而另一个基础正常，意味着令牌的访问列表未包含该基础——这不是范围或认证问题。请将用户引导至 https://airtable.com/create/tokens 进行授权。
- **速率限制是按基础计算的，而不是按令牌。** 在 `baseA` 上 5 个请求/秒和在 `baseB` 上 5 个请求/秒是没问题的；仅在 `baseA` 上 6 个请求/秒将被限制。监控 `429` 响应中的 `Retry-After` 头。

...
使用时代调色板（NES、Game Boy、PICO-8）将任何图像转换为复古像素艺术。
...
将任何图像转换为复古像素艺术，然后可选地将其动画化为带有时代特效（雨、萤火虫、雪、余烬）的短 MP4 或 GIF。
...
每个脚本都可导入或直接运行。预设可以锁定到硬件调色板以获得时代准确的色彩（NES、Game Boy、PICO-8 等），或使用自适应 N 色量化来获得街机/SNES 风格的外观。
...
在生成之前，请与用户确认风格。不同的预设会产生非常不同的输出，重新生成成本高昂。
...
**不要**连续调用 `clarify` 超过两次。一次用于风格，一次用于场景（如果需要动画）。如果用户在消息中明确要求了特定风格和场景，则完全跳过 `clarify`。

### 步骤 3 — 生成

首先运行 `pixel_art()`；如果请求了动画，则对结果链接运行 `pixel_art_video()`。
...
| `teletext` | BBC Teletext | 8 种纯色 | 10px | 厚重的原色 |
| `mspaint` | Windows MS Paint | 24 种固定色 | 8px | 怀旧桌面风格 |
...
命名调色板位于 `scripts/palettes.py` 中（完整列表见 `references/palettes.md` — 总共 28 个命名调色板）。任何预设都可以被覆盖：
...
**像素转换：**
1.  增强对比度/色彩/锐度（对于较小调色板效果更强）
2.  在量化前进行色调分离以简化色调区域
3.  使用 `Image.NEAREST` 按 `block` 下采样（硬像素，无插值）
4.  使用 Floyd-Steinberg 抖动进行量化 — 使用自适应 N 色调色板或命名硬件调色板
5.  使用 `Image.NEAREST` 重新上采样

在下采样**之后**进行量化，可以使抖动与最终像素网格对齐。在之前量化会浪费误差扩散在即将消失的细节上。

**视频叠加：**
- 每帧复制基础帧（静态背景）
- 叠加无状态逐帧粒子绘制（每种特效一个函数）
...
- 非常小的源图像（宽度 <100px）在 8-10px 块大小下会崩溃。如果源图像很小，请先放大。
- 非整数的 `block` 或 `palette` 会破坏量化 — 保持它们为正整数。
- 动画粒子数量针对约 640x480 画布进行了调整。在非常大的图像上，您可能需要使用不同种子进行第二次传递以获得合适的密度。
- `mono_green` / `mono_amber` 强制 `color=0.0`（去饱和）。如果覆盖此设置并保留色度，2 色调色板可能会在平滑区域产生条纹。
- `clarify` 循环：每轮最多调用两次（风格，然后场景）。不要用更多选择打扰用户。

## 验证

- PNG 在输出路径创建
- 在预设的块大小下可见清晰的方形像素块
- 颜色数量与预设匹配（目视图像或运行 `Image.open(p).getcolors()`）
- 视频是有效的 MP4（`ffprobe` 可以打开）且大小非零

## 归属

命名硬件调色板和 `pixel_art_video.py` 中的程序化动画循环移植自 [pixel-art-studio](https://github.com/Synero/pixel-art-studio) (MIT)。详见此技能目录中的 `ATTRIBUTION.md`。
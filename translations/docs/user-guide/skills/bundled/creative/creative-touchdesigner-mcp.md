...
通过 twozero MCP 控制正在运行的 TouchDesigner 实例 —— 创建算子、设置参数、连接线路、执行 Python、构建实时视觉效果。包含 36 个原生工具。
...
1.  **切勿猜测参数名。** 首先为算子类型调用 `td_get_par_info`。你的训练数据对于 TD 2025.32 是错误的。
2.  **如果触发 `tdAttributeError`，立即停止。** 在继续之前，对失败的节点调用 `td_get_operator_info`。
3.  **切勿在脚本回调中硬编码绝对路径。** 使用 `me.parent()` / `scriptOp.parent()`。
4.  **优先使用原生 MCP 工具而非 `td_execute_python`。** 使用 `td_create_operator`、`td_set_operator_pars`、`td_get_errors` 等。仅在复杂的多步骤逻辑中回退到 `td_execute_python`。
5.  **在构建前调用 `td_get_hints`。** 它会返回与你正在处理的算子类型相关的特定模式。

## 架构

```
Hermes Agent -> MCP (可流式传输的 HTTP) -> twozero.tox (端口 40404) -> TD Python
```

包含 36 个原生工具。免费插件（无需付费/许可 —— 已于 2026 年 4 月确认）。
上下文感知（知道选中的 OP、当前网络）。
...
2.  **启用 MCP：** 点击 twozero 图标 → 设置 → mcp → "auto start MCP" → 是
...
-   **非商业版 TD** 将分辨率限制在 1280×1280。使用 `outputresolution = 'custom'` 并明确设置宽度/高度。
...
**重要：将清理和创建拆分为独立的 MCP 调用。** 在一个 `td_execute_python` 脚本中销毁和重新创建同名的节点会导致 "Invalid OP object" 错误。参见陷阱 #11b。
...
| 工具 | 说明 |
|------|------|
| `td_get_screenshot` | 将单个 OP 的视图捕获到文件 |
...
上表涵盖了典型创意工作流中使用的 32 个工具。其余 4 个工具（`td_project_quit`、`td_test_session`、`td_dev_log`、`td_clear_dev_log`）是管理/开发模式实用程序——完整的 36 个工具参考及完整的参数模式，请参阅 `references/mcp-tools.md`。

## 关键实现规则

**GLSL 时间：** GLSL TOP 中不要使用 `uTDCurrentTime`。使用 Values 页面：
```python
...
**Feedback TOP：** 使用 `top` 参数引用，而不是直接的输入连线。"Not enough sources" 在首次烹饪后会解决。"Cook dependency loop" 警告是预期的。
...
3.  **不要使用 Lag CHOP 进行频谱平滑。** Lag CHOP 在时间切片模式下运行，会将 256 个样本扩展到 2400+，将所有值平均到接近零（~1e-06）。着色器接收不到可用的数据。这是测试中音频同步失败的首要原因。
4.  **也不要使用 Filter CHOP** —— 频谱数据存在相同的时间切片扩展问题。
5.  **平滑处理应在 GLSL 着色器中进行**（如果需要），通过带有反馈纹理的时间线性插值：`mix(prevValue, newValue, 0.3)`。这样可以实现帧完美同步，且流水线延迟为零。
6.  **CHOP to TOP 的 dataformat = 'r'**，layout = 'rowscropped'。频谱输出是 256x2（立体声）。在 y=0.25 处采样第一通道。
...
完整的构建脚本和着色器代码请参见 `references/network-patterns.md`。

## 操作器快速参考

| 类别 | 颜色 | Python 类 / MCP 类型 | 后缀 |
|--------|-------|-------------|--------|
...
- `td_execute_python` 对 TD Python 执行环境和文件系统拥有不受限制的访问权限，权限与 TD 进程用户相同。
- `setup.sh` 从官方 404zero.com URL 下载 twozero.tox。如有疑虑，请验证下载内容。
- 此技能从不向 localhost 之外发送数据。所有 MCP 通信都是本地的。
...
| `references/operators.md` | 所有操作器类别及其参数和用例 |

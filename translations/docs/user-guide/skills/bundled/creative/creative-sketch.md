...
如果 `gsd-sketch` 显示为同级技能（通过 `npx get-shit-done-cc --hermes` 安装），则优先使用 **`gsd-sketch`** 以获得完整的工作流：包含持久化的 `.planning/sketches/` 和 MANIFEST、前沿模式分析、跨历史原型的一致性审计以及与 GSD 其余部分的集成。本技能是轻量级独立版本——无需状态机制的一次性原型设计。

## 核心方法

```
需求输入 → 变体生成 → 对比分析 → 选择优胜者（或迭代）
```

### 1. 需求输入（如果用户已提供足够信息则跳过）
...
-   **交互性：** 链接可点击、悬停效果真实、至少有一个状态转换（打开/关闭、筛选、切换）。一个冻结的静态图像比一个粗糙的动态原型更糟糕。
...
`browser_vision` 返回页面上实际内容的 AI 描述以及截图路径——能捕捉到纯源代码检查会遗漏的布局 bug（例如，字体导入静默失败、flex 容器塌陷）。修复并重新导航，直到每个变体看起来都正确。
...
所有变体构建完成后，将它们作为比较来呈现。不要只是列出——**提出观点**：

```markdown
## 首页屏幕的三种方案

| 维度 | 冷静编辑风格 | 实用密集风格 | 趣味分割风格 |
|-----------|----------------|-------------------|---------------|
| 密度   | 低            | 高              | 中等        |
| 主要操作可见性 | 低 | 高 | 中等 |
| 可扫描性 | 高 | 中等 | 低 |
...
**我的看法：** 实用密集风格适合高级用户，冷静编辑风格适合内容导向的受众。趣味分割风格最弱——试图兼顾两者，但都没有做好。
```

让用户选择一个优胜者，或将两个组合成混合体，或要求进行另一轮设计。
...
3.  **悬停可识别的可操作元素**（按钮、行、标签页）

超过这个程度就是对一个一次性原型过度工程化。低于这个程度就是一张截图。

## 前沿模式（选择接下来要设计什么）

如果原型已经存在，并且用户说 "what should I sketch next?"：

-   **一致性差距**——来自不同原型的两个优胜变体做出了尚未组合在一起的独立选择
-   **未设计的屏幕**——被提及但从未探索过
-   **状态覆盖**——设计了理想路径，但没有设计空状态 / 加载中 / 错误 / 1000 条项目
...
-   保持变体的可丢弃性——一个你觉得需要保留的原型应该被提升为真正的项目代码，而不是作为资产来管理
...
改编自 GSD (Get Shit Done) 项目的 `/gsd-sketch` 工作流——MIT © 2025 Lex Christopherson ([gsd-build/get-shit-done](https://github.com/gsd-build/get-shit-done))。完整的 GSD 系统包含持久化的原型状态、主题/变体模式参考以及一致性审计工作流；通过 `npx get-shit-done-cc --hermes --global` 安装。
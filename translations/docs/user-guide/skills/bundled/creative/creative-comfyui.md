...
- `template-integrity.md` — 将 `comfyui-workflow-templates` 从编辑器格式转换为 API 格式：重路由旁路、带点的动态输入键（`values.a`、`resize_type.width`）、云端特性（302 重定向、1 个并发免费层任务、1080p VRAM 上限）、兼容 Discord 的 ffmpeg 拼接。由 [@purzbeats](https://github.com/purzbeats) 编写。每当您从官方模板开始时，请加载此文档。
...
| `run_batch.py` | 使用参数扫描将工作流提交 N 次，并行度取决于您的套餐等级 |
...
**示例工作流 (`workflows/`):** SD 1.5, SDXL, Flux Dev, SDXL img2img, SDXL inpaint, ESRGAN 超分, AnimateDiff 视频, Wan T2V。参见 `workflows/README.md`。

## 使用时机

- 用户要求使用 Stable Diffusion、SDXL、Flux、SD3 等生成图像。
- 用户想要运行特定的 ComfyUI 工作流文件。
- 用户想要链接生成步骤（txt2img → 超分 → 面部修复）。
- 用户需要 ControlNet、inpainting、img2img 或其他高级流水线。
- 用户要求管理 ComfyUI 队列、检查模型或安装自定义节点。
- 用户想要通过 AnimateDiff、Hunyuan、Wan、AudioCraft 等进行视频/音频/3D 生成。
...
curl -s http://127.0.0.1:8188/system_stats 2>/dev/null && echo "server: 正在运行"

# 这台机器能在本地运行 ComfyUI 吗？（GPU/VRAM/磁盘检查）
python3 scripts/hardware_check.py
```

如果什么都没安装，请参阅下面的 **设置与入门** — 但务必先运行硬件检查。
...
- ComfyUI 网页界面 → **Workflow → Export (API)**（较新的 UI）或
  旧的 "Save (API Format)" 按钮（较旧的 UI）
...
# 云端运行（导出一次 API 密钥；自动使用正确的 /api 路由）
...
# img2img / 修复：传递 --input-image 来自动上传并引用
python3 scripts/run_workflow.py \
  --workflow sdxl_img2img.json \
  --input-image image=./photo.png \
  --args '{"prompt": "make it watercolor", "denoise": 0.6}'

# 批量 / 扫描：8个随机种子，并行数量受云端套餐限制
...
`seed` 设置为 `-1`（或在 `--randomize-seed` 时省略）会为每次运行生成一个新的随机种子。

### 步骤 4：呈现结果

脚本会向标准输出发送 JSON，描述每个输出文件：
...
| **执行（使用脚本）** | | |
| "一切就绪了吗？" | 脚本 | `health_check.py`（可选 `--workflow X --smoke-test`） |
| "这个工作流里我能改什么？" | 脚本 | `extract_schema.py W.json` |
| "检查工作流 W 的依赖是否满足" | 脚本 | `check_deps.py W.json` |
...
| "8个带随机种子的变体" | 脚本 | `run_batch.py --count 8 --randomize-seed ...` |
| "给我看实时进度" | 脚本 | `ws_monitor.py --prompt-id <id>` |
| "获取任务 X 的错误日志" | 脚本 | `fetch_logs.py <prompt_id>` |
| **直接 REST** | | |
...
当用户要求设置 ComfyUI 时，**首先要做的是询问他们想要 Comfy Cloud（托管、零安装、需要 API 密钥）还是 Local（在他们的机器上安装 ComfyUI）**。在他们回答之前，不要开始运行安装命令或硬件检查。
...
>
> - **Comfy Cloud** — 托管在 RTX 6000 Pro GPU 上，所有常见模型已预安装，零设置。需要 API 密钥（实际运行工作流需要付费订阅；免费套餐是只读的）。如果您没有合适的 GPU，这是最佳选择。
...
- **Cloud** → 跳转到**路径 A**。
- **Local** → 首先运行硬件检查，然后根据检查结果从路径 B–E 中选择一个。
- **不确定** → 运行硬件检查，让检查结果决定。
...
# 可选：同时探测 `torch` 以获取实际的 CUDA/MPS 信息：
...
| `marginal` | SD1.5 可用；SDXL 勉强；Flux/视频不太可能 | 轻量工作流可用本地，否则选择**路径 A（Cloud）** |
| `cloud` | 没有可用的 GPU，<6 GB VRAM，<16 GB Apple 统一内存，Intel Mac，Rosetta Python | **切换到 Cloud**，除非用户明确强制本地安装 |

脚本还会显示 `wsl: true`（WSL2 与 NVIDIA 透传）和 `rosetta: true`（Apple Silicon 上的 x86_64 Python — 必须重新安装为 ARM64）。

如果判定结果是 `cloud` 但用户想要本地安装，不要静默继续。逐字显示 `notes` 数组，并询问用户是否想 (a) 切换到 Cloud 或 (b) 强制进行本地安装（在现代模型上会导致内存不足或慢得无法使用）。
### 选择安装路径

首先使用硬件检查。下表是当用户已告知其硬件配置时的备选方案：

| 情况 | 推荐路径 |
|-----------|------------------|
| 硬件检查结果为 `verdict: cloud` | **路径 A：Comfy Cloud** |
| 无 GPU / 想无负担试用 | **路径 A：Comfy Cloud** |
...
它在内部运行 `hardware_check.py`，当结果为 `cloud` 时拒绝本地安装（除非使用 `--force-cloud-override`），选择正确的 `comfy-cli` 标志，并优先使用 `pipx`/`uvx` 而非全局 `pip` 以避免污染系统 Python 环境。

---

### 路径 A：Comfy Cloud（无需本地安装）

适用于没有合适 GPU 或希望零配置的用户。托管在 RTX 6000 Pro 上。
...
comfy --skip-prompt install --cpu                 # 仅 CPU（速度慢）
...
最简单的方法是使用 `run_workflow.py` 脚本的 `--input-image` 参数：
...
该标志会上传 `photo.png`，然后将其在服务器端的文件名注入到模式中名为 `image` 的参数。对于局部重绘，需要同时传递两个参数：
...
- **认证：** `X-API-Key` 请求头（对于 WebSocket 使用 `?token=KEY`）
- **API 密钥：** 设置一次 `$COMFY_CLOUD_API_KEY` 环境变量，脚本会自动获取
- **输出下载：** `/api/view` 返回一个 302 重定向到一个签名 URL；脚本会跟随重定向，并在从存储后端获取之前移除 `X-API-Key` 请求头（避免将 API 密钥泄露给 S3/CloudFront）。
- **与本地 ComfyUI 的端点差异：**
  ...
  - `subfolder` 参数在上传时被接受但被忽略 — 云端使用扁平命名空间。
- **并发任务数：** 免费/标准版：1，创作者版：3，专业版：5。额外的任务会自动排队。使用 `run_batch.py --parallel N` 来充分利用你的套餐额度。
...
1.  **需要 API 格式** — 每个脚本和 `/api/prompt` 端点都期望 API 格式的工作流 JSON。脚本会检测编辑器格式（顶层的 `nodes` 和 `links` 数组）并提示你通过 "Workflow → Export (API)"（新版 UI）或 "Save (API Format)"（旧版 UI）重新导出。

2.  **服务器必须正在运行** — 所有执行都需要一个活动的服务器。使用 `comfy launch --background` 启动一个。使用 `curl http://127.0.0.1:8188/system_stats` 验证。

3.  **模型名称必须精确** — 区分大小写，包含文件扩展名。`check_deps.py` 会进行模糊匹配（带/不带扩展名和文件夹前缀），但工作流本身必须使用规范名称。使用 `comfy model list` 来发现已安装的模型。
...
6.  **云端免费套餐 API 限制** — `/api/prompt`、`/api/view`、`/api/upload/*`、`/api/object_info` 在免费账户上均返回 403。`health_check.py` 和 `check_deps.py` 会优雅地处理此情况并显示明确的信息。

7.  **视频/音频工作流的超时设置** — 当输出节点是 `VHS_VideoCombine`、`SaveVideo` 等时，会自动检测；默认值从 300 秒增加到 900 秒。可以使用 `--timeout 1800` 显式覆盖。

8.  **输出文件名中的路径遍历** — 服务器提供的文件名会通过 `safe_path_join` 处理，以防止任何路径逃逸出 `--output-dir`。请保持此保护开启 — 使用自定义保存节点的工作流可能会产生任意路径。

9.  **工作流 JSON 是任意代码** — 自定义节点运行 Python 代码，因此提交未知工作流与使用 `eval` 具有相同的信任风险。在运行来自不受信任来源的工作流之前，请先检查。

10. **自动随机化种子** — 在 `--args` 中传递 `seed: -1`（或使用 `--randomize-seed` 并省略种子参数）以获得每次运行的新种子。实际种子会记录到 stderr。

11. **`tracking` 提示** — `comfy` 的首次运行可能会提示进行数据分析。使用 `comfy --skip-prompt tracking disable` 以非交互方式跳过。`comfyui_setup.sh` 会为你执行此操作。
...
- [ ] `comfy model list` 显示至少一个检查点模型（本地）或者 `/api/experiment/models/checkpoints` 返回模型（云端）
- [ ] 工作流 JSON 是 API 格式
- [ ] `check_deps.py` 报告 `is_ready: true`（或者在云端免费套餐上仅报告 `node_check_skipped`）

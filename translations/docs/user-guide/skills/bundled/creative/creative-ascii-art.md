...
在任何文本周围绘制装饰性 ASCII 艺术边框/框架。包含 70+ 种内置设计。
...
1.  **文本作为横幅** → 如果已安装则使用 pyfiglet，否则使用 asciified API 通过 curl
2.  **用有趣的角色艺术包裹消息** → cowsay
3.  **添加装饰性边框/框架** → boxes（可与 pyfiglet/asciified 结合使用）
4.  **特定事物的艺术**（猫、火箭、龙）→ ascii.co.uk 通过 curl + 解析

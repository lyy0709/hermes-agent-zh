...
**在编写任何一行代码之前**，阐明创意概念。这件作品传达了什么？是什么让观众停止滚动？是什么让它区别于代码教程示例？用户的提示是一个起点——用创造性的雄心去诠释它。
...
**积极主动地创造。** 如果用户要求“一个粒子系统”，请提供一个具有涌现集群行为、拖尾幽灵回波、调色板偏移深度雾化以及会呼吸的背景噪声场的粒子系统。至少包含一个用户未要求但会欣赏的视觉细节。
...
5.  **导出** —— 捕获输出：PNG 用 `saveCanvas()`，GIF 用 `saveGif()`，MP4 用 `saveFrames()` + ffmpeg，无头批处理用 Puppeteer
...
每个参数都应改变算法的*思考方式*，而不仅仅是它的*外观*。改变噪波倍频的“湍流”参数是好的。仅改变 `ellipse()` 半径的“粒子大小”滑块是肤浅的。
...
- **运动词汇**：缓慢漂移？爆炸性迸发？呼吸脉动？机械精度？
- **独特之处**：是什么让这个草图独一无二？

将用户的提示映射到美学选择上。“放松的生成背景”与“故障数据可视化”在各个方面都要求不同。
...
对于**交互式生成艺术**（种子探索、参数调整）：从 `templates/viewer.html` 开始。先阅读模板，保留固定部分（种子导航、操作），替换算法和参数控件。这为用户提供了种子前一个/后一个/随机/跳转、带实时更新的参数滑块以及 PNG 下载功能——全部已连接好。
...
对于生成式艺术平台，将 p5 的 PRNG 替换为平台的确定性随机数生成器：
...
对于通过 Puppeteer 进行无头渲染，草图**必须**在 setup 中使用 `noLoop()`。如果不使用，p5 的绘制循环会自由运行，而截图速度较慢 — 草图会超前运行，导致帧被跳过或重复。
...
| `references/webgl-and-3d.md` | WEBGL 渲染器、3D 图元、相机、光照、材质、自定义几何体、GLSL 着色器（`createShader()`、`createFilterShader()`）、帧缓冲区、后期处理 |
...
### SCAMPER 转换
选取已知的生成模式（流场、粒子系统、L-系统、元胞自动机）并进行系统性转换：
- **替代**：用文本字符替换圆形，用渐变替换线条
- **组合**：合并两种模式（流场 + 沃罗诺伊图）
- **适应**：将 2D 模式应用于 3D 投影
- **修改**：夸张化比例，扭曲坐标空间
- **用途转换**：用物理模拟实现排版，用排序算法处理色彩
- **消除**：移除网格、移除颜色、移除对称性
- **反向**：反向运行模拟，反转参数空间
...
1. 锚定用户概念（例如“孤独”）
2. 在三个距离层级上生成联想：
   - 近距离（明显联想）：空房间、单人身影、寂静
   - 中距离（有趣联想）：鱼群中反向游动的单条鱼、没有通知的手机、地铁车厢间的间隙
   - 远距离（抽象联想）：质数、渐近曲线、凌晨三点的颜色

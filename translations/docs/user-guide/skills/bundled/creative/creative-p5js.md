...
**首次渲染的卓越性不容妥协。** 输出必须在首次加载时就具有视觉冲击力。如果它看起来像 p5.js 教程练习、默认配置或“AI 生成的创意编程”，那就是错误的。在交付前重新思考。

**超越参考词汇表。** 参考资料中的噪声函数、粒子系统、调色板和着色器效果是起始词汇。对于每个项目，都要组合、分层和创新。目录是颜料的调色板——你来创作画作。

**积极主动地创造。** 如果用户要求“一个粒子系统”，那就提供一个具有涌现集群行为、拖尾幽灵回响、调色板偏移深度雾以及会呼吸的背景噪声场的粒子系统。至少包含一个用户未要求但会欣赏的视觉细节。
...
5.  **导出** —— 捕获输出：PNG 用 `saveCanvas()`，GIF 用 `saveGif()`，MP4 用 `saveFrames()` + ffmpeg，无头批量渲染用 Puppeteer
...
| **噪波词汇** | Perlin 噪波, simplex, 分形（倍频）, 域扭曲, 旋度噪波 | `references/visual-effects.md` § 噪波 |
...
| **分层** | `createGraphics()` 离屏缓冲区, 阿尔法合成, 遮罩 | `references/core-api.md` § 离屏缓冲区 |
| **纹理** | Perlin 表面, 点画, 影线, 半色调, 像素排序 | `references/visual-effects.md` § 纹理生成 |

### 每个项目的变体规则

绝不使用默认配置。对于每个项目：
- **自定义调色板** — 绝不使用原始的 `fill(255, 0, 0)`。始终使用包含 3-7 种颜色的设计好的调色板。
- **自定义描边粗细词汇** — 细小的点缀 (0.5), 中等结构 (1-2), 粗体强调 (3-5)。
- **背景处理** — 绝不使用纯色的 `background(0)` 或 `background(255)`。始终使用纹理、渐变或分层。
- **运动多样性** — 不同元素使用不同速度。主元素 1x，次要元素 0.3x，环境元素 0.1x。
...
每个参数都应该改变算法的*思考方式*，而不仅仅是它的*外观*。一个改变噪波倍频的“湍流”参数是好的。一个只改变 `ellipse()` 半径的“粒子大小”滑块是肤浅的。
...
将用户的提示映射到美学选择上。“轻松的生成本背景”与“故障数据可视化”在各方面都要求不同。
...
对于**交互式生成艺术**（种子探索、参数调整）：从 `templates/viewer.html` 开始。先阅读模板，保留固定部分（种子导航、操作），替换算法和参数控件。这为用户提供了种子 上一个/下一个/随机/跳转、带实时更新的参数滑块以及 PNG 下载功能——全部已连接好。
...
- 使用 Chrome DevTools 性能选项卡验证 60fps
- 在目标导出分辨率下测试，而不仅仅是窗口大小
- 调整参数，直到视觉效果符合步骤 1 中的概念
...
- **是否符合愿景？** 将输出与创意概念进行比较。如果看起来太普通，请回到步骤 1
- **分辨率检查**：在目标显示尺寸下是否清晰？是否有锯齿伪影？
- **性能检查**：在浏览器中是否能保持 60fps？（动画最低 30fps）
- **颜色检查**：颜色搭配是否协调？在亮色和暗色显示器上测试
- **边界情况**：在画布边缘会发生什么？调整大小时呢？运行 10 分钟后呢？

## 关键实现说明

### 性能 —— 首先禁用 FES

友好错误系统 (FES) 会增加高达 10 倍的开销。在每个生产草图中都禁用它：
...
function setup() {
  randomSeed(CONFIG.seed);
  noiseSeed(CONFIG.seed);
  // 现在所有的 random() 和 noise() 调用都是确定性的了
}
```
...
全局模式会污染 `window` 对象。对于生产环境，请使用实例模式：
...
对于通过 Puppeteer 进行无头渲染，画布**必须**在 setup 中使用 `noLoop()`。如果不这样做，p5 的绘制循环会自由运行，而截图速度较慢 — 画布会超前运行，导致帧被跳过或重复。
...
对于多场景视频，请使用按片段架构：每个场景一个 HTML 文件，独立渲染，使用 `ffmpeg -f concat` 拼接。参见 `references/export-pipeline.md` § 按片段架构。
...
3.  发展中距离联想 — 它们足够具体以便可视化，又足够出人意料而显得有趣
...
创建具有灵活艺术风格 × 氛围组合的原创知识漫画。

## 使用时机

当用户要求创建知识/教育漫画、传记漫画、教程漫画，或使用“知识漫画”、“教育漫画”或“Logicomix 风格”等术语时触发此技能。用户提供内容（文本、文件路径、URL 或主题），并可选择指定艺术风格、氛围、布局、宽高比或语言。
...
角色一致性由 `characters/characters.md`（在第 3 步中编写）中的**文本描述**驱动，这些描述会内联嵌入到每一页的提示词中（第 5 步）。第 7.1 步中生成的可选 PNG 角色表是面向人工的审查工件，而不是 `image_generate` 的输入。
...
| 参考图像 | 文件路径 | 用于风格/调色板特征提取的参考图像（不传递给图像模型）。参见上方的[参考图像](#参考图像)。 |
...
- **兼容性矩阵**和**内容信号 → 预设**表位于 [references/auto-selection.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/creative/baoyu-comic/references/auto-selection.md)。在第 2 步推荐组合之前请先阅读。

## 文件结构

输出目录：`comic/{topic-slug}/`
- Slug：来自主题的 2-4 个单词短横线连接（例如 `alan-turing-bio`）
- 冲突：附加时间戳（例如 `turing-story-20260118-143052`）

**内容**：
| 文件 | 描述 |
|------|-------------|
| `source-{slug}.md` | 保存的源内容（短横线连接的 slug 与输出目录匹配） |
| `analysis.md` | 内容分析 |
| `storyboard.md` | 带分格分解的分镜 |
...
**超时处理（关键）**：`clarify` 可能返回 `"The user did not provide a response within the time limit. Use your best judgement to make the choice and proceed."` —— 这**不是**用户同意使用所有默认设置的许可。

- 将其视为**仅针对该单个问题**的默认选择。继续按顺序询问剩余的步骤 2 问题；每个问题都是一个独立的确认点。
- **在您的下一条消息中，向用户明确展示默认选择**，以便他们有机会更正：例如，`"风格：已默认设置为 ohmsha 预设（clarify 超时）。请说一个词来切换。"` —— 一个未报告的默认选择与从未询问过是无法区分的。
- 在一个超时后，**不要**将步骤 2 折叠成一次性的“使用所有默认值”操作。如果用户确实不在，那么对于所有五个问题他们都将同样不在 —— 但当他们返回时，他们可以更正可见的默认值，而无法更正不可见的默认值。
...
**切勿依赖 shell 的 CWD 持久性来指定 `-o` 路径。** 终端工具的持久 shell CWD 可能在批次之间发生变化（会话过期、`TERMINAL_LIFETIME_SECONDS`、失败的 `cd` 导致您位于错误的目录）。`curl -o relative/path.png` 是一个无声的隐患：如果 CWD 已经漂移，文件会悄无声息地保存在其他地方，且没有错误。**始终向 `-o` 传递一个完全限定的绝对路径**，或者向终端工具传递 `workdir=<abs path>`。2026 年 4 月事件：一个 10 页漫画的第 06-09 页被保存到了仓库根目录，而不是 `comic/<slug>/`，因为批次 3 继承了批次 2 的一个陈旧 CWD，而 `curl -o 06-page-skills.png` 写入了错误的目录。然后 Agent 花费了几轮时间声称文件存在于它们并不存在的地方。

**7.1 角色设定图** —— 当漫画是多页且角色重复出现时，生成它（到 `characters/characters.png`，宽高比 `landscape`）。对于简单的预设（例如，四格极简风格）或单页漫画，则跳过。在调用 `image_generate` 之前，`characters/characters.md` 处的提示词文件必须存在。渲染的 PNG 是一个**面向人类的审阅工件**（以便用户可以视觉上验证角色设计），也是后续重新生成或手动编辑提示词的参考 —— 它**不**驱动步骤 7.2。页面提示词在步骤 5 中已经根据 `characters/characters.md` 中的**文本描述**编写完成；`image_generate` 无法接受图像作为视觉输入。

**7.2 页面** —— 每个页面的提示词在调用 `image_generate` **之前**必须已经位于 `prompts/NN-{cover|page}-[slug].md`。因为 `image_generate` 仅接受提示词，角色一致性是通过**在步骤 5 期间，将角色描述（源自 `characters/characters.md`）内联嵌入到每个页面提示词中**来强制执行的。无论是否在 7.1 中生成 PNG 设定图，都会统一进行这种嵌入；PNG 仅作为审阅/重新生成的辅助工具。
...
- `references/layouts/` - 布局（标准、电影式、密集、跨页、混合、网络漫画、四格）
...
- **为 `curl -o` 使用绝对路径** —— 切勿依赖跨批次持久 shell 的当前工作目录。一个无声的陷阱：文件会保存到错误的目录，随后在预期路径上执行 `ls` 会显示为空。请参阅步骤 7 的“下载步骤”。
- 对敏感的公众人物使用风格化的替代方案
- **步骤 2 需要确认** - 不要跳过
- **步骤 4/6 是条件性的** - 仅当用户在步骤 2 中要求时才执行
- **步骤 7.1 角色设定表** - 推荐用于多页漫画，对于简单预设是可选的。PNG 文件是审查/重新生成的辅助工具；页面提示词（在步骤 5 中编写）使用的是 `characters/characters.md` 中的文本描述，而不是 PNG 文件。`image_generate` 不接受图像作为视觉输入

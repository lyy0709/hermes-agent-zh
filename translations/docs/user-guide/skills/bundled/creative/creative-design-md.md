...
令牌提供精确值。正文告诉 Agent 这些值*为何*存在以及如何应用它们。CLI（`npx @google/design.md`）会检查结构 + WCAG 对比度，对比版本以发现回归，并导出到 Tailwind 或 W3C DTCG JSON。
...
2.  使用 `write_file` 在他们的项目根目录中**编写 `DESIGN.md`**。始终包含 `name:` 和 `colors:`；其他部分可选但鼓励添加。
...
# 比较两个版本，在回归时失败（退出码 1 = 回归）
npx -y @google/design.md diff DESIGN.md DESIGN-v2.md

# 导出到 Tailwind 主题 JSON
npx -y @google/design.md export --format tailwind DESIGN.md > tailwind.theme.json

# 导出到 W3C DTCG（设计令牌格式模块）JSON
...
- `wcag-contrast` (警告/信息) — 组件 `textColor` 与 `backgroundColor` 相对于 WCAG AA (4.5:1) 和 AAA (7:1) 的比率
...
-   **十六进制颜色必须是带引号的字符串。** 否则 YAML 会因 `#` 而解析失败，或者像 `#1A1C1E` 这样的值会被奇怪地截断。
-   **负尺寸也需要引号。** `letterSpacing: -0.02em` 会被解析为 YAML 流 — 应写为 `letterSpacing: "-0.02em"`。
-   **章节顺序是强制执行的。** 如果用户以随机顺序提供正文，请在保存前将其重新排序以匹配规范列表。

...
DESIGN.md 是 Google 的开源规范（Apache-2.0，`google-labs-code/design.md`），用于向编码 Agent 描述视觉识别系统。一个文件结合了：

- **YAML front matter** — 机器可读的设计令牌（规范性值）
- **Markdown 正文** — 人类可读的原理说明，按规范章节组织

令牌提供精确值。正文告诉 Agent 这些值*为何*存在以及如何应用它们。CLI（`npx @google/design.md`）会检查结构 + WCAG 对比度，对比版本以发现退化，并可导出为 Tailwind 或 W3C DTCG JSON。

## 何时使用此技能

- 用户要求提供 DESIGN.md 文件、设计令牌或设计系统规范
- 用户希望在多个项目或工具中保持一致的 UI/品牌
- 用户粘贴了现有的 DESIGN.md 并要求检查、对比、导出或扩展它
- 用户要求将风格指南转换为 Agent 可使用的格式
- 用户希望对他们的调色板进行对比度 / WCAG 可访问性验证

对于纯粹的视觉灵感或布局示例，请改用 `popular-web-designs`。对于从头开始设计一次性 HTML 工件（原型、演示文稿、落地页、组件实验室）时的*过程和品味*，请使用 `claude-design`。此技能用于*正式的规范文件*本身。
...
未知的章节会被保留，不会报错。如果值类型有效，则接受未知的令牌名称。未知的组件属性会产生警告。

## 工作流：编写新的 DESIGN.md

1.  **询问用户**（或推断）品牌基调、强调色和排版方向。如果他们提供了网站、图片或氛围描述，请将其转换为上述令牌结构。
...
5.  **如果用户已有现有项目**，同时将 Tailwind 或 DTCG 导出文件写入文件旁边（`tailwind.theme.json`, `tokens.json`）。
...
所有命令都接受 `-` 表示 stdin。`lint` 在遇到错误时返回退出码 1。如果需要结构化地报告发现的问题，请使用 `--format json` 标志并解析输出。

### 检查规则参考（7条规则捕获的内容）

- `broken-ref` (error) — `{colors.missing}` 指向不存在的令牌
- `duplicate-section` (error) — 相同的 `## Heading` 出现两次
- `invalid-color`, `invalid-dimension`, `invalid-typography` (error)
- `wcag-contrast` (warning/info) — 组件 `textColor` 与 `backgroundColor` 相对于 WCAG AA (4.5:1) 和 AAA (7:1) 的对比度比率
- `unknown-component-property` (warning) — 超出上述白名单

当用户关心可访问性时，请在总结中明确指出这一点 — WCAG 发现是使用 CLI 最重要的原因。

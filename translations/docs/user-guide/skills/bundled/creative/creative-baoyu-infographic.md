...
当用户要求创建信息图、视觉摘要、信息图表，或使用“信息图”、“可视化”、“高密度信息大图”等术语时触发此技能。用户提供内容（文本、文件路径、URL 或主题），并可选择指定布局、风格、宽高比或语言。
...
| 旅程路线图 | `winding-roadmap` + `storybook-watercolor` |
...
当用户输入包含以下关键词时，**自动选择**关联的布局，并在第 3 步中将关联风格作为首要推荐。跳过基于内容的布局推断。

如果快捷方式有**提示词备注**，则将其作为附加风格指令追加到生成的提示词（第 5 步）中。

| 用户关键词 | 布局 | 推荐风格 | 默认宽高比 | 提示词备注 |
|--------------|--------|--------------------|----------------|--------------|
| 高密度信息大图 / high-density-info | `dense-modules` | `morandi-journal`, `pop-laboratory`, `retro-pop-grid` | portrait | — |
| 信息图 / infographic | `bento-grid` | `craft-handmade` | landscape | 极简主义：干净画布、充足留白、无复杂背景纹理。仅使用简单的卡通元素和图标。 |
...
Slug：基于主题的 2-4 个单词 kebab-case。冲突时追加 `-YYYYMMDD-HHMMSS`。

## 核心原则

- 忠实保留源数据 — 不进行摘要或改写（但在包含到输出中之前，**务必去除任何凭据、API 密钥、Token 或机密信息**）
- 在构建内容结构前定义学习目标
...
**规则**：仅使用 Markdown。不添加新信息。忠实保留数据。从输出中去除任何凭据或机密信息。
...
**Q1 — 组合**：提供 3+ 个布局×风格组合及理由。请用户选择一个。
...
**加载参考**：从 `references/layouts/<layout>.md` 读取选定的布局，从 `references/styles/<style>.md` 读取选定的风格。
...
使用 `image_generate` 工具和第 5 步组装的提示词。
...
1.  **数据完整性至关重要** — 切勿对源统计数据进行摘要、转述或更改。“73% 增长”必须保持为“73% 增长”，而不是“显著增长”。
2.  **去除机密信息** — 在将源内容包含到任何输出文件之前，务必扫描其中的 API 密钥、Token 或凭据。

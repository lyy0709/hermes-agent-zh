...
使用 21 种布局类型和 21 种视觉风格生成专业信息图。分析内容，推荐布局×风格组合，并生成可直接发布的信息图。当用户要求创建“信息图”、“视觉摘要”、“信息图”、“可视化”或“高密度信息大图”时使用。
...
当用户要求创建信息图、视觉摘要、信息图表，或使用“信息图”、“可视化”或“高密度信息大图”等术语时触发此技能。用户提供内容（文本、文件路径、URL 或主题），并可选择指定布局、风格、宽高比或语言。
...
| `hand-drawn-edu` | 马卡龙柔和色、手绘抖动、简笔画人物 |
...
如果快捷方式有**提示词备注**，则将其作为附加风格指令附加到生成的提示词（步骤 5）中。
...
- 忠实保留源数据 — 不进行总结或改写（但在包含到输出中之前，**务必去除任何凭据、API 密钥、Token 或机密信息**）
- 在构建内容结构前定义学习目标
...
使用 `clarify` 工具向用户确认选项。由于 `clarify` 一次处理一个问题，请先问最重要的问题：

**Q1 — 组合**：提供 3 个以上布局×风格组合及其理由。请用户选择一个。
...
1.  **数据完整性至关重要** — 切勿总结、转述或更改源统计数据。“73% 增长”必须保持为“73% 增长”，而不是“显著增长”。

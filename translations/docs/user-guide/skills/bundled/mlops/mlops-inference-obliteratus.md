...
| 平台 | linux, macos |
| 标签 | `消除`, `去审查`, `拒绝移除`, `LLM`, `权重投影`, `SVD`, `机制可解释性`, `HuggingFace`, `模型手术` |
...
| 推理模型 (R1 蒸馏)     | `surgical`         | 具备 CoT 感知，保留思维链    |
...
- **advanced** （默认，推荐）— 多个 SVD 方向，保持范数的投影，2 次精炼处理。中等速度（约 10-20 分钟）。
...
- **surgical** — SAE 特征 + 神经元掩码 + 注意力头手术 + 按专家处理。非常慢（约 1-2 小时）。最适合推理模型。
...
| `--refinement-passes` | 迭代处理次数（1-5） | 2 |
...
| `obliteratus tourney <model>` | 锦标赛：所有方法进行正面比较 |
...
2. **约 10 亿参数以下的模型对消融反应不佳** — 它们的拒绝行为浅薄且碎片化，使得提取清晰的方向变得困难。预期部分结果（20-40% 的拒绝率残留）。30 亿参数以上的模型具有更清晰的拒绝方向，反应好得多（使用 `advanced` 通常能达到 0% 拒绝率）。
...
7. **VRAM 估计是近似的** — 4 位量化有帮助，但在提取期间峰值使用量可能激增。
...
12. **光谱认证 RED 很常见** — 即使实际拒绝率为 0%，光谱检查也经常标记为“不完整”。检查实际拒绝率，而不是仅仅依赖光谱认证。

...
包含 9 个 CLI 方法、28 个分析模块、横跨 5 个计算层级的 116 个模型预设、锦标赛评估以及基于遥测的推荐。

无需重新训练或微调，即可从开源权重的 LLM 中移除拒绝行为（防护栏）。使用机制可解释性技术——包括均值差分法、SVD、白化 SVD、LEACE 概念擦除、SAE 分解、贝叶斯核投影等——来识别并精准切除模型权重中的拒绝方向，同时保留推理能力。

**许可证警告：** OBLITERATUS 采用 AGPL-3.0 许可证。切勿将其作为 Python 库导入。始终通过 CLI（`obliteratus` 命令）或子进程调用。这可以保持 Hermes Agent 的 MIT 许可证的纯净性。

## 视频指南

Hermes Agent 使用 OBLITERATUS 消除 Gemma 防护栏的流程演示：
...
| 推理模型（R1 蒸馏）                 | `surgical`      | 具备 CoT 意识，保留思维链                  |
...
- **advanced** （默认，推荐）— 多个 SVD 方向，保范投影，2 次精炼。中等速度（约 10-20 分钟）。
...
2. **约 10 亿参数以下的模型对消融反应不佳** — 它们的拒绝行为浅薄且碎片化，使得难以提取干净的方向。预期部分结果（20-40% 的拒绝率残留）。30 亿参数以上的模型具有更清晰的拒绝方向，反应好得多（使用 `advanced` 通常能达到 0% 拒绝率）。
...
6. **量化模型无法重新量化** — 对全精度模型进行消融，然后量化输出。

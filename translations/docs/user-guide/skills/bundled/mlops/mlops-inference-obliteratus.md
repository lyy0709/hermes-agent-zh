...
description: "使用 OBLITERATUS 从开源权重 LLM 中移除拒绝行为——利用机制可解释性技术（均值差分、SVD、白化 SVD、LEACE、SAE 分解等）在保留推理能力的同时剔除护栏。"
---
...
使用 OBLITERATUS 从开源权重 LLM 中移除拒绝行为——利用机制可解释性技术（均值差分、SVD、白化 SVD、LEACE、SAE 分解等）在保留推理能力的同时剔除护栏。包含 9 种 CLI 方法、28 个分析模块、横跨 5 个计算层级的 116 个模型预设、锦标赛评估以及遥测驱动的推荐。当用户想要对 LLM 进行去审查、消除或移除拒绝行为时使用。
...
无需重新训练或微调，即可从开源权重 LLM 中移除拒绝行为（护栏）。使用机制可解释性技术——包括均值差分、SVD、白化 SVD、LEACE 概念擦除、SAE 分解、贝叶斯核投影等——来识别并精准地从模型权重中切除拒绝方向，同时保留推理能力。
...
| 24 GB    | ~32B 参数     | Qwen3-32B, Llama 3.1 70B (tight), Command-R |
...
| 推理模型 (R1 distills)     | `surgical`         | 具备 CoT 感知，保留思维链    |
...
（不通过 CLI 提供 — 需要 Python 导入，这违反了 AGPL 边界。仅在用户明确希望在他们自己的 AGPL 项目中将 OBLITERATUS 用作库时提及。）

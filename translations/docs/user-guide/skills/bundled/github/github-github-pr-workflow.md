...
完整的拉取请求生命周期 —— 创建分支、提交更改、打开 PR、监控 CI 状态、自动修复失败并合并。与 gh CLI 配合使用，或通过 curl 回退到 git + GitHub REST API。
...
管理 PR 生命周期的完整指南。每个部分首先展示 `gh` 方式，然后是针对没有 `gh` 的机器的 `git` + `curl` 回退方案。

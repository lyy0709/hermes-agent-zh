...
description: "通过分析 git diff 来审查代码变更，在 PR 上留下行内评论，并执行彻底的推送前审查"
---
...
通过分析 git diff 来审查代码变更，在 PR 上留下行内评论，并执行彻底的推送前审查。可与 gh CLI 配合使用，或回退到使用 git + curl 通过 GitHub REST API 操作。
...
在推送前对本地变更执行代码审查，或审查 GitHub 上开放的 PR。此技能大部分使用纯 `git` — `gh`/`curl` 的分工仅对 PR 级别的交互有影响。
...
2.  **逐文件审查** — 对变更的文件使用 `read_file` 获取完整上下文，并使用 diff 查看具体变更：
...
3. 对于每个更改的文件，如果需要更多上下文，使用 `read_file`
4. 应用上述清单
...
当用户要求你“评审 PR #N”、“查看这个 PR”或给你一个 PR URL 时，遵循此流程：
...
除了行内评论外，留下一个顶层的总结，以便 PR 作者能一目了然地了解全貌。使用 `references/review-output-template.md` 中的审查输出格式。

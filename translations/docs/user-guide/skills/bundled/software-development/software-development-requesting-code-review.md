...
description: "预提交验证流水线——静态安全扫描、基线感知质量门禁、独立审查者子代理和自动修复循环"
---
...
预提交验证流水线——静态安全扫描、基线感知质量门禁、独立审查者子代理和自动修复循环。在代码变更后、提交、推送或开启 PR 前使用。
...
代码落地前的自动化验证流水线。包含静态扫描、基线感知质量门禁、一个独立的审查者子代理和一个自动修复循环。
...
- 在子代理驱动开发的每个任务后（两阶段审查）

**跳过场景：** 仅文档变更、纯配置调整，或当用户说“跳过验证”时。

**此技能与 github-code-review 的区别：** 此技能在提交前验证**你**的变更。`github-code-review` 则在 GitHub 上审查**其他人**的 PR 并提供内联评论。
...
如果 `git diff --cached` 为空但 `git diff` 显示变更，请告知用户先执行 `git add <files>`。如果仍然为空，运行 `git status` —— 无需验证。
...
- **delegate_task 返回非 JSON** —— 用更严格的提示词重试一次，然后视为失败

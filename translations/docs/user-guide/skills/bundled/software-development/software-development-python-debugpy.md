...
这会绕过沙盒环境保证 —— 调试时没问题，但在推送前请使用包装器重新运行以确认。
...
4.  **`debugpy.listen` 只有在你也调用了 `wait_for_client()` 时才会阻塞。** 如果没有调用，执行会继续，你的第一个断点可能在客户端附加之前就触发了。
...
9. **分叉 / 多进程。** pdb 不会跟随分叉。每个子进程都需要自己的 `breakpoint()` 或 `set_trace()`。对于 Hermes 子 Agent，一次只调试一个进程。
...
```
触发处理程序。`nc 127.0.0.1 4444`，然后 `w` 查看挂起的帧，`!import asyncio; asyncio.all_tasks()` 查看还有哪些任务在等待。

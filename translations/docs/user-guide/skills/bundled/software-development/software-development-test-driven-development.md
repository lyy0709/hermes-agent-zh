---
title: "测试驱动开发 — TDD：强制执行红-绿-重构，先写测试后写代码"
sidebar_label: "测试驱动开发"
description: "TDD：强制执行红-绿-重构，先写测试后写代码"
---
...
**“事后测试能达到相同目标 — 这是精神而非仪式”**

不。事后测试回答“这段代码做了什么？”测试先行回答“这段代码应该做什么？”

事后测试受你的实现影响。你测试你构建的东西，而不是要求的东西。测试先行迫使你在实现前发现边界情况。
...
| “事后测试能达到相同目标” | 事后测试 = “这段代码做了什么？” 测试先行 = “这段代码应该做什么？” |
...
| “手动测试更快” | 手动测试无法证明边界情况。每次变更你都需要重新测试。 |
...
- **测试 Mock 行为而非真实行为** — Mock 应验证交互，而非替换被测系统
- **测试实现细节** — 测试行为/结果，而非内部方法调用
- **仅测试快乐路径** — 始终测试边界情况、错误和边界

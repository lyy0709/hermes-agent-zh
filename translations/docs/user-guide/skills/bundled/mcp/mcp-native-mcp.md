...
客户端会以指数退避（1秒、2秒、4秒、8秒、16秒，上限 60 秒）重试最多 5 次。如果服务器根本不可达，则在 5 次尝试后放弃。请检查服务器进程和网络连接。
...
-   从 Agent 的视角看，MCP 工具是同步调用的，但实际上是在专用的后台事件循环中异步运行的

...
### 流式 JSON 输出
对于实时 Token 流式传输，使用 `stream-json` 并配合 `--verbose`：
```
terminal(command="claude -p 'Write a summary' --output-format stream-json --verbose --include-partial-messages", timeout=60)
```

返回换行分隔的 JSON 事件。使用 jq 过滤以获取实时文本：
```
...
```
`--replay-user-messages` 会在标准输出上重新发送用户消息以进行确认。
...
| `--allow-dangerously-skip-permissions` | 将其启用为一个*选项*，而不是默认启用 |
...
| `--setting-sources <sources>` | 要加载的配置源，逗号分隔：`user`、`project`、`local` |
...
### 技能（自然语言调用）
与斜杠命令（手动调用）不同，`.claude/skills/` 目录下的技能是 Markdown 指南，当任务匹配时，Claude 会通过自然语言自动调用它们：
...
### 专业提示："ultrathink"
在提示词中使用关键词 "ultrathink"，以在特定轮次中启用最大程度的推理努力。无论当前的 `/effort` 设置如何，这都会触发最深的思考模式。
...
```
在 `.claude/worktrees/feature-x` 创建一个隔离的 git 工作树，并为其创建一个 tmux 会话。在可用时使用 iTerm2 原生窗格；添加 `--tmux=classic` 以使用传统的 tmux。
...
- 这与 CLAUDE.md 是分开的 — 这是 Claude 自己关于项目的笔记，跨会话累积
...
10. **为不同的任务启动新会话** — 会话持续 5 小时；新鲜的上下文更高效。
...
6. **寻找 `❯` 提示符** — 表示 Claude 正在等待输入（完成或提问）

...
当用户说类似“播放 X”、“暂停”、“跳过”、“将 X 加入队列”、“正在播放什么”、“搜索 X”、“添加到我的 X 播放列表”、“创建一个播放列表”、“将此保存到我的资料库”等时。
...
改变播放状态的操作需要 Spotify Premium；搜索/资料库/播放列表操作在免费账户上可用。
...
- **不要在每次操作前都调用 `get_state`。** Spotify 接受播放/暂停/跳过而无需预先检查。只有当用户询问“正在播放什么”或者你需要推理设备/曲目时才检查状态。
- **除非被要求，否则不要描述搜索结果。** 如果用户说“播放 X”，就搜索，获取顶部 URI，播放它。如果错了，他们会听到的。

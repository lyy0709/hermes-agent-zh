...
如果设置了 `$HERMES_TENANT`，则该任务属于租户命名空间。读取或写入持久化记忆时，请在记忆条目前加上租户前缀，以防上下文在租户间泄露：
...
您可以通过在 `~/.hermes/config.yaml` 中添加 `notification_sources` 来配置消息网关以接收跨配置文件的看板任务通知。
- `notification_sources: ['*']` 接受来自所有配置文件的订阅。
- `notification_sources: ['default', 'zilor-ppt']` 或 `"default,zilor-ppt"` 将订阅限制到指定的配置文件。
- 省略该键将保持默认行为（配置文件隔离）。

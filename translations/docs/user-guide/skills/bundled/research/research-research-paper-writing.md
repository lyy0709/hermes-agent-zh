---
title: "研究论文撰写 — 为 NeurIPS/ICML/ICLR 撰写 ML 论文：从设计到提交"
sidebar_label: "研究论文撰写"
description: "为 NeurIPS/ICML/ICLR 撰写 ML 论文：从设计到提交"
---
...
为 NeurIPS/ICML/ICLR 撰写 ML 论文：从设计到提交。
...
用于产出可发表 ML/AI 研究论文的端到端流水线，目标期刊为 **NeurIPS、ICML、ICLR、ACL、AAAI 和 COLM**。此技能涵盖完整的研究生命周期：实验设计、执行、监控、分析、论文撰写、审阅、修订和提交。
...
- **通过额外实验或修订来回应审稿意见**
- **在会议格式之间转换**论文
- **撰写非实证性论文**——理论、综述、基准或立场论文（参见[超越实证 ML 的论文类型](#paper-types-beyond-empirical-ml)）
- **为 NLP、HCI 或对齐研究设计人工评估**
- **准备录用后的交付物**——海报、演讲、代码发布

## 核心理念

1.  **积极主动。** 交付完整草稿，而非问题。科学家很忙——产出他们可以回应的具体内容，然后迭代。
2.  **绝不捏造引用。** AI 生成的引用错误率约为 40%。务必通过编程方式获取。将无法验证的引用标记为 `[CITATION NEEDED]`。
3.  **论文是故事，而非实验集合。** 每篇论文都需要一个用一句话清晰陈述的明确贡献。如果做不到这一点，论文就尚未准备好。
4.  **实验服务于主张。** 每个实验必须明确说明其支持哪个主张。绝不运行与论文叙述无关的实验。
5.  **尽早提交，频繁提交。** 每完成一批实验、每次论文草稿更新——都要用描述性信息提交。Git 日志就是实验历史。
...
| **中**（存在一些模糊性） | 撰写草稿并标记不确定之处，继续推进 |
| **低**（存在重大未知） | 通过 `clarify` 提出 1-2 个有针对性的问题，然后起草 |

| 章节 | 是否自主起草？ | 随草稿标记 |
|---------|-------------------|-----------------|
| 摘要 | 是 | "将贡献框架化为 X——如需调整请告知" |
...
**Git 纪律**：每个完成的实验批次都应提交并附上描述性信息。例如：
```
添加蒙特卡洛约束结果（5 次运行，Sonnet 4.6，政策备忘录任务）
添加 Haiku 基线比较：廉价模型层级的自推理与优化基线
```
...
**预算紧张时**：在投入全面扫描之前，先运行试点实验（1-2 个种子，任务子集）。使用更便宜的模型调试流水线，然后为目标模型进行最终运行。
...
| **Git + LaTeX** | 使用 `.gitignore` 处理辅助文件的 `git` | 技术团队，需要基于分支的审阅 |
...
扁平搜索（一轮查询）通常会遗漏重要的相关工作。使用受深度研究流水线启发的迭代式**先广度后深度**模式：
...
第 3 轮（针对性）：填补特定空白
  ...
  → 当新查询返回的论文大部分已见过时停止
```
**何时停止**：如果一轮检索返回的论文中已有超过 80% 在你的收藏中，则检索已饱和。通常 2-3 轮就足够了。对于综述论文，预计需要 4-5 轮。

**对于基于 Agent 的工作流**：通过 `delegate_task` 并行委派每一轮的查询。收集结果，去重，然后根据综合学习成果生成下一轮的查询。

### 步骤 1.3：验证每一条引用

**切勿凭记忆生成 BibTeX。务必通过编程方式获取。**

对于每一条引用，遵循以下强制性的 5 步流程：

```
引用验证（每条引用必须执行）：
1. 搜索 → 使用特定关键词查询 Semantic Scholar 或 Exa MCP
2. 验证 → 在 2 个以上来源（Semantic Scholar + arXiv/CrossRef）确认论文存在
...
**不好的**："Smith 等人引入了 X。Jones 等人引入了 Y。我们结合了两者。"

---

## 阶段 2：实验设计

**目标**：设计能直接支持论文主张的实验。每个实验都必须回答一个具体问题。
...
**规则**：如果一个实验没有映射到一个主张，就不要运行它。

### 步骤 2.2：设计基线

强有力的基线是区分被接受论文和被拒稿论文的关键。审稿人会问："他们和 X 比较了吗？"
...
| 决策 | 选项 | 指导原则 |
|----------|---------|----------|
| **标注员类型** | 专家、众包工作者、终端用户 | 与你主张所需的要求相匹配 |
| **量表** | 李克特（1-5）、成对比较、排序 | 对于 LLM 输出，成对比较比李克特量表更可靠 |
| **样本量** | 每个标注员和总项目数 | 功效分析或至少 100 个项目，3 名以上标注员 |
| **一致性指标** | Cohen's kappa、Krippendorff's alpha、ICC | 对于 >2 名标注员使用 Krippendorff's alpha；同时报告原始一致性 |
...
完整指南（包括人工评估数据的统计检验、众包质量控制模式和 IRB 指导）请参阅 [references/human-evaluation.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/human-evaluation.md)。
...
**并行执行**：同时运行独立的实验，但要注意 API 速率限制。在同一 API 上同时运行 4 个或更多实验会相互拖慢速度。
...
**静默模式**：如果自上次检查以来没有任何变化，请用 `[SILENT]` 响应，以抑制对用户的通知。仅在出现新情况时报告。
...
**为什么需要日志，而不仅仅是 Git？** Git 跟踪文件变化。日志跟踪推理过程：为什么尝试 X，学到了什么，这对下一个实验意味着什么。在撰写论文时，这棵树对于方法部分（"我们观察到 X，这促使我们进行 Y"）和诚实地报告失败至关重要。
...
McNemar 检验、自助法置信区间和 Cohen's h 的完整实现请参阅 [references/experiment-patterns.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/experiment-patterns.md)。
### 步骤 4.3：确定故事线

分析后，明确回答：
1.  **主要发现是什么？** 用一句话陈述。
2.  **什么让你感到意外？** 意外的结果往往能成就最好的论文。
3.  **什么失败了？** 失败的实验可能最具启发性。诚实地报告失败能增强论文的说服力。
...
| 方法未超越基线，但**揭示了新东西** | 将贡献重新定位为理解/分析 | ICLR（重视理解）、研讨会论文 |
| 针对流行主张的明确负面结果 | 写出来——领域需要知道 | NeurIPS 数据集与基准测试、TMLR、研讨会 |
| 结果不确定，没有清晰的故事线 | 转向——运行不同的实验或重新定位 | 不要强行写一篇不存在的论文 |

**如何撰写负面结果论文：**
-   开篇说明社区普遍相信什么，以及测试它的重要性
-   描述你严谨的方法论（必须无懈可击——审稿人会更加严格地审查）
-   清晰地呈现无效结果并提供统计证据
-   分析**为什么**预期结果没有出现
-   讨论对领域的影响

**明确欢迎负面结果的会议/期刊**：NeurIPS（数据集与基准测试赛道）、TMLR、ML 可重复性挑战赛、主要会议的研讨会。有些研讨会专门征集负面结果。
...
| 所有实验已完成，但有些失败了 | 记录失败，进入阶段 5 |

### 步骤 4.6：撰写实验日志（连接写作的桥梁）

在进入论文写作之前，创建一个结构化的实验日志，作为结果与正文之间的桥梁。这是连接实验和文稿最重要的纽带——没有它，写作 Agent 将不得不从原始结果文件中重新推导故事线。
...
此流水线中的任何输出——论文草稿、实验脚本、分析——都可以迭代优化。自推理研究为每种优化策略何时有效、何时失败提供了经验证据。使用本节来选择正确的方法。
...
| 中端模型 + 开放性任务 | **添加范围约束的自推理** | 添加固定事实、结构或交付物以限定改进空间。 |
| 前沿模型 + 约束性任务 | **自推理** | 即使在约束性任务中，也能在 2/3 的情况下胜出。 |
| 前沿模型 + 无约束任务 | **批判与修订** 或 **单次通过** | 自推理排在最后。模型的自我评估已足够好。 |
| 具体技术任务（系统设计） | **批判与修订** | 直接的查找-修复循环更高效。 |
| 模板填充任务（一种正确结构） | **单次通过** 或 **保守策略** | 决策空间最小。迭代不增加价值。 |
| 带有测试用例的代码 | **自推理（代码变体）** | 在修复前，对*为什么*失败进行结构化分析。恢复率 62% 对 43%。 |
...
这种差距是结构性的，而非暂时的。随着成本下降，今天的前沿模型将成为明天的中端模型。最佳应用点会移动，但永远不会消失。

### Autoreason 循环（摘要）

每一轮循环都会由全新的、独立的 Agent 产生三个候选方案：

1.  **批评者** → 找出当前方案 A 的问题（不提供修复）
2.  **作者 B** → 基于批评意见修订 A
3.  **合成者** → 合并 A 和 B（使用随机化标签）
4.  **评审团** → 3 位盲审的 CoT 评审员通过 Borda 计数法对 A、B、AB 进行排序
5.  **收敛** → 方案 A 连续赢得 k=2 轮循环 → 完成

**关键参数：**
-   k=2 收敛（k=1 过早，k=3 成本过高且无质量增益）
...
-   **为批评者提供真实数据**：实际的实验数据、结果 JSON、统计输出。没有这些，模型会编造虚假的消融研究和置信区间。
-   **至少使用 3 个有效的评审员**：一个损坏的评审员解析器不会增加噪音——它会完全阻止达到平衡。
...
| 失败模式 | 检测方法 | 修复方法 |
|---------|-----------|-----|
| 不收敛（A 从未获胜） | 超过 20 轮循环，A 获胜率 <15% | 为任务添加范围限制 |
| 合成漂移 | 字数无限制增长 | 限制结构和交付物 |
| 低于单次循环的质量 | 基线得分高于迭代输出 | 切换到单次循环；模型可能太弱 |
| 过拟合（代码） | 公开测试通过率高，私有测试通过率低 | 使用结构化分析，而不仅仅是测试反馈 |
| 评审员损坏 | 解析失败导致评审团少于 3 人 | 在继续之前修复解析器 |

完整的提示词、Borda 计分细节、模型选择指南、范围限制设计模式以及计算预算参考，请参见 [references/autoreason-methodology.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/autoreason-methodology.md)。
...
| 修订轮次 | 完整的论文草稿、特定的审稿人意见 | 其他所有内容 |

**原则：**
-   **`experiment_log.md` 是主要的上下文桥梁** —— 它总结了写作所需的一切，而无需加载原始数据文件（见步骤 4.6）
-   **委派时，一次只加载一个部分的上下文**。负责起草方法的子 Agent 不需要文献综述笔记。
-   **提供摘要，不要包含原始文件**。对于一个 200 行的结果 JSON，加载一个 10 行的汇总表。对于一篇 50 页的相关论文，加载其 5 句话的摘要 + 你关于其相关性的 2 行笔记。
...
**最关键的一个洞察**：你的论文不是一系列实验的集合——它是一个由一个清晰的贡献支撑、并由证据支持的故事。

每一篇成功的机器学习论文都围绕 Neel Nanda 所称的“叙事”展开：一个简短、严谨、基于证据的技术故事，其结论是读者所关心的。
...
| **那又怎样** | 读者为何应该关心 | 这是否与公认的社区问题相关联？ |

**如果你不能用一句话陈述你的贡献，那么你还没有一篇论文。**

### 本指南背后的来源

本技能综合了在顶级会议上发表过大量论文的研究人员的写作理念。写作理念层最初由 [Orchestra Research](https://github.com/orchestra-research) 作为 `ml-paper-writing` 技能整理。
| 来源 | 核心贡献 | 链接 |
|--------|-----------------|------|
...
| **Gopen & Swan** | 读者预期的 7 条原则 | [科学写作的科学](https://cseweb.ucsd.edu/~swanson/papers/science-of-writing.pdf) |
| **Zachary Lipton** | 措辞选择，消除模糊表述 | [科技写作启发式方法](https://www.approximatelycorrect.com/2018/01/29/heuristics-technical-scientific-writing-machine-learning-perspective/) |
| **Jacob Steinhardt** (UC Berkeley) | 精确性，一致的术语 | [写作技巧](https://bounded-regret.ghost.io/) |
| **Ethan Perez** (Anthropic) | 微观层面的清晰度技巧 | [简易论文写作技巧](https://ethanperez.net/easy-paper-writing-tips/) |
...
**为什么？** 大多数审稿人在阅读你的方法部分之前就已形成判断。读者接触你论文的顺序是：标题 → 摘要 → 引言 → 图表 → 可能阅读其余部分。
...
**第一轮 — 按章节撰写 + 立即精炼：**
对于每个章节，先撰写完整草稿，然后在同一上下文中立即进行精炼。这能在章节内容新鲜时捕捉局部问题（清晰度、流畅度、完整性）。

**第二轮 — 基于全文上下文的全局精炼：**
所有章节起草完毕后，带着对完整论文的认识重新审视每个章节。这能捕捉跨章节问题：冗余、术语不一致、叙事流畅度，以及一个章节承诺了但另一个章节未兑现的缺口。

```
第二轮精炼提示（针对每个章节）：
"在完整论文的上下文中审阅 [章节]。
- 它是否与论文其余部分契合？是否与其他章节有冗余？
- 术语是否与引言和方法部分保持一致？
- 是否有任何内容可以在不削弱主旨的情况下删减？
- 叙事是否从前一章节流畅过渡到下一章节？
...
将此清单附加到每个精炼提示后。这些是 LLM 编写 LaTeX 时最常见的错误：
...
图 1 是大多数读者看的第二样东西（在摘要之后）。在撰写引言之前先起草它——这会迫使你理清核心思想。

| 图 1 类型 | 何时使用 | 示例 |
|---------------|-------------|---------|
| **方法示意图** | 新架构或流水线 | 展示你系统的 TikZ 流程图 |
| **结果预告** | 一个引人注目的结果讲述整个故事 | 条形图："我们的方法 vs 基线"，有明显差距 |
| **问题图示** | 问题不直观 | 展示你修复的故障模式的前后对比 |
| **概念图** | 抽象贡献需要视觉基础 | 方法属性的 2x2 矩阵 |
**规则**：图 1 必须在不阅读任何文字的情况下即可理解。仅凭标题就应传达核心思想。有目的地使用颜色——不要仅仅为了装饰。
...
按方法论组织，而非逐篇论文罗列。慷慨引用——审稿人很可能就是相关论文的作者。

### 步骤 5.7：局限性（必需）

所有主要会议都要求此项。诚实有益：
- 审稿人被指示不要因诚实地承认局限性而扣分
- 通过首先指出弱点来预先应对批评
- 解释为何这些局限性不会削弱核心主张
...
- 总结关键发现（2-3 句话，非列表形式）
- 意义：这对该领域意味着什么？
- 未来工作：2-3 个具体的后续步骤（避免模糊的"我们将 X 留给未来工作"）
...
所有主要会议场所的附录篇幅均无限制，且对可复现性至关重要。结构如下：

| 附录部分 | 内容 |
|-----------------|---------------|
| **证明与推导** | 正文中篇幅过长的完整证明。正文可以陈述定理并注明"证明见附录 A"。 |
| **补充实验** | 消融实验、规模曲线、各数据集细分结果、超参数敏感性分析 |
...
- 使用 `\appendix` 命令，然后是 `\section{A: 证明}` 等
...
| 谨慎使用 `\vspace{-Xpt}` | 0.1-0.3 页 | 如果使用巧妙则风险低，如果明显则风险高 |
| 移除定性示例 | 0.5-1 页 | 中——审稿人喜欢示例 |
| 缩小图表尺寸 | 0.25-0.5 页 | 高——图表必须保持可读性 |

**切勿**：减小字体大小、更改页边距、删除必需部分（局限性、更广泛影响），或在正文中使用 `\small`/`\footnotesize`。

### 步骤 5.10：伦理与更广泛影响声明

现在大多数会议场所都要求或强烈建议提供伦理/更广泛影响声明。这不是官样文章——审稿人会阅读，并可能标记伦理问题，导致直接拒稿。
...
| **公平性与偏见** | 您的方法/数据是否存在已知偏见？ | 所有会议场所（隐含要求） |
| **环境影响** | 大规模训练的计算碳足迹 | ICML，NeurIPS 也日益要求 |
...
```latex
\section*{更广泛影响声明}
% NeurIPS/ICML：位于结论之后，不计入页面限制

% 1. 积极应用（1-2 句话）
这项工作实现了[具体应用]，可能使[特定群体]受益。

% 2. 风险与缓解措施（1-3 句话，具体说明）
[方法/模型]可能被滥用于[具体风险]。我们通过[具体缓解措施，例如，仅发布大于 X 尺寸的模型权重、包含安全过滤器、记录失败模式]来缓解此风险。
...
如果你的论文引入了**新数据集**或**发布了模型**，请包含结构化文档。审稿人对此的期望越来越高，并且 NeurIPS 数据集与基准测试赛道要求提供。
...
如果未修改的模板无法编译，先修复此问题（通常是缺少 TeX 包 — 通过 `tlmgr install <package>` 安装）。
...
系统性地进行：标题/作者 → 摘要 → 引言 → 方法 → 实验 → 相关工作 → 结论 → 参考文献 → 附录。每完成一节后编译一次。
...
| 不频繁编译 | 错误累积 | 每节完成后编译 |
| 图形使用栅格 PNG | 在论文中模糊 | 始终通过 `savefig('fig.pdf')` 使用矢量 PDF |
...
模板位于 `templates/` 目录中。有关编译设置（VS Code, CLI, Overleaf, 其他 IDE），请参见 [templates/README.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/templates/README.md)。
...
关于不同会议间的转换，请参阅第 7 阶段（提交准备）——它涵盖了完整的转换工作流、页数变化表以及被拒后的指导。

### 专业的 LaTeX 导言区

在任何论文中添加以下包以获得专业质量。它们与所有主流会议样式文件兼容：
...
% 表格
\usepackage{booktabs}               % 专业的表格线（\toprule, \midrule, \bottomrule）
...
- 检查会议模板是否已加载这些包中的任何一个（尤其是 `algorithm`、`amsmath`、`graphicx`）。不要重复加载。

### siunitx 表格对齐

`siunitx` 使数字密集的表格可读性显著提高：
...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    ...
    \label{fig:results-b}
  \end{subfigure}
  \caption{我们的方法在两个数据集上的比较。(a) 展示了缩放行为，(b) 展示了消融实验结果。两者均使用 5 个随机种子。}
  \label{fig:results}
\end{figure}
```
...
\While{$\text{streak} < k$}{
  ...
  \ForEach{评委 $J_i$}{
    $\text{rank}_i \gets J_i(\text{shuffle}(A, B, AB))$ \tcp*{盲排}
  }
...
\end{tikzpicture}
\caption{架构概览。编码器将输入 $x$ 映射到潜在表示 $z$，解码器对其进行重建。}
...
with plt.style.context(['science', 'no-latex']):
    ...
    ax.plot(x, y2, label='基线', color='#D55E00', linestyle='--')
    ax.set_xlabel('训练步数')
    ax.set_ylabel('准确率')
    ax.legend()
...
**目标**：在提交前模拟审稿过程。及早发现弱点。

### 步骤 6.1：模拟审稿（集成模式）

从多个角度生成审稿意见。自动化研究流水线（尤其是 SakanaAI 的 AI-Scientist）的关键见解是：**带有元审稿人的集成审稿比单次审稿能产生校准度更高的反馈。**

**步骤 1：生成 N 个独立的审稿意见** (N=3-5)

使用不同的模型或温度设置。每个审稿人只看到论文，看不到其他审稿意见。**默认采用负面偏见** —— LLM 在评估中存在众所周知的积极性偏见。

```
你是 [会议名称] 的专家审稿人。你要求严格且全面。
如果论文有弱点或你对某个主张不确定，请明确指出
并在你的评分中反映出来。不要给予怀疑的好处。

根据官方审稿指南评审这篇论文。评估：

1. 严谨性（主张是否有充分支持？基线是否公平且强大？）
2. 清晰度（论文是否写得好？专家能否复现？）
3. 重要性（这对社区重要吗？）
4. 原创性（新的见解，而不仅仅是增量组合？）

以结构化 JSON 格式提供你的审稿意见：
{
  "summary": "2-3 句摘要",
  "strengths": ["优势 1", "优势 2", ...],
  "weaknesses": ["弱点 1（最关键的）", "弱点 2", ...],
  "questions": ["给作者的问题 1", ...],
...
将所有 N 份审稿意见输入给一个元审稿人：

```
你是 [会议名称] 的领域主席。你收到了
一篇论文的 [N] 份独立审稿意见。你的工作是：
...
每个审稿人在看到元审稿意见后可以完善自己的审稿意见。使用提前终止哨兵：如果审稿人回复“我已完成”（无更改），则停止迭代。

**审稿的模型选择**：审稿最好使用可用的最强模型完成，即使你写论文时用的是更便宜的模型。审稿模型的选择应独立于写作模型。

**少样本校准**：如果可能，包含 1-2 篇来自目标会议的真实已发表审稿意见作为示例。这能显著提高分数校准度。有关示例审稿意见，请参阅 [references/reviewer-guidelines.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/reviewer-guidelines.md)。
### 步骤 6.1b：视觉审查环节（VLM）

仅基于文本的审查会遗漏一整类问题：图表质量、布局问题、视觉一致性。如果你能访问具备视觉能力的模型，请对编译后的 PDF 进行单独的**视觉审查**：
...
在模拟审查之后，运行一个单独的验证环节。这能捕捉到审稿人可能遗漏的事实性错误：

```
声明验证协议：
1. 从论文中提取每一个事实性声明（数字、比较、趋势）
2. 对于每个声明，追溯其到支持它的具体实验/结果
3. 验证论文中的数字与实际结果文件是否匹配
4. 将任何没有可追溯来源的声明标记为 [待验证]
```

对于基于 Agent 的工作流：将验证任务**委派给一个新的子 Agent**，该子 Agent 仅接收论文文本和原始结果文件。全新的上下文可以防止确认偏差——验证者不会"记得"结果应该是什么。
...
在回应实际的审稿意见（提交后）时，撰写反驳意见与修订论文是不同的技能：
...
**规则**：
- 回应每一个问题——审稿人会注意到你是否跳过某个问题
- 以最强的回应开头
- 简洁直接——审稿人要阅读数十份反驳意见
- 如果在反驳期内进行了实验，请包含新结果
- 即使面对薄弱的批评，也绝不要防御性或轻蔑
...
paper/
  ...
  paper_v2_post_review.tex     # 模拟审查后版本
...
每个会议都有强制性的清单。请仔细完成——不完整的清单可能导致直接拒稿。
...
提交前格式检查：
- [ ] 遵守页数限制（参考文献和附录除外）
- [ ] 所有图表都是矢量（PDF）或高分辨率位图（600 DPI PNG）
- [ ] 所有图表在灰度模式下可读
- [ ] 所有表格使用 booktabs
- [ ] 参考文献编译正确（引用中没有"?"）
- [ ] 关键区域没有过满的 hboxes
...
# 从 .tex 文件中提取 \cite{...}，并针对 .bib 文件检查每一项
python3 -c "
import re
...
| 会议 | 特殊要求 |
|-------|---------------------|
| **NeurIPS** | 附录中包含论文检查清单，如果被接受则需提供通俗摘要 |
| **ICML** | 更广泛影响声明（在结论之后，不计入页数限制） |
| **ICLR** | 需要 LLM 披露声明，互惠评审协议 |
| **ACL** | 强制性的局限性章节，负责任的 NLP 检查清单 |
...
| 任意 → COLM | 不定 → 9 | 为语言模型焦点重构 |

删减页数时：将证明移至附录，压缩相关工作，合并表格，使用子图。
扩展时：添加消融实验，扩展局限性章节，包含额外的基线，添加定性示例。

**被拒稿后**：在新版本中解决审稿人的关切，但不要包含“修改说明”章节或提及之前的提交（盲审）。
...
| 希望确立优先权 | 如果担心被抢先发表，请立即发布 — 但要接受匿名性的权衡。 |
...
**版本控制策略：**
- **v1**：初始提交（与会议提交内容一致）
- **v2**：接收后包含最终版修正（在摘要中添加“accepted at [会议名称]”）
...
发布干净、可运行的代码能显著增加引用次数和审稿人的信任度。在提交最终版时一并打包代码。
...
大多数会议都需要海报展示环节。海报设计原则：

| 元素 | 指南 |
|---------|-----------|
| **尺寸** | 检查会议场地要求（通常为 24"x36" 或 A0 纵向/横向） |
| **内容** | 标题、作者、一句话贡献、方法图、2-3 个关键结果、结论 |
| **布局** | 从左到右、从上到下（Z 型）或分栏式 |
| **文字** | 标题在 3 米外可读，正文在 1 米外可读。不要完整段落——仅使用要点。 |
...
**幻灯片设计规则：**
- 每张幻灯片一个核心观点
- 最小化文字——口头讲述细节，不要全部投影出来
- 对关键图表使用动画，逐步构建理解
- 最后包含一张“要点总结”幻灯片（一句话贡献）
- 为预期的问题准备备用幻灯片
...
- **博客文章**：800-1500 字。为 ML 从业者撰写，而不是审稿人。跳过形式化描述，强调直觉和实际意义。
- **项目页面**：包含摘要、图表、演示、代码链接、BibTeX 的 HTML 页面。使用 GitHub Pages。

**时机**：在论文出现在会议论文集或 arXiv 最终版后的 1-2 天内发布。
...
| **看重什么** | 有趣的想法、初步结果、观点文章 | 具有强基线的完整实证故事 |
| **arXiv** | 随时发布 | 时机很重要（参见 arXiv 策略） |
| **贡献门槛** | 新颖的方向、有趣的负面结果、进行中的工作 | 具有强证据的重大进展 |
...
| **Findings** | 8 | 扎实的工作，但仅以微弱差距未进入主会议 |

**短论文策略**：选择一个主张并彻底支持它。不要试图将一篇长论文压缩到4页——写一篇不同的、更聚焦的论文。

---

## 非实证性机器学习论文类型

上述主要流程针对的是实证性机器学习论文。其他论文类型需要不同的结构和证据标准。关于每种类型的详细指导，请参阅 [references/paper-types.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/paper-types.md)。
...
**与实证论文的主要区别**：
- 贡献是定理、界限或不可能性结果——而非实验数据
- “方法”部分被“预备知识”和“主要结果”取代
- 证据是证明，而非实验（尽管对理论进行实证验证是受欢迎的）
...
- 对假设进行编号并在定理中引用：“在假设1-3下，...”

### 综述 / 教程论文

**结构**：引言 → 分类法 / 组织框架 → 详细覆盖 → 开放问题 → 结论
...
- 必须证明基准测试具有挑战性（基线模型无法饱和其性能）
- 必须证明基准测试能衡量你所声称的内容（结构效度）
- 最佳投稿渠道：NeurIPS 数据集与基准测试方向、ACL（资源论文）、LREC-COLING

### 观点论文

**结构**：引言 → 背景 → 论点 / 论证 → 支持性证据 → 反驳论点 → 影响
...
| **subagent-driven-development** | 阶段 5（起草）：并行章节写作，并进行两阶段审查（规范符合性审查，然后是质量审查） | `skill_view("subagent-driven-development")` |
...
| **`execute_code`** | 运行 Python 进行引文验证、统计分析、数据聚合。可通过 RPC 访问工具。 |
...
delegate_task("根据这些实验脚本和配置起草方法部分。
  包括：伪代码、所有超参数、足以复现的架构细节。使用 LaTeX 按照 neurips2025 模板规范撰写。")

delegate_task("起草相关工作部分。使用 web_search 和 web_extract 查找论文。通过 Semantic Scholar 验证每篇引用。按方法论分组。")
...
cronjob("create", {
  ...
  "prompt": "检查实验状态：
    ...
    5. 报告：结果表格、关键发现、下一步
    6. 如果没有变化：用 [SILENT] 响应"
})
```

**[SILENT] 协议**：当自上次检查以来没有任何变化时，请准确响应 `[SILENT]`。这会抑制向用户发送通知。仅在有值得了解的真正变化时才报告。
...
| 贡献框架 | 存在多个有效框架时 |
| 实验优先级 | 当待办事项列表中的实验数量超过时间允许时 |
| 提交准备情况 | 最终提交之前 |

**不要询问**（积极主动，做出选择，标记出来）：
- 措辞选择、章节顺序
- 具体突出哪些结果
- 引用完整性（根据找到的内容起草，注明差距）
...
- 2: 拒绝——技术缺陷
- 1: 强烈拒绝——已知结果或伦理问题
请参阅 [references/reviewer-guidelines.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/reviewer-guidelines.md) 了解详细的审稿指南、常见问题以及反驳策略。
...
| 问题 | 解决方案 |
|-------|----------|
| 摘要过于笼统 | 如果第一句话可以放在任何机器学习论文的开头，请删除它。从你的具体贡献开始写起。 |
| 引言超过 1.5 页 | 将背景部分拆分到相关工作部分。将贡献要点前置。 |
| 实验缺乏明确的声明 | 在每个实验前添加："本实验旨在验证 [具体声明]..." |
| 审稿人认为论文难以理解 | 添加路标语句，使用一致的术语，使图注自成一体。 |
| 缺乏统计显著性 | 添加误差线、运行次数、统计检验、置信区间。 |
| 实验范围蔓延 | 每个实验都必须对应一个具体的声明。删除不对应的实验。 |
| 论文被拒，需要重新提交 | 参见第 7 阶段中的会议重投。在不提及审稿意见的情况下，解决审稿人的关切。 |
...
| 理论论文缺乏直观解释 | 在正式证明之前，添加带有通俗语言解释的证明草图。参见 [references/paper-types.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/paper-types.md)。 |
| 结果为阴性/无效 | 参见第 4.3 阶段关于处理阴性结果的部分。考虑研讨会、TMLR 或重新构建为分析性论文。 |
...
| [references/checklists.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/checklists.md) | NeurIPS 16 项清单，ICML，ICLR，ACL 要求，通用提交前检查清单 |
...
| [references/human-evaluation.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/human-evaluation.md) | 人工评估设计，标注指南，一致性指标，众包质量控制，IRB 指南 |
...
- [Perez: 简单的论文写作技巧](https://ethanperez.net/easy-paper-writing-tips/)

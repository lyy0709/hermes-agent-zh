...
- **回应审稿意见**，进行额外实验或修订
- **转换**论文格式以适应不同会议要求
- **撰写非实证性论文**——理论、综述、基准测试或立场论文（参见[超越实证 ML 的论文类型](#paper-types-beyond-empirical-ml)）
- **设计人类评估**用于 NLP、HCI 或对齐研究
- **准备录用后交付物**——海报、演讲、代码发布

## 核心理念

1.  **积极主动。** 交付完整草稿，而非问题。科学家很忙——产出具体内容供其反馈，然后迭代。
2.  **绝不捏造引用。** AI 生成的引用错误率约为 40%。务必通过程序获取。将无法验证的引用标记为 `[CITATION NEEDED]`。
3.  **论文是故事，而非实验集合。** 每篇论文都需要一个用单句清晰陈述的核心贡献。如果做不到，论文就尚未准备好。
4.  **实验服务于主张。** 每个实验必须明确说明其支持哪个主张。绝不运行与论文叙事无关的实验。
...
平面搜索（一轮查询）通常会遗漏重要的相关工作。使用受深度研究流水线启发的迭代式**先广度后深度**模式：
...
**对于基于 Agent 的工作流**：通过 `delegate_task` 并行委派每一轮的查询。收集结果，去重，然后根据综合的学习成果生成下一轮的查询。
...
强大的基线是区分被接收论文和被拒稿论文的关键。审稿人会问："他们和 X 比较了吗？"
...
许多 NLP、HCI 和 alignment 论文需要人工评估作为主要或补充证据。在运行自动化实验之前设计好这一点——人工评估通常有更长的准备时间（IRB 批准、标注员招募）。
...
**并行执行**：同时运行独立的实验，但要注意 API 速率限制。在同一 API 上并发运行 4 个以上实验会相互拖慢速度。

### 步骤 3.2：设置监控（Cron 模式）

对于长时间运行的实验，设置定期状态检查。定时任务提示词应遵循此模板：
...
| 错误的模型 ID | 错误信息引用模型名称 | 修复 ID 并重新运行 |
...
**为什么需要日志，而不仅仅是 git？** Git 跟踪文件变化。日志跟踪推理过程：为什么尝试 X，学到了什么，这对下一个实验意味着什么。在撰写论文时，这棵树对于方法部分（"我们观察到 X，这促使我们进行 Y"）以及诚实地报告失败至关重要。
...
| 针对流行主张的明确负面结果 | 写出来——领域需要知道 | NeurIPS 数据集与基准测试赛道、TMLR、研讨会 |
| 结果不确定，没有清晰的故事线 | 转向——运行不同的实验或重新构建 | 不要强行写一篇不存在的论文 |

**如何撰写负面结果论文：**
-   首先说明社区普遍相信什么，以及为何测试它很重要
-   描述你严谨的方法论（必须无懈可击——审稿人会更加仔细地审查）
-   清晰地呈现无效结果并提供统计证据
-   分析**为什么**预期结果没有出现
-   讨论对领域的影响

**明确欢迎负面结果的会议/期刊**：NeurIPS（数据集与基准测试赛道）、TMLR、ML 可复现性挑战赛、主要会议的研讨会。有些研讨会专门征集负面结果。
...
在进入论文写作之前，创建一个结构化的实验日志，作为结果与正文之间的桥梁。这是连接实验和写作之间最重要的纽带——没有它，写作 Agent 将不得不从原始结果文件中重新推导故事线。
...
| 非常弱的模型（Llama 8B 级别） | **单次通过** | 模型太弱，无法产生多样化的候选方案。应投资于生成质量。 |
### 生成-评估差距
...
每一轮循环都会由全新的、独立的 Agent 生成三个候选方案：

1.  **批评者** → 找出当前方案 A 的问题（不提供修复）
2.  **作者 B** → 基于批评修订方案 A
3.  **合成器** → 合并方案 A 和 B（使用随机化标签）
...
-   **为批评者提供真实依据**：实际的实验数据、结果 JSON、统计输出。没有这些，模型会编造虚假的消融研究和置信区间。
-   **至少使用 3 位有效的评审员**：一个损坏的评审员解析器不会增加噪音——它会完全阻止达到平衡状态。
...
| 失败模式 | 检测方法 | 修复方法 |
|---------|-----------|-----|
| 不收敛（A 从未获胜） | 超过 20 轮循环，A 获胜率 <15% | 为任务添加范围约束 |
| 合成漂移 | 字数无限增长 | 约束结构和交付物 |
| 低于单轮循环的退化 | 基线得分高于迭代输出 | 切换到单轮循环；模型可能太弱 |
| 过拟合（代码） | 公开测试通过率高，私有测试通过率低 | 使用结构化分析，而不仅仅是测试反馈 |
| 评审员损坏 | 解析失败导致评审团少于 3 人 | 在继续之前修复解析器 |

完整的提示词、Borda 计分细节、模型选择指南、范围约束设计模式和计算预算参考，请参阅 [references/autoreason-methodology.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/autoreason-methodology.md)。
...
| 起草任务 | 加载到上下文 | 不加载 |
|---------------|------------------|-------------|
| 撰写引言 | `experiment_log.md`、贡献声明、5-10 篇最相关的论文摘要 | 原始结果 JSON、完整实验脚本、所有文献笔记 |
...
**最关键的一个洞察**：你的论文不是一系列实验的集合——它是一个由一个清晰的贡献支撑、并有证据支持的故事。
...
| **Zachary Lipton** | 措辞选择，消除模糊表述 | [科学写作启发式方法](https://www.approximatelycorrect.com/2018/01/29/heuristics-technical-scientific-writing-machine-learning-perspective/) |
...
**第一轮 — 按章节撰写 + 立即精炼：**
对每个章节，先撰写完整草稿，然后在同一上下文中立即进行精炼。这能在章节内容新鲜时捕捉局部问题（清晰度、流畅度、完整性）。

**第二轮 — 基于全文上下文的全局精炼：**
所有章节起草完毕后，带着对整篇论文的认知重新审视每个章节。这能捕捉跨章节问题：冗余、术语不一致、叙事流畅度，以及一个章节承诺了但另一个章节未兑现的缺口。

```
第二轮精炼提示词（针对每个章节）：
"在完整论文的上下文中审阅 [SECTION]。
- 它是否与论文其余部分契合？是否与其他章节存在冗余？
- 术语是否与引言和方法部分保持一致？
- 是否有任何内容可以删减而不削弱核心信息？
- 叙事是否从前一章节流畅过渡到下一章节？
...
将此清单附加到每次精炼提示词后。这是 LLM 编写 LaTeX 时最常见的错误：
...
- 突出令人惊讶的结果："Scaling Data-Constrained Language Models"（暗示你能够做到）
...
图 1 是大多数读者查看的第二项内容（在摘要之后）。在撰写引言之前先起草它——这会迫使你理清核心思想。

| 图 1 类型 | 何时使用 | 示例 |
|---------------|-------------|---------|
| **方法示意图** | 新架构或流水线 | 展示你系统的 TikZ 流程图 |
| **结果预告** | 一个引人注目的结果能讲述整个故事 | 条形图："我们的方法 vs 基线"，有清晰的差距 |
| **问题图示** | 问题不直观 | 展示你修复的故障模式的前后对比 |
| **概念图** | 抽象贡献需要视觉基础 | 方法属性的 2x2 矩阵 |
**规则**：图 1 必须在不阅读任何文字的情况下即可理解。仅凭图注就应传达核心思想。有目的地使用颜色——不要仅仅为了装饰。
...
- 清晰的问题陈述
- 简要的方法概述
- 2-4 个贡献点列表（每个贡献点最多 1-2 行，采用双栏格式）
- 方法部分应从第 2-3 页开始
...
- 总结关键发现（2-3 句话，不是列表）
- 意义：这对该领域意味着什么？
- 未来工作：2-3 个具体的下一步（不要模糊地说"我们将 X 留给未来工作"）
...
所有主要会议对附录页数没有限制，附录对于可复现性至关重要。结构如下：

| 附录部分 | 内容 |
|-----------------|---------------|
| **证明与推导** | 正文中过长的完整证明。正文可以陈述定理并注明"证明见附录 A"。 |
| **补充实验** | 消融实验、扩展曲线、各数据集细分、超参数敏感性分析 |
...
**请勿**：减小字体大小、更改页边距、删除必需部分（局限性、更广泛影响），或对正文使用 `\small`/`\footnotesize`。

### 步骤 5.10：伦理与更广泛影响声明

现在大多数会议都要求或强烈建议提供伦理/更广泛影响声明。这不是模板——审稿人会阅读，并且可以标记伦理问题，导致直接拒稿。
...
| **公平性与偏见** | 您的方法/数据是否存在已知的偏见？ | 所有会议（隐含要求） |
...
**常见错误：**
- 写"我们预见没有负面影响"（这几乎从不正确——审稿人不信任这种说法）
- 含糊其辞："这可能被滥用"，但没有具体说明如何滥用
- 忽略大规模工作的计算成本
- 在要求披露的会议上忘记披露 LLM 的使用
...
如果你的论文引入了**新数据集**或**发布了模型**，请包含结构化文档。审稿人对此的期望越来越高，并且 NeurIPS 数据集与基准赛道要求提供。

**数据集数据表** (Gebru 等人, 2021) — 在附录中包含：
...
**模型卡** (Mitchell 等人, 2019) — 对于模型发布，在附录中包含：
...
系统性地进行：标题/作者 → 摘要 → 引言 → 方法 → 实验 → 相关工作 → 结论 → 参考文献 → 附录。每节之后进行编译。
...
| 不频繁编译 | 错误累积 | 每节之后编译 |
| 图形使用栅格 PNG | 论文中模糊 | 始终通过 `savefig('fig.pdf')` 使用矢量 PDF |
...
关于不同会议间的格式转换，请参阅第 7 阶段（提交准备）——它涵盖了完整的转换工作流、页数变化表以及拒稿后指导。

### 专业 LaTeX 导言区

在任何论文中添加这些包以获得专业质量。它们与所有主流会议样式文件兼容：
...
**注意：**
- `microtype` 是对视觉质量影响最大的包。它在亚像素级别调整字符间距。务必包含。
...
从多个角度生成审阅意见。来自自动化研究流水线（特别是 SakanaAI 的 AI-Scientist）的关键见解是：**带有元审阅者的集成审阅，比单次审阅能产生校准度更高的反馈。**
...
**少样本校准**：如果可能，包含 1-2 篇来自目标会议/期刊的真实已发表审阅意见作为示例。这能显著提高分数校准度。请参阅 [references/reviewer-guidelines.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/reviewer-guidelines.md) 获取示例审阅意见。
### 步骤 6.1b：视觉审查环节（VLM）
...
| 优先级 | 行动 |
|----------|--------|
| **严重**（技术缺陷、缺少基线） | 必须修复。可能需要新的实验 → 回到阶段 2 |
...
在回应实际的审稿意见（提交后）时，反驳意见的撰写与修订是不同的技能：
...
- "File not found" → 错误的图片路径或缺少 .sty 文件
...
- [ ] 添加公开的代码/数据 URL（真实的 GitHub，而非匿名链接）
- [ ] 解决元审稿人要求的任何强制性修改
...
| 提交到 ICLR | ICLR 明确允许在提交前发布到 arXiv。但不要在提交的论文中包含作者姓名。 |
| 论文已在 arXiv 上，提交到新会议 | 大多数会议都接受。在评审期间，**不要**更新 arXiv 版本以包含引用评审意见的修改。 |
| 研讨会论文 | 任何时候发布到 arXiv 都可以 — 研讨会通常不是双盲的。 |
...
**版本控制策略：**
- **v1**：初始提交（与会议提交内容一致）
- **v2**：接收后，包含最终版修正（在摘要中添加"accepted at [会议名称]"）
...
**预发布清单：**
```
- [ ] 代码可以从干净的克隆开始运行（在新机器或 Docker 上测试）
- [ ] 所有依赖项固定到特定版本
- [ ] 没有硬编码的绝对路径
...
**制作**：在会议前 2 周以上预订。织物海报更轻便，便于携带。许多会议现在也支持虚拟/数字海报。
...
| **看重什么** | 有趣的想法、初步结果、立场文章 | 具有强基线的完整实证故事 |
| **arXiv** | 随时发布 | 时机很重要（见 arXiv 策略） |
| **贡献门槛** | 新颖方向、有趣的负面结果、进行中的工作 | 具有强证据的重大进展 |
...
| **发现论文** | 8 | 扎实的工作，仅以微弱差距未进入主会议 |

**短论文策略**：选择一个主张并彻底地支持它。不要试图将一篇长论文压缩到4页——写一篇不同的、更聚焦的论文。

---

## 经验性机器学习之外的论文类型

上述主要流水线针对的是经验性机器学习论文。其他类型的论文需要不同的结构和证据标准。关于每种类型的详细指导，请参阅 [references/paper-types.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/paper-types.md)。
...
| **subagent-driven-development** | 阶段 5（起草）：并行章节写作，并进行两阶段审查（规范符合性，然后是质量） | `skill_view("subagent-driven-development")` |
...
**此技能取代了 `ml-paper-writing`** —— 它包含了 ml-paper-writing 的所有内容，以及完整的实验/分析流水线和自动推理方法。
...
| **`execute_code`** | 运行 Python 进行引文验证、统计分析、数据聚合。通过 RPC 拥有工具访问权限。 |
...
在做出重大决策或阶段转换后更新记忆。这会在不同会话间持久保存。
...
| 论文被拒，需要重新提交 | 参见第 7 阶段的"会议重新提交"。在不提及审稿意见的情况下，解决审稿人的关切。 |
| 缺少更广泛影响声明 | 参见步骤 5.10。大多数会议都要求此项。"没有负面影响"几乎不可信。 |
...
| 结果为阴性/无效 | 参见第 4.3 阶段关于处理阴性结果的部分。考虑研讨会、TMLR 或重新定位为分析性论文。 |
...
| 文档 | 内容 |
|----------|----------|
| [references/writing-guide.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/writing-guide.md) | Gopen & Swan 7 原则，Perez 微技巧，Lipton 措辞选择，Steinhardt 精确性，图表设计 |

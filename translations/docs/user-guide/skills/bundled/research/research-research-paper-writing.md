...
2.  **绝不捏造引用。** AI 生成的引用错误率约 40%。务必通过程序获取。将无法验证的引用标记为 `[CITATION NEEDED]`。
3.  **论文是故事，而非实验集合。** 每篇论文都需要一个用一句话清晰陈述的明确贡献。如果做不到，论文就尚未准备好。
...
| **低**（存在重大未知） | 通过 `clarify` 提出 1-2 个针对性问题，然后起草 |

| 章节 | 是否自主起草？ | 随草稿标记 |
|---------|-------------------|-----------------|
| 摘要 | 是 | "将贡献框架化为 X——如需请调整" |
| 引言 | 是 | "强调了问题 Y——如有错误请纠正" |
| 方法 | 是 | "包含了细节 A、B、C——请补充缺失部分" |
| 实验 | 是 | "突出了结果 1、2、3——如需请重新排序" |
...
**预算紧张时**：在投入全面实验之前，先运行试点实验（1-2 个种子，任务子集）。使用更便宜的模型调试流水线，然后为目标模型切换进行最终运行。
...
| **Git + LaTeX** | 使用 `.gitignore` 忽略辅助文件的 `git` | 技术团队，需要基于分支的审阅 |
...
第 3 轮（针对性）：填补特定空白
  ...
  → 当新查询返回的论文大部分已见过时停止
```
**何时停止**：如果一轮搜索返回的论文中已有超过 80% 在你的收藏中，则搜索已饱和。通常 2-3 轮就足够了。对于综述论文，预计需要 4-5 轮。

**对于基于 Agent 的工作流**：通过 `delegate_task` 并行委派每轮的查询。收集结果，去重，然后根据综合学习成果生成下一轮的查询。

### 步骤 1.3：验证每一条引用

**切勿**凭记忆生成 BibTeX。**必须**通过编程方式获取。

对于每条引用，遵循以下强制性的 5 步流程：
...
- NLP 会议（ACL、EMNLP）的审稿人对于生成任务期望看到它

**关键设计决策：**

| 决策 | 选项 | 指导原则 |
|----------|---------|----------|
| **标注员类型** | 专家、众包工作者、终端用户 | 匹配你的主张所需 |
| **量表** | 李克特量表（1-5）、成对比较、排序 | 对于 LLM 输出，成对比较比李克特量表更可靠 |
...
完整指南（包括人工评估数据的统计检验、众包质量控制模式以及 IRB 指导）请参阅 [references/human-evaluation.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/human-evaluation.md)。
...
**静默模式**：如果自上次检查以来没有任何变化，则用 `[SILENT]` 响应，以抑制对用户的通知。仅在出现新情况时报告。
...
**关键点**：脚本应始终检查现有结果并跳过已完成的工作。这使得重新运行安全且高效。
...
Git 提交记录了发生了什么，但没有记录**探索树**——即基于所学知识决定下一步尝试哪些内容的决策过程。维护一个结构化的实验日志来捕获这棵树：
...
**为什么需要日志，而不仅仅是 Git？** Git 跟踪文件更改。日志跟踪推理过程：为什么尝试 X，学到了什么，这对下一个实验意味着什么。在撰写论文时，这棵树对于方法部分（"我们观察到 X，这促使我们进行 Y"）和诚实的失败报告来说是无价的。

**选择最佳路径**：当日志显示分支树（exp_001 → exp_002a, exp_002b, exp_003）时，确定最能支持论文主张的路径。将死胡同分支记录在附录中，作为消融研究或负面结果。
...
**如何撰写负面结果论文：**
- 首先说明社区普遍相信什么，以及测试它的重要性
- 描述你严谨的方法论（必须无懈可击——审稿人会更加严格地审查）
- 清晰地呈现无效结果并提供统计证据
- 分析**为什么**预期结果没有出现
- 讨论对领域的影响

**明确欢迎负面结果的会议/期刊**：NeurIPS（数据集与基准测试赛道）、TMLR、ML 可重复性挑战赛、主要会议的研讨会。一些研讨会特别征集负面结果。
...
在进入论文写作之前，创建一个结构化的实验日志，作为连接结果与正文的桥梁。这是实验和论文草稿之间最重要的连接组织——没有它，写作 Agent 将不得不从原始结果文件中重新推导故事线。
...
**为什么这很重要**：在起草时，Agent（或委派的子 Agent）可以加载 `experiment_log.md` 和 LaTeX 模板，并基于实际结果生成初稿。没有这个桥梁，写作 Agent 必须解析原始的 JSON/CSV 文件并推断故事线——这是产生幻觉或误报数字的常见原因。
...
此流水线中的任何输出——论文草稿、实验脚本、分析——都可以迭代优化。自推理研究为每种优化策略何时有效、何时失败提供了经验证据。使用本节选择正确的方法。

### 快速决策表

| 你的情况 | 策略 | 原因 |
|---------------|----------|-----|
| 中端模型 + 约束性任务 | **自推理** | 最佳点。生成与评估之间的差距最大。基线会主动破坏弱模型的输出。 |
| 中端模型 + 开放性任务 | **添加范围约束的自推理** | 添加固定事实、结构或可交付成果来限定改进空间。 |
| 前沿模型 + 约束性任务 | **自推理** | 即使在约束性任务中，也能在 2/3 的情况下胜出。 |
| 前沿模型 + 无约束任务 | **批判与修订** 或 **单次通过** | 自推理排在最后。模型的自我评估足够好。 |
| 具体技术任务（系统设计） | **批判与修订** | 直接的查找-修复循环更高效。 |
| 模板填充任务（一种正确结构） | **单次通过** 或 **保守策略** | 决策空间最小。迭代不增加价值。 |
| 带有测试用例的代码 | **自推理（代码变体）** | 在修复之前，对*为什么*失败进行结构化分析。恢复率 62% 对 43%。 |
...
──────────────────┼────────────┼───────────┼────────┼─────────────────
弱 (Llama 8B)     │ 差        │ 差        │ 小     │ 无 — 无法生成多样候选方案
中 (Haiku 3.5)    │ 尚可      │ 差        │ 大     │ 最大 — 42/42 完美 Borda 评分
...
| 失败模式 | 检测方法 | 修复方法 |
|---------|-----------|-----|
| 不收敛（A 从未获胜） | 超过 20 轮，A 获胜率 &lt;15% | 为任务添加范围约束 |
| 合成漂移 | 字数无限制增长 | 约束结构和交付物 |
| 低于单轮效果 | 基线得分高于迭代输出 | 切换到单轮；模型可能太弱 |
| 过拟合（代码） | 公开测试通过率高，私有测试通过率低 | 使用结构化分析，而不仅仅是测试反馈 |
| 评审员损坏 | 解析失败导致评审团少于 3 人 | 在继续之前修复解析器 |

完整的提示词、Borda 评分细节、模型选择指南、范围约束设计模式和计算预算参考，请参见 [references/autoreason-methodology.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/autoreason-methodology.md)。
...
-   **委派时，一次只加载一个章节的上下文**。负责起草方法的子 Agent 不需要文献综述笔记。
-   **总结，不要包含原始文件**。对于一个 200 行的结果 JSON，加载一个 10 行的汇总表。对于一篇 50 页的相关论文，加载其 5 句话的摘要 + 你关于其相关性的 2 行笔记。
...
**如果你不能用一句话概括你的贡献，那么你还没有一篇论文。**

### 本指南背后的来源

本技能综合了在顶级会议上发表过大量论文的研究人员的写作理念。写作理念层最初由 [Orchestra Research](https://github.com/orchestra-research) 整理为 `ml-paper-writing` 技能。
| 来源 | 核心贡献 | 链接 |
|--------|-----------------|------|
...
| **Zachary Lipton** | 措辞选择，消除模糊表达 | [科学写作启发式方法](https://www.approximatelycorrect.com/2018/01/29/heuristics-technical-scientific-writing-machine-learning-perspective/) |
...
**为什么？** 大多数审稿人在阅读你的方法部分之前就已经形成了判断。读者接触你论文的顺序是：标题 → 摘要 → 引言 → 图表 → 可能阅读其余部分。
...
**第 2 轮 — 基于全文上下文的全局精炼：**
所有章节起草完毕后，带着对完整论文的了解重新审视每个章节。这能捕捉跨章节问题：冗余、术语不一致、叙事流畅度，以及一个章节承诺了但另一个章节未兑现的空白。
...
将此清单附加到每个精炼提示词后。这些是 LLM 编写 LaTeX 时最常见的错误：
...
标题是论文中被阅读次数最多的单一元素。它决定了是否有人会点击查看摘要。
...
| **结果预告** | 一个引人注目的结果能讲述整个故事 | 条形图："我们的方法 vs 基线"，有明显差距 |
| **问题图示** | 问题不直观 | 展示你修复的故障模式的前后对比 |
| **概念图** | 抽象贡献需要视觉基础 | 方法属性的 2x2 矩阵 |
**规则**：图1必须在不阅读任何文字的情况下即可理解。仅凭标题就应传达核心思想。有目的地使用颜色——不要仅仅为了装饰。
...
**切勿**：减小字体大小、更改页边距、删除必需章节（局限性、更广泛影响），或对正文使用 `\small`/`\footnotesize`。

### 步骤 5.10：伦理与更广泛影响声明

现在大多数会议场所都要求或强烈建议提供伦理/更广泛影响声明。这不是例行公事——审稿人会阅读，并可能标记伦理问题，导致直接拒稿。
...
| **LLM 使用披露** | 是否在写作或实验中使用 AI？ | ICLR（强制），ACL |
...
**常见错误：**
- 写"我们预见没有负面影响"（这几乎从不正确——审稿人不信任此说法）
...
```
### 步骤 5.11：数据表与模型卡片（如适用）

如果你的论文引入了**新数据集**或**发布了模型**，请包含结构化文档。审稿人对此的期望越来越高，并且 NeurIPS 数据集与基准测试赛道要求提供此类文档。

**数据集数据表** (Gebru et al., 2021) — 在附录中包含：

```
数据集文档（附录）：
- 动机：为何创建此数据集？它支持什么任务？
- 构成：数据实例是什么？数量多少？数据类型是什么？
- 收集：数据如何收集？来源是什么？
- 预处理：应用了哪些清洗/过滤？
- 分发：数据集如何分发？使用何种许可证？
- 维护：由谁维护？如何报告问题？
...
**模型卡片** (Mitchell et al., 2019) — 对于模型发布，在附录中包含：
...
| 随意添加包 | 导致冲突，破坏模板 | 仅在必要时添加 |
| 过早删除模板内容 | 失去格式参考 | 在完成前保留为注释 |
| 不频繁编译 | 错误累积 | 每完成一节后编译 |
| 图表使用栅格 PNG | 在论文中模糊不清 | 始终通过 `savefig('fig.pdf')` 使用矢量 PDF |
...
关于不同投稿平台间的转换，请参阅第 7 阶段（投稿准备）——它涵盖了完整的转换工作流、页数变化对照表以及拒稿后指导。
...
% 排版
\usepackage{microtype}              % 微排版改进（字符突出、扩展）
                                     % 显著提升文本美观度——务必包含

% 表格
\usepackage{booktabs}               % 专业表格线（\toprule, \midrule, \bottomrule）
\usepackage{siunitx}                % 一致的数值格式、小数点对齐
...
\While{$\text{streak} < k$}{
  ...
  \ForEach{评审员 $J_i$}{
    $\text{rank}_i \gets J_i(\text{shuffle}(A, B, AB))$ \tcp*{盲审排名}
  }
...
从多个角度生成审阅意见。自动化研究流水线（特别是 SakanaAI 的 AI-Scientist）的关键见解是：**带有元审阅的集成审阅比单次审阅能产生校准度更高的反馈。**

**步骤 1：生成 N 个独立的审阅意见** (N=3-5)

使用不同的模型或温度设置。每个审阅者只能看到论文，看不到其他审阅意见。**默认采用负面偏见** —— LLM 在评估中存在众所周知的积极性偏见。
...
每个审阅者在看到元审阅意见后可以完善自己的审阅意见。使用提前终止哨兵：如果审阅者回复 "I am done"（无更改），则停止迭代。

**用于审阅的模型选择**：审阅最好使用可用的最强模型完成，即使你是用更便宜的模型撰写论文的。审阅模型的选择应独立于写作模型。

**少样本校准**：如果可能，包含 1-2 个来自目标会议/期刊的真实已发表审阅意见作为示例。这能显著提高分数校准度。请参阅 [references/reviewer-guidelines.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/reviewer-guidelines.md) 获取示例审阅意见。
### 步骤 6.1b：视觉审查环节（VLM）
...
检查以下内容：
1.  图表质量：图表是否清晰可读？标签是否清晰可辨？颜色是否易于区分？
2.  图-标题对齐：每个标题是否准确描述了其对应的图？
3.  布局问题：孤立的章节标题、尴尬的分页、图表远离其引用位置
4.  表格格式：列是否对齐、小数精度是否一致、最佳结果是否加粗
5.  视觉一致性：所有图表是否使用相同的配色方案、字体大小是否一致
6.  灰度可读性：如果以黑白打印，图表是否仍能理解？

对于每个问题，请指定页码和确切位置。
```

这能捕捉到基于文本的审查无法发现的问题：坐标轴标签难以辨认的图表、距离首次引用位置 3 页远的图表、图 2 和图 5 之间不一致的调色板，或者明显超出列宽的表格。
...
对于基于 Agent 的工作流：将验证任务委派给一个**全新的子 Agent**，该子 Agent 仅接收论文文本和原始结果文件。全新的上下文可以防止确认偏差——验证者不会"记得"结果应该是什么。
...
在回应实际审稿意见（提交后）时，撰写反驳意见与修订论文是不同的技能：

**格式**：逐点回应。针对每位审稿人的意见：
```
> R1-W1: "论文缺少与方法 X 的比较。"

我们感谢审稿人提出的这个建议。我们已在表 3（修订版）中添加了与方法 X 的比较。
...
提交前格式检查：
- [ ] 遵守页数限制（参考文献和附录除外）
- [ ] 所有图表均为矢量（PDF）或高分辨率位图（600 DPI PNG）
- [ ] 所有图表在灰度模式下可读
- [ ] 所有表格使用 booktabs
- [ ] 参考文献编译正确（引用中没有"?"）
...
扩展时：添加消融实验，扩展局限性章节，包含额外基线，添加定性示例。

**被拒稿后**：在新版本中解决审稿人的关切，但不要包含"修改"章节或提及之前的提交（盲审）。
...
| 情况 | 建议 |
|-----------|---------------|
| 提交至双盲会议（NeurIPS、ICML、ACL） | 在提交截止日期**之后**发布到 arXiv，而非之前。提前发布可能在技术上违反匿名政策，尽管执行力度不一。 |
| 提交至 ICLR | ICLR 明确允许在提交前发布到 arXiv。但不要在提交的论文中包含作者姓名。 |
| 论文已在 arXiv 上，提交至新会议 | 大多数会议都接受。在评审期间，**不要**更新 arXiv 版本以包含引用评审意见的修改。 |
| 研讨会论文 | 随时发布到 arXiv 都可以 — 研讨会通常不是双盲的。 |
| 希望确立优先权 | 如果担心被抢先发表，请立即发布 — 但要接受匿名性方面的权衡。 |
...
| 计算机视觉 | `cs.CV` | 视觉模型 |
| 信息检索 | `cs.IR` | 搜索，推荐 |
**列出主要类别 + 1-2 个交叉列出的类别。** 更多类别意味着更高的可见性，但仅在真正相关时才进行交叉列出。
...
发布干净、可运行的代码能显著增加引用次数和审稿人的信任度。将代码与最终稿一起打包。
...
your-method/
  ...
  LICENSE                # 研究代码推荐使用 MIT 或 Apache 2.0 许可证
  configs/               # 实验配置
  src/                   # 核心方法实现
  scripts/               # 训练、评估、分析脚本
    train.py
...
**预发布检查清单：**
```
- [ ] 代码可以从一个干净的克隆仓库运行（在新机器或 Docker 上测试）
- [ ] 所有依赖项都固定到特定版本
- [ ] 没有硬编码的绝对路径
...
| **研讨会报告** | 10-15 分钟 | 根据研讨会听众进行调整——可能需要更多背景介绍。 |
...
- **博客文章**：800-1500 词。为 ML 从业者撰写，而不是审稿人。跳过形式化内容，强调直觉和实际意义。
...
| **arXiv** | 随时可以发布 | 时机很重要（参见 arXiv 策略） |
| **贡献门槛** | 新颖的方向、有趣的负面结果、进行中的工作 | 具有强有力证据的重大进展 |
...
| **研究结果** | 8 | 扎实的工作，仅以微弱差距未能入选主会议 |

**短论文策略**：选择一个主张并彻底地支持它。不要试图将一篇长论文压缩到 4 页——写一篇不同的、更聚焦的论文。

---

## 经验性 ML 之外的论文类型

上述主要流程针对的是经验性 ML 论文。其他类型的论文需要不同的结构和证据标准。关于每种类型的详细指导，请参阅 [references/paper-types.md](https://github.com/NousResearch/hermes-agent/blob/main/skills/research/research-paper-writing/references/paper-types.md)。
...
- 必须证明基准具有挑战性（基线模型无法饱和其性能）
- 必须证明基准能衡量你所声称的内容（结构效度）
- 最佳投稿渠道：NeurIPS 数据集与基准方向、ACL（资源论文）、LREC-COLING
...
| **`execute_code`** | 运行 Python 进行引用验证、统计分析、数据聚合。可通过 RPC 访问工具。 |
...
每个委派任务都作为一个**全新的子 Agent** 运行，没有共享上下文——请在提示词中提供所有必要信息。收集输出并进行整合。
...
**[SILENT] 协议**：如果自上次检查以来没有任何变化，则完全用 `[SILENT]` 响应。这会抑制向用户发送通知。仅在有值得了解的真正变化时才报告。
...
| 问题 | 解决方案 |
|-------|----------|
| 摘要过于笼统 | 如果第一句话可以放在任何机器学习论文的开头，就删除它。从你的具体贡献开始写起。 |
| 引言超过 1.5 页 | 将背景部分拆分到相关工作。将贡献要点前置。 |
...
| 论文被拒，需要重新提交 | 参见第 7 阶段的"会议重投"。在不提及审稿意见的情况下，解决审稿人的关切。 |
| 缺少更广泛影响声明 | 参见步骤 5.10。大多数会议都要求此声明。"无负面影响"几乎不可信。 |
...
- [Lipton: 科技写作的启发式方法](https://www.approximatelycorrect.com/2018/01/29/heuristics-technical-scientific-writing-machine-learning-perspective/)

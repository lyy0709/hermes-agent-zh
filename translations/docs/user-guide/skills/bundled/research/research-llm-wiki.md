...
构建和维护一个持久的、可积累的知识库，以互联的 Markdown 文件形式存在。
基于 [Andrej Karpathy 的 LLM Wiki 模式](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)。

与传统 RAG（每次查询都从头重新发现知识）不同，该 Wiki 一次性编译知识并保持其最新状态。交叉引用已经存在。矛盾之处已被标记。综合反映了所有已摄入的内容。
...
对于大型 Wiki（100+ 页面），在创建任何新内容之前，还应针对手头主题快速运行 `search_files`。
...
- 每个操作必须追加到 `log.md`
- **来源标记：** 在综合了 3 个以上来源的页面上，在段落末尾追加 `^[raw/articles/source-file.md]`，这些段落的声明来自特定来源。这使读者无需重新阅读整个原始文件即可追溯每个声明。对于 `sources:` frontmatter 已足够的单一来源页面，此标记可选。
...
`confidence` 和 `contested` 是可选的，但对于观点性强或快速变化的主题建议使用。整理会显示 `contested: true` 和 `confidence: low` 的页面以供审查，这样薄弱的声明就不会悄无声息地固化为公认的 Wiki 事实。
...
**扩展规则：** 当任何章节的条目超过 50 个时，按首字母或子域将其拆分为子章节。当索引总条目数超过 200 个时，创建一个 `_meta/topic-map.md` 文件，按主题对页面进行分组，以便更快地导航。
...
> 当此文件超过 500 个条目时，进行轮换：重命名为 log-YYYY.md，重新开始。
...
① **捕获原始来源：**
   ...
   - **添加原始 frontmatter**（`source_url`、`ingested`、正文的 `sha256`）。
     当重新摄取同一 URL 时：重新计算 sha256，与存储的值进行比较 —— 如果相同则跳过，如果不同则标记漂移并更新。这在每次重新摄取时进行成本足够低，并能捕获静默的源更改。

② **与用户讨论要点** —— 什么有趣，什么对该领域重要。（在自动化/定时任务上下文中跳过此步骤 —— 直接进行。）

③ **检查已存在的内容** —— 搜索 index.md 并使用 `search_files` 查找提及的实体/概念的现有页面。这是成长中的 wiki 和一堆重复内容之间的区别。
...
④ **编写或更新 wiki 页面：**
   ...
   - **来源标注：** 在综合了 3+ 个来源的页面上，将 `^[raw/articles/source.md]` 标记附加到其声明可追溯到特定来源的段落。
...
① **阅读 `index.md`** 以识别相关页面。
② **对于拥有 100+ 页面的 wiki**，同时对所有 `.md` 文件进行 `search_files` 以查找关键术语 —— 仅靠索引可能会遗漏相关内容。
③ **使用 `read_file` 阅读相关页面**。
④ **从整理的知识中综合出答案**。引用你引用的 wiki 页面：“基于 [[page-a]] 和 [[page-b]]...”
...
⑤ **陈旧内容：** `updated` 日期比提及相同实体的最新来源日期早 90 天以上的页面。

⑥ **矛盾：** 同一主题但声明冲突的页面。查找共享标签/实体但陈述不同事实的页面。将所有带有 `contested: true` 或 `contradictions:` frontmatter 的页面呈现给用户审查。

⑦ **质量信号：** 列出带有 `confidence: low` 的页面，以及任何仅引用单一来源但未设置置信度字段的页面 —— 这些是需要寻找佐证或降级为 `confidence: medium` 的候选页面。

⑧ **来源漂移：** 对于 `raw/` 中每个带有 `sha256:` frontmatter 的文件，重新计算哈希值并标记不匹配。不匹配表明原始文件被编辑过（不应该发生 —— raw/ 是不可变的）或从后来发生变化的 URL 摄取的。这不是硬性错误，但值得报告。
⑨ **页面大小：** 标记超过 200 行的页面——这些是潜在的拆分候选。

⑩ **标签审计：** 列出所有使用中的标签，标记任何不在 SCHEMA.md 分类法中的标签。

⑪ **日志轮转：** 如果 log.md 超过 500 条记录，则进行轮转。

⑫ **报告发现的问题**，需包含具体的文件路径和建议的操作，并按严重性分组（损坏链接 > 孤立页面 > 源文件偏移 > 争议页面 > 陈旧内容 > 样式问题）。
...
2. 将页面移动到 `_archive/` 目录下，保留其原始路径（例如，`_archive/entities/old-page.md`）
3. 从 `index.md` 中移除
4. 更新任何链接到该页面的页面——将 wikilink 替换为纯文本 + "（已归档）"
5. 记录归档操作
...
它通过 Obsidian Sync 同步知识库，无需 GUI——非常适合在服务器上运行的 Agent，它们向 wiki 写入内容，而 Obsidian 桌面端在另一台设备上读取。
...
- **保持页面易于浏览**——一个 wiki 页面应该在 30 秒内可读。超过 200 行的页面应进行拆分。将详细分析移至专门的深度分析页面。
- **大规模更新前先询问**——如果一次导入将涉及 10 个以上的现有页面，请先与用户确认范围。
- **轮转日志**——当 log.md 超过 500 条记录时，将其重命名为 `log-YYYY.md` 并重新开始。Agent 应在 lint 期间检查日志大小。
- **明确处理矛盾**——不要静默覆盖。用日期注明两种说法，在 frontmatter 中标记，并标记供用户审查。

## 相关工具

[llm-wiki-compiler](https://github.com/atomicmemory/llm-wiki-compiler) 是一个 Node.js CLI，它将源文件编译成一个具有相同 Karpathy 灵感的概念 wiki。它与 Obsidian 兼容，因此希望使用计划任务/CLI 驱动的编译流水线的用户可以将它指向此技能维护的同一个知识库。权衡：它负责页面生成（取代了 Agent 在页面创建上的判断），并且针对小型语料库进行了优化。当您希望 Agent 参与循环进行策展时，请使用此技能；当您希望批量编译源目录时，请使用 llmwiki。
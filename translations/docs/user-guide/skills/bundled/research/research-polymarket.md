---
title: "Polymarket — 查询 Polymarket 预测市场数据 — 搜索市场、获取价格、订单簿和价格历史"
...
完整端点参考及 curl 示例，请参见 `references/api-endpoints.md`。
...
1. **搜索**：使用 Gamma API 的 public-search 端点及其查询词
2. **解析**：解析响应 — 提取事件及其嵌套的市场
3. **呈现**：呈现市场问题、当前价格（百分比形式）和交易量
4. **深入分析**：如果被要求 — 使用 clobTokenIds 查询订单簿，使用 conditionId 查询历史
...
Gamma API 返回的 `outcomePrices`、`outcomes` 和 `clobTokenIds` 是 JSON 响应内的 JSON 字符串（双重编码）。使用 Python 处理时，使用 `json.loads(market['outcomePrices'])` 来解析它们以获取实际数组。

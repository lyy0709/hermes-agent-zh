...
除了步骤 8 中所需的环境变量，你还可以通过 `~/.hermes/config.yaml` 自定义 Slack Bot 行为。
...
| `platforms.slack.extra.reply_in_thread` | `true` | 当为 `false` 时，频道消息获得直接回复而非线程回复。现有线程内的消息仍在线程内回复。 |
...
:::info
与 Discord 和 Telegram 不同，Slack 没有等效的 `free_response_channels`。Slack 适配器要求在频道中使用 `@提及` 来开始对话。但是，一旦 Bot 在线程中拥有活跃会话，后续的线程回复就不需要提及。在私信中，Bot 始终无需提及即可响应。
:::
...
| 机器人在私信中工作，但在频道中不工作 | **最常见的问题。** 将 `message.channels` 和 `message.groups` 添加到事件订阅，重新安装应用，并使用 `/invite @Hermes Agent` 邀请机器人加入频道 |
...
| 更改了权限范围/事件但无变化 | 在更改任何权限范围或事件订阅后，您**必须重新安装**应用到您的工作区 |

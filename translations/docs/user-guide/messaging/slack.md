...
| **所需的认证 Token** | 机器人 Token (`xoxb-`) + 应用级 Token (`xapp-`) |
...
3.  选择你的工作区，粘贴 JSON 内容，检查，点击 **下一步** → **创建**
...
你将进入应用的 **基本信息** 页面。继续下面的步骤 2-6。

---

## 步骤 2：配置机器人 Token 权限范围

在侧边栏中导航到 **功能 → OAuth 和权限**。滚动到 **权限范围 → 机器人 Token 权限范围** 并添加以下内容：
...
:::tip
你始终可以在 **设置 → 基本信息 → 应用级 Token** 下找到或重新生成应用级 Token。
:::
...
如果机器人在 DM 中工作但**在频道中不工作**，你几乎肯定忘记了添加 `message.channels`（用于公共频道）和/或 `message.groups`（用于私密频道）。没有这些事件，Slack 根本不会将频道消息传递给机器人。
:::
...
此步骤启用与机器人的直接消息。没有它，用户尝试 DM 机器人时会看到 **"向此应用发送消息的功能已关闭"**。
...
2.  点击 **安装到工作区**
3.  检查权限并点击 **允许**
4.  授权后，你将看到一个以 `xoxb-` 开头的 **机器人用户 OAuth Token**
...
成员 ID 的格式类似于 `U01ABC2DEF3`。你至少需要提供你自己的成员 ID。
...
启动消息网关后，你需要 **邀请 Bot** 加入任何你希望它响应的频道：
...
4.  **Save**。如果权限范围或斜杠命令发生更改，Slack 会提示你重新安装应用。

### 旧版 `/hermes <子命令>` 仍然有效

为了与旧版清单保持向后兼容，你仍然可以输入 `/hermes btw run the tests`——Hermes 会以与 `/btw run the tests` 相同的方式路由它。自由格式的问题也有效：`/hermes what's the weather?` 会被视为常规消息。

### 在主题内使用命令（`!cmd` 前缀）

Slack 本身会阻止在主题回复中使用原生斜杠命令——在主题中尝试 `/queue`，Slack 会回复 *"/queue is not supported in threads. Sorry!"*。没有应用端设置可以重新启用它们；Slack 永远不会将它们传递给 Hermes。

作为一种变通方法，Hermes 将前导的 `!` 识别为可在主题（以及其他任何地方）使用的替代命令前缀。在常规主题回复中输入 `!queue`、`!stop`、`!model gpt-5.4` 等——Hermes 会将其视为与斜杠形式完全相同，并在同一主题中回复。
...
|---------|----------|
| **私信** | Bot 响应每条消息——无需 @提及 |
| **频道** | Bot **仅在 @提及时响应**（例如，`@Hermes Agent what time is it?`）。在频道中，Hermes 会在附加到该消息的主题中回复。 |
| **主题** | 如果你在现有主题内 @提及 Hermes，它会在同一主题中回复。一旦 Bot 在主题中拥有活动会话，**该主题中的后续回复无需 @提及**——Bot 会自然地跟随对话。 |

:::tip
在频道中，始终 @提及 Bot 以开始对话。一旦 Bot 在主题中处于活动状态，你可以在该主题中回复而无需提及它。在主题之外，未 @提及的消息将被忽略，以防止在繁忙的频道中产生噪音。
:::
...
platforms:
  slack:
    # 控制多部分响应如何被主题化
    # "off"   — 从不将回复主题化到原始消息
    # "first" — 第一个分块主题化到用户消息（默认）
    # "all"   — 所有分块都主题化到用户消息
    reply_to_mode: "first"

    extra:
      # 是否在主题中回复（默认：true）。
      # 当为 false 时，频道消息会直接在频道中回复，而不是在主题中。
      # 现有主题内的消息仍然在主题内回复。
...
| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `platforms.slack.reply_to_mode` | `"first"` | 多部分消息的主题模式：`"off"`、`"first"` 或 `"all"` |
| `platforms.slack.extra.reply_in_thread` | `true` | 当为 `false` 时，频道消息会直接回复，而不是在主题中。现有主题内的消息仍然在主题内回复。 |
| `platforms.slack.extra.reply_broadcast` | `false` | 当为 `true` 时，主题回复也会发布到主频道。只有第一个分块会被广播。 |
### 会话隔离
...
当设置为 `true`（默认值）时，共享频道中的每个用户都将拥有自己独立的对话会话。两个人在 `#general` 频道中与 Hermes 交谈将拥有独立的历史记录和上下文。
...
在繁忙的工作区中，当 Slack 默认的“机器人记住此线程”行为让用户感到意外时，请将此设置为 `true`——例如，在一个漫长的技术支持线程中，机器人在开始时提供了帮助，而你希望它保持静默，除非被再次明确提及。私信和活动的交互式会话不受影响。
:::

:::info
Slack 支持两种模式：默认情况下需要 `@提及` 来开始对话，但你可以通过 `SLACK_FREE_RESPONSE_CHANNELS`（逗号分隔的频道 ID）或 `config.yaml` 中的 `slack.free_response_channels` 选择特定频道退出此限制。一旦机器人在线程中拥有活动会话，后续的线程回复就不需要提及。在私信中，机器人总是无需提及即可响应。
:::

### 频道允许列表 (`allowed_channels`)

将机器人限制在一组固定的 Slack 频道中——当机器人被邀请到许多频道但只应在少数频道中响应时非常有用。设置后，来自**不在此列表中的频道**的消息将被**静默忽略**，即使机器人被 `@提及`。

**私信不受此过滤器限制**，因此授权用户始终可以通过直接消息联系到机器人。
...
- 空 / 未设置 → 无限制（完全向后兼容）。
- 非空 → 频道 ID 必须在列表中，否则消息将在任何其他门控（提及要求、`free_response_channels` 等）运行之前被丢弃。
- Slack 频道 ID 以 `C`（公开）、`G`（私密）或 `D`（私信）开头。通过 Slack UI 的“打开频道详情” → “关于”面板或通过 API 查找它们。
...
```yaml
slack:
  # 当未授权用户（不在 SLACK_ALLOWED_USERS 中）向机器人发送私信时发生的情况
  # "pair"   — 提示他们输入配对码（默认）
  # "ignore" — 静默丢弃消息
...
将 `SLACK_HOME_CHANNEL` 设置为一个频道 ID，Hermes 将在此频道中发送定时消息、定时任务结果和其他主动通知。要查找频道 ID：
...
来自此文件的令牌将与通过 `SLACK_BOT_TOKEN` 指定的任何令牌合并。重复的令牌会自动去重。
### 工作原理
...
- 当消息到达时，Hermes 使用特定工作区的正确客户端进行响应。
- 主 `bot_user_id`（来自第一个 Token）用于向后兼容那些期望单一机器人身份的功能。
...
为特定的 Slack 频道分配临时的系统提示词。该提示词在每次对话轮次运行时注入——永远不会持久化到对话历史记录中——因此更改会立即生效。
...
键是 Slack 频道 ID（通过频道详情 → "关于" → 滚动到底部找到）。匹配频道中的所有消息都会将该提示词作为临时系统指令注入。

## 按频道技能绑定

每当在特定频道或私信中开始新会话时，自动加载一个技能。与按频道提示词（每次轮次都注入）不同，技能绑定在**会话开始时**将技能内容作为用户消息注入——它成为对话历史的一部分，并且不需要在后续轮次中重新加载。

这非常适合具有特定用途的私信或频道（抽认卡、特定领域问答机器人、支持分流频道等），在这些场景中，你不希望模型自身的技能选择器来决定是否在每次简短回复时加载。
...
- 技能仅在会话开始时加载（新会话或自动重置后）。如果你更改了绑定，请运行 `/new` 或等待会话自动重置以使其生效。
- 可以与 `channel_prompts` 结合使用，在技能指令之上添加按频道的语气/约束。
...
| 私信中显示 "Sending messages to this app has been turned off" | 在应用主页设置中启用 **消息选项卡**（参见步骤 5） |
...
:::warning
**务必设置 `SLACK_ALLOWED_USERS`**，填入授权用户的 Member ID。如果没有此设置，
作为安全措施，消息网关将默认**拒绝所有消息**。切勿分享你的机器人 Token ——
像对待密码一样对待它们。
:::

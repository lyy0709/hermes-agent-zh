...
将 Hermes Agent 作为机器人连接到 Google Chat。该集成使用 Cloud Pub/Sub 拉取订阅来处理入站事件，并使用 Chat REST API 处理出站消息。其使用体验类似于 Slack Socket Mode 或 Telegram 长轮询：你的 Hermes 进程不需要公共 URL、隧道或 TLS 证书。它连接、认证并监听一个订阅——就像 Telegram 机器人监听一个 Token 一样。

:::note Workspace 版本
Google Chat 是 Google Workspace 的一部分。你可以将此集成用于个人 Workspace（通过 Google 注册的 `@yourdomain.com`）或你拥有发布应用管理员权限的工作 Workspace。仅 Gmail 账户无法托管 Chat 应用。
:::
...
一个常见的错误是搜索特定于 Chat 的 IAM 角色并在项目级别授予它。该角色并不存在。Chat 机器人的权限来自在空间中安装应用，而不是来自 IAM。你的服务账户只需要在你下一步创建的订阅上拥有 Pub/Sub 订阅者权限。
:::
...
创建后，主题的详细信息页面有一个 **Subscriptions** 选项卡。创建一个订阅：
...
- **头像 URL**：任何公开的 PNG（Google 有一些默认选项）。
- **描述**：显示在应用目录中的简短句子。
...
- **可见性**：限制在你的工作空间（或特定用户）——在测试期间不要发布给所有人。
...
在浏览器中打开 Google Chat。通过在 **+ New Chat** 菜单中搜索其名称，开始与你的应用进行私聊。当你第一次向它发送消息时，Google 会发送一个 `ADDED_TO_SPACE` 事件，Hermes 使用该事件来缓存机器人自身的 `users/{id}`，用于过滤自身消息。
...
在测试私聊中发送“hola”。机器人会发布一个“Hermes is thinking…”标记，然后在同一位置编辑该消息以显示真实响应——没有“message deleted”墓碑标记。
---
...
Agent 的系统提示词包含一个特定于 Google Chat 的提示，使其了解这些限制并避免使用无法渲染的格式。
...
开箱即用，机器人可以发布文本、通过 URL 的内联图片以及音频/视频/文档的下载卡片。要投递**原生** Chat 附件 — 即人类用户拖放文件时获得的相同文件小部件 — 每个用户需要通过针对每个用户的 OAuth 流程对机器人进行一次授权。

### 为什么需要单独的流程

Google Chat 的 `media.upload` 端点硬性拒绝服务账户认证：

> 此方法不支持使用服务账户进行应用身份验证。请使用用户账户进行身份验证。
...
这将写入 `~/.hermes/google_chat_user_client_secret.json`。这是共享的基础设施 — 它标识的是 OAuth *应用*，而不是任何单个用户。无论以后有多少用户授权，每个主机一个文件就足够了。

### 每个用户的授权（在聊天中）

每个用户在与机器人的私聊中运行一次该流程：
...
令牌保存在 `~/.hermes/google_chat_user_tokens/<sanitized_email>.json`。后续在该用户的私聊中的文件请求将使用*他们的*令牌，因此机器人以他们的身份上传，消息会出现在他们的空间中。

稍后撤销：`/setup-files revoke` 仅删除该用户的令牌。其他用户的令牌不受影响。

### 范围

该流程仅请求一个范围：`chat.messages.create`。这既涵盖了 `media.upload`，也涵盖了引用已上传 `attachmentDataRef` 的 `messages.create`。没有 Drive，没有更广泛的 Chat 范围 — 这是有意为之的最小权限。

### 多用户行为

当请求者还没有每个用户的令牌时，机器人会回退到位于 `~/.hermes/google_chat_user_token.json` 的遗留单用户令牌（如果来自多用户安装之前的设置）。当两者都不可用时，机器人会发布清晰的文本通知，告诉请求者运行 `/setup-files`。

用户撤销仅清除他们自己的槽位。来自一个用户令牌的 401/403 错误仅驱逐该用户的缓存。用户之间不会互相干扰。
...
1. 在控制台中检查 Pub/Sub 订阅是否有未送达的消息。如果有，说明 Hermes 未通过身份验证 — 请验证 `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON` 以及该服务账户是否在订阅上被列为 `Pub/Sub Subscriber`。
2. 如果订阅有零条消息，说明 Google Chat 没有发布。请再次检查**主题**上的 IAM 绑定：`chat-api-push@system.gserviceaccount.com` 必须具有 `Pub/Sub Publisher` 权限。
...
检查日志中是否有 `[GoogleChat] Pub/Sub stream died` — 如果这些重复出现，你的服务账户凭据可能已被轮换或订阅已被删除。经过 10 次尝试后，适配器会将自己标记为致命错误。
...
Chat API 的默认配额允许每个空间每分钟 60 条消息。如果你的 Agent 生成长流式响应并超过该限制，适配器会使用指数退避进行重试 — 但你仍然会看到用户可见的延迟。请考虑使用简洁的响应或在 GCP 控制台中提高配额。

**机器人不断发布 "/setup-files" 通知而不是文件。**

请求者没有每个用户的 OAuth 令牌，也没有遗留的回退令牌。在他们的私聊中运行 `/setup-files` 并遵循步骤 10。交换完成后，下一个文件请求将原生上传，无需重启消息网关。

**`/setup-files start` 显示 "No client credentials stored on the host."**
...
授权码是一次性的且有效期很短（通常几分钟）。发送 `/setup-files start` 以获取新的 URL 并重试。
---

## 安全注意事项

- **服务账号权限范围**：适配器请求 `chat.bot` 和 `pubsub` 权限。
  IAM 应是实际的执行机制——授予你的服务账号最小必要权限（在订阅上拥有 `roles/pubsub.subscriber` + `roles/pubsub.viewer` 角色），而不是项目级或组织级的 Pub/Sub 角色。
- **附件下载保护**：Hermes 仅会将服务账号的持有者令牌附加到主机与一个简短的 Google 自有域名允许列表（`googleapis.com`、`drive.google.com`、`lh[3-6].googleusercontent.com` 等）匹配的 URL。任何其他主机都会在 HTTP 请求发出前被拒绝，以防止精心构造的事件将持有者令牌重定向到 GCE 元数据服务这类 SSRF 场景。
- **信息脱敏**：服务账号邮箱、订阅路径和主题路径会通过 `agent/redact.py` 从日志输出中剥离。调试信封转储（`GOOGLE_CHAT_DEBUG_RAW=1`）会经过相同的脱敏过滤器，并在 DEBUG 级别记录日志。
- **合规性**：如果你计划将此机器人连接到受监管的工作区（任何具有数据驻留或 AI 治理策略的环境），请在首次安装前获得批准。
- **用户 OAuth 权限范围**：每个用户的附件流程*仅*请求 `chat.messages.create` 权限——这是覆盖 `media.upload` 及后续 `messages.create` 所需的最小权限。Token 以纯 JSON 格式持久化存储在 `~/.hermes/google_chat_user_tokens/<sanitized_email>.json`（文件系统权限是保护机制——与服务账号密钥文件模型相同）。每个 Token 仅属于一个用户；撤销操作也仅限于该用户。
...
将 Hermes Agent 作为机器人连接到 Google Chat。该集成使用 Cloud Pub/Sub 拉取订阅来处理入站事件，并使用 Chat REST API 处理出站消息。其易用性与 Slack Socket Mode 或 Telegram 长轮询相当：你的 Hermes 进程不需要公共 URL、隧道或 TLS 证书。它连接、认证并监听一个订阅——就像 Telegram 机器人监听一个 Token 一样。
...
| **认证** | 对订阅具有 `roles/pubsub.subscriber` 权限的服务账户 JSON |
...
一个常见的错误是搜索特定于 Chat 的 IAM 角色并在项目级别授予它。该角色并不存在。Chat 机器人的权限来自在空间中安装应用，而不是来自 IAM。你的服务账户只需要在下一步创建的订阅上拥有 Pub/Sub 订阅者权限。
:::
...
创建后，主题的详情页面有一个 **Subscriptions** 选项卡。创建一个订阅：

- 订阅 ID：`hermes-chat-events-sub`
- 传递类型：**Pull**
- 消息保留期：**7 天**（这样在 Hermes 重启时积压的消息可以保留）
- 其余设置保持默认。
...
GOOGLE_CHAT_MAX_BYTES=16777216                  # 16 MiB —— 传输中消息字节数的上限
```
...
在测试私聊中发送 "hola"。机器人会发布一个 "Hermes is thinking…" 标记，然后在同一条消息中编辑并替换为真实回复——不会出现 "message deleted" 墓碑消息。
...
消息大小限制：每条消息 4000 个字符。更长的 Agent 回复会自动拆分成多条消息。
...
开箱即用，机器人可以发布文本、通过 URL 的内联图片以及音频/视频/文档的下载卡片。要投递**原生**的 Chat 附件——即人类用户拖放文件时获得的相同文件小部件——每个用户需要通过一个针对每个用户的 OAuth 流程对机器人进行一次授权。
...
这会将 `~/.hermes/google_chat_user_client_secret.json`。这是共享的基础设施——它标识的是 OAuth *应用*，而不是任何单个用户。无论之后有多少用户授权，每个主机一个文件就足够了。
...
3. 他们打开该 URL，点击 **Allow**，然后观察浏览器无法加载 `http://localhost:1/?...&code=...`。这个失败是预期的——授权码就在 URL 地址栏中。
4. 他们复制失败的 URL（或仅复制 `code=...` 的值）并粘贴回聊天中，格式为 `/setup-files <PASTED_URL>`。机器人将其交换为刷新令牌。

令牌会保存在 `~/.hermes/google_chat_user_tokens/<sanitized_email>.json`。后续在该用户的私聊中请求文件时，将使用*他们的*令牌，因此机器人会以他们的身份上传，并且消息会出现在他们的空间中。
...
当请求者还没有每个用户的令牌时，机器人会回退到 `~/.hermes/google_chat_user_token.json` 中的旧版单用户令牌（如果来自多用户安装前的设置）。当两者都不可用时，机器人会发布清晰的文本通知，告诉请求者运行 `/setup-files`。
...
2. 如果订阅的消息数为零，说明 Google Chat 没有发布。请再次检查**主题**上的 IAM 绑定：`chat-api-push@system.gserviceaccount.com` 必须拥有 `Pub/Sub Publisher` 权限。
...
检查日志中的 `[GoogleChat] Pub/Sub stream died`——如果这些重复出现，你的服务账号凭据可能已被轮换或订阅已被删除。在 10 次尝试后，适配器会将自身标记为致命错误。
...
请求者没有每个用户的 OAuth 令牌，并且也没有旧版回退令牌。在他们的私聊中运行 `/setup-files` 并遵循步骤 10。交换完成后，下一个文件请求将原生上传，无需重启消息网关。

**`/setup-files start` 显示 "No client credentials stored on the host."**
一次性主机设置尚未完成。在运行 Hermes 的主机上，从终端执行：
...
授权码是一次性且短效的（通常只有几分钟）。发送 `/setup-files start` 以获取新的 URL 并重试。

---

## 安全注意事项

-   **服务账号权限范围**：适配器请求 `chat.bot` 和 `pubsub` 权限范围。实际的权限控制应由 IAM 执行——授予你的服务账号最小必要权限（订阅上的 `roles/pubsub.subscriber` + `roles/pubsub.viewer`），而不是项目级或组织级的 Pub/Sub 角色。
-   **附件下载保护**：Hermes 仅会将服务账号的持有者令牌附加到主机名与一个简短的 Google 自有域名白名单（`googleapis.com`、`drive.google.com`、`lh[3-6].googleusercontent.com` 等）匹配的 URL。任何其他主机都会在 HTTP 请求发出前被拒绝，以防止精心构造的事件将持有者令牌重定向到 GCE 元数据服务，从而防范 SSRF 场景。
-   **信息脱敏**：服务账号邮箱、订阅路径和主题路径会通过 `agent/redact.py` 从日志输出中剥离。调试信封转储（`GOOGLE_CHAT_DEBUG_RAW=1`）会经过相同的脱敏过滤器，并在 DEBUG 级别记录日志。
-   **合规性**：如果你计划将此机器人连接到受监管的工作区（任何有数据驻留或 AI 治理政策的环境），请在首次安装前获得相应批准。
-   **用户 OAuth 权限范围**：每个用户的附件流程*仅*请求 `chat.messages.create`——这是覆盖 `media.upload` 及后续 `messages.create` 所需的最小权限。令牌以纯 JSON 格式持久化存储在 `~/.hermes/google_chat_user_tokens/<sanitized_email>.json`（文件系统权限是保护机制——与服务账号密钥文件的模型相同）。每个令牌严格归属于一个用户；撤销操作也仅限于该用户。
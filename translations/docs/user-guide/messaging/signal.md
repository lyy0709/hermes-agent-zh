...
:::info 无需新的 Python 依赖
Signal 适配器使用 `httpx`（已是 Hermes 核心依赖）进行所有通信。无需额外的 Python 包。你只需要在外部安装 signal-cli。
:::
...
:::caution
signal-cli **不在** apt 或 snap 仓库中。上面的 Linux 安装直接从 [GitHub 发布页](https://github.com/AsamK/signal-cli/releases)下载。
:::
...
Signal-cli 作为**链接设备**工作——类似于 WhatsApp Web，但用于 Signal。你的手机保持为主要设备。
...
Agent 可以通过响应中的 `MEDIA:` 标签发送媒体文件。支持以下发送方法：
...
所有传出的媒体都通过 Signal 的标准附件 API 处理。与某些平台不同，Signal 在协议层面不区分语音消息和文件附件。

附件大小限制：**100 MB**（双向）。
:::warning
**Signal 服务器会对附件上传进行速率限制**，适配器使用调度器进行多图片发送，将图片分批（每组 32 张）并限制上传速率以匹配 Signal 服务器策略。
:::

### 原生格式化、回复引用和反应

Signal 消息以**原生格式化**呈现，而不是字面上的 Markdown 字符。适配器将 Markdown（`**粗体**`、`*斜体*`、`` `代码` ``、`~~删除线~~`、`||剧透||`、标题）转换为 Signal 的 `bodyRanges`，这样文本在接收方客户端显示为真实的样式，而不是可见的 `**` / `` ` `` 字符。
...
所有这些都不需要额外的配置——在最近的 signal-cli 版本中默认启用。如果你的 `signal-cli` 版本太旧，Hermes 会回退到纯文本发送并记录一次性警告。
...
如果你在自己的电话号码上（而不是单独的机器人号码）将 signal-cli 作为**链接的辅助设备**运行，你可以通过 Signal 的“给自己发消息”功能与 Hermes 交互。

只需从你的手机给自己发送一条消息——signal-cli 会接收到它，Hermes 会在同一会话中回复。
...
**无需额外配置。** 只要 `SIGNAL_ACCOUNT` 与你的电话号码匹配，这就会自动生效。
...
| 问题 | 解决方案 |
|---------|----------|
| 设置期间 **"无法连接到 signal-cli"** | 确保 signal-cli 守护进程正在运行：`signal-cli --account +你的号码 daemon --http 127.0.0.1:8080` |
| **未收到消息** | 检查 `SIGNAL_ALLOWED_USERS` 是否包含发送者的号码（E.164 格式，带 `+` 前缀） |
| **"在 PATH 中找不到 signal-cli"** | 安装 signal-cli 并确保其在你的 PATH 中，或使用 Docker |
| **连接不断断开** | 检查 signal-cli 日志中的错误。确保已安装 Java 17+。 |
| **忽略群组消息** | 配置 `SIGNAL_GROUP_ALLOWED_USERS` 为特定的群组 ID，或使用 `*` 允许所有群组。 |
| **机器人不响应任何人** | 配置 `SIGNAL_ALLOWED_USERS`，使用私信配对，或者如果你想要更广泛的访问权限，通过消息网关策略明确允许所有用户。 |
...
- `~/.local/share/signal-cli/` 中的 signal-cli 会话数据包含账户凭据——像保护密码一样保护它

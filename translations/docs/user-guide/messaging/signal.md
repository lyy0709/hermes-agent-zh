...
附件大小限制：**100 MB**（双向）。
:::warning
**Signal 服务器会对附件上传进行速率限制**，适配器使用调度器进行多图片发送，将图片分批为 32 组一组，并限制上传速度以匹配 Signal 服务器策略。
:::

### 原生格式、回复引用和反应

Signal 消息以**原生格式**呈现，而不是字面意义上的 Markdown 字符。适配器将 Markdown（`**粗体**`、`*斜体*`、`` `代码` ``、`~~删除线~~`、`||剧透||`、标题）转换为 Signal 的 `bodyRanges`，以便文本在接收方客户端显示为真实样式，而不是可见的 `**` / `` ` `` 字符。

**回复引用。** 当 Hermes 回复特定消息时，它现在会发布一个引用原始消息的原生回复——与 Signal 用户自己使用“回复”时看到的 UI 效果相同。这对于响应入站消息生成的回复是自动的。

**反应。** Agent 可以通过标准反应 API 对消息做出反应；反应在 Signal 中显示为引用消息上的表情符号反应，而不是额外的文本。

所有这些都不需要额外配置——在最近的 signal-cli 版本中默认启用。如果您的 `signal-cli` 版本太旧，Hermes 会回退到纯文本发送并记录一次性警告。
...
Signal 不支持编辑已发送的消息。因此，Hermes 在 Signal 上抑制了消息网关的工具进度气泡，即使启用了 `/verbose` 并为该平台保存了非 `off` 模式。

您仍然可以在 CLI 中看到工具活动，最终的 Signal 回复可以包含正常的助手输出。如果您需要在聊天中实时查看每个工具的进度，请使用支持消息编辑的消息平台。
...
只需从您的手机向自己发送一条消息——signal-cli 会接收到它，Hermes 会在同一会话中回复。

**工作原理：**
- “给自己发消息”以 `syncMessage.sentMessage` 信封形式到达
- 适配器检测到这些消息是发送给机器人自身账户时，会将其作为常规入站消息处理
- 回显保护（发送时间戳跟踪）可防止无限循环——机器人自己的回复会自动过滤掉
...
:::warning
**始终配置访问控制。** 机器人默认具有终端访问权限。如果没有 `SIGNAL_ALLOWED_USERS` 或私信配对，消息网关会拒绝所有传入消息作为安全措施。
:::

...
:::info
Yuanbao 是一个主要供腾讯内部和企业环境使用的企业通讯平台。它使用 WebSocket 进行实时通信，采用基于 HMAC 的身份验证，并支持包括图片、文件和语音消息在内的富媒体。
:::
...
2.  在应用中，进入 **PAI → 我的机器人** 并创建一个新的机器人
...
:::tip 自动设置主频道
如果未配置主频道，第一个向机器人发送消息的用户将被自动设为主频道所有者。如果当前主频道是群聊，第一个私聊消息会将其升级为直接频道。
:::
...
要向机器人发送文件，只需直接在 Yuanbao 聊天中附加文件。机器人会自动下载并处理文件附件。
...
当您要求机器人创建或导出文件时，它会直接将文件发送到您的 Yuanbao 聊天中。
...
# 允许私聊机器人的用户 ID，逗号分隔（仅在 DM_POLICY=allowlist 时使用）
...
Yuanbao 有最大消息大小限制。Hermes 会自动对大型响应进行分块，并采用支持 Markdown 的分割方式（尊重代码块、表格和段落边界）。

### 连接参数

以下连接参数已内置在适配器中，并具有合理的默认值：

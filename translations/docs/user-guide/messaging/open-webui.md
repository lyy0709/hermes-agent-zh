...
Open WebUI 连接到 Hermes Agent 的 API 服务器，就像连接到 OpenAI 一样。您的 Agent 使用其完整的工具集（终端、文件操作、网络搜索、记忆、技能）处理请求，并返回最终响应。
...
如果您希望在本地将 Hermes + Open WebUI 连接在一起，并有一个可重复使用的启动器，请运行：
...
`ENABLE_OLLAMA_API=false` 会抑制默认的 Ollama 后端，否则它会显示为空并弄乱模型选择器。如果您确实同时运行了 Ollama，请省略此参数。
...
这是默认方式，无需额外配置。Open WebUI 发送标准 OpenAI 格式的请求，Hermes Agent 会相应响应。每个请求都包含完整的对话历史。
...
使用 Responses API 时，Open WebUI 以 Responses 格式（`input` 数组 + `instructions`）发送请求，并且 Hermes Agent 可以通过 `previous_response_id` 在多个回合中保留完整的工具调用历史。当 `stream: true` 时，Hermes 还会流式传输规范原生的 `function_call` 和 `function_call_output` 项，这使得能够在渲染 Responses 事件的客户端中实现自定义的结构化工具调用 UI。

:::note
目前，即使在 Responses 模式下，Open WebUI 仍在客户端管理对话历史——它在每个请求中发送完整的消息历史记录，而不是使用 `previous_response_id`。如今 Responses 模式的主要优势是结构化的事件流：文本增量、`function_call` 和 `function_call_output` 项作为 OpenAI Responses SSE 事件到达，而不是 Chat Completions 块。
:::
...
2. Hermes Agent 创建一个包含其完整工具集的 AIAgent 实例
...
你的 Agent 可以访问与使用 CLI 或 Telegram 时相同的所有工具和功能——唯一的区别是前端。
...
- **Docker 网络**：从 Docker 内部看，`localhost` 指的是容器，而不是你的主机。使用 `host.docker.internal` 或 `--network=host`。
...
:::warning
Open WebUI 在首次启动后，会将其 OpenAI 兼容的连接设置持久化到自己的数据库中。如果你在管理界面中不小心保存了错误的密钥，仅修复环境变量是不够的——需要在 **管理员设置 → 连接** 中更新或删除已保存的连接，或者重置 Open WebUI 数据目录 / 数据库。
:::
...
模型下拉菜单将显示 `alice` 和 `bob` 作为不同的模型。你可以通过管理面板将模型分配给 Open WebUI 用户，从而为每个用户提供自己独立的 Hermes Agent。

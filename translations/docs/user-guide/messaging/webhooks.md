...
| `prompt` | 否 | 使用点号表示法访问有效负载的模板字符串（例如 `{pull_request.title}`）。如果省略，则完整的 JSON 有效负载将被转储到提示词中。 |
| `skills` | 否 | 为 Agent 运行加载的技能名称列表。 |
| `deliver` | 否 | 发送响应的位置：`github_comment`、`telegram`、`discord`、`slack`、`signal`、`sms`、`whatsapp`、`matrix`、`mattermost`、`homeassistant`、`email`、`dingtalk`、`feishu`、`wecom`、`weixin`、`bluebubbles` 或 `log`（默认）。 |
...
提示词使用点号表示法访问 Webhook 有效负载中的嵌套字段：

- `{pull_request.title}` 解析为 `payload["pull_request"]["title"]`
- `{repository.full_name}` 解析为 `payload["repository"]["full_name"]`
- `{__raw__}` —— 特殊 Token，将**整个有效负载**作为缩进 JSON 转储（截断为 4000 个字符）。对于监控警报或需要完整上下文的通用 Webhook 很有用。
- 缺失的键将保留为字面量 `{key}` 字符串（无错误）
...
GitLab webhook 的工作方式类似，但使用不同的身份验证机制。GitLab 将密钥作为纯文本 `X-Gitlab-Token` 头发送（精确字符串匹配，而非 HMAC）。
...
Agent 可以通过终端工具在 `webhook-subscriptions` 技能的引导下创建订阅。要求 Agent "为 GitHub issues 设置一个 webhook"，它将运行相应的 `hermes webhook subscribe` 命令。
...
如果配置了密钥但不存在可识别的签名头，则请求将被拒绝。

### 密钥是必需的
每个路由都必须有一个密钥——可以直接在路由上设置，也可以从全局 `secret` 继承。没有密钥的路由会导致适配器在启动时失败并报错。仅用于开发/测试时，可以将密钥设置为 `"INSECURE_NO_AUTH"` 以完全跳过验证。
...
交付 ID（来自 `X-GitHub-Delivery`、`X-Request-ID` 或时间戳回退）会被缓存**1 小时**。重复的交付（例如 Webhook 重试）会被静默跳过并返回 `200` 响应，以防止重复运行 Agent。
...
- 幂等性缓存应能防止此问题——检查 Webhook 源是否发送了交付 ID 标头（`X-GitHub-Delivery` 或 `X-Request-ID`）
- 交付 ID 会缓存 1 小时

...
LINE 的回复 Token 是一次性的，大约在入站事件后 60 秒过期。慢速 LLM 无法及时回复，这通常会导致强制使用付费的 Push API 调用。

当 LLM 运行时间超过 `LINE_SLOW_RESPONSE_THRESHOLD` 秒（默认 `45`）时，适配器会消耗原始的回复 Token 来发送一个 **Template Buttons** 气泡：

> 🤔 还在思考。请在下方点击以在答案准备好时获取。
>
> [ 获取答案 ]
...
| 变量 | 必需 | 默认值 | 描述 |
|---|---|---|---|
| `LINE_CHANNEL_ACCESS_TOKEN` | 是 | — | 长时效频道访问 Token |
...
| `LINE_ALLOWED_ROOMS` | 至少一个 | — | 逗号分隔的聊天室 ID（R 前缀） |
...
| `LINE_DELIVERED_TEXT` | 否 | "Already replied ✅" | 当再次点击已投递的按钮时的回复 |
| `LINE_INTERRUPTED_TEXT` | 否 | "Run was interrupted before completion." | 当点击 `/stop` 产生的孤立按钮时的回复 |
...
**回传按钮从未出现。** 要么 LLM 响应速度快于 `LINE_SLOW_RESPONSE_THRESHOLD`，要么另一个气泡（工具进度、流式输出）先消耗了回复 Token。请参阅“慢速 LLM 响应”下的抑制配置块。

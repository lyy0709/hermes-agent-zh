...
将 Hermes 连接到腾讯的个人即时通讯平台 [微信](https://weixin.qq.com/)。该适配器使用腾讯的 **iLink Bot API** 连接个人微信账号——这与企业微信不同。消息通过长轮询方式传递，因此不需要公共端点或 Webhook。
...
- `@` 提及用于扫描二维码的个人微信账号**不等于** `@` 提及 iLink 机器人——机器人是一个独立的身份。
- 下面的 `WEIXIN_GROUP_POLICY` / `WEIXIN_GROUP_ALLOWED_USERS` 设置仅在 iLink 实际为您的账号类型返回群组事件时才生效。如果 iLink 不返回，无论策略如何，群组消息都不会到达 Hermes。

实际上，大多数部署只能可靠地实现与 iLink 机器人的私聊。如果配置后群组消息传递仍不工作，限制在于 iLink 端，而非 Hermes。每当 `WEIXIN_GROUP_POLICY` 设置为 `disabled` 以外的值时，消息网关在启动时会记录一条 `WARNING` 日志。
:::
...
- **私聊消息**——可配置的访问策略；群聊消息取决于 iLink 是否实际为连接的身份传递群组事件（对于 iLink 机器人账号通常不是这样——请参阅上面的警告）
- **媒体支持**——图片、视频、文件和语音消息
...
- **智能消息分块**——消息在限制内保持为单个气泡；仅超大的有效负载在逻辑边界处分割
...
`WEIXIN_ALLOWED_USERS` 是一个**入站过滤器**，而非邀请系统。二维码登录将一个 iLink 机器人身份与 Hermes 连接。其他人不应使用自己的账户扫描 Hermes 的二维码；他们必须通过微信向已连接的 iLink 机器人/联系人发送消息，并且只有当发送者的微信用户 ID 存在于 `WEIXIN_ALLOWED_USERS` 中时，Hermes 才会处理该私聊消息。
...
如果只有扫描二维码的账户可以与 Hermes 对话，请确认其他用户是在向 iLink 机器人身份本身发送消息，而不是向执行二维码登录的个人微信账户发送消息。iLink 机器人是一个独立的身份，并且普通的微信联系人/群组路由可能受到腾讯 iLink 行为的限制。

### 群组策略

控制机器人在**当 iLink 为已连接身份传递群组事件时**在哪些群组中响应。对于二维码登录的 iLink 机器人身份（例如 `...@im.bot`），群组事件通常根本不会被传递，因此此策略可能无效——请参阅本页顶部的 iLink 机器人限制警告。
...
# 注意：这是一个以逗号分隔的群聊 ID 列表，尽管变量名包含 "USERS"，但这不是成员用户 ID。
# 配置时请记住这一点。
WEIXIN_GROUP_ALLOWED_USERS=group_id_1,group_id_2
```

:::note
微信的默认群组策略是 `disabled`（与企业微信默认 `open` 不同）。这是有意为之——个人微信账户可能加入许多群组，而 iLink 机器人身份通常根本无法接收普通的微信群组消息。如果您将 `WEIXIN_GROUP_POLICY` 设置为 `disabled` 以外的任何值，消息网关在启动时会记录一条 `WARNING`。
:::
...
| **语音** | 如果有文本转录可用，则提取为文本。否则，音频（SILK 格式）会被下载并缓存。 |
...
4. 当 Agent 处理消息时，消息网关会自动触发输入状态指示

## 长轮询连接

适配器使用 HTTP 长轮询（非 WebSocket）来接收消息：
...
| 设置过程中二维码过期 | 二维码最多自动刷新 3 次。如果持续过期，请检查你的网络连接 |
| Bot 不回复私聊 | 检查 `WEIXIN_DM_POLICY`——如果设置为 `allowlist`，发送者必须在 `WEIXIN_ALLOWED_USERS` 中 |
| Bot 忽略群聊消息 | 群聊策略默认为 `disabled`。设置 `WEIXIN_GROUP_POLICY=open` 或 `allowlist`——但请注意，通过二维码登录的 iLink bot 身份 (`...@im.bot`) 通常根本无法接收普通的微信群聊消息。如果消息网关日志显示没有群聊消息的原始入站事件，则限制来自 iLink 端，而非 Hermes。 |

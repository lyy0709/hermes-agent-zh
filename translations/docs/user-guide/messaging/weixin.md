...
将 Hermes 连接到 [微信](https://weixin.qq.com/)，这是腾讯的个人即时通讯平台。此适配器使用腾讯的 **iLink Bot API** 连接个人微信账号——这与企业微信不同。消息通过长轮询方式传递，因此不需要公共端点或 Webhook。

:::info
此适配器适用于**个人微信账号**。如果您需要企业微信，请改用 [WeCom 适配器](./wecom.md)。
:::

:::warning iLink bot 身份——普通微信群可能无法工作
二维码登录会将 Hermes 连接到一个 **iLink bot 身份**（例如 `a5ace6fd482e@im.bot`），**而不是**一个完全可脚本化的普通个人微信账号。这意味着：

- iLink bot 身份通常**无法像普通联系人一样被邀请加入普通微信群**。
- 对于大多数 bot 类型的账号，iLink 通常**不会将普通微信群事件**（包括对用于二维码登录的个人账号的 `@` 提及）传递给消息网关。
- `@` 提及用于扫描二维码的个人微信账号**不等于** `@` 提及 iLink bot——bot 是一个独立的身份。
- 下面的 `WEIXIN_GROUP_POLICY` / `WEIXIN_GROUP_ALLOWED_USERS` 设置仅在 iLink 实际为您的账号类型返回群组事件时才生效。如果 iLink 不返回，无论策略如何，群组消息都不会到达 Hermes。

实际上，大多数部署只能可靠地实现与 iLink bot 的私聊。如果配置后群组消息传递不工作，限制在于 iLink 端，而不是 Hermes。每当 `WEIXIN_GROUP_POLICY` 设置为 `disabled` 以外的值时，消息网关在启动时都会记录一条 `WARNING` 日志。
:::
...
- **私聊消息**——可配置的访问策略；群聊消息取决于 iLink 是否为已连接的身份实际传递群组事件（对于 iLink bot 账号通常不是这样——请参阅上面的警告）
- **媒体支持**——图片、视频、文件和语音消息
...
- **智能消息分块**——消息在限制内保持为单个气泡；只有超大的负载才会在逻辑边界处分割
- **输入指示器**——在 Agent 处理时在微信客户端显示“正在输入…”状态
- **SSRF 防护**——在下载前验证出站媒体 URL
- **消息去重**——5 分钟滑动窗口防止重复处理
...
| `split_multiline_messages` | `false` | 当为 `true` 时，将多行回复分割成多个聊天消息（旧版行为）。当为 `false` 时，将多行回复保持为一条消息，除非它们超过长度限制。 |
| `text_batch_delay_seconds` | `3.0` | 在将缓冲的快速文本消息突发作为单个合并请求刷新之前的静默期（秒）。iLink 单独传递消息，因此此防抖避免每个片段触发一次 Agent 调用。设置为 `0` 可立即分派每条消息。 |
| `text_batch_split_delay_seconds` | `5.0` | 当最新片段接近分割阈值（iLink 可能已分块的长消息）时使用的扩展刷新延迟。 |
## 访问策略
...
`WEIXIN_ALLOWED_USERS` 是一个**入站过滤器**，而不是邀请系统。二维码登录将一个 iLink 机器人身份连接到 Hermes。其他人**不**使用自己的账户扫描 Hermes 的二维码；他们必须通过微信向已连接的 iLink 机器人/联系人发送消息，并且只有当发送者的微信用户 ID 存在于 `WEIXIN_ALLOWED_USERS` 中时，Hermes 才会处理该私聊消息。

一个实用的设置流程是：

1. 使用 `hermes gateway setup` 将 Hermes 配对一次，并记下已连接的 iLink 机器人账户。
2. 让每个被允许的用户向该机器人/联系人发送一条私聊消息。
3. 从消息网关日志或入站事件负载中读取发送者/用户 ID。
4. 将这些 ID 添加到 `WEIXIN_ALLOWED_USERS`，然后重启消息网关。

如果只有扫描二维码的账户可以与 Hermes 对话，请确认其他用户是在向 iLink 机器人身份本身发送消息，而不是向执行二维码登录的个人微信账户发送消息。iLink 机器人是一个独立的身份，普通的微信联系人/群组路由可能受到腾讯 iLink 行为的限制。

### 群聊策略

控制机器人在**当 iLink 为已连接身份传递群聊事件时**在哪些群组中响应。对于二维码登录的 iLink 机器人身份（例如 `...@im.bot`），群聊事件通常根本不会被传递，因此此策略可能无效——请参阅本页顶部的 iLink 机器人限制警告。
...
:::note
微信的默认群聊策略是 `disabled`（与默认值为 `open` 的企业微信不同）。这是有意为之——个人微信账户可能加入了许多群组，而 iLink 机器人身份通常根本无法接收普通的微信群聊消息。如果您将 `WEIXIN_GROUP_POLICY` 设置为 `disabled` 以外的任何值，消息网关会在启动时记录一条 `WARNING` 日志。
:::
...
| **语音** | 如果有文本转录可用，则提取为文本。否则音频（SILK 格式）会被下载并缓存。 |

**引用消息：** 来自引用（回复）消息的媒体也会被提取，以便 Agent 了解用户正在回复的内容。
...
iLink Bot API 要求为给定对等方的每条出站消息回显一个 `context_token`。适配器维护一个磁盘支持的上下文 Token 存储：
...
只要消息符合平台限制，就会作为单个聊天消息传递。只有过大的负载才会被拆分传递：

- 最大消息长度：**4000 个字符**
- 低于限制的消息即使包含多个段落或换行符也保持完整
- 过大的消息在逻辑边界（段落、空行、代码块）处拆分
- 尽可能保持代码块完整（除非代码块本身超过限制，否则永远不会在块内拆分）
- 过大的单个块回退到基础适配器的截断逻辑
...
4. 当 Agent 处理消息时，消息网关会自动触发打字指示器
...
1.  **连接：** 验证凭据并启动轮询循环
2.  **轮询：** 调用 `getupdates` 并设置 35 秒超时；服务器会保持请求直到消息到达或超时
3.  **分发：** 入站消息通过 `asyncio.create_task` 并发分发
4.  **同步缓冲区：** 一个持久的同步游标 (`get_updates_buf`) 会保存到磁盘，以便适配器在重启后能从正确的位置恢复
...
入站消息使用消息 ID 进行去重，时间窗口为 5 分钟。这可以防止在网络波动或轮询响应重叠时重复处理消息。

### Token 锁

同一时间只能有一个 Weixin 消息网关实例使用给定的 token。适配器在启动时获取一个作用域锁，并在关闭时释放。如果另一个消息网关已经在使用相同的 token，启动将失败并显示提示性错误信息。
...
| `WEIXIN_GROUP_ALLOWED_USERS` | — | _(空)_ | 用于群聊白名单的逗号分隔的**群聊 ID**（非成员用户 ID）。变量名是历史遗留的——它期望的是群 ID，而不是用户 ID。 |
| `WEIXIN_HOME_CHANNEL` | — | — | 用于定时任务/通知输出的聊天 ID |
| `WEIXIN_HOME_CHANNEL_NAME` | — | `Home` | 主频道的显示名称 |
| `WEIXIN_ALLOW_ALL_USERS` | — | — | 消息网关级别的标志，允许所有用户（由设置向导使用） |
...
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | 在你的 `.env` 文件中设置 `WEIXIN_ACCOUNT_ID` 或运行 `hermes gateway setup` |
...
| 设置过程中二维码过期 | 二维码最多会自动刷新 3 次。如果持续过期，请检查你的网络连接 |
| Bot 不回复私聊 | 检查 `WEIXIN_DM_POLICY`——如果设置为 `allowlist`，发送者必须在 `WEIXIN_ALLOWED_USERS` 中 |
| Bot 忽略群消息 | 群聊策略默认为 `disabled`。设置 `WEIXIN_GROUP_POLICY=open` 或 `allowlist`——但请注意，通过二维码登录的 iLink bot 身份 (`...@im.bot`) 通常根本无法接收普通的微信群消息。如果消息网关日志显示没有群消息的原始入站事件，则限制在 iLink 端，而非 Hermes。 |
...
| 消息出现重复 | 适配器通过消息 ID 去重。如果看到重复消息，请检查是否运行了多个消息网关实例 |
| `iLink POST ... HTTP 4xx/5xx` | 来自 iLink 服务的 API 错误。检查你的 token 有效性和网络连接 |

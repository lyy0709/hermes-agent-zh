...
Hermes Agent 与飞书和 Lark 集成，作为一个功能齐全的机器人。连接后，您可以在私聊或群聊中与 Agent 对话，在家庭聊天中接收定时任务结果，并通过常规的消息网关流程发送文本、图片、音频和文件附件。

该集成支持两种连接模式：

- `websocket` — 推荐使用；Hermes 建立出站连接，您不需要公共的 webhook 端点
...
当飞书发送 URL 验证挑战（`type: url_verification`）时，webhook 会自动响应，以便您可以在飞书开发者控制台中完成订阅设置。当设置了 `FEISHU_VERIFICATION_TOKEN` 时，挑战响应会受到门控 — 缺少或不匹配 Token 的挑战请求将被拒绝，这样未经身份验证的远程方就无法通过回显攻击者控制的挑战数据来证明端点控制权。
...
在飞书/Lark 聊天中使用 `/set-home` 将其标记为用于接收定时任务结果和跨平台通知的家庭频道。
你也可以预先配置：
...
在生产环境中使用时，请设置一个飞书 Open ID 白名单：

```bash
FEISHU_ALLOWED_USERS=ou_xxx,ou_yyy
```

如果你将白名单留空，任何能联系到该机器人的人都可能使用它。在群聊中，消息处理前会检查发送者的 `open_id` 是否在白名单中。
...
此密钥可在飞书应用配置的 **事件订阅** 部分找到。设置后，适配器会使用以下签名算法验证每个 Webhook 请求：
...
:::tip
在 WebSocket 模式下，签名验证由 SDK 自身处理，因此 `FEISHU_ENCRYPT_KEY` 是可选的。在 Webhook 模式下，强烈建议生产环境使用。
:::

### 验证令牌

这是另一层身份验证，用于检查 Webhook 负载内部的 `token` 字段：

```bash
FEISHU_VERIFICATION_TOKEN=your-verification-token
```

此令牌同样可在飞书应用配置的 **事件订阅** 部分找到。设置后，每个入站 Webhook 负载的 `header` 对象中必须包含一个匹配的 `token`。不匹配的令牌将被拒绝，并返回 HTTP 401 状态码。
...
在所有模式下，机器人必须在消息处理前被明确 @提及（或 @all）。私聊消息始终不受此限制。
...
**命令批准** 也是以这种方式工作的 —— 当 Agent 需要运行危险命令时，它会发送一个带有“允许一次”/“允许本次会话”/“始终允许”/“拒绝”按钮的交互式卡片。用户点击按钮后，卡片操作回调会将批准决定传回给 Agent。
...
:::warning
缺少以上任何步骤，飞书都能成功 *发送* 交互式卡片（发送仅需要 `im:message:send` 权限），但点击任何按钮都会返回错误 200340。卡片看起来能正常工作 —— 只有当用户与之交互时，错误才会显现。
:::
## 文档评论智能回复

除了聊天功能外，适配器还能回复**飞书文档**上的 `@` 提及。当用户评论文档（局部文本选择或全文档评论）并 @ 提及机器人时，Hermes 会读取文档内容及周围的评论线程，并在该线程中内联发布一个 LLM 回复。

该功能由 `drive.notice.comment_add_v1` 事件驱动，处理程序会：

- 并行获取文档内容和评论时间线（全文档线程为 20 条消息，局部选择线程为 12 条）。
- 运行 Agent，并使用限定于该单次评论会话的 `feishu_doc` + `feishu_drive` 工具集。
- 将回复分块为 4000 字符，并以线程回复的形式发布。
- 为每个文档会话缓存 1 小时，上限为 50 条消息，以便同一文档上的后续评论能保持上下文。

### 三级访问控制

文档评论回复**仅限显式授权**——没有隐式的允许所有模式。权限按以下顺序解析（每个字段首次匹配即生效）：
...
规则存储在 `~/.hermes/feishu_comment_rules.json` 中（配对授权存储在 `~/.hermes/feishu_comment_pairing.json` 中），支持基于修改时间缓存的即时重载——编辑会在下一个评论事件时生效，无需重启消息网关。
...
如果飞书 API 拒绝了帖子负载（例如，由于不支持的 Markdown 结构），适配器会自动回退为发送纯文本，并去除 Markdown 格式。这种两阶段回退确保了消息总能送达。
...
适配器包含对快速突发消息的去抖动功能，以避免压垮 Agent：
...
platforms:
  feishu:
    extra:
      ...
      ws_ping_interval: 30         # WebSocket 保活 ping 之间的秒数（可选；未设置时使用 SDK 默认值）
```
  ...
  feishu:
    extra:
      ...
      group_rules:
        ...
        "oc_free_chat":
          policy: "open"
          require_mention: false       # 为此聊天覆盖全局的 FEISHU_REQUIRE_MENTION 设置
```
...
在 `group_rules` 条目上设置 `require_mention: false` 可以跳过该特定聊天的 @-提及要求。当省略时，聊天将继承全局的 `FEISHU_REQUIRE_MENTION` 值。
...
| `HERMES_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | 每次文本批处理合并的最大消息数 |
| `HERMES_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | 每次文本批处理合并的最大字符数 |
...
| `Webhook rejected: invalid verification token` | 确保 `FEISHU_VERIFICATION_TOKEN` 与您飞书应用“事件订阅”配置中的令牌匹配 |
| `Webhook rejected: invalid signature` | 确保 `FEISHU_ENCRYPT_KEY` 与您飞书应用配置中的加密密钥匹配 |
| 发送的消息显示为纯文本 | 飞书 API 拒绝了消息负载；这是正常的回退行为。查看日志获取详细信息。 |
| 机器人未收到图片/文件 | 为您的飞书应用授予 `im:message` 和 `im:resource` 权限范围 |
| 机器人身份未自动检测 | 通常是访问飞书机器人信息端点的瞬时网络问题。作为临时解决方案，手动设置 `FEISHU_BOT_OPEN_ID` 和 `FEISHU_BOT_NAME`。 |

...
当 Hermes 在您的笔记本电脑、工作站或私有服务器上运行时，使用 WebSocket 模式。无需公开 URL。官方的 Lark SDK 会建立并维护一个持久的出站 WebSocket 连接，并具有自动重连功能。
...
此密钥可在您飞书应用的 **事件订阅** 部分找到。设置后，适配器会使用以下签名算法验证每个 webhook 请求：

```
SHA256(timestamp + nonce + encrypt_key + body)
```

计算出的哈希值会与 `x-lark-signature` 头部进行时序安全比较。签名无效或缺失的请求将被拒绝，并返回 HTTP 401。
...
此令牌同样可在您飞书应用的 **事件订阅** 部分找到。设置后，每个入站 webhook 负载在其 `header` 对象中必须包含匹配的 `token`。不匹配的令牌将被拒绝，并返回 HTTP 401。
...
在所有模式下，消息被处理前，机器人必须在群中被显式 @ 提及（或 @all）。私聊消息始终绕过此限制。
...
对于按聊天控制，请在 `group_rules` 条目上设置 `require_mention` — 请参阅下面的[按群访问控制](#per-group-access-control)。

### 机器人身份

Hermes 在启动时自动检测机器人的 `open_id` 和显示名称。仅当自动检测无法访问飞书 API，或者您的应用使用租户范围的用户 ID 时，才需要手动设置这些值：
...
默认情况下，Hermes 会忽略其他机器人发送的消息。当您希望 Hermes 参与 A2A 编排或接收同一群组中其他机器人的通知时，请启用机器人间消息传递。
...
对等机器人不需要添加到 `FEISHU_ALLOWED_USERS` — 该允许列表仅适用于人类发送者。

授予 `application:bot.basic_info:read` 权限范围以显示对等机器人名称；没有此权限，对等机器人仍能正确路由，但显示为其 `open_id`。
...
由消息网关驱动的更新提示使用原生的飞书 `Yes` / `No` 卡片，而不是回退到纯文本回复。当 `hermes update --gateway` 需要确认时，适配器会将选定的答案记录在 Hermes 的 `.update_response` 文件中，并用已解决状态的内联卡片替换原卡片。

卡片操作事件以 `MessageType.COMMAND` 类型分发，因此它们会流经正常的命令处理流水线。

**命令批准**也是以这种方式工作的 — 当 Agent 需要运行危险命令时，它会发送一个带有"允许一次"/"会话"/"始终允许"/"拒绝"按钮的交互式卡片。用户点击按钮，卡片操作回调将批准决定传回给 Agent。
...
除了聊天，适配器还可以回复**飞书/Lark 文档**上的 `@` 提及。当用户评论文档（本地文本选择或整篇文档评论）并 @ 提及机器人时，Hermes 会读取文档内容以及周围的评论线程，并在该线程中内联发布 LLM 回复。

该功能由 `drive.notice.comment_add_v1` 事件驱动，处理程序会：

- 并行获取文档内容和评论时间线（整篇文档线程为 20 条消息，本地选择线程为 12 条消息）。
- 运行 Agent，使用限定在该单个评论会话范围内的 `feishu_doc` + `feishu_drive` 工具集。
- 将回复分块为 4000 个字符，并作为线程回复发布回去。
- 按文档缓存会话 1 小时，上限为 50 条消息，以便同一文档上的后续评论能保持上下文。

### 三层访问控制

文档评论回复是**显式授权** — 没有隐式的允许所有模式。权限按以下顺序解析（每个字段首次匹配即生效）：
...
规则存储在 `~/.hermes/feishu_comment_rules.json` 中（配对授权存储在 `~/.hermes/feishu_comment_pairing.json` 中），支持基于修改时间缓存的实时重载 — 编辑在下一个评论事件时生效，无需重启消息网关。
...
当 Agent 正在工作时，机器人会在你的消息上显示 `Typing` 反应。回复到达时会被清除，如果处理失败则替换为 `CrossMark`。
...
入站消息使用消息 ID 进行去重，TTL 为 24 小时。去重状态在重启时持久化到 `~/.hermes/feishu_seen_message_ids.json`。
...
| 帖子消息显示为纯文本 | 飞书 API 拒绝了帖子负载；这是正常的回退行为。检查日志以获取详细信息。 |
| 机器人未收到图片/文件 | 为你的飞书应用授予 `im:message` 和 `im:resource` 权限范围 |
| 机器人身份未自动检测 | 通常是访问飞书机器人信息端点的瞬时网络问题。作为变通方法，手动设置 `FEISHU_BOT_OPEN_ID` 和 `FEISHU_BOT_NAME`。 |
| 启用 `FEISHU_ALLOW_BOTS` 后，对等机器人消息仍被忽略 | Hermes 尚无法识别自身 — 设置 `FEISHU_BOT_OPEN_ID`（如果你的应用使用 `sender_id_type=user_id`，还需设置 `FEISHU_BOT_USER_ID`）。 |
| 对等机器人显示为 `ou_xxxxxx` 而不是名称 | 授予 `application:bot.basic_info:read` 范围。 |
| 点击审批按钮时出现错误 200340 | 在飞书开发者控制台中启用**交互卡片**能力并配置**卡片请求 URL**。参见上文的 [必需的飞书应用配置](#required-feishu-app-configuration)。 |
| `Webhook rate limit exceeded` | 同一 IP 每分钟超过 120 个请求。这通常是配置错误或循环造成的。 |

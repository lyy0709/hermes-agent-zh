...
Hermes Agent 与飞书和 Lark 集成，作为一个功能齐全的机器人。连接后，你可以在私聊或群聊中与 Agent 对话，在家庭聊天中接收定时任务结果，并通过常规的消息网关流程发送文本、图片、音频和文件附件。

该集成支持两种连接模式：

- `websocket` — 推荐；Hermes 建立出站连接，你不需要公开的 webhook 端点
- `webhook` — 当你希望飞书/Lark 通过 HTTP 将事件推送到你的消息网关时很有用
...
选择 **飞书 / Lark** 并用你的飞书或 Lark 手机应用扫描二维码。Hermes 将自动创建一个具有正确权限的机器人应用并保存凭证。
...
当 Hermes 在你的笔记本电脑、工作站或私有服务器上运行时，使用 WebSocket 模式。不需要公开 URL。官方的 Lark SDK 会建立并维护一个持久的出站 WebSocket 连接，并自动重连。
...
当飞书发送 URL 验证挑战（`type: url_verification`）时，webhook 会自动响应，以便你可以在飞书开发者控制台中完成订阅设置。当设置了 `FEISHU_VERIFICATION_TOKEN` 时，挑战响应的处理会受其限制 — 缺少或不匹配 Token 的挑战请求将被拒绝，这样未经身份验证的远程方就无法通过回显攻击者控制的挑战数据来证明端点控制权。
...
如果你将允许列表留空，任何能接触到机器人的用户都可能使用它。在群聊中，消息处理前会检查发送者的 open_id 是否在允许列表中。
...
计算出的哈希值会与 `x-lark-signature` 头部进行时序安全的比较。签名无效或缺失的请求将被拒绝，并返回 HTTP 401。
...
```
此 Token 也存在于你的飞书应用的 **事件订阅** 部分。设置后，每个入站 webhook 负载的 `header` 对象中必须包含匹配的 `token`。不匹配的 Token 会以 HTTP 401 拒绝。
...
在所有模式下，消息被处理前，机器人必须在群中被明确 @提及（或 @all）。私聊消息始终绕过此关卡。

设置 `FEISHU_REQUIRE_MENTION=false` 可以让 Hermes 读取所有群消息，而无需 @提及：

```bash
FEISHU_REQUIRE_MENTION=false
```

对于每个聊天的控制，可以在 `group_rules` 条目上设置 `require_mention` — 请参阅下面的 [按群访问控制](#per-group-access-control)。

### 机器人身份

Hermes 在启动时自动检测机器人的 `open_id` 和显示名称。你只需要在自动检测无法访问飞书 API，或者你的应用使用租户范围的用户 ID 时手动设置这些：
...
默认情况下，Hermes 会忽略其他机器人发送的消息。当你希望 Hermes 参与 A2A 编排或接收同一群组中其他机器人的通知时，请启用机器人间消息传递。
...
授予 `application:bot.basic_info:read` 权限范围以显示对方机器人名称；没有此权限，对方机器人仍能正确路由，但会显示为它们的 `open_id`。

## 交互式卡片操作

当用户点击机器人发送的交互式卡片上的按钮或与之交互时，适配器会将这些事件路由为合成的 `/card` 命令事件：

- 按钮点击变为：`/card button {"key": "value", ...}`
- 卡片定义中的操作 `value` 负载作为 JSON 包含在内。
- 卡片操作会进行去重，有一个 15 分钟的时间窗口，以防止重复处理。

由消息网关驱动的更新提示使用原生的飞书 `Yes` / `No` 卡片，而不是回退到纯文本回复。当 `hermes update --gateway` 需要确认时，适配器会将选定的答案记录在 Hermes 的 `.update_response` 文件中，并用已解决状态内联替换卡片。

卡片操作事件以 `MessageType.COMMAND` 类型分发，因此它们会流经正常的命令处理流水线。

**命令批准** 也是以这种方式工作的 — 当 Agent 需要运行危险命令时，它会发送一个带有"允许一次"/"会话"/"始终"/"拒绝"按钮的交互式卡片。用户点击一个按钮，卡片操作回调将批准决定传回给 Agent。
...
:::warning
如果没有完成所有三个步骤，飞书将成功 *发送* 交互式卡片（发送仅需要 `im:message:send` 权限），但点击任何按钮都会返回错误 200340。卡片看起来可以工作 — 只有当用户与之交互时，错误才会显现。
:::

## 文档评论智能回复

除了聊天，适配器还可以回复 **飞书/ Lark 文档** 上的 `@` 提及。当用户评论文档（本地文本选择或全文评论）并 @提及机器人时，Hermes 会读取文档内容以及周围的评论线程，并在该线程上内联发布 LLM 回复。

该功能由 `drive.notice.comment_add_v1` 事件驱动，处理程序会：

- 并行获取文档内容和评论时间线（全文线程 20 条消息，本地选择线程 12 条消息）。
- 运行 Agent，使用限定在该单个评论会话的 `feishu_doc` + `feishu_drive` 工具集。
- 将回复分块为 4000 个字符，并将其作为线程回复发布。
- 缓存每个文档的会话 1 小时，上限为 50 条消息，以便同一文档上的后续评论能保持上下文。

### 三层访问控制

文档评论回复是 **仅限明确授予** 的 — 没有隐式的允许所有模式。权限按以下顺序解析（每个字段，首次匹配生效）：
...
规则文件位于 `~/.hermes/feishu_comment_rules.json`（配对授权信息在 `~/.hermes/feishu_comment_pairing.json`），支持基于修改时间（mtime）缓存的热重载——编辑规则后，在下一个评论事件发生时生效，无需重启消息网关。
...
| **音频** | .ogg, .mp3, .wav, .m4a, .aac, .flac, .opus, .webm | 下载并缓存；小文本文件会自动提取内容 |
...
对于基于文本的小型文档（.txt, .md），文件内容会自动注入到消息文本中，以便 Agent 无需使用工具即可直接读取。
...
| `send_image` / `send_image_file` | 将图片上传至飞书，然后以原生图片气泡形式发送（可附带标题） |
| `send_document` | 将文件上传至飞书 API，然后作为文件附件发送 |
| `send_voice` | 将音频文件作为飞书文件附件上传 |
| `send_video` | 上传视频并以原生媒体消息形式发送 |
| `send_animation` | GIF 会降级为文件附件发送（飞书没有原生 GIF 气泡） |
...
如果飞书 API 拒绝了 post 负载（例如，由于不支持的 Markdown 结构），适配器会自动回退为发送纯文本，并去除 Markdown 格式。这种两阶段回退机制确保消息总能送达。
...
适配器会跟踪每个 IP 地址的连续错误响应。在 6 小时窗口内，同一 IP 连续出现 25 次错误后，会记录一条警告。这有助于检测配置错误的客户端或探测尝试。
...
platforms:
  feishu:
    extra:
      default_group_policy: "open"     # 未在 group_rules 中列出的群聊的默认策略
      admins:                          # 可以管理机器人设置的用户
        - "ou_admin_open_id"
      group_rules:
        "oc_group_chat_id_1":
...
在 `group_rules` 条目上设置 `require_mention: false` 可以跳过该特定聊天中的 @-提及要求。当省略时，聊天将继承全局的 `FEISHU_REQUIRE_MENTION` 值。
...
| 设置 | 环境变量 | 默认值 |
|---------|---------|---------|
| 缓存大小 | `HERMES_FEISHU_DEDUP_CACHE_SIZE` | 2048 条条目 |
...
WebSocket 和按群组 ACL 设置通过 `config.yaml` 中的 `platforms.feishu.extra` 配置（参见上面的 [WebSocket 调优](#websocket-tuning) 和 [按群组访问控制](#per-group-access-control)）。
...
| 机器人身份未自动检测 | 通常是访问飞书机器人信息端点的瞬时网络问题。作为临时解决方法，手动设置 `FEISHU_BOT_OPEN_ID` 和 `FEISHU_BOT_NAME`。 |
| 启用 `FEISHU_ALLOW_BOTS` 后，其他机器人的消息仍被忽略 | Hermes 尚无法识别自身 — 设置 `FEISHU_BOT_OPEN_ID`（如果你的应用使用 `sender_id_type=user_id`，还需设置 `FEISHU_BOT_USER_ID`）。 |
| 其他机器人显示为 `ou_xxxxxx` 而不是名称 | 授予 `application:bot.basic_info:read` 范围。 |
| 点击审批按钮时出现错误 200340 | 在飞书开发者控制台中启用**交互卡片**功能并配置**卡片请求 URL**。参见上面的 [必需的飞书应用配置](#required-feishu-app-configuration)。 |

...
机器人应该在几秒钟内上线。在 Telegram 上给它发条消息来验证。
...
在运行提及和自由响应检查之前，Telegram 主题 `31` 和 `42` 中的消息总是被忽略。
...
Telegram Bot API 9.4（2026 年 2 月）引入了**私聊主题** —— 机器人可以直接在 1 对 1 私聊中创建论坛式主题线程，无需超级群组。这让你可以在与 Hermes 的现有私聊中运行多个隔离的工作空间。

### 使用场景

如果你同时处理多个长期运行的项目，主题可以保持它们的上下文分离：
...
1. 在消息网关启动时，Hermes 为每个尚未分配 `thread_id` 的主题调用 `createForumTopic`
...
带有 `skill` 字段的主题会在该主题中启动新会话时自动加载该技能。这就像在对话开始时输入 `/技能名称` 一样 —— 技能内容被注入到第一条消息中，后续消息会在对话历史记录中看到它。

例如，一个带有 `skill: arxiv` 的主题，每当其会话重置时（由于空闲超时、每日重置或手动 `/reset`），都会预加载 arxiv 技能。
:::tip
在配置之外创建的主题（例如通过手动调用 Telegram API）会在收到 `forum_topic_created` 服务消息时自动被发现。你也可以在消息网关运行时将主题添加到配置中——它们会在下一次缓存未命中时被拾取。
:::

## 群组论坛主题技能绑定

启用**主题模式**（也称为“论坛主题”）的超级群组已经实现了每个主题的会话隔离——每个 `thread_id` 都映射到自己的对话。但你可能希望在特定群组主题中收到消息时**自动加载一个技能**，就像 DM 主题技能绑定的工作方式一样。
...
2. 如果匹配的条目有 `skill` 字段，则该技能会为会话自动加载——与 DM 主题技能绑定完全相同
...
:::tip
要查找主题的 `thread_id`，请在 Telegram Web 或桌面版中打开主题并查看 URL：`https://t.me/c/1234567890/5` — 最后一个数字 (`5`) 就是 `thread_id`。超级群组的 `chat_id` 是群组 ID 加上 `-100` 前缀（例如，群组 `1234567890` 变为 `-1001234567890`）。
:::

## 近期 Bot API 功能

- **Bot API 9.4 (2026年2月):** 私聊主题 — 机器人可以通过 `createForumTopic` 在 1 对 1 DM 聊天中创建论坛主题。请参阅上文的[私聊主题](#private-chat-topics-bot-api-94)。
- **隐私政策:** Telegram 现在要求机器人拥有隐私政策。通过 BotFather 使用 `/setprivacy_policy` 设置，或者 Telegram 可能会自动生成一个占位符。如果你的机器人是面向公众的，这一点尤其重要。
- **消息流式传输:** Bot API 9.x 增加了对长响应流式传输的支持，这可以改善冗长 Agent 回复的感知延迟。
...
2. **模型选择** — 分页的模型列表，带有 **上一页**/**下一页** 导航、一个返回提供商的 **返回** 按钮和 **取消**。
...
默认情况下，Telegram 适配器通过**长轮询**连接——消息网关向 Telegram 的服务器发起出站连接。这在任何地方都有效，但会保持一个持久连接。

**Webhook 模式**是一种替代方案，Telegram 通过 HTTPS 将更新推送到你的服务器。这对于**无服务器和云部署**（Fly.io、Railway 等）非常理想，因为入站 HTTP 可以唤醒挂起的机器。
...
当设置了 `TELEGRAM_WEBHOOK_URL` 时，消息网关会启动一个 HTTP 服务器，监听 `0.0.0.0:<端口>`，并向 Telegram 注册 webhook URL。URL 路径从 webhook URL 中提取（默认为 `/telegram`）。
...
在某些受限网络中，`api.telegram.org` 可能解析到一个无法访问的 IP。Telegram 适配器包含一个**备用 IP** 机制，可以在保留正确的 TLS 主机名和 SNI 的同时，透明地重试连接到备用 IP。
### 工作原理
...
:::tip
通常您不需要手动配置此项。通过 DoH 的自动发现功能可以处理大多数受限制网络场景。只有在您的网络也屏蔽了 DoH 时，才需要设置 `TELEGRAM_FALLBACK_IPS` 环境变量。
:::

## 代理支持

如果您的网络需要通过 HTTP 代理才能访问互联网（在企业环境中很常见），Telegram 适配器会自动读取标准的代理环境变量，并通过该代理路由所有连接。
...
:::note
与 Discord（反应是叠加的）不同，Telegram 的 Bot API 在一次调用中会替换机器人的所有反应。从 👀 到 ✅/❌ 的转换是原子性的——您不会同时看到两者。
:::
...
| 机器人令牌被撤销/无效 | 通过 BotFather 的 `/revoke` 然后 `/newbot` 或 `/token` 命令生成新令牌。更新您的 `.env` 文件。 |
...
当 Agent 尝试运行可能危险的命令时，它会在聊天中向您请求批准：
...
:::warning
始终设置 `TELEGRAM_ALLOWED_USERS` 以限制谁可以与您的机器人交互。如果不设置，作为安全措施，消息网关默认会拒绝所有用户。
:::

切勿公开分享您的机器人令牌。如果令牌泄露，请立即通过 BotFather 的 `/revoke` 命令撤销它。

更多详细信息，请参阅[安全文档](/user-guide/security)。您也可以使用[私信配对](/user-guide/messaging#dm-pairing-alternative-to-allowlists)来实现更动态的用户授权方式。
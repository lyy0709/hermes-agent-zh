...
提示时选择 **Telegram**。向导会要求您输入机器人 Token 和允许的用户 ID，然后为您写入配置。
...
如果您已经有 `docker_volumes:` 部分，请将新的挂载添加到同一列表中。YAML 中的重复键会静默覆盖较早的键。
...
此列表中的任何文件都会在支持它的平台（Telegram、Discord、Signal、Slack、WhatsApp、飞书、Matrix 等）上作为原生附件发送；在不支持原生附件的平台上，它会回退为链接或纯文本指示器。**粗体**的类别是在最近几个版本中添加的 —— 如果您之前依赖模型说 `here is the file: /path/to/report.docx`，请改用 `MEDIA:/path/to/report.docx` 以实现原生发送。
## Webhook 模式
...
| 变量 | 必需 | 描述 |
|----------|----------|-------------|
| `TELEGRAM_WEBHOOK_URL` | 是 | Telegram 将发送更新到的公共 HTTPS URL。URL 路径会自动提取（例如，从上面的示例中提取 `/telegram`）。 |
...
Telegram Bot API 9.4（2026年2月）引入了**私聊话题**——机器人可以直接在1对1的私聊中创建论坛风格的话题线程，无需超级群组。这让你可以在与 Hermes 的现有私聊中运行多个隔离的工作空间。
...
:::caution 先决条件
在将话题添加到配置之前，用户必须**在与机器人的私聊中启用话题模式**：
...
带有 `skill` 字段的话题在话题中启动新会话时会自动加载该技能。这完全等同于在对话开始时输入 `/skill-name`——技能内容被注入到第一条消息中，后续消息会在对话历史记录中看到它。
...
一种 ChatGPT 风格的多会话私聊——一个机器人，多个并行对话。与上面由操作员策划的 `extra.dm_topics` 不同，此模式是**用户驱动**的：无需配置，无需预先声明话题名称。最终用户通过 `/topic` 命令启用它，然后点击 Telegram 的 **+** 按钮创建任意数量的话题，每个话题都是一个完全独立的 Hermes 会话。
...
当用户首次运行 `/topic` 时，Hermes 会调用 `getMe` 来验证两个标志。如果任一标志未开启，Hermes 会发送 BotFather Threads 设置页面的截图并解释需要切换哪些设置——在满足先决条件之前，不会进行激活。
...
激活后，**根私信成为一个大厅**：普通提示词将被拒绝，并引导用户指向 **All Messages**。系统命令（`/status`、`/sessions`、`/usage`、`/help` 等）在根私信中仍然有效。
...
当 Hermes 为话题生成会话标题时（通过自动标题流水线，在第一次交换之后），Telegram 话题本身会被重命名以匹配——例如，"New Topic" 变为 "Database migration plan"。重命名是尽力而为的：失败会被记录，但不会中断会话。

### 在话题内使用 `/new`

重置当前话题的会话（新的会话 ID，全新的历史记录），而不影响其他话题。Hermes 会回复一个提醒，说明对于并行工作，通常需要的是创建另一个话题（通过 **All Messages**）。
...
这将当前话题绑定到现有的 Hermes 会话，而不是重新开始。适用于继续在话题模式启用之前开始的对话。限制：
...
- 话题模式的 SQLite 迁移是**选择加入的**：它在第一次 `/topic` 调用时运行，而不是在消息网关启动时。在此配置文件中用户运行 `/topic` 之前，`state.db` 保持不变
- 每个入站私信消息都会查找其 `(chat_id, thread_id)` 绑定。如果存在，查找会通过 `SessionStore.switch_session()` 将消息路由到绑定的会话，从而保持磁盘上的会话键到会话 ID 映射的一致性
- 话题内的 `/new` 会重写绑定行以指向新的会话 ID，因此下一条消息会保持在新的会话上
- 在 `extra.dm_topics` 中声明的话题**永远不会被自动重命名**——即使启用了多会话模式，操作员选择的名称也会被保留
- 启用论坛功能的私信中的 General（置顶）话题被视为根大厅，无论 Telegram 传递其消息时是使用 `message_thread_id=1` 还是没有 thread_id
- 根大厅提醒的速率限制为每聊天每 30 秒一条消息——忘记话题模式已开启并在根私信中输入十个提示词的用户不会收到十条回复
- BotFather 设置截图的速率限制为每聊天每 5 分钟发送一次——在 Threads 设置仍处于禁用状态时重复尝试 `/topic` 不会重新上传同一张图片
- 在话题内启动的 `/background <prompt>` 会将其结果发送回同一话题；后台会话不会触发所属话题的自动重命名
- `/topic` 本身受机器人的用户授权检查保护——未经授权的私信会收到拒绝而不是激活

### 禁用多会话模式

在根私信中发送 `/topic off`。Hermes 将该行关闭，清除该聊天的 `(thread_id → session_id)` 绑定，根私信恢复为正常的 Hermes 聊天。Telegram 中的现有话题不会被删除——它们只是不再被作为独立会话进行管理。稍后重新运行 `/topic` 可以重新开启。
...
如果降级到早于 `/topic` 功能的 Hermes 版本，该功能将停止工作——`telegram_dm_topic_mode` 和 `telegram_dm_topic_bindings` 表仍保留在 `state.db` 中，但会被旧代码忽略。私信恢复为原生的每话题隔离（每个 `message_thread_id` 仍然通过 `build_session_key` 获得自己的会话），因此现有的 Telegram 话题继续作为并行会话工作。根私信不再是大厅——那里的消息会像以前一样进入 Agent。重新升级会完全恢复之前的多会话模式。

## 群组论坛话题技能绑定

启用**话题模式**的超级群组（也称为"论坛话题"）已经实现了每个话题的会话隔离——每个 `thread_id` 映射到自己的对话。但您可能希望在消息到达特定群组话题时**自动加载一个技能**，就像私信话题技能绑定的工作方式一样。
### 使用场景

一个包含不同工作流论坛话题的团队超级群组：
...
2. 如果匹配的条目包含 `skill` 字段，则该技能会自动加载到会话中 — 与 DM 话题技能绑定相同
...
- **较大或较宽的表格** 会回退到带有对齐列的**围栏代码块**，这样就不会崩溃。会添加一行提示，让 Agent 知道在 Telegram 上更倾向于使用散文式后续回复，而不是更多表格。

无需配置 — 适配器会根据每条消息选择正确的回退方式。如果你想要遗留的“始终代码块”行为，可以通过在 `config.yaml` 中设置 `telegram.pretty_tables: false` 来禁用表格规范化（默认值：`true`）。
...
启用后，Hermes 会将 Telegram 的 `LinkPreviewOptions(is_disabled=True)` 附加到每条出站消息，并在较旧的 `python-telegram-bot` 版本上回退到遗留的 `disable_web_page_preview` 参数。

## 群组白名单

Telegram 群组和论坛聊天有两个正交的访问控制门，你可以配置：

- **发送者用户 ID** (`group_allow_from` / `TELEGRAM_GROUP_ALLOWED_USERS`) — 仅适用于群组/论坛消息的发送者范围白名单。当你希望特定用户能够在群组中调用机器人，但又不想将他们添加到 `TELEGRAM_ALLOWED_USERS`（这会同时授予他们 DM 访问权限）时，请使用此选项。
...
- `TELEGRAM_ALLOWED_USERS` 涵盖所有聊天类型（DM、群组、论坛）。
- `TELEGRAM_GROUP_ALLOWED_USERS` 仅授权列出的发送者在群组/论坛中访问。除非他们也在 `TELEGRAM_ALLOWED_USERS` 中列出，否则他们仍然无法通过 DM 联系机器人。
- `TELEGRAM_GROUP_ALLOWED_CHATS` 中的聊天会授权该聊天的每个成员，无论发送者是谁。
- 在任何这些变量中使用 `*` 以允许任何发送者/聊天。
- 这是在现有的提及/模式触发器以及 `group_topics` + `ignored_threads` 之上的附加层。
### 迁移自 PR #17686 之前

在此拆分之前，`TELEGRAM_GROUP_ALLOWED_USERS` 是唯一的配置项，用户在其中填入**聊天 ID**。为了向后兼容，`TELEGRAM_GROUP_ALLOWED_USERS` 中形如聊天 ID（以 `-` 开头）的值仍会被视为聊天 ID 并生效，同时会记录一次弃用警告。迁移方法：
...
1.  **提供商选择** — 显示每个可用提供商的按钮及其模型数量（例如，“OpenAI (15)”，当前提供商显示为“✓ Anthropic (12)”）。
2.  **模型选择** — 分页显示的模型列表，带有 **Prev**/**Next** 导航按钮、返回提供商列表的 **Back** 按钮以及 **Cancel** 按钮。

当前模型和提供商显示在顶部。所有导航操作都通过原地编辑同一条消息完成（不会使聊天记录杂乱）。

:::tip
如果你知道确切的模型名称，可以直接输入 `/model <名称>` 来跳过选择器。你也可以输入 `/model <名称> --global` 来跨会话持久化更改。
:::

## DNS-over-HTTPS 备用 IP

在某些受限网络中，`api.telegram.org` 可能解析到一个无法访问的 IP 地址。Telegram 适配器包含一个**备用 IP** 机制，该机制会透明地尝试使用备用 IP 进行连接重试，同时保持正确的 TLS 主机名和 SNI。
...
:::tip
通常你不需要手动配置这个。通过 DoH 的自动发现机制可以处理大多数受限网络场景。只有在你的网络上也屏蔽了 DoH 时，才需要设置 `TELEGRAM_FALLBACK_IPS` 环境变量。
:::
...
| 机器人回复“unauthorized” | 你的用户 ID 不在 `TELEGRAM_ALLOWED_USERS` 中。使用 @userinfobot 仔细核对。 |
| 机器人忽略群组消息 | 隐私模式很可能已开启。禁用它（步骤 3）或使机器人成为群组管理员。**注意：更改隐私设置后，记得移除并重新添加机器人。** |
...
| Webhook 未接收更新 | 验证 `TELEGRAM_WEBHOOK_URL` 是否可公开访问（使用 `curl` 测试）。确保你的平台/反向代理将来自该 URL 端口的入站 HTTPS 流量路由到由 `TELEGRAM_WEBHOOK_PORT` 配置的本地监听端口（它们不需要是相同的数字）。确保 SSL/TLS 已激活——Telegram 只向 HTTPS URL 发送数据。检查防火墙规则。 |
## 执行批准

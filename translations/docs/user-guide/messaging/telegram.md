...
对于**云部署**（Fly.io、Railway、Render 等），**Webhook 模式**更具成本效益。这些平台可以在收到入站 HTTP 流量时自动唤醒挂起的机器，但不会因出站连接而唤醒。由于轮询是出站的，轮询机器人永远无法休眠。Webhook 模式翻转了方向 —— Telegram 将更新推送到您机器人的 HTTPS URL，从而实现空闲时休眠的部署。
...
带有 `skill` 字段的话题会在该话题中启动新会话时自动加载该技能。这就像在对话开始时输入 `/技能名称` 一样 —— 技能内容被注入到第一条消息中，后续消息会在对话历史记录中看到它。
...
启用**话题模式**的超级群组（也称为“论坛话题”）已经实现了每个话题的会话隔离——每个 `thread_id` 对应其独立的对话。但你可能希望**在特定群组话题中收到消息时自动加载一个技能**，就像私聊话题技能绑定的工作方式一样。
...
2. **模型选择** — 分页显示的模型列表，带有 **上一页**/**下一页** 导航、一个返回提供商的 **返回** 按钮和 **取消** 按钮。
...
在某些受限网络中，`api.telegram.org` 可能解析到一个无法访问的 IP。Telegram 适配器包含一个**备用 IP** 机制，可以透明地重试连接到备用 IP，同时保留正确的 TLS 主机名和 SNI。
...
| 机器人 Token 被撤销/无效 | 通过 BotFather 的 `/revoke` 然后 `/newbot` 或 `/token` 命令生成新 Token。更新你的 `.env` 文件。 |

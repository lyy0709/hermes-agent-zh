...
提示时选择 **Telegram**。向导会询问您的机器人 Token 和允许的用户 ID，然后为您写入配置。
...
当设置了 `TELEGRAM_WEBHOOK_URL` 时，消息网关会启动一个 HTTP webhook 服务器，而不是轮询。当未设置时，则使用轮询模式 —— 与之前版本的行为没有变化。
...
在任何 Telegram 聊天（私聊或群组）中使用 `/sethome` 命令，将其指定为**主频道**。定时任务会将它们的结果发送到此频道。
...
Telegram Bot API 9.4（2026 年 2 月）引入了**私聊主题** —— 机器人可以直接在 1 对 1 私聊中创建论坛式主题线程，无需超级群组。这让你可以在与 Hermes 的现有私聊中运行多个独立的工作空间。
...
在 `~/.hermes/config.yaml` 文件的 `platforms.telegram.extra.dm_topics` 下添加主题：
...
4. 每个主题中的消息都有独立的对话历史、记忆刷新和上下文窗口。

### 技能绑定

带有 `skill` 字段的主题，在该主题中启动新会话时会自动加载该技能。这完全等同于在对话开始时输入 `/skill-name` — 技能内容会被注入到第一条消息中，后续消息会在对话历史中看到它。
...
启用了**主题模式**（也称为“论坛主题”）的超级群组已经实现了每个主题的会话隔离 — 每个 `thread_id` 都映射到自己的对话。但你可能希望在特定群组主题中收到消息时**自动加载一个技能**，就像 DM 主题技能绑定一样。
...
2. 如果匹配的条目有 `skill` 字段，则该技能会自动加载到会话中 — 与 DM 主题技能绑定完全相同。
...
- **Bot API 9.4 (2026年2月):** 私聊主题 — 机器人可以通过 `createForumTopic` 在 1 对 1 DM 聊天中创建论坛主题。参见上文的[私聊主题](#private-chat-topics-bot-api-94)。
- **隐私政策:** Telegram 现在要求机器人拥有隐私政策。通过 BotFather 使用 `/setprivacy_policy` 设置，否则 Telegram 可能会自动生成一个占位符。如果你的机器人是面向公众的，这一点尤其重要。

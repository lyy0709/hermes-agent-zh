...
:::warning
请保管好您的机器人 Token。任何拥有此 Token 的人都可以控制您的机器人。如果泄露，请立即通过 BotFather 中的 `/revoke` 命令撤销它。
:::
...
:::tip
禁用隐私模式的替代方案：将机器人提升为**群组管理员**。管理员机器人无论隐私设置如何，始终会接收所有消息，这样可以避免需要切换全局隐私模式。
:::
...
机器人应该在几秒钟内上线。在 Telegram 上给它发送一条消息进行验证。
...
此列表中的任何文件在支持它的平台（Telegram、Discord、Signal、Slack、WhatsApp、Feishu、Matrix 等）上都会作为原生附件发送；在不支持原生附件的平台上，它会回退到链接或纯文本指示器。**粗体**的类别是在最近几个版本中添加的 —— 如果您之前依赖模型说 `here is the file: /path/to/report.docx`，请改用 `MEDIA:/path/to/report.docx` 以实现原生发送。
## Webhook 模式

默认情况下，Hermes 使用**长轮询**连接到 Telegram —— 消息网关向 Telegram 的服务器发起出站请求以获取新更新。这对于本地和持续运行的部署效果很好。

对于**云部署**（Fly.io、Railway、Render 等），**Webhook 模式**更具成本效益。这些平台可以在收到入站 HTTP 流量时自动唤醒挂起的机器，但不会因出站连接而唤醒。由于轮询是出站的，轮询模式的机器人永远无法休眠。Webhook 模式反转了方向 —— Telegram 将更新推送到你机器人的 HTTPS URL，从而支持空闲时休眠的部署。
...
如果你在机器人私聊中启用了话题模式，投递到根聊天的定时任务消息会落在仅限系统的“大厅”中 —— 在那里回复不会打开会话，并且你会看到“主聊天仅用于系统命令”的通知。创建一个专门的论坛话题（例如 `Cron`）并设置：

```bash
TELEGRAM_CRON_THREAD_ID=<话题线程ID>
```

`TELEGRAM_CRON_THREAD_ID` 仅针对定时任务投递覆盖 `TELEGRAM_HOME_CHANNEL_THREAD_ID`。在该话题中的回复会继续该话题现有的会话。
...
如果你更希望**Agent 本身**处理音频 —— 用于说话人分离、自定义转录工具，或者仅仅是存档录音 —— 请在 `~/.hermes/config.yaml` 中设置 `stt.enabled: false`：
...
然后，你的工具或技能可以直接读取该路径（例如，将其交给本地说话人分离流水线、更丰富的转录模型，或上传到长期存储）。文件扩展名反映了 Telegram 交付的原始格式（语音笔记为 `.ogg`，音频附件为 `.mp3`/`.m4a`/等）。
...
如果没有安装 ffmpeg，Edge TTS 音频会作为常规音频文件发送（仍然可以播放，但会使用矩形播放器而不是语音气泡）。

在 `config.yaml` 文件的 `tts.provider` 键下配置 TTS 提供商。

## 通过本地 Bot API 服务器处理大文件（>20MB）

Telegram 的**公共** Bot API 将 `getFile` 下载限制在 **20 MB**，因此任何大于此限制的语音笔记、音频文件、视频或文档都会被 Hermes 静默拒绝，并回复“文件过大”。解决此问题的文档化方法是运行一个**本地** [telegram-bot-api](https://github.com/tdlib/telegram-bot-api) 守护进程——这是 Telegram 使用的相同服务器软件，但在你的网络上运行。本地服务器将文件上限提高到 **2 GB**，并且当 Hermes 检测到配置了自定义 `base_url` 时，会自动解除其内部限制。
...
platforms:
  telegram:
    extra:
      ...
      local_mode: true        # 参见下面的步骤 5 —— 仅当机器人数据目录
...
- 在“文件过大”错误消息中报告活动限制（`Maximum: 2048 MB.`），以便清楚知道你处于哪种模式
...
1. **不使用 `--local`**（默认）：文件通过 HTTP 在 `/file/bot<TOKEN>/<path>` 提供，与公共 Bot API 相同。20MB 上限仍然有效。仅适用于网络修复（例如当 `api.telegram.org` 无法访问但你可以自托管）；这不是你为了提升大小限制所需要的。
2. **使用 `--local`**（通过上面的 `TELEGRAM_LOCAL=1` 设置）：文件被写入服务器的文件系统，并且 `getFile` 响应返回一个**绝对路径**而不是 HTTP URL。20MB 上限被解除。然后 Hermes 必须**从磁盘**读取字节，而不是通过 HTTP。

要使磁盘读取路径正常工作，请在上面的配置中设置 `local_mode: true`，**并**确保 Hermes 进程可以读取服务器返回的路径。有两种场景：
...
- **不同的机器**——机器人服务器运行在一台主机上（例如 NAS、单独的 VM），而 Hermes 运行在另一台上。服务器的数据目录必须与 Hermes 机器共享，且共享路径必须是服务器报告的**相同绝对路径**（通常是 `/var/lib/telegram-bot-api`）。NFS 对此效果很好；如果你不想在文件系统级别处理 uid 不匹配，使用带有 `uid=` 挂载重新映射的 CIFS/SMB 会更友好。
如果设置了 `local_mode: true` 但 Hermes 无法 `stat` 返回的文件路径（权限问题或挂载错误），python-telegram-bot 会静默回退到对本地服务器进行 HTTP `getFile` 请求——在 `--local` 模式下，服务器会响应 `404 Not Found`。该问题的症状在 `gateway.log` 中显示为：
...
如果你看到这个错误，说明容量限制解除功能正常，但文件共享功能异常。请以运行消息网关的用户身份，从 Hermes 主机上验证 `ls -la /var/lib/telegram-bot-api/<TOKEN>/voice/`，并确认单个文件可以通过 `cat` 命令读取且没有权限错误。

### 步骤 6：测试

向机器人发送一个大于 20 MB 的语音消息或音频文件。跟踪消息网关日志：

```bash
tail -f ~/.hermes/logs/gateway.log | grep -iE "telegram|cache"
```

你应该能看到一行 `[Telegram] Cached user voice at /home/<user>/.hermes/cache/audio/...` 的记录，并且**没有**“文件过大”的拒绝信息。结合 `stt.enabled: false`（上文设置），原始音频文件的路径随后会进入 Agent 的入站消息中，供下游处理。
...
- 当 `telegram.require_mention: true` 时，群组消息在以下情况下会被接受：
  - 回复机器人消息
  - 使用 `@机器人用户名` 提及
  - 使用 `/命令@机器人用户名`（Telegram 包含机器人名称的机器人菜单命令形式）
  - 匹配 `telegram.mention_patterns` 中配置的正则表达式唤醒词
- 在包含多个 Hermes 机器人的群组中，`telegram.exclusive_bot_mentions` 可以保持路由确定性。当一条消息明确提及一个或多个 Telegram 机器人用户名时，只有被提及的机器人配置文件会处理它；其他 Hermes 机器人在回复和唤醒词回退逻辑运行前会忽略它。此功能默认启用。
...
如果你在同一个 Telegram 群组中运行多个 Hermes 配置文件，请为每个配置文件创建一个 Telegram 机器人 Token，并为每个配置文件启动一个消息网关。不要在多个运行中的消息网关中重复使用同一个机器人 Token；Telegram 会拒绝对同一 Token 的并发轮询。
...
在这种设置下，像 `@research_bot @ops_bot summarize this` 这样的群组消息只会被 `research_bot` 和 `ops_bot` 处理。群组中的其他 Hermes 机器人将保持静默，即使该消息是回复它们之前的消息，或者在其他情况下匹配共享的唤醒词。

仅在需要保留旧版行为（即显式提及不应覆盖回复和唤醒词触发器）的遗留群组中，才设置 `exclusive_bot_mentions: false`。
...
4.  **提及过滤器：** 如果设置了 `telegram.require_mention: true`，普通的群聊消息会被忽略，除非该消息是斜杠命令、回复机器人、`@机器人用户名` 提及，或者匹配配置的 `mention_patterns`。
5.  **多机器人路由：** 如果群组包含多个机器人，请确保每个 Hermes 配置文件使用唯一的机器人 Token，并保持 `exclusive_bot_mentions` 启用，除非你特意需要旧版的共享触发行为。
...
在提及和自由响应检查运行之前，Telegram 话题 `31` 和 `42` 中的消息始终被忽略。
...
Telegram Bot API 9.4（2026年2月）引入了**私聊话题**——机器人可以直接在1对1私聊中创建论坛式的话题线程，无需超级群组。这让你可以在与 Hermes 的现有私聊中运行多个独立的工作空间。
...
如果没有启用，Hermes 会在启动时记录 `The chat is not a forum` 并跳过话题创建。这是 Telegram 客户端的设置——机器人无法通过编程方式启用。
:::
...
默认情况下，发送到根私聊（在任何话题之外）的消息会被正常处理。设置 `ignore_root_dm: true` 可将根私聊变为一个大厅——对于配置了私聊话题的用户，普通消息会被静默忽略，而系统命令（`/start`、`/help`、`/status` 等）仍然有效。
...
此检查是**按聊天进行的**：只有在 `dm_topics` 中至少有一条记录的用户，其根私聊才会受到影响。未配置话题的用户不受影响。

### 技能绑定

带有 `skill` 字段的话题，在话题中启动新会话时会自动加载该技能。这就像在对话开始时输入 `/skill-name` 一样——技能内容被注入到第一条消息中，后续消息会在对话历史中看到它。
...
一种 ChatGPT 风格的多会话私聊模式——一个机器人，多个并行对话。与上面由操作员配置的 `extra.dm_topics` 不同，此模式是**用户驱动**的：无需配置，无需预先声明话题名称。最终用户通过 `/topic` 命令启用它，然后点击 Telegram 的 **+** 按钮创建任意数量的话题，每个话题都是一个完全独立的 Hermes 会话。
...
| `/topic` | 根私聊，已启用 | 显示状态：可恢复的未绑定会话 |
| `/topic` | 在话题内部 | 显示当前话题的会话绑定信息 |
...
2. **不要**禁止用户创建话题（保持 `allows_users_to_create_topics` 为开启状态）

当用户首次运行 `/topic` 时，Hermes 会调用 `getMe` 来验证这两个标志。如果其中任何一个未开启，Hermes 会发送 BotFather 话题设置页面的截图并解释需要切换哪些设置——在满足前提条件之前，不会进行激活。
...
每个话题都有自己的对话历史、模型状态、工具执行和会话 ID。隔离键为 `agent:main:telegram:dm:{chat_id}:{thread_id}`——与配置驱动的私信话题隔离相同。

### 自动重命名话题

当 Hermes 为话题生成会话标题时（通过自动标题流水线，在第一次交互之后），Telegram 话题本身会被重命名以匹配——例如，“新话题”变为“数据库迁移计划”。重命名是尽力而为的：失败会被记录，但不会中断会话。
...
当此标志开启时，Hermes 仍然会生成内部会话标题（供 `hermes sessions`、TUI 等使用），但从不编辑 Telegram 话题名称。当您在 BotFather 话题模式下手动组织话题，并且不希望每次首次回复都覆盖标题时，这很有用。

### 在话题内使用 `/new`

重置当前话题的会话（新的会话 ID，全新的历史记录），而不影响其他话题。Hermes 会回复一个提醒，说明对于并行工作，通常您需要的是创建另一个话题（通过**所有消息**）。
...
这将当前话题绑定到现有的 Hermes 会话，而不是重新开始。对于继续在话题模式启用之前开始的对话很有用。限制条件：
...
显示当前话题的绑定信息：会话标题、会话 ID，以及关于 `/new` 与创建另一个话题的提示。

### 底层原理

- 激活状态持久化到 `state.db` 中的 `telegram_dm_topic_mode(chat_id, user_id, enabled, ...)`
- 每个话题绑定持久化到 `telegram_dm_topic_bindings(chat_id, thread_id, session_id, ...)`，并在 `session_id` 上设置 `ON DELETE CASCADE`——清理会话会自动清除其话题绑定
- 话题模式的 SQLite 迁移是**选择加入的**：它在第一次调用 `/topic` 时运行，从不在消息网关启动时运行。在此配置文件中用户运行 `/topic` 之前，`state.db` 保持不变
- 每个入站私信消息都会查找其 `(chat_id, thread_id)` 绑定。如果存在，查找会通过 `SessionStore.switch_session()` 将消息路由到绑定的会话，从而保持磁盘上的会话键到会话 ID 映射的一致性
- 在话题内使用 `/new` 会重写绑定行，使其指向新的会话 ID，因此下一条消息会保持在新的会话上
- 在 `extra.dm_topics` 中声明的话题**永远不会被自动重命名**——即使在启用多会话模式时，操作员选择的名称也会被保留
- 设置 `extra.disable_topic_auto_rename: true` 以关闭聊天中**所有**话题的自动重命名（包括通过话题模式创建的临时话题）
- 在启用论坛功能的私信中，通用（置顶）话题被视为根大厅，无论 Telegram 传递其消息时是使用 `message_thread_id=1` 还是没有 thread_id
- 根大厅提醒的频率限制为每聊天每 30 秒一条消息——忘记话题模式已开启并在根私信中键入十个提示词的用户不会收到十条回复
- BotFather 设置截图的频率限制为每聊天每 5 分钟发送一次——在话题设置仍被禁用时重复尝试 `/topic` 不会重新上传相同的图片
- 在话题内启动的 `/background <prompt>` 会将其结果传递回同一话题；后台会话不会触发所属话题的自动重命名
- `/topic` 本身受机器人用户授权检查的保护——未经授权的私信会收到拒绝信息，而不是激活

### 禁用多会话模式

在根私信中发送 `/topic off`。Hermes 将该行关闭，清除聊天的 `(thread_id → session_id)` 绑定，并且根私信恢复为正常的 Hermes 聊天。Telegram 中的现有话题不会被删除——它们只是不再被作为独立会话进行管理。稍后重新运行 `/topic` 可以重新开启。
...
如果您降级到早于 `/topic` 功能的 Hermes 版本，该功能将停止工作——`telegram_dm_topic_mode` 和 `telegram_dm_topic_bindings` 表仍保留在 `state.db` 中，但会被旧代码忽略。私信恢复为原生的每线程隔离（每个 `message_thread_id` 仍然通过 `build_session_key` 获得自己的会话），因此您现有的 Telegram 话题继续作为并行会话工作。根私信不再是大厅——那里的消息会像以前一样进入 Agent。重新升级会完全恢复之前的多会话模式。
## 群组论坛话题技能绑定

已启用**话题模式**的超级群组（也称为“论坛话题”）已经实现了按话题的会话隔离——每个 `thread_id` 对应其独立的对话。但你可能希望**在特定群组话题中收到消息时自动加载一个技能**，就像私信话题技能绑定的工作方式一样。
...
:::tip
要查找话题的 `thread_id`，请在 Telegram Web 或桌面版中打开话题并查看 URL：`https://t.me/c/1234567890/5` —— 最后一个数字 (`5`) 就是 `thread_id`。超级群组的 `chat_id` 是群组 ID 加上 `-100` 前缀（例如，群组 `1234567890` 变为 `-1001234567890`）。
:::
...
| 值 | 行为 |
|---|---|
| `auto` | 在支持的聊天（当前为 Telegram 私信）中使用原生草稿流式传输；否则使用传统的基于编辑的路径。如果草稿帧失败，会优雅地回退。 |
| `draft` | 强制使用原生草稿。如果聊天不支持草稿（例如群组/话题），则记录降级并回退到编辑方式。 |
| `edit` (默认) | 为所有聊天类型使用传统的渐进式 `editMessageText` 轮询。 |
...
**在私信中使用 `edit`（默认）你会看到什么** —— 消息网关发送一条普通的预览消息，并通过 `editMessageText` 逐步更新它，避免 Telegram 草稿预览的折叠/回滚效果。

**在私信中使用 `auto` 或 `draft` 你会看到什么** —— Telegram 显示一个逐 Token 更新的动画草稿预览。当回复完成时，它会作为一条常规消息发送，并且草稿预览在客户端上自然清除。草稿没有消息 ID，所以最终答案才是留在聊天记录中的内容。

**那群组、超级群组、论坛话题呢？** Telegram 将 `sendMessageDraft` 限制在私聊（私信）中。消息网关会透明地回退到基于编辑的路径来处理其他所有情况——用户体验与之前相同。

**如果草稿帧失败怎么办？** 任何失败（临时网络错误、服务器端拒绝、较旧的 python-telegram-bot 安装）都会将该响应的其余部分切换回基于编辑的路径。下一次响应会重新尝试。
...
- **较大或较宽的表格** 回退到带有对齐列的**围栏代码块**，这样就不会折叠。会添加一行提示词提示，以便 Agent 知道在 Telegram 上更倾向于使用散文后续回复，而不是更多表格。
无需配置——适配器会根据每条消息自动选择合适的回退方案。如果你想要传统的“始终使用代码块”行为，可以在 `config.yaml` 中设置 `telegram.pretty_tables: false` 来禁用表格标准化（默认值：`true`）。
...
- **发送者用户 ID** (`group_allow_from` / `TELEGRAM_GROUP_ALLOWED_USERS`) —— 仅适用于群组/论坛消息的发送者范围白名单。当你想让特定用户能够在群组中调用机器人，但又不想将他们添加到 `TELEGRAM_ALLOWED_USERS`（这也会授予他们私聊访问权限）时，请使用此选项。
- **聊天 ID** (`group_allowed_chats` / `TELEGRAM_GROUP_ALLOWED_CHATS`) —— 聊天范围的白名单。这些群组/论坛的任何成员都可以与机器人交互。适用于团队/支持机器人，其中群组成员身份本身就是访问信号。
...
在此拆分之前，`TELEGRAM_GROUP_ALLOWED_USERS` 是唯一的控制开关，用户在其中放置的是**聊天 ID**。为了向后兼容，`TELEGRAM_GROUP_ALLOWED_USERS` 中形状为聊天 ID（以 `-` 开头）的值仍会被视为聊天 ID，并且会记录一次弃用警告。迁移方法：
...
在典型设置中，`group_allowed_chats` 是一个硬性门控：来自列表之外群组的消息会被静默丢弃，即使有成员明确 @提及机器人。这对于支持/团队机器人来说是合适的默认行为。
  ...
  platforms:
    telegram:
      extra:
        ...
        guest_mode: true       # 非白名单群组：仅在 @提及 时允许
```
...
当 `guest_mode: true` 时，来自非白名单群组的消息**仅**在明确 @提及机器人时才会被处理。每次交互都需要提及 —— 访客交互没有会话粘性，因此机器人永远不会在没有被 ping 的情况下自动参与朋友群聊的线程。

私聊和白名单群组的行为与之前完全相同。

## 斜杠命令访问控制

默认情况下，每个被允许的用户都可以运行每个斜杠命令。要将你的白名单拆分为**管理员**（完全斜杠命令访问权限）和**普通用户**（仅你明确启用的命令），请在平台的 `extra` 块中添加 `allow_admin_from` 和 `user_allowed_commands`：
...
- 普通聊天（非斜杠命令消息）不受影响。非管理员用户仍然可以正常与 Agent 交谈，只是不能触发任意命令。
...
使用 `/whoami` 查看当前活动范围、你的层级（管理员 / 用户 / 无限制）以及你可以运行哪些斜杠命令。
## 交互式模型选择器
...
当前模型和提供商显示在顶部。所有导航都通过原地编辑同一条消息完成（不会使聊天记录杂乱）。
...
:::tip
通常你不需要手动配置这个。通过 DoH 的自动发现机制可以处理大多数受限网络场景。只有在你的网络上也阻止了 DoH 时，才需要设置 `TELEGRAM_FALLBACK_IPS` 环境变量。
:::
...
| 机器人忽略群组消息 | 隐私模式可能已开启。禁用它（步骤 3）或使机器人成为群组管理员。**更改隐私设置后，记得移除并重新添加机器人。** |
...
通过 `~/.hermes/config.yaml` 中的 `agent.clarify_timeout` 配置响应超时（默认为 `600` 秒）。如果你在超时时间内没有响应，Agent 会解除阻塞并发送一个哨兵消息，然后进行调整，而不是一直等待。
...
| `all` | 每条外发消息都会触发推送通知。这是旧版行为；如果你确实希望收到每个工具调用的通知，可以选择此模式。 |
...
:::warning
始终设置 `TELEGRAM_ALLOWED_USERS` 以限制谁可以与你的机器人交互。如果不设置，作为安全措施，消息网关默认会拒绝所有用户。
:::

切勿公开分享你的机器人令牌。如果令牌泄露，请立即通过 BotFather 的 `/revoke` 命令撤销它。

更多详细信息，请参阅[安全文档](/user-guide/security)。你也可以使用[DM 配对](/user-guide/messaging#dm-pairing-alternative-to-allowlists)来实现更动态的用户授权方法。
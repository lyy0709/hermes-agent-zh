...
:::info 仅限消息网关适配器：无外部依赖
本页介绍电子邮件消息网关适配器，它使用 Python 内置的 `imaplib`、`smtplib` 和 `email` 模块。此网关路径不需要额外的包或外部服务。
:::

这与捆绑的 [Himalaya 电子邮件技能](/docs/user-guide/skills/bundled/email/email-himalaya) 是分开的，后者允许 Agent 通过终端命令管理电子邮件，但需要外部的 `himalaya` CLI 以及 Himalaya 配置文件。

| 使用场景 | 需要配置什么 | 外部依赖 |
|---|---|---|
| 让人们向 Hermes Agent 发送电子邮件并接收回复 | 本页的电子邮件消息网关适配器 | 除了 IMAP/SMTP 电子邮件账户外，无其他依赖 |
| 让 Agent 通过终端工具检查、撰写、移动和管理邮箱消息 | Himalaya 电子邮件技能 | `himalaya` CLI 和 `~/.config/himalaya/config.toml` |

---

## 先决条件

- 为你的 Hermes Agent 准备一个**专用的电子邮件账户**（不要使用你的个人电子邮件）
- 在电子邮件账户上**启用 IMAP**
...
- **主题行** 会作为上下文包含（例如，`[Subject: Deploy to production]`）
- **回复邮件**（主题以 `Re:` 开头）会跳过主题前缀——线程上下文已建立
- **附件** 会缓存在本地：
  - 图像（JPEG、PNG、GIF、WebP）→ 可供视觉工具使用
  - 文档（PDF、ZIP 等）→ 可供文件访问使用
- **纯 HTML 邮件** 会被剥离标签以提取纯文本
- **自己发送的消息** 会被过滤掉，以防止回复循环
- **自动/无回复发件人** 会被静默忽略——包括 `noreply@`、`mailer-daemon@`、`bounce@`、`no-reply@`，以及带有 `Auto-Submitted`、`Precedence: bulk` 或 `List-Unsubscribe` 标头的电子邮件

### 发送回复

回复通过 SMTP 发送，并保持正确的电子邮件线程：

- **In-Reply-To** 和 **References** 标头用于维护线程
...
Agent 可以在回复中发送文件附件。在响应中包含 `MEDIA:/path/to/file`，该文件就会附加到外发邮件中。
...
启用后，附件和内联部分会在有效负载解码前被跳过。电子邮件正文文本仍会正常处理。
...
2.  **未设置允许列表** → 未知发件人会收到配对码
3.  **`EMAIL_ALLOW_ALL_USERS=true`** → 接受任何发件人（谨慎使用）

:::warning
**务必配置 `EMAIL_ALLOWED_USERS`。** 如果不配置，任何知道 Agent 电子邮件地址的人都可能发送命令。默认情况下，Agent 拥有终端访问权限。
:::
...
| **"Authentication failed"** | 对于 Gmail，必须使用应用专用密码，而不是常规密码。确保首先启用了双重认证。 |
| **重复回复** | 确保只有一个消息网关实例在运行。检查 `hermes gateway status`。 |
| **响应缓慢** | 默认轮询间隔为 15 秒。使用 `EMAIL_POLL_INTERVAL=5` 可减少间隔以获得更快的响应（但会增加 IMAP 连接数）。 |
| **回复未形成线程** | 适配器使用 In-Reply-To 标头。某些电子邮件客户端（尤其是基于网页的）可能无法正确与自动消息形成线程。 |
...
:::warning
**使用专用的电子邮件账户。** 不要使用你的个人电子邮件——Agent 会将密码存储在 `.env` 中，并通过 IMAP 拥有完整的收件箱访问权限。
:::

- 使用**应用专用密码**而不是你的主密码（对于启用了双重认证的 Gmail 是必需的）

...
如果你的机器人显示在线但从不回复消息，**消息内容意图**几乎肯定是禁用的。回到 [开发者门户](https://discord.com/developers/applications)，选择你的应用 → Bot → Privileged Gateway Intents，确保**消息内容意图**切换为 ON。点击 **Save Changes**。
:::
...
提示时选择 **Discord**，然后在询问时粘贴你的机器人 Token 和用户 ID。
...
| `DISCORD_COMMAND_SYNC_POLICY` | 否 | `"safe"` | 控制原生斜杠命令的启动同步。`"safe"` 会对比现有的全局命令，仅更新已更改的部分，当 Discord 元数据变更无法通过补丁应用时重新创建命令。`"bulk"` 保留旧的 `tree.sync()` 行为。`"off"` 完全跳过启动同步。 |
| `DISCORD_REQUIRE_MENTION` | 否 | `true` | 当为 `true` 时，Bot 仅在服务器频道中被 `@提及` 时才响应。设置为 `false` 以在每个频道中响应所有消息。 |
| `DISCORD_THREAD_REQUIRE_MENTION` | 否 | `false` | 当为 `true` 时，禁用线程内的提及快捷方式 —— 线程与频道采用相同的门控方式，即使在 Bot 已参与后也需要 `@提及`。当多个 Bot 共享一个线程且你希望每个 Bot 仅在显式 `@提及` 时触发时使用此选项。 |
...
| `DISCORD_ALLOW_MENTION_EVERYONE` | 否 | `false` | 当为 `false`（默认）时，即使 Bot 的响应包含这些标记，也无法提及 `@everyone` 或 `@here`。设置为 `true` 以重新启用。请参阅下面的[提及控制](#mention-control)。 |
...
启用后，机器人在服务器频道中仅在直接 `@提及` 时才会响应。无论此设置如何，私信始终会得到响应。

#### `discord.thread_require_mention`

**类型:** boolean — **默认值:** `false`

默认情况下，一旦机器人参与了一个主题（在 `@提及` 时自动创建或回复过一次），它就会继续响应该主题中后续的每一条消息，而无需再次被 `@提及`。这对于一对一对话来说是合适的默认行为。

在**多机器人主题**中，用户每轮只与一个机器人对话，这个默认设置就会成为一个隐患——主题中的其他机器人也会对每条消息做出反应，消耗额度并刷屏频道。将 `thread_require_mention` 设置为 `true` 可以禁用主题内的快捷方式，并以与频道相同的方式控制主题。显式的 `@提及` 仍像以前一样有效。
...
频道 ID 列表，在此类频道中机器人无需 `@提及` 即可响应所有消息。接受逗号分隔的字符串或 YAML 列表：
...
启用后，常规文本频道中的每次 `@提及` 都会自动为该对话创建一个新主题。这可以保持主频道整洁，并为每个对话提供独立的会话历史记录。一旦主题创建，该主题中的后续消息就不需要 `@提及`——机器人知道它已经参与其中。对于多机器人设置，请将 [`thread_require_mention`](#discordthread_require_mention) 设置为 `true` 以禁用此主题内快捷方式。
...
频道 ID 列表，在此类频道中机器人**永不**响应，即使被直接 `@提及`。此设置具有最高优先级——如果一个频道在此列表中，机器人会静默忽略那里的所有消息，无论 `require_mention`、`free_response_channels` 或任何其他设置如何。
...
频道 ID 列表，在此类频道中机器人直接在频道内响应，而不是自动创建主题。这仅在 `auto_thread` 为 `true`（默认值）时有效。在这些频道中，机器人像普通消息一样内联回复，而不是生成新主题。
...
每个频道的临时系统提示词，在匹配的 Discord 频道或主题中的每一轮对话中注入，但不会持久化到对话历史记录中。
...
行为：
- 精确的主题/频道 ID 匹配优先。
- 如果消息到达主题或论坛帖子内部，并且该主题没有明确的条目，Hermes 会回退到父频道/论坛 ID。
...
当为 `true` 时：Alice 和 Bob 在 `#research` 中聊天时，各自与 Hermes 拥有独立的对话。当为 `false` 时：整个频道共享一个对话记录和一个运行中的 Agent 槽位。

```yaml
group_sessions_per_user: true
```

有关每种模式的完整影响，请参阅上方的[会话模型](#session-model-in-discord)部分。
...
启用后，会在消息网关中提供 `/verbose` 斜杠命令，让你无需编辑 config.yaml 即可循环切换工具进度模式（`关闭 → 新任务 → 全部 → 详细 → 关闭`）。
...
默认情况下，每个允许的用户都可以运行每个斜杠命令。若要将你的允许列表拆分为**管理员**（拥有完整的斜杠命令访问权限）和**普通用户**（仅能运行你明确启用的命令），请在 Discord 平台的 `extra` 块中添加 `allow_admin_from` 和 `user_allowed_commands`：
...
- 在某个作用域（私信或服务器频道）中，位于 `allow_admin_from` 列表中的用户可以运行**所有**已注册的斜杠命令——包括内置命令和插件注册的命令——通过实时命令注册表。
- 不在 `allow_admin_from` 列表中的用户只能运行 `user_allowed_commands` 中列出的命令，以及始终允许的基础命令：`/help` 和 `/whoami`。
- 普通聊天（非斜杠消息）不受影响。非管理员用户仍可正常与 Agent 对话；他们只是不能触发任意命令。
...
选择器在 120 秒后超时。只有授权用户（位于 `DISCORD_ALLOWED_USERS` 中的用户）可以与之交互。如果你知道模型名称，可以直接输入 `/model <名称>`。
...
如果你针对同一个 Discord 应用运行多个 Hermes 网关（例如，测试环境和生产环境），则其中只有一个应拥有全局斜杠命令注册权——否则最后启动的将获胜，导致注册信息来回变动。请在“跟随者”网关上关闭斜杠命令注册：
...
在“主”网关上保持此设置为 `true` 可保持正常行为——为内置命令和已安装技能提供全局 `/` 菜单命令。
...
你可以指定一个“主频道”，让机器人发送主动消息（例如定时任务输出、提醒和通知）。有两种设置方式：

### 使用斜杠命令

在任何机器人所在的 Discord 频道中输入 `/sethome`。该频道将成为主频道。
...
- **主题帖名称** 源自消息的第一行（去除 Markdown 标题前缀，最多 100 个字符）。当消息仅包含附件时，将使用文件名作为备用的主题帖名称。
- **附件** 会随新主题帖的起始消息一起发送——无需单独的上传步骤，也不会出现部分发送的情况。
- **一次调用，一个主题帖**：每次向论坛发送消息都会创建一个新的主题帖。因此，连续向同一论坛发送消息将产生不同的主题帖。
- **检测分为三层**：首先是频道目录缓存，其次是进程本地探测缓存，最后是实时的 `GET /channels/{id}` 探测（其结果会在进程生命周期内被记忆）。

刷新目录（在支持该功能的平台上使用 `/channels refresh` 命令，或重启消息网关）会将机器人启动后创建的任何论坛频道填充到缓存中。
...
:::warning
务必设置 `DISCORD_ALLOWED_USERS`（或 `DISCORD_ALLOWED_ROLES`）以限制可以与机器人交互的人员。如果两者都未设置，作为安全措施，消息网关默认会拒绝所有用户。只授权你信任的人——被授权的用户拥有对 Agent 能力的完全访问权限，包括工具使用和系统访问。
:::

### 基于角色的访问控制

对于通过角色而非个人用户列表来管理访问权限的服务器（例如，版主团队、支持人员、内部工具），请使用 `DISCORD_ALLOWED_ROLES`——这是一个逗号分隔的角色 ID 列表。拥有其中任何一个角色的成员都将被授权。
...
当版主团队人员变动时，这是首选模式——新成员在获得角色的那一刻即可获得访问权限，无需编辑 `.env` 文件或重启消息网关。

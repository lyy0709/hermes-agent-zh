...
| **自由回复频道** | 你可以使用 `DISCORD_FREE_RESPONSE_CHANNELS` 将特定频道设置为无需提及，或使用 `DISCORD_REQUIRE_MENTION=false` 全局禁用提及。在这些频道中的消息会被内联回复——自动创建线程的功能被跳过，以保持频道为轻量级聊天。 |
| **线程** | Hermes 在同一线程内回复。除非该线程或其父频道被配置为自由回复，否则提及规则仍然适用。线程的会话历史与父频道隔离。 |
| **多用户共享频道** | 默认情况下，出于安全和清晰度考虑，Hermes 会在频道内为每个用户隔离会话历史。两个人在同一频道中交谈不会共享一个对话记录，除非你明确禁用此功能。 |
...
- 如果没有**服务器成员意图**，机器人将无法解析允许用户列表中的用户名，并且可能无法识别是谁在给它发消息。

:::warning[这是 Discord 机器人无法工作的首要原因]
如果你的机器人显示在线但从不回复消息，那么**消息内容意图**几乎肯定是禁用的。请返回 [开发者门户](https://discord.com/developers/applications)，选择你的应用 → Bot → Privileged Gateway Intents，并确保**消息内容意图**已切换为 ON。点击 **Save Changes**。
:::
...
:::tip
开发者模式也允许你以同样的方式复制**频道 ID** 和**服务器 ID**——右键单击频道或服务器名称并选择 Copy ID。如果你想手动设置主频道，你将需要频道 ID。
:::
...
当提示时选择 **Discord**，然后在被询问时粘贴你的机器人令牌和用户 ID。
...
:::tip
你可以在后台或作为 systemd 服务运行 `hermes gateway` 以实现持久运行。有关详细信息，请参阅部署文档。
:::
...
| `DISCORD_ALLOW_MENTION_ROLES` | 否 | `false` | 当为 `false`（默认）时，Bot 无法提及 `@role`。设置为 `true` 以允许。 |
...
`~/.hermes/config.yaml` 中的 `discord` 部分与上述环境变量对应。`config.yaml` 中的设置作为默认值应用 —— 如果已设置等效的环境变量，则以环境变量为准。
...
启用后，bot 仅在服务器频道中被直接 `@提及` 时才回复。无论此设置如何，私信（DM）总会得到回复。

#### `discord.free_response_channels`

**类型：** string 或 list — **默认值：** `""`

无需 `@提及`、bot 会回复所有消息的频道 ID。接受逗号分隔的字符串或 YAML 列表：
...
如果某个主题（thread）的父频道在此列表中，该主题也变为免提及。

免回复频道也**跳过自动创建主题**——bot 会直接在该频道内回复，而不是为每条消息创建一个新主题。这使频道可以作为一个轻量级的聊天界面使用。如果你想要主题行为，请不要将频道列为免回复频道（使用正常的 `@提及` 流程即可）。

#### `discord.auto_thread`

**类型：** boolean — **默认值：** `true`

启用后，在常规文本频道中的每次 `@提及` 都会自动为该对话创建一个新主题。这可以保持主频道整洁，并为每次对话提供独立的会话历史记录。一旦主题创建，该主题中的后续消息就不再需要 `@提及`——bot 知道它已经在参与其中。

在现有主题或私信中发送的消息不受此设置影响。列在 `discord.free_response_channels` 或 `discord.no_thread_channels` 中的频道也会绕过自动创建主题，转而获得直接回复。

#### `discord.reactions`

**类型：** boolean — **默认值：** `true`

控制 bot 是否在消息上添加表情符号反应作为视觉反馈：
- 👀 当 bot 开始处理你的消息时添加
- ✅ 当响应成功发送时添加
- ❌ 如果在处理过程中发生错误时添加

如果你觉得这些反应分散注意力，或者 bot 的角色没有**添加反应**权限，请禁用此功能。

#### `discord.ignored_channels`

**类型：** string 或 list — **默认值：** `[]`

bot **永不**回复的频道 ID，即使被直接 `@提及`。此设置具有最高优先级——如果一个频道在此列表中，bot 会静默忽略该处的所有消息，无论 `require_mention`、`free_response_channels` 或其他任何设置如何。
...
bot 直接在频道内回复而不是自动创建主题的频道 ID。这仅在 `auto_thread` 为 `true`（默认值）时有效。在这些频道中，bot 会像普通消息一样直接回复，而不是创建一个新主题。

```yaml
discord:
  no_thread_channels:
    - 1234567890  # Bot 在此处直接回复
```

适用于专门用于 bot 交互的频道，在这些频道中创建主题会增加不必要的干扰。

#### `discord.channel_prompts`

**类型：** mapping — **默认值：** `{}`

针对每个频道的临时系统提示词，它们会在匹配的 Discord 频道或主题中的每一轮对话中被注入，而不会被持久化到对话历史记录中。
...
行为：
- 精确匹配的主题/频道 ID 优先。
- 如果消息到达某个主题或论坛帖子内部，且该主题没有明确的条目，Hermes 会回退到父频道/论坛 ID。
...
这是一个全局消息网关设置（非 Discord 特有），用于控制同一频道中的用户是否拥有独立的会话历史记录。

当为 `true` 时：Alice 和 Bob 在 `#research` 中交谈时，各自与 Hermes 拥有独立的对话。当为 `false` 时：整个频道共享一个对话记录和一个正在运行的 Agent 槽位。

```yaml
group_sessions_per_user: true
```

关于每种模式的完整含义，请参阅上方的[会话模型](#session-model-in-discord)部分。

#### `display.tool_progress`

**类型：** string — **默认值：** `"all"` — **可选值：** `off`, `new`, `all`, `verbose`

控制 bot 在处理过程中是否在聊天中发送进度消息（例如，“正在读取文件...”、“正在运行终端命令...”）。这是一个全局消息网关设置，适用于所有平台。
...
启用后，在消息网关中提供 `/verbose` 斜杠命令，允许你在不编辑 config.yaml 的情况下循环切换工具进度模式（`off → new → all → verbose → off`）。
...
默认情况下，每个被允许的用户都可以运行每个斜杠命令。要将你的允许列表分为**管理员**（拥有完整的斜杠命令访问权限）和**普通用户**（仅能运行你明确启用的命令），请在 Discord 平台的 `extra` 块中添加 `allow_admin_from` 和 `user_allowed_commands`：
  ...
  platforms:
    discord:
      extra:
        ...
        # /help 和 /whoami 始终被允许，以便用户可以查看其访问权限。
...
- 在某个作用域（私信或服务器频道）的 `allow_admin_from` 列表中的用户，可以通过实时命令注册表运行**所有**已注册的斜杠命令——包括内置命令和插件注册的命令。
- 不在 `allow_admin_from` 列表中的用户，只能运行 `user_allowed_commands` 中列出的命令，以及始终允许的基础命令：`/help` 和 `/whoami`。
- 普通聊天（非斜杠消息）不受影响。非管理员用户仍然可以正常与 Agent 对话；他们只是不能触发任意命令。
- **向后兼容：** 如果某个作用域未设置 `allow_admin_from`，则该作用域的斜杠命令门控将被禁用。现有安装无需更改即可继续工作。
- 私信管理员状态并不意味着服务器频道管理员状态。每个作用域都有自己的管理员列表。

使用 `/whoami` 查看当前作用域、你的层级（管理员 / 用户 / 无限制）以及你可以运行的斜杠命令。
...
无需额外配置——任何通过 `hermes skills install` 安装的技能，在下一次消息网关重启时都会自动注册为 Discord 斜杠命令。

### 禁用斜杠命令注册

如果你针对同一个 Discord 应用运行多个 Hermes 消息网关（例如，测试环境和生产环境），则其中只有一个应该拥有全局斜杠命令注册权——否则最后启动的会胜出，导致注册信息来回变动。请在“跟随者”消息网关上关闭斜杠命令注册：
...
在“主”消息网关上保持此设置为 `true`，可以保持正常行为——为内置命令和已安装技能提供全局 `/` 菜单命令。
...
| 音频 / 语音 | `send_voice` — 尽可能作为原生语音消息发送，否则作为文件附件 |
| 文档 (PDF/ZIP/docx 等) | `send_document` — 原生附件，带下载按钮 |

Discord 的每次上传大小限制取决于服务器的提升等级（免费 25 MB，最高 500 MB）。如果 Hermes 收到 HTTP 413 错误，适配器会回退到指向本地缓存路径的链接，而不是静默失败。

## 主频道

你可以指定一个“主频道”，让机器人发送主动消息（例如定时任务输出、提醒和通知）。有两种设置方法：
...
Discord 论坛频道（类型 15）不接受直接消息——论坛中的每个帖子都必须是一个主题。Hermes 会自动检测论坛频道，并在需要向该频道发送消息时创建一个新的主题帖子，因此 `send_message`、TTS、图像、语音消息和文件附件都可以正常工作，无需 Agent 进行特殊处理。

- **主题名称** 源自消息的第一行（去除 Markdown 标题前缀，最多 100 个字符）。当消息仅包含附件时，使用文件名作为备用主题名称。
- **附件** 随新主题的起始消息一起发送——无需单独的上传步骤，不会出现部分发送。
- **一次调用，一个主题：** 每次向论坛发送消息都会创建一个新主题。因此，连续向同一个论坛发送消息会产生不同的主题。
- **检测分为三层：** 首先是频道目录缓存，其次是进程本地探测缓存，最后是实时 `GET /channels/{id}` 探测（其结果随后在进程生命周期内被记忆）。

刷新目录（在暴露该命令的平台上使用 `/channels refresh`，或重启消息网关）会将机器人启动后创建的任何论坛频道填充到缓存中。
## 故障排除
...
**修复**：前往 [开发者门户](https://discord.com/developers/applications) → 你的应用 → Bot → Privileged Gateway Intents → 启用 **Message Content Intent** → 保存更改。重启消息网关。
...
**修复**：在 Discord 中，进入频道设置 → 权限 → 添加 Bot 的角色，并启用 **查看频道** 和 **读取消息历史** 权限。

### 403 Forbidden 错误

**原因**：Bot 缺少必要的权限。

**修复**：使用步骤 5 中的 URL，以正确的权限重新邀请 Bot，或者在服务器设置 → 角色中手动调整 Bot 的角色权限。
...
**修复**：将你的用户 ID 添加到 `~/.hermes/.env` 文件中的 `DISCORD_ALLOWED_USERS` 环境变量，然后重启消息网关。

### 同一频道中的人意外地共享上下文

**原因**：`group_sessions_per_user` 被禁用，或者平台无法为该上下文中的消息提供用户 ID。

**修复**：在 `~/.hermes/config.yaml` 中设置此选项并重启消息网关：

```yaml
group_sessions_per_user: true
```

如果你有意想要一个共享房间的对话，请保持其关闭状态 —— 只是需要预期会共享对话历史记录和共享中断行为。

## 安全

:::warning
始终设置 `DISCORD_ALLOWED_USERS`（或 `DISCORD_ALLOWED_ROLES`）以限制可以与 Bot 交互的人员。如果两者都未设置，作为安全措施，消息网关默认会拒绝所有用户。只授权你信任的人 —— 授权用户可以完全访问 Agent 的能力，包括工具使用和系统访问。
:::
...
- **自动启用 Server Members Intent**。当设置 `DISCORD_ALLOWED_ROLES` 时，Bot 在连接时会启用 Members 意图 —— 这是 Discord 在成员记录中发送角色信息所必需的。
- **使用角色 ID，而非名称**。从 Discord 获取：**用户设置 → 高级 → 开发者模式 ON**，然后右键单击任意角色 → **复制角色 ID**。
- **私信回退**。在私信中，角色检查会扫描共同所在的服务器；在任何共享服务器中拥有允许角色的用户在私信中也会被授权。

当版主团队人员变动时，这是首选模式 —— 新版主在获得角色的那一刻就获得了访问权限，无需编辑 `.env` 文件或重启消息网关。

### 提及控制

默认情况下，Hermes 会阻止 Bot 提及 `@everyone`、`@here` 和角色提及，即使其回复中包含这些标记。这可以防止措辞不当的提示词或回显的用户内容向整个服务器发送垃圾信息。单独的 `@user` 提及和回复引用提及（那个小的"正在回复…"标签）保持启用，以便正常对话仍可进行。
...
discord:
  allow_mentions:
    everyone: false      # 允许 Bot 提及 @everyone / @here
    roles: false         # 允许 Bot 提及 @role 角色
    users: true          # 允许 Bot 提及单个 @user 用户
...
:::tip
除非你确切知道为什么需要它们，否则请将 `everyone` 和 `roles` 保持为 `false`。LLM 很容易在看似正常的回复中生成 `@everyone` 字符串；如果没有此保护，这将通知你服务器的每个成员。
:::

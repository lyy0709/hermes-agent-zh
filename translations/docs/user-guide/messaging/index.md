...
当消息网关重启（或关闭时有正在进行的会话）时，它可以向每个平台的主频道发送一次性的"Agent 已恢复"/"Agent 被中断"消息。这由 `gateway-config.yaml` 中每个平台的 `gateway_restart_notification` 标志控制，默认为 `true`：
...
可以在嘈杂或低优先级的平台上禁用它，同时在您的主聊天中保持启用。无论有多少个会话正在进行，每次重启只发送一次通知。

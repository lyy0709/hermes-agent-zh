...
**语音** = TTS 语音回复和/或语音消息转录。**图片** = 发送/接收图片。**文件** = 发送/接收文件附件。**线程** = 线程化对话。**回应** = 消息上的表情回应。**输入中** = 处理时显示输入指示器。**流式传输** = 通过编辑进行渐进式消息更新。
...
这将引导你使用方向键选择来配置每个平台，显示哪些平台已经配置完成，并在完成后提供启动/重启网关的选项。
...
| `/undo` | 移除最后一次交互 |
| `/status` | 显示会话信息 |
| `/whoami` | 显示你在此作用域下的斜杠命令访问权限（管理员 / 用户 / 无限制） |
| `/stop` | 停止正在运行的 Agent |
...
允许列表回答的是"这个人能否联系到机器人？" **管理员/用户划分**回答的是"既然他们能联系到，他们被允许做什么？"
...
**未来可能限制的内容：** 更多功能层面（工具访问、模型切换、昂贵操作）将挂载在相同的管理员/用户划分下，随着我们添加它们。现在配置此划分意味着未来的限制将清晰落地，而无需您重新建模谁是管理员。
...
**向后兼容性：** 如果某个作用域未设置 `allow_admin_from`，则该作用域的层级划分被禁用，每个被允许的用户都拥有完全访问权限。现有安装无需更改即可继续工作 — 当您需要区分时再选择启用。
...
- `queue` — 后续消息等待，并在当前任务完成后作为下一个回合运行。
- `steer` — 后续消息通过 `/steer` 注入到当前运行中，在下一个工具调用后到达 Agent。不中断，不开始新回合。如果 Agent 尚未启动，则回退到 `queue` 行为。
...
在任何平台上第一次向忙碌的 Agent 发送消息时，Hermes 会在忙碌确认消息后附加一行提示，解释此设置（`"💡 首次提示 — …"`）。该提示在每个安装中只出现一次 — `onboarding.seen.busy_input_prompt` 下的一个标志会锁定它。删除该键可以再次看到提示。

如果您觉得忙碌确认消息很烦人 — 特别是在语音输入或快速连续发送消息时 — 请设置 `display.busy_ack_enabled: false`。您的输入仍然会正常排队/引导/中断，只是聊天回复被静音了。
...
在单独的后台会话中运行提示词，让 Agent 独立处理任务，同时保持主聊天界面的响应性：
...
每个 `/background` 提示词都会生成一个**独立的 Agent 实例**异步运行：

- **隔离的会话** — 后台 Agent 拥有自己的会话和对话历史。它不了解你当前的聊天上下文，只接收你提供的提示词。
- **相同的配置** — 继承当前消息网关设置中的模型、提供商、工具集、推理设置和提供商路由。
- **非阻塞** — 你的主聊天界面保持完全交互性。在后台任务运行时，你可以发送消息、运行其他命令或启动更多后台任务。
- **结果交付** — 任务完成后，结果会发送回**你发出命令的同一聊天或频道**，并带有“✅ 后台任务完成”前缀。如果失败，你会看到“❌ 后台任务失败”及错误信息。

### 后台进程通知

当运行后台会话的 Agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，消息网关可以向你的聊天推送状态更新。通过 `~/.hermes/config.yaml` 中的 `display.background_process_notifications` 来控制此行为：
...
- **PATH** — 安装时你的完整 shell PATH，并预先添加了 venv 的 `bin/` 和 `node_modules/.bin`。这确保用户安装的工具（Node.js、ffmpeg 等）对消息网关子进程（如 WhatsApp 桥接器）可用。
...
消息网关通常会同时运行多个适配器（Telegram + Discord + Slack 等）。以下部分涵盖了跨所有平台的日常运维操作。

### `/platform` 命令

消息网关启动后，可以从任何已连接的 CLI 会话或聊天中使用 `/platform` 斜杠命令来检查和操控单个适配器，而无需重启整个消息网关：
...
每个适配器都包裹在一个断路器中。可重试的故障（网络波动、速率限制响应、上游 5xx 响应、WebSocket 断开连接）重复发生会导致断路器跳闸——适配器会自动暂停，当配置了其他活跃平台时，会向该平台的主频道发送操作员通知，并输出一条结构化日志行。

断路器**不会**自动恢复——它会保持打开状态，直到你手动运行 `/platform resume <name>`。这是有意为之的：如果某个平台持续中断，你肯定不希望消息网关不断尝试重连。
...
可以在嘈杂或低优先级的平台上禁用它，同时在你主要的聊天平台上保持启用。无论有多少个进行中的会话，每次重启只发送一次通知。

### 跨消息网关重启的会话恢复

当消息网关在工具调用或生成进行中关闭时，受影响的会话会被标记为 `restart_interrupted`。在下一次启动时，消息网关会为每个此类会话安排自动恢复——用户在聊天中会收到一个简短的提示（“重启后发送任何消息，我将尝试从您离开的地方继续。”），当他们回复时，会话会从最后提交的回合处继续。
...
无需配置。如果你不想要这个提示，请在平台上设置 `gateway_restart_notification: false`。

### 移动端友好的进度默认设置

Telegram 通常是一个移动端收件箱，因此默认设置针对该界面进行了调整：

- **`tool_progress`** 默认为 **`off`** —— 没有每个工具的进度面包屑流填满聊天。
- **`busy_ack_detail`** 默认为 **`off`** —— 忙碌状态确认和长时间运行的心跳保持简洁（没有 `iteration 21/60` 这样的调试细节）。
- **`interim_assistant_messages`** 保持 **`on`** —— 真正的回合中助手评论（模型直接告诉你它将要做什么）是信号，不是噪音。
- **`long_running_notifications`** 保持 **`on`** —— 一个原地编辑的“⏳ 工作中 — N 分钟”气泡每隔几分钟更新一次，这样你就能看到心跳，而不是盯着 `typing…` 看半小时。

你可以为每个平台选择退出上述保持启用的默认设置，或者重新启用详细的进度显示：
...
工具进度消息、“仍在工作…”心跳和状态回调气泡也可以在最终响应到达后自动删除。通过 `display.platforms.<platform>.cleanup_progress` 为每个平台启用：
...
默认为 `false`。只有适配器实现了 `delete_message` 的平台才会遵循此设置（目前是 Telegram 和 Discord）。失败的运行会**跳过**清理，因此气泡会作为面包屑保留。

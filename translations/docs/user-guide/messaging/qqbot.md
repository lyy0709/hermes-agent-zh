...
| `QQ_ALLOWED_USERS` | 允许访问私聊的、逗号分隔的用户 OpenID | open (所有用户) |
| `QQ_GROUP_ALLOWED_USERS` | 允许访问群聊的、逗号分隔的群组 OpenID | — |
...
1.  **QQ 内置 ASR** (免费，始终首先尝试) — QQ 在语音消息附件中提供 `asr_refer_text`，它使用腾讯自家的语音识别

...
Hermes Agent 与钉钉集成作为聊天机器人，让您可以通过私聊或群聊与您的 AI 助手对话。机器人通过钉钉的 Stream 模式连接——这是一种长连接的 WebSocket 连接，无需公共 URL 或 Webhook 服务器——并通过钉钉的会话 Webhook API 使用 Markdown 格式的消息进行回复。

在开始设置之前，以下是大多数人想知道的部分：Hermes 在您的钉钉工作空间中如何运行。

## Hermes 的行为方式

| 场景 | 行为 |
|---------|----------|
| **私聊 (1:1 聊天)** | Hermes 会回复每条消息。无需 `@提及`。每个私聊都有其独立的会话。 |
| **群聊** | 当您 `@提及` Hermes 时，它才会回复。如果没有提及，Hermes 会忽略该消息。 |
| **多用户共享群组** | 默认情况下，Hermes 在群组内为每个用户隔离会话历史。除非您明确禁用此功能，否则两个人在同一个群组中的对话不会共享一个记录。 |
...
2. 使用您的钉钉管理员账号登录。
3. 点击 **应用开发** → **企业内部开发** → **创建应用**（选择 **H5微应用** 或 **机器人**，具体取决于您的控制台版本）。
4. 填写：
...
:::warning[凭证仅显示一次]
Client Secret 仅在您创建应用时显示一次。如果丢失，您需要重新生成。切勿公开分享这些凭证或将其提交到 Git。
:::
...
2. 或者，机器人会记录每条传入消息的 `sender_id`。启动消息网关，向机器人发送一条消息，然后在日志中查看您的 ID。
...
**解决方法**：验证在您的应用设置中机器人能力已启用且选择了 Stream 模式。检查您的用户 ID 是否在 `DINGTALK_ALLOWED_USERS` 中。重启消息网关。
...
**解决方法**：验证 `DINGTALK_CLIENT_ID` 和 `DINGTALK_CLIENT_SECRET` 在 `~/.hermes/.env` 中是否正确设置。Client ID 是您的 AppKey，Client Secret 是来自钉钉开发者后台的 AppSecret。

### Stream 断开连接 / 重连循环

**原因**：网络不稳定、钉钉平台维护或凭证问题。

**解决方法**：适配器会自动以指数退避方式重连（2s → 5s → 10s → 30s → 60s）。检查您的凭证是否有效且您的应用未被停用。验证您的网络是否允许出站 WebSocket 连接。
...
- **消息长度限制**：每条回复限制在 20,000 个字符以内。更长的回复会被截断。
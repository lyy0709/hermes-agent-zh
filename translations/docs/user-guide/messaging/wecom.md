...
为了进行精细控制，你可以限制在特定群组中允许哪些用户与机器人交互。这需要在 `config.yaml` 中配置：
...
如果未为群组配置 `allow_from`，则该群组中的所有用户都被允许 (假设该群组本身通过了顶层策略检查)。
...
**自动降级：** 当媒体超过原生类型的尺寸限制但低于绝对 20 MB 文件限制时，它会自动作为通用文件附件发送：
...
适配器与企业微信的消息网关 `wss://openws.work.weixin.qq.com` 保持持久的 WebSocket 连接。
...
入站消息使用消息 ID 进行去重，时间窗口为 5 分钟，最大缓存 1000 个条目。这可以防止在重连或网络波动期间重复处理消息。
...
| `WECOM_ALLOWED_USERS` | — | _(空)_ | 消息网关级别白名单的逗号分隔用户 ID |
...
| `invalid secret (errcode=40013)` | 验证 secret 是否与你的机器人凭证匹配 |

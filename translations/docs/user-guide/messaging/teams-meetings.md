...
关于上线后操作、上线检查清单和操作员工作表，请使用专用指南：[操作 Teams 会议流水线](/docs/guides/operate-teams-meeting-pipeline)。
...
- 如果您希望使用录音加语音转文字回退功能，则需要安装 `ffmpeg`

## 步骤 1：添加 Microsoft Graph 凭据

将 Graph 仅限应用程序凭据添加到 `~/.hermes/.env`：
...
Microsoft Graph 将 Webhook 订阅限制在 72 小时内，并且不会自动续订。**您必须在正式上线前安排 `hermes teams-pipeline maintain-subscriptions`**，否则在任何手动订阅创建三天后，通知将静默停止。请参阅操作员手册中的[自动化订阅续订](/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production) — 提供了三种选项（Hermes 定时任务、systemd 定时器、普通 crontab）。

:::

关于订阅维护和上线后操作流程，请继续阅读指南：[操作 Teams 会议流水线](/docs/guides/operate-teams-meeting-pipeline)。
...
| 优先转录稿从未成功 | 检查转录稿资源的 Graph 权限，以及该会议的转录稿文件是否存在 |
| 录音回退失败 | 确认 `ffmpeg` 已安装，并且 Graph 应用程序可以访问录音文件 |

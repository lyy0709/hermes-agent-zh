...
6.  **跨会话隔离** —— 会话之间无法访问彼此的数据或状态；定时任务的存储路径经过强化，可防御路径遍历攻击
...
| `destructive_slash_confirm` | `true` | 为 true 时，破坏性会话斜杠命令（`/clear`、`/new`、`/reset`、`/undo`）在丢弃对话状态前会提示。三选项对话框（批准一次 / 始终批准 / 取消）通过 Telegram、Discord 和 Slack 的原生是/否按钮路由；其他平台使用文本回退。点击 **始终批准** 的用户会将此键翻转为 `false`。TUI 使用其自己的模态覆盖层（设置 `HERMES_TUI_NO_CONFIRM=1` 可在 TUI 中退出此功能）。 |
...
当 YOLO 激活时，Hermes 会显示两个持续的视觉提醒，因此很难忘记审批提示已被绕过：

*   当 YOLO 已激活时，会话开始时显示红色横幅行：`⚠ YOLO 模式 —— 所有审批提示已绕过`。YOLO 关闭时隐藏，以保持默认横幅简洁。
*   状态栏中显示 `⚠ YOLO` 片段，在你切换 YOLO 开关时实时更新（富文本渲染器和纯文本回退）。

:::danger
YOLO 模式会禁用会话的**所有**危险命令安全检查——**除了**硬性阻止列表（见下文）。仅在你完全信任生成的命令时使用（例如，在一次性环境中经过充分测试的自动化脚本）。
:::
...
阻止列表是 `--yolo` 之下的底线。它会在审批层甚至看到命令之前就触发，并且没有覆盖标志。当前涵盖的模式（非详尽；与 `tools/approval.py::UNRECOVERABLE_BLOCKLIST` 保持同步）：
| 模式 | 为何严格禁止 |
|---|---|
| `rm -rf /` 及其明显变体 | 擦除文件系统根目录 |
| `rm -rf --no-preserve-root /` | 明确表示“是的，我指的就是根目录”的变体 |
...
:::info
**容器绕过**：当在 `docker`、`singularity`、`modal` 或 `daytona` 后端中运行时，危险命令检查会被**跳过**，因为容器本身就是安全边界。容器内的破坏性命令不会对宿主机造成损害。
:::
...
:::warning
如果**未配置任何允许列表**且未设置 `GATEWAY_ALLOW_ALL_USERS`，**所有用户都将被拒绝**。消息网关在启动时会记录警告：
...
`SETUID`/`SETGID` **不**在基础列表中——它们是在容器以 root 身份启动且 init/entrypoint 必须降低权限（s6 权限降低路径）时有条件地添加的。当容器已经以非 root `--user` 运行时，它们会被跳过。`/run` tmpfs 也从基础列表中分离出来，并按镜像挂载（默认加固为 `noexec`，仅对从 `/run` 执行的 s6-overlay 镜像设置为 `exec`）。
...
某些技能需要沙盒中的**文件**（不仅仅是环境变量）——例如，Google Workspace 将 OAuth 令牌存储为活动配置文件 `HERMES_HOME` 下的 `google_token.json`。技能在前置元数据中声明这些文件：
...
加载后，Hermes 会检查这些文件是否存在于活动配置文件的 `HERMES_HOME` 中，并注册它们以便挂载：
...
路径相对于 `~/.hermes/`。文件在容器内挂载到 `/root/.hermes/`。此列表由 `tools/credential_files.py`（`terminal.credential_files`）读取——它位于 `terminal:` 块下，但由凭证文件模块加载，而非核心终端后端，因此不属于捆绑的 `DEFAULT_CONFIG` 快照的一部分。
...
mcp_servers:
  github:
    ...
    env:
      GITHUB_PERSONAL_ACCESS_TOKEN: "ghp_..."  # 仅传递此项
```
...
您可以限制 Agent 通过其网页和浏览器工具可以访问哪些网站。这对于防止 Agent 访问内部服务、管理面板或其他敏感 URL 非常有用。
...
某些设置确实需要访问私有/内部 URL —— 例如将 `home.arpa` 解析到 RFC 1918 地址空间的家庭网络、仅限局域网的 Ollama/llama.cpp 端点、内部维基、云元数据调试等。针对这些情况，提供了一个全局禁用选项：
...
启用后，Web 工具、浏览器、视觉 URL 获取以及消息网关的媒体下载将不再拒绝 RFC 1918 / 环回 / 链路本地 / CGNAT / 云元数据目标地址。**这是一个有意的信任边界** —— 仅在允许 Agent 在本地网络上运行任意提示词注入 URL 的风险可接受的机器上启用。面向公众的消息网关应保持此选项关闭。

主机子串防护（即使底层 IP 是公网，也能阻止类似 Unicode 域名欺骗）无论此设置如何都会保持启用。
...
Tirith 为 Linux（x86_64 / aarch64）和 macOS（x86_64 / arm64）提供了预构建的二进制文件。在没有预构建二进制文件的平台（Windows 等）上，tirith 会被静默跳过 —— 模式匹配防护仍然运行，CLI 不会显示“不可用”的横幅。要在 Windows 上使用 tirith，请在 WSL 下运行 Hermes。
...
7.  **设置 `terminal.cwd`** —— 不要让 Agent 在敏感目录下操作
...
SSH 连接详细信息保存在 `.env` 中（而不是 `config.yaml`），因此它们不会被签入或随配置文件导出而共享。这使消息网关的消息传递连接与 Agent 的命令执行保持分离。

## 供应链安全通告检查

Hermes 内置了一个安全通告扫描器，用于标记活动虚拟环境中与已知受感染版本（例如 2026 年 5 月的 `mistralai 2.4.6` 投毒等供应链蠕虫）的精选目录匹配的 Python 包。实现位于 `hermes_cli/security_advisories.py` 中。
...
确认关闭的信息会持久化到 `config.security.acked_advisories` 中，并在重启后保留。旧的安全通告**不会**从目录中删除 —— 保留它们可以让新安装的版本对可能仍在私有镜像中缓存的、历史上被投毒的版本发出警告。
检查本身仅使用标准库，每次启动时通过每个安全公告的 `importlib.metadata.version()` 查询来运行，因此每次启动时运行都是安全的。

### 可选依赖的惰性安装

许多功能（Mistral TTS、ElevenLabs、Honcho 记忆、Bedrock、Slack、Matrix……）依赖于并非每个用户都需要的 Python 包。Hermes 会在首次使用时**惰性**安装这些包，而不是在 `hermes-agent[all]` 下急切安装。实现代码位于 `tools/lazy_deps.py`。

这解决了以下权衡问题：

- **脆弱性。** 当某个额外依赖项的传递依赖在 PyPI 上变得不可用（因恶意软件被隔离、被撤回、上传损坏）时，整个 `[all]` 的解析会失败，新安装会静默回退到精简层级——一次性丢失 10 多个不相关的额外功能。惰性安装隔离了每个后端，因此一个被污染的依赖项不会破坏不相关的功能。
...
| 仅限 PyPI 名称 | 规范接受 `"package>=1.0,<2"` 语法。不接受 `--index-url`、`git+https://` 或 file: 路径——恶意的 `config.yaml` 无法重定向安装 |
| 允许列表 | 只有出现在代码树内 `LAZY_DEPS` 映射中的规范才能通过此路径安装。功能名称中的拼写错误**不会**获得“安装任意内容”的语义 |
| 可禁用 | 设置 `security.allow_lazy_installs: false` 可完全禁用运行时安装。适用于受限网络或严格的安全策略 |
| 无静默重试 | 失败会以 `FeatureUnavailable` 形式暴露——不会缓存错误状态，不会出现重试风暴 |

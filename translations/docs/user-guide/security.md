...
3. **容器隔离** —— 使用 Docker/Singularity/Modal 沙盒隔离并采用强化设置
4. **MCP 凭证过滤** —— 为 MCP 子进程隔离环境变量
5. **上下文文件扫描** —— 检测项目文件中的提示词注入
6. **跨会话隔离** —— 会话之间无法访问彼此的数据或状态；定时任务的存储路径经过强化，可抵御路径遍历攻击
...
| `mcp_reload_confirm` | `true` | 为 true 时，`/reload-mcp` 在重建 MCP 工具集之前会询问。重建会使提供商提示词缓存失效（工具模式存在于系统提示词中），因此下一条消息会重新发送完整的输入 Token。点击 **始终批准** 的用户会将此键翻转为 `false`。 |
| `destructive_slash_confirm` | `true` | 为 true 时，破坏性会话斜杠命令（`/clear`、`/new`、`/reset`、`/undo`）在丢弃对话状态前会提示。三选项对话框（批准一次 / 始终批准 / 取消）通过 Telegram、Discord 和 Slack 的原生是/否按钮路由；其他平台使用文本回退。点击 **始终批准** 的用户会将此键翻转为 `false`。TUI 使用其自己的模态覆盖层（设置 `HERMES_TUI_NO_CONFIRM=1` 可在 TUI 中退出）。 |
...
- 当 YOLO 已激活时，会话开始时显示红色横幅行：`⚠ YOLO 模式 —— 所有审批提示已绕过`。YOLO 关闭时隐藏，以便默认横幅保持整洁。
- 在所有宽度层级的状态栏中显示 `⚠ YOLO` 片段，在切换 YOLO 开关时实时更新（富文本渲染器和纯文本回退）。

:::danger
YOLO 模式会禁用会话的**所有**危险命令安全检查 —— **除了**硬性阻止列表（见下文）。仅在完全信任生成的命令时使用（例如，在一次性环境中经过充分测试的自动化脚本）。
:::

对于破坏性会话斜杠命令（`/clear`、`/new` / `/reset`、`/undo`、`/quit --delete` —— `/exit --delete` 是别名），CLI 在运行它们之前也会提示确认。请参阅 [斜杠命令 —— 破坏性命令的确认提示](../reference/slash-commands.md#confirmation-prompts-for-destructive-commands)。

### 硬性阻止列表（始终生效的底线）

有些命令具有灾难性后果 —— 不可逆的文件系统擦除、fork 炸弹、直接块设备写入 —— Hermes **无论**以下情况如何，都拒绝运行它们：
...
| `dd if=` | 磁盘复制 |
| `> /dev/sd` | 写入块设备 |
| `DROP TABLE/DATABASE` | SQL DROP 语句 |
| `DELETE FROM` (不带 WHERE) | 不带 WHERE 子句的 SQL DELETE 语句 |
| `TRUNCATE TABLE` | SQL TRUNCATE 语句 |
| `> /etc/` | 覆盖系统配置 |
...
| `tee` 输出到 `/etc/`、`~/.ssh/`、`~/.hermes/.env` | 通过 tee 覆盖敏感文件 |
| `>` / `>>` 输出到 `/etc/`、`~/.ssh/`、`~/.hermes/.env` | 通过重定向覆盖敏感文件 |
...
| `gateway run` 配合 `&`/`disown`/`nohup`/`setsid` | 防止在服务管理器之外启动消息网关 |

:::info
**容器绕过**：当在 `docker`、`singularity`、`modal` 或 `daytona` 后端中运行时，危险命令检查会被**跳过**，因为容器本身就是安全边界。容器内的破坏性命令不会对宿主机造成伤害。
:::

### 批准流程 (CLI)

在交互式 CLI 中，危险命令会显示一个内联的批准提示：

```
  ⚠️  危险命令：递归删除
      rm -rf /tmp/old-project

      [o] 单次允许  |  [s] 会话内允许  |  [a] 永久允许  |  [d] 拒绝
...
通过“永久允许”批准的命令会保存到 `~/.hermes/config.yaml`：
...
为了实现更灵活的授权，Hermes 包含一个基于代码的配对系统。无需预先提供用户 ID，未知用户会收到一个一次性配对码，由 bot 所有者通过 CLI 批准。
...
当一个技能被加载（通过 `skill_view` 或 `/skill` 命令）并声明了 `required_environment_variables` 时，环境中实际设置的任何这些变量都会自动注册为透传变量。缺失的变量（仍处于需要设置的状态）**不会**被注册。
...
某些技能在沙盒中需要**文件**（不仅仅是环境变量）——例如，Google Workspace 将 OAuth 令牌存储为活动配置文件的 `HERMES_HOME` 下的 `google_token.json`。技能在前言中声明这些文件：
...
- 技能守卫在安装前会扫描技能内容以查找可疑的环境变量访问模式
- 缺失/未设置的变量永远不会被注册（不存在的东西无法泄露）
- Hermes 基础设施密钥（提供商 API 密钥、消息网关令牌）**绝不应**添加到 `env_passthrough`——它们有专门的机制

## MCP 凭证处理

MCP（Model Context Protocol）服务器子进程接收一个**经过过滤的环境**，以防止意外凭证泄露。
...
SSRF 防护在面向互联网使用时始终处于活动状态，DNS 失败被视为被阻止（故障关闭）。重定向链在每一跳都会重新验证，以防止基于重定向的绕过。

#### 有意允许私有 URL

某些设置确实需要访问私有/内部 URL——例如解析到 RFC 1918 地址空间的家庭网络 `home.arpa`、仅限局域网的 Ollama/llama.cpp 端点、内部维基、云元数据调试等。对于这些情况，有一个全局选择退出选项：

```yaml
security:
  allow_private_urls: true   # 默认值：false
```
启用后，网页工具、浏览器、视觉 URL 抓取和消息网关媒体下载将不再拒绝 RFC 1918 / 环回 / 链路本地 / CGNAT / 云元数据目标。**这是一个有意的信任边界**——仅在允许 Agent 针对本地网络运行任意提示词注入 URL 的风险可接受的机器上启用此功能。面向公众的消息网关应保持其关闭状态。

主机子串防护（即使底层 IP 是公共的，也能阻止类似 Unicode 域名欺骗）无论此设置如何都将保持启用。
...
Tirith 的判定结果会集成到审批流程中：安全的命令直接通过，而可疑和被阻止的命令都会触发用户审批，并显示完整的 tirith 发现结果（严重性、标题、描述、更安全的替代方案）。用户可以批准或拒绝——默认选择是拒绝，以保持无人值守场景的安全。
...
8.  **以非 root 用户身份运行**——切勿以 root 用户身份运行消息网关
...
SSH 连接详细信息保存在 `.env` 中（而不是 `config.yaml`），这样它们就不会随配置文件导出而被签入或共享。这使消息网关的消息传递连接与 Agent 的命令执行保持分离。

## 供应链安全通告检查

Hermes 内置了一个安全通告扫描器，用于标记活动虚拟环境中与已知受感染版本精选目录匹配的 Python 包（例如 2026 年 5 月的 `mistralai 2.4.6` 投毒等供应链蠕虫）。实现位于 `hermes_cli/security_advisories.py` 中。
...
每个安全通告都有一个稳定的 ID。一旦你阅读并处理了它，就可以永久关闭它：

```bash
hermes doctor --ack <advisory-id>
```

确认关闭会持久化到 `config.security.acked_advisories` 中，并在重启后保留。旧的安全通告**不会**从目录中移除——将它们保留在原处，可以让新安装的系统对可能仍在私有镜像中缓存的、历史上被投毒的版本保持警惕。

检查本身仅使用标准库，并且每个安全通告只进行一次 `importlib.metadata.version()` 查找，因此每次启动时运行都是安全的。

### 可选依赖的延迟安装

许多功能（Mistral TTS、ElevenLabs、Honcho 记忆、Bedrock、Slack、Matrix……）依赖于并非每个用户都需要的 Python 包。Hermes 在首次使用时**延迟**安装这些包，而不是在 `hermes-agent[all]` 下急切安装。实现位于 `tools/lazy_deps.py` 中。
这种权衡解决了以下问题：

- **脆弱性**。当某个额外功能的传递依赖在 PyPI 上变得不可用（因恶意软件被隔离、被撤回、上传损坏）时，整个 `[all]` 解析过程都会失败，新安装会静默回退到一个精简的层级——一次性丢失 10 多个不相关的额外功能。惰性安装隔离了每个后端，因此一个被污染的依赖不会破坏不相关的功能。
- **臃肿**。一个只与一个提供商对话的用户不再需要拉取数百个他们永远不会导入的包。

工作原理：

1. 后端模块在其首次导入路径的顶部调用 `ensure("feature.name")`。
2. 如果依赖缺失，`ensure` 会检查 `config.yaml` 中的 `security.allow_lazy_installs`（默认为 `true`），并为允许列表中的规范运行一个虚拟环境范围内的 `pip install`。
3. 如果安装失败或用户禁用了惰性安装，调用会引发 `FeatureUnavailable` 异常，并附带实际的 pip 标准错误输出，以及指向 `hermes tools` 的说明。
...
| 仅限 PyPI 名称 | 规范接受 `"package>=1.0,<2"` 语法。不允许 `--index-url`、`git+https://` 或 file: 路径——恶意的 `config.yaml` 无法重定向安装 |
| 允许列表 | 只有出现在代码库内 `LAZY_DEPS` 映射中的规范才能通过此路径安装。功能名称中的拼写错误**不会**获得“安装任何东西”的语义 |
| 可选择性退出 | 设置 `security.allow_lazy_installs: false` 以完全禁用运行时安装。适用于受限网络或严格的安全策略 |
...
禁用后，需要可选依赖的后端会告诉用户手动运行安装（`pip install …`）或通过 `hermes tools` 选择不同的后端。
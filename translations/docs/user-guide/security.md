...
| **off** | 禁用所有审批检查——相当于使用 `--yolo` 运行。所有命令都无需提示直接执行。 |
...
当 YOLO 激活时，Hermes 会显示两个持久性的视觉提醒，因此很难忘记审批提示已被绕过：

*   会话开始时，如果 YOLO 已激活，会显示红色横幅：`⚠ YOLO 模式 — 所有审批提示已绕过`。当 YOLO 关闭时隐藏，以保持默认横幅简洁。
*   在所有宽度层级的状态栏中显示 `⚠ YOLO` 片段，在你切换 YOLO 开关时实时更新（富文本渲染器和纯文本回退）。
...
阻止列表是 `--yolo` 之下的底线。它甚至在审批层看到命令之前就会触发，并且没有覆盖标志。当前涵盖的模式（非详尽列表；与 `tools/approval.py::UNRECOVERABLE_BLOCKLIST` 保持同步）：
...
如果你触发了阻止列表，工具调用会向 Agent 返回解释性错误，并且不会运行任何命令。如果合法的工作流需要这些命令之一（例如，你是擦除和重装流水线的操作员），请在 Agent 外部运行它。
...
:::info
**容器绕过**：在 `docker`、`singularity`、`modal`、`daytona` 或 `vercel_sandbox` 后端中运行时，危险命令检查会被**跳过**，因为容器本身就是安全边界。容器内的破坏性命令无法危害宿主机。
:::
...
:::warning
如果**未配置任何允许列表**且未设置 `GATEWAY_ALLOW_ALL_USERS`，则**所有用户都将被拒绝**。网关在启动时会记录警告：
...
:::tip
对于生产环境的消息网关部署，请使用 `docker`、`modal`、`daytona` 或 `vercel_sandbox` 后端，以将 Agent 命令与您的主机系统隔离。这完全消除了对危险命令审批的需求。
:::
...
有关完整详细信息，请参阅配置指南中的[网站阻止列表](/user-guide/configuration#website-blocklist)。
...
SSRF 防护在面向互联网使用时始终处于活动状态，DNS 失败被视为被阻止（故障关闭）。重定向链在每个跳转点都会重新验证，以防止基于重定向的绕过。

#### 有意允许私有 URL

某些设置确实需要访问私有/内部 URL —— 将 `home.arpa` 解析到 RFC 1918 地址空间的家庭网络、仅限局域网的 Ollama/llama.cpp 端点、内部维基、云元数据调试等。对于这些情况，有一个全局选择退出选项：
...
启用后，网络工具、浏览器、视觉 URL 获取以及消息网关媒体下载将不再拒绝 RFC 1918 / 环回 / 链路本地 / CGNAT / 云元数据目标。**这是一个有意的信任边界** —— 仅在允许 Agent 针对本地网络运行任意提示词注入 URL 的风险可接受的机器上启用它。面向公众的消息网关应保持其关闭。
...
当 `tirith_fail_open` 为 `true`（默认值）时，如果 tirith 未安装或超时，命令将继续执行。在高安全环境中设置为 `false`，以在 tirith 不可用时阻止命令。

Tirith 为 Linux（x86_64 / aarch64）和 macOS（x86_64 / arm64）提供预构建的二进制文件。在没有预构建二进制文件的平台（Windows 等）上，tirith 会被静默跳过 —— 模式匹配防护仍然运行，CLI 不会显示“不可用”横幅。要在 Windows 上使用 tirith，请在 WSL 下运行 Hermes。
...
为了获得最高安全性，请在独立的机器或虚拟机上运行消息网关。在 `config.yaml` 中设置 `terminal.backend: ssh`，然后通过 `~/.hermes/.env` 中的环境变量提供主机详细信息：
...
SSH 连接详细信息存放在 `.env` 文件中（而非 `config.yaml`），这样它们就不会随配置文件导出而被签入或共享。这可以将消息网关的消息连接与 Agent 的命令执行分离开来。

## 供应链安全通告检查

Hermes 内置了一个安全通告扫描器，它会标记活动虚拟环境中与已知受感染版本（例如 2026 年 5 月的 `mistralai 2.4.6` 投毒等供应链蠕虫）的精选目录匹配的 Python 包。实现代码位于 `hermes_cli/security_advisories.py`。
...
确认信息会持久化到 `config.security.acked_advisories` 中，并在重启后保留。旧的安全通告**不会**从目录中移除——将它们保留在原处，可以让新安装的系统对可能仍缓存在私有镜像中的历史受感染版本保持警惕。

检查本身仅使用标准库，并且每个通告只进行一次 `importlib.metadata.version()` 查找，因此每次启动时运行都是安全的。

### 可选依赖的惰性安装

许多功能（Mistral TTS、ElevenLabs、Honcho memory、Bedrock、Slack、Matrix……）依赖于并非每个用户都需要的 Python 包。Hermes 在首次使用时**惰性**安装这些包，而不是在 `hermes-agent[all]` 下急切安装。实现代码位于 `tools/lazy_deps.py`。

这解决了以下权衡问题：

- **脆弱性。** 当某个额外依赖的传递依赖在 PyPI 上变得不可用（因恶意软件被隔离、被撤回、上传损坏）时，整个 `[all]` 的解析都会失败，新安装会静默回退到一个精简的层级——一次性丢失 10 多个不相关的额外功能。惰性安装隔离了每个后端，因此一个被污染的依赖不会破坏不相关的功能。
...
| 保证 | 含义 |
|---|---|
| 仅限虚拟环境范围 | 安装目标为活动虚拟环境中的 `sys.executable`——绝不针对系统 Python |
| 仅按名称从 PyPI 安装 | 规范接受 `"package>=1.0,<2"` 语法。不接受 `--index-url`、`git+https://` 或 file: 路径——恶意的 `config.yaml` 无法重定向安装 |
| 允许列表 | 只有出现在代码树内 `LAZY_DEPS` 映射中的规范才能通过此路径安装。功能名称中的拼写错误**不会**获得“安装任何内容”的语义 |
| 选择退出 | 设置 `security.allow_lazy_installs: false` 以完全禁用运行时安装。适用于受限网络或严格的安全策略 |
...
禁用后，需要可选依赖项的后端会告知用户手动运行安装（`pip install …`）或通过 `hermes tools` 选择不同的后端。
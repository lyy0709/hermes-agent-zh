...
- **支持鼠标的文本选择**——拖动以统一背景高亮文本，而非使用 SGR 反色。使用终端的常规复制手势进行复制。
- **备用屏幕渲染**——差异更新意味着流式传输时无闪烁，退出后无滚动历史混乱。
- **输入框辅助功能**——长代码片段的内联粘贴折叠、`Cmd+V` / `Ctrl+V` 文本粘贴（带剪贴板图片回退）、括号粘贴安全机制，以及图片/文件路径附件规范化。

相同的 [皮肤](features/skins.md) 和 [人格](features/personality.md) 适用。在会话中使用 `/skin ares`、`/personality pirate` 进行切换，UI 会实时重绘。查看 [皮肤与主题](features/skins.md) 获取完整的可自定义键列表以及它们分别适用于经典 CLI 还是 TUI——TUI 遵循横幅调色板、UI 颜色、提示符字形/颜色、会话显示、补全菜单、选择背景色、`tool_prefix` 和 `help_header`。
...
- **鼠标拖动** 以统一的选择背景高亮文本。
- **`Cmd+V` / `Ctrl+V`** 首先尝试正常的文本粘贴，然后回退到 OSC52/原生剪贴板读取，最后当剪贴板或粘贴的有效负载解析为图片时，回退到图片附件。
...
所有斜杠命令功能不变。少数几个是 TUI 特有的——它们产生更丰富的输出或以覆盖层而非内联面板的形式呈现：
...
TUI 附带了有主见的每部分默认值，将轮次作为实时转录流式传输，而非一堆 V 形符号：

- `thinking` —— **展开**。推理在模型发出时内联流式传输。
- `tools` —— **展开**。工具调用及其结果以展开形式呈现。
- `subagents` —— 回退到全局 `details_mode`（默认在 V 形符号下折叠——在委派实际发生前保持安静）。
- `activity` —— **隐藏**。对于大多数日常使用来说，环境元数据（消息网关提示、终端对等性提示、后台通知）是噪音。工具失败仍然会在失败的工具行内联渲染；当每个面板都隐藏时，环境错误/警告会通过浮动警报后备机制呈现。

每部分覆盖优先于该部分的默认值和全局 `details_mode`。要重塑布局：
...
在 `display.sections` 中明确设置的任何内容都会覆盖默认值，因此现有配置可以保持不变地工作。

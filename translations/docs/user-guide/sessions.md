...
每次对话——无论是来自 CLI、Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Teams 还是任何其他消息平台——都会作为包含完整消息历史的会话存储。会话通过两个互补的系统进行追踪：
...
如果你给会话设置了标题（见下文[会话命名](#session-naming)），可以按名称恢复：
...
当你按名称恢复会话时（`hermes -c "my project"`），它会自动选择该谱系中最新的会话。
...
3.  加载每个会话的对话内容，截断至约 100K 字符，并围绕匹配项居中显示
4.  发送到快速摘要模型以生成聚焦摘要
...
- 在确实删除了行的清理操作之后，会对 `state.db` 执行 `VACUUM` 以回收磁盘空间（SQLite 在普通的 DELETE 操作后不会缩小文件）
- 清理操作最多每 `sessions.min_interval_hours`（默认 24）运行一次；最后一次运行的时间戳记录在 `state.db` 内部，因此在同一个 `HERMES_HOME` 下的每个 Hermes 进程之间共享

默认是**关闭的** — 会话历史对于 `session_search` 的召回很有价值，静默删除可能会让用户感到意外。在 `~/.hermes/config.yaml` 中启用：
...
:::tip
数据库增长缓慢（典型情况：数百个会话约 10-15 MB），并且会话历史记录为跨过去对话的 `session_search` 召回提供支持，因此自动清理功能默认是禁用的。如果你运行繁重的消息网关/定时任务工作负载，并且 `state.db` 显著影响性能（观察到的故障模式：384 MB 的 state.db 和约 1000 个会话会减慢 FTS5 插入和 `/resume` 列表显示速度），请启用它。使用 `hermes sessions prune` 进行一次性清理，而无需开启自动扫描。
:::
...
例如，如果用户发送一张图片并要求 Hermes 用它制作一个表情包，Hermes 可能会用视觉模型检查该图片一次并运行图像处理脚本。未来的轮次不会自动在上下文中携带原始的 JPEG 文件。它们只携带写入对话的任何内容，例如用户的请求、简短的图像描述、本地缓存路径或最终的助手回复。

上下文增长最常见的原因不是媒体文件本身。而是冗长的文本：粘贴的转录、完整的日志、大量的工具输出、冗长的差异对比、重复的状态报告以及详细的证明转储。相比于将大型工件复制到聊天中，更推荐使用摘要、文件路径、重点摘录和基于工具的查找。

:::tip
当会话变长时使用 `/compress`，需要新线程时使用 `/new`，而 `hermes sessions prune` 仅在你想要从存储中删除旧的已结束会话时使用。压缩会减少活动上下文；它不是隐私删除。为 `/new` 传递一个名称（例如 `/new payments-refactor`）可以预先设置新会话的初始标题——这对于稍后使用 `/resume <name>` 或在 `/sessions` 选择器中找到它很有用。
:::
...
- **隐藏**系统消息、工具结果和内部推理
- **限制**为最后 10 次交换，并带有 "... N earlier messages ..." 指示器
- 使用**暗淡的样式**以区别于活动对话
...
:::tip
会话 ID 遵循格式 `YYYYMMDD_HHMMSS_<hex>` — CLI/TUI 会话使用 6 位十六进制后缀（例如 `20250305_091523_a1b2c3`），消息网关会话使用 8 位后缀（例如 `20250305_091523_a1b2c3d4`）。您可以通过 ID（完整或唯一前缀）或标题恢复会话 — 两者都适用于 `-c` 和 `-r` 参数。
:::

## 跨平台交接

在 CLI 会话中使用 `/handoff <platform>` 将实时对话转移到消息平台的归属频道。Agent 会从 CLI 中断的地方无缝接续 — 相同的会话 ID、完整的角色感知对话记录、工具调用等所有内容。
...
1.  CLI 验证 `<platform>` 是否已启用并设置了归属频道（在目标聊天中运行一次 `/sethome` 进行配置）。
2.  CLI 将会话标记为待处理并**阻塞轮询消息网关**。如果 Agent 正在响应中，则会拒绝 — 请等待当前响应完成。
3.  消息网关监听器认领交接任务，并向目标适配器请求一个新的对话线程：
    *   **Telegram** — 打开一个新的论坛主题（如果聊天中启用了 Bot API 9.4+ 的主题模式，则为私信主题；或论坛超级群组主题）。
    *   **Discord** — 在归属文本频道下创建一个 1440 分钟自动归档的线程。
    *   **Slack** — 发布一条种子消息，并使用其 `ts` 作为线程锚点。
    *   **WhatsApp / Signal / Matrix / SMS** — 无原生线程支持，直接回退到归属频道。
4.  消息网关将目标平台密钥重新绑定到您现有的 CLI 会话 ID，然后伪造一个合成的用户回合，要求 Agent 确认并总结。回复将出现在新线程中。
...
6.  从此刻起，对话将在该平台上进行。在新线程中回复 — 该频道中任何授权用户共享同一会话，之后线程中的任何真实用户消息都可以无缝加入，因为线程会话的键值不包含 `user_id`。

**恢复回 CLI：** 当您想回到桌面环境时，只需运行 `/resume <title>`（或在 shell 中运行 `hermes -r "<title>"`），即可从平台中断的地方继续。

**失败模式：**
*   未配置归属频道 → CLI 拒绝并提示 `/sethome`。
*   平台未启用 / 消息网关未运行 → CLI 在 60 秒后超时并显示明确消息，您的 CLI 会话保持不变。
*   线程创建失败（权限不足、主题模式关闭） → 直接回退到归属频道，交接仍能完成；虽然没有线程隔离，但交接本身有效。
*   `adapter.send` 失败（速率限制、瞬时 API 错误） → 交接标记为失败并附原因；该行记录会被清除，以便您重试。

**值得了解的局限性：** 对于不支持线程且具有多用户群组归属频道的平台，合成的回合会作为私信风格的会话进行键控。这对于自用私信归属频道（典型设置）有效，但对于真正共享的群组聊天并不理想。线程功能覆盖了 Telegram / Discord / Slack — 这是最常见的情况 — 因此大多数设置不会遇到此问题。
...
Hermes 会在首次交流后自动为每个会话生成一个简短的描述性标题（3–7 个词）。这使用一个快速的辅助模型在后台线程中运行，因此不会增加延迟。当使用 `hermes sessions list` 或 `hermes sessions browse` 浏览会话时，您会看到自动生成的标题。
...
标题会立即应用。如果会话尚未在数据库中创建（例如，您在发送第一条消息之前运行了 `/title`），它会被排队并在会话启动后应用。
...
如需更深入的分析——Token 使用量、成本估算、工具使用细分和活动模式——请使用 [`hermes insights`](/docs/reference/cli-commands#hermes-insights)。
...
- `messages` —— FTS5 匹配位置前后约 5 条消息，锚点消息已标记（上下文中的命中）
- `bookend_end` —— 会话的最后 3 条用户+助手消息（解决方案/决策）
- `match_message_id`、`messages_before`、`messages_after`

书签 + 窗口共同重构了 目标 → 匹配 → 解决方案，而无需支付整个转录的成本。在真实会话数据库上的典型耗时：15–50ms。
...
返回以锚点为中心、前后 ±`window` 条消息的窗口。没有 FTS5，没有书签——只有切片。在发现调用之后，当你需要比默认的 ±5 窗口更多上下文时使用。
...
- `sort` —— `newest` 或 `oldest`，在 FTS5 排名之上。省略则仅按相关性排序（默认；适合探索性回忆）。使用 `newest` 处理“我们上次把 X 留在哪了”这类问题，使用 `oldest` 处理“X 是怎么开始的”这类问题。
...
> *“当用户引用过去对话中的内容，或者你怀疑存在相关的先前上下文时，使用 session_search 来回忆它，而不是要求用户重复。”*

典型触发词：“我们之前做过这个”、“记得当时”、“上次”、“正如我提到的”，或者任何引用当前窗口中没有的项目/人员/概念。
...
- 在确实删除了行的清理操作之后，`state.db` 会被 `VACUUM` 以回收磁盘空间（SQLite 不会在普通的 DELETE 操作后缩小文件）
- 清理操作最多每 `sessions.min_interval_hours`（默认 24）运行一次；最后一次运行的时间戳在 `state.db` 内部跟踪，因此它在同一 `HERMES_HOME` 下的每个 Hermes 进程之间共享

默认是**关闭的** — 会话历史对于 `session_search` 召回很有价值，静默删除可能会让用户感到意外。在 `~/.hermes/config.yaml` 中启用：
...
:::tip
数据库增长缓慢（典型情况：数百个会话占用 10-15 MB），并且会话历史为跨过去对话的 `session_search` 召回提供支持，因此自动清理功能默认是禁用的。如果你运行繁重的消息网关/定时任务工作负载，并且 `state.db` 显著影响性能（观察到的故障模式：384 MB 的 state.db 和约 1000 个会话会减慢 FTS5 插入和 `/resume` 列表加载速度），请启用它。使用 `hermes sessions prune` 进行一次性清理，而无需开启自动扫描。
:::
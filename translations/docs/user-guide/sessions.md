...
每次对话——无论是来自 CLI、Telegram、Discord、Slack、WhatsApp、Signal、Matrix 还是任何其他消息平台——都会作为包含完整消息历史的会话被存储。会话通过两个互补的系统进行追踪：
...
- **隐藏**系统消息、工具结果和内部推理
- **限制**为最后 10 次交流，并带有 "... N earlier messages ..." 指示器
- 使用**暗淡样式**以区别于活跃对话
...
Hermes 会在第一次交流后自动为每个会话生成一个简短的描述性标题（3–7 个词）。这使用一个快速的辅助模型在后台线程中运行，因此不会增加延迟。当你使用 `hermes sessions list` 或 `hermes sessions browse` 浏览会话时，会看到自动生成的标题。
...
- **唯一性** —— 任何两个会话不能共享相同的标题
- **最多 100 个字符** —— 保持列表输出整洁
- **经过清理** —— 控制字符、零宽度字符和 RTL 覆盖符会被自动剥离
- **正常的 Unicode 字符都可以** —— 表情符号、中日韩文字、带重音的字符都可以使用
### 压缩时的自动谱系

当会话的上下文被压缩时（通过 `/compress` 手动压缩或自动压缩），Hermes 会创建一个新的延续会话。如果原始会话有标题，新会话会自动获得一个带编号的标题：
...
要进行更深入的分析——Token 使用情况、成本估算、工具细分和活动模式——请使用 [`hermes insights`](/docs/reference/cli-commands#hermes-insights)。
...
3.  加载每个会话的对话，截断到约 100K 字符，并围绕匹配项居中
4.  发送到快速摘要模型以生成聚焦摘要
...
> *“当用户引用过去对话中的内容，或者你怀疑存在相关的先前上下文时，请使用 session_search 来回忆它，然后再要求用户重复。”*
...
- 中断处理也保持按用户隔离，因为运行中的 Agent 密钥与隔离的会话密钥相匹配
...
这将使群组/频道恢复到每个房间一个共享会话，从而保留共享的对话上下文，但也会共享 Token 成本、中断状态和上下文增长。
...
- 在确实删除了行的清理操作之后，`state.db` 会执行 `VACUUM` 以回收磁盘空间（SQLite 在普通的 DELETE 操作后不会缩小文件）
- 清理操作最多每 `sessions.min_interval_hours`（默认 24）运行一次；最后一次运行的时间戳记录在 `state.db` 内部，因此在同一 `HERMES_HOME` 下的每个 Hermes 进程之间共享
...
:::tip
数据库增长缓慢（典型情况：数百个会话约 10-15 MB），并且会话历史记录为跨过去对话的 `session_search` 召回提供支持，因此自动清理功能默认是禁用的。如果你运行繁重的消息网关/定时任务工作负载，并且 `state.db` 显著影响性能（观察到的故障模式：384 MB 的 state.db 包含约 1000 个会话，导致 FTS5 插入和 `/resume` 列表变慢），请启用它。使用 `hermes sessions prune` 进行一次性清理，而无需开启自动扫描。
:::
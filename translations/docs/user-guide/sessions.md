...
- **隐藏**系统消息、工具结果和内部推理
- **限制**为最近 10 次交流，并带有“... N earlier messages ...”指示符
- 使用**暗淡样式**以区别于活跃对话
...
2.  按会话分组结果，取前 N 个（默认 3 个）唯一会话
3.  加载每个会话的对话，截断至约 100K 字符，并围绕匹配项居中
4.  发送到快速摘要模型以生成聚焦摘要
...
| 群组线程/主题 | `agent:main:<platform>:group:<chat_id>:<thread_id>` | 线程所有参与者共享一个会话（默认）。使用 `thread_sessions_per_user: true` 时，每个用户一个会话。 |
...
这将把群组/频道恢复为每个房间共享一个会话，这可以保留共享的对话上下文，但也会共享 Token 成本、中断状态和上下文增长。
...
:::tip
数据库增长缓慢（典型情况：数百个会话占用 10-15 MB）。清除主要用于删除你不再需要用于搜索回忆的旧对话。
:::
...
每一次对话——无论是来自 CLI、Telegram、Discord、Slack、WhatsApp、Signal、Matrix 还是任何其他消息平台——都会作为包含完整消息历史的会话被存储。会话通过两个互补的系统进行追踪：
...
- **隐藏**系统消息、工具结果和内部推理
- **限制**为最后 10 次交互，并带有 "... N earlier messages ..." 指示器
- 使用**暗淡样式**以区别于活跃对话
...
Hermes 会在第一次交互后，自动为每个会话生成一个简短的描述性标题（3–7 个词）。这使用一个快速的辅助模型在后台线程中运行，因此不会增加延迟。当你使用 `hermes sessions list` 或 `hermes sessions browse` 浏览会话时，会看到自动生成的标题。
...
当一个会话的上下文被压缩时（通过 `/compress` 手动或自动），Hermes 会创建一个新的延续会话。如果原始会话有标题，新会话会自动获得一个带编号的标题：
```
"我的项目" → "我的项目 #2" → "我的项目 #3"
```

当你按名称恢复会话时（`hermes -c "我的项目"`），它会自动选择该谱系中最新的会话。
...
这将把群组/频道恢复为每个房间共享一个会话，这保留了共享的对话上下文，但也共享了 Token 成本、中断状态和上下文增长。
...
- 在确实删除了行的清理操作之后，`state.db` 会被 `VACUUM` 以回收磁盘空间（SQLite 不会在普通 DELETE 操作后缩小文件）
- 清理操作最多每 `sessions.min_interval_hours`（默认 24）运行一次；上次运行的时间戳在 `state.db` 内部跟踪，因此在同一 `HERMES_HOME` 下的每个 Hermes 进程之间共享

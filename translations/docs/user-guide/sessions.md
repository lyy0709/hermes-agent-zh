...
*   附件路径和提取/派生的文本可以出现在记录中，但原始图像、音频或二进制文件字节不会重复复制到未来的提示词中。
...
上下文增长最常见的原因不是媒体文件本身。而是冗长的文本：粘贴的转录稿、完整的日志、大型工具输出、长差异对比、重复的状态报告以及详细的证明转储。相比于将大型工件复制到聊天中，更推荐使用摘要、文件路径、重点摘录和基于工具的查找。

:::tip
当会话变长时使用 `/compress`，需要新线程时使用 `/new`，而 `hermes sessions prune` 仅在你想要从存储中删除旧的已结束会话时使用。压缩会减少活动上下文；它不是隐私删除。
向 `/new` 传递一个名称（例如 `/new payments-refactor`）可以预先设置新会话的初始标题——这对于稍后使用 `/resume <name>` 或在 `/sessions` 选择器中找到它很有用。
:::
...
*   使用**暗淡的样式**以区别于活动对话
要禁用完整回顾并保持简洁的一行摘要行为，请在 `~/.hermes/config.yaml` 中设置：
...
:::tip
会话 ID 的格式为 `YYYYMMDD_HHMMSS_<hex>` — CLI/TUI 会话使用 6 位十六进制后缀（例如 `20250305_091523_a1b2c3`），消息网关会话使用 8 位后缀（例如 `20250305_091523_a1b2c3d4`）。您可以通过 ID（完整或唯一前缀）或标题来恢复会话 — 两者都适用于 `-c` 和 `-r` 参数。
:::
...
2.  CLI 将会话标记为待处理状态并**阻塞轮询消息网关**。如果 Agent 正在响应中，CLI 会拒绝操作 — 请等待当前响应完成。
3.  消息网关的监听器认领交接任务，并向目标适配器请求一个新线程：
    ...
    *   **WhatsApp / Signal / Matrix / SMS** — 没有原生线程支持，直接回退到主频道。
4.  消息网关将目标键重新绑定到您现有的 CLI 会话 ID，然后伪造一个合成的用户回合，要求 Agent 确认并总结。回复将出现在新线程中。
...
*   平台未启用 / 消息网关未运行 → CLI 在 60 秒后超时，并显示明确消息，您的 CLI 会话保持不变。
*   线程创建失败（权限不足、主题模式关闭） → 直接回退到主频道，交接仍能完成；虽然没有线程隔离，但交接本身有效。
*   `adapter.send` 失败（速率限制、瞬时 API 错误） → 交接被标记为失败并附原因；该行记录会被清除，以便您重试。
...
Hermes 会在首次交流后自动为每个会话生成一个简短的描述性标题（3–7 个词）。这使用快速的辅助模型在后台线程中运行，因此不会增加延迟。当使用 `hermes sessions list` 或 `hermes sessions browse` 浏览会话时，您会看到自动生成的标题。
...
标题会立即应用。如果会话尚未在数据库中创建（例如，您在发送第一条消息之前运行了 `/title`），则会被排队，并在会话开始时应用。
...
当您通过名称恢复会话时（`hermes -c "my project"`），它会自动选择谱系中最新的会话。
...
要进行更深入的分析——Token 使用量、成本估算、工具使用细分和活动模式——请使用 [`hermes insights`](/reference/cli-commands#hermes-insights)。

## 会话搜索工具

Agent 内置了一个 `session_search` 工具，它使用 SQLite 的 FTS5 引擎对所有过去的对话进行全文搜索——并允许 Agent 滚动浏览它找到的任何会话。无需 LLM 调用，无需摘要，无需截断。每次调用都返回数据库中的实际消息。
...
- `messages` — FTS5 匹配项周围约 ±5 条消息，并标记锚点消息（上下文中的命中项）
- `bookend_end` — 会话的最后 3 条用户+助手消息（解决方案/决策）
- `match_message_id`, `messages_before`, `messages_after`

书签 + 窗口共同重构了 目标 → 匹配 → 解决方案，而无需支付整个转录的成本。在实际会话数据库上的典型耗时：15–50 毫秒。
...
返回以锚点为中心、±`window` 条消息的窗口。没有 FTS5，没有书签——只有切片。当你在发现调用后需要比默认的 ±5 窗口更多的上下文时使用。
...
- 简单关键字：`docker deployment`（FTS5 默认为 AND）
- 短语：`"exact phrase"`
...
- `sort` — `newest` 或 `oldest`，在 FTS5 排名之上。省略则仅按相关性排序（默认值；适用于探索性回忆）。使用 `newest` 处理“我们上次把 X 留在了哪里”的问题，使用 `oldest` 处理“X 是如何开始的”问题。
...
典型触发词：“我们之前做过这个”、“记得当时”、“上次”、“正如我提到的”，或者任何引用不在当前窗口中的项目/人员/概念。
...
| 群聊 | `agent:main:<platform>:group:<chat_id>:<user_id>` | 当平台暴露用户 ID 时，群内按用户划分 |
| 群组线程/主题 | `agent:main:<platform>:group:<chat_id>:<thread_id>` | 所有线程参与者共享一个会话（默认）。使用 `thread_sessions_per_user: true` 时按用户划分。 |
| 频道 | `agent:main:<platform>:channel:<chat_id>:<user_id>` | 当平台暴露用户 ID 时，频道内按用户划分 |
...
这将使群组/频道恢复为每个房间一个共享会话，这保留了共享的对话上下文，但也共享了 Token 成本、中断状态和上下文增长。
...
:::note 遗留的 JSONL 对话记录
在 state.db 成为规范存储之前创建的会话可能在 `~/.hermes/sessions/` 中残留有 `*.jsonl` 文件。Hermes 不再写入或读取这些文件。在验证相应的会话存在于 state.db 中后，可以安全地删除它们。
:::
...
- 清理操作最多每 `sessions.min_interval_hours`（默认 24）小时运行一次；最后一次运行的时间戳在 `state.db` 内部跟踪，因此在同一 `HERMES_HOME` 下的每个 Hermes 进程之间共享

默认是**关闭**的 — 会话历史对于 `session_search` 召回很有价值，静默删除可能会让用户感到意外。在 `~/.hermes/config.yaml` 中启用：

...
1.  **SQLite 数据库** (`~/.hermes/state.db`) —— 存储结构化的会话元数据，并支持 FTS5 全文搜索
2.  **JSONL 转录文件** (`~/.hermes/sessions/`) —— 存储原始的对话转录，包括工具调用（消息网关）
...
Hermes 存储会话历史以便恢复对话，但它不会持续重新发送它曾经处理过的每一个字节。在每一轮交互中，模型看到的是选定的系统提示词、当前的对话窗口，以及 Hermes 为该轮交互明确注入的任何内容。
...
导致上下文增长的最常见原因不是媒体文件本身。而是冗长的文本：粘贴的转录、完整的日志、大量的工具输出、冗长的差异对比、重复的状态报告以及详细的证明转储。相比于将大型工件复制到聊天中，更推荐使用摘要、文件路径、重点摘录和基于工具的查找。
...
- 使用**暗淡的样式**以区别于活动对话
要禁用完整回顾并保持最小化单行摘要行为，请在 `~/.hermes/config.yaml` 中设置：
...
:::tip
会话 ID 遵循格式 `YYYYMMDD_HHMMSS_<hex>` — CLI/TUI 会话使用 6 字符十六进制后缀（例如 `20250305_091523_a1b2c3`），消息网关会话使用 8 字符后缀（例如 `20250305_091523_a1b2c3d4`）。你可以通过 ID（完整或唯一前缀）或标题恢复会话 — 两者都适用于 `-c` 和 `-r` 参数。
:::

## 跨平台交接

在 CLI 会话中使用 `/handoff <platform>` 将实时对话转移到消息平台的归属频道。Agent 会从 CLI 中断的地方无缝接续 — 相同的会话 ID、完整的角色感知转录、工具调用等所有内容。
...
1. CLI 验证 `<platform>` 已启用并设置了归属频道（在目标聊天中运行一次 `/sethome` 进行配置）。
2. CLI 将会话标记为待处理状态并**阻塞轮询消息网关**。如果 Agent 正在响应中，则拒绝请求 — 请等待当前响应完成。
3. 消息网关监听器认领交接任务，并向目标适配器请求新线程：
   - **Telegram** — 开启一个新论坛话题（如果聊天中启用了 Bot API 9.4+ 的话题模式，则为私聊话题；或论坛超级群组话题）。
...
5. 当消息网关确认成功后，CLI 打印一个 `/resume` 提示并干净退出：

   ```
   ↻ 交接完成。会话现已在 telegram 上激活。
     稍后可通过以下命令在此 CLI 恢复：/resume my-session-title
   ```
...
- 平台未启用 / 消息网关未运行 → CLI 在 60 秒后超时，显示明确消息，你的 CLI 会话保持原状。
- 线程创建失败（权限不足、话题模式关闭） → 直接回退到归属频道，交接仍能完成；虽无线程隔离，但交接本身有效。
- `adapter.send` 失败（速率限制、临时 API 错误） → 交接标记为失败并附原因；该行记录会被清除，以便重试。

**值得了解的局限性：** 对于不支持线程且拥有多用户群组归属频道的平台，合成回合会以私聊风格会话进行键控。这对于自用私聊归属频道（典型设置）有效，但对于真正共享的群组聊天并不理想。线程功能覆盖了 Telegram / Discord / Slack — 这是最常见的情况 — 因此大多数设置不会遇到此问题。
...
Hermes 在首次交流后会自动为每个会话生成一个简短的描述性标题（3–7 个词）。这使用快速辅助模型在后台线程中运行，因此不会增加延迟。使用 `hermes sessions list` 或 `hermes sessions browse` 浏览会话时，你会看到自动生成的标题。
...
标题会立即应用。如果会话尚未在数据库中创建（例如，在发送第一条消息之前运行 `/title`），则会被排队，并在会话启动后应用。
...
当你通过名称恢复会话时（`hermes -c "my project"`），它会自动选择谱系中最新的会话。
...
2.  按会话对结果进行分组，取前 N 个（默认 3 个）唯一的会话
3.  加载每个会话的对话内容，截取约 100K 字符，内容围绕匹配项展开
4.  发送到快速摘要模型以生成聚焦的摘要
...
> *“当用户引用过去对话中的内容，或者你怀疑存在相关的先前上下文时，请使用 session_search 来回忆，而不是要求用户重复。”*
...
| 群组线程/话题 | `agent:main:<platform>:group:<chat_id>:<thread_id>` | 所有线程参与者共享一个会话（默认）。使用 `thread_sessions_per_user: true` 时为每个用户一个会话。 |
...
| 消息网关索引 | `~/.hermes/sessions/sessions.json` | 将会话密钥映射到活跃会话 ID |

SQLite 数据库使用 WAL 模式，支持并发读取和单个写入，这非常适合消息网关的多平台架构。
### 数据库模式

`state.db` 中的关键表：

- **sessions** — 会话元数据（id、source、user_id、model、title、时间戳、Token 计数）。标题有唯一索引（允许 NULL 标题，仅非 NULL 标题必须唯一）。
...
- 消息网关会话会根据配置的重置策略自动重置
- 在重置之前，Agent 会保存即将过期会话的记忆和技能
- 选择启用的自动清理：当 `sessions.auto_prune` 为 `true` 时，早于 `sessions.retention_days`（默认 90 天）的已结束会话会在 CLI/消息网关启动时被清理
- 在确实删除了行的清理操作之后，会对 `state.db` 执行 `VACUUM` 以回收磁盘空间（SQLite 在普通的 DELETE 操作后不会缩小文件大小）
- 清理操作最多每 `sessions.min_interval_hours`（默认 24 小时）运行一次；最后一次运行的时间戳记录在 `state.db` 内部，因此在同一 `HERMES_HOME` 下的每个 Hermes 进程之间共享

默认情况下**关闭** — 会话历史记录对于 `session_search` 的召回很有价值，静默删除可能会让用户感到意外。在 `~/.hermes/config.yaml` 中启用：
...
:::tip
数据库增长缓慢（典型情况：数百个会话占用 10-15 MB），并且会话历史记录支持跨过去对话的 `session_search` 召回功能，因此自动清理功能默认是禁用的。如果你运行的是繁重的消息网关/定时任务工作负载，并且 `state.db` 显著影响了性能（观察到的故障模式：384 MB 的 state.db 包含约 1000 个会话，导致 FTS5 插入和 `/resume` 列表变慢），请启用此功能。使用 `hermes sessions prune` 进行一次性清理，而无需开启自动扫描。
:::
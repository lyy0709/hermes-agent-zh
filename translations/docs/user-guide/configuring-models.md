...
本页介绍如何通过仪表板配置这两类模型。如果您更喜欢配置文件或 CLI，请跳至底部的[替代方法](#替代方法)。

:::tip 最快路径：Nous Portal
[Nous Portal](/docs/user-guide/features/tool-gateway) 在一个订阅下提供 300 多个模型。在新安装后，运行 `hermes setup --portal` 登录并一键将 Nous 设置为您的提供商。使用 `hermes portal status` 检查已连接的配置。
:::
...
2. **使用情况分析** — 排名卡片，显示所选时间段内运行过会话的每个模型，包含 Token 数量、成本和能力徽章。
...
点击任意辅助行上的 **Change**。会打开相同的选择器，行为相同 — 选择提供商 + 模型，点击 Switch。该行将更新为显示 `provider · model` 而不是 `auto (use main model)`。
...
Hermes 仅在有有效凭据时才列出提供商。检查侧边栏的 **Keys** — 您应该看到以下之一：API 密钥、成功的 OAuth 或自定义端点 URL。如果您想要的提供商不在那里，请运行 `hermes setup` 进行连接，或前往 **Keys** 添加环境变量。

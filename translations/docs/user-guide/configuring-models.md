...
[Nous Portal](/user-guide/features/tool-gateway) 在一个订阅下提供 300 多个模型。在新安装后，运行 `hermes setup --portal` 以登录并一键将 Nous 设置为您的提供商。使用 `hermes portal status` 检查已连接的内容。
...
curl -X POST -H "Content-Type: application/json" -H "X-Hermes-Session-Token: $TOKEN" \
  ...
  http://localhost:PORT/api/model/set
```
会话 Token 会在启动时注入到仪表盘 HTML 中，并在每次服务器重启时轮换。如果你正在针对运行中的仪表盘编写脚本，可以从浏览器开发者工具中获取它（`window.__HERMES_SESSION_TOKEN__`）。
...
- **辅助模型** — Agent 卸载的较小辅助任务。包括上下文压缩、视觉（图像分析）、网页摘要、批准评分、MCP 工具路由、会话标题生成和技能搜索。每个任务都有自己的槽位，可以独立覆盖。

本页介绍如何通过仪表板配置这两种模型。如果您更喜欢配置文件或 CLI，请直接跳到底部的[替代方法](#替代方法)。
...
2. **使用分析** — 显示在选定时间段内运行过会话的每个模型的排名卡片，包含 Token 数量、成本和能力徽章。
...
- **左侧** — 已认证的提供商。只有您已设置好的提供商（设置了 API 密钥、完成 OAuth 或定义为自定义端点）才会显示在此处。如果缺少某个提供商，请前往 **Keys** 添加其凭据。
...
选择一个模型，点击 **Switch**，Hermes 会将其写入 `~/.hermes/config.yaml` 的 `model` 部分。**这仅适用于新会话** — 您已打开的任何聊天标签页将继续使用其启动时的模型。要热交换当前聊天，请在其中使用 `/model` 斜杠命令。
...
| **压缩** | 当您为了总结上下文而在 Opus/M2.7 上消耗推理 Token 时。一个快速的聊天模型可以以 1/50 的成本完成这项工作。 |
...
在任何辅助行上点击 **Change**。会打开相同的选择器，行为相同 — 选择提供商 + 模型，点击 Switch。该行将更新为显示 `provider · model` 而不是 `auto (use main model)`。
...
当卡片当前被分配到某个任务时，会带有 `main` 或 `aux · <task>` 徽章 — 这样您一眼就能看出您历史模型中的哪些被用在了哪里。
...
- **仪表板聊天标签页** (`/chat`)：下一个新的 PTY。当前打开的聊天保持其模型 — 在其中使用 `/model` 进行热交换。
...
Hermes 仅在有有效凭据时才列出提供商。检查侧边栏中的 **Keys** — 您应该看到以下之一：API 密钥、成功的 OAuth 或自定义端点 URL。如果您想要的提供商不在那里，请运行 `hermes setup` 来连接它，或转到 **Keys** 并添加环境变量。

### 我运行中的聊天主模型没有改变

这是预期的。仪表板写入 `config.yaml`，新会话会读取它。当前打开的聊天是一个实时的 Agent 进程 — 它保持其启动时的模型。在聊天中使用 `/model <name>` 来热交换该特定会话。
...
或在 shell 中（简短形式，`provider/model`）：
...
`hermes model` 引导您选择提供商、进行认证（OAuth 流程会打开浏览器；API 密钥提供商会提示输入密钥），然后从该提供商的精选目录中选择特定模型。选择结果会写入 `~/.hermes/config.yaml` 中的 `model.provider` 和 `model.model`。
...
会话 Token 在启动时注入到仪表板 HTML 中，并在每次服务器重启时轮换。如果您正在针对运行中的仪表板编写脚本，请从浏览器开发者工具中获取它（`window.__HERMES_SESSION_TOKEN__`）。
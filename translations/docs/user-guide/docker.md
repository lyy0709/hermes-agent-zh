...
- **避免并发写入风险** — 上面关于切勿针对同一数据目录运行两个网关的警告，同样适用于单个容器内的配置文件。
...
容器默认以 root 用户运行。如果你的主机 `~/.hermes/` 目录是由非 root 用户创建的，权限应该可以正常工作。如果出现错误，请确保数据目录可写：

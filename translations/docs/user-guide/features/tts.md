...
**Groq API** — 需要 `GROQ_API_KEY`。当您想要免费的托管 STT 选项时，是良好的云端回退方案。
...
**xAI Grok STT** — 需要 `XAI_API_KEY`。以 multipart/form-data 格式发送到 `https://api.x.ai/v1/stt`。如果您已经在使用 xAI 进行聊天或 TTS，并且希望一个 API 密钥处理所有事情，这是一个不错的选择。自动检测顺序将其放在 Groq 之后 — 显式设置 `stt.provider: xai` 以强制使用它。

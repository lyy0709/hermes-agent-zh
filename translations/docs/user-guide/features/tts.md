...
sidebar_position: 9
title: "语音与 TTS"
description: "支持所有平台的文本转语音和语音消息转录"
---
...
**速度控制**：默认情况下，全局的 `tts.speed` 值适用于所有提供商。每个提供商都可以用自己的 `speed` 设置覆盖它（例如，`tts.openai.speed: 1.5`）。特定于提供商的速度设置优先于全局值。默认值为 `1.0`（正常速度）。
...
如果没有安装 ffmpeg，Edge TTS、MiniMax TTS、NeuTTS、KittenTTS 和 Piper 的音频将作为常规音频文件发送（可播放，但显示为矩形播放器而非语音气泡）。
...
如果您想要的 TTS 引擎不受原生支持（VoxCPM、MLX-Kokoro、XTTS CLI、语音克隆脚本，任何暴露 CLI 的工具），您可以将其配置为 **命令类型提供商**，而无需编写任何 Python 代码。Hermes 将输入文本写入一个临时的 UTF-8 文件，运行您的 shell 命令，并读取命令生成的音频文件。

在 `tts.providers.<name>` 下声明一个或多个提供商，并通过 `tts.provider: <name>` 在它们之间切换 —— 就像在 `edge` 和 `openai` 等内置提供商之间切换一样。
...
命令类型的提供商会运行您配置的任何 shell 命令，并具有您的用户权限。Hermes 会引用占位符值并强制执行配置的超时时间，但命令模板本身是受信任的本地输入——请像对待您 PATH 上的 shell 脚本一样对待它。

### Python 插件提供商

对于无法表示为单个 shell 命令的 TTS 引擎——没有 CLI 的 Python SDK、流式引擎、语音列表 API、需要 OAuth 刷新的身份验证——可以通过 `ctx.register_tts_provider()` 注册 Python 插件。该插件**与**[自定义命令提供商](#custom-command-providers)注册表**共存**（不会替换）；选择适合您引擎的接口。
...
内置提供商始终优先，命令提供商优先于同名的插件——因此插件可以安全地注册到任何非内置名称，而无需担心覆盖您现有的配置。
...
启用它（`hermes plugins enable my-tts`），将 `tts.provider` 指向它（在 `config.yaml` 中设置 `tts.provider: my-tts`），`text_to_speech` 工具将通过您的插件路由。
...
- `get_setup_schema()` → 返回 `{name, badge, tag, env_vars: [{key, prompt, url}]}` 以支持 `hermes tools` / `hermes setup` 中的选择器行。如果没有这个，插件仍然有效，但其在选择器中的行信息最少。
- `stream(text, *, voice, model, format, **extra)` → 迭代器，为流式交付生成音频字节（默认引发 `NotImplementedError`）。
- `voice_compatible` 属性 → 如果您的输出与 Opus 兼容且消息网关应将其作为语音气泡交付，则设置为 `True`（默认 `False` = 常规音频附件）。

有关完整的抽象基类（包括文档字符串），请参阅 `agent/tts_provider.py`。

## 语音消息转录 (STT)

在 Telegram、Discord、WhatsApp、Slack 或 Signal 上发送的语音消息会自动转录，并将文本注入到对话中。Agent 将转录文本视为普通文本。
| 提供商 | 质量 | 成本 | API 密钥 |
|----------|---------|------|---------|
...
:::info 零配置
当 `faster-whisper` 安装后，本地转录功能开箱即用。如果不可用，Hermes 也可以使用来自常见安装位置（如 `/opt/homebrew/bin`）的本地 `whisper` CLI，或通过 `HERMES_LOCAL_STT_COMMAND` 使用自定义命令。
:::
...
**Groq API** — 需要 `GROQ_API_KEY`。当您需要一个免费的托管 STT 选项时，是一个很好的云端备选方案。

**OpenAI API** — 优先接受 `VOICE_TOOLS_OPENAI_KEY`，否则回退到 `OPENAI_API_KEY`。支持 `whisper-1`、`gpt-4o-mini-transcribe` 和 `gpt-4o-transcribe`。
...
**xAI Grok STT** — 需要 `XAI_API_KEY`。以 multipart/form-data 格式发送到 `https://api.x.ai/v1/stt`。如果您已经在使用 xAI 进行聊天或 TTS，并希望所有功能使用一个 API 密钥，这是一个不错的选择。自动检测顺序将其排在 Groq 之后 — 显式设置 `stt.provider: xai` 来强制使用它。
...
- **没有可用的** → 语音消息会原样通过，并向用户发送准确的说明

### STT 自定义命令提供商

如果您想要的 STT 引擎不受原生支持（Doubao ASR、NVIDIA Parakeet、whisper.cpp 构建、开源 SenseVoice CLI，任何暴露 shell 命令的工具），您可以将其配置为**命令类型提供商**，而无需编写任何 Python 代码。Hermes 会针对音频文件运行您的 shell 命令并读回转录文本。

在 `stt.providers.<name>` 下声明一个或多个提供商，并通过 `stt.provider: <name>` 在它们之间切换 — 其结构与 TTS 的[命令提供商注册表](#custom-command-providers)相同，但适配了输入=音频 → 输出=转录的方向。
...
您的命令模板可以引用这些占位符。Hermes 会在渲染时替换它们，并为每个值根据周围的上下文（裸值 / 单引号 / 双引号）进行 shell 引用，因此带有空格的路径是安全的。
...
这让你可以使用注册表来处理既写入文件的 CLI（`whisper-cli`、`parakeet-asr`）也处理输出转录文本到 stdout 的 curl 风格单行命令（`curl … | jq -r .text`）。
...
| `language`      | `en`    | 转发给 `{language}`。默认为 `stt.language`，然后 `en`。                                     |
| `model`         | 空值   | 转发给 `{model}`。`transcribe_audio()` 的 `model=` 参数会覆盖此值。                |

#### STT 命令提供商行为说明

- **内置项始终优先。** 声明 `stt.providers.openai: type: command` 不会覆盖真正的 OpenAI Whisper 处理器。内置名称在命令提供商解析器运行之前就被短路处理了。
- **进程树清理。** 运行超过 `timeout` 的命令，其整个进程树（不仅仅是 shell 包装器）都会被终止。长时间运行的 ASR 流水线如果 fork 了模型加载子进程，也能被可靠地回收。
- **Shell 引用是自动的。** `'…'` 内的占位符会进行单引号安全转义；`"…"` 内的占位符会进行 `$`/`` ` ``/`"` 转义；引号外的占位符会进行 `shlex.quote` 转义。不要预先引用占位符的值。

#### STT 命令提供商安全性

shell 命令在与 Hermes 相同的用户下运行，拥有完整的文件系统访问权限——与 `tts.providers.<name>: type: command` 和 `HERMES_LOCAL_STT_COMMAND` 的信任模型相同。只声明你信任的来源的命令提供商。

### Python 插件提供商 (STT)

对于既非内置、也无法用 shell 命令表达的 STT 引擎（需要 Python SDK、OAuth 刷新认证、流式分块等），可以通过 `ctx.register_transcription_provider()` 注册一个 Python 插件。该插件与 6 个内置提供商（`local`、`local_command`、`groq`、`openai`、`mistral`、`xai`）以及 `stt.providers.<name>: type: command` 注册表**共存**——内置项保留其原生实现，并且在名称冲突时始终优先；命令提供商在同名插件之前优先（配置比插件安装更本地化）。

#### 何时选择哪种 (STT)

| 后端具有…                                                 | 使用                                                              |
|--------------------------------------------------------------|------------------------------------------------------------------|
| 一个接受音频文件并输出文本的单一 shell 命令 | `stt.providers.<name>: type: command`（无需 Python）        |
| 只需要传统的单命令应急方案        | `HERMES_LOCAL_STT_COMMAND` 环境变量（为向后兼容保留）  |
| 没有 CLI 的 Python SDK                                     | `register_transcription_provider()` 插件                      |
| OAuth 刷新认证、流式分块、语音列表元数据 | `register_transcription_provider()` 插件                      |
| 已有内置项覆盖（`local`、`groq`、`openai`、…）  | 设置 `stt.provider: <name>` —— 内置项是内联的               |

#### 解析顺序

1. **`stt.provider` 是内置名称** → 内置分发。**始终优先。**
2. **`stt.provider` 匹配到设置了 `command:` 的 `stt.providers.<name>`** → 命令提供商运行器（参见 [STT 自定义命令提供商](#stt-custom-command-providers)）。在同名插件之前优先。
...
插件从 `config.yaml` 中的 `stt.<provider>` 读取其每个提供商的配置，这与内置项读取 `stt.openai.model` / `stt.mistral.model` 的方式类似：
...
stt:
  ...
  my-stt:
    ...
    language: ja          # 作为 language= 转发给 transcribe()
...
class MySTTProvider(TranscriptionProvider):
    ...
    def transcribe(self, file_path, *, model=None, language=None, **extra):
        ...
        # 不要抛出异常 —— 将异常转换为错误信封，这样
        # gateway/CLI 调用者在失败时能看到一致的形状。
        try:
            import my_stt_sdk
...
- `get_setup_schema()` → 返回 `{name, badge, tag, env_vars: [{key, prompt, url}]}`，用于在 `hermes tools` / `hermes setup` 中驱动选择器行（STT 的选择器类别尚未发布——此元数据可供插件使用，以实现向前兼容）。

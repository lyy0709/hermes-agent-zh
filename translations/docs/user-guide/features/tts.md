...
**速度控制**：全局的 `tts.speed` 值默认应用于所有提供商。每个提供商都可以用自己的 `speed` 设置覆盖它（例如，`tts.openai.speed: 1.5`）。提供商特定的速度优先于全局值。默认值为 `1.0`（正常速度）。
...
如果没有 ffmpeg，Edge TTS、MiniMax TTS、NeuTTS 和 KittenTTS 的音频将作为常规音频文件发送（可播放，但显示为矩形播放器而不是语音气泡）。
...
:::info 零配置
当安装了 `faster-whisper` 时，本地转录开箱即用。如果不可用，Hermes 也可以从常见安装位置（如 `/opt/homebrew/bin`）使用本地的 `whisper` CLI，或通过 `HERMES_LOCAL_STT_COMMAND` 使用自定义命令。
:::

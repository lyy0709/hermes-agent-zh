...
如果没有安装 ffmpeg，Edge TTS、MiniMax TTS 和 NeuTTS 的音频将作为常规音频文件发送（可播放，但显示为矩形播放器而非语音气泡）。
...
:::info 零配置
当安装了 `faster-whisper` 时，本地转录功能开箱即用。如果不可用，Hermes 也可以从常见安装位置（如 `/opt/homebrew/bin`）使用本地 `whisper` CLI，或通过 `HERMES_LOCAL_STT_COMMAND` 使用自定义命令。
:::

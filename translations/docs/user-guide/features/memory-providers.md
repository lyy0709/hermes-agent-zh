...
具有辩证推理、会话范围上下文注入、语义搜索和持久化结论的 AI 原生跨会话用户建模。基础上下文现在包含会话摘要以及用户表示和同伴卡片，使 Agent 能够了解已经讨论过的内容。
...
**架构:** 两层上下文注入 —— 基础层（会话摘要 + 表示 + 同伴卡片，根据 `contextCadence` 刷新）加上辩证补充层（LLM 推理，根据 `dialecticCadence` 刷新）。辩证层根据是否存在基础上下文，自动选择冷启动提示词（通用用户事实）或热启动提示词（会话范围上下文）。
...
**设置向导:**
```bash
hermes memory setup        # 选择 "honcho" — 运行 Honcho 特定的后设置
```
...
Honcho 将会话建模为交换消息的对等体——每个 Hermes 配置文件有一个用户对等体加上一个 AI 对等体，所有对等体共享一个工作区。工作区是共享的执行环境：用户对等体在所有配置文件中是全局的，每个 AI 对等体是其独立的身份。每个 AI 对等体都根据自己的观察构建独立的表示/卡片，因此 `coder` 配置文件保持面向代码，而 `writer` 配置文件在相同的用户面前保持面向编辑。

映射关系：

| 概念 | 含义 |
|---------|-----------|
| **工作区** | 共享的执行环境。一个工作区下的所有 Hermes 配置文件都看到相同的用户身份。 |
...
| 开关 | 效果 |
|--------|--------|
| `observeMe` | Honcho 从该对等体自己的消息中构建其表示 |
| `observeOthers` | 该对等体观察另一个对等体的消息（为跨对等体推理提供信息） |
...
由 Volcengine（字节跳动）开发的上下文数据库，具有文件系统风格的知识层次结构、分层检索以及自动将记忆提取为 6 个类别的功能。
...
具备知识图谱、实体解析和多策略检索的长期记忆。`hindsight_reflect` 工具提供跨记忆综合功能，这是其他提供商所不具备的。自动保留完整的对话轮次（包括工具调用），并具有会话级别的文档跟踪。
...
| `retain_context` | `conversation between Hermes Agent and the User` | 用于已保留记忆的上下文标签 |
| `retain_tags` | — | 应用于已保留记忆的默认标签；与每次调用工具时的标签合并 |
...
完整配置参考请参阅 [插件 README](https://github.com/NousResearch/hermes-agent/blob/main/plugins/memory/hindsight/README.md)。
...
| **需要** | 无需（SQLite 始终可用）。NumPy 可选，用于 HRR 代数运算。 |
| **数据存储** | 本地 SQLite |
| **成本** | 免费 |
...
| `profile_frequency` | `50` | 在首次对话轮次及之后每 N 轮次包含画像事实 |
...
- **多容器模式** —— 启用 `enable_custom_container_tags` 并设置 `custom_containers` 列表，以允许 Agent 跨命名容器进行读写。自动操作（同步、预取）保持在主容器上。
...
- **云端提供商**（RetainDB）自动派生按配置文件作用域的项目名称

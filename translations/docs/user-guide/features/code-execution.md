...
**何时切换到 `strict` 模式：** 你需要最大的可重现性 —— 无论用户激活了哪个虚拟环境，你都希望每次会话使用相同的解释器，并且希望脚本与项目树隔离（没有通过相对路径意外读取项目文件的风险）。
...
| **标准输出** | 50 KB | 输出被截断，并显示 `[output truncated at 50KB]` 通知 |
...
这意味着脚本内部的工具调用行为与普通工具调用完全相同 —— 相同的速率限制、相同的错误处理、相同的能力。唯一的限制是 `terminal()` 仅限前台（没有 `background` 或 `pty` 参数）。

## 错误处理

当脚本失败时，Agent 会收到结构化的错误信息：

-   **非零退出码：** 标准错误包含在输出中，因此 Agent 可以看到完整的回溯
...
:::danger 安全模型
子进程在**最小化环境**中运行。默认情况下，API 密钥、Token 和凭证会被剥离。脚本只能通过 RPC 通道访问工具 —— 除非明确允许，否则它无法从环境变量中读取密钥。
:::
...
Hermes 总是将脚本和自动生成的 `hermes_tools.py` RPC 存根写入一个在执行后会被清理的临时暂存目录。在 `strict` 模式下，脚本也在那里*运行*；在 `project` 模式下，它在会话的工作目录中运行（暂存目录保留在 `PYTHONPATH` 上，因此导入仍然可以解析）。子进程在其自己的进程组中运行，以便在超时或中断时可以干净地终止。
...
**经验法则：** 当你需要以编程方式调用 Hermes 工具并在调用之间有逻辑时，使用 `execute_code`。对于运行 shell 命令、构建和进程，使用 `terminal`。

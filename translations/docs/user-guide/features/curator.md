...
技能策展器是针对 **Agent 创建技能** 的后台维护流程。它跟踪每个技能的查看、使用和修补频率，将长期未使用的技能在 `活跃 → 陈旧 → 已归档` 状态间移动，并定期启动一个简短的辅助模型审查，提出合并或修补偏离的建议。
...
策展器 **绝不触碰** 捆绑技能（随仓库发布）或从 Hub 安装的技能（来自 [agentskills.io](https://agentskills.io)）。它只审查 Agent 自身创建的技能。它也 **绝不自动删除** ——最坏的结果是归档到 `~/.hermes/skills/.archive/`，这是可恢复的。
...
1. 距离上次策展器运行是否已过去足够时间（`interval_hours`，默认 **7 天**），以及
2. Agent 是否已空闲足够长时间（`min_idle_hours`，默认 **2 小时**）。

如果两者都为真，它会启动一个 `AIAgent` 的后台分支——这与记忆/技能自我改进提示使用的模式相同。该分支在其自己的提示缓存中运行，绝不接触活跃的对话。

一次运行包含两个阶段：

1.  **自动状态转换**（确定性的，无需 LLM）。未使用超过 `stale_after_days`（30 天）的技能变为 `陈旧`；未使用超过 `archive_after_days`（90 天）的技能被移动到 `~/.hermes/skills/.archive/`。
...
curator:
  ...
  auxiliary:
    provider: null             # null = 使用主辅助客户端解析
    model: null
```

要完全禁用，请设置 `curator.enabled: false`。

要为 LLM 审查阶段使用比主模型更便宜的辅助模型，请将 `curator.auxiliary.provider` 和 `curator.auxiliary.model` 设置为特定值（例如 `openrouter` + `google/gemini-3-flash-preview`）。
...
hermes curator run --sync     # 同上，但阻塞直到 LLM 阶段完成
...
`hermes curator status` 还会列出五个最近最少使用的技能——这是快速查看哪些技能可能即将变为陈旧的方法。
...
`~/.hermes/skills/` 中的所有其他内容都可供策展器处理。这包括：

- Agent 在对话期间通过 `skill_manage(action="create")` 保存的技能。
- 您手动编写的 `SKILL.md` 创建的技能。
- 通过您为 Hermes 指定的外部技能目录添加的技能。

如果您想保护某个特定技能不被触碰——例如您依赖的手动编写的技能——请使用 `hermes curator pin <name>`。请参阅下一节。

## 固定技能

固定是针对自动化和 Agent 驱动更改的硬性防护。一旦技能被固定：

- **策展器** 在自动状态转换（`活跃 → 陈旧 → 已归档`）期间跳过它，并且其 LLM 审查阶段被指示不要动它。
...
该标志作为 `"pinned": true` 存储在技能在 `~/.hermes/skills/.usage.json` 中的条目上，因此会在会话间持续存在。

只有 **Agent 创建** 的技能可以被固定——捆绑和从 Hub 安装的技能首先就不受策展器变更的影响，如果您尝试固定它们，`hermes curator pin` 会拒绝并给出解释性消息。

如果您需要自己更新一个固定的技能，请直接用编辑器编辑 `~/.hermes/skills/<name>/SKILL.md`。固定只保护 Agent 的工具路径，而不限制您自己的文件系统访问。

## 使用遥测

策展器在 `~/.hermes/skills/.usage.json` 维护一个伴随文件，每个技能有一个条目：
...
`REPORT.md` 是快速查看给定运行所做工作的方式——哪些技能转换了状态，LLM 审查者说了什么，修补了哪些技能。对于无需 grep `agent.log` 的审计很有用。
...
这将技能从 `~/.hermes/skills/.archive/` 移回活跃树，并将其状态重置为 `活跃`。如果此后有捆绑或从 Hub 安装的技能以相同名称安装（会遮蔽上游），恢复操作将拒绝执行。

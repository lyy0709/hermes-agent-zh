...
sidebar_position: 3
title: "技能策展器"
description: "为 Agent 创建的技能提供后台维护——使用跟踪、陈旧度管理、归档和 LLM 驱动的审查"
---

# 技能策展器

技能策展器是 **Agent 创建技能** 的后台维护流程。它跟踪每个技能的查看、使用和修补频率，将长期未使用的技能在 `活跃 → 陈旧 → 归档` 状态间移动，并定期启动一个简短的辅助模型审查，以提出合并或修补偏离的建议。
...
如果两者都为真，它会启动一个 `AIAgent` 的后台分支——这与记忆/技能自我改进提示所使用的模式相同。该分支在其自己的提示词缓存中运行，绝不触及活跃的会话。

:::info 首次运行行为
在全新安装（或在 `hermes update` 后，预策展器安装的首次触发）时，策展器 **不会立即运行**。首次观察会将 `last_run_at` 设置为“现在”，并将第一次真正的运行推迟一个完整的 `interval_hours`。这为您提供了一个完整的间隔时间来审查您的技能库，固定任何重要内容，或在策展器触及它们之前完全选择退出。
...
1.  **自动状态转换**（确定性的，无需 LLM）。未使用超过 `stale_after_days`（30 天）的技能变为 `stale`；未使用超过 `archive_after_days`（90 天）的技能将被移动到 `~/.hermes/skills/.archive/`。
2.  **LLM 审查**（单次辅助模型运行，`max_iterations=8`）。分支 Agent 会检查所有 Agent 创建的技能，可以使用 `skill_view` 读取其中任何一个，并针对每个技能决定是保留、修补（通过 `skill_manage`）、合并重叠的技能，还是通过终端工具归档。

已固定的技能对技能策展器的自动状态转换和 Agent 自身的 `skill_manage` 工具都是禁止访问的。请参阅下面的 [固定技能](#pinning-a-skill)。
...
:::note 旧版配置
早期版本使用了一个独立的 `curator.auxiliary.{provider,model}` 块。该路径仍然有效，但会输出弃用日志行——请迁移到上面的 `auxiliary.curator`，以便技能策展器与其他所有辅助任务共享相同的基础设施（`hermes model`、仪表板 Models 选项卡、`base_url`、`api_key`、`timeout`、`extra_body`）。
:::
...
hermes curator run --sync     # 同上，但阻塞直到 LLM 审查完成
...
在每次真正的技能策展器运行之前，Hermes 会在 `~/.hermes/skills/.curator_backups/<utc-iso>/skills.tar.gz` 处创建 `~/.hermes/skills/` 的 tar.gz 快照。如果一次运行归档或合并了您不希望被触及的内容，您可以通过一个命令撤销整个运行：
...
快照会被修剪到 `curator.backup.keep`（默认为 5）以限制磁盘使用：
...
设置 `curator.backup.enabled: false` 以禁用自动快照。当备份被禁用时，手动命令 `hermes curator backup` 仍然有效，但前提是你先设置了 `enabled: true` —— 该标志对称地控制两条路径，因此无法在变更性运行中意外跳过运行前快照。

`hermes curator status` 还会列出五个最近最少使用的技能 —— 这是快速查看接下来哪些技能可能变得陈旧的方法。
...
`~/.hermes/skills/` 中的所有其他内容都是管理员可以处理的。这包括：
...
这里的来源是**二元的**（捆绑/中心安装 vs. 其他所有）。管理员无法区分你为私有工作流手动编写的技能与自我改进循环在会话中保存的技能。两者都属于 "Agent 创建" 类别。

在第一次实际运行之前（默认安装后 7 天），请花点时间：

1. 运行 `hermes curator run --dry-run` 以查看管理员会提出什么建议。
...
如果你想保护特定技能不被触及 —— 例如你依赖的手写技能 —— 请使用 `hermes curator pin <name>`。请参阅下一节。

## 固定技能

固定是针对自动和 Agent 驱动更改的硬性防护。一旦技能被固定：

- **管理员**在自动转换期间（`active → stale → archived`）会跳过它，并且其 LLM 审查过程被指示不要动它。
- **Agent 的 `skill_manage` 工具**拒绝对其执行任何写入操作。对 `edit`、`patch`、`delete`、`write_file` 和 `remove_file` 的调用会返回一个拒绝信息，告诉模型要求用户运行 `hermes curator unpin <name>`。这可以防止 Agent 在对话中静默重写技能。
...
只有 **Agent 创建的** 技能可以被固定 —— 捆绑和中心安装的技能首先永远不会受到管理员变更的影响，如果你尝试固定它们，`hermes curator pin` 会拒绝并给出解释性消息。

如果你需要自己更新固定的技能，请直接使用编辑器编辑 `~/.hermes/skills/<name>/SKILL.md`。固定只保护 Agent 的工具路径，而不限制你自己的文件系统访问。

## 使用遥测

管理员在 `~/.hermes/skills/.usage.json` 维护一个伴生文件，每个技能有一个条目：
...
每次管理员运行都会在 `~/.hermes/logs/curator/` 下写入一个带时间戳的目录：
...
`REPORT.md` 是查看给定运行所做操作的快速方式 —— 哪些技能发生了转换，LLM 审查员说了什么，它修补了哪些技能。无需 grep `agent.log` 即可进行审计。
...
这将技能从 `~/.hermes/skills/.archive/` 移回活动树，并将其状态重置为 `active`。如果捆绑或中心安装的技能后来以相同名称安装（会遮蔽上游），恢复操作将拒绝执行。
...
- **仅针对一次运行：** `hermes curator pause` —— 暂停在会话之间持续存在；使用 `resume` 重新启用。

如果 `min_idle_hours` 尚未过去，管理员也会拒绝运行，因此在活跃的开发机器上，它自然只会在安静时段运行。

...
:::tip 可预测的大规模成本
批量运行会启动许多并发的 Agent 会话，每个会话都会进行模型调用和工具调用。[Nous Portal](/user-guide/features/tool-gateway) 订阅将模型访问权限与网络搜索、图像生成、TTS 和云浏览器捆绑在一张账单下——当您希望获得稳定的单轨迹成本，而无需在五个供应商账户之间平衡速率限制时，这非常有用。使用 `hermes setup --portal` 进行设置，然后将 `--model` 指向一个 Nous 模型。
:::
...
| 参数 | 描述 |
|-----------|-------------|
| `--reasoning_effort` | 推理努力级别：`none`、`minimal`、`low`、`medium`、`high`、`xhigh` |
...
在当前实现中，分布为**每个独立的工具集**分配一个概率。采样器独立地决定每个工具集是否启用，然后保证至少有一个工具集被启用。这与手动编写的预构建组合表不同。
...
- **无推理过滤器：** 零个助手回合包含推理（没有 `<REASONING_SCRATCHPAD>` 或原生思考 Token）的样本将被丢弃
- **损坏条目过滤器：** 包含幻觉工具名称（不在有效工具列表中）的条目在最终合并期间被过滤掉
- **推理统计：** 跟踪整个运行中带有/不带有推理的回合百分比
...
对于需要特定执行环境的基准测试，每个提示词可以指定自己的容器镜像：

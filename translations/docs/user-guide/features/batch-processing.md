...
批量运行器 (`batch_runner.py`) 处理一个包含提示词的 JSONL 数据集，通过一个具有工具访问权限的完整 Agent 会话运行每个提示词。每个提示词都有其独立的执行环境。输出是结构化的轨迹数据，包含完整的对话历史、工具调用统计信息和推理覆盖率指标。
...
:::tip 可预测的大规模成本
批量运行会启动许多并发的 Agent 会话，每个会话都会进行模型调用和工具调用。[Nous Portal](/docs/user-guide/features/tool-gateway) 订阅将模型访问权限与网络搜索、图像生成、TTS 和云浏览器捆绑在一张账单下——当您希望获得稳定的单轨迹成本，而无需在五个供应商账户之间处理速率限制时，这非常有用。使用 `hermes setup --portal` 进行设置，然后将 `--model` 指向 Nous 模型。
:::
...
| `--run_name` | (必填) | 此次运行的名称（用于输出目录和检查点） |
...
每个提示词都会从**分布**中随机采样一组工具集。这确保了训练数据涵盖不同的工具组合。使用 `--list_distributions` 查看所有可用的分布。
...
`conversations` 字段使用类似 ShareGPT 的格式，包含 `from` 和 `value` 字段。工具统计信息被标准化，包含所有可能的工具并带有零默认值，确保条目之间的模式一致，以便与 HuggingFace 数据集兼容。

## 检查点

批量运行器具有强大的检查点功能，以实现容错：

- **检查点文件：** 每个批次完成后保存，跟踪哪些提示词索引已完成
- **基于内容的恢复：** 在 `--resume` 时，运行器扫描现有的批次文件，并通过其实际文本内容（不仅仅是索引）匹配已完成的提示词，即使数据集顺序发生变化也能恢复
- **失败的提示词：** 只有成功完成的提示词才被标记为完成——失败的提示词将在恢复时重试
- **批次合并：** 完成后，所有批次文件（包括之前运行的）将合并到一个 `trajectories.jsonl` 文件中

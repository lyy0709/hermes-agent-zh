...
完整集合请参阅[工具集参考](/reference/toolsets-reference)，包括平台预设，如 `hermes-cli`、`hermes-telegram`，以及动态 MCP 工具集，如 `mcp-<server>`。
...
这意味着 Docker 后端的行为类似于一个持久化的沙盒虚拟机，而不是每个命令都使用一个新容器。如果您 `pip install foo` 一次，它将在会话的其余部分存在。如果您 `cd /workspace/project`，后续的 `ls` 调用将看到该目录。有关完整的生命周期细节以及控制 `/workspace` 和 `/root` 是否在 Hermes 重启后保留的 `container_persistent` 标志，请参阅[配置 → Docker 后端](../configuration.md#docker-backend)。

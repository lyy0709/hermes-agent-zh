...
| **媒体** | `vision_analyze`, `image_generate`, `video_generate`, `video_analyze`, `text_to_speech` | 多模态分析与生成。`video_generate` 和 `video_analyze` 需选择启用（通过 `hermes tools` 或 `--toolsets` 添加 `video_gen` / `video` 工具集）。 |
...
完整的工具集列表，包括平台预设（如 `hermes-cli`、`hermes-telegram`）和动态 MCP 工具集（如 `mcp-<server>`），请参阅[工具集参考](/reference/toolsets-reference)。
...
**一个持久化容器，在整个进程中共享。** Hermes 在首次使用时启动一个长期运行的容器（`docker run -d ... sleep 2h`），并通过 `docker exec` 将每个终端、文件和 `execute_code` 调用路由到同一个容器中。工作目录更改、安装的包、环境调整以及写入 `/workspace` 的文件，在 Hermes 进程的生命周期内，从一个工具调用延续到下一个，跨越 `/new`、`/reset` 和 `delegate_task` 子 Agent。容器在关闭时停止并移除。
...
terminal:
  ...
  container_disk: 51200         # 磁盘空间，单位 MB（默认：50GB）

...
使用自然语言或 cron 表达式安排任务自动运行。Hermes 通过一个统一的 `cronjob` 工具来管理定时任务，它采用操作风格，而不是提供独立的 schedule/list/remove 工具。
...
- 在新的 Agent 会话中运行，使用常规的静态工具列表
- 在**无 Agent 模式**下运行 —— 按计划运行脚本，其标准输出原样传递，完全不涉及 LLM（请参阅下面的[无 Agent 模式](#无-agent-模式仅脚本任务)部分）

所有这些功能都通过 `cronjob` 工具提供给 Hermes 自身，因此您可以用自然语言请求来创建、暂停、编辑和移除任务 —— 无需使用 CLI。

:::warning
由 Cron 运行的会话不能递归创建更多定时任务。Hermes 在 cron 执行过程中禁用了定时任务管理工具，以防止失控的调度循环。
:::
...
定时任务默认在脱离任何代码仓库的情况下运行 —— 不会加载 `AGENTS.md`、`CLAUDE.md` 或 `.cursorrules`，并且终端 / 文件 / 代码执行工具将从消息网关启动时的工作目录运行。传递 `--workdir`（CLI）或 `workdir=`（工具调用）来改变这一点：
...
:::note 序列化
带有 `workdir` 的任务在调度器触发时按顺序运行，而不是在并行池中运行。这是有意为之 —— `TERMINAL_CWD` 是进程全局的，因此两个同时运行的 workdir 任务会相互破坏对方的工作目录。没有 workdir 的任务仍然像以前一样并行运行。
:::
...
:::tip 任务引用
下面的 `<job_id>` 占位符（以及在[生命周期操作](#生命周期操作)中）也接受任务的名称（不区分大小写）—— 当您记得 `morning-digest` 但不记得十六进制 ID 时，这很方便。确切的 job ID 优先于名称匹配；如果引用不是 ID 并且一个名称匹配多个任务，命令将拒绝执行并打印候选 ID，以便您进行区分。
:::
...
**定时任务由消息网关守护进程处理。** 网关每 60 秒触发一次调度器，在独立的 Agent 会话中运行所有到期的任务。
...
`~/.hermes/cron/.tick.lock` 处的文件锁可防止重叠的调度器周期重复运行同一批任务。
...
| `"origin,all"` | 交付到原始位置 **加上** 其他每个已连接的频道 | 组合任何令牌 |

Agent 的最终响应会自动交付。您无需在定时任务提示词中调用 `send_message`。

### 路由意图 (`all`)

`all` 允许您将一个定时任务发送到您配置的每个消息频道，而无需按名称枚举它们。它**在触发时解析**，因此，在您设置 `TELEGRAM_HOME_CHANNEL` 之前创建的任务，将在您设置后的下一个周期获取 Telegram。
...
当启用 Telegram 话题模式时，根私聊被保留为系统大厅 — 发送到那里的回复会被大厅提醒拒绝，并且 `reply_to_message_id` 会被丢弃，因此您无法回复到达主聊天的定时任务消息。
...
默认情况下，交付的定时任务输出会包装有页眉和页脚，以便收件人知道它来自计划任务：
...
失败的任务无论是否有 `[SILENT]` 标记都会交付 — 只有成功的运行可以被静默。
...
对于不需要 LLM 推理的重复性任务 — 经典的看门狗、磁盘/内存警报、心跳、CI ping — 在创建时传递 `no_agent=True`。调度器按计划运行您的脚本并直接交付其标准输出，完全跳过 Agent：
```bash
...
- 脚本标准输出（修剪后）→ 作为消息原文发送。
- **空的标准输出 → 静默执行**，不发送任何内容。这是看门狗模式："只在有问题时才说话"。
- 非零退出或超时 → 发送错误警报，因此损坏的看门狗不会静默失败。
- 最后一行是 `{"wakeAgent": false}` → 静默执行（与 LLM 作业使用的相同门控机制）。
- 没有 Token，没有模型，没有提供商回退 —— 作业从不接触推理层。
...
当消息内容完全由脚本决定时（看门狗、阈值警报、心跳），它会自动选择 `no_agent=True`。同一个工具还允许 Agent 暂停、恢复、编辑和删除作业 —— 因此整个生命周期都是聊天驱动的，无需任何人接触 CLI。

有关工作示例，请参阅 [纯脚本定时任务指南](/docs/guides/cron-script-only)。

## 使用 `context_from` 链接作业

定时任务在隔离的会话中运行，不保留先前运行的记忆。但有时一个作业的输出正是下一个作业所需要的。`context_from` 参数会自动建立这种连接 —— 在运行时，Job B 的提示词会自动将 Job A 的最新输出作为上下文前置。
...
# 从以下命令获取作业 1 的 ID：cronjob(action="list")
cronjob(
    action="create",
...
Agent 的最终响应会自动发送 —— 你**不需要**在定时任务提示词中为同一目标包含 `send_message`。如果定时任务运行调用了 `send_message` 到调度器已经要发送的完全相同的目标，Hermes 会跳过该重复发送，并告诉模型将面向用户的内容放在最终响应中。仅对额外或不同的目标使用 `send_message`。
...
```
对于 `update` 操作，传递 `skills=[]` 以移除所有附加的技能。

## 定时任务可用的工具集

Cron 会在一个全新的 Agent 会话中运行每个任务，且没有附加任何聊天平台。默认情况下，cron Agent 会获得**你在 `hermes tools` 中为 `cron` 平台配置的工具集**——而不是 CLI 的默认设置，也不是所有可用的工具。

```bash
hermes tools
# → 在 curses UI 中选择 "cron" 平台
# → 像配置 Telegram/Discord 等平台一样，切换工具集的开启/关闭状态
```

通过 `cronjob.create` 上的 `enabled_toolsets` 字段（或通过 `cronjob.update` 对现有任务进行设置），可以实现更精细的每任务控制：
...
当任务设置了 `enabled_toolsets` 时，该设置优先；否则，`hermes tools` 中 cron 平台的配置优先；再否则，Hermes 会回退到内置的默认值。这对于成本控制很重要：在每个微小的“获取新闻”任务中都携带 `moa`、`browser`、`delegation` 等工具，会在每次 LLM 调用时增加工具模式提示词的开销。

### 完全跳过 Agent：`wakeAgent`

如果你的定时任务附加了预检查脚本（通过 `script=`），该脚本可以在运行时决定 Hermes 是否应该调用 Agent。脚本输出最后一行标准输出，格式如下：

```text
{"wakeAgent": false}
```

…这样 cron 就会完全跳过本次触发的 Agent 运行。这对于频繁的轮询（每 1-5 分钟）很有用，这些轮询只需要在状态实际发生变化时才唤醒 LLM——否则你会为一次次没有实际内容的 Agent 轮转付费。
...
`wakeAgent` 门控为你提供了一种零成本的方式来决定一个计划任务是否应该消耗任何 LLM Token。以下三种模式涵盖了大多数用例。

**文件变更门控** —— 仅在被监视的文件自上次成功运行后有新内容时才运行。调度器会记录每个任务的 `last_run_at`；将其与文件的修改时间（mtime）进行比较。
...
**SQL 计数门控** —— 仅当你的数据库中有新行需要处理时才运行。脚本还可以通过 `context` 将计数传递给 Agent，这样 Agent 无需重新查询就知道要处理多少数据。
...
:::tip
Hermes 自身的 `~/.hermes/state.db` 是一个内部模式，会在不同版本间变化。不要从预运行门控中查询它——请指向你自己的数据库或数据源。
:::

致谢：这套配方是由 @iankar8 在 [#2654](https://github.com/NousResearch/hermes-agent/pull/2654) 中的探索所启发，该探索提议添加 sql/file/command 触发器作为一种并行机制。`script` + `wakeAgent` 门控已经以零成本覆盖了所有三种情况，因此这项工作最终以文档形式落地。

### 任务链式调用：`context_from`

一个定时任务可以通过在 `context_from` 中列出一个或多个其他任务的名称（或 ID）来使用它们最近一次成功的输出：
...
被引用任务最近一次完成的输出会作为上下文注入到本次运行的提示词上方。每个上游条目必须是有效的任务 ID 或名称（参见 `cronjob action="list"`）。注意：链式调用读取的是*最近一次完成的*输出——它不会等待在同一时间点正在运行的上游任务。
...
在创建和更新时，会扫描计划任务的提示词是否存在提示词注入和凭据泄露模式。包含不可见 Unicode 技巧、SSH 后门尝试或明显秘密泄露负载的提示词会被阻止。
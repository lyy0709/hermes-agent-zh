...
**流式传输** (`"stream": true`)：返回服务器发送事件（SSE），包含逐 Token 的响应块。当在配置中启用流式传输时，Token 会在 LLM 生成时实时发出。当禁用时，完整的响应会作为单个 SSE 块发送。

**流中的工具进度**：当 Agent 在流式请求期间调用工具时，在工具开始执行时（例如 `` `💻 pwd` ``, `` `🔍 Python docs` ``），简短的进度指示器会被注入到内容流中。这些会作为内联 Markdown 出现在 Agent 的响应文本之前，为 Open WebUI 等前端提供工具执行的实时可见性。

### POST /v1/responses

OpenAI Responses API 格式。通过 `previous_response_id` 支持服务器端对话状态 —— 服务器存储完整的对话历史（包括工具调用和结果），因此多轮上下文得以保留，而无需客户端管理。
...
当前端发送 `system` 消息（Chat Completions）或 `instructions` 字段（Responses API）时，hermes-agent **会将其叠加在**其核心系统提示词之上。你的 Agent 保留其所有工具、记忆和技能 —— 前端的系统提示词添加额外的指令。
...
:::warning 安全
API 服务器授予对 hermes-agent 工具集的完全访问权限，**包括终端命令**。当绑定到非回环地址如 `0.0.0.0` 时，`API_SERVER_KEY` 是**必需的**。同时保持 `API_SERVER_CORS_ORIGINS` 范围狭窄以控制浏览器访问。

默认的绑定地址（`127.0.0.1`）仅用于本地使用。默认情况下浏览器访问被禁用；仅对明确信任的来源启用。
:::
...
要为多个用户提供他们自己隔离的 Hermes 实例（独立的配置、记忆、技能），请使用[配置文件](/docs/user-guide/features/profiles)：

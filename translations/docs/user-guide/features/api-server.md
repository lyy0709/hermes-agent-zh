...
**内联图像输入：** 用户消息可以将 `content` 作为包含 `text` 和 `image_url` 部分的数组发送。同时支持远程 `http(s)` URL 和 `data:image/...` URL：
...
**内联图像输入：** `input[].content` 可以包含 `input_text` 和 `input_image` 部分。同时支持远程 URL 和 `data:image/...` URL：
...
将 Agent 列为可用模型。广告的模型名称默认为[配置文件](/docs/user-guide/profiles)名称（或默认配置文件的 `hermes-agent`）。大多数前端进行模型发现时都需要此接口。
...
## Runs API（支持流式传输的替代方案）

除了 `/v1/chat/completions` 和 `/v1/responses`，服务器还暴露了一个 **runs** API，适用于长会话场景，客户端希望订阅进度事件，而不是自行管理流式传输。
...
服务器发送事件流，包含运行的工具调用进度、Token 增量以及生命周期事件。专为希望在不丢失状态的情况下附加/分离的仪表板和富客户端设计。

## Jobs API（后台定时任务）

服务器暴露了一个轻量级的 jobs CRUD 接口，用于从远程客户端管理定时/后台 Agent 运行。所有端点都受相同的 Bearer 认证保护。
...
当前端发送 `system` 消息（Chat Completions）或 `instructions` 字段（Responses API）时，hermes-agent **会将其叠加在**其核心系统提示词之上。你的 Agent 保留其所有工具、记忆和技能 —— 前端的系统提示词添加了额外的指令。
...
默认绑定地址（`127.0.0.1`）仅用于本地使用。默认禁用浏览器访问；仅对明确信任的来源启用。
:::
...
在 Open WebUI 中，将每个连接作为单独的连接添加。模型下拉菜单会将 `alice` 和 `bob` 显示为不同的模型，每个模型都由一个完全隔离的 Hermes 实例支持。详情请参阅 [Open WebUI 指南](/docs/user-guide/messaging/open-webui#multi-user-setup-with-profiles)。

## 限制

- **响应存储** — 存储的响应（用于 `previous_response_id`）会持久化保存在 SQLite 中，并且在消息网关重启后仍然存在。最多存储 100 个响应（采用 LRU 淘汰策略）。
...
该 API 服务器也用作**消息网关代理模式**的后端。当另一个 Hermes 网关实例配置了指向此 API 服务器的 `GATEWAY_PROXY_URL` 时，它会将所有消息转发到这里，而不是运行自己的 Agent。这支持了拆分部署——例如，一个处理 Matrix E2EE 的 Docker 容器将消息中继到主机端的 Agent。

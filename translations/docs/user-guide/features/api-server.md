...
你的 Agent 会使用其完整的工具集（终端、文件操作、网络搜索、记忆、技能）处理请求并返回最终响应。在流式传输时，工具进度指示器会内联显示，以便前端可以展示 Agent 正在做什么。
...
**流式传输** (`"stream": true`)：返回服务器发送事件（SSE），包含逐 Token 的响应块。当在配置中启用流式传输时，Token 会在 LLM 生成时实时发出。当禁用时，完整响应会作为单个 SSE 块发送。

**流中的工具进度**：当 Agent 在流式请求期间调用工具时，在工具开始执行时（例如 `` `💻 pwd` ``, `` `🔍 Python docs` ``），简短的进度指示器会被注入到内容流中。这些指示器在 Agent 的响应文本之前以内联 Markdown 的形式出现，为 Open WebUI 等前端提供工具执行的实时可见性。

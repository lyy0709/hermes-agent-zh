...
2. 一个在 `gateway:startup` 事件上触发的消息网关钩子，该钩子会使用你消息网关解析出的模型/凭证生成一个一次性 Agent，并运行 BOOT.md 中的指令。
...
如果没有这些，一个裸的 `AIAgent()` 会回退到内置默认值，并且针对任何非默认端点都会返回 401 错误。
...
你应该会看到 `Running BOOT.md (N chars)`，然后是 `boot-md completed: ...`（Agent 执行内容的摘要）或者当 Agent 回复 `[SILENT]` 时看到 `boot-md completed (nothing to report)`。
...
- **非 Agent 变体：** 如果不需要完整的 Agent 循环，可以完全跳过 `AIAgent`，让处理程序直接通过 `httpx` 发布固定的通知。更便宜、更快，并且不依赖提供商。

#### 为什么这不是内置功能

Hermes 的早期版本将此作为内置钩子提供，并在每次消息网关启动时使用裸默认值静默生成一个 Agent。这让使用自定义端点的用户感到意外，并且对于不知道它正在运行的用户来说，该功能是不可见的。将其保留为一个有文档记录的模式——由你在你的钩子目录中构建——意味着你可以确切地看到它的作用，并通过编写文件来选择加入。
...
[插件](/docs/user-guide/features/plugins)可以注册在**CLI 和消息网关**会话中都会触发的钩子。这些钩子通过插件 `register()` 函数中的 `ctx.register_hook()` 以编程方式注册。
...
| 钩子 | 触发时机 | 返回值 |
|------|-----------|---------|
| [`pre_tool_call`](#pre_tool_call) | 在任何工具执行之前 | `{"action": "block", "message": str}` 以否决调用 |
...
| [`post_approval_response`](#post_approval_response) | 用户响应批准提示后（或超时） | 忽略 |
| [`transform_tool_result`](#transform_tool_result) | 在任何工具返回后，在结果交还给模型之前 | `str` 以替换结果，`None` 保持不变 |
...
Agent 将短路该工具调用，并将 `message` 作为错误返回给模型。第一个匹配的阻止指令生效（Python 插件优先注册，然后是 shell 钩子）。任何其他返回值都会被忽略，因此现有的仅作为观察者的回调函数可以保持不变地工作。
...
**使用场景：** 记录工具结果、收集指标、追踪工具成功率/失败率、延迟仪表板、按工具预算告警、在特定工具完成时发送通知。
...
**触发时机：** 在 `run_agent.py` 的 `run_conversation()` 函数内部，在上下文压缩之后、主 `while` 循环之前。每次 `run_conversation()` 调用触发一次（即每个用户轮次一次），而不是工具循环内的每次 API 调用一次。

**返回值：** 如果回调函数返回一个包含 `"context"` 键的字典，或者一个普通的非空字符串，该文本将被附加到当前轮次的用户消息中。返回 `None` 表示不注入。
...
在工具调用循环完成且 Agent 已生成最终响应后，**每轮触发一次**。仅在**成功**的轮次触发——如果轮次被中断则不触发。
...
当网关为活动聊天**交换新的会话密钥**时触发 — 用户调用了 `/new`、`/reset`、`/clear`，或者适配器在空闲窗口后选取了一个新的会话。这允许插件对会话状态已被清空这一事实做出反应，而无需等待下一个 `on_session_start`。
...
**使用场景：** 重置按 `session_id` 键控的每个会话缓存，发出“会话轮换”分析事件，初始化一个新的状态桶。
...
| `child_role` | `str \| None` | 在子 Agent 上设置的角色标签（如果该功能未启用则为 `None`） |
...
**触发位置：** 在 `tools/delegate_tool.py` 中，`ThreadPoolExecutor.as_completed()` 处理完所有子 Future 之后。触发被调度到父线程，因此钩子作者无需考虑并发回调执行。

**返回值：** 忽略。

**使用场景：** 记录编排活动、累计子任务时长用于计费、编写委派后审计记录。
...
在消息网关中，**每个传入的 `MessageEvent` 触发一次**，在内部事件守卫之后，但在身份验证/配对和 Agent 调度**之前**。这是网关级消息流策略（仅监听窗口、人工交接、按聊天路由等）的拦截点，这些策略无法清晰地适配到任何单一平台适配器中。
...
| 参数 | 类型 | 描述 |
|-----------|------|-------------|
| `event` | `MessageEvent` | 规范化的入站消息（包含 `.text`、`.source`、`.message_id`、`.internal` 等属性）。 |
| `gateway` | `GatewayRunner` | 活动的网关运行器，因此插件可以调用 `gateway.adapters[platform].send(...)` 进行旁路回复（如所有者通知等）。 |
| `session_store` | `SessionStore` | 用于通过 `session_store.append_to_transcript(...)` 静默摄取对话记录。 |

**触发位置：** 在 `gateway/run.py` 的 `GatewayRunner._handle_message()` 函数内部，计算完 `is_internal` 后立即触发。**内部事件完全跳过此钩子**（它们是系统生成的——后台进程完成等——不得被面向用户的策略拦截）。
...
**使用场景：** 仅监听的群聊（仅在标记时响应；将环境消息缓冲到上下文中）；人工交接（在所有者手动处理聊天时静默摄取客户消息）；按配置文件进行速率限制；策略驱动的路由。
...
在向用户显示审批请求**之前立即触发**——涵盖所有界面：交互式 CLI、Ink TUI、网关平台（Telegram、Discord、Slack、WhatsApp、Matrix 等）和 ACP 客户端（VS Code、Zed、JetBrains 等）。
...
**使用场景：** 桌面通知、推送警报、审计日志记录、Slack webhook、升级路由、指标收集。
...
**使用场景：** 从 `web_extract` 输出中编辑组织特定的 PII、将长的 JSON 工具响应包装在摘要标题中、向 `read_file` 结果中注入检索增强提示、将 `delegate_task` 子 Agent 报告重写为项目特定的模式。
...
**使用场景：** 为产生大量输出的命令（`du -ah`、`find`、`tree`）注入摘要、用项目特定的标记标记输出以便下游钩子知道如何处理、去除运行间波动并破坏提示缓存的时序噪声。
...
在你的 `cli-config.yaml` 中声明 shell 脚本钩子，每当相应的插件钩子事件触发时，Hermes 将把它们作为子进程运行——在 CLI 和消息网关会话中都是如此。无需编写 Python 插件。

当你想要一个即插即用的单文件脚本（Bash、Python 或任何带有 shebang 的脚本）来执行以下操作时，请使用 shell 钩子：

- **阻止工具调用** —— 拒绝危险的 `terminal` 命令、强制执行按目录策略、要求对破坏性的 `write_file` / `patch` 操作进行批准。
...
Shell hooks 通过在 CLI 启动 (`hermes_cli/main.py`) 和 gateway 启动 (`gateway/run.py`) 时调用 `agent.shell_hooks.register_from_config(cfg)` 来注册。它们与 Python 插件钩子自然组合——两者都通过同一个调度器执行。
...
```jsonc
// 阻止一个 pre_tool_call (两种形式都被接受；在内部规范化):
...
Agent 对文件的上下文内视图**不会**自动重新读取——重新格式化只影响磁盘上的文件。后续的 `read_file` 调用会获取格式化后的版本。
...
| 命令 | 功能 |
|---------|--------------|
| `hermes hooks list` | 列出已配置的钩子，包括匹配器、超时和授权状态 |
| `hermes hooks test <event> [--for-tool X] [--payload-file F]` | 针对合成负载触发所有匹配的钩子，并打印解析后的响应 |
| `hermes hooks revoke <command>` | 移除所有匹配 `<command>` 的允许列表条目（下次重启时生效） |
...
- 仅引用您编写或已完全审查的脚本。
- 将脚本保存在 `~/.hermes/agent-hooks/` 目录中，以便路径易于审计。
- 在拉取共享配置后，重新运行 `hermes hooks doctor`，以便在钩子注册前发现新添加的钩子。

...
:::info
消息网关钩子仅在**消息网关**（Telegram、Discord、Slack、WhatsApp）中触发。CLI 不会加载消息网关钩子。对于在所有地方都有效的钩子，请使用[插件钩子](#plugin-hooks)。
:::
...
- 回调函数接收**关键字参数**。为了向前兼容，请始终接受 `**kwargs`——未来版本可能会添加新参数而不会破坏你的插件。
...
| [`pre_llm_call`](#pre_llm_call) | 每轮一次，在工具调用循环开始之前 | 上下文注入 |
| [`post_llm_call`](#post_llm_call) | 每轮一次，在工具调用循环结束之后 | 忽略 |
...
**触发时机：** 在 `run_agent.py` 的 `run_conversation()` 函数内，在上下文压缩之后，主 `while` 循环之前。每次 `run_conversation()` 调用触发一次（即每轮用户对话一次），而不是工具循环内的每次 API 调用一次。
...
**触发时机：** 在 `run_agent.py` 的 `run_conversation()` 函数内，在工具循环退出并产生最终响应之后。受 `if final_response and not interrupted` 保护——因此当用户中途中断或 Agent 达到迭代限制而未产生响应时，它**不会**触发。
...
**触发时机：** 在 `run_agent.py` 的 `run_conversation()` 函数内，在新会话的第一轮期间——具体是在系统提示词构建之后，工具循环开始之前。检查条件是 `if not conversation_history`（没有先前的消息 = 新会话）。
**返回值：** 忽略。
...
**触发位置：** 两处：
1. **`run_agent.py`** — 在每次 `run_conversation()` 调用结束时，所有清理工作之后。即使回合出错，也会始终触发。

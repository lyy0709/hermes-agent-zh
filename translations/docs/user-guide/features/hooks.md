...
| **[Shell 钩子](#shell-hooks)** | `~/.hermes/config.yaml` 中指向 shell 脚本的 `hooks:` 块 | CLI + 消息网关 | 即插即用脚本，用于阻塞、自动格式化、上下文注入 |

所有三套系统都是非阻塞的 —— 任何钩子中的错误都会被捕获并记录，永远不会导致 Agent 崩溃。

## 消息网关事件钩子

消息网关钩子在网关操作期间（Telegram、Discord、Slack、WhatsApp）自动触发，不会阻塞主 Agent 流水线。
...
消息网关附带一个内置的 `boot-md` 钩子，每次启动时都会查找 `~/.hermes/BOOT.md`。如果文件存在，Agent 会在后台会话中运行其指令。无需安装 —— 只需创建文件。
...
Agent 在后台线程中运行这些指令，因此不会阻塞网关启动。如果无需关注任何事项，Agent 会回复 `[SILENT]` 且不发送任何消息。
...
当新会话开始时，向外部服务发送 POST 请求：

```yaml
# ~/.hermes/hooks/session-webhook/HOOK.yaml
name: session-webhook
description: 新会话开始时通知外部服务
events:
  - session:start
  - session:reset
```
...
1.  消息网关启动时，`HookRegistry.discover_and_load()` 会扫描 `~/.hermes/hooks/`
...
[插件](/docs/user-guide/features/plugins)可以注册在**CLI 和消息网关**会话中都触发的钩子。这些钩子通过插件 `register()` 函数中的 `ctx.register_hook()` 以编程方式注册。
...
| [`pre_llm_call`](#pre_llm_call) | 每轮一次，在工具调用循环之前 | `{"context": str}` 以将上下文预置到用户消息前 |
| [`post_llm_call`](#post_llm_call) | 每轮一次，在工具调用循环之后 | 忽略 |
| [`on_session_start`](#on_session_start) | 新会话创建时（仅首次轮次） | 忽略 |
...
Agent 将短路该工具调用，并将 `message` 作为错误返回给模型。第一个匹配的阻止指令生效（先注册的 Python 插件，然后是 shell 钩子）。任何其他返回值都会被忽略，因此现有的仅作为观察者的回调函数可以保持不变地工作。
...
**触发时机：** 在 `model_tools.py` 的 `handle_function_call()` 函数内，工具的处理函数返回后触发。每次工具调用触发一次。如果工具抛出了未处理的异常则**不**触发（错误会被捕获并作为错误 JSON 字符串返回，此时 `post_tool_call` 会触发，并将该错误字符串作为 `result` 参数）。
...
**触发时机：** 在 `run_agent.py` 的 `run_conversation()` 函数内，上下文压缩之后，主 `while` 循环之前。每次 `run_conversation()` 调用触发一次（即每轮用户交互一次），而不是工具循环内的每次 API 调用触发一次。
...
**上下文注入的位置：** 始终是**用户消息**，而不是系统提示词。这可以保留提示词缓存 — 系统提示词在各轮次间保持相同，因此缓存的 Token 可以被复用。系统提示词是 Hermes 的领域（模型指导、工具强制执行、人格、技能）。插件在用户输入旁边贡献上下文。
...
**触发位置：** 在 `run_agent.py` 的 `run_conversation()` 函数内，在新会话的第一个回合期间 — 具体是在系统提示词构建之后，工具循环开始之前。判断条件是 `if not conversation_history`（没有先前的消息 = 新会话）。
...
2.  **`cli.py`** — 在 CLI 的 atexit 处理程序中，但**仅当**退出发生时 Agent 正在处理中途（`_agent_running=True`）。这可以捕获处理过程中的 Ctrl+C 和 `/exit`。在这种情况下，`completed=False` 且 `interrupted=True`。
...
**触发时机：** 在 `gateway/run.py` 中，新会话密钥分配后、下一条入站消息被处理前。在网关上，顺序是：`on_session_finalize(old_id)` → 交换 → `on_session_reset(new_id)` → 在第一个入站轮次时 `on_session_start(new_id)`。

**返回值：** 忽略。

**使用场景：** 重置按 `session_id` 键控的每会话缓存，发出“会话已轮换”分析数据，初始化新的状态桶。
...
在 `delegate_task` 完成后，**每个子 Agent 触发一次**。无论你委派的是单个任务还是三个任务的一批，此钩子都会为每个子 Agent 触发一次，在父线程上串行执行。
...
**触发时机：** 在 `tools/delegate_tool.py` 中，`ThreadPoolExecutor.as_completed()` 处理完所有子 Future 之后。触发被编排到父线程，因此钩子作者无需考虑并发回调执行。
...
:::info
在重度委派的情况下（例如，编排器角色 × 5 个叶子 × 嵌套深度），`subagent_stop` 每轮会触发多次。请保持回调函数快速执行；将耗时的工作推送到后台队列。
:::
...
在网关中，**每个传入的 `MessageEvent` 触发一次**，在内置事件守卫之后，但在认证/配对和 Agent 分发**之前**。这是网关级消息流策略（仅监听窗口、人工交接、每聊天路由等）的拦截点，这些策略无法清晰地放入任何单个平台适配器中。
...
**触发时机：** 在 `gateway/run.py` 的 `GatewayRunner._handle_message()` 内部，`is_internal` 计算完成后立即触发。**内部事件完全跳过此钩子**（它们是系统生成的——后台进程完成等——不得被面向用户的策略所把关）。

**返回值：** `None` 或一个字典。第一个被识别的动作字典生效；其余插件结果将被忽略。插件回调中的异常会被捕获并记录；网关在出错时总是会回退到正常的分发流程。

| 返回值 | 效果 |
|--------|--------|
| `{"action": "skip", "reason": "..."}` | 丢弃消息——没有 Agent 回复，没有配对流程，没有认证。假定插件已处理该消息（例如，静默摄取到对话记录中）。 |
...
这是连接自定义通知器的合适位置 —— 例如，一个 macOS 菜单栏应用，弹出允许/拒绝通知；或者一个审计日志，记录每个带有上下文的审批请求。
...
**返回值：** 忽略。此处的钩子仅为观察者；它们不能否决或预先回答审批。使用 [`pre_tool_call`](#pre_tool_call) 在工具到达审批系统之前阻止它。
...
当你想要一个即插即用的单文件脚本（Bash、Python 或任何带有 shebang 的脚本）来执行以下操作时，请使用 shell 钩子：

- **阻止工具调用** —— 拒绝危险的 `terminal` 命令、强制执行每个目录的策略、要求对破坏性的 `write_file` / `patch` 操作进行审批。
...
Shell 钩子通过在 CLI 启动（`hermes_cli/main.py`）和消息网关启动（`gateway/run.py`）时调用 `agent.shell_hooks.register_from_config(cfg)` 来注册。它们与 Python 插件钩子自然组合 —— 两者都通过同一个调度器流转。
...
对于非工具事件（`pre_llm_call`、`subagent_stop`、会话生命周期），`tool_name` 和 `tool_input` 为 `null`。`extra` 字典携带所有事件特定的关键字参数（`user_message`、`conversation_history`、`child_role`、`duration_ms`、…）。不可序列化的值会被字符串化，而不是被省略。

**stdout — 可选的响应：**

```jsonc
// 阻止一个 pre_tool_call（两种格式都接受；内部会进行规范化）：
...
Agent 在上下文视图中看到的文件**不会**自动重新读取 — 重新格式化只影响磁盘上的文件。后续的 `read_file` 调用会获取格式化后的版本。
...
Claude Code 的 `UserPromptSubmit` 事件特意不作为单独的 Hermes 事件 — `pre_llm_call` 在相同位置触发并且已经支持上下文注入。在这里使用它。
...
非 TTY 运行（消息网关、定时任务、CI）需要这三种之一 — 否则任何新添加的钩子会静默保持未注册状态并记录警告。
...
| `hermes hooks test <event> [--for-tool X] [--payload-file F]` | 针对合成载荷触发每个匹配的钩子并打印解析后的响应 |
| `hermes hooks revoke <command>` | 删除与 `<command>` 匹配的每个允许列表条目（在下次重启时生效） |
| `hermes hooks doctor` | 对于每个已配置的钩子：检查执行权限、允许列表状态、修改时间偏移、JSON 输出有效性和粗略执行时间 |
...
Python 插件钩子和 Shell 钩子都流经同一个 `invoke_hook()` 调度器。Python 插件首先注册（`discover_and_load()`），Shell 钩子其次（`register_from_config()`），因此在平局情况下，Python 的 `pre_tool_call` 阻止决定具有优先权。第一个有效的阻止获胜 — 一旦任何回调产生带有非空消息的 `{"action": "block", "message": str}`，聚合器就会返回。
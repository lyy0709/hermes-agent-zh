...
网关附带一个内置的 `boot-md` 钩子，每次启动时都会查找 `~/.hermes/BOOT.md`。如果文件存在，Agent 会在后台会话中运行其指令。无需安装——只需创建文件。
...
Agent 在后台线程中运行这些指令，因此不会阻塞网关启动。如果无需关注任何事项，Agent 会回复 `[SILENT]`，并且不会发送任何消息。
...
在创建新会话时向外部服务发送 POST 请求：

```yaml
# ~/.hermes/hooks/session-webhook/HOOK.yaml
name: session-webhook
description: 在新会话创建时通知外部服务
events:
  - session:start
  - session:reset
```
...
1. 在消息网关启动时，`HookRegistry.discover_and_load()` 会扫描 `~/.hermes/hooks/` 目录
...
:::info
消息网关钩子仅在**消息网关**（Telegram、Discord、Slack、WhatsApp）中触发。CLI 不会加载消息网关钩子。如需在 CLI 和网关中都生效的钩子，请使用[插件钩子](#plugin-hooks)。
:::
...
| 钩子 | 触发时机 | 返回值 |
|------|-----------|---------|
| [`pre_tool_call`](#pre_tool_call) | 在任何工具执行**之前** | `{"action": "block", "message": str}` 以否决调用 |
| [`post_tool_call`](#post_tool_call) | 在任何工具返回**之后** | 忽略 |
| [`pre_llm_call`](#pre_llm_call) | 每轮对话一次，在工具调用循环**之前** | `{"context": str}` 以在用户消息前添加上下文 |
| [`post_llm_call`](#post_llm_call) | 每轮对话一次，在工具调用循环**之后** | 忽略 |
...
| [`subagent_stop`](#subagent_stop) | 一个 `delegate_task` 子 Agent 已退出时 | 忽略 |
| [`pre_gateway_dispatch`](#pre_gateway_dispatch) | 消息网关收到用户消息后，在认证和分发**之前** | `{"action": "skip" \| "rewrite" \| "allow", ...}` 以影响流程 |
...
**触发位置：** 在 `model_tools.py` 的 `handle_function_call()` 函数内部，在工具的处理器运行之前。每次工具调用触发一次 —— 如果模型并行调用了 3 个工具，则此钩子会触发 3 次。
...
Agent 会短路该工具调用，并将 `message` 作为错误返回给模型。第一个匹配的阻止指令生效（Python 插件优先注册，然后是 shell 钩子）。任何其他返回值都会被忽略，因此现有的仅观察回调函数可以保持不变地继续工作。
...
**触发时机：** 在 `model_tools.py` 的 `handle_function_call()` 函数内，工具的处理程序返回后触发。每次工具调用触发一次。如果工具抛出了未处理的异常（错误会被捕获并作为错误 JSON 字符串返回，此时 `post_tool_call` 会以该错误字符串作为 `result` 触发），则**不会**触发此钩子。
...
在**每轮对话开始时**，工具调用循环开始**之前**触发。这是**唯一一个返回值会被使用的钩子**——它可以向当前轮次的用户消息中注入上下文。
...
**触发时机：** 在 `run_agent.py` 的 `run_conversation()` 函数内，工具循环退出并产生最终响应后触发。受 `if final_response and not interrupted` 保护——因此当用户中途中断或 Agent 达到迭代限制但未产生响应时，它**不会**触发。
...
**使用场景：** 在会话 ID 被丢弃之前持久化最终的会话指标、关闭每个会话的资源、发出最终的遥测事件、排空队列中的写入操作。
...
**触发位置：** 在 `gateway/run.py` 中，在新会话密钥分配之后，但在处理下一条入站消息之前。在网关上，顺序是：`on_session_finalize(old_id)` → 交换 → `on_session_reset(new_id)` → 在第一个入站回合时 `on_session_start(new_id)`。
**返回值：** 忽略。
...
在 `delegate_task` 完成后，**每个子 Agent 触发一次**。无论你委派的是单个任务还是三个任务的一批，此钩子都会为每个子 Agent 触发一次，并在父线程上串行执行。
...
| `child_role` | `str \| None` | 设置在子 Agent 上的编排器角色标签（如果该功能未启用则为 `None`） |
...
**触发位置：** 在 `tools/delegate_tool.py` 中，`ThreadPoolExecutor.as_completed()` 处理完所有子任务之后。触发被调度到父线程，因此钩子作者无需考虑并发回调执行的问题。
...
在消息网关中，**每个传入的 `MessageEvent` 触发一次**，在内部事件守卫之后，但在身份验证/配对和 Agent 调度**之前**。这是消息网关级别消息流策略（仅监听窗口、人工接管、按聊天路由等）的拦截点，这些策略不适合放入任何单一的平台适配器。
...
| `session_store` | `SessionStore` | 用于通过 `session_store.append_to_transcript(...)` 静默摄入对话记录。 |

**触发位置：** 在 `gateway/run.py` 的 `GatewayRunner._handle_message()` 内部，计算完 `is_internal` 后立即触发。**内部事件完全跳过此钩子**（它们是系统生成的——后台进程完成等——不得被面向用户的策略拦截）。

**返回值：** `None` 或一个字典。第一个被识别的操作字典生效；其余插件结果将被忽略。插件回调中的异常会被捕获并记录；发生错误时，消息网关总是会回退到正常调度。

| 返回值 | 效果 |
|--------|--------|
| `{"action": "skip", "reason": "..."}` | 丢弃消息——不进行 Agent 回复、不执行配对流程、不进行身份验证。假定插件已处理该消息（例如，静默摄入对话记录）。 |
...
**使用场景：** 仅监听的群聊（仅在标记时回复；将环境消息缓冲到上下文中）；人工接管（在所有者手动处理聊天时静默摄入客户消息）；按配置文件进行速率限制；策略驱动的路由。
...
在你的 `cli-config.yaml` 中声明 shell 脚本钩子，每当相应的插件钩子事件触发时，Hermes 都会将其作为子进程运行——无论是在 CLI 还是会话中。无需编写 Python 插件。
...
每次事件触发时，Hermes 会为每个匹配的钩子（在匹配器允许的情况下）生成一个子进程，将 JSON 负载通过 **stdin** 管道传入，并将 **stdout** 作为 JSON 读取回来。
...
#### 3. 将 `git status` 注入到每一轮对话中（相当于 Claude-Code 的 `UserPromptSubmit`）
...
非 TTY 运行（消息网关、定时任务、CI）需要以下三者之一——否则任何新添加的钩子将静默保持未注册状态并记录警告。

**脚本编辑会被静默信任。** 允许列表基于确切的命令字符串进行匹配，而非脚本的哈希值，因此编辑磁盘上的脚本不会使授权失效。`hermes hooks doctor` 会标记修改时间偏移，以便您发现编辑并决定是否重新批准。
...
- 仅引用您编写或已完全审查的脚本。
- 将脚本保留在 `~/.hermes/agent-hooks/` 目录内，以便路径易于审计。
- 在拉取共享配置后重新运行 `hermes hooks doctor`，以便在新添加的钩子注册前发现它们。
- 如果您的 config.yaml 在团队中进行版本控制，请以审查 CI 配置相同的方式审查更改 `hooks:` 部分的 PR。

...
如果您想要一个包含推荐配置和实际使用模式的实用设置教程，请参阅[使用 Hermes 的语音模式](/docs/guides/use-voice-mode-with-hermes)。
...
Whisper 有时会从静默或背景噪音中生成幻听文本（"Thank you for watching"、"Subscribe" 等）。Agent 使用一组 26 个已知的跨多种语言的幻觉短语，加上一个捕获重复变体的正则表达式模式，来过滤掉这些内容。
---
...
**服务器频道：** 机器人仅在您 @提及它时才会响应（例如 `@hermesbyt4 你好`）。请确保从提及弹出窗口中选择**机器人用户**，而不是同名的角色。
...
最具沉浸感的语音功能：机器人加入 Discord 语音频道，聆听用户说话，转录他们的语音，通过 Agent 处理，并在语音频道中语音回复。
...
前往 [Discord 开发者门户](https://discord.com/developers/applications) → 您的应用程序 → **安装** → **默认安装设置** → **服务器安装**：
...
:::warning
将机器人重新邀请到它已在的服务器将更新其权限，而不会移除它。您不会丢失任何数据或配置。
:::

#### 2. 特权网关意图

在 [开发者门户](https://discord.com/developers/applications) → 您的应用程序 → **机器人** → **特权网关意图** 中，启用所有三项：
...
:::info
运行 `/voice join` 之前，您必须在一个语音频道中。机器人会加入您所在的同一个语音频道。
:::
...
- Agent 的回复会以文本形式发送到频道，并在语音频道中说出
...
- 确保你在 Discord 中没有静音
- 机器人需要从 Discord 收到 SPEAKING 事件才能映射你的音频 — 请在加入后几秒内开始说话

...
3.  **基础设置正常工作** — 运行 `hermes` 以验证在启用语音前 Agent 能响应文本

:::tip
`~/.hermes/` 目录和默认的 `config.yaml` 会在你首次运行 `hermes` 时自动创建。你只需要为 API 密钥手动创建 `~/.hermes/.env` 文件。
:::
...
启用 TTS 后，Agent 会在生成文本时**逐句**朗读其回复 — 你无需等待完整响应：
...
Whisper 有时会从静默或背景噪音中生成幻听文本（"Thank you for watching"、"Subscribe" 等）。Agent 使用一组 26 个已知的跨多种语言的幻觉短语，加上一个能捕获重复变体的正则表达式模式，来过滤掉这些内容。
---
...
| **服务器频道** | 在机器人所在的文本频道中输入 | 是 (`@机器人名`) | 必须将机器人邀请到服务器 |

**私信（个人使用推荐）：** 只需打开与机器人的私信并输入——无需 @ 提及。语音回复和所有命令在频道中的工作方式相同。

**服务器频道：** 机器人只在你 @ 提及时才会回复（例如 `@hermesbyt4 你好`）。请确保从提及弹出窗口中选择**机器人用户**，而不是同名的角色。
...
最沉浸式的语音功能：机器人加入 Discord 语音频道，监听用户说话，转录他们的语音，通过 Agent 处理，并在语音频道中语音回复。

### 设置

#### 1. Discord 机器人权限

如果你已经为文本设置了一个 Discord 机器人（参见 [Discord 设置指南](../messaging/discord.md)），你需要添加语音权限。
...
所有三个对于完整的语音频道功能都是必需的。**服务器成员意图**尤其关键——没有它，机器人无法识别谁在语音频道中说话。
...
:::info
在运行 `/voice join` 之前，你必须在一个语音频道中。机器人会加入你所在的同一个语音频道。
:::

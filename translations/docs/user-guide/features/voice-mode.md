...
语音模式在 **经典 CLI** (`hermes chat`) 和 **TUI** (`hermes --tui`) 中均可用。两者行为相同 —— 相同的斜杠命令、相同的 VAD 静默检测、相同的流式 TTS、相同的幻觉过滤器。TUI 还会将崩溃诊断日志转发到 `~/.hermes/logs/`，以便在奇特的音频后端上出现按键通话失败时，可以报告完整的堆栈跟踪，而不是悄无声息地消失。
...
启用 TTS 时，Agent 会在生成文本时**逐句**说出其回复——你无需等待完整响应：
...
Whisper 有时会从静默或背景噪音中生成幻听文本（"Thank you for watching"、"Subscribe" 等）。Agent 会使用一组 26 个已知的跨语言幻觉短语，外加一个捕获重复变体的正则表达式模式来过滤掉这些内容。
...
最具沉浸感的语音功能：机器人加入 Discord 语音频道，听取用户讲话，转录他们的语音，通过 Agent 处理，并在语音频道中说出回复。
...
所有三项对于完整的语音频道功能都是必需的。**Server Members Intent** 尤其关键——没有它，机器人无法识别谁在语音频道中讲话。
...
:::info
运行 `/voice join` 之前，你必须在一个语音频道中。机器人会加入你所在的同一个语音频道。
:::
...
tts:
  ...
  openai:
    ...
    base_url: "https://api.openai.com/v1"  # 可选：覆盖自托管或 OpenAI 兼容端点的 URL
  neutts:
    ref_audio: ''
    ref_text: ''
...
1.  输入 `@` 并选择**机器人用户**（带有 # 标识符），而不是同名的**角色**
2.  或者改用私信 — 无需提及
...
-   确保你在 Discord 中没有静音
-   机器人需要从 Discord 收到 SPEAKING 事件才能映射你的音频 — 在加入后的几秒钟内开始说话

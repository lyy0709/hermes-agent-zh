...
图表会以内联图片形式显示。PDF 报告会显示为可下载的文件。电子表格会以 `.xlsx` 格式上传。Agent 无需编写 `MEDIA:` 标签或做任何特殊处理——它只需生成文件并在响应中提及文件的绝对路径。网关会从文本中提取路径，将其从可见消息中移除，并以原生方式上传文件。
...
`.py`、`.log` 和其他源文件扩展名被有意排除，这样 Agent 就不会自动发送任意的源文件；如果你想向用户发送代码，请使用代码块。

## 鼓励 Agent 生成文件

默认情况下，Agent 不会主动生成文件——它需要知道要这样做。有两种方法可以引导它：

**按会话：** 明确要求（"将比较结果以图表形式发送给我"、"将数据以 CSV 格式返回"）或编写你自己的自定义指令/人格条目，使其在消息平台上偏向于生成文件形式的回复。

**项目级别：** 在 Agent 工作的项目中的 `AGENTS.md` / `CLAUDE.md` / `.cursorrules` 中添加这种偏好，或者添加到你在 `~/.hermes/config.yaml` 中 `agent.custom_instructions` 下的全局自定义指令中。

Agent 需要使用的机制很简单：将文件渲染到绝对路径（例如 `/tmp/q3-revenue.png`），并在回复中以纯文本形式提及该路径。网关会处理剩下的事情。围栏代码块或反引号内的路径会被忽略，因此代码示例永远不会被破坏。

## 看板：文件随任务完成通知一起发送

如果你使用 Hermes 的看板多 Agent 工作流，工作 Agent 可以将可交付文件附加到它们的 `kanban_complete` 调用中：
...
当网关通知器将"任务完成"消息发送给在 Slack/Telegram 等平台订阅了该任务的人时，它还会将每个文件作为原生附件上传到该聊天中。用户可以在一个地方获得可交付成果和摘要。

通知器运行时磁盘上不存在的文件会被静默跳过。

## 通过 MCP 连接更多服务

除了文件交付流水线，Agent 还可以通过 MCP（Model Context Protocol）连接到其他服务。MCP 生态系统为大多数流行工具提供了社区服务器——安装你需要的任何服务：
...
Perplexity Computer 的 Slack 集成基于相同的理念：Agent 生成一个可交付成果（图表、PDF、幻灯片），并将其作为原生附件发布回线程中。Hermes Agent 的交付模式在本地提供了相同的面向用户的模式：

*   生成发生在用户自己的 venv / 沙盒中（没有远程租户）。

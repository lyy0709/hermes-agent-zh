...
:::tip Nous 订阅用户
如果您拥有付费的 [Nous Portal](https://portal.nousresearch.com) 订阅，您可以通过 **[工具网关](tool-gateway.md)** 使用浏览器自动化功能，无需任何单独的 API 密钥。新安装可以通过运行 `hermes setup --portal` 登录并一次性开启所有网关工具；现有安装可以通过 `hermes model` 或 `hermes tools` 选择 **Nous Subscription** 作为浏览器提供商。
:::
...
在 [browser-use.com](https://browser-use.com) 获取您的 API 密钥。Browser Use 通过其 REST API 提供云浏览器。如果同时设置了 Browserbase 和 Browser Use 的凭据，Browserbase 具有优先权。
...
[Camofox](https://github.com/jo-inc/camofox-browser) 是一个自托管的 Node.js 服务器，包装了 Camoufox（一个具有 C++ 指纹欺骗功能的 Firefox 分支）。它提供本地反检测浏览，无需云依赖。
...
启用 VNC 后，浏览器将以有头模式运行，并可以在浏览器中通过 `http://localhost:6080` (noVNC) 实时观看。你也可以使用原生 VNC 客户端连接到 `localhost:5901`。
...
默认情况下，每个 Camofox 会话都会获得一个随机身份 —— Cookie 和登录状态在 Agent 重启后不会保留。要启用持久化浏览器会话，请将以下内容添加到 `~/.hermes/config.yaml`：
...
如果标志被放在了错误的路径下，Hermes 会静默回退到使用随机的临时 `userId`，你的登录状态将在每次会话中丢失。
:::

##### Hermes 做了什么
- 向 Camofox 发送一个确定性的、作用域限定在配置文件内的 `userId`，以便服务器可以在不同会话间复用相同的 Firefox 配置文件。
...
Hermes 从配置文件作用域的目录 `~/.hermes/browser_auth/camofox/`（或非默认配置文件下 `$HERMES_HOME` 下的等效目录）派生出稳定的 `userId`。实际的浏览器配置文件数据存储在 Camofox 服务器端，以该 `userId` 为键。要完全重置一个持久化配置文件，请在 Camofox 服务器上清除它，并删除 Hermes 配置文件中对应的状态目录。
...
| `browser.camofox.session_key` | `CAMOFOX_SESSION_KEY` | 创建标签页时发送的 `sessionKey`（又名 `listItemId`）。用于在接管时匹配现有的标签页。如果未设置，则默认为每个任务的值。 |
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | 当为 true 时，Hermes 在首次使用时调用 `GET /tabs?userId=<user_id>`，并在创建新标签页之前复用现有的标签页。 |
...
- Hermes **不会**调用 `DELETE /sessions/<user_id>` —— 该端点会清除所有用户数据，如果触发，将会销毁外部应用程序的会话。
...
**选择 `session_key`：** 如果你希望 Hermes 可靠地附加到*特定*的现有标签页，请将 `session_key` 设置为外部应用程序创建该标签页时使用的 `listItemId`。如果你不设置 `session_key` 而只设置 `user_id`，Hermes 会生成一个每个任务唯一的 `session_key`（`task_<id>`）—— Hermes 将与外部应用程序共享 Cookie 和配置文件，但会同时打开自己的标签页，而不是复用现有的。
**并发注意事项：** 外部应用和 Hermes 可以同时驱动同一个 Camofox `userId`，但 Camofox 不会在客户端之间协调每个标签页的焦点。请在应用层协调所有权（例如，当 Hermes 运行时，外部应用暂停）。

#### VNC 实时视图

当 Camofox 在带界面模式下运行（显示可见的浏览器窗口）时，它会在其健康检查响应中暴露一个 VNC 端口。Hermes 会自动发现这一点，并在导航响应中包含 VNC URL，因此 Agent 可以分享一个链接供您实时观看浏览器。

### 通过 CDP 连接本地 Chromium 系列浏览器 (`/browser connect`)

除了云提供商，您还可以通过 Chrome DevTools Protocol (CDP) 将 Hermes 浏览器工具附加到您自己正在运行的 Chrome、Brave、Chromium 或 Edge 实例。这在您希望实时查看 Agent 的操作、与需要您自己的 cookies/会话的页面交互，或避免云浏览器成本时非常有用。

:::note
`/browser connect` 是一个**交互式 CLI 斜杠命令**——它不由消息网关分发。如果您尝试在 WebUI、Telegram、Discord 或其他网关聊天中运行它，该消息将作为纯文本发送给 Agent，并且命令不会执行。请从终端启动 Hermes（`hermes` 或 `hermes chat`）并在那里输入 `/browser connect`。
:::
...
如果浏览器尚未在远程调试模式下运行，Hermes 将尝试使用 `--remote-debugging-port=9222` 自动启动一个受支持的 Chromium 系列浏览器。检测范围包括 Brave、Google Chrome、Chromium 和 Microsoft Edge，以及常见的 Linux 安装路径，例如 `/opt/brave-bin/brave` 和 `/snap/bin/brave`。

:::tip
要手动启动启用了 CDP 的 Chromium 系列浏览器，请使用专用的用户数据目录，这样即使浏览器已使用您的常规配置文件运行，调试端口也能实际启动：
...
**为什么使用 `--user-data-dir`？** 如果没有它，在常规实例已在运行时启动 Chromium 系列浏览器通常会在现有进程上打开一个新窗口——而该现有进程启动时没有使用 `--remote-debugging-port`，因此端口 9222 永远不会打开。专用的用户数据目录会强制启动一个新的浏览器进程，调试端口才会实际监听。`--no-first-run --no-default-browser-check` 会跳过新配置文件的首次启动向导。
:::

当通过 CDP 连接时，所有浏览器工具（`browser_navigate`、`browser_click` 等）都在您的实时浏览器实例上操作，而不是启动云会话。

### WSL2 + Windows Chrome：优先使用 MCP 而非 `/browser connect`

如果 Hermes 在 WSL2 内运行，但您想要控制的 Chrome 窗口在 Windows 主机上运行，`/browser connect` 通常不是最佳选择。

原因：

- `/browser connect` 期望 Hermes 本身能够访问一个可用的 CDP 端点
- 现代 Chrome 实时调试会话通常暴露一个主机本地的端点，该端点无法像经典的 `9222` 端口那样直接从 WSL 访问
- 即使 Windows Chrome 可调试，最简洁的集成方式通常是让 Windows 端的浏览器 MCP 服务器附加到 Chrome，并让 Hermes 与该 MCP 服务器通信

对于这种设置，建议通过 Hermes MCP 支持使用 `chrome-devtools-mcp`。
...
如果您**没有**设置任何云凭证并且不使用 `/browser connect`，Hermes 仍然可以通过由 `agent-browser` 驱动的本地 Chromium 安装来使用浏览器工具。

### 可选环境变量

```bash
# 用于更好的 CAPTCHA 解决能力的住宅代理（默认："true"）
...
# 额外的 Chromium 启动标志（逗号或换行分隔）。当 Hermes 检测到 root 或受 AppArmor 限制的
# 非特权用户命名空间（Ubuntu 23.10+、DGX Spark、许多容器镜像）时，会自动注入
# `--no-sandbox,--disable-dev-shm-usage`，因此大多数用户无需设置此项。
# 仅在需要 Hermes 未自动添加的标志时才手动设置；设置此变量会禁用自动注入。
...
导航到指定 URL。必须在调用其他浏览器工具之前调用。初始化 Browserbase 会话。
...
:::tip
对于简单的信息检索，建议优先使用 `web_search` 或 `web_extract` —— 它们更快、更便宜。当需要**与页面交互**时（点击按钮、填写表单、处理动态内容），再使用浏览器工具。
:::
...
当使用 `expression` 参数调用时，`browser_console` 也会评估 JavaScript —— 与 DevTools 控制台相同，结果会被解析返回（JSON 序列化的对象变为字典；原始值保持原始值）。
...
当当前会话有 CDP 监督器处于活动状态时（对于任何在支持 CDP 的后端上运行过 `browser_navigate` 的会话来说很典型），评估会通过监督器的持久 WebSocket 运行 —— 没有子进程启动成本。否则，会回退到标准的 agent-browser CLI 路径。两种方式的行为相同；只有延迟会变化。

### `browser_cdp`

原始 Chrome DevTools 协议透传 —— 用于其他工具未涵盖的浏览器操作的逃生舱。用于处理原生对话框、iframe 范围内的评估、cookie/网络控制，或 Agent 需要的任何 CDP 动词。
...
**跨域 iframe：** 传递 `frame_id`（来自 `browser_snapshot.frame_tree.children[]`，其中 `is_oopif=true`）以通过监督器针对该 iframe 的实时会话路由 CDP 调用。这就是在 Browserbase 上跨域 iframe 内部的 `Runtime.evaluate` 的工作原理，否则无状态 CDP 连接会遇到签名 URL 过期。示例：
...
同源 iframe 不需要 `frame_id` —— 改为使用来自顶级 `Runtime.evaluate` 的 `document.querySelector('iframe').contentDocument`。
...
**检测是自动进行的**，通过一个持久的 CDP 监控器实现 — 每个任务一个 WebSocket，订阅 Page/Runtime/Target 事件。监控器还会在快照中填充一个 `frame_tree` 字段，以便 Agent 可以查看当前页面的 iframe 结构，包括跨域 (OOPIF) iframe。
...
:::note
如果你的计划不支持付费功能，Hermes 会自动回退 — 首先禁用 `keepAlive`，然后是代理 — 以便免费计划上的浏览仍然可以工作。
:::

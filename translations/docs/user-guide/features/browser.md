---
title: 浏览器自动化
description: 通过多种提供商控制浏览器，包括通过 CDP 连接本地 Chromium 系列浏览器或使用云浏览器，实现网页交互、表单填写、数据抓取等功能。
sidebar_label: 浏览器
sidebar_position: 5
---
...
在 [browser-use.com](https://browser-use.com) 获取您的 API 密钥。Browser Use 通过其 REST API 提供云浏览器。如果同时设置了 Browserbase 和 Browser Use 的凭据，Browserbase 将优先使用。
...
当配置了云提供商时，Hermes 会自动为解析为私有/回环/LAN 地址的 URL（`localhost`、`127.0.0.1`、`192.168.x.x`、`10.x.x.x`、`172.16-31.x.x`、`*.local`、`*.lan`、`*.internal`、IPv6 回环 `::1`、链路本地 `169.254.x.x`）启动一个**本地 Chromium 侧车**。公共 URL 在同一会话中继续使用云提供商。

这解决了常见的“我在本地开发但使用 Browserbase”的工作流程 — Agent 可以截取 `http://localhost:3000` 的仪表板截图**并且**抓取 `https://github.com`，而无需您切换提供商或禁用 SSRF 防护。云提供商永远不会看到私有 URL。
...
禁用自动路由后，私有 URL 将被拒绝，并显示 `"Blocked: URL targets a private or internal address"`，除非您还设置了 `browser.allow_private_urls: true`（这将允许云提供商尝试访问它们 — 通常不会成功，因为 Browserbase 等无法访问您的 LAN）。

要求：本地侧车使用与纯本地模式相同的 `agent-browser` CLI，因此您需要安装它（`hermes setup tools → Browser Automation` 会自动安装）。从公共 URL 导航后重定向到私有地址仍然会被阻止（您不能使用重定向到内部的技巧通过公共路径访问您的 LAN）。
...
# 强制进行干净的重建（例如，在升级 VERSION/RELEASE 后）
make reset
...
`make up` 会立即启动默认容器。如果您想要自定义运行时设置，例如更大的 Node 堆、VNC 或持久化的配置文件目录，请先构建镜像，然后自己运行它：
...
```
启用 VNC 后，浏览器将以有头模式运行，并可在你的浏览器中通过 `http://localhost:6080` (noVNC) 实时观看。你也可以使用原生 VNC 客户端连接到 `localhost:5901`。
...
默认情况下，每个 Camofox 会话使用一个随机身份——Cookie 和登录状态在 Agent 重启后不会保留。要启用持久化浏览器会话，请在 `~/.hermes/config.yaml` 中添加以下内容：
...
如果该标志放在错误的路径下，Hermes 会静默回退到使用随机的临时 `userId`，你的登录状态将在每次会话中丢失。
:::
...
##### Hermes 没有做什么
- 它不会强制 Camofox 服务器进行持久化。Hermes 只发送一个稳定的 `userId`；服务器必须通过将该 `userId` 映射到持久的 Firefox 配置文件目录来遵守它。
...
Hermes 从配置文件作用域的目录 `~/.hermes/browser_auth/camofox/`（或非默认配置文件下 `$HERMES_HOME` 中的等效目录）派生出稳定的 `userId`。实际的浏览器配置文件数据存储在 Camofox 服务器端，以该 `userId` 为键。要完全重置一个持久化配置文件，请在 Camofox 服务器端清除它，并删除 Hermes 配置文件中对应的状态目录。
...
| 设置 | 环境变量 | 效果 |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Hermes 在创建标签页时使用的 Camofox `userId`。设置此项将使会话进入“外部管理”模式。 |
| `browser.camofox.session_key` | `CAMOFOX_SESSION_KEY` | 创建标签页时发送的 `sessionKey`（又名 `listItemId`）。用于在接管时匹配现有标签页。如果未设置，则默认为每个任务的值。 |
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | 当为 true 时，Hermes 在首次使用时调用 `GET /tabs?userId=<user_id>`，并在创建新标签页之前重用现有标签页。 |

环境变量优先于 `config.yaml`。任一种形式都有效：
...
**标签页接管的工作原理（当 `adopt_existing_tab: true` 时）：**
...
接管仅在会话的 `tab_id` 被填充之前触发。如果外部应用程序在运行过程中关闭了被接管的标签页，下一次浏览器工具调用将出现 Camofox 错误——Hermes 不会在每次调用时重新轮询新的标签页。

**选择 `session_key`：** 如果你希望 Hermes 可靠地附加到*特定*的现有标签页，请将 `session_key` 设置为外部应用程序创建该标签页时使用的 `listItemId`。如果你不设置 `session_key` 而只设置 `user_id`，Hermes 会生成一个每个任务唯一的 `session_key`（`task_<id>`）—— Hermes 将与外部应用程序共享 Cookie 和配置文件，但会同时打开自己的标签页，而不是重用一个。

**并发注意事项：** 外部应用程序和 Hermes 可以同时驱动同一个 Camofox `userId`，但 Camofox 不会在客户端之间协调每个标签页的焦点。请在应用层协调所有权（例如，在 Hermes 运行时，外部应用程序暂停）。
#### VNC 实时视图

当 Camofox 以有头模式运行（显示可见的浏览器窗口）时，它会在健康检查响应中暴露一个 VNC 端口。Hermes 会自动发现此端口，并在导航响应中包含 VNC URL，因此 Agent 可以分享一个链接供您实时观看浏览器操作。

### 通过 CDP 连接本地 Chromium 系列浏览器 (`/browser connect`)

除了云服务提供商，您还可以通过 Chrome DevTools Protocol (CDP) 将 Hermes 浏览器工具连接到您自己正在运行的 Chrome、Brave、Chromium 或 Edge 实例。这在您希望实时查看 Agent 的操作、与需要您自己的 cookies/会话的页面交互，或避免云浏览器成本时非常有用。

:::note
`/browser connect` 是一个**交互式 CLI 斜杠命令**——它不由消息网关分发。如果您尝试在 WebUI、Telegram、Discord 或其他消息网关聊天中运行它，该消息将作为纯文本发送给 Agent，命令不会执行。请从终端启动 Hermes (`hermes` 或 `hermes chat`)，并在那里输入 `/browser connect`。
:::
...
如果浏览器尚未在远程调试模式下运行，Hermes 将尝试自动启动一个支持远程调试的 Chromium 系列浏览器，使用 `--remote-debugging-port=9222` 参数。检测范围包括 Brave、Google Chrome、Chromium 和 Microsoft Edge，以及常见的 Linux 安装路径，例如 `/opt/brave-bin/brave` 和 `/snap/bin/brave`。

:::tip
要手动启动启用了 CDP 的 Chromium 系列浏览器，请使用专用的用户数据目录，这样即使浏览器已使用您的常规配置文件运行，调试端口也能正常启动：
...
**为什么需要 `--user-data-dir`？** 如果没有它，在常规实例已在运行时启动 Chromium 系列浏览器，通常会在现有进程中打开一个新窗口——而该现有进程并未使用 `--remote-debugging-port` 启动，因此端口 9222 永远不会打开。专用的用户数据目录会强制启动一个新的浏览器进程，调试端口才会实际监听。`--no-first-run --no-default-browser-check` 会跳过新配置文件的首次启动向导。
:::

通过 CDP 连接后，所有浏览器工具 (`browser_navigate`、`browser_click` 等) 都将在您的实时浏览器实例上操作，而不是启动云会话。

### WSL2 + Windows Chrome：优先选择 MCP 而非 `/browser connect`

如果 Hermes 在 WSL2 内运行，但您想要控制的 Chrome 窗口运行在 Windows 主机上，`/browser connect` 通常不是最佳选择。

原因：

- `/browser connect` 期望 Hermes 本身能够访问一个可用的 CDP 端点
- 现代 Chrome 实时调试会话通常暴露一个主机本地的端点，该端点无法像传统的 `9222` 端口那样直接从 WSL 访问
- 即使 Windows Chrome 可调试，最简洁的集成方式通常是让 Windows 端的浏览器 MCP 服务器连接到 Chrome，并让 Hermes 与该 MCP 服务器通信
...
如果您**没有**设置任何云服务凭证，并且不使用 `/browser connect`，Hermes 仍然可以通过 `agent-browser` 驱动的本地 Chromium 安装来使用浏览器工具。

### 可选环境变量

```bash
# 用于更好的验证码解决的住宅代理 (默认: "true")
...
# 额外的 Chromium 启动标志（逗号或换行分隔）。当 Hermes 检测到 root 用户或受 AppArmor 限制的非特权用户命名空间（Ubuntu 23.10+、DGX Spark、许多容器镜像）时，会自动注入 `--no-sandbox,--disable-dev-shm-usage`，因此大多数用户无需设置此项。仅在需要 Hermes 未自动添加的标志时才手动设置；设置此变量会禁用自动注入。
...
获取当前页面无障碍树的基于文本的快照。返回带有引用 ID（如 `@e1`、`@e2`）的交互式元素，供 `browser_click` 和 `browser_type` 使用。
...
屏幕截图会持久保存，文件路径会与 AI 分析结果一起返回。在消息平台（Telegram、Discord、Slack、WhatsApp）上，你可以要求 Agent 分享屏幕截图——它将通过 `MEDIA:` 机制作为原生照片附件发送。
...
`browser_console` 在调用时如果带有 `expression` 参数，也会执行 JavaScript 表达式——其形式与 DevTools 控制台相同，结果会被解析返回（JSON 序列化的对象变为字典；原始值保持原始值）。
...
当当前会话有 CDP 监管器处于活动状态时（对于任何针对支持 CDP 的后端运行过 `browser_navigate` 的会话来说很典型），评估将通过监管器的持久 WebSocket 运行——没有子进程启动成本。否则，将回退到标准的 agent-browser CLI 路径。两种方式的行为相同；只有延迟会发生变化。

### `browser_cdp`

原始 Chrome DevTools 协议透传——用于其他工具未涵盖的浏览器操作的逃生舱口。用于原生对话框处理、iframe 作用域内的评估、cookie/网络控制或 Agent 需要的任何 CDP 动词。

**仅在会话开始时 CDP 端点可达时才可用**——这意味着 `/browser connect` 已附加到正在运行的 Chrome、Brave、Chromium 或 Edge 浏览器，或者在 `config.yaml` 中设置了 `browser.cdp_url`。默认的本地 agent-browser 模式、Camofox 和云提供商（Browserbase、Browser Use、Firecrawl）目前不向此工具公开 CDP——云提供商有每个会话的 CDP URL，但实时会话路由是后续功能。
...
浏览器级别的方法（`Target.*`、`Browser.*`、`Storage.*`）省略 `target_id`。页面级别的方法（`Page.*`、`Runtime.*`、`DOM.*`、`Emulation.*`）需要来自 `Target.getTargets` 的 `target_id`。每个无状态调用都是独立的——会话在调用之间不会持久化。

**跨域 iframe：** 传递 `frame_id`（来自 `browser_snapshot.frame_tree.children[]`，其中 `is_oopif=true`）以通过监管器针对该 iframe 的实时会话路由 CDP 调用。这就是在 Browserbase 上跨域 iframe 内部 `Runtime.evaluate` 的工作原理，无状态 CDP 连接会遇到签名 URL 过期问题。示例：
...
**检测自动进行**，通过一个持久的 CDP 监控器实现——每个任务一个 WebSocket，用于订阅 Page/Runtime/Target 事件。该监控器还会在快照中填充一个 `frame_tree` 字段，以便 Agent 能够查看当前页面的 iframe 结构，包括跨域 (OOPIF) iframe。
...
**在 Browserbase 上的工作原理。** Browserbase 的 CDP 代理会在服务器端约 10 毫秒内自动关闭真实的原生对话框，因此我们无法使用 `Page.handleJavaScriptDialog`。监控器通过 `Page.addScriptToEvaluateOnNewDocument` 注入一个小脚本，该脚本用同步 XHR 覆盖 `window.alert`/`confirm`/`prompt`。我们通过 `Fetch.enable` 拦截这些 XHR——页面的 JS 线程会一直阻塞在 XHR 上，直到我们调用 `Fetch.fulfillRequest` 并传入 Agent 的响应。`prompt()` 的返回值会原封不动地返回到页面 JS 中。
...
| `auto_accept` | 捕获，立即接受。在导航到带有激进 `beforeunload` 提示的页面时很有用。 |

`browser_snapshot.frame_tree` 内的 **Frame tree** 被限制为最多 30 个 frame 和 OOPIF 深度 2，以在广告繁多的页面上限制负载大小。当达到限制时，会显示一个 `truncated: true` 标志；需要完整树的 Agent 可以使用 `browser_cdp` 配合 `Page.getFrameTree`。
...
:::note
如果你的计划不包含付费功能，Hermes 会自动回退——首先禁用 `keepAlive`，然后是代理——以便在免费计划上浏览仍然有效。
:::
...
- **快照大小**——大型页面可能会在 8000 字符处被截断或由 LLM 总结

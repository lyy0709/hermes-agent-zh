...
:::tip Nous 订阅用户
如果您拥有付费的 [Nous Portal](https://portal.nousresearch.com) 订阅，您可以通过 **[工具网关](tool-gateway.md)** 使用浏览器自动化，无需任何单独的 API 密钥。新安装可以运行 `hermes setup --portal` 登录并一次性开启所有网关工具；现有安装可以通过 `hermes model` 或 `hermes tools` 选择 **Nous Subscription** 作为浏览器提供商。
:::
...
在 [browser-use.com](https://browser-use.com) 获取您的 API 密钥。Browser Use 通过其 REST API 提供云浏览器。如果同时设置了 Browserbase 和 Browser Use 的凭据，Browserbase 将优先。
...
当配置了云提供商时，Hermes 会自动为解析为私有/环回/LAN 地址的 URL（`localhost`、`127.0.0.1`、`192.168.x.x`、`10.x.x.x`、`172.16-31.x.x`、`*.local`、`*.lan`、`*.internal`、IPv6 环回 `::1`、链路本地 `169.254.x.x`）启动一个**本地 Chromium 辅助进程**。公共 URL 在同一会话中继续使用云提供商。

这解决了常见的“我在本地开发但使用 Browserbase”的工作流问题 — Agent 可以截取 `http://localhost:3000` 的仪表板截图**并且**抓取 `https://github.com`，而您无需切换提供商或禁用 SSRF 防护。云提供商永远不会看到私有 URL。
...
要求：本地辅助进程使用与纯本地模式相同的 `agent-browser` CLI，因此您需要安装它（`hermes setup tools → Browser Automation` 会自动安装它）。从公共 URL 导航后重定向到私有地址仍然会被阻止（您不能使用重定向到内部的技巧通过公共路径访问您的 LAN）。
...
启用 VNC 后，浏览器将以有头模式运行，你可以在浏览器中通过 `http://localhost:6080` (noVNC) 实时观看。你也可以使用原生 VNC 客户端连接到 `localhost:5901`。
...
如果 Camofox 在 Docker 中运行，并且你希望它能打开宿主机上提供的 Web 应用，请启用回环地址重写。`CAMOFOX_URL` 仍应指向宿主机发布的控制 API，但像 `http://127.0.0.1:3000` 这样的页面 URL 必须从容器内部以 `http://host.docker.internal:3000` 的形式打开：
...
browser:
  camofox:
    ...
    loopback_host_alias: host.docker.internal  # 默认值；如果需要，可以使用局域网 IP
```
...
重写仅适用于带有回环主机（`localhost`、`127.0.0.1`、`::1`）的页面导航 URL。它不会改变 `CAMOFOX_URL`。对于非 Docker 安装的 Camofox，请保持禁用此功能，因为浏览器已经在宿主机上运行，回环 URL 是正确的。
...
如果标志放在了错误的路径下，Hermes 会静默回退到一个随机的临时 `userId`，你的登录状态将在每次会话中丢失。
:::

##### Hermes 做了什么
- 向 Camofox 发送一个确定性的、基于配置文件的 `userId`，以便服务器可以在不同会话间复用同一个 Firefox 配置文件。
- 在清理时跳过服务器端的上下文销毁，因此 Cookie 和登录状态可以在 Agent 任务之间保留。
- 将 `userId` 限定在活动的 Hermes 配置文件中，因此不同的 Hermes 配置文件会获得不同的浏览器配置文件（配置文件隔离）。
...
Hermes 从配置文件作用域的目录 `~/.hermes/browser_auth/camofox/`（或非默认配置文件下 `$HERMES_HOME` 中的等效目录）派生出稳定的 `userId`。实际的浏览器配置文件数据存在于 Camofox 服务器端，以该 `userId` 为键。要完全重置一个持久化配置文件，请在 Camofox 服务器端清除它，并删除 Hermes 配置文件中对应的状态目录。

#### 外部管理的 Camofox 会话

当另一个应用程序（桌面助手、自定义集成、另一个 Agent）驱动可见的 Camofox 浏览器时，配置 Hermes 在同一身份内操作，而不是生成自己独立的配置文件。
...
| `browser.camofox.session_key` | `CAMOFOX_SESSION_KEY` | 创建标签页时发送的 `sessionKey`（又名 `listItemId`）。用于在接管时匹配现有标签页。如果未设置，默认为每个任务的值。 |
...
1.  在进程启动后的第一次浏览器工具调用时，Hermes 会发起 `GET /tabs?userId=<user_id>` 请求（5秒超时）。
2.  如果响应中的任何标签页的 `listItemId == session_key`，Hermes 将接管该组中最近创建的那个标签页。
3.  否则，Hermes 将接管该用户最近创建的标签页（任何 `listItemId`）。
4.  如果不存在标签页或请求失败，Hermes 将在下一次操作时回退到创建新标签页。

接管机制仅在会话的 `tab_id` 被填充之前触发。如果外部应用在运行中途关闭了被接管的标签页，下一次浏览器工具调用将出现 Camofox 错误——Hermes 不会在每次调用时重新轮询新的标签页。

**选择 `session_key`：** 如果你希望 Hermes 可靠地附加到*特定*的现有标签页，请将 `session_key` 设置为外部应用创建该标签页时使用的 `listItemId`。如果你不设置 `session_key` 而只设置 `user_id`，Hermes 会生成一个基于任务的 `session_key`（`task_<id>`）——Hermes 将与外部应用共享 Cookie 和配置文件，但会打开自己的标签页，而不是复用现有的。

**并发注意事项：** 外部应用和 Hermes 可以同时驱动同一个 Camofox `userId`，但 Camofox 不会在客户端之间协调每个标签页的焦点。请在应用层协调所有权（例如，在 Hermes 运行时，外部应用暂停）。

#### VNC 实时视图

当 Camofox 在带界面模式下运行（显示可见的浏览器窗口）时，它会在其健康检查响应中暴露一个 VNC 端口。Hermes 会自动发现此端口，并在导航响应中包含 VNC URL，因此 Agent 可以分享链接让你实时观看浏览器操作。
...
如果浏览器尚未以远程调试模式运行，Hermes 将尝试自动启动一个支持远程调试的 Chromium 系列浏览器，并附带 `--remote-debugging-port=9222` 参数。检测范围包括 Brave、Google Chrome、Chromium 和 Microsoft Edge，以及常见的 Linux 安装路径，如 `/opt/brave-bin/brave` 和 `/snap/bin/brave`。

:::tip
要手动启动启用了 CDP 的 Chromium 系列浏览器，请使用专用的用户数据目录，这样即使浏览器已使用你的常规配置文件运行，调试端口也能正常启用：
...
**为什么需要 `--user-data-dir`？** 如果没有它，在常规实例已在运行时启动 Chromium 系列浏览器，通常会在现有进程中打开一个新窗口——而该现有进程启动时没有 `--remote-debugging-port` 参数，因此端口 9222 永远不会打开。专用的用户数据目录会强制启动一个新的浏览器进程，调试端口才会真正监听。`--no-first-run --no-default-browser-check` 会跳过新配置文件的首次启动向导。
:::
...
# 示例：600 (10分钟), 1800 (30分钟), 21600 (6小时 最大值)
...
获取浏览器控制台输出（日志/警告/错误消息）以及当前页面未捕获的 JavaScript 异常。对于检测无障碍树中不出现的静默 JS 错误至关重要。
...
`browser_console` 在调用时如果带有 `expression` 参数，也会执行 JavaScript 表达式 —— 其形式与 DevTools 控制台相同，结果会被解析返回（JSON 序列化的对象变为字典；原始值保持原始类型）。
...
当当前会话有 CDP 监管器处于活动状态时（对于任何针对支持 CDP 的后端运行过 `browser_navigate` 的会话来说很典型），评估会通过监管器的持久 WebSocket 运行 —— 没有子进程启动开销。否则，会回退到标准的 agent-browser CLI 路径。两种方式的行为是相同的；只有延迟会发生变化。

### `browser_cdp`

原始 Chrome DevTools 协议透传 —— 用于其他工具未涵盖的浏览器操作的逃生舱。用于处理原生对话框、iframe 作用域内的评估、cookie/网络控制，或 Agent 需要的任何 CDP 命令。

**仅在会话开始时可以访问到 CDP 端点时才可用** —— 这意味着 `/browser connect` 已附加到正在运行的 Chrome、Brave、Chromium 或 Edge 浏览器，或者在 `config.yaml` 中设置了 `browser.cdp_url`。默认的本地 agent-browser 模式、Camofox 和云提供商（Browserbase、Browser Use、Firecrawl）目前不向此工具公开 CDP —— 云提供商有每个会话的 CDP URL，但实时会话路由是后续功能。
...
**跨域 iframe：** 传递 `frame_id`（来自 `browser_snapshot.frame_tree.children[]`，其中 `is_oopif=true`）以通过监管器针对该 iframe 的实时会话路由 CDP 调用。这就是在 Browserbase 上跨域 iframe 内部执行 `Runtime.evaluate` 的方式，否则无状态 CDP 连接会遇到签名 URL 过期问题。示例：
...
同源 iframe 不需要 `frame_id` —— 改为从顶层的 `Runtime.evaluate` 使用 `document.querySelector('iframe').contentDocument`。
...
**检测自动发生**，通过一个持久的 CDP 监控器实现 — 每个任务一个 WebSocket，订阅 Page/Runtime/Target 事件。该监控器还会在快照中填充一个 `frame_tree` 字段，以便 Agent 可以查看当前页面的 iframe 结构，包括跨域 (OOPIF) iframe。
...
`browser_snapshot.frame_tree` 中的 **框架树** 被限制为最多 30 个框架和 OOPIF 深度 2，以在广告繁多的页面上限制负载大小。当达到限制时，会显示一个 `truncated: true` 标志；需要完整树的 Agent 可以使用 `browser_cdp` 配合 `Page.getFrameTree`。
...
:::note
如果你的计划不包含付费功能，Hermes 会自动回退 — 首先禁用 `keepAlive`，然后是代理 — 因此在免费计划上浏览仍然有效。
:::

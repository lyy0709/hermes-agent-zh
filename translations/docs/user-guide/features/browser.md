...
当配置了云提供商时，对于解析为私有/环回/LAN 地址的 URL（`localhost`、`127.0.0.1`、`192.168.x.x`、`10.x.x.x`、`172.16-31.x.x`、`*.local`、`*.lan`、`*.internal`、IPv6 环回 `::1`、链路本地 `169.254.x.x`），Hermes 会自动启动一个**本地 Chromium 侧车**。公共 URL 在同一会话中继续使用云提供商。
...
默认情况下，每个 Camofox 会话会获得一个随机身份 —— Cookie 和登录状态在 Agent 重启后不会保留。要启用持久化浏览器会话，请将以下内容添加到 `~/.hermes/config.yaml`：
...
接管仅在会话的 `tab_id` 被填充之前触发。如果外部应用程序在运行过程中关闭了被接管的标签页，下一次浏览器工具调用将出现 Camofox 错误 —— Hermes 不会在每次调用时重新轮询寻找新的标签页。
...
当 Camofox 在 headed 模式（带有可见浏览器窗口）下运行时，它会在健康检查响应中暴露一个 VNC 端口。Hermes 会自动发现这一点，并在导航响应中包含 VNC URL，因此 Agent 可以分享一个链接供您实时观看浏览器。

### 通过 CDP 连接本地 Chrome (`/browser connect`)

除了云提供商，您还可以通过 Chrome DevTools Protocol (CDP) 将 Hermes 浏览器工具附加到您自己正在运行的 Chrome 实例。这在您想实时查看 Agent 的操作、与需要您自己的 cookies/会话的页面交互，或者避免云浏览器成本时非常有用。
...
:::tip
要手动启动启用了 CDP 的 Chrome，请使用专用的用户数据目录，这样即使 Chrome 已使用您的常规配置文件运行，调试端口也能正常启动：
...
**为什么需要 `--user-data-dir`？** 如果没有它，在常规 Chrome 实例已运行时启动 Chrome 通常会在现有进程上打开一个新窗口——而该现有进程并未使用 `--remote-debugging-port` 启动，因此端口 9222 永远不会打开。专用的用户数据目录会强制启动一个新的 Chrome 进程，调试端口才能真正监听。`--no-first-run --no-default-browser-check` 会跳过新配置文件的首次启动向导。
:::
...
如果您**没有**设置任何云凭据并且不使用 `/browser connect`，Hermes 仍然可以通过由 `agent-browser` 驱动的本地 Chromium 安装来使用浏览器工具。
...
:::tip
对于简单的信息检索，建议使用 `web_search` 或 `web_extract`——它们更快、更便宜。当您需要**与**页面**交互**时（点击按钮、填写表单、处理动态内容），再使用浏览器工具。
:::
...
列出当前页面上的所有图片及其 URL 和替代文本。用于查找要分析的图片。

### `browser_vision`

截取屏幕截图并使用视觉 AI 进行分析。当文本快照无法捕捉重要的视觉信息时使用此工具——尤其适用于验证码、复杂布局或视觉验证挑战。

屏幕截图会被持久保存，文件路径会与 AI 分析结果一同返回。在消息平台（Telegram、Discord、Slack、WhatsApp）上，你可以要求 Agent 分享屏幕截图——它将通过 `MEDIA:` 机制作为原生照片附件发送。
...
当当前会话有 CDP 监管器处于活动状态时（通常针对任何在支持 CDP 的后端上运行过 `browser_navigate` 的会话），评估会通过监管器的持久 WebSocket 运行——无需启动子进程的开销。否则，会回退到标准的 agent-browser CLI 路径。两种方式的行为完全相同；只有延迟会发生变化。

### `browser_cdp`

原始 Chrome DevTools 协议透传——这是其他工具未涵盖的浏览器操作的逃生舱。用于原生对话框处理、iframe 作用域内的评估、cookie/网络控制，或 Agent 需要的任何 CDP 命令。

**仅在会话开始时 CDP 端点可达时才可用**——这意味着 `/browser connect` 已连接到正在运行的 Chrome，或者在 `config.yaml` 中设置了 `browser.cdp_url`。默认的本地 agent-browser 模式、Camofox 和云提供商（Browserbase、Browser Use、Firecrawl）目前不向此工具暴露 CDP——云提供商有每个会话的 CDP URL，但实时会话路由是后续功能。
...
**跨域 iframe：** 传递 `frame_id`（来自 `browser_snapshot.frame_tree.children[]`，其中 `is_oopif=true`）以通过监管器针对该 iframe 的实时会话路由 CDP 调用。这就是在 Browserbase 上跨域 iframe 内部执行 `Runtime.evaluate` 的方式，因为无状态 CDP 连接会遇到签名 URL 过期的问题。示例：
...
**检测通过持久的 CDP 监管器自动进行**——每个任务一个 WebSocket，订阅 Page/Runtime/Target 事件。监管器还会在快照中填充一个 `frame_tree` 字段，以便 Agent 可以查看当前页面的 iframe 结构，包括跨域（OOPIF）iframe。
...
**在 Browserbase 上的工作原理。** Browserbase 的 CDP 代理会在服务器端自动关闭真正的原生对话框（约 10 毫秒内），因此我们无法使用 `Page.handleJavaScriptDialog`。监管器通过 `Page.addScriptToEvaluateOnNewDocument` 注入一个小脚本，该脚本用同步 XHR 覆盖 `window.alert`/`confirm`/`prompt`。我们通过 `Fetch.enable` 拦截这些 XHR——页面的 JS 线程会一直阻塞在 XHR 上，直到我们调用 `Fetch.fulfillRequest` 并附上 Agent 的响应。`prompt()` 的返回值会原封不动地返回到页面 JS 中。
**对话框策略**在 `config.yaml` 的 `browser.dialog_policy` 下配置：

| 策略 | 行为 |
|--------|----------|
| `must_respond` (默认) | 捕获对话框，在快照中显示，等待显式的 `browser_dialog()` 调用。在 `browser.dialog_timeout_s` (默认 300 秒) 后安全自动关闭，以防止有缺陷的 Agent 无限期阻塞。 |
...
`browser_snapshot.frame_tree` 中的**框架树**被限制为最多 30 个框架和 OOPIF 深度为 2，以限制广告密集型页面的负载大小。当达到限制时，会设置 `truncated: true` 标志；需要完整框架树的 Agent 可以使用 `browser_cdp` 并调用 `Page.getFrameTree`。
...
- **快照大小** —— 大型页面可能会被截断或在 8000 字符处由 LLM 进行总结

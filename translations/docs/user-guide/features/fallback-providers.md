...
当您的主 LLM 提供商遇到错误时 — 速率限制、服务器过载、认证失败、连接中断 — Hermes 可以在会话中自动切换到备用提供商:模型对，而不会丢失您的对话。
...
:::info 一次性触发
备用在每个会话中**最多激活一次**。如果备用提供商也失败，则正常的错误处理会接管（重试，然后显示错误消息）。这可以防止级联故障转移循环。
:::
...
所有三者——辅助任务、压缩、备用模型——的工作方式相同：设置 `provider` 来选择处理请求的提供商，`model` 来选择模型，`base_url` 来指向自定义端点（覆盖 provider）。
...
| 定时任务 | 仅每项任务提供商覆盖（无自动回退） | 每项任务的 `provider` / `model` |
...
当您的主要 LLM 提供商遇到错误时 — 如速率限制、服务器过载、认证失败、连接中断 — Hermes 可以在会话中自动切换到备用提供商:模型组合，而不会丢失您的对话。
...
:::info 一次性
备用机制在每次会话中**最多激活一次**。如果备用提供商也失败，则正常的错误处理会接管（重试，然后显示错误消息）。这可以防止级联故障转移循环。
:::
...
如果解析出的提供商在调用时失败，Hermes 还有一个内部重试机制：如果该提供商不是 OpenRouter 且没有设置显式的 `base_url`，它会将 OpenRouter 作为最后的备用方案。
...
所有这三者——辅助任务、压缩、备用模型——的工作方式相同：设置 `provider` 来选择处理请求的提供商，设置 `model` 来选择模型，设置 `base_url` 来指向自定义端点（会覆盖 provider）。
...
由 `delegate_task` 生成的子 Agent **不**使用主备用模型。但是，为了优化成本，可以将它们路由到不同的提供商:模型组合：

...
凭证池处理同一提供商的轮换（例如，多个 OpenRouter 密钥）。本页涵盖跨提供商的备用机制。两者都是可选的，并且独立工作。
...
| 阿里云 / 灵积 | `alibaba` | `DASHSCOPE_API_KEY` |
...
:::info 单次触发
备用机制在**每个会话中最多激活一次**。如果备用提供商也失败，则正常的错误处理机制会接管（重试，然后显示错误消息）。这可以防止级联故障转移循环。
:::
...
| 提供商 | 描述 | 要求 |
|----------|-------------|-------------|
| `"auto"` | 按顺序尝试提供商直到一个能工作（默认） | 至少配置了一个提供商 |

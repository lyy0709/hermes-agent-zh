...
| 阿里云 Coding Plan | `alibaba-coding-plan` | `ALIBABA_CODING_PLAN_API_KEY`（回退到 `DASHSCOPE_API_KEY`） |
...
:::tip
`fallback_model` 没有对应的环境变量——它完全通过 `config.yaml` 配置。这是有意为之：回退配置是一个深思熟虑的选择，不应被过时的 shell 导出变量覆盖。
:::
...
如果你想要一个不同于“主 Agent 模型优先”的回退顺序，请明确配置 `fallback_chain`。每个条目至少需要 `provider`；`model`、`base_url` 和 `api_key` 是可选的。

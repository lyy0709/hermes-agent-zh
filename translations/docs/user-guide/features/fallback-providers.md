...
当您的主要 LLM 提供商遇到错误时 — 如速率限制、服务器过载、认证失败、连接中断 — Hermes 可以在会话中自动切换到备用提供商:模型对，而不会丢失您的对话。
...
:::tip
`fallback_model` 没有对应的环境变量 — 它专门通过 `config.yaml` 配置。这是有意为之：备用配置是一个深思熟虑的选择，不应被过时的 shell 导出变量覆盖。
:::
...
### 自动检测链
当任务的提供商设置为 `"auto"`（默认值）时，Hermes 会按顺序尝试各个提供商，直到有一个能正常工作：
...
如果解析出的提供商在调用时失败，Hermes 还有一个内部重试机制：如果提供商不是 OpenRouter 并且没有设置显式的 `base_url`，它会将 OpenRouter 作为最后的备用方案。
...
所有这三者——辅助任务、压缩、备用模型——的工作方式相同：设置 `provider` 来选择处理请求的提供商，设置 `model` 来选择模型，设置 `base_url` 来指向自定义端点（覆盖 provider）。
...
| 提供商 | 描述 | 要求 |
|----------|-------------|-------------|
| `"auto"` | 按顺序尝试提供商直到有一个能工作（默认） | 至少配置了一个提供商 |
...
这等同于配置 `auxiliary.compression.provider` 和 `auxiliary.compression.model`。如果两者都设置了，`auxiliary.compression` 的值优先。

如果没有可用的压缩提供商，Hermes 会丢弃对话中间轮次而不生成摘要，而不是让会话失败。

---

## 委派任务提供商覆盖

由 `delegate_task` 生成的子 Agent **不**使用主备用模型。但是，为了优化成本，可以将它们路由到不同的提供商:模型对：

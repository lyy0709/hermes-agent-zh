...
仪表盘插件允许您向 Web 仪表盘添加自定义标签页。插件可以显示自己的 UI、调用 Hermes API，并可选择注册后端端点——所有这些都无需触及仪表盘源代码。
...
单个插件可以从一个目录同时扩展 CLI/消息网关（通过 `plugin.yaml` + `__init__.py`）和仪表盘（通过 `dashboard/`）。

## 清单参考

`manifest.json` 文件向仪表盘描述您的插件：
...
插件不捆绑 React 或 UI 组件——它们使用在 `window.__HERMES_PLUGIN_SDK__` 上公开的 SDK。这避免了版本冲突，并保持插件包体积小巧。
...
SDK.fetchJSON          // 用于自定义端点的原始 fetch — 自动处理身份验证
...
`fetchJSON` 会自动注入会话身份验证令牌、处理错误并解析 JSON。
...
插件 API 路由绕过会话令牌身份验证，因为仪表盘服务器仅绑定到 localhost。
...
您可以使用仪表盘的 CSS 自定义属性（例如 `--color-border`、`--color-foreground`）来匹配活动主题。

## 插件加载流程

1.  仪表盘加载 — `main.tsx` 在 `window.__HERMES_PLUGIN_SDK__` 上公开 SDK
2.  `App.tsx` 调用 `usePlugins()`，后者获取 `GET /api/dashboard/plugins`
3.  对于每个插件：注入 CSS `<link>`（如果已声明），加载 JS `<script>`
4.  插件 JS 调用 `window.__HERMES_PLUGINS__.register(name, Component)`
5.  仪表盘将标签页添加到导航栏，并将组件作为路由挂载

插件在其脚本加载后有最多 2 秒的时间进行注册。如果插件加载失败，仪表盘将继续运行而不加载它。
...
用户插件优先——如果同一插件名称存在于多个来源中，则用户版本获胜。

要在添加新插件后强制重新扫描而无需重启服务器：
...
要尝试它，请运行 `hermes dashboard` —— “示例”标签页将出现在“技能”之后。

## 提示

-   **无需构建步骤** — 编写纯 JavaScript IIFE。如果您更喜欢 JSX，请使用任何构建工具（esbuild、Vite、webpack），以 React 作为外部依赖，输出目标为 IIFE。
-   **保持包体积小巧** — React 和所有 UI 组件都由 SDK 提供。您的包应仅包含您的插件逻辑。
-   **使用主题变量** — 在 CSS 中引用 `var(--color-*)` 以自动匹配用户选择的任何主题。
-   **本地测试** — 运行 `hermes dashboard --no-open` 并使用浏览器开发者工具验证您的插件是否正确加载和注册。
...
> **需要引导教程？** 阅读 [看板教程](./kanban-tutorial) — 包含四个用户场景（独立开发者、舰队式运营、带重试的角色流水线、熔断器）以及每个场景的仪表盘截图。本页是参考文档；教程是叙述性指南。

Hermes 看板是一个持久化的任务板，在你所有的 Hermes 人格之间共享，允许多个具名 Agent 协作处理工作，而无需依赖脆弱的进程内子 Agent 集群。每个任务都是 `~/.hermes/kanban.db` 中的一行；每次交接都是任何人都可以读写的一行；每个工作者都是一个拥有自己身份的完整操作系统进程。

### 两个界面：模型通过工具交流，你通过 CLI 交流

看板有两个入口，都基于同一个 `~/.hermes/kanban.db`：

- **Agent 通过专用的 `kanban_*` 工具集驱动看板** — `kanban_show`、`kanban_list`、`kanban_complete`、`kanban_block`、`kanban_heartbeat`、`kanban_comment`、`kanban_create`、`kanban_link`、`kanban_unblock`。调度器在生成每个工作者时，其模式中已包含这些工具；编排者人格也可以显式启用 `kanban` 工具集。模型通过直接调用工具来读取和路由任务，*而不是*通过执行 `hermes kanban` 命令。请参阅下面的 [工作者如何与看板交互](#how-workers-interact-with-the-board)。
- **你（以及脚本和定时任务）通过 CLI 上的 `hermes kanban …`、斜杠命令 `/kanban …` 或仪表盘来驱动看板**。这些是供人类和自动化流程使用的界面 — 即那些背后没有工具调用模型的场景。

两个界面都通过同一个 `kanban_db` 层进行路由，因此读取操作看到的是统一的视图，写入操作不会产生偏差。本页其余部分展示 CLI 示例，因为它们易于复制粘贴，但每个 CLI 动词都有模型使用的等效工具调用。

这种形式覆盖了 `delegate_task` 无法处理的工作负载：

- **研究分类** — 并行研究员 + 分析师 + 撰稿人，人类参与循环。
- **计划运维** — 每日重复简报，数周内构建日志。
...
- **链接** — 记录父 → 子依赖关系的 `task_links` 行。当所有父任务都处于 `done` 状态时，调度器会将 `todo` 提升为 `ready`。
- **评论** — Agent 间的通信协议。Agent 和人类可以追加评论；当工作者（重新）生成时，它会读取完整的评论线程作为其上下文的一部分。
- **工作空间** — 工作者在其中操作的目录。有三种类型：
  - `scratch`（默认）— `~/.hermes/kanban/workspaces/<id>/` 下的新临时目录（或在非默认看板下为 `~/.hermes/kanban/boards/<slug>/workspaces/<id>/`）。
  - `dir:<path>` — 一个现有的共享目录（Obsidian 库、邮件操作目录、每个账户的文件夹）。**必须是绝对路径。** 像 `dir:../tenants/foo/` 这样的相对路径在调度时会被拒绝，因为它们会根据调度器当前所在的任何 CWD 进行解析，这是不明确的，并且是一个混淆代理的逃逸向量。该路径在其他方面是受信任的 — 这是你的机器，你的文件系统，工作者以你的用户 ID 运行。这是受信任的本地用户威胁模型；看板在设计上是单主机的。
  - `worktree` — 用于编码任务的 `.worktrees/<id>/` 下的 git 工作树。使用 `worktree:<path>` 来固定确切的目标路径。工作者端的 `git worktree add` 会创建它，并在提供时使用 `--branch`。
- **调度器** — 一个长期运行的循环，每 N 秒（默认 60 秒）执行一次：回收过期的认领，回收崩溃的工作者（PID 消失但 TTL 尚未过期），提升就绪任务，原子性地认领，生成分配的人格。默认情况下在 **消息网关内部** 运行（`kanban.dispatch_in_gateway: true`）。一个调度器每次扫描所有看板；工作者生成时带有固定的 `HERMES_KANBAN_BOARD`，因此它们无法看到其他看板。在同一任务上连续 `kanban.failure_limit` 次（默认：2）生成失败后，调度器会自动将其阻塞，并将最后一个错误作为原因 — 防止在那些人格不存在、工作空间无法挂载等任务上产生抖动。
...
所有仪表板 API 端点都接受 `?board=<slug>` 参数用于看板范围限定。事件 WebSocket 在连接时固定到一个看板；在 UI 中切换会针对新看板打开一个新的 WS。
...
**Worker 不会通过 shell 调用 `hermes kanban`。** 当调度器生成一个 worker 时，它会在子进程的环境中设置 `HERMES_KANBAN_TASK=t_abcd`，这个环境变量会在模型的工具集模式中启用一个专门的**看板工具集**。同样的工具集也对那些在其工具集配置中启用了 `kanban` 的编排器配置文件可用。这些工具通过 Python 的 `kanban_db` 层直接读取和修改看板，与 CLI 的方式相同。一个运行中的 worker 像调用其他工具一样调用这些工具；它从不接触也不需要 `hermes kanban` CLI。
...
kanban_create(
    ...
    assignee="writer",
    parents=["t_r1", "t_r2"],                     # 当两个任务都完成时提升为就绪状态
    body="one-pager, 300 words, neutral tone",
)
kanban_complete(summary="decomposed into 2 research tasks + 1 writer; linked dependencies")
```

标记为"（编排器）"的工具 — `kanban_list`、`kanban_create`、`kanban_link`、`kanban_unblock` 以及对其他任务的 `kanban_comment` — 通过同一个工具集提供；约定（由 `kanban-orchestrator` 技能强制执行）是：worker 配置文件不进行任务分解或路由无关工作，而编排器配置文件不执行具体实现工作。由调度器生成的 worker 在破坏性生命周期操作上仍然受任务范围限制，无法修改无关任务。

### 为什么使用工具而不是通过 shell 调用 `hermes kanban`

三个原因：

1.  **后端可移植性。** 那些终端工具指向远程后端（Docker / Modal / Singularity / SSH）的 Worker 会在*容器内部*运行 `hermes kanban complete`，而容器内没有安装 `hermes`，也没有挂载 `~/.hermes/kanban.db`。看板工具在 Agent 自身的 Python 进程中运行，无论终端后端是什么，总是能访问 `~/.hermes/kanban.db`。
...
**对普通会话的零模式占用。** 一个常规的 `hermes chat` 会话在其模式中没有任何 `kanban_*` 工具，除非活动配置文件明确为编排器工作启用了 `kanban` 工具集。由调度器生成的任务 worker 获得任务范围内的工具，因为设置了 `HERMES_KANBAN_TASK`；编排器配置文件通过配置获得更广泛的路由接口。对于从不接触看板的用户，没有工具膨胀问题。
...
`summary` 是人类可读的结项说明，而 `metadata` 是机器可读的交接信息，下游 Agent、审查者或仪表板可以复用，而无需解析文本。
...
请勿将秘密、原始日志、Token、OAuth 材料以及无关的对话记录放入 `metadata`。请存储指针和摘要。如果一个任务没有文件或测试，请在 `summary` 中明确说明，并使用 `metadata` 记录确实存在的证据，例如源 URL、问题 ID 或人工审查步骤。

### Worker 技能

任何应该能够处理看板任务的配置文件都必须加载 `kanban-worker` 技能。它教会 worker 完整的生命周期，使用的是**工具调用**，而不是 CLI 命令：
1. 启动时，调用 `kanban_show()` 读取标题、正文、父级交接、先前尝试和完整的评论线程。
2. 通过终端工具 `cd $HERMES_KANBAN_WORKSPACE` 并在该目录下工作。
3. 在长时间操作期间，每隔几分钟调用一次 `kanban_heartbeat(note="...")`。**如果你的工作可能运行超过 1 小时，请至少每小时调用一次 `kanban_heartbeat`** —— 调度器会回收那些运行时间超过 `kanban.dispatch_stale_timeout_seconds`（默认 4 小时）且在过去一小时内没有心跳的任务，假设工作进程崩溃且未清理。回收是良性的（任务会回到 `ready` 状态等待重新调度，不会增加失败计数），但你会丢失当前运行的进度。
4. 使用 `kanban_complete(summary="...", metadata={...})` 完成任务，如果遇到阻碍则使用 `kanban_block(reason="...")`。
...
调度器在生成每个工作进程时也会自动传递 `--skills kanban-worker`，因此即使配置文件的默认技能配置不包含它，工作进程也始终可以访问模式库。

### 为特定任务固定额外技能

有时单个任务需要分配者配置文件默认不包含的专家上下文 —— 需要 `translation` 技能的翻译工作、需要 `github-code-review` 的审查任务、需要 `security-pr-audit` 的安全审计。与其每次都编辑分配者的配置文件，不如直接将技能附加到任务上。
...
这些技能是内置 `kanban-worker` 的**附加项** —— 调度器为每个技能（以及内置技能）发出一个 `--skills <name>` 标志，因此工作进程启动时会加载所有技能。技能名称必须与分配者配置文件上实际安装的技能匹配（运行 `hermes skills list` 查看可用的技能）；没有运行时安装。

### 编排器技能

一个**行为良好的编排器不会自己完成工作。** 它将用户的目标分解为任务，链接它们，将每个任务分配给你设置的配置文件之一，然后退后一步。`kanban-orchestrator` 技能将此编码为工具调用模式：反诱惑规则、Step-0 配置文件发现提示词（调度器对未知的分配者名称会静默失败，因此编排器必须将每张卡片基于你机器上实际存在的配置文件），以及一个基于 `kanban_create` / `kanban_link` / `kanban_comment` 的分解手册。
...
`/kanban` CLI 和斜杠命令足以无头运行看板，但可视化看板通常是人工介入的合适界面：分类、跨配置文件监督、阅读评论线程以及在列之间拖拽卡片。Hermes 将此作为**捆绑的仪表板插件**在 `plugins/kanban/` 中提供 —— 不是核心功能，也不是独立服务 —— 遵循[扩展仪表板](./extending-the-dashboard)中概述的模型。
使用以下命令打开：
...
- **Kanban 标签页**：按状态显示列：`triage`、`todo`、`ready`、`running`、`blocked`、`done`（切换开关打开时还有 `archived`）。
  - `triage` 是存放粗略想法的暂存列。默认情况下（`kanban.auto_decompose: true`），调度器会自动对进入此列的任务运行**分解器**——编排器配置文件会读取粗略想法，查看你的配置文件列表（包含描述），并将任务分解成一个小的子任务图，路由到最合适的专家。原始任务作为每个子任务的父任务保持活动状态，以便在所有子任务完成时，编排器会重新唤醒以判断完成情况。点击页面顶部的 **Orchestration: Auto/Manual** 药丸（或设置 `kanban.auto_decompose: false`）切换到手动模式，在此模式下，triage 任务会保持不动，直到你点击卡片上的 **⚗ Decompose** 或运行 `hermes kanban decompose <id>`。对于不需要分解的任务（或没有编排器配置文件的设置），**✨ Specify** 按钮会通过相同的 LLM 机制进行单任务规范重写（标题 + 包含目标、方法、验收标准的正文）。请参阅下面的[自动与手动编排](#auto-vs-manual-orchestration)。
- 卡片显示任务 ID、标题、优先级徽章、租户标签、分配的配置文件、评论/链接数量、**进度药丸**（当任务有依赖项时显示 `N/M` 个子任务完成）以及“N 前创建”。每个卡片都有一个复选框，支持多选。
- **Running 列内按配置文件分组**——工具栏复选框可切换 Running 列按分配者进行子分组。
- **通过 WebSocket 实时更新**——插件以短轮询间隔跟踪仅追加的 `task_events` 表；当任何配置文件（CLI、消息网关或另一个仪表板标签页）执行操作时，看板会立即反映变化。重新加载进行了防抖处理，因此一系列事件只会触发一次重新获取。
...
- **内联创建**——点击任何列标题上的 `+` 可以输入标题、分配者、优先级，以及（可选）从现有任务的下拉列表中选择父任务。按 Enter 创建任务，按 Shift+Enter 在标题字段中插入换行符，或按 Escape 取消。从 Triage 列创建会自动将新任务放入 triage。
- **多选与批量操作**——按住 Shift/Ctrl 点击卡片或勾选其复选框以将其添加到选择中。顶部会出现一个批量操作栏，包含批量状态转换、归档和重新分配（通过配置文件下拉列表或“(取消分配)”）。破坏性批量操作会先进行确认。每个 ID 的部分失败会报告，但不会中止其余操作。
...
**自动（默认）**——`kanban.auto_decompose: true`。消息网关内嵌的调度器在每个滴答运行**分解器**，受 `kanban.auto_decompose_per_tick` 限制（默认每个滴答 3 个任务），因此批量加载的 triage 任务不会突发消耗辅助 LLM。分解器读取粗略想法，查看你已安装的配置文件及其描述，并要求 LLM 生成一个 JSON 任务图：生成哪些任务、分配给谁以及谁依赖谁。原始的 triage 任务成为图中每个叶节点的父任务，因此它会保持活动状态，直到整个图完成——然后提升回 `ready`，以便其分配者（编排器配置文件）可以判断完成情况，并在工作未完成时添加更多任务。这就是“丢下一句话，然后走开”的流程。
**手动模式** — `kanban.auto_decompose: false`。待处理任务将停留在待处理列，直到您手动操作。点击卡片上的 **⚗ 分解** 按钮，运行 `hermes kanban decompose <id>`（或 `--all`），或在聊天中使用 `/kanban decompose <id>`。这匹配了看板在引入分解器之前的行为，适用于您希望完全控制任务执行时机的情况。

您可以通过看板页面顶部的 **编排：自动/手动** 药丸（翠绿色 = 自动，灰暗色 = 手动）在两种模式间切换，或直接编辑 `config.yaml`。这两种模式都与 `hermes kanban specify` 共存 —— 当您不希望进行任务拆分时，它仍然可以作为单任务规格重写工具使用。

分解器的路由决策依赖于个人资料描述，这是您通过 `hermes profile create --description "..."`、`hermes profile describe <name> --text "..."`、`hermes profile describe <name> --auto`（根据个人资料已安装的技能和模型由 LLM 生成）或在仪表板展开的 **编排设置** 面板中为每个个人资料设置的标签化原语。没有描述的个人资料仍会出现在列表中 —— 它们可以通过名称路由，只是精确度较低。分解器**绝不会**将子任务分配给 `assignee=None`：当 LLM 选择了一个未知的个人资料时，子任务会被路由到 `kanban.default_assignee`（如果未设置，则回退到活动的默认个人资料）。
...
| `auto_decompose_per_tick` | `3` | 每次调度器运行的最大分解次数。超出部分推迟到下一次运行。 |
| `orchestrator_profile` | `""` | 负责分解的个人资料。为空 = 回退到活动的默认个人资料。 |
| `default_assignee` | `""` | 当 LLM 选择未知个人资料时，子任务的归属地。为空 = 回退到活动的默认个人资料。 |
...
| 方法 | 路径 | 用途 |
|---|---|---|
| `GET` | `/board?tenant=<name>&include_archived=…` | 按状态列分组的完整看板，以及用于过滤器下拉列表的租户 + 被分配者 |
...
| `POST` | `/tasks/bulk` | 对 `ids` 中的每个 ID 应用相同的补丁（状态 / 归档 / 被分配者 / 优先级）。每个 ID 的失败会报告，但不中断其他任务 |
| `POST` | `/tasks/:id/comments` | 追加评论 |
| `POST` | `/tasks/:id/specify` | 运行待处理规格化器 —— 辅助 LLM 充实任务正文并将其从 `triage` 提升到 `todo`。返回 `{ok, task_id, reason, new_title}`；在“不在待处理状态”/无辅助客户端/LLM 错误时，`ok=false` 并附带人类可读的原因，返回 200 状态码，而非 4xx |
...
| `POST` | `/profiles/:name/describe-auto` | 通过 `auxiliary.profile_describer` 为个人资料生成描述。持久化时设置 `description_auto: true`，以便仪表板可以显示“待审核”徽章。 |
| `GET` | `/orchestration` | 读取看板编排设置（`orchestrator_profile`、`default_assignee`、`auto_decompose`）以及回退后的*已解析*有效值。 |
| `PUT` | `/orchestration` | 更新 `config.yaml` 中三个编排键的一个或多个。验证非空的个人资料名称确实存在。 |
...
每个处理器都是一个轻量包装器——该插件约 700 行 Python 代码（包含路由、WebSocket 尾部、批量批处理器和配置读取器），不添加任何新的业务逻辑。一个微小的 `_conn()` 辅助函数会在每次读写时自动初始化 `kanban.db`，因此全新安装无论用户是先打开仪表盘、直接调用 REST API，还是运行 `hermes kanban init` 都能正常工作。

### 仪表盘配置

在 `~/.hermes/config.yaml` 的 `dashboard.kanban` 下，任何这些键都会更改标签页的默认值——插件在加载时通过 `GET /config` 读取它们：
...
`~/.hermes/kanban.db` 中的任务特意设计为与配置文件无关（这是协调原语）。如果你使用 `hermes -p <profile> dashboard` 打开仪表盘，看板仍然会显示主机上任何其他配置文件创建的任务。所有配置文件属于同一用户，但如果存在多个角色，这一点值得注意。

### 实时更新

`task_events` 是一个仅追加的 SQLite 表，具有单调递增的 `id`。WebSocket 端点保存每个客户端最后看到的事件 id，并在新行到达时推送它们。当一批事件到达时，前端会重新加载（非常廉价的）看板端点——这比尝试根据每种事件类型修补本地状态更简单、更正确。WAL 模式意味着读取循环永远不会阻塞调度器的 `BEGIN IMMEDIATE` 声明事务。

### 扩展它

该插件使用标准的 Hermes 仪表盘插件契约——完整的清单参考、Shell 插槽、页面作用域插槽和插件 SDK，请参阅[扩展仪表盘](./extending-the-dashboard)。额外的列、自定义卡片样式、租户过滤布局或完整的 `tab.override` 替换都可以在不分叉此插件的情况下实现。

要禁用而不删除：在 `config.yaml` 中添加 `dashboard.plugins.kanban.enabled: false`（或删除 `plugins/kanban/dashboard/manifest.json`）。

### 范围边界

GUI 特意设计得很薄。插件所做的一切都可以通过 CLI 访问；插件只是让人类操作更舒适。自动分配、预算、治理门控和组织结构图视图仍属于用户空间——一个路由器配置文件、另一个插件或重用 `tools/approval.py`——正如设计规范中超出范围部分所列。

## CLI 命令参考

这是**你**（或脚本、定时任务、仪表盘）用来驱动看板的接口。在调度器内部运行的工作器使用 `kanban_*` [工具接口](#how-workers-interact-with-the-board) 进行相同的操作——这里的 CLI 和那里的工具都通过 `kanban_db` 路由，因此这两个接口在构造上是一致的。
...
`--max-retries` 是调度器针对每个任务的熔断器覆盖设置。`--max-retries 1` 会在第一次非成功尝试时阻塞任务，而 `--max-retries 3` 允许两次重试并在第三次失败时阻塞。省略此参数则使用 `config.yaml` 中的 `kanban.failure_limit`，然后是内置默认值。
...
- 您想了解您的舰队在做什么，但又不想停止编排器 → `/kanban list --mine` 或 `/kanban stats` 会检查看板，而不影响您的主要对话。

### `/kanban create` 上的自动订阅（仅限网关）

当您通过网关使用 `/kanban create "…"` 创建任务时，发起聊天的会话（平台 + 聊天 ID + 线程 ID）会自动订阅该任务的终端事件（`completed`、`blocked`、`gave_up`、`crashed`、`timed_out`）。每个终端事件您都会收到一条消息回复——包括 `completed` 时工作线程结果摘要的第一行——无需轮询或记住任务 ID。
...
一旦任务达到 `done` 或 `archived` 状态，订阅会自动移除。如果您使用 `--json`（机器输出）脚本化创建任务，则会跳过自动订阅——假设脚本化调用方希望通过 `/kanban notify-subscribe` 显式管理订阅。

### 消息传递中的输出截断

网关平台有实际的消息长度限制。如果 `/kanban list`、`/kanban show` 或 `/kanban tail` 产生的输出超过约 3800 个字符，响应将被截断，并带有 `… (truncated; use \`hermes kanban …\` in your terminal for full output)` 的页脚。CLI 界面没有此类限制。
...
| **P9 分类规范器** | 粗略想法 → `triage` → `hermes kanban specify` 扩展正文 → `todo` | "将这一句话转化为规范化的任务" |
...
```
Worker 会接收 `$HERMES_TENANT` 并通过前缀对其记忆写入进行命名空间隔离。看板、调度器和配置文件定义都是共享的；只有数据是作用域内的。

## 消息网关通知

当您从消息网关（Telegram、Discord、Slack 等）运行 `/kanban create …` 时，发起聊天的会话会自动订阅新任务。消息网关的后台通知器每隔几秒轮询一次 `task_events`，并将每个终端事件（`completed`、`blocked`、`gave_up`、`crashed`、`timed_out`）的一条消息发送到该聊天会话。已完成的任务还会发送 worker 的 `--result` 的第一行，这样您无需执行 `/kanban show` 就能看到结果。

您可以从 CLI 显式管理订阅——这在脚本/定时任务希望通知一个非其发起的聊天会话时非常有用：
...
任务是工作的逻辑单元；**运行** 是对其执行的一次尝试。当调度器认领一个就绪任务时，它会在 `task_runs` 中创建一行，并将 `tasks.current_run_id` 指向它。当该次尝试结束时——无论是完成、阻塞、崩溃、超时、启动失败还是被回收——运行行会以 `outcome` 关闭，并且任务的指针会被清空。一个被尝试了三次的任务会有三行 `task_runs` 记录。

为什么需要两个表而不是直接修改任务：您需要 **完整的尝试历史** 来进行实际的事后分析（"第二次审查尝试得以批准，第三次合并了"），并且需要一个干净的地方来挂载每次尝试的元数据——哪些文件被更改、哪些测试运行了、审查者记录了哪些发现。这些是运行事实，而不是任务事实。

运行也是 **结构化交接** 的存放位置。当 worker 完成任务时（通过 `kanban_complete(...)`），它可以传递：
...
下游子任务会读取每个父任务最近一次已完成的运行的摘要和元数据。重试的 worker 会读取其自身任务的先前尝试（结果、摘要、错误），这样它们就不会重复已经失败的路径。
...
当您（人类）需要关闭一个 worker 无法处理的任务时（例如，一个被放弃的任务，或者您从仪表板手动标记为完成的任务），可以通过 CLI 实现相同的交接：
...
运行记录会在仪表板（抽屉中的"运行历史"部分，每次尝试对应一个彩色行）和 REST API（`GET /api/plugins/kanban/tasks/:id` 返回一个 `runs[]` 数组）中公开。使用 `PATCH /api/plugins/kanban/tasks/:id` 并附带 `{status: "done", summary, metadata}` 会将两者转发给内核，因此仪表板的"标记完成"按钮与 CLI 等效。`task_events` 行携带它们所属的 `run_id`，以便 UI 可以按尝试对它们进行分组，并且 `completed` 事件在其有效负载中嵌入了第一行摘要（上限为 400 个字符），这样消息网关通知器无需进行第二次 SQL 往返即可渲染结构化交接。

**批量关闭注意事项。** `hermes kanban complete a b c --summary X` 会被拒绝——结构化交接是针对每次运行的，因此将相同的摘要复制粘贴到 N 个任务几乎总是错误的。**不带** `--summary` / `--metadata` 的批量关闭对于常见的"我完成了一堆管理任务"的情况仍然有效。

**状态变更导致的回收运行。** 如果您在仪表板中将一个正在运行的任务从 `running` 拖走（回到 `ready`，或直接到 `todo`），或者归档一个仍在运行的任务，正在进行的运行会以 `outcome='reclaimed'` 关闭，而不是被孤立。当 `tasks.current_run_id` 为 `NULL` 时，`task_runs` 行总是处于终端状态，反之亦然——这个不变量在 CLI、仪表板、调度器和通知器中都成立。
...
**抽屉实时刷新。** 当仪表板的 WebSocket 事件流报告用户当前正在查看的任务有新事件时，抽屉会重新加载自身（通过一个按任务的事件计数器，该计数器被编织到其 `useEffect` 依赖项列表中）。不再需要关闭并重新打开抽屉来查看运行的新行或更新的结果。
### 向前兼容性

`tasks` 表上的两个可为空的列是为 v2 工作流路由预留的：`workflow_template_id`（此任务属于哪个模板）和 `current_step_key`（该模板中哪个步骤处于活动状态）。v1 内核在路由时会忽略它们，但允许客户端写入，因此 v2 版本可以添加路由机制而无需再次进行模式迁移。

## 事件参考

每次状态转换都会向 `task_events` 表追加一行。每行都带有一个可选的 `run_id`，以便 UI 可以按尝试对事件进行分组。事件类型分为三个集群，便于过滤（`hermes kanban watch --kinds completed,gave_up,timed_out`）：
...
| `completed` | `{result_len, summary?}` | Worker 写入了 `--result` / `--summary` 且任务达到 `done` 状态。`summary` 是第一行交接内容（上限 400 字符）；完整版本保存在运行记录行中。如果在从未被认领的任务上调用 `complete_task` 并带有交接字段，则会合成一个零时长的运行记录，以便 `run_id` 仍指向某个内容。 |
| `blocked` | `{reason}` | Worker 或人工将任务翻转为 `blocked` 状态。在从未被认领的任务上调用并带有 `--reason` 时，会合成一个零时长的运行记录。 |
| `unblocked` | — | `blocked → ready`，手动或通过 `/unblock` 命令。`run_id` 为 `NULL`。 |
| `archived` | — | 从默认看板中隐藏。如果任务仍在运行，则携带作为副作用被回收的运行记录的 `run_id`。 |
...
| `heartbeat` | `{note?}` | Worker 在长时间操作期间调用 `hermes kanban heartbeat $TASK` 以发出存活信号。 |
...
| `stale` | `{elapsed_seconds, last_heartbeat_at, heartbeat_age_seconds, timeout_seconds, pid, terminated}` | 任务运行时间超过 `kanban.dispatch_stale_timeout_seconds`（默认 4 小时）**且**在过去一小时内没有收到 `kanban_heartbeat`。调度器向本地主机上的 worker（如果有）发送 SIGTERM，将任务重置为 `ready` 状态以重新调度。**不**增加失败计数器（stale 是调度器侧的缺席检测，不是 worker 的故障）。运行长时间操作的 worker 应至少每小时调用一次 `kanban_heartbeat` 以避免此情况。 |
| `respawn_guarded` | `{reason}` | 调度器拒绝在此刻重新生成此就绪任务。原因：`blocker_auth`（上次失败是配额/认证/429 错误 — 等待速率窗口重置）、`recent_success`（过去一小时内有一次成功的运行 — 等待审查后再重新运行）、`active_pr`（最近的评论中出现了 GitHub PR URL — 之前的 worker 已经打开了 PR）。任务保持 `ready` 状态；下一次调度有机会再次生成。如果底层条件持续存在，正常的 `consecutive_failures` 断路器将在 `failure_limit` 次失败后通过 `gave_up` 自动阻塞。 |
| `spawn_failed` | `{error, failures}` | 一次生成尝试失败（缺少 PATH、工作空间无法挂载等）。计数器递增；任务返回 `ready` 状态以重试。 |
| `protocol_violation` | `{pid, claimer, exit_code}` | Worker 在任务仍处于 `running` 状态时成功退出，通常是因为它在未调用 `kanban_complete` 或 `kanban_block` 的情况下进行了应答。调度器还会发出 `gave_up` 并立即自动阻塞，而不是重试。 |
| `gave_up` | `{failures, effective_limit, limit_source, error}` | 断路器在 N 次连续非成功尝试后触发。任务自动阻塞并附带最后一个错误。有效限制按以下顺序解析：任务 `max_retries`，然后是调度器 `failure_limit` / `kanban.failure_limit`，最后是内置默认值。 |

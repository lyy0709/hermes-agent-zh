...
**在以下情况使用看板：** 工作跨越 Agent 边界，需要能在重启后存活，可能需要人工输入，可能由不同的角色接手，或者需要在事后可被发现。
...
- **工作空间** — 工作者在其中操作的目录。有三种类型：
  ...
  - `dir:<path>` — 一个现有的共享目录（Obsidian 保险库、邮件操作目录、每个账户的文件夹）。**必须是绝对路径。** 像 `dir:../tenants/foo/` 这样的相对路径在调度时会被拒绝，因为它们会相对于调度器碰巧所在的当前工作目录进行解析，这是不明确的，并且是一个混淆代理的逃逸向量。该路径在其他方面是受信任的 — 这是你的机器，你的文件系统，工作者以你的用户 ID 运行。这是受信任的本地用户威胁模型；看板在设计上是单主机的。
  - `worktree` — 用于编码任务的 `.worktrees/<id>/` 下的 git 工作树。工作者端的 `git worktree add` 创建它。
- **调度器** — 一个长期运行的循环，每 N 秒（默认 60 秒）：回收过期的认领，回收崩溃的工作者（PID 消失但 TTL 尚未过期），提升就绪的任务，原子性地认领，生成分配的人格。默认在**消息网关内部**运行（`kanban.dispatch_in_gateway: true`）。一个调度器每次扫描所有看板；工作者生成时带有固定的 `HERMES_KANBAN_BOARD`，因此它们无法看到其他看板。在同一任务上连续 `kanban.failure_limit` 次生成失败后（默认：2），调度器会自动将其阻塞，并将最后一个错误作为原因 — 防止在人格不存在、工作空间无法挂载等任务上产生抖动。
...
Slug 会经过验证：小写字母数字 + 连字符 + 下划线，1-64 个字符，必须以字母数字开头。大写输入会自动转换为小写。任何其他字符（斜杠、空格、点、`..`）在 CLI 层会被拒绝，因此无法通过路径遍历技巧来命名看板。

### 通过仪表板管理看板

`hermes dashboard` → Kanban 标签页在存在多个看板（或任何看板有任务）时，顶部会显示一个看板切换器。单看板用户只会看到一个小的 `+ New board` 按钮；在需要之前，切换器是隐藏的。
...
所有仪表板 API 端点都接受 `?board=<slug>` 参数来限定看板范围。事件 WebSocket 在连接时固定到一个看板；在 UI 中切换会针对新看板打开一个新的 WebSocket。

## 快速开始

以下命令是**你**（人类）设置看板和创建任务。一旦任务被分配，调度器会启动被分配配置文件的 Agent 作为工作进程，之后**模型通过 `kanban_*` 工具调用来驱动任务，而不是 CLI 命令**——参见[工作进程如何与看板交互](#how-workers-interact-with-the-board)。
...
当调度器获取到 `t_abcd` 并启动 `researcher` 配置文件时，该工作进程的模型要做的第一件事就是调用 `kanban_show()` 来读取其任务。它不会运行 `hermes kanban show t_abcd`。

### 消息网关嵌入式调度器（默认）

调度器运行在消息网关进程内部。无需安装，无需管理单独的服务——只要消息网关启动，就绪的任务会在下一个轮询周期（默认为 60 秒）被获取。
...
可以通过运行时环境变量 `HERMES_KANBAN_DISPATCH_IN_GATEWAY=0` 覆盖配置标志以进行调试。标准的消息网关监控适用：直接运行 `hermes gateway start`，或将消息网关配置为 systemd 用户单元（参见消息网关文档）。如果没有运行的消息网关，`ready` 任务会保持原状，直到有消息网关启动——`hermes kanban create` 在创建时会对此发出警告。

将 `hermes kanban daemon` 作为独立进程运行是**已弃用**的；请使用消息网关。如果你确实无法运行消息网关（无头主机策略禁止长时间运行的服务等），`--force` 逃生舱口会在一个发布周期内保持旧的独立守护进程存活，但同时对同一个 `kanban.db` 运行消息网关嵌入式调度器**和**独立守护进程会导致声明竞争，且不受支持。
...
| `kanban_complete` | 完成任务，附带结构化的 `summary` + `metadata` 交接信息。 | 至少 `summary` / `result` 中的一个 |
...
标有"（编排器）"的工具 — `kanban_list`、`kanban_create`、`kanban_link`、`kanban_unblock` 以及对其他任务的 `kanban_comment` — 通过同一个工具集提供；约定（由 `kanban-orchestrator` 技能强制执行）是：worker 配置文件不进行任务分解或路由无关工作，而编排器配置文件不执行具体实现工作。由调度器生成的 worker 在破坏性的生命周期操作上仍然受限于其任务范围，无法修改无关任务。
...
`summary` 是人类可读的结语，而 `metadata` 是机器可读的交接信息，下游 Agent、审阅者或仪表板无需解析文本即可复用。
...
1. 在启动时，调用 `kanban_show()` 来读取标题、正文、父级交接、先前尝试和完整的评论线程。
...
`kanban-worker` 是一个捆绑技能，在安装和更新期间会同步到每个配置文件中——没有单独的技能中心安装步骤。请在你用于看板工作者的任何配置文件（`researcher`、`writer`、`ops` 等）中验证其存在：
...
调度器在启动每个工作者时也会自动传递 `--skills kanban-worker`，因此即使配置文件的默认技能配置不包含它，工作者也始终可以访问模式库。

### 为特定任务固定额外技能

有时单个任务需要受派者配置文件默认不携带的专业上下文——需要 `translation` 技能的翻译工作、需要 `github-code-review` 的审查任务、需要 `security-pr-audit` 的安全审计。与其每次都编辑受派者的配置文件，不如直接将技能附加到任务上。
...
这些技能是内置 `kanban-worker` 的**附加项**——调度器为每个技能（以及内置技能）发出一个 `--skills <name>` 标志，因此工作者启动时会加载所有技能。技能名称必须与受派者配置文件上实际安装的技能匹配（运行 `hermes skills list` 查看可用内容）；没有运行时安装。

### 编排器技能

一个**行为良好的编排器不会自己完成工作。** 它将用户的目标分解为任务，链接它们，将每个分配给已设置的配置文件之一，然后退后一步。`kanban-orchestrator` 技能将此编码为工具调用模式：反诱惑规则、Step-0 配置文件发现提示词（调度器对未知受派者名称会静默失败，因此编排器必须将每个卡片锚定在机器上实际存在的配置文件中），以及基于 `kanban_create` / `kanban_link` / `kanban_comment` 的分解手册。
...
`kanban-orchestrator` 是一个捆绑技能。它在安装和更新期间会同步到每个配置文件中，因此没有单独的技能中心安装步骤。请在你的编排器配置文件中验证其存在：
...
`/kanban` CLI 和斜杠命令足以无头运行看板，但可视化看板通常是人工介入的合适界面：分类、跨配置文件监督、阅读评论线程以及在列之间拖放卡片。Hermes 将其作为**捆绑的仪表板插件**提供，位于 `plugins/kanban/`——不是核心功能，也不是独立服务——遵循[扩展仪表板](./extending-the-dashboard)中概述的模型。
...
- 一个 **Kanban** 标签页，每个状态显示一列：`triage`、`todo`、`ready`、`running`、`blocked`、`done`（加上切换开启时的 `archived`）。
  - `triage` 是粗略想法的暂存列，预期由规范制定者进行充实。使用 `hermes kanban create --triage`（或通过 Triage 列的内联创建）创建的任务会放在这里，调度器在人类或规范制定者将其提升到 `todo` / `ready` 之前不会处理它们。运行 `hermes kanban specify <id>` 让辅助 LLM 将分类任务扩展为具体规范（标题 + 包含目标、方法、验收标准的正文）并一次性将其提升到 `todo`；`--all` 一次性处理每个分类任务。在 `config.yaml` 的 `auxiliary.triage_specifier` 下配置运行规范制定器的模型。
- 卡片显示任务 ID、标题、优先级徽章、租户标签、分配的配置文件、评论/链接计数、**进度药丸**（当任务有依赖项时显示 `N/M` 个子项完成）以及“N 前创建”。每个卡片的复选框支持多选。
- **Running 列内按配置文件分车道**——工具栏复选框切换 Running 列按受派者子分组。
- **通过 WebSocket 实时更新**——插件以短轮询间隔跟踪仅追加的 `task_events` 表；当任何配置文件（CLI、消息网关或另一个仪表板标签页）采取行动时，看板会立即反映变化。重新加载经过防抖处理，因此突发事件只会触发一次重新获取。
...
- **内联创建**——点击任何列标题上的 `+` 输入标题、受派者、优先级，以及（可选）从所有现有任务的下拉列表中选择父任务。从 Triage 列创建会自动将新任务放入分类。
- **多选与批量操作**——shift/ctrl-点击卡片或勾选其复选框将其添加到选择中。顶部会出现一个批量操作栏，包含批量状态转换、归档和重新分配（通过配置文件下拉列表，或“(取消分配)”）。破坏性批量操作会先确认。每个 ID 的部分失败会报告，而不会中止其余操作。
- **点击卡片**（不带 shift/ctrl）打开侧边抽屉（按 Escape 或点击外部关闭），包含：
  - **可编辑标题**——点击标题重命名。
  - **可编辑受派者 / 优先级**——点击元数据行重写。
  - **可编辑描述**——默认以 Markdown 渲染（标题、粗体、斜体、行内代码、围栏代码、`http(s)` / `mailto:` 链接、项目符号列表），带有一个“编辑”按钮可切换为文本区域。Markdown 渲染是一个微小的、XSS 安全的渲染器——每次替换都在 HTML 转义的输入上运行，只有 `http(s)` / `mailto:` 链接通过，并且始终设置 `target="_blank"` + `rel="noopener noreferrer"`。
  - **依赖关系编辑器**——父项和子项的芯片列表，每个都带有一个 `×` 用于取消链接，加上用于添加新父项或子项的所有其他任务的下拉列表。循环尝试会在服务器端被拒绝，并显示明确消息。
...
- **工具栏过滤器**——自由文本搜索、租户下拉列表（默认为 `config.yaml` 中的 `dashboard.kanban.default_tenant`）、受派者下拉列表、“显示已归档”切换开关、“按配置文件分车道”切换开关，以及一个**推动调度器**按钮，这样你就不必等待下一个 60 秒的滴答。
视觉上目标是熟悉的 Linear/Fusion 布局：深色主题、带计数的列标题、彩色状态点、优先级和租户的胶囊标签。该插件仅读取主题 CSS 变量（`--color-*`、`--radius`、`--font-mono` 等），因此它会根据当前激活的仪表板主题自动换肤。

### 架构

GUI 严格是一个**通过数据库读取 + 通过 kanban_db 写入**的层，不包含自身的领域逻辑：
...
每个处理器都是一个薄包装 — 插件大约有 700 行 Python 代码（路由器 + WebSocket 跟踪 + 批量批处理器 + 配置读取器），不添加新的业务逻辑。一个微小的 `_conn()` 辅助函数会在每次读写时自动初始化 `kanban.db`，因此新安装无论用户是先打开仪表板、直接访问 REST API 还是运行 `hermes kanban init` 都能正常工作。
...
GUI 被刻意设计得很轻量。插件所做的所有功能都可以通过 CLI 访问；插件只是为了让人类使用更舒适。自动分配、预算、治理门禁和组织结构图视图仍属于用户空间——可以是路由配置文件、另一个插件，或者重用 `tools/approval.py`——正如设计规范中超出范围部分所列出的那样。

## CLI 命令参考

这是**你**（或脚本、定时任务、仪表板）用来驱动看板所使用的接口。在调度器中运行的 Worker 使用 `kanban_*` [工具接口](#how-workers-interact-with-the-board) 进行相同的操作——这里的 CLI 和那里的工具都通过 `kanban_db` 路由，因此这两个接口在构造上是一致的。
...
hermes kanban heartbeat <id> [--note "..."]            # 长时间操作的工作进程活跃信号
...
所有命令也可以在交互式 CLI 和消息网关中作为斜杠命令使用（见下文 [`/kanban` 斜杠命令](#kanban-slash-command)）。

## `/kanban` 斜杠命令 {#kanban-slash-command}

每个 `hermes kanban <action>` 动词也可以作为 `/kanban <action>` 访问——无论是在交互式 `hermes chat` 会话**内部**，还是从任何网关平台（Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Mattermost、电子邮件、SMS）。这两个接口都调用完全相同的 `hermes_cli.kanban.run_slash()` 入口点，该入口点复用了 `hermes kanban` 的 argparse 树，因此参数接口、标志和输出格式在 CLI、`/kanban` 和 `hermes kanban` 之间是相同的。你无需离开聊天界面即可驱动看板。
...
网关通常会在 Agent 仍在思考时排队处理斜杠命令和用户消息——这是为了防止你在第一个回合还在进行时意外启动第二个回合。**`/kanban` 被明确豁免于此防护。** 看板位于 `~/.hermes/kanban.db` 中，而不是在运行中 Agent 的状态中，因此读取（`list`、`show`、`context`、`tail`、`watch`、`stats`、`runs`）和写入（`comment`、`unblock`、`block`、`assign`、`archive`、`create`、`link`、…）都会立即执行，即使在回合进行中。

这正是这种分离的全部意义：

- 一个 Worker 阻塞等待同伴 → 你从手机发送 `/kanban unblock t_abcd`，调度器会在下一次调度时拾取该同伴。被阻塞的 Worker 不会被中断——它只是不再被阻塞。
- 你发现一个需要人工上下文的卡片 → `/kanban comment t_xyz "use the 2026 schema, not 2025"` 会落到任务线程上，该任务的**下一次**运行将在 `kanban_show()` 中读取它。
- 你想知道你的工作进程在做什么，又不想停止编排器 → `/kanban list --mine` 或 `/kanban stats` 会检查看板，而不影响你的主对话。
...
一旦任务达到 `done` 或 `archived` 状态，订阅会自动移除。如果您使用 `--json`（机器输出）通过脚本创建任务，则会跳过自动订阅——假设脚本化调用方希望通过 `/kanban notify-subscribe` 显式管理订阅。
...
| **P9 分类指定器** | 粗略想法 → `triage` → `hermes kanban specify` 扩展正文 → `todo` | "将这个单行想法转化为一个已指定的任务" |
...
Worker 会收到 `$HERMES_TENANT` 并通过前缀对其记忆写入进行命名空间隔离。看板、调度器和配置文件定义都是共享的；只有数据是作用域隔离的。
...
任务是一个逻辑工作单元；**运行记录** 是一次执行它的尝试。当调度器认领一个就绪任务时，它会在 `task_runs` 中创建一行，并将 `tasks.current_run_id` 指向它。当该尝试结束时——完成、阻塞、崩溃、超时、生成失败、被回收——运行记录行会以一个 `outcome` 关闭，并且任务的指针被清除。一个被尝试了三次的任务会有三行 `task_runs` 记录。
...
```
当您（人类）需要完成一个工作者无法完成的任务时——例如被放弃的任务，或您从仪表板手动标记为完成的任务——可以通过 CLI 执行相同的交接：
...
**状态变更回收的运行。** 如果您在仪表板中将正在运行的任务从 `running` 拖走（回到 `ready`，或直接到 `todo`），或者归档一个仍在运行的任务，正在进行的运行将以 `outcome='reclaimed'` 关闭，而不是被孤立。当 `tasks.current_run_id` 为 `NULL` 时，`task_runs` 行始终处于终止状态，反之亦然——这个不变量在 CLI、仪表板、调度器和通知器之间保持一致。
...
**实时抽屉刷新。** 当仪表板的 WebSocket 事件流报告用户当前正在查看的任务有新事件时，抽屉会自行重新加载（通过每个任务的事件计数器传递到其 `useEffect` 依赖项列表中）。不再需要关闭和重新打开以查看运行的新行或更新的结果。

### 向前兼容性

`tasks` 上的两个可为空的列保留给 v2 工作流路由：`workflow_template_id`（此任务所属的模板）和 `current_step_key`（该模板中哪个步骤处于活动状态）。v1 内核在路由时忽略它们，但允许客户端写入它们，因此 v2 版本可以添加路由机制而无需另一次模式迁移。
...
| `completed` | `{result_len, summary?}` | 工作者写入了 `--result` / `--summary` 且任务达到 `done`。`summary` 是首行交接（400 字符上限）；完整版本位于运行行上。如果在从未被认领的任务上调用 `complete_task` 并带有交接字段，则会合成一个零持续时间的运行，因此 `run_id` 仍然指向某个内容。 |

...
> **想要一个完整的操作指南？** 请阅读 [看板教程](./kanban-tutorial) — 其中包含四个用户故事（独立开发者、舰队式运营、带重试的角色流水线、熔断器），并附有每个故事的仪表盘截图。本页是参考文档；教程是叙述性指南。

Hermes 看板是一个持久化的任务板，在所有 Hermes 人格之间共享，允许多个具名 Agent 协作处理工作，而无需依赖脆弱的进程内子 Agent 群。每个任务都是 `~/.hermes/kanban.db` 中的一行；每次交接都是任何人都可以读写的一行；每个工作者都是一个拥有自己身份的完整操作系统进程。
...
两个界面都通过同一个 `kanban_db` 层路由，因此读取操作看到的是统一的视图，写入操作不会出现偏差。本页其余部分展示 CLI 示例，因为它们易于复制粘贴，但每个 CLI 动词都有模型使用的等效工具调用。
...
- **舰队式工作** — 一个专家管理 N 个对象（50 个社交账户，12 个监控服务）。
...
- **工作空间** — 工作者在其中操作的目录。有三种类型：
  - `scratch`（默认）— `~/.hermes/kanban/workspaces/<id>/` 下的全新临时目录（或在非默认看板上为 `~/.hermes/kanban/boards/<slug>/workspaces/<id>/`）。
  - `dir:<path>` — 一个现有的共享目录（Obsidian 保险库、邮件操作目录、每个账户的文件夹）。**必须是绝对路径。** 像 `dir:../tenants/foo/` 这样的相对路径在调度时会被拒绝，因为它们会相对于调度器当前所在的任何 CWD 进行解析，这是不明确的，并且是一个混乱的代理逃逸向量。该路径在其他情况下是受信任的 — 这是你的机器，你的文件系统，工作者以你的用户 ID 运行。这是受信任的本地用户威胁模型；看板在设计上是单主机的。
  - `worktree` — 用于编码任务的 `.worktrees/<id>/` 下的 git 工作树。工作者端的 `git worktree add` 会创建它。
- **调度器** — 一个长期运行的循环，每 N 秒（默认 60 秒）执行一次：回收过期的认领、回收崩溃的工作者（PID 消失但 TTL 尚未过期）、提升就绪的任务、原子性地认领、生成分配的人格。默认情况下在**消息网关内部**运行（`kanban.dispatch_in_gateway: true`）。一个调度器每次扫描所有看板；工作者生成时带有固定的 `HERMES_KANBAN_BOARD` 环境变量，因此它们无法看到其他看板。在同一任务上连续 `kanban.failure_limit` 次（默认：2）生成失败后，调度器会自动将其阻塞，并将最后一个错误作为原因 — 防止在人格不存在、工作空间无法挂载等任务上产生抖动。
...
-   **看板下拉菜单** —— 选择活动看板。你的选择会保存到浏览器的 `localStorage` 中，因此在重新加载时它会持久化，而不会改变你已打开终端中 CLI 的 `current` 指针。
...
以下命令是**你**（人类）设置看板和创建任务。一旦任务被分配，调度器就会将分配的个人资料作为 Worker 启动，从那里开始，**模型通过 `kanban_*` 工具调用来驱动任务，而不是 CLI 命令**——参见 [Worker 如何与看板交互](#how-workers-interact-with-the-board)。
...
当调度器获取到 `t_abcd` 并启动 `researcher` 个人资料作为 Worker 时，该 Worker 的模型做的第一件事就是调用 `kanban_show()` 来读取其任务。它不会运行 `hermes kanban show t_abcd`。
...
可以通过 `HERMES_KANBAN_DISPATCH_IN_GATEWAY=0` 在运行时覆盖配置标志以进行调试。标准的消息网关监控适用：直接运行 `hermes gateway start`，或者将消息网关配置为 systemd 用户单元（参见消息网关文档）。如果没有运行的消息网关，`ready` 任务会保持原状，直到有消息网关启动——`hermes kanban create` 在创建时会对此发出警告。
...
| `kanban_complete` | 使用 `summary` + `metadata` 结构化交接完成任务。 | `summary` / `result` 至少一个 |
| `kanban_block` | 因 `reason` 而升级以请求人工输入。 | `reason` |
| `kanban_heartbeat` | 在长时间操作期间发送存活信号。纯副作用。 | — |
| `kanban_comment` | 向任务线程追加一条持久性注释。 | `task_id`, `body` |
| `kanban_create` | （编排器）使用 `assignee`、可选的 `parents`、`skills` 等参数创建子任务。 | `title`, `assignee` |
...
三个"（编排器）"工具——`kanban_create`、`kanban_link` 以及对其他任务的 `kanban_comment`——对所有 worker 都可用；约定（由 `kanban-orchestrator` 技能强制执行）是：worker 配置文件不进行任务分发，而编排器配置文件不执行具体任务。

### 为什么使用工具而不是通过 shell 调用 `hermes kanban`

三个原因：

1.  **后端可移植性。** 终端工具指向远程后端（Docker / Modal / Singularity / SSH）的 worker 会在*容器内部*运行 `hermes kanban complete`，而容器内没有安装 `hermes` 且 `~/.hermes/kanban.db` 未挂载。看板工具在 Agent 自身的 Python 进程中运行，无论终端后端是什么，始终能访问 `~/.hermes/kanban.db`。
...
**对普通会话的零模式影响。** 一个常规的 `hermes chat` 会话在其模式中没有任何 `kanban_*` 工具。每个工具的 `check_fn` 仅在设置了 `HERMES_KANBAN_TASK` 时才返回 True，而这仅在调度器生成此进程时发生。对于从不接触看板的用户，没有工具膨胀问题。
...
`summary` 是人类可读的总结，而 `metadata` 是机器可读的交接信息，下游 Agent、审阅者或仪表板无需解析文本即可复用。
...
请勿将秘密、原始日志、Token、OAuth 材料以及无关的对话记录放入 `metadata`。请存储指针和摘要。如果任务没有文件或测试，请在 `summary` 中明确说明，并使用 `metadata` 存储确实存在的证据，例如源 URL、问题 ID 或手动审阅步骤。
...
`kanban-worker` 是一个捆绑技能，在安装和更新期间会同步到每个配置文件中——没有单独的 Skills Hub 安装步骤。请验证你用于看板 worker 的任何配置文件（`researcher`、`writer`、`ops` 等）中都存在此技能：
```bash
...
调度器在启动每个 worker 时也会自动传递 `--skills kanban-worker`，因此即使配置文件的默认技能配置不包含它，worker 也始终拥有模式库。

### 将额外技能固定到特定任务

有时单个任务需要执行者配置文件默认不携带的特定上下文——需要 `translation` 技能的翻译工作、需要 `github-code-review` 的审查任务、需要 `security-pr-audit` 的安全审计。与其每次都编辑执行者的配置文件，不如直接将技能附加到任务上。
...
**从仪表板**，在内联创建表单的 **skills** 字段中输入以逗号分隔的技能。

这些技能是内置 `kanban-worker` 的**附加项**——调度器为每个技能（以及内置技能）发出一个 `--skills <name>` 标志，因此 worker 启动时会加载所有技能。技能名称必须与执行者配置文件上实际安装的技能匹配（运行 `hermes skills list` 查看可用的技能）；没有运行时安装。

### 编排器技能

一个**行为良好的编排器不会自己完成工作。** 它将用户的目标分解为任务，链接它们，将每个任务分配给专家，然后退后一步。`kanban-orchestrator` 技能将此编码为工具调用模式：反诱惑规则、标准专家名册（`researcher`、`writer`、`analyst`、`backend-eng`、`reviewer`、`ops`），以及一个基于 `kanban_create` / `kanban_link` / `kanban_comment` 的分解手册。
...
- 一个 **Kanban** 标签页，每个状态显示一列：`triage`、`todo`、`ready`、`running`、`blocked`、`done`（加上切换开启时的 `archived`）。
  - `triage` 是粗略想法的暂存列，预计由规范制定者进行充实。使用 `hermes kanban create --triage`（或通过 Triage 列的内联创建）创建的任务会放在这里，调度器在人类或规范制定者将它们提升到 `todo` / `ready` 之前不会处理它们。运行 `hermes kanban specify <id>` 让辅助 LLM 将分类任务扩展为具体规范（标题 + 包含目标、方法、验收标准的正文）并一次性将其提升到 `todo`；`--all` 一次性处理每个分类任务。在 `config.yaml` 的 `auxiliary.triage_specifier` 下配置运行规范制定者的模型。
- 卡片显示任务 ID、标题、优先级徽章、租户标签、分配的配置文件、评论/链接计数、**进度药丸**（当任务有依赖项时显示 `N/M` 个子任务完成）以及“N 前创建”。每张卡片的复选框支持多选。
- **Running 列内的按配置文件泳道** —— 工具栏复选框切换 Running 列按执行者进行子分组。
- **通过 WebSocket 实时更新** —— 插件在短轮询间隔内跟踪仅追加的 `task_events` 表；当任何配置文件（CLI、消息网关或另一个仪表板标签页）采取行动时，看板会立即反映变化。重新加载是防抖的，因此一系列事件只会触发一次重新获取。
- **拖放**卡片在列之间移动以更改状态。放置操作发送 `PATCH /api/plugins/kanban/tasks/:id`，该请求通过 CLI 使用的相同 `kanban_db` 代码路由 —— 三个界面永远不会漂移。移动到破坏性状态（`done`、`archived`、`blocked`）会提示确认。触摸设备使用基于指针的回退，因此看板在平板电脑上也可用。
- **内联创建** —— 点击任何列标题上的 `+` 输入标题、执行者、优先级，以及（可选）从覆盖所有现有任务的下拉列表中选择父任务。从 Triage 列创建会自动将新任务置于分类状态。
- **多选与批量操作** —— shift/ctrl-点击卡片或勾选其复选框将其添加到选择中。顶部会出现一个批量操作栏，包含批量状态转换、归档和重新分配（通过配置文件下拉列表，或“(取消分配)”）。破坏性批量操作首先确认。每个 ID 的部分失败会被报告，而不会中止其余操作。
- **点击卡片**（不带 shift/ctrl）打开侧边抽屉（按 Escape 或点击外部关闭），包含：
  - **可编辑标题** —— 点击标题进行重命名。
  - **可编辑执行者 / 优先级** —— 点击元数据行进行重写。
  - **可编辑描述** —— 默认以 Markdown 渲染（标题、粗体、斜体、行内代码、代码块、`http(s)` / `mailto:` 链接、项目符号列表），带有一个“编辑”按钮，点击后切换为文本区域。Markdown 渲染是一个小巧、XSS 安全的渲染器 —— 每个替换都在 HTML 转义的输入上运行，只有 `http(s)` / `mailto:` 链接会通过，并且始终设置 `target="_blank"` + `rel="noopener noreferrer"`。
  - **依赖关系编辑器** —— 父任务和子任务的芯片列表，每个都带有 `×` 以取消链接，加上覆盖所有其他任务的下拉列表以添加新的父任务或子任务。循环尝试会在服务器端被拒绝，并显示明确的消息。
  - **状态操作行**（→ triage / → ready / → running / block / unblock / complete / archive），破坏性转换会提示确认。对于 **Triage** 列中的卡片，该行还暴露一个 **✨ Specify** 按钮，调用辅助 LLM（`config.yaml` 中的 `auxiliary.triage_specifier`）将单行描述扩展为具体规范（标题 + 包含目标、方法、验收标准的正文）并将任务提升到 `todo`。相同的行为可以通过 CLI（`hermes kanban specify <id>` / `--all`）、任何消息网关平台（`/kanban specify <id>`）以及通过 `POST /api/plugins/kanban/tasks/:id/specify` 以编程方式实现。
  - 结果部分（同样以 Markdown 渲染）、按 Enter 提交的评论线程、最近 20 个事件。
- **工具栏过滤器** —— 自由文本搜索、租户下拉列表（默认为 `config.yaml` 中的 `dashboard.kanban.default_tenant`）、执行者下拉列表、“显示已归档”切换开关、“按配置文件泳道”切换开关，以及一个**推动调度器**按钮，这样你就不必等待下一个 60 秒的滴答。
...
│  plugins/kanban/       │     CLI /kanban 命令使用的路径     │
...
每个处理程序都是一个薄包装 — 插件大约有 700 行 Python 代码（路由器 + WebSocket 追踪 + 批量批处理器 + 配置读取器），不添加任何新的业务逻辑。一个微小的 `_conn()` 辅助函数会在每次读写时自动初始化 `kanban.db`，因此无论是用户先打开仪表板、直接访问 REST API，还是运行 `hermes kanban init`，新安装都能正常工作。
...
WebSocket 额外增加了一步：它需要仪表板的临时会话 Token 作为 `?token=…` 查询参数（浏览器无法在升级请求上设置 `Authorization`），这与浏览器内 PTY 桥接器使用的模式相匹配。
...
该插件使用标准的 Hermes 仪表板插件契约 — 有关完整清单参考、shell 插槽、页面作用域插槽和 Plugin SDK，请参阅[扩展仪表板](./extending-the-dashboard)。额外的列、自定义卡片样式、租户过滤布局或完整的 `tab.override` 替换都可以表达，而无需分叉此插件。

要禁用而不删除：在 `config.yaml` 中添加 `dashboard.plugins.kanban.enabled: false`（或删除 `plugins/kanban/dashboard/manifest.json`）。
### 范围边界

GUI 被刻意设计得很薄。插件所做的一切都可以通过 CLI 访问；插件只是为了让人类使用更舒适。自动分配、预算、治理门禁和组织结构图视图仍属于用户空间——可以是路由器配置文件、另一个插件，或者重用 `tools/approval.py`——正如设计规范中超出范围部分所列出的那样。
...
hermes kanban log <id> [--tail 字节数]                  # 来自 ~/.hermes/kanban/logs/ 的工作器日志
...
hermes kanban specify [<id> | --all] [--tenant T]      # 将待处理列中的想法充实
        [--author 名称] [--json]                       #   为完整规范并提升为待办事项
hermes kanban gc [--event-retention-days N]            # 工作空间 + 旧 events + 旧日志
...
每个 `hermes kanban <动作>` 动词也可以作为 `/kanban <动作>` 访问——无论是在交互式 `hermes chat` 会话**内部**，还是从任何网关平台（Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Mattermost、电子邮件、SMS）。这两个界面都调用完全相同的 `hermes_cli.kanban.run_slash()` 入口点，该入口点复用了 `hermes kanban` 的 argparse 树，因此参数界面、标志和输出格式在 CLI、`/kanban` 和 `hermes kanban` 之间是相同的。你无需离开聊天界面即可驱动看板。
...
/kanban specify --all --tenant engineering  # 一次性处理某个租户中的所有待处理任务
```
...
- 一个工作器因等待对等方而阻塞 → 你从手机发送 `/kanban unblock t_abcd`，调度器会在下一次调度时拾取该对等方。被阻塞的工作器不会被中断——它只是不再被阻塞。
- 你发现一个卡片需要人工上下文 → `/kanban comment t_xyz "使用 2026 模式，而不是 2025"` 会落在任务线程上，该任务的**下一次**运行将在 `kanban_show()` 中读取它。
- 你想知道你的工作器群在做什么，而不停止编排器 → `/kanban list --mine` 或 `/kanban stats` 会检查看板，而不影响你的主对话。
...
一旦任务达到 `done` 或 `archived` 状态，订阅会自动移除。如果您使用 `--json`（机器输出）通过脚本创建任务，则会跳过自动订阅——假设脚本化的调用者希望通过 `/kanban notify-subscribe` 显式管理订阅。
...
Worker 会收到 `$HERMES_TENANT` 并通过前缀对其记忆写入进行命名空间隔离。看板、调度器和配置定义都是共享的；只有数据是作用域隔离的。
...
任务是工作的逻辑单元；**运行记录** 是对其执行的一次尝试。当调度器认领一个就绪任务时，它会在 `task_runs` 中创建一行，并将 `tasks.current_run_id` 指向它。当该次尝试结束时——完成、阻塞、崩溃、超时、生成失败、被回收——运行记录行会以一个 `outcome` 关闭，并且任务的指针被清除。一个被尝试了三次的任务将有三行 `task_runs` 记录。

为什么需要两个表而不是直接修改任务：您需要 **完整的尝试历史** 来进行实际的事后分析（"第二次审阅者尝试得以批准，第三次合并了"），并且您需要一个干净的地方来挂载每次尝试的元数据——哪些文件被更改、哪些测试运行了、审阅者记录了哪些发现。这些是运行事实，而不是任务事实。
...
```
当您（人类）需要完成一个工作者无法完成的任务时——例如被放弃的任务，或您从仪表板手动标记为已完成的任务——可以通过 CLI 执行相同的交接：
...
**状态变更回收的运行。** 如果您在仪表板中将一个正在运行的任务从 `running` 拖走（回到 `ready`，或直接到 `todo`），或者归档一个仍在运行的任务，正在进行的运行将以 `outcome='reclaimed'` 关闭，而不是被孤立。当 `tasks.current_run_id` 为 `NULL` 时，`task_runs` 行始终处于终止状态，反之亦然——这个不变量在 CLI、仪表板、调度器和通知器之间都成立。
...
**实时抽屉刷新。** 当仪表板的 WebSocket 事件流报告用户当前正在查看的任务有新事件时，抽屉会自行重新加载（通过一个按任务的事件计数器，该计数器被编织到其 `useEffect` 依赖项列表中）。不再需要关闭并重新打开来查看运行的新行或更新的结果。
...
| `completed` | `{result_len, summary?}` | 工作者写入了 `--result` / `--summary` 且任务达到 `done`。`summary` 是首行交接信息（400 字符上限）；完整版本位于运行行上。如果在从未被认领的任务上调用 `complete_task` 并带有交接字段，则会合成一个零持续时间的运行，以便 `run_id` 仍然指向某个内容。 |
...
Kanban 设计上是单主机的。`~/.hermes/kanban.db` 是一个本地的 SQLite 文件，并且调度器在同一台机器上启动工作进程。不支持在两个主机之间运行共享看板——没有用于协调“主机 A 上的工作进程 X，主机 B 上的工作进程 Y”的底层原语，并且故障检测路径假设 PID 是主机本地的。如果你需要多主机支持，请在每个主机上运行一个独立的看板，并使用 `delegate_task` / 消息队列来桥接它们。

## 设计规范

完整的设计——架构、并发正确性、与其他系统的比较、实施计划、风险、待解决问题——位于 `docs/hermes-kanban-v1-spec.pdf` 文件中。在提交任何行为变更的 PR 之前，请先阅读该文档。
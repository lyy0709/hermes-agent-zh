...
> **需要引导教程？** 请阅读 [看板教程](./kanban-tutorial) — 包含四个用户故事（独立开发者、舰队耕作、带重试的角色流水线、熔断器）以及每个故事的仪表盘截图。本页是参考文档；教程是叙述性指南。

Hermes 看板是一个持久化的任务面板，在所有 Hermes 人格之间共享，允许多个具名 Agent 协作处理工作，而无需依赖脆弱的进程内子 Agent 集群。每个任务都是 `~/.hermes/kanban.db` 中的一行；每次交接都是任何人都可以读写的一行；每个工作者都是一个拥有自己身份的完整操作系统进程。

### 两个界面：模型通过工具交流，你通过 CLI 交流

看板有两个入口，都基于同一个 `~/.hermes/kanban.db`：

- **Agent 通过专用的 `kanban_*` 工具集驱动看板** — `kanban_show`、`kanban_list`、`kanban_complete`、`kanban_block`、`kanban_heartbeat`、`kanban_comment`、`kanban_create`、`kanban_link`、`kanban_unblock`。调度器在生成每个工作者时，其模式中已包含这些工具；编排器人格也可以显式启用 `kanban` 工具集。模型通过直接调用工具来读取和路由任务，*而不是*通过 shell 调用 `hermes kanban`。请参阅下面的 [工作者如何与看板交互](#how-workers-interact-with-the-board)。
- **你（以及脚本和定时任务）通过 CLI 上的 `hermes kanban …`**、斜杠命令 `/kanban …` 或仪表盘来驱动看板。这些是为人类和自动化设计的 — 即那些背后没有工具调用模型的场景。

两个界面都通过同一个 `kanban_db` 层路由，因此读取能看到一致的视图，写入不会漂移。本页其余部分展示 CLI 示例，因为它们易于复制粘贴，但每个 CLI 命令都有一个模型使用的等效工具调用。
...
有关完整的设计原理、与 Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise 的对比分析，以及八种经典协作模式，请参阅代码仓库中的 `docs/hermes-kanban-v1-spec.pdf`。
...
- **看板** — 一个独立的任务队列，拥有自己的 SQLite 数据库、工作空间目录和调度器循环。单个安装可以拥有多个看板（例如，每个项目、仓库或域一个）；请参阅下面的 [看板（多项目）](#boards-multi-project)。单项目用户停留在 `default` 看板上，并且在此文档部分之外永远不会看到“看板”这个词。
...
2.  `HERMES_KANBAN_BOARD` 环境变量（由调度器在启动 Worker 时设置，因此 Worker 看不到其他看板）。
3.  `~/.hermes/kanban/current` —— 由 `hermes kanban boards switch` 持久保存的 slug。
4.  `default`。

Slug 会经过验证：小写字母数字 + 连字符 + 下划线，1-64 个字符，必须以字母数字开头。大写输入会自动转换为小写。其他任何字符（斜杠、空格、点、`..`）都会在 CLI 层被拒绝，因此路径遍历技巧无法命名看板。

### 通过仪表板管理看板

`hermes dashboard` → Kanban 标签页在存在多个看板（或任何看板有任务）时，顶部会显示一个看板切换器。单看板用户只看到一个小的 `+ New board` 按钮；切换器在需要时才会显示。

-   **看板下拉菜单** —— 选择活动看板。你的选择会保存到浏览器的 `localStorage` 中，因此在重新加载时保持不变，而不会改变 CLI 的 `current` 指针（如果你在终端中保持打开状态）。
...
-   **上传** —— 在仪表板抽屉中打开一个任务，并使用 **附件** 部分的 *上传文件* 按钮（一次可以上传多个文件）。每次上传限制为 25 MB。
-   **存储** —— 文件存储在默认看板的 `<hermes-home>/kanban/attachments/<task_id>/` 下，或命名看板的 `<hermes-home>/kanban/boards/<slug>/attachments/<task_id>/` 下。设置 `HERMES_KANBAN_ATTACHMENTS_ROOT` 以固定到自定义位置。
...
当调度器拾取 `t_abcd` 并启动 `researcher` 个人资料时，该 Worker 的模型所做的第一件事就是调用 `kanban_show()` 来读取其任务。它不会运行 `hermes kanban show t_abcd`。
...
将 `hermes kanban daemon` 作为独立进程运行的方式**已弃用**；请使用消息网关。如果你确实无法运行消息网关（例如无头主机策略禁止长时间运行的服务等），可以使用 `--force` 作为逃生通道，让旧的独立守护进程再存活一个发布周期，但同时运行一个嵌入消息网关的调度器**和**一个独立守护进程来处理同一个 `kanban.db` 会导致任务认领竞争，这是不受支持的。

### 幂等创建（用于自动化 / Webhook）

```bash
# 第一次调用创建任务。任何后续使用相同 key 的调用
# 将返回现有任务 ID，而不是重复创建。
...
**工作器不会通过 shell 调用 `hermes kanban`。** 当调度器生成一个工作器时，它会在子进程的环境中设置 `HERMES_KANBAN_TASK=t_abcd`，这个环境变量会激活模型架构中一个专用的**看板工具集**。同样的工具集也适用于在其工具集配置中启用了 `kanban` 的编排器配置文件。这些工具通过 Python 的 `kanban_db` 层直接读取和修改看板，与 CLI 的方式相同。一个运行中的工作器像调用其他工具一样调用这些工具；它从不接触也不需要 `hermes kanban` CLI。
...
标有"（编排器）"的工具——`kanban_list`、`kanban_create`、`kanban_link`、`kanban_unblock` 以及对其他任务的 `kanban_comment`——可通过同一个工具集使用；约定（由 `kanban-orchestrator` 技能强制执行）是：工作器配置文件不进行任务分解或路由无关工作，而编排器配置文件不执行具体实现工作。由调度器生成的工作器在破坏性生命周期操作上仍然受限于其任务范围，无法修改无关任务。

### 为什么使用工具而不是通过 shell 调用 `hermes kanban`

三个原因：

1.  **后端可移植性。** 如果工作器的终端工具指向远程后端（Docker / Modal / Singularity / SSH），那么 `hermes kanban complete` 命令将在容器*内部*运行，而容器内可能没有安装 `hermes`，也没有挂载 `~/.hermes/kanban.db`。看板工具在 Agent 自身的 Python 进程中运行，无论终端后端是什么，总能访问到 `~/.hermes/kanban.db`。
...
**对普通会话零架构影响。** 一个常规的 `hermes chat` 会话在其架构中没有任何 `kanban_*` 工具，除非活动配置文件明确启用了 `kanban` 工具集用于编排器工作。由调度器生成的任务工作器获得任务范围内的工具，因为设置了 `HERMES_KANBAN_TASK`；编排器配置文件通过配置获得更广泛的路由接口。对于从不接触看板的用户，没有工具膨胀问题。
`kanban-worker` 和 `kanban-orchestrator` 技能教会模型在何时以及以何种顺序调用工具。

### 推荐的交接证据

`kanban_complete(summary=..., metadata={...})` 被设计得非常灵活：
`summary` 是人类可读的收尾总结，而 `metadata` 是机器可读的交接信息，下游 Agent、审查者或仪表板可以直接复用，无需从文本中提取。
...
请勿将秘密、原始日志、Token、OAuth 材料以及无关的对话记录放入 `metadata`。请存储指针和摘要。如果任务没有文件或测试，请在 `summary` 中明确说明，并使用 `metadata` 存放确实存在的证据，例如源 URL、问题 ID 或手动审查步骤。

### 工作者技能

任何需要能够处理看板任务的配置文件都必须加载 `kanban-worker` 技能。它通过**工具调用**（而非 CLI 命令）来教导工作者完整的生命周期：
...
`kanban-worker` 是一个捆绑技能，在安装和更新期间会同步到每个配置文件中——没有单独的 Skills Hub 安装步骤。请验证你用于看板工作者的任何配置文件（`researcher`、`writer`、`ops` 等）中都存在该技能：
...
调度器在生成每个工作者时也会自动传递 `--skills kanban-worker`，因此即使配置文件的默认技能配置不包含它，工作者也始终可以访问模式库。

### 为特定任务固定额外技能

有时单个任务需要受派者配置文件默认不携带的专家上下文——例如需要 `translation` 技能的翻译工作、需要 `github-code-review` 的审查任务、需要 `security-pr-audit` 的安全审计。与其每次都编辑受派者的配置文件，不如直接将技能附加到任务上。

**从编排器 Agent 出发**（通常情况——一个 Agent 将工作路由给另一个 Agent），使用 `kanban_create` 工具的 `skills` 数组：
...
**从仪表板出发**，在内联创建表单的 **skills** 字段中，以逗号分隔输入技能名称。
...
默认情况下，每个工作者对其卡片只有**一次机会**——完成工作，调用 `kanban_complete`/`kanban_block`，然后退出。传递 `--goal`（CLI）或 `goal_mode=True`（`kanban_create` 工具 / 仪表板）可以让该工作者在**目标循环**中运行，这与 `/goal` 斜杠命令背后的 Ralph 风格引擎相同：在每次轮次后，一个辅助判断器会根据卡片的标题 + 正文（视为验收标准）检查工作者的输出，如果工作未完成——并且轮次预算仍有剩余——工作者将在**同一会话**中继续工作，直到判断器同意、工作者自行终止任务，或者预算耗尽（此时卡片会被**阻塞**以进行人工审查，而不是静默退出）。
...
```
将其用于开放式、多步骤或“持续执行直到满足 X 条件”的任务卡片。对于廉价的一次性工作，请跳过它——每次判断的额外开销不值得，而且调度器现有的重试/熔断机制已经能处理临时的 Worker 故障。判断的效果完全取决于你的目标文本质量，因此请将正文内容写成**明确的验收标准**。

### 编排器技能

一个**行为良好的编排器不会自己执行工作。** 它会将用户的目标分解为任务，链接它们，将每个任务分配给你已设置的配置文件之一，然后退居幕后。`kanban-orchestrator` 技能将此编码为工具调用模式：反诱惑规则、一个 Step-0 配置文件发现提示词（调度器在遇到未知的受分配者名称时会静默失败，因此编排器必须将每个卡片都基于你机器上实际存在的配置文件），以及一个基于 `kanban_create` / `kanban_link` / `kanban_comment` 的分解操作手册。
...
`kanban-orchestrator` 是一个捆绑的技能。它在安装和更新期间会同步到每个配置文件中，因此没有单独的技能中心安装步骤。在你的编排器配置文件中验证它是否存在：

```bash
hermes -p orchestrator skills list | grep kanban-orchestrator
```

如果捆绑的副本缺失，请为该配置文件恢复它：
...
`/kanban` CLI 和斜杠命令足以无头运行看板，但可视化看板通常是人工介入场景的合适界面：分类、跨配置文件监督、阅读评论线程以及在列之间拖放卡片。Hermes 将此作为**捆绑的仪表板插件**提供，位于 `plugins/kanban/` —— 不是核心功能，也不是独立服务 —— 遵循 [扩展仪表板](./extending-the-dashboard) 中概述的模式。
...
- 一个 **Kanban** 标签页，每个状态显示一列：`triage`、`todo`、`ready`、`running`、`blocked`、`done`（加上切换开启时的 `archived`）。
  - `triage` 是存放粗略想法的暂存列。默认情况下（`kanban.auto_decompose: true`），调度器会自动对到达此处的任务运行**分解器** —— 编排器配置文件读取粗略想法，查看你的配置文件列表（包含描述），并将任务分解成一个小的子任务图，路由到最合适的专家。原始任务作为每个子任务的父任务保持活动状态，以便在所有子任务完成时，编排器会再次唤醒以判断完成情况。切换页面顶部的 **Orchestration: Auto/Manual** 药丸（或设置 `kanban.auto_decompose: false`）可切换到手动模式，在此模式下，分类任务将保持不动，直到你点击卡片上的 **⚗ Decompose** 或运行 `hermes kanban decompose <id>`。对于不需要分解的任务（或没有编排器配置文件的设置），**✨ Specify** 按钮通过相同的 LLM 机制执行单任务规范重写（标题 + 包含目标、方法、验收标准的正文）。参见下面的 [自动与手动编排](#auto-vs-manual-orchestration)。
- 卡片显示任务 ID、标题、优先级徽章、租户标签、分配的配置文件、评论/链接数量、一个**进度药丸**（当任务有依赖项时显示 `N/M` 个子任务完成情况）以及“N 前创建”。每个卡片的复选框支持多选。
- **Running 列内的每个配置文件泳道** —— 工具栏复选框可切换 Running 列按受分配者进行子分组。
- **通过 WebSocket 实时更新** —— 插件以短轮询间隔跟踪仅追加的 `task_events` 表；当任何配置文件（CLI、消息网关或另一个仪表板标签页）执行操作时，看板会立即反映变化。重新加载是防抖的，因此一系列事件只会触发一次重新获取。
- **拖放**卡片在列之间移动以更改状态。放置操作会发送 `PATCH /api/plugins/kanban/tasks/:id`，该请求通过 CLI 使用的相同 `kanban_db` 代码路由 —— 三个界面永远不会出现不一致。移动到破坏性状态（`done`、`archived`、`blocked`）会提示确认。触摸设备使用基于指针的回退方案，因此看板可在平板电脑上使用。
- **内联创建** —— 点击任何列标题上的 `+` 可以输入标题、受分配者、优先级，并（可选）从覆盖所有现有任务的下拉列表中选择父任务。按 Enter 创建任务，Shift+Enter 在标题字段中插入换行符，或 Escape 取消。从 Triage 列创建会自动将新任务放入分类列。
...
- **点击卡片**（不带 shift/ctrl）打开侧边抽屉（按 Escape 或点击外部关闭），其中包含：
  ...
  - **可编辑描述** —— 默认以 Markdown 渲染（标题、粗体、斜体、行内代码、代码块、`http(s)` / `mailto:` 链接、项目符号列表），带有一个“编辑”按钮，点击后会切换为文本区域。Markdown 渲染是一个微型的、XSS 安全的渲染器 —— 每次替换都在 HTML 转义的输入上运行，只有 `http(s)` / `mailto:` 链接会通过，并且始终设置 `target="_blank"` + `rel="noopener noreferrer"`。
  - **依赖关系编辑器** —— 父任务和子任务的芯片列表，每个都带有一个 `×` 用于取消链接，加上覆盖所有其他任务的下拉列表以添加新的父任务或子任务。循环依赖的尝试会在服务器端被拒绝，并显示明确的消息。
  - **状态操作行**（→ triage / → ready / → running / block / unblock / complete / archive），破坏性转换会提示确认。对于 **Triage** 列中的卡片，该行还暴露了两个 LLM 驱动的操作：**⚗ Decompose** 将任务分解成一个子任务图，根据描述路由到专家配置文件（编排器驱动的路径），而 **✨ Specify** 执行单任务规范重写。当 LLM 判断任务不需要分解时，Decompose 会回退到 specify 风格的提升，因此它是严格超集。两者都可以通过 CLI（`hermes kanban decompose <id>` / `specify <id>` / `--all`）、任何消息网关平台（`/kanban decompose <id>`）以及通过 `POST /api/plugins/kanban/tasks/:id/decompose` 和 `…/specify` 以编程方式访问。在 `config.yaml` 中的 `auxiliary.kanban_decomposer` 和 `auxiliary.triage_specifier` 下配置模型。
  - 结果部分（同样以 Markdown 渲染）、带 Enter 提交的评论线程、最后 20 个事件。
- **工具栏过滤器** —— 自由文本搜索、租户下拉列表（默认为 `config.yaml` 中的 `dashboard.kanban.default_tenant`）、受分配者下拉列表、“显示已归档”切换开关、“按配置文件泳道”切换开关，以及一个**提醒调度器**按钮，这样你就不必等待下一个 60 秒的周期。
从视觉上看，目标是实现熟悉的 Linear / Fusion 布局：深色主题、带计数的列标题、彩色状态点、用于优先级和租户的药丸形标签。该插件仅读取主题 CSS 变量（`--color-*`、`--radius`、`--font-mono` 等），因此它会根据当前激活的仪表板主题自动换肤。

### 自动与手动编排

看板有两种方式处理你拖入 Triage 列的任务：

**自动（默认）** — `kanban.auto_decompose: true`。消息网关内嵌的调度器在每个调度周期运行**分解器**，上限由 `kanban.auto_decompose_per_tick` 控制（默认每周期 3 个任务），这样批量加载的待处理任务就不会突发消耗辅助 LLM。分解器读取粗略的想法，查看你已安装的配置文件及其描述，并要求 LLM 生成一个 JSON 任务图：生成哪些任务、分配给谁以及它们之间的依赖关系。原始的待处理任务成为图中每个叶子节点的父任务，因此它会一直保持活动状态，直到整个图完成——然后提升回 `ready` 状态，以便其负责人（编排器配置文件）可以判断完成情况，并在工作未完成时添加更多任务。这就是“丢下一句话，然后走开”的工作流。

**手动** — `kanban.auto_decompose: false`。待处理任务会一直停留在待处理状态，直到你采取行动。点击卡片上的 **⚗ Decompose** 按钮，运行 `hermes kanban decompose <id>`（或 `--all`），或在聊天中使用 `/kanban decompose <id>`。这与看板在分解器之前的行为相匹配，适用于当你希望完全控制何时运行什么任务时。

你可以通过看板页面顶部的 **Orchestration: Auto/Manual** 药丸标签（翠绿色 = 自动，灰暗色 = 手动）在两种模式之间切换，或者直接编辑 `config.yaml`。两种模式都与 `hermes kanban specify` 共存——当你不想进行扇出分解时，它仍然可以作为单任务规范重写工具使用。

分解器的路由决策依赖于配置文件描述，这是每个配置文件的标签原语，你可以通过 `hermes profile create --description "..."`、`hermes profile describe <name> --text "..."`、`hermes profile describe <name> --auto`（根据配置文件已安装的技能和模型由 LLM 生成）或仪表板扩展的 **Orchestration settings** 面板中的每个配置文件编辑器来设置。没有描述的配置文件仍然会出现在列表中——它们可以通过名称路由，只是精度较低。分解器**永远不会**将子任务分配给 `assignee=None`：当 LLM 选择了一个未知的配置文件时，子任务会被路由到 `kanban.default_assignee`（如果未设置，则回退到当前活动的默认配置文件）。

配置选项（均在 `~/.hermes/config.yaml` 的 `kanban:` 下）：

| 键 | 默认值 | 用途 |
|---|---|---|
| `auto_decompose` | `true` | 调度器每个周期自动运行分解器。 |
| `auto_decompose_per_tick` | `3` | 每个调度器周期分解任务的上限。超出部分推迟到下一个周期。 |
| `orchestrator_profile` | `""` | 拥有分解权限的配置文件。为空时回退到当前活动的默认配置文件。 |
| `default_assignee` | `""` | 当 LLM 选择未知配置文件时，子任务的分配目标。为空时回退到当前活动的默认配置文件。 |
...
│  plugins/kanban/       │     与 CLI /kanban 命令相同        │
...
| 方法 | 路径 | 用途 |
|---|---|---|
| `GET` | `/board?tenant=<name>&include_archived=…` | 按状态列分组的完整看板，以及用于过滤器下拉列表的租户和负责人 |
...
| `POST` | `/tasks/bulk` | 对 `ids` 中的每个 id 应用相同的补丁（状态 / 归档 / 负责人 / 优先级）。每个 id 的失败会报告，但不会中止其他任务 |
| `POST` | `/tasks/:id/comments` | 追加评论 |
| `POST` | `/tasks/:id/specify` | 运行待处理任务规范器——辅助 LLM 充实任务正文并将其从 `triage` 提升到 `todo`。返回 `{ok, task_id, reason, new_title}`；`ok=false` 时附带人类可读的原因（如“不在待处理状态”/无辅助客户端/LLM 错误）返回 200，而不是 4xx |
| `POST` | `/tasks/:id/decompose` | 运行看板分解器——辅助 LLM 生成任务图，助手原子性地创建子任务 + 链接根任务 + 将状态从 `triage` 翻转为 `todo`。返回 `{ok, task_id, reason, fanout, child_ids, new_title}`。与 `/specify` 相同的 LLM 错误返回 200 的约定。 |
...
| `PUT` | `/orchestration` | 更新 `config.yaml` 中三个编排键的一个或多个。验证非空的配置文件名称是否实际存在。 |
...
每个处理器都是一个轻量级包装器——该插件大约有 700 行 Python 代码（路由 + WebSocket 尾部 + 批量批处理器 + 配置读取器），并且没有添加任何新的业务逻辑。一个微小的 `_conn()` 辅助函数会在每次读写时自动初始化 `kanban.db`，因此无论是用户先打开仪表板、直接访问 REST API，还是运行 `hermes kanban init`，新安装都能正常工作。
...
该插件使用标准的 Hermes 仪表板插件契约——完整的清单参考、shell 插槽、页面作用域插槽和 Plugin SDK，请参阅[扩展仪表板](./extending-the-dashboard)。额外的列、自定义卡片装饰、租户过滤的布局或完整的 `tab.override` 替换都可以在不分叉此插件的情况下实现。
...
hermes kanban specify [<id> | --all] [--tenant T]      # 将待办列中的想法充实为完整规范并提升为待办
        [--author NAME] [--json]
...
`--max-retries` 是调度器针对每个任务的熔断器覆盖设置。`--max-retries 1` 会在第一次非成功尝试后阻塞任务，而 `--max-retries 3` 允许两次重试，在第三次失败时阻塞。省略此参数则使用 `config.yaml` 中的 `kanban.failure_limit`，然后是内置默认值。

### 并发、调度和子任务提升配置

| 配置键 | 默认值 | 作用 |
|------------|---------|--------------|
| `kanban.max_in_progress` | 未设置（无限制） | 限制同时运行的任务数量。当看板上已有 N 个任务正在运行时，调度器会跳过生成更多任务 —— 这对于慢速工作器（本地 LLM、资源受限的主机）很有用，这样它们可以在更多任务堆积和超时之前完成现有任务。无效或低于 1 的值会记录警告并表现为无限制。 |
| `kanban.auto_promote_children` | `true` | 在 `decompose_triage_task()` 生成没有父任务阻塞依赖的子任务后，它们会自动提升到 `ready` 状态，以便调度器可以拾取它们。设置为 `false` 则需要手动审核 —— 子任务保持在 `todo` 状态，直到你提升它们。 |
...
当任务在之前的运行中遇到配额/认证/429 错误 (`blocker_auth`)，或在防护窗口内成功完成运行 (`recent_success`)，或最近的任务评论链接到 GitHub PR (`active_pr`) 时，调度器会拒绝重新生成就绪任务。这可以防止在人工跟进期间对同一错误或任务重复出现工作器风暴。参见[事件参考](#event-reference)中的 `respawn_guarded` 行。
...
`hermes kanban swarm` 一次性创建一个持久的 **Kanban Swarm v1** 图：一个已完成的根/白板卡片、N 个并行工作器卡片、一个依赖所有工作器的验证器卡片，以及一个依赖验证器的合成器卡片。共享的 Swarm 上下文（"白板"）作为结构化 JSON 评论存储在根卡片上，以便任何工作器都可以读取。
...
生成的图会正常调度 —— 工作器并行运行，验证器在所有工作器完成后唤醒，合成器在验证器标记工作无误后唤醒。
...
/kanban specify t_abcd                  # 将一句话分类任务扩展为完整规范
...
网关通常在 Agent 仍在思考时排队斜杠命令和用户消息 —— 这是为了防止你在第一个回合仍在进行时意外启动第二个回合。**`/kanban` 明确豁免于此防护。** 看板位于 `~/.hermes/kanban.db` 中，而不是在运行中 Agent 的状态中，因此读取操作（`list`、`show`、`context`、`tail`、`watch`、`stats`、`runs`）和写入操作（`comment`、`unblock`、`block`、`assign`、`archive`、`create`、`link`、…）都会立即执行，即使在回合进行中也是如此。

这种分离的整个意义在于：

- 一个工作器因等待对等方而阻塞 → 你从手机发送 `/kanban unblock t_abcd`，调度器会在下一个调度周期拾取该对等方。被阻塞的工作器不会被中断 —— 它只是不再被阻塞。
- 你发现一个需要人工上下文的卡片 → `/kanban comment t_xyz "use the 2026 schema, not 2025"` 会出现在任务线程中，该任务的*下一次*运行将在 `kanban_show()` 中读取它。
- 你想知道你的工作器群在做什么而不停止编排器 → `/kanban list --mine` 或 `/kanban stats` 会检查看板而不影响你的主对话。
### 在 `/kanban create` 时自动订阅（仅限消息网关）

当你通过消息网关使用 `/kanban create "…"` 创建任务时，发起聊天的会话（平台 + 聊天 ID + 线程 ID）会自动订阅该任务的终止事件（`completed`、`blocked`、`gave_up`、`crashed`、`timed_out`）。每个终止事件你都会收到一条消息——包括 `completed` 事件中 worker 结果摘要的第一行——无需轮询或记住任务 ID。

```
你> /kanban create "转录今天的播客" --assignee transcriber
机器人> 已创建 t_9fc1a3  (ready, assignee=transcriber)
...
消息网关平台有实际的消息长度限制。如果 `/kanban list`、`/kanban show` 或 `/kanban tail` 产生的输出超过约 3800 个字符，响应会被截断，并带有 `… (truncated; use \`hermes kanban …\` in your terminal for full output)` 的页脚。CLI 界面没有此类限制。

### 自动补全

在交互式 CLI 中，输入 `/kanban ` 并按 Tab 键会循环显示内置的子命令列表（`list`、`ls`、`show`、`create`、`assign`、`link`、`unlink`、`claim`、`comment`、`complete`、`block`、`unblock`、`archive`、`tail`、`dispatch`、`context`、`init`、`gc`）。上面 CLI 参考中列出的其余动词（`watch`、`stats`、`runs`、`log`、`assignees`、`heartbeat`、`notify-subscribe`、`notify-list`、`notify-unsubscribe`、`daemon`）也有效——只是它们尚未出现在自动补全提示列表中。
...
| **P3 投票 / 法定人数** | N 个同级任务 + 1 个聚合任务 | 3 个研究员 → 1 个审阅者挑选 |
...
运行记录也是 **结构化交接** 的存放位置。当一个 worker 完成任务时（通过 `kanban_complete(...)`），它可以传递：

- `summary`（工具参数）/ `--summary`（CLI）——人工交接；记录在运行记录上；下游子任务在其 `build_worker_context` 中能看到它。
- `metadata`（工具参数）/ `--metadata`（CLI）——运行记录上的自由格式 JSON 字典；子任务会看到它与摘要一起序列化。
- `result`（工具参数）/ `--result`（CLI）——记录在任务行上的简短日志行（遗留字段，为向后兼容而保留）。

下游子任务会读取每个父任务最近一次完成的运行记录的摘要 + 元数据。重试的 worker 会读取其自身任务的先前尝试（结果、摘要、错误），这样它们就不会重复已经失败的路径。
...
**状态变更回收的运行。** 如果您在仪表板上将一个正在运行的任务从 `running` 拖走（回到 `ready`，或直接到 `todo`），或者归档一个仍在运行的任务，正在进行的运行将以 `outcome='reclaimed'` 关闭，而不是被孤立。当 `tasks.current_run_id` 为 `NULL` 时，`task_runs` 行总是处于终止状态，反之亦然——这个不变量在 CLI、仪表板、调度器和通知器中都成立。

**从未被认领的完成任务的合成运行。** 完成或阻塞一个从未被认领的任务（例如，人类从仪表板上用摘要关闭一个 `ready` 任务，或者 CLI 用户运行 `hermes kanban complete <ready-task> --summary X`）否则会丢失交接信息。相反，内核会插入一个零持续时间的运行行（`started_at == ended_at`），携带摘要/元数据/原因，以便尝试历史保持完整。`completed` / `blocked` 事件的 `run_id` 指向该行。
...
`tasks` 上的两个可为空的列是为 v2 工作流路由保留的：`workflow_template_id`（此任务属于哪个模板）和 `current_step_key`（该模板中的哪个步骤处于活动状态）。v1 内核在路由时会忽略它们，但允许客户端写入它们，因此 v2 版本可以添加路由机制而无需另一次模式迁移。
...
| `stale` | `{elapsed_seconds, last_heartbeat_at, heartbeat_age_seconds, timeout_seconds, pid, terminated}` | 任务运行时间超过 `kanban.dispatch_stale_timeout_seconds`（默认 4 小时）*并且* 在过去一小时内没有收到 `kanban_heartbeat`。调度器 SIGTERM 主机本地的工作者（如果有），并将任务重置为 `ready` 以重新调度。*不* 增加失败计数器（陈旧是调度器侧的缺席检测，不是工作者故障）。运行长时间操作的工作者应至少每小时调用一次 `kanban_heartbeat` 以避免此情况。 |
| `respawn_guarded` | `{reason}` | 调度器拒绝在此刻重新生成此就绪任务。原因：`blocker_auth`（上次失败是配额/认证/429 错误——等待速率窗口重置）、`recent_success`（在过去一小时内发生了完成的运行——在重新运行前等待审查）、`active_pr`（最近的评论中出现了 GitHub PR URL——先前的工作者已经打开了 PR）。任务保持在 `ready`；下一次机会可以再次尝试生成。如果底层条件持续存在，正常的 `consecutive_failures` 断路器将在 `failure_limit` 次失败后通过 `gave_up` 自动阻塞。 |
| `spawn_failed` | `{error, failures}` | 一次生成尝试失败（缺少 PATH、工作空间无法挂载等）。计数器递增；任务返回 `ready` 重试。 |
| `protocol_violation` | `{pid, claimer, exit_code}` | 工作者在任务仍为 `running` 时成功退出，通常是因为它在未调用 `kanban_complete` 或 `kanban_block` 的情况下应答。调度器也会发出 `gave_up` 并立即自动阻塞，而不是重试。 |
| `gave_up` | `{failures, effective_limit, limit_source, error}` | 在 N 次连续非成功尝试后，断路器触发。任务自动阻塞并附带最后一个错误。有效限制解析为任务 `max_retries`，然后是调度器 `failure_limit` / `kanban.failure_limit`，最后是内置默认值。 |
`hermes kanban tail <id>` 显示单个任务的这些信息。`hermes kanban watch` 则在整个看板范围内流式输出。

## 不在范围内

Kanban 设计上是单主机的。`~/.hermes/kanban.db` 是一个本地的 SQLite 文件，并且调度器在同一台机器上生成工作进程。不支持在两个主机之间运行共享看板——没有用于“主机 A 上的工作进程 X，主机 B 上的工作进程 Y”的协调原语，并且崩溃检测路径假设 PID 是主机本地的。如果你需要多主机支持，请在每个主机上运行一个独立的看板，并使用 `delegate_task` / 消息队列来桥接它们。

...
默认的 `hermes-agent` 安装不包含 HTTP 栈或 PTY 助手 —— 这些是可选的额外组件。**Web 仪表盘**需要 FastAPI 和 Uvicorn（`web` 额外组件）。**Chat** 标签页还需要 `ptyprocess` 来在伪终端后生成嵌入式 TUI（POSIX 系统上的 `pty` 额外组件）。使用以下命令安装两者：
...
浏览和检查所有 Agent 会话。每一行显示会话标题、来源平台图标（CLI、Telegram、Discord、Slack、cron）、模型名称、消息数量、工具调用次数以及上次活跃时间。活跃会话会用一个脉动徽章标记。

- **搜索** — 使用 FTS5 在所有消息内容中进行全文搜索。结果会显示高亮片段，展开时会自动滚动到第一条匹配的消息。
- **展开** — 点击一个会话以加载其完整的消息历史记录。消息按角色（用户、助手、系统、工具）进行颜色编码，并以支持语法高亮的 Markdown 格式渲染。
...
:::warning 安全
Web 仪表板会读取和写入你的 `.env` 文件，该文件包含 API 密钥和机密信息。默认情况下，它绑定到 `127.0.0.1` —— 仅可从你的本地机器访问。如果你绑定到 `0.0.0.0`，你网络上的任何人都可以查看和修改你的凭据。仪表板本身没有身份验证机制。
:::
...
这将重新读取 `~/.hermes/.env` 到正在运行的进程环境中。当你通过仪表板添加了新的提供商密钥并希望立即使用时，这很有用。
...
前端使用 React 19、TypeScript、Tailwind CSS v4 和 shadcn/ui 风格的组件构建。生产构建输出到 `hermes_cli/web_dist/` 目录，由 FastAPI 服务器作为静态 SPA 提供服务。
...
**实时切换主题**：从标题栏——点击语言切换器旁边的调色板图标。选择会持久化到 `config.yaml` 中的 `dashboard.theme` 配置项，并在页面加载时恢复。

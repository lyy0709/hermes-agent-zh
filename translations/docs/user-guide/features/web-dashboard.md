...
`web` 额外组件会引入 FastAPI/Uvicorn；`pty` 会引入 `ptyprocess` (POSIX) 或 `pywinpty` (原生 Windows —— 请注意嵌入式 TUI 本身仍需要 WSL)。`pip install hermes-agent[all]` 包含这两个额外组件，如果您还想要消息/语音等功能，这是最简单的方法。
...
Hermes Desktop 通常启动自己的本地后端，但它也可以通过 **Settings → Gateway → Remote gateway** 连接到运行在远程机器（虚拟机、家庭实验室服务器等）上的仪表盘。这是“Desktop 显示后端已就绪但聊天功能从不工作”报告的最常见原因，因为 Desktop 的就绪检查验证的内容少于实时聊天连接实际需要的内容。

:::info 先决条件：远程主机上必须运行一个 `hermes dashboard`
Desktop 连接的“远程后端”**就是**运行在远程机器上的 `hermes dashboard` 进程 —— 即本文档记录的同一服务器。在以下任何步骤生效之前，它必须已启动并可访问；Desktop 是连接到它，而不是为您启动它。请将其保持在 `systemd`/`tmux`/等 下运行，以便在注销和重启后仍能存活。**消息网关** (Telegram/Discord/Slack/等) 是一个*独立的*长时间运行进程 —— 如果您依赖消息渠道，请独立启动它；它不是桌面应用程序连接的对象。
:::
Desktop 的“远程后端就绪”探针仅访问 `GET /api/status`，这是一个公共端点——只要*任何*仪表板在主机上运行，它就会响应。实时聊天连接是到 `/api/ws`（和 `/api/pty`）的**独立** WebSocket，该套接字还受两个状态探针从未触及的检查控制：
...
2.  **绑定主机必须允许客户端并匹配 Host 头。** 环回绑定（`127.0.0.1`）仅接受环回客户端，因此无论凭据如何，远程机器都会在套接字层被拒绝。绑定到非环回地址（`--host 0.0.0.0`），以便对等 IP 守卫允许远程客户端通过。您在 Desktop 中输入的远程 URL 必须通过仪表板绑定的同一主机访问——DNS 重绑定守卫要求 Host 头匹配。
...
如果 `/api/status` 显示门已开启且带有 `"basic"` 提供商，但 Desktop 登录后*仍然*无法连接，则问题已超出基本设置范围——获取新的 `desktop.log`（设置 → 消息网关 → 打开日志）以及同一重试窗口内的仪表板日志，并查找 `/api/ws` 关闭代码（4403 = 聊天 WS 被请求守卫拒绝，例如 Host/对等不匹配；4401 = WS 票据未通过身份验证）。

### 配置

`config.yaml` 的基于表单的编辑器。所有 150 多个配置字段都从 `DEFAULT_CONFIG` 自动发现，并组织到选项卡式类别中：

![配置管理页面——左侧为部分过滤器，右侧为自动发现的字段](/img/dashboard/admin-config.png)
...
- **统计** — 摘要栏显示总会话数、存储中活动会话数、存档数、总消息数以及按来源的细分。
- **展开** — 点击会话以加载其完整消息历史记录。消息按角色（用户、助手、系统、工具）进行颜色编码，并呈现为带有语法高亮的 Markdown。
...
- **编辑** — 打开预填模态框以更改任务的提示词、计划、名称或交付目标
- **立即触发** — 在正常计划之外立即执行任务
- **删除** — 永久删除定时任务

### 技能

浏览、搜索和切换已安装的技能和工具集，并从中心安装新的技能和工具集。技能从 `~/.hermes/skills/` 加载并按类别分组。
...
- **启用 / 禁用** — 启用或禁用订阅。禁用的路由保留在订阅文件中，但网关会拒绝其传入事件（403）。网关会热重载该文件，因此更改在下一个事件时生效 — 无需重启。
...
- **配置** — 打开每个平台的表单，包含该频道所需的确切字段（机器人令牌、应用令牌、服务器 URL、允许列表等）。密钥显示为密码输入框并存储为隐藏值；留空字段将保留现有值。必填字段已标记并经过验证。“设置指南”链接指向该平台的凭证文档。
- **启用 / 禁用** — 启用或禁用频道。凭证保留在磁盘上；仅活动状态发生变化。
...
- **主机** — 实时系统状态：操作系统/内核、架构、主机名、Python 和 Hermes 版本、CPU 核心数 + 利用率、内存、Hermes 主目录的磁盘使用情况、运行时间和平均负载。（CPU/内存/磁盘信息在安装了 `psutil` 时提供；身份字段始终显示。）Hermes 版本显示一个**更新状态徽章**（最新 / 落后 N 个提交）和一个**检查更新**按钮。当 git 或 pip 安装有可用更新时，一个**立即更新**按钮会打开一个确认对话框 — 显示你将拉取多少个提交 — 然后在后台运行 `hermes update`。在 Docker/Nix/Homebrew 安装中，仪表板无法就地应用更新，因此会显示正确的带外命令。
...
- **凭证池** — 添加和移除 Agent 轮询使用的轮换 API 密钥（按提供商）。列表中的密钥会被脱敏；原始值仅由 Agent 获取。
...
- **Shell 钩子** — 列出已配置的钩子及其同意状态 + 可执行状态，**创建**一个钩子（事件、命令、匹配器、超时，带有选择加入的同意授予），以及移除一个钩子。钩子运行任意命令，因此创建表单带有安全警告，并且钩子仅在同意授予后才会触发。
...
:::warning 安全
Web 仪表板读取和写入你的 `.env` 文件，该文件包含 API 密钥和机密信息。它默认绑定到 `127.0.0.1` — 仅可从本地机器访问。如果你绑定到 `0.0.0.0`，你网络上的任何人都可以查看和修改你的凭证。仪表板本身没有身份验证。
:::
...
| `DELETE /api/api/credentials/pool/{provider}/{index}` | 移除一个密钥（索引从 1 开始） |
| `GET /api/memory` | 当前激活的提供商 + 可用提供商 + 内置文件大小 |
...
| `GET /api/hermes/update/check` | 报告更新可用性（落后提交数、安装方法），但不应用更新。对于落后的 git/pip 安装，还会返回一个 `commits` 列表（`sha`、`summary`、`author`、`at`），显示已更改的内容。`?force=1` 可强制刷新 6 小时缓存 |
...
当仪表板绑定到公共或非环回地址（即除 `127.0.0.1` / `localhost` 之外的任何地址）时，Hermes Agent 会启用身份验证门。每个请求都必须携带经过验证的会话 cookie，否则将被重定向到登录页面。系统内置了三种提供商：

- **[用户名/密码](#usernamepassword-provider-no-oauth-idp)** — 为自托管/本地部署/家庭实验室仪表板添加身份验证的最简单方式。无需外部身份提供商。**仅限在受信任的网络或 VPN 后使用 — 不适用于公开暴露到互联网。**
...
- **[自托管 OIDC](#self-hosted-oidc-provider)** — 用于通过标准 OpenID Connect 引入您自己的身份提供商（Keycloak、Auth0、Okta、通过 OIDC 桥接的 Google、GitHub 等）。不涉及 Nous Portal；适用于前端有符合标准的 OIDC 服务器时的公开互联网暴露。
...
`--insecure` 会跳过门控，并提供一个未经身份验证的仪表板，该仪表板可以读取/写入您的 `.env`（API 密钥、机密信息）并可以运行 Agent 命令。**请勿将其用于远程连接。** 要将仪表板暴露给另一台机器，请配置[用户名/密码提供商](#usernamepassword-provider-no-oauth-idp)（或 OAuth）并保持 `--insecure` 关闭。此标志仅作为在完全受信任、有防火墙的单主机网络上的最后逃生手段。
:::
### 故障关闭语义
...
因为每次登录都会通过 Nous Portal 验证并受您的 Nous 账户保护，**所以 Nous 提供商适合向公共互联网公开仪表盘。**

#### 注册仪表盘

要使用 Nous 提供商，您需要一个 OAuth 客户端 ID（格式为 `agent:{id}`）。有两种获取方式：

- **CLI — `hermes dashboard register`。** 在仪表盘所在的主机上运行。它会解析您现有的 Nous 登录（如果尚未登录，请先运行 `hermes setup`），向 Portal 注册一个自托管的 OAuth 客户端，并将 `HERMES_DASHBOARD_OAUTH_CLIENT_ID` 写入 `~/.hermes/.env`。可选标志：`--name`（人类可读的标签，否则自动生成）和 `--redirect-uri`（面向互联网主机的公共 HTTPS 回调 URL）。
...
根据 Hermes Agent 的约定（`~/.hermes/.env` 仅用于 API 密钥/机密），**对于本地开发、内部部署以及任何您直接控制的部署，`config.yaml` 是设置这些值的推荐位置**。环境变量路径的存在，是为了让托管平台的密钥注入能够推送每次部署的 `client_id`，而无需任何人在镜像内编辑 `config.yaml` —— 这是其主要目的。
...
```
拒绝将仪表盘绑定到 0.0.0.0 — OAuth 认证网关在非环回绑定时启用，但未注册任何认证提供商。
...
**3. 登录。** 打开 `http://<主机>:9119/`，您将被重定向到 `/login`。点击**使用 Nous Research 登录** → 在 Portal 进行身份验证 → 返回已认证的仪表盘。从任何机器验证网关：
...
然后 `GET /api/auth/me` 会返回已验证的会话（`provider: nous`）。对于面向互联网的主机，使用 `--redirect-uri https://hermes.example.com/auth/callback` 进行注册，并设置 `HERMES_DASHBOARD_PUBLIC_URL`，以便 OAuth 回调解析到您的公共 URL（参见[公共 URL 覆盖](#public-url-override)）。
...
它插入到与 OAuth 提供商相同的网关中：网关在非环回绑定且不带 `--insecure` 时启用，登录页面为此提供商呈现凭据表单（而不是“使用 X 登录”按钮），并且登录后的一切 —— 会话 Cookie、透明刷新、WS 票据、注销、审计日志 —— 都与 OAuth 路径相同。会话是提供商自己生成的无状态 HMAC 签名 Token，因此**没有数据库，也没有外部 IDP**。密码哈希使用标准库 `scrypt`（无第三方依赖）。
:::warning 仅限在受信任的网络中使用——不适用于公共互联网
用户名/密码提供商适用于在**受信任的网络**中或仅通过**VPN**访问的自托管/本地/家庭实验室仪表板。它保护的是单个共享凭证，背后没有外部身份提供商、MFA 或每用户账户，因此**不适合将仪表板直接暴露在公共互联网上**。对于面向互联网的仪表板，请改用 [Nous Research 提供商](#default-provider-nous-research)（或您自己的[自托管 OIDC](#self-hosted-oidc-provider) / [自定义 OAuth](#custom-providers) 提供商）。
:::

#### 配置

与 Nous 提供商类似，它从 `config.yaml`（规范配置）读取，当环境变量设置为非空时，环境变量优先。仅当配置了 `username` 加上 `password_hash`（首选）或 `password` 时才会激活——否则它不执行任何操作，因此 OAuth 用户和环回/`--insecure` 操作者不受影响。
...
dashboard:
  basic_auth:
    ...
    # ...或者使用明文密码（加载时在内存中哈希化；静态存储时安全性较低）：
    # password: "s3cret"
...
| `HERMES_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | 明文；**优先于配置文件中的 `password_hash`**，因此您可以通过环境变量轮换 |
...
当 `secret` 为空时，会生成一个随机的每进程签名密钥。这对于单个进程来说没问题，但意味着**每次重启都会使所有会话失效**，并且会话**无法跨多个工作进程**。对于需要重启后保持/多工作进程部署，请设置一个明确的 `secret`。
:::
...
只需三步，即可从零开始在受信任的网络上建立一个密码保护的仪表板。

**1. 在 `~/.hermes/.env` 中设置凭证。** 对密码进行哈希处理，这样就不会有明文静态存储，并设置一个稳定的签名密钥，以便会话在重启后得以保留：
...
**2. 在可访问的地址上运行仪表板。** 非环回绑定且不使用 `--insecure` 选项会启用网关，而用户名和哈希值会激活 `basic` 提供商：

```bash
hermes dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. 登录。** 打开 `http://<host>:9119/`，您将被重定向到 `/login` —— 一个**凭证表单**（而不是“使用 X 登录”按钮）。输入 `admin` / 您的密码 → 进入已认证的仪表板。从任何机器验证网关：
...
`GET /api/auth/me` 随后会返回已验证的会话（`provider: basic`）。请将其置于 VPN 之后——参见上面的警告；对于公共主机，请改用 [Nous Research](#default-provider-nous-research) 或[自托管 OIDC](#self-hosted-oidc-provider) 提供商。

#### 编写您自己的密码提供商

`basic` 只是一个扩展点的实现。任何插件都可以注册一个密码提供商：在您的 `DashboardAuthProvider` 子类上设置 `supports_password = True`，并实现 `complete_password_login(*, username, password) -> Session`（拒绝时抛出 `InvalidCredentialsError`，如果您的后端存储宕机则抛出 `ProviderError`）。对于纯密码提供商，OAuth 的 `start_login` / `complete_login` 方法可以留作 `NotImplementedError` 存根。这是用于 LDAP 绑定、凭证数据库或任何其他非重定向认证方案的途径——框架会为您处理表单、路由、Cookie 和刷新。

### 自托管 OIDC 提供商

如果您运行自己的身份提供商，捆绑的 `plugins/dashboard_auth/self_hosted` 插件会使用**标准 OpenID Connect** 对仪表板进行身份验证——无需针对每个 IDP 编写代码，也不涉及 Nous Portal。它已针对任何符合规范的 OIDC 服务器进行验证并与之兼容：

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

与 Nous 提供商类似，它会自动加载，并且仅在配置完成后注册自身，因此对于环回/`--insecure` 仪表板，它不执行任何操作。
...
```
**环境变量** — 操作员覆盖（当设置为非空时，环境变量优先于 `config.yaml`；空值视为未设置）：
...
在你的 IDP 中，注册一个使用授权码 + PKCE (S256) 授权的**公共**应用程序/客户端，并将仪表板的回调 URL 添加为允许的重定向 URI。回调 URL 是 `<dashboard public URL>/auth/callback`（关于仪表板在代理后如何推导其公共 URL，请参见[公共 URL 覆盖](#public-url-override)）。
...
ID Token 是建立身份的依据 — 访问令牌被视为不透明的（OIDC 规范不要求它必须是 JWT）。端点 URL 必须使用 HTTPS（本地开发 IDP 允许使用环回 `http://`），并且发现文档中公布的 `issuer` 必须与你配置的匹配（允许存在尾部斜杠的差异）。当 IDP 颁发刷新令牌时，将通过标准的 `refresh_token` 授权用于静默重新认证；登出时会调用 IDP 的 RFC 7009 `revocation_endpoint`（如果已公布）。

> **机密客户端**（那些带有 `client_secret` 的）目前不支持 — 请配置一个公共 + PKCE 客户端，这是面向浏览器的仪表板的典型选择。

#### 操作示例：Keycloak

[Keycloak](https://www.keycloak.org/) 是最容易搭建用于本地测试的自托管 OIDC 服务器之一 — 它在开发模式下（内存数据库）作为单个容器运行，并暴露标准的 OIDC 发现。本指南将帮助你在几分钟内从零开始实现可用的仪表板登录。
...
启动后，该领域在 `http://localhost:8080/realms/hermes/.well-known/openid-configuration`（颁发者 `http://localhost:8080/realms/hermes`）公布标准的 OIDC 发现。管理控制台位于 `http://localhost:8080/` (`admin` / `admin`)。

**2. 将仪表板指向它。** 自托管插件允许环回 `http://` 颁发者（任何非环回颁发者都需要 HTTPS），因此本地的 Keycloak 可以直接使用：
...
`HERMES_DASHBOARD_PUBLIC_URL` 告诉仪表板其 OAuth 回调 URL 是 `http://localhost:9119/auth/callback` — 这是上面领域注册的重定向 URI。绑定到 `0.0.0.0`（非环回绑定）且不使用 `--insecure` 会启用 OAuth 网关。
...
> 如果你在不同的主机/端口上绑定或浏览，请将该来源的 `…/auth/callback` 添加到 Keycloak 管理控制台（Clients → hermes-dashboard → Settings）中客户端的**有效重定向 URI**。相同的模式适用于 Authentik、Zitadel、Authelia 和其他 OIDC 服务器 — 只有颁发者 URL 和客户端注册界面不同。

### 公共 URL 覆盖

默认情况下，仪表板从请求中重构 OAuth 回调 URL — `X-Forwarded-Host` + `X-Forwarded-Proto` + `X-Forwarded-Prefix`（当 uvicorn 配置了 `proxy_headers=True` 时，`start_server` 在网关下启用此配置）。这在反向代理正确设置了所有三个标头的情况下可以开箱即用。

对于部署在不能可靠转发这些标头的反向代理后面的情况（手动 nginx 设置、本地入口、具有部分代理链的自定义域名部署），请将 `dashboard.public_url`（或 `HERMES_DASHBOARD_PUBLIC_URL`）设置为仪表板被访问的**完整公共 URL**：
```yaml
dashboard:
...
> **注意：** `public_url` 仅覆盖 OAuth 回调 URL。`Secure` cookie 标志仍由 `request.url.scheme` 控制（在 `proxy_headers` 下遵循 `X-Forwarded-Proto`），因此在 TLS 终止的公共部署上使用 `http://` 的 `public_url` 将产生非 Secure cookie。这是操作员容易犯的错误 —— 请将 `public_url` 与上游正确的 TLS 终止配对使用。
...
5.  服务器在 `POST /api/oauth/token` 处用 code 换取访问令牌，根据 Portal 的 JWKS (`/.well-known/jwks.json`) 验证 JWT 签名，并设置 `hermes_session_at` cookie。
...
| 名称 | 生命周期 | 说明 |
|------|----------|-------|
| `hermes_session_at` | 令牌 TTL (15 分钟) | HttpOnly, SameSite=Lax, 在 HTTPS 下为 Secure |
| `hermes_session_pkce` | 10 分钟 | HttpOnly；在往返过程中保存 PKCE 验证器和提供商提示 |
| `hermes_session_rt` | v1 中未使用 | 为向前兼容保留；当 `refresh_token` 为空时不写入 |

所有三个 cookie 都是 `Path=/` 和 `SameSite=Lax`。`Secure` 标志在通过 HTTPS 访问仪表板时设置（通过请求 URL 方案检测 —— 在 `proxy_headers=True` 下遵循来自上游 TLS 终结器的 `X-Forwarded-Proto`）。
...
Hermes Desktop 可以驱动运行在另一台机器上的 Hermes 后端（VPS、家庭服务器、Tailscale 后面的 Mini）。在应用中，这位于 **Settings → Gateway → Remote gateway** 下，它要求提供 **Remote URL** 和 **Sign in** 的方式。（关于桌面应用本身 —— 安装、设置、聊天 —— 请参阅 [Hermes Desktop](/user-guide/desktop) 页面。）

您可以使用捆绑的认证提供商之一来保护远程仪表板，桌面应用会根据后端广告的提供商进行登录。对于您自己机器之外可访问的后端 —— VPS、公共主机、任何面向互联网的设备 —— 推荐的提供商是 **OAuth (Nous Portal)**（使用 [`hermes dashboard register`](#registering-a-dashboard) 注册，并使用 *Sign in with Nous Research* 登录）。捆绑的 [用户名/密码提供商](#usernamepassword-provider-no-oauth-idp) 是当后端位于受信任的 LAN 或仅通过 VPN 可访问时的最快选项，但**不适合直接暴露在公共互联网上**。将仪表板绑定到非环回地址会启用其认证网关；登录后，Desktop 会自动为聊天 WebSocket 重用会话 —— 无需复制或粘贴令牌。
以下配方使用用户名/密码路径，因为这是在受信任网络上最快搭建的方式；如需 OAuth 路径，请参阅[默认提供商：Nous Research](#default-provider-nous-research)。

### 在后端（远程机器上）

```bash
# 1. 在 ~/.hermes/.env（密钥文件，0600）中设置仪表板登录凭据。
cat >> ~/.hermes/.env <<'EOF'
HERMES_DASHBOARD_BASIC_AUTH_USERNAME=admin
HERMES_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
# 推荐：一个稳定的签名密钥，以便会话在重启后得以保留。
HERMES_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.hermes/.env
...
如果将仪表板作为 systemd 服务运行，当单元文件包含 `EnvironmentFile=%h/.hermes/.env` 时，`~/.hermes/.env` 会自动被加载，因此凭据在启动时就在环境中。
...
除了应用内设置，你还可以在启动桌面应用之前通过环境变量将其指向后端。当设置了 `HERMES_DESKTOP_REMOTE_URL` 时，它会覆盖应用内保存的 URL（消息网关设置面板会显示一个 "env override" 徽章并禁用编辑）；你仍然需要从面板使用你的用户名和密码**登录**。
...
- **没有"登录"按钮——而是要求会话 Token** — 用户名/密码提供商未激活（`/api/status` 不会列出 `"basic"`）。请确保用户名和密码（或密码哈希）已设置，并且仪表板进程已加载它们。
- **每次重启后都退出登录** — 将 `HERMES_DASHBOARD_BASIC_AUTH_SECRET` 设置为一个稳定的值；否则签名密钥会在每次启动时重新生成。
- **连接被拒绝 / 超时** — 后端绑定到了 `127.0.0.1`（默认值）而不是可访问的地址，或者防火墙/VPN 阻止了该端口。请绑定到 `0.0.0.0` 或 tailscale IP，并向你的受信任网络开放该端口。
...
仪表板内置了六个主题，并可以通过用户定义的主题、插件标签页和后端 API 路由进行扩展——都是即插即用的，无需克隆仓库。

**实时切换主题**：从标题栏——点击语言切换器旁边的调色板图标。选择会持久化到 `config.yaml` 中的 `dashboard.theme` 下，并在页面加载时恢复。

**独立更改字体**：使用同一个选择器——主题列表下方的**字体**部分会覆盖当前活动主题的 UI 字体。该选择在主题切换之间保持持久（`config.yaml` → `dashboard.font`）；选择**主题默认**以清除它并返回活动主题自身的字体。

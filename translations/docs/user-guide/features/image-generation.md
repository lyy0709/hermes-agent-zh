---
title: 图像生成
description: 使用 FLUX 2 Pro 生成高质量图像，并通过 FAL.ai 自动进行超分辨率放大。
sidebar_label: 图像生成
sidebar_position: 6
---
...
2.  **超分辨率放大** — 生成的图像会自动使用 Clarity Upscaler (`fal-ai/clarity-upscaler`) 进行 2 倍放大
3.  **交付** — 返回放大后的图像 URL
...
```
创作一幅栖息在古老树枝上的睿智老猫头鹰的肖像画
```

```
为我制作一幅带有飞行汽车和霓虹灯光的未来主义城市景观
```
...
超分辨率放大器在保持原始构图的同时增强了细节和分辨率。如果超分辨率放大器失败（网络问题、速率限制），则会自动返回原始分辨率的图像。
...
调试日志会保存到 `./logs/image_tools_debug_<session_id>.json`，其中包含每个生成请求、参数、时间以及任何错误的详细信息。

...
当遇到缺失的值时，Hermes 仅在技能实际在本地 CLI 中加载时才会安全地询问。你可以跳过设置并继续使用该技能。消息界面永远不会在聊天中询问密钥 —— 它们会告诉你在本地使用 `hermes setup` 或 `~/.hermes/.env` 来设置。
...
设置存储在您 config.yaml 中的 `skills.config` 下。`hermes config migrate` 会提示未配置的设置，而 `hermes config show` 会显示它们。当技能加载时，其解析后的配置值会被注入到上下文中，以便 Agent 自动知晓配置值。
...
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | 可通过 `hermes skills search <查询> --source skills-sh` 搜索。当 skills.sh 的别名与仓库文件夹名不同时，Hermes 会解析别名风格的技能。 |
...
| `github` | `openai/skills/k8s` | 直接通过 GitHub 仓库/路径安装和自定义 taps。 |
...
:::tip GitHub 速率限制
技能中心操作使用 GitHub API，未认证用户的速率限制为 60 次请求/小时。如果在安装或搜索时看到速率限制错误，请在 `.env` 文件中设置 `GITHUB_TOKEN`，以将限制提高到 5,000 次请求/小时。发生此情况时，错误消息会包含可操作的提示。
:::

...
捆绑的 `plan` 技能是一个很好的例子。运行 `/plan [请求]` 会加载该技能的指令，告诉 Hermes 在需要时检查上下文，编写一个 Markdown 实施计划而不是执行任务，并将结果保存在相对于活动工作空间/后端工作目录的 `.hermes/plans/` 目录下。
...
当设置此字段时，技能在不兼容的平台上会自动从系统提示词、`skills_list()` 和斜杠命令中隐藏。如果省略，技能将在所有平台上加载。
...
该指令在交付前会被剥离，因此用户永远不会看到它。粒度设计上，每个响应是“全有或全无”的：发出一次 `[[as_document]]`，同一响应中的每个图像路径都将作为文档交付。这与 `[[audio_as_voice]]` 的作用范围一致。

...
所有技能都位于 **`~/.hermes/skills/`** —— 这是主目录和唯一可信源。在全新安装时，捆绑的技能会从代码仓库复制过来。从中心安装的和由 Agent 创建的技能也会放在这里。Agent 可以修改或删除任何技能。
...
捆绑的 `plan` 技能是一个很好的例子。运行 `/plan [请求]` 会加载该技能的指令，告诉 Hermes 在需要时检查上下文，编写一个 Markdown 实现计划而不是执行任务，并将结果保存在相对于活动工作空间/后端工作目录的 `.hermes/plans/` 下。
...
设置后，该技能在不兼容的平台上会自动从系统提示词、`skills_list()` 和斜杠命令中隐藏。如果省略，则技能在所有平台上加载。
...
设置存储在您 config.yaml 中的 `skills.config` 下。`hermes config migrate` 会提示未配置的设置，而 `hermes config show` 会显示它们。当技能加载时，其解析后的配置值会被注入到上下文中，以便 Agent 自动了解配置值。
...
技能包是微小的 YAML 文件，将多个技能分组在一个斜杠命令下。当您运行 `/<bundle-name>` 时，包中列出的每个技能都会同时加载——当特定任务总是受益于同一组技能一起使用时，这非常有用。
...
技能包位于 **`~/.hermes/skill-bundles/<slug>.yaml`** 中，看起来像这样：
...
- `instruction`（可选）——附加到加载的技能内容前的额外指导。对于规范“我们如何总是将这些技能一起使用”很有用。
...
- **当 slug 冲突时，技能包优先于单个技能**。如果您将一个技能包命名为 `research`，并且您还有一个名为 `research` 的技能，则 `/research` 会调用技能包。这是有意为之——您通过命名它选择了使用技能包。
- **缺失的技能会被跳过，而不是致命错误**。如果一个技能包列出了 `skill-foo` 而您尚未安装它，该技能包仍然会加载能够解析的技能，并且 Agent 会收到一个列出被跳过内容的注释。
- **技能包在所有界面都有效**——交互式 CLI、TUI、仪表板聊天以及每个消息网关平台（Telegram、Discord、Slack……）——因为调度与单个技能命令一样集中处理。
- **技能包不会使提示词缓存失效**。它们在调用时生成一个新的用户消息，与 `/<skill-name>` 的方式相同——没有系统提示词变更。
### 何时使用技能包优于手动安装每个技能

在以下情况下使用技能包：
- 你总是为重复性任务搭配相同的技能（`/backend-dev`、`/release-prep`、`/incident-response`）。
- 你希望获得比连续输入多个 `/skill` 调用更简短的心智模型（少一个字符）。
- 你希望通过将技能包 YAML 文件提交到共享的 dotfiles 仓库，并符号链接到 `~/.hermes/skill-bundles/` 来分发团队范围的“任务配置文件”。

技能包只是一个 YAML 别名——它不会为你安装技能。技能本身必须已经存在（在 `~/.hermes/skills/` 或外部技能目录中）。否则，技能包调用将直接跳过缺失的技能。

## Agent 管理的技能（skill_manage 工具）

Agent 可以通过 `skill_manage` 工具创建、更新和删除自己的技能。这是 Agent 的**程序性记忆**——当它发现一个非平凡的工作流时，它会将方法保存为技能以供将来重用。
...
这是一种基于 URL 的发现机制，来自发布 `/.well-known/skills/index.json` 的网站。它不是一个单一的集中式中心，而是一种网络发现约定。
...
Hermes 可以从 LobeHub 的公共目录中搜索并转换 Agent 条目为可安装的 Hermes 技能。
...
直接从任何 HTTP(S) URL 安装单个文件 `SKILL.md` —— 当作者在自己的网站上托管技能时很有用（没有中心列表，无需输入 GitHub 路径）。Hermes 会获取 URL，解析 YAML frontmatter，进行安全扫描，然后安装。

- Hermes 源 ID：`url`
- 标识符：URL 本身（无需前缀）
- 范围：**仅限单个文件 `SKILL.md`**。带有 `references/` 或 `scripts/` 的多文件技能需要一个清单，应通过上述其他源之一发布。
...
2. 来自 URL 路径的父目录名称（例如 `.../my-skill/SKILL.md` → `my-skill`，或 `.../my-skill.md` → `my-skill`），当它是一个有效的标识符时（`^[a-z][a-z0-9_-]*$`）。
3. 在具有 TTY 的终端上进行交互式提示。
4. 在非交互式界面（TUI 内的 `/skills install` 斜杠命令、消息网关平台、脚本）上，会显示一个指向 `--name` 覆盖选项的清晰错误。

```bash
# Frontmatter 没有名称且 URL 段无帮助 —— 提供一个：
...
信任级别始终为 `community` —— 运行与其他所有源相同的安全扫描。URL 被存储为安装标识符，因此当您想要刷新时，`hermes skills update` 会自动从同一 URL 重新获取。

### 安全扫描和 `--force`

所有从中心安装的技能都会经过一个**安全扫描器**，检查数据泄露、提示词注入、破坏性命令、供应链信号和其他威胁。
...
当您已审查过第三方技能并希望覆盖非危险策略阻止时，使用 `--force`：
...
| `trusted` | 受信任的注册表/仓库，如 `openai/skills`、`anthropics/skills`、`huggingface/skills` | 比社区源更宽松的策略 |
| `community` | 其他所有内容 (`skills.sh`、知名端点、自定义 GitHub 仓库、大多数市场) | 非危险发现可以用 `--force` 覆盖；`dangerous` 判定保持阻止 |
...
:::tip GitHub 速率限制
技能中心操作使用 GitHub API，对于未经身份验证的用户，其速率限制为每小时 60 个请求。如果您在安装或搜索时看到速率限制错误，请在您的 `.env` 文件中设置 `GITHUB_TOKEN`，以将限制提高到每小时 5,000 个请求。发生这种情况时，错误消息会包含一个可行的提示。
:::
### 发布自定义技能源

如果你想分享一组精选的技能——给你的团队、组织或公开分享——你可以将它们发布为一个**源**：一个其他 Hermes 用户可以通过 `hermes skills tap add <owner/repo>` 添加的 GitHub 仓库。无需服务器，无需注册中心，无需发布流水线。只是一个包含 `SKILL.md` 文件的目录。

#### 仓库布局

一个源可以是任何 GitHub 仓库（公开或私有——私有仓库需要 `GITHUB_TOKEN`），布局如下：
...
- 像 `references/`、`templates/`、`scripts/`、`assets/` 这样的子目录会在安装时与 `SKILL.md` 一起下载。
- 目录名称以 `.` 或 `_` 开头的技能将被忽略。

Hermes 通过列出源路径下的每个子目录并探测每个目录中的 `SKILL.md` 来发现技能。
...
如果你的技能不在 `skills/` 目录下（常见于向现有项目添加 `skills/` 子树时），请编辑 `~/.hermes/.hub/taps.json` 中的源条目：
...
`hermes skills tap add` CLI 默认将新源的 `path` 设置为 `"skills/"`；如果需要不同的路径，请直接编辑该文件。`hermes skills tap list` 会显示每个源的有效路径。

#### 直接安装单个技能（无需添加源）

用户也可以从任何公开的 GitHub 仓库安装单个技能，而无需将整个仓库添加为源：

```bash
hermes skills install owner/repo/skills/my-workflow
```

当你想分享一个技能，而不要求用户订阅你的整个注册中心时，这很有用。

#### 源的信任级别

新源默认被分配为 `community` 信任级别。从这些源安装的技能会经过标准的安全扫描，并在首次安装时显示第三方警告面板。如果你的组织或一个广泛受信任的来源应该获得更高的信任级别，请将其仓库添加到 `tools/skills_hub.py` 中的 `TRUSTED_REPOS`（需要提交 Hermes 核心 PR）。

#### 源管理

```bash
hermes skills tap list                                # 显示所有已配置的源
...
源存储在 `~/.hermes/.hub/taps.json` 中（按需创建）。

## 捆绑技能更新 (`hermes skills reset`)

Hermes 在仓库内的 `skills/` 目录中附带了一组捆绑技能。在安装时和每次 `hermes update` 时，同步过程会将这些技能复制到 `~/.hermes/skills/` 中，并在 `~/.hermes/skills/.bundled_manifest` 处记录一个清单，将每个技能名称映射到同步时的内容哈希（**原始哈希**）。
...
这种保护机制很好，但有一个尖锐的边缘情况。如果你编辑了一个捆绑技能，然后后来想放弃你的更改，通过从 `~/.hermes/hermes-agent/skills/` 复制粘贴回捆绑版本，清单仍然保存着*旧的*原始哈希（来自上次成功同步运行的时间）。你新复制粘贴的内容（当前的捆绑哈希）将与该过时的原始哈希不匹配，因此同步会继续将其标记为用户已修改。
...
/skills list
```
官方可选技能仍使用标识符，如 `official/security/1password` 和 `official/migration/openclaw-migration`。
...
- **WANDB_API_KEY** — 用于 [Weights & Biases](https://wandb.ai/) 指标跟踪的 API 密钥
...
Agent 调用 `rl_list_environments()`，该方法通过 AST 解析扫描 `tinker-atropos/tinker_atropos/environments/` 目录，查找继承自 `BaseEnv` 的 Python 类。每个环境定义：
...
**可配置字段**（可以修改）：
- `group_size` — 每个数据项的完成次数（默认值：16）
- `batch_size` — 训练批次大小（默认值：128）
...
3. 生成三个进程：
   ...
   - **Tinker 训练器**（`launch_training.py`）— LoRA 训练 + 在端口 8001 上的 FastAPI 推理服务器
...
在提交完整的训练运行之前，您可以使用 `rl_test_inference` 测试环境是否正常工作。这使用 OpenRouter 运行几步推理和评分——不需要 Tinker API，只需要一个 `OPENROUTER_API_KEY`。
...
训练循环：
1. 从 Atropos 获取一批 rollout（提示词 + 完成 + 分数）

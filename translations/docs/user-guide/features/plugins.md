...
位于 `./.hermes/plugins/` 下的项目本地插件默认是禁用的。只有在可信的仓库中，通过在启动 Hermes 前设置 `HERMES_ENABLE_PROJECT_PLUGINS=true` 才能启用它们。
...
plugins:
  enabled:
    - my-tool-plugin
    - disk-cleanup
  disabled:       # 可选的拒绝列表 — 如果名称同时出现在两者中，此列表始终优先
    - noisy-plugin
```
...
在 `hermes plugins install owner/repo` 之后，系统会询问 `Enable 'name' now? [y/N]` — 默认值为否。对于脚本化安装，可以使用 `--enable` 或 `--no-enable` 跳过提示。

### 现有用户的迁移

当你升级到支持手动启用插件的 Hermes 版本（配置模式 v21+）时，任何已安装在 `~/.hermes/plugins/` 下且未在 `plugins.disabled` 中的用户插件，将**自动被保留**在 `plugins.enabled` 中。你现有的设置将继续工作。捆绑插件**不会**被保留 — 即使是现有用户也必须明确选择启用。
...
| [`pre_llm_call`](/docs/user-guide/features/hooks#pre_llm_call) | 每回合一次，在 LLM 循环之前 — 可以返回 `{"context": "..."}` 以 [将上下文注入用户消息](/docs/user-guide/features/hooks#pre_llm_call) |
| [`post_llm_call`](/docs/user-guide/features/hooks#post_llm_call) | 每回合一次，在 LLM 循环之后（仅限成功回合） |
| [`on_session_start`](/docs/user-guide/features/hooks#on_session_start) | 新会话创建时（仅限第一回合） |
...
记忆提供商和上下文引擎是**提供商插件** — 每种类型一次只能有一个处于活动状态。通用插件可以以任意组合启用。
...
- **提供商插件部分** — 显示当前选择。按 ENTER 键进入单选选择器，在那里选择一个活动提供商。
- 捆绑插件出现在同一列表中，并带有 `[bundled]` 标签。
...
- 如果 Agent 处于**空闲**状态（等待用户输入），消息将作为下一个输入排队并开始新回合。
- 如果 Agent 处于**回合中**（正在运行），消息会中断当前操作 — 就像用户输入新消息并按 Enter 键一样。

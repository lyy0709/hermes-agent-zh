...
位于 `./.hermes/plugins/` 下的项目本地插件默认是禁用的。只有在启动 Hermes 前设置 `HERMES_ENABLE_PROJECT_PLUGINS=true`，才能为受信任的仓库启用它们。
...
| 环境变量门控 | `requires_env: [API_KEY]` 在 plugin.yaml 中 — 在 `hermes plugins install` 期间提示 |
...
| 来源 | 路径 | 用例 |
|--------|------|----------|
| 内置 | `<repo>/plugins/` | 随 Hermes 发布 — 参见 [内置插件](/docs/user-guide/features/built-in-plugins) |
...
当名称冲突时，后发现的源会覆盖先发现的源，因此与内置插件同名的用户插件会替换它。

## 插件是选择启用的

**每个插件 — 用户安装的、内置的或 pip 安装的 — 默认都是禁用的。** 发现机制会找到它们（因此它们会出现在 `hermes plugins` 和 `/plugins` 中），但在你将插件名称添加到 `~/.hermes/config.yaml` 中的 `plugins.enabled` 之前，不会加载任何内容。这可以防止任何带有钩子或工具的内容在你明确同意之前运行。
...
plugins:
  enabled:
    - my-tool-plugin
    - disk-cleanup
  disabled:       # 可选的拒绝列表 — 如果名称同时出现在两者中，则此列表始终优先
    - noisy-plugin
```
...
当你升级到具有选择启用插件功能的 Hermes 版本（配置模式 v21+）时，任何已安装在 `~/.hermes/plugins/` 下且未在 `plugins.disabled` 中的用户插件，将**自动被保留**在 `plugins.enabled` 中。你现有的设置将继续工作。内置插件**不会**被保留 — 即使是现有用户也必须明确选择启用。
...
| **上下文引擎** | 替换内置的上下文压缩器 | 单选（一个活跃） | `plugins/context_engine/` |

记忆提供商和上下文引擎是**提供商插件** — 每种类型一次只能有一个处于活跃状态。通用插件可以以任意组合启用。
...
- **提供商插件部分** — 显示当前选择。按 ENTER 键进入单选选择器，在那里选择一个活跃的提供商。
- 内置插件出现在同一列表中，并带有 `[bundled]` 标签。
...
新安装或内置插件的默认状态是 `not enabled`。`hermes plugins list` 显示所有三种不同的状态，以便你能区分哪些是明确关闭的，哪些只是等待启用。

...
位于 `./.hermes/plugins/` 的项目本地插件默认是禁用的。只有在启动 Hermes 前设置 `HERMES_ENABLE_PROJECT_PLUGINS=true`，才能为受信任的仓库启用它们。
...
**每个插件 — 无论是用户安装的、捆绑的还是 pip 安装的 — 默认都是禁用的。** 发现机制会找到它们（因此它们会出现在 `hermes plugins` 和 `/plugins` 中），但在你将插件名称添加到 `~/.hermes/config.yaml` 中的 `plugins.enabled` 列表之前，不会加载任何内容。这可以防止任何带有钩子或工具的内容在你明确同意之前运行。
...
plugins:
  enabled:
    - my-tool-plugin
    - disk-cleanup
  disabled:       # 可选的拒绝列表 — 如果名称同时出现在两个列表中，此列表始终优先
    - noisy-plugin
```
...
当你升级到具有手动启用插件功能（配置模式 v21+）的 Hermes 版本时，任何已安装在 `~/.hermes/plugins/` 下且未在 `plugins.disabled` 中的用户插件，将**自动被保留**并添加到 `plugins.enabled` 中。你现有的设置将继续工作。捆绑插件**不会**被保留 — 即使是现有用户也必须明确选择启用。
...
记忆提供商和上下文引擎属于**提供商插件**——每种类型一次只能激活一个。通用插件可以任意组合启用。

## NixOS 声明式插件

在 NixOS 上，可以通过模块选项声明式地安装插件——无需使用 `hermes plugins install`。完整细节请参阅 **[Nix 设置指南](/docs/getting-started/nix-setup#plugins)**。
...
声明式插件会以 `nix-managed-` 前缀符号链接——它们与手动安装的插件共存，并且当从 Nix 配置中移除时会自动清理。

...
当名称冲突时，后出现的来源会覆盖先出现的，因此与捆绑插件同名的用户插件会替换它。
...
当你升级到具有手动启用插件功能的 Hermes 版本（配置模式 v21+）时，任何已安装在 `~/.hermes/plugins/` 下且尚未在 `plugins.disabled` 中的用户插件，将**自动被保留**在 `plugins.enabled` 中。你现有的设置将继续工作。捆绑插件**不会**被保留 — 即使是现有用户也必须明确选择启用。
...
```python
ctx.inject_message("Webhook 有新数据到达", role="user")
```
...
- 如果 Agent **正在处理中**（主动运行），消息会中断当前操作 — 就像用户输入新消息并按 Enter 键一样。

...
发生名称冲突时，后出现的源会覆盖先出现的源，因此与捆绑插件同名的用户插件会替换它。
### 插件子类别

在每个源中，Hermes 还识别子类别目录，这些目录将插件路由到专门的发现系统：
...
| `plugins/memory/<name>/` | 记忆提供商（子类 `MemoryProvider`） | **自有加载器**，位于 `plugins/memory/__init__.py`（类型：`exclusive`——一次只能激活一个） |
| `plugins/context_engine/<name>/` | 上下文压缩引擎（`ctx.register_context_engine()`） | **自有加载器**，位于 `plugins/context_engine/__init__.py`（一次只能激活一个） |
| `plugins/model-providers/<name>/` | LLM 提供商配置文件（`register_provider(ProviderProfile(...))`） | **自有加载器**，位于 `providers/__init__.py`（首次调用 `get_provider_profile()` 时延迟扫描） |
...
**通用插件和用户安装的后端默认是禁用的**——发现系统能找到它们（因此它们会出现在 `hermes plugins` 和 `/plugins` 中），但任何带有钩子或工具的插件都不会加载，除非你将插件名称添加到 `~/.hermes/config.yaml` 中的 `plugins.enabled` 列表。这可以防止未经你明确同意的第三方代码运行。
...
| 插件类型 | 其激活方式 |
|---|---|
| **捆绑的平台插件**（IRC、Teams 等，位于 `plugins/platforms/` 下） | 自动加载，以便所有已分发的消息网关通道都可用。实际通道通过 `config.yaml` 中的 `gateway.platforms.<name>.enabled` 开启。 |
...
简而言之：**捆绑的“始终可用”基础设施自动加载；第三方通用插件需要选择启用。** `plugins.enabled` 允许列表是专门为用户放入 `~/.hermes/plugins/` 的任意代码设置的门槛。

### 现有用户的迁移

当你升级到支持选择启用插件的 Hermes 版本（配置模式 v21+）时，任何已安装在 `~/.hermes/plugins/` 下且未在 `plugins.disabled` 中的用户插件，**将自动被保留**到 `plugins.enabled` 中。你现有的设置将继续工作。捆绑的独立插件**不会**被保留——即使是现有用户也需要明确选择启用。（捆绑的平台/后端插件从来不需要保留，因为它们从未被限制。）

## 可用的钩子

插件可以为这些生命周期事件注册回调。完整详情、回调签名和示例，请参阅 **[事件钩子页面](/docs/user-guide/features/hooks#plugin-hooks)**。
...
| **模型提供商** | 声明推理后端（OpenRouter, Anthropic, …） | 多注册，通过 `--provider` / `config.yaml` 选择 | `plugins/model-providers/` |
...
| **推理后端**（LLM 提供商：OpenAI 兼容, Codex, Anthropic-Messages, Bedrock） | 提供商插件 — 在 `plugins/model-providers/<name>/` 中 `register_provider(ProviderProfile(...))` | **[模型提供商插件](/docs/developer-guide/model-provider-plugin)** · [添加提供商](/docs/developer-guide/adding-providers) |
| **消息网关通道**（Discord / Telegram / IRC / Teams / 等） | 平台插件 — 在 `plugins/platforms/<name>/` 中 `ctx.register_platform()` | [添加平台适配器](/docs/developer-guide/adding-platform-adapters) |
...
| **TTS 后端**（任何 CLI — Piper, VoxCPM, Kokoro, xtts, 语音克隆脚本, …） | 配置驱动 — 在 `config.yaml` 的 `tts.providers.<name>` 下声明 `type: command` | [TTS 设置](/docs/user-guide/features/tts#custom-command-providers) |
...
| **通过 MCP 的外部工具**（文件系统, GitHub, Linear, Notion, 任何 MCP 服务器） | 配置驱动 — 在 `config.yaml` 中声明 `mcp_servers.<name>` 并指定 `command:` / `url:`。Hermes 自动发现服务器的工具并将其与内置工具一起注册。 | [MCP](/docs/user-guide/features/mcp) |
...
:::note
并非所有东西都是 Python 插件。一些扩展表面特意使用**配置驱动的 shell 命令**（TTS, STT, shell 钩子），这样你已有的任何 CLI 无需编写 Python 即可成为插件。其他是**外部服务器**（MCP），Agent 连接到它们并自动注册其工具。还有一些是**即插即用目录**（消息网关钩子），它们有自己的清单格式。请根据适合你用例的集成风格选择正确的表面；上表中的每个编写指南都涵盖了占位符、发现和示例。
:::

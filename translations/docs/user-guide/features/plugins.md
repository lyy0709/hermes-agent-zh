...
如果你想为自己、你的团队或某个项目创建自定义工具，这通常是正确的路径。开发者指南中的[添加工具](/docs/developer-guide/adding-tools)页面适用于位于 `tools/` 和 `toolsets.py` 中的 Hermes 核心内置工具。
...
`./.hermes/plugins/` 下的项目本地插件默认是禁用的。只有在启动 Hermes 之前设置 `HERMES_ENABLE_PROJECT_PLUGINS=true`，才能为受信任的仓库启用它们。
...
后续来源会覆盖较早来源的同名冲突，因此与捆绑插件同名的用户插件会替换它。

### 插件子类别

在每个来源中，Hermes 还识别子类别目录，这些目录将插件路由到专门的发现系统：
...
位于 `~/.hermes/plugins/model-providers/<name>/` 和 `~/.hermes/plugins/memory/<name>/` 的用户插件会覆盖同名的捆绑插件——遵循 `register_provider()` / `register_memory_provider()` 中的“后写者胜”原则。只需放入一个目录，它就会替换内置插件，无需编辑仓库。

## 插件默认需要手动启用（少数例外）

**通用插件和用户安装的后端默认是禁用的**——发现机制会找到它们（因此它们会出现在 `hermes plugins` 和 `/plugins` 中），但任何带有钩子或工具的插件都不会加载，除非你将插件名称添加到 `~/.hermes/config.yaml` 中的 `plugins.enabled` 列表。这可以防止未经你明确同意的第三方代码运行。
...
在 `hermes plugins install owner/repo` 之后，系统会询问 `现在启用 'name' 吗？ [y/N]` —— 默认是“否”。对于脚本化安装，可以使用 `--enable` 或 `--no-enable` 跳过提示。
...
| 插件类型 | 其激活方式 |
|---|---|
| **捆绑的平台插件** (IRC、Teams 等，位于 `plugins/platforms/` 下) | 自动加载，以便所有随附的消息网关通道都可用。实际通道通过 `config.yaml` 中的 `gateway.platforms.<name>.enabled` 开启。 |
| **捆绑的后端** (位于 `plugins/image_gen/` 下的图像生成提供商等) | 自动加载，以便默认后端“开箱即用”。选择通过 `config.yaml` 中的 `<category>.provider` 进行（例如 `image_gen.provider: openai`）。 |
| **记忆提供商** (`plugins/memory/`) | 全部被发现；通过 `config.yaml` 中的 `memory.provider` 选择一个激活。 |
| **上下文引擎** (`plugins/context_engine/`) | 全部被发现；通过 `config.yaml` 中的 `context.engine` 选择一个激活。 |
| **模型提供商** (`plugins/model-providers/`) | `plugins/model-providers/` 下的所有捆绑提供商在首次调用 `get_provider_profile()` 时被发现和注册。用户通过 `--provider` 或 `config.yaml` 每次选择一个。 |
| **通过 pip 安装的 `backend` 插件** | 通过 `plugins.enabled` 选择启用（与通用插件相同）。 |
| **用户安装的平台插件** (位于 `~/.hermes/plugins/platforms/` 下) | 通过 `plugins.enabled` 选择启用——第三方网关适配器需要明确同意。 |

简而言之：**捆绑的“始终可用”基础设施自动加载；第三方通用插件需要手动启用。** `plugins.enabled` 允许列表是专门为用户放入 `~/.hermes/plugins/` 的任意代码设置的门槛。

### 现有用户的迁移

当你升级到支持手动启用插件的 Hermes 版本（配置模式 v21+）时，任何已安装在 `~/.hermes/plugins/` 下且未在 `plugins.disabled` 中的用户插件，**将自动被保留**到 `plugins.enabled` 中。你现有的设置将继续工作。捆绑的独立插件**不会**被保留——即使是现有用户也必须明确选择启用。（捆绑的平台/后端插件从来不需要保留，因为它们从未被限制。）
...
| [`pre_llm_call`](/docs/user-guide/features/hooks#pre_llm_call) | 每轮对话一次，在 LLM 循环之前——可以返回 `{"context": "..."}` 来 [将上下文注入用户消息](/docs/user-guide/features/hooks#pre_llm_call) |
...
| **模型提供商** | 声明一个推理后端 (OpenRouter, Anthropic, …) | 多重注册，通过 `--provider` / `config.yaml` 选择 | `plugins/model-providers/` |

记忆提供商和上下文引擎是**提供商插件**——每种类型一次只能激活一个。模型提供商也是插件，但可以同时加载多个；用户通过 `--provider` 或 `config.yaml` 每次选择一个。通用插件可以以任意组合启用。
## 可插拔接口 — 各功能对应的实现方式

上表展示了四种插件类别，但在“通用插件”中，`PluginContext` 暴露了多个不同的扩展点 — 并且 Hermes 也接受 Python 插件系统之外的扩展（配置驱动的后端、Shell 挂钩命令、外部服务器等）。使用此表查找您想要构建功能对应的正确文档：
...
| **生命周期钩子**（LLM 调用前/后、会话开始/结束、工具过滤器） | Python 插件 — `ctx.register_hook()` | [钩子参考](/docs/user-guide/features/hooks) · [构建 Hermes 插件](/docs/guides/build-a-hermes-plugin) |
...
| 您的插件附带的捆绑**技能** | Python 插件 — `ctx.register_skill()` | [创建技能](/docs/developer-guide/creating-skills) |
...
| **TTS 后端**（任何 CLI — Piper、VoxCPM、Kokoro、xtts、语音克隆脚本、…） | 配置驱动 — 在 `config.yaml` 中的 `tts.providers.<name>` 下声明 `type: command` | [TTS 设置](/docs/user-guide/features/tts#custom-command-providers) |
...
| **Shell 钩子**（在事件发生时运行 Shell 命令 — 通知、审计日志、桌面提醒） | 配置驱动 — 在 `config.yaml` 中的 `hooks:` 下声明 | [Shell 钩子](/docs/user-guide/features/hooks#shell-hooks) |

:::note
并非所有东西都是 Python 插件。一些扩展面特意使用**配置驱动的 Shell 命令**（TTS、STT、Shell 钩子），这样您已有的任何 CLI 无需编写 Python 代码即可成为插件。其他则是 Agent 连接并自动从中注册工具的**外部服务器**（MCP）。还有一些是**即插即用目录**（消息网关钩子），它们有自己的清单格式。请根据您的用例选择合适的集成方式；上表中的开发指南各自涵盖了占位符、发现机制和示例。
:::

## NixOS 声明式插件

在 NixOS 上，可以通过模块选项以声明方式安装插件 — 无需 `hermes plugins install`。完整详情请参见 **[Nix 设置指南](/docs/getting-started/nix-setup#plugins)**。
...
声明式插件会以 `nix-managed-` 前缀符号链接 — 它们与手动安装的插件共存，并在从 Nix 配置中移除时自动清理。
...
- **提供商插件部分** — 显示当前选择。按 ENTER 键进入单选选择器，您可以在其中选择一个活动的提供商。
...
- 如果 Agent **正在处理中**（正在主动运行），消息会中断当前操作 — 就像用户输入新消息并按 Enter 键一样。

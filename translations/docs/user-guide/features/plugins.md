...
| 注册记忆后端 | 在 `plugins/memory/<name>/__init__.py` 中继承 `MemoryProvider` — 参见[记忆提供商插件](/docs/developer-guide/memory-provider-plugin)（使用独立的发现系统） |
| 注册推理后端（LLM 提供商） | 在 `plugins/model-providers/<name>/__init__.py` 中使用 `register_provider(ProviderProfile(...))` — 参见[模型提供商插件](/docs/developer-guide/model-provider-plugin)（使用独立的发现系统） |
...
在每个来源内部，Hermes 还识别将插件路由到专门发现系统的子类别目录：
...
放置在 `~/.hermes/plugins/model-providers/<name>/` 和 `~/.hermes/plugins/memory/<name>/` 下的用户插件会覆盖同名的捆绑插件——在 `register_provider()` / `register_memory_provider()` 中遵循“后写者胜”原则。只需放入一个目录，它就会替换内置插件，无需编辑仓库。

## 插件默认需要手动启用（少数例外）

**通用插件和用户安装的后端默认是禁用的**——发现机制会找到它们（因此它们会出现在 `hermes plugins` 和 `/plugins` 中），但任何带有钩子或工具的插件都不会加载，除非你将插件名称添加到 `~/.hermes/config.yaml` 中的 `plugins.enabled` 列表。这可以防止未经你明确同意就运行第三方代码。
...
| 插件类型 | 其激活方式 |
|---|---|
| **捆绑的平台插件** (IRC、Teams 等，位于 `plugins/platforms/` 下) | 自动加载，以便所有已提供的消息网关通道都可用。实际通道通过 `config.yaml` 中的 `gateway.platforms.<name>.enabled` 开启。 |
...
| **记忆提供商** (`plugins/memory/`) | 全部被发现；只有一个处于活动状态，由 `config.yaml` 中的 `memory.provider` 选择。 |
| **上下文引擎** (`plugins/context_engine/`) | 全部被发现；只有一个处于活动状态，由 `config.yaml` 中的 `context.engine` 选择。 |
| **模型提供商** (`plugins/model-providers/`) | 所有 33 个提供商在首次调用 `get_provider_profile()` 时被发现并注册。用户通过 `--provider` 或 `config.yaml` 一次选择一个。 |
| **通过 pip 安装的 `backend` 插件** | 通过 `plugins.enabled` 选择启用（与通用插件相同）。 |
| **用户安装的平台插件** (位于 `~/.hermes/plugins/platforms/` 下) | 通过 `plugins.enabled` 选择启用——第三方消息网关适配器需要明确同意。 |

简而言之：**捆绑的“始终可用”基础设施会自动加载；第三方通用插件需要手动启用。** `plugins.enabled` 允许列表是专门为放入 `~/.hermes/plugins/` 的任意代码设置的门槛。

### 现有用户的迁移

当你升级到具有手动启用插件功能的 Hermes 版本（配置模式 v21+）时，任何已安装在 `~/.hermes/plugins/` 下且未在 `plugins.disabled` 中的用户插件，**将自动被保留**到 `plugins.enabled` 中。你现有的设置将继续工作。捆绑的独立插件**不会**被保留——即使是现有用户也必须明确选择启用。（捆绑的平台/后端插件从来不需要保留，因为它们从未被限制。）
...
| [`subagent_stop`](/docs/user-guide/features/hooks#subagent_stop) | 每次 `delegate_task` 完成后，每个子 Agent 一次 |
...
| **模型提供商** | 声明一个推理后端 (OpenRouter, Anthropic, …) | 多注册，通过 `--provider` / `config.yaml` 选择 | `plugins/model-providers/` |

记忆提供商和上下文引擎是**提供商插件**——每种类型一次只能有一个处于活动状态。模型提供商也是插件，但可以同时加载多个；用户通过 `--provider` 或 `config.yaml` 一次选择一个。通用插件可以以任意组合启用。
## 可插拔接口 — 每种功能该去哪里

上表展示了四种插件类别，但在“通用插件”中，`PluginContext` 暴露了几个不同的扩展点 — 并且 Hermes 也接受 Python 插件系统之外的扩展（配置驱动的后端、Shell 钩子命令、外部服务器等）。使用此表来找到你想构建功能对应的正确文档：
...
:::note
并非所有东西都是 Python 插件。一些扩展面特意使用**配置驱动的 Shell 命令**（TTS、STT、Shell 钩子），这样你已有的任何 CLI 都可以成为插件，而无需编写 Python。其他是 Agent 连接并自动从中注册工具的**外部服务器**（MCP）。还有一些是**即插即用目录**（消息网关钩子），它们有自己的清单格式。请根据适合你用例的集成风格选择正确的扩展面；上表中的编写指南各自涵盖了占位符、发现和示例。
:::
...
声明式插件通过 `nix-managed-` 前缀符号链接 — 它们与手动安装的插件共存，并在从 Nix 配置中移除时自动清理。
...
- **提供商插件部分** — 显示当前选择。按 ENTER 键进入单选选择器，您可以在其中选择一个活跃的提供商。
...
- 如果 Agent 处于**空闲**状态（等待用户输入），该消息将作为下一个输入排队，并开始一个新的回合。
- 如果 Agent 处于**回合中**（正在主动运行），该消息会中断当前操作 — 就像用户输入新消息并按 Enter 键一样。

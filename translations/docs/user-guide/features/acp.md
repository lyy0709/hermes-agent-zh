...
它特意排除了那些不适合典型编辑器用户体验的功能，例如消息传递和定时任务管理。
...
在服务器运行期间，ACP 会话由 ACP 适配器的内存会话管理器跟踪。
...
底层的 `AIAgent` 仍然使用 Hermes 正常的持久化/日志路径，但 ACP 的 `list/load/resume/fork` 操作范围仅限于当前运行的 ACP 服务器进程。
...
- 是否安装了 ACP 额外组件 (`pip install -e '.[acp]'`)

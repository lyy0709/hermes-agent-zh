...
- **[技能系统](skills.md)** — Agent 可按需加载的知识文档。技能遵循渐进式披露模式以最小化 Token 使用，并与 [agentskills.io](https://agentskills.io/specification) 开放标准兼容。
...
- **[上下文引用](context-references.md)** — 输入 `@` 后跟引用来将文件、文件夹、git diff 和 URL 直接注入您的消息。Hermes 会内联展开引用并自动附加内容。
...
- **[子 Agent 委派](delegation.md)** — `delegate_task` 工具会生成具有独立上下文、受限工具集和自身终端会话的子 Agent 实例。默认（可配置）运行 3 个并发子 Agent 以实现并行工作流。
- **[代码执行](code-execution.md)** — `execute_code` 工具允许 Agent 编写 Python 脚本，以编程方式调用 Hermes 工具，通过沙盒化的 RPC 执行将多步骤工作流压缩到单个 LLM 轮次中。
...
- **[图像生成](image-generation.md)** — 使用 FAL.ai 根据文本提示词生成图像。支持九种模型（FLUX 2 Klein/Pro、GPT-Image 1.5/2、Nano Banana Pro、Ideogram V3、Recraft V4 Pro、Qwen、Z-Image Turbo）；可通过 `hermes tools` 选择。
- **[语音与 TTS](tts.md)** — 在所有消息平台上实现文本转语音输出和语音消息转录，提供九种提供商选项：Edge TTS（免费）、ElevenLabs、OpenAI TTS、MiniMax、Mistral Voxtral、Google Gemini、xAI、NeuTTS 和 KittenTTS。
...
- **[记忆提供商](memory-providers.md)** — 插入外部记忆后端（Honcho、OpenViking、Mem0、Hindsight、Holographic、RetainDB、ByteRover、Supermemory），用于跨会话用户建模和超越内置记忆系统的个性化。

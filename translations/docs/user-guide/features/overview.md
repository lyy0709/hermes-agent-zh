...
- **[代码执行](code-execution.md)** — `execute_code` 工具允许 Agent 编写 Python 脚本，以编程方式调用 Hermes 工具，通过沙盒化的 RPC 执行，将多步骤工作流压缩到单个 LLM 轮次中。
...
- **[提供商路由](provider-routing.md)** — 精细控制哪些 AI 提供商处理您的请求。通过排序、白名单、黑名单和优先级排序，优化成本、速度或质量。
...
- **[API 服务器](api-server.md)** — 将 Hermes 作为 OpenAI 兼容的 HTTP 端点暴露。连接任何支持 OpenAI 格式的前端 — Open WebUI, LobeChat, LibreChat 等。
- **[IDE 集成 (ACP)](acp.md)** — 在兼容 ACP 的编辑器（如 VS Code, Zed, JetBrains）中使用 Hermes。聊天、工具活动、文件差异和终端命令会在您的编辑器内呈现。

...
- **[持久化记忆](memory.md)** — 有界、精选的记忆，可在会话间持久保存。Hermes 会记住您的偏好、项目、环境以及它通过 `MEMORY.md` 和 `USER.md` 学到的内容。
...
- **[子 Agent 委派](delegation.md)** — `delegate_task` 工具可生成具有独立上下文、受限工具集和自身终端会话的子 Agent 实例。最多可同时运行 3 个子 Agent 以实现并行工作流。
...
- **[图像生成](image-generation.md)** — 使用 FAL.ai 的 FLUX 2 Pro 模型，通过 Clarity Upscaler 自动进行 2 倍超分辨率，从文本提示词生成图像。
...
- **[记忆提供商](memory-providers.md)** — 插入外部记忆后端（Honcho、OpenViking、Mem0、Hindsight、Holographic、RetainDB、ByteRover），用于跨会话用户建模和个性化，超越内置记忆系统。
...
- **[人格与 SOUL.md](personality.md)** — 完全可定制的 Agent 人格。`SOUL.md` 是主要的身份文件——系统提示词中的第一项内容——您可以在每个会话中切换内置或自定义的 `/personality` 预设。

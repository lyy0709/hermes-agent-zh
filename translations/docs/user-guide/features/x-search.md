...
| 凭证 | 来源 | 设置 |
|------------|--------|-------|
| **SuperGrok / X Premium+ OAuth** (推荐) | 在 `accounts.x.ai` 进行浏览器登录，自动刷新 | `hermes auth add xai-oauth` —— 参见 [xAI Grok OAuth (SuperGrok / X Premium+)](../../guides/xai-grok-oauth.md) |
...
任一选择都满足启用条件。你可以选择你已有的任何凭证；该工具对两者同样有效。如果两者最终都配置了，在调用时 OAuth 是首选。
...
- `degraded` —— 当设置了任何限制性过滤器（`allowed_x_handles`、`excluded_x_handles`、`from_date`、`to_date`）**且**两个引用通道都返回空时，为 `true`。在这种情况下，`answer` 是根据模型自身的知识综合而成，而不是来自 X 索引，因此应将其视为无来源的。否则为 `false`（包括“未设置过滤器”的情况 —— 一个宽泛的无来源答案只是一个答案，而不是过滤器未命中）
...
当两个认证路径都失败时，该工具会显示此信息。请在 `~/.hermes/.env` 中设置 `XAI_API_KEY`，或者运行 `hermes auth add xai-oauth` 并完成浏览器登录。然后重新启动你的会话，以便 Agent 重新读取工具注册表。

### “`x_search` 未为此模型启用”

配置的 `x_search.model` 没有访问服务器端 `x_search` 工具的权限。切换到 `grok-4.20-reasoning`（默认值）或支持该工具的其他 Grok 模型。请查看 [xAI 文档](https://docs.x.ai/) 以获取当前列表。
...
- **xAI 索引间隙。** 一些活跃账户会间歇性地无法在 `x_search` 中显示，即使它们定期发帖。几分钟后重试，或者当你需要某个确切句柄的时间线时，使用 `xurl` 技能进行直接的 X API 读取。

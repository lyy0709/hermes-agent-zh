...
如果你曾希望 Hermes 使用某个已存在于别处的工具，MCP 通常是实现这一目标最简洁的方式。

## MCP 带来的优势

- **访问外部工具生态系统**，无需先编写原生 Hermes 工具
- **在同一个配置中**支持本地 stdio 服务器和远程 HTTP MCP 服务器
- 启动时**自动发现并注册工具**
- 当服务器支持时，为 MCP 资源和提示词提供**实用工具包装器**
...
这些实用工具现在具备能力感知功能：
- 只有当 MCP 会话**实际支持**资源操作时，Hermes 才会注册资源实用工具
- 只有当 MCP 会话**实际支持**提示词操作时，Hermes 才会注册提示词实用工具
...
这对于能力动态变化的 MCP 服务器非常有用（例如，当加载新数据库模式时添加工具，或当服务下线时移除工具的服务器）。
...
只有显式配置的 `env` 加上一个安全的基线环境会被传递。这减少了意外泄露密钥的风险。
...
除了可以连接**到** MCP 服务器，Hermes 也可以**作为**一个 MCP 服务器。这使得其他支持 MCP 的 Agent（Claude Code、Cursor、Codex 或任何 MCP 客户端）能够使用 Hermes 的消息传递能力 —— 列出会话、读取消息历史记录，并通过所有你已连接的平台发送消息。

### 何时使用此功能

- 你希望 Claude Code、Cursor 或其他编程 Agent 能通过 Hermes 发送和读取 Telegram/Discord/Slack 消息
...
MCP 服务器暴露了 10 个工具，与 OpenClaw 的频道桥接功能相匹配，外加一个 Hermes 特有的频道浏览器：

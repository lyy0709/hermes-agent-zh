...
Hermes 随仓库捆绑了一小部分插件。它们位于 `<repo>/plugins/<name>/` 目录下，并与 `~/.hermes/plugins/` 中的用户安装插件一起自动加载。它们使用与第三方插件相同的插件接口——钩子、工具、斜杠命令——只是维护在仓库树内。
...
捆绑插件在默认情况下是禁用的。发现机制能找到它们（它们会出现在 `hermes plugins list` 和交互式 `hermes plugins` UI 中），但除非你显式启用它们，否则不会加载任何插件：
...
这与用户安装插件使用的机制相同。捆绑插件永远不会自动启用——无论是全新安装，还是现有用户升级到新版本的 Hermes。你总是需要显式选择启用。
...
将 Hermes 的轮次、LLM 调用和工具调用追踪到 [Langfuse](https://langfuse.com) —— 一个开源的 LLM 可观测性平台。每个轮次对应一个 span，每个 API 调用对应一个 generation，每个工具调用对应一个 tool observation。使用总量、按类型的 Token 计数和成本估算均来自 Hermes 规范的 `agent.usage_pricing` 数据，因此 Langfuse 仪表板看到的细分（输入 / 输出 / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`）与 `hermes logs` 中显示的相同。

该插件采用故障开放策略：未安装 SDK、缺少凭据或发生瞬时 Langfuse 错误 —— 所有这些在钩子中都会静默地变为无操作。Agent 循环永远不会受到影响。
...
向导会收集您的密钥，`pip install` 安装 `langfuse` SDK，并为您将 `observability/langfuse` 添加到 `plugins.enabled` 中。重启 Hermes，下一次轮次就会发送追踪信息。
...
| 钩子 | 行为 |
|---|---|
| `pre_api_request` / `pre_llm_call` | 打开（或复用）一个每轮次的根 span "Hermes turn"。为此 API 调用启动一个 `generation` 子 observation，并将序列化的最近消息作为输入。 |
...
| `HERMES_LANGFUSE_MAX_CHARS` | `12000` | 消息内容 / 工具参数 / 工具结果每个字段的截断长度 |
| `HERMES_LANGFUSE_DEBUG` | `false` | 详细的插件日志记录到 `agent.log` |

以 Hermes 为前缀和标准的 SDK 环境变量（`LANGFUSE_PUBLIC_KEY`、`LANGFUSE_SECRET_KEY`、`LANGFUSE_BASE_URL`）均可接受 —— 当两者都设置时，以 Hermes 为前缀的变量优先。

**性能：** Langfuse 客户端在第一次钩子调用后被缓存。如果凭据或 SDK 缺失，该决定也会被缓存 —— 后续的钩子会快速返回，无需重新检查环境变量或重新加载配置。

**禁用：** `hermes plugins disable observability/langfuse`。插件模块仍会被发现，但在您重新启用之前，不会运行任何模块代码。

### google_meet

让 Agent **加入、转录并参与 Google Meet 通话** —— 在会议中做笔记，会后总结讨论内容，跟进特定要点，并（可选地）通过 TTS 将回复语音播报回通话中。
...
Agent 会启动加入会议，在通话进行时将转录流式传输回其上下文，并在会议结束时（或当您告诉它停止时）生成一份结构化摘要。

**使用场景：** 定期的站会，您希望有一个机器人来为异步参与者转录和总结；类似证词式的访谈，您需要结构化笔记；任何原本需要 Fireflies / Otter / Grain 的场景。当您不希望有 AI 监听时 —— 请不要启用它。

**禁用：** `hermes plugins disable google_meet`。任何缓存的转录稿和录音将保留在 `~/.hermes/cache/google_meet/` 中，直到您手动删除。

### hermes-achievements

在仪表板中添加一个 **Steam 风格的成就标签页** —— 60 多个可收集的、分层的徽章，根据您真实的 Hermes 会话历史生成。工具链壮举、调试模式、氛围编码连续记录、技能/记忆使用情况、模型/提供商多样性、生活习性（周末和夜间会话）。最初由 [@PCinkusz](https://github.com/PCinkusz) 作为外部插件创作；现已纳入主仓库，以便与 Hermes 功能变更保持同步。

**工作原理：**

- 在仪表板后端扫描您整个 `~/.hermes/state.db` 会话历史
...
**等级进阶：** 铜牌 → 银牌 → 金牌 → 钻石 → 奥林匹亚。每张卡片都包含一个"统计标准"部分，列出了正在追踪的确切指标。
...
| 已解锁 | 至少达到一个等级 |
| 已发现 | 已知成就，进度可见，但尚未获得 |
| 隐藏 | 在 Hermes 从您的历史记录中检测到第一个相关信号之前保持隐藏 |
...
| `GET /scan-status` | 后台扫描器状态：`idle` / `running` / `failed`，上次运行时长，运行次数 |
...
| 文件 | 内容 |
|---|---|
| `state.json` | 解锁历史：您获得了哪些徽章以及何时获得。在 Hermes 更新之间保持稳定。 |
...
- **冷扫描期间的增量结果** — 扫描器大约每 250 个会话发布一次部分快照，因此随着扫描的进行，每次仪表板刷新都会显示更多已解锁的徽章。无需长时间盯着零进度。
- 热重新扫描会重用每个会话的统计信息，前提是该会话的 `started_at` + `last_active` 指纹与检查点匹配 — 即使在大历史记录上也能在几秒钟内完成。
- 内存中快照的 TTL 为 120 秒；过时的请求会立即提供旧快照并触发后台刷新。您绝不会仅仅因为 TTL 过期而等待加载指示器。
...
**选择退出：** 删除或重命名 `plugins/hermes-achievements/dashboard/manifest.json`，或者使用 `~/.hermes/plugins/hermes-achievements/` 中同名的、不提供仪表板的用户插件覆盖它。插件在 `$HERMES_HOME/plugins/hermes-achievements/` 下的状态文件将保留 — 重新安装会保留您的解锁历史。

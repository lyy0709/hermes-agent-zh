...
Hermes 随仓库捆绑了一小部分插件。它们位于 `<repo>/plugins/<name>/` 目录下，并与 `~/.hermes/plugins/` 中的用户安装插件一同自动加载。它们使用与第三方插件相同的插件接口——钩子、工具、斜杠命令——只是维护在仓库树内。
...
发生名称冲突时，后扫描的来源会覆盖先前的——例如，名为 `disk-cleanup` 的用户插件将替换捆绑的同名插件。

`plugins/memory/` 和 `plugins/context_engine/` 目录被特意排除在捆绑插件扫描之外。这些目录使用其自己的发现路径，因为记忆提供商和上下文引擎是通过 `hermes memory setup` / 配置中的 `context.engine` 配置的单选提供商。

## 捆绑插件需手动启用

捆绑插件在默认情况下是禁用的。发现机制能找到它们（它们会出现在 `hermes plugins list` 和交互式 `hermes plugins` UI 中），但除非你显式启用，否则不会加载任何插件：
...
| `security-guidance` | 钩子 | 在 `write_file`/`patch` 操作时模式匹配危险代码，并附加安全警告（或阻止）——包含 25 条规则（基于 Anthropic 的 `claude-plugins-official` 模式的 Apache-2.0 分支） |
| `observability/langfuse` | 钩子 | 将轮次 / LLM 调用 / 工具追踪到 [Langfuse](https://langfuse.com) |
| `observability/nemo_relay` | 钩子 | 将可观测性事件（轮次 / LLM 调用 / 工具）中继到 NVIDIA NeMo 端点 |
| `teams_pipeline` | 独立插件 | Microsoft Teams 会议流水线——基于 Graph API，以转录为先的会议摘要 |
...
**安全性** — 清理操作仅触及 `HERMES_HOME` 或 `/tmp/hermes-*` 下的路径。Windows 挂载点（`/mnt/c/...`）会被拒绝。已知的顶级状态目录（`logs/`、`memories/`、`sessions/`、`cron/`、`cache/`、`skills/`、`plugins/`、`disk-cleanup/` 自身）即使为空也永远不会被移除——全新安装不会在第一次会话结束时被清空。
...
对文件写入进行快速模式匹配的安全警告。当 Agent 的 `write_file` / `patch` / `skill_manage` 调用包含的内容匹配已知的危险代码模式时 —— 例如 `pickle.load`、未使用 `SafeLoader` 的 `yaml.load`、`eval(`、`os.system`、`subprocess(...,  shell=True)`、JS `child_process.exec`、React `dangerouslySetInnerHTML`、原始的 `.innerHTML =` / `.outerHTML =` / `document.write`、Node `crypto.createCipher`、AES ECB 模式、禁用 TLS 验证、易受 XXE 攻击的 `xml.etree` / `minidom` 解析器、没有 SRI 的 `<script src="//..." >`、未使用 `weights_only=True` 的 `torch.load`、GitHub Actions `${{ github.event.* }}` 注入 —— 该插件会在工具结果后附加一个 `⚠️ Security guidance` 块。

文件仍会被写入。模型会在下一轮的 tool message 中读取警告，并可以修复代码或说明在此上下文中该结构为何是安全的。模式匹配存在一定的误报率，因此默认行为是警告（而非阻止）。
...
| `SECURITY_GUIDANCE_DISABLE=1` | 紧急关闭开关 —— 插件加载但不执行任何操作 |

**启用：** `hermes plugins enable security-guidance`（或在 `hermes plugins` 中勾选复选框）。

**再次禁用：** `hermes plugins disable security-guidance`。

**它目前不做什么：** 上游的 Anthropic 插件还有另外两层 —— 一层是在每个涉及文件的 Agent 轮次后进行 LLM 差异审查，另一层是代理式提交时审查，用于追踪跨文件的数据流。这两层都尚未移植。Agent 已经可以通过 `delegate_task` 按需运行这些审查。

### observability/langfuse

将 Hermes 轮次、LLM 调用和工具调用追踪到 [Langfuse](https://langfuse.com) —— 一个开源的 LLM 可观测性平台。每个轮次一个 span，每次 API 调用一个 generation，每次工具调用一个 tool observation。使用总量、按类型的 Token 计数和成本估算来自 Hermes 规范的 `agent.usage_pricing` 数字，因此 Langfuse 仪表板看到的细分（输入 / 输出 / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`）与 `hermes logs` 中显示的相同。
...
向导会收集你的密钥，为你 `pip install` `langfuse` SDK，并将 `observability/langfuse` 添加到 `plugins.enabled` 中。重启 Hermes，下一个轮次就会发送追踪信息。
...
| `post_tool_call` | 使用清理过的 `result` 关闭 tool observation。`read_file` 的有效载荷会被摘要化（头部 + 尾部 + 省略的行数），以便巨大的文件读取保持在 `HERMES_LANGFUSE_MAX_CHARS` 限制内。 |
...
带 Hermes 前缀的环境变量和标准 SDK 环境变量（`LANGFUSE_PUBLIC_KEY`、`LANGFUSE_SECRET_KEY`、`LANGFUSE_BASE_URL`）均可接受 —— 当两者都设置时，带 Hermes 前缀的优先。

**性能：** Langfuse 客户端在第一次钩子调用后被缓存。如果凭据或 SDK 缺失，该决定也会被缓存 —— 后续的钩子会快速返回，无需重新检查环境变量或重新加载配置。

**禁用：** `hermes plugins disable observability/langfuse`。插件模块仍会被发现，但在你重新启用之前，不会运行任何模块代码。

### google_meet

让 Agent **加入、转录并参与 Google Meet 通话** —— 在会议中做笔记，会后总结讨论内容，跟进具体要点，并（可选地）通过 TTS 将回复语音播报到通话中。
**新增功能：**

- 一个无头虚拟参与者，通过浏览器自动化加入 Meet URL
- 通过配置的 STT 提供商对会议音频进行实时转录
- Agent 可以调用 `meet_summarize` / `meet_speak` / `meet_followup` 工具集，对其听到的内容采取行动
...
# 需要一个具有 Meet 访问权限的 Google 账户。如果会议设置了“仅受邀参与者可以加入”，
# 可能需要主持人批准。
```
...
Agent 会启动加入会议的操作，在通话过程中将转录内容流式传输回其上下文，并在会议结束时（或当你告诉它停止时）生成一份结构化的摘要。

**使用场景：** 定期的站会，你希望有一个机器人来为异步参与者转录和总结；需要结构化笔记的取证式访谈；任何原本需要使用 Fireflies / Otter / Grain 的场景。当你不想让 AI 旁听时 —— 不要启用它。

**禁用：** `hermes plugins disable google_meet`。任何缓存的转录稿和录音将保留在 `~/.hermes/cache/google_meet/` 中，直到你手动删除。

### hermes-achievements

在仪表板中添加一个 **Steam 风格的成就标签页** —— 根据你真实的 Hermes 会话历史生成 60 多个可收集的分级徽章。包括工具链成就、调试模式、氛围编码连胜、技能/记忆使用情况、模型/提供商多样性、生活习性（周末和夜间会话）。最初由 [@PCinkusz](https://github.com/PCinkusz) 作为外部插件开发；现已纳入主仓库，以便与 Hermes 的功能变更保持同步。
...
**等级进阶：** 铜级 → 银级 → 金级 → 钻石级 → 奥林匹亚级。每张卡片都包含一个“统计内容”部分，列出了正在追踪的确切指标。
...
- 对约 8,000 个会话的冷扫描需要几分钟。它在首次仪表板请求时在后台线程中运行；UI 会看到一个待处理的占位符并轮询 `/scan-status`。
- **冷扫描期间的增量结果** —— 扫描器大约每 250 个会话发布一次部分快照，因此每次仪表板刷新都会随着扫描的进行显示更多已解锁的徽章。无需长时间盯着零进度。
...
**选择退出：** 删除或重命名 `plugins/hermes-achievements/dashboard/manifest.json`，或者用 `~/.hermes/plugins/hermes-achievements/` 中同名的用户插件覆盖它，该用户插件不提供仪表板。插件在 `$HERMES_HOME/plugins/hermes-achievements/` 下的状态文件将保留 —— 重新安装会保留你的解锁历史。
...
反例——应保持为用户可安装的插件，而非捆绑打包的内容：需要 API 密钥的第三方集成、小众工作流、依赖树庞大的组件，以及任何会显著改变 Agent 默认行为的内容。
...
这与用户安装插件使用的机制相同。捆绑插件永远不会自动启用——无论是全新安装，还是现有用户升级到更新的 Hermes 版本。你总是需要显式选择启用。
...
**安全性** — 清理操作仅会触及 `HERMES_HOME` 或 `/tmp/hermes-*` 下的路径。Windows 挂载点 (`/mnt/c/...`) 会被拒绝。已知的顶级状态目录（`logs/`、`memories/`、`sessions/`、`cron/`、`cache/`、`skills/`、`plugins/`、`disk-cleanup/` 自身）即使为空也永远不会被移除——这样新安装的系统不会在第一次会话结束时被清空。
...
将 Hermes 的轮次、LLM 调用和工具调用追踪到 [Langfuse](https://langfuse.com) —— 一个开源的 LLM 可观测性平台。每个轮次一个跨度，每次 API 调用一个生成，每次工具调用一个工具观察。使用总量、按类型的 Token 计数和成本估算来自 Hermes 的规范 `agent.usage_pricing` 数字，因此 Langfuse 仪表板看到的细分（输入 / 输出 / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`）与 `hermes logs` 中显示的相同。

该插件采用故障开放策略：未安装 SDK、无凭据或 Langfuse 暂时性错误 —— 所有这些在钩子中都会静默地变为无操作。Agent 循环永远不会受到影响。
...
向导会收集您的密钥，`pip install` 安装 `langfuse` SDK，并为您将 `observability/langfuse` 添加到 `plugins.enabled`。重启 Hermes，下一个轮次就会发送追踪。
...
带 Hermes 前缀和标准的 SDK 环境变量（`LANGFUSE_PUBLIC_KEY`、`LANGFUSE_SECRET_KEY`、`LANGFUSE_BASE_URL`）都被接受 —— 当两者都设置时，带 Hermes 前缀的优先。
...
允许 Agent **加入、转录并参与 Google Meet 通话** —— 为会议做笔记，会后总结讨论内容，跟进具体要点，并（可选地）通过 TTS 将回复语音播报到通话中。
...
Agent 会启动会议加入，在通话进行时将转录流式传输回其上下文，并在会议结束时（或当您告诉它停止时）生成一份结构化摘要。

**使用场景：** 定期的站会，您希望有一个机器人进行转录 + 总结，供异步参与者使用；类似取证的访谈，您需要结构化笔记；任何原本需要 Fireflies / Otter / Grain 的场景。当您不希望有 AI 监听时 —— 不要启用它。

**禁用：** `hermes plugins disable google_meet`。任何缓存的转录稿和录音将保留在 `~/.hermes/cache/google_meet/` 中，直到您将其删除。

### hermes-achievements

向仪表板添加一个 **Steam 风格的成就标签页** —— 从您真实的 Hermes 会话历史中生成的 60 多个可收集、分级的徽章。工具链壮举、调试模式、氛围编码连胜、技能/记忆使用情况、模型/提供商多样性、生活风格怪癖（周末和夜间会话）。最初由 [@PCinkusz](https://github.com/PCinkusz) 作为外部插件创作；现已纳入主仓库，以便与 Hermes 功能变更保持同步。

**工作原理：**

- 在仪表板后端扫描您整个 `~/.hermes/state.db` 会话历史
- 每个会话的统计数据通过 `(started_at, last_active)` 指纹进行缓存，因此只有新的或更改的会话会在后续扫描中重新分析
...
- **冷扫描期间的增量结果** — 扫描器大约每处理 250 个会话发布一次部分快照，因此随着扫描的进行，每次仪表板刷新都会显示更多已解锁的徽章。无需长时间盯着零进度。
...
**选择退出：** 删除或重命名 `plugins/hermes-achievements/dashboard/manifest.json`，或者用 `~/.hermes/plugins/hermes-achievements/` 中同名的用户插件覆盖它，该插件不提供仪表板。插件在 `$HERMES_HOME/plugins/hermes-achievements/` 下的状态文件会保留 — 重新安装会保留你的解锁历史。

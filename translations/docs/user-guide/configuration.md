...
默认情况下，每个终端命令都在其自己的子进程中运行——工作目录、环境变量和 shell 变量在命令之间重置。当启用**持久化 Shell** 时，一个长期存活的 bash 进程会在 `execute()` 调用之间保持活动状态，以便状态在命令之间得以保留。
...
- `hermes config migrate` 扫描所有启用的技能，查找未配置的设置，并提示你进行配置
...
Agent 还会自动对文件读取进行去重——如果同一文件区域被读取两次且文件未更改，则返回轻量级存根，而不是重新发送内容。这在上下文压缩后会重置，以便 Agent 可以在其内容被摘要化后重新读取文件。
...
:::warning 摘要模型上下文长度要求
摘要模型的上下文窗口**必须**至少与你的主 Agent 模型一样大。压缩器将对话的整个中间部分发送给摘要模型——如果该模型的上下文窗口小于主模型的，摘要调用将因上下文长度错误而失败。发生这种情况时，中间轮次的对话将被**丢弃而不进行摘要**，从而静默地丢失对话上下文。如果你覆盖了模型，请验证其上下文长度满足或超过主模型的上下文长度。
:::
...
LLM 流式传输连接有两层超时。两者都会为本地提供商（localhost、LAN IP）自动调整——对于大多数设置无需配置。
...
**套接字读取超时**控制 httpx 等待提供商下一个数据块的时间。本地 LLM 在产生第一个 Token 之前，可能需要几分钟时间在大型上下文上进行预填充，因此当 Hermes 检测到本地端点时，会将此超时提升至 30 分钟。如果你显式设置了 `HERMES_STREAM_READ_TIMEOUT`，则无论端点检测结果如何，都将始终使用该值。
...
辅助任务可用的提供商：`auto`、`openrouter`、`nous`、`codex`、`copilot`、`anthropic`、`main`、`zai`、`kimi-coding`、`kimi-coding-cn`、`arcee`、`minimax`、[提供商注册表](/docs/reference/environment-variables)中注册的任何提供商，或你的 `custom_providers` 列表中的任何命名自定义提供商（例如 `provider: "beans"`）。
...
2. **OpenAI 执行规范**（仅限 GPT 和 Codex 模型）——针对 GPT 特有故障模式的额外指导：放弃处理部分结果、跳过先决条件查找、产生幻觉而不使用工具、以及未经验证就声明“完成”。
...
**速度回退层级：** 特定提供商的速度（例如 `tts.edge.speed`）→ 全局 `tts.speed` → 默认值 `1.0`。设置全局 `tts.speed` 以在所有提供商间应用统一的速度，或按提供商覆盖以进行细粒度控制。
...
display:
  ...
  tool_progress_overrides:
    ...
    telegram: verbose         # Telegram 上显示详细进度
...
启用后，机器人会在第一个 Token 到达时发送一条消息，然后随着更多 Token 到达而渐进式地编辑它。不支持消息编辑的平台（Signal、Email、Home Assistant）会在首次尝试时自动检测到——流式传输会为该会话优雅地禁用，不会导致消息泛滥。

对于无需渐进式 Token 编辑的、自然的回合中助手更新，请设置 `display.interim_assistant_messages: true`。
...
定义自定义命令，这些命令运行 shell 命令而无需调用 LLM——零 Token 消耗，即时执行。在消息平台（Telegram、Discord 等）上尤其有用，可用于快速服务器检查或实用脚本。
...
discord:
  ...
  free_response_channels: ""     # 逗号分隔的频道 ID 列表，在此列表中 bot 无需 @提及 即可响应每条消息
  auto_thread: true              # 在频道中 @提及 时自动创建线程
```

- `require_mention` — 为 `true`（默认）时，bot 仅在服务器频道中被 `@BotName` 提及时才响应。私信始终无需提及即可工作。
- `free_response_channels` — 逗号分隔的频道 ID 列表，在此列表中 bot 无需提及即可响应每条消息。
...
共享文件每行包含一个域名规则（空行和以 `#` 开头的注释行将被忽略）。缺失或不可读的文件会记录警告，但不会禁用其他网络工具。

...
对于 Modal 和 Daytona 等云沙盒，`container_persistent: true` 意味着 Hermes 将尝试在沙盒重新创建过程中保留文件系统状态。它不保证同一个活动沙盒、PID 空间或后台进程稍后仍在运行。
...
compression:
  ...
  threshold: 0.50                                   # 在上下文限制达到此百分比时触发压缩
...
:::warning 摘要模型上下文长度要求
摘要模型的上下文窗口**必须**至少与你的主 Agent 模型一样大。压缩器将对话的完整中间部分发送给摘要模型——如果该模型的上下文窗口小于主模型的，摘要调用将因上下文长度错误而失败。发生这种情况时，中间轮次的对话将被**丢弃而不进行摘要**，从而静默地丢失对话上下文。如果你覆盖了模型，请验证其上下文长度是否达到或超过主模型的。
:::
...
**Socket 读取超时**控制 httpx 等待提供商下一个数据块的时间。本地 LLM 在产生第一个 Token 之前，可能需要几分钟的时间来预填充大型上下文，因此当 Hermes 检测到本地端点时，会将此超时提升至 30 分钟。如果你显式设置了 `HERMES_STREAM_READ_TIMEOUT`，则无论端点检测结果如何，都将始终使用该值。
...
2. **OpenAI 执行规范**（仅限 GPT 和 Codex 模型）—— 针对 GPT 特有故障模式的额外指导：放弃处理部分结果、跳过先决条件查找、产生幻觉而不使用工具、以及未经验证就宣布“完成”。
...
不同平台对详细程度的需求不同。例如，Signal 无法编辑消息，因此每个进度更新都会成为一条独立的消息 —— 很嘈杂。使用 `tool_progress_overrides` 来设置每个平台的模式：
...
`interim_assistant_messages` 仅适用于网关。启用后，Hermes 将已完成的中途助手更新作为单独的聊天消息发送。这独立于 `tool_progress`，并且不需要网关流式传输。
...
voice:
  ...
  auto_tts: false               # 启用 /voice on 时自动进行语音回复
...
启用后，机器人会在第一个 Token 到达时发送一条消息，然后随着更多 Token 到达而渐进式地编辑它。不支持消息编辑的平台（Signal、Email、Home Assistant）会在首次尝试时自动检测到——流式传输会为该会话优雅地禁用，而不会产生消息洪流。

对于无需渐进式 Token 编辑的、自然的中间回合助手更新，请设置 `display.interim_assistant_messages: true`。
...
定义自定义命令，这些命令运行 shell 命令而无需调用 LLM——零 Token 使用，即时执行。对于快速服务器检查或实用脚本，这在消息平台（Telegram、Discord 等）上尤其有用。
...
用法：在 CLI 或任何消息平台中键入 `/status`、`/disk`、`/update` 或 `/gpu`。该命令在主机上本地运行并直接返回输出——无需 LLM 调用，不消耗 Token。
...
- `tirith_enabled` — 为 `true` 时，终端命令在执行前由 [Tirith](https://github.com/StackGuardian/tirith) 扫描，以检测潜在的危险操作。
...
共享文件每行包含一个域名规则（空行和以 `#` 开头的注释行会被忽略）。缺失或不可读的文件会记录警告，但不会禁用其他网络工具。

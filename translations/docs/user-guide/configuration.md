...
运行 `hermes setup --portal` — 一次 OAuth 即可获得模型提供商和所有四个工具网关工具，无需手动编辑 YAML。Portal 订阅者还可享受按 Token 计费的提供商 10% 的折扣。请参阅 [Nous Portal](/integrations/nous-portal)。
:::
...
updates:
  ...
  backup_keep: 5                 # 保留此数量的更新前备份 zip 文件
...
Hermes 支持六种终端后端。每种都决定了 Agent 的 shell 命令实际执行的位置 — 您的本地机器、Docker 容器、通过 SSH 的远程服务器、Modal 云沙盒（直接或通过 Nous 管理的网关）、Daytona 工作区或 Singularity/Apptainer 容器。
...
```
对于 Modal 和 Daytona 等云沙盒，`container_persistent: true` 意味着 Hermes 将尝试在沙盒重建时保留文件系统状态。它并不保证同一个活动沙盒、PID 空间或后台进程稍后仍在运行。
...
| **docker** | 单个持久化 Docker 容器（跨会话、`/new`、子 Agent 共享） | 完全（命名空间、cap-drop） | 安全沙盒、CI/CD |
...
**单个持久化容器，跨 Hermes 进程共享。** Hermes 在首次使用时启动一个长期运行的容器，并通过 `docker exec` 将每个终端、文件和 `execute_code` 调用路由到同一个容器中——跨会话、`/new`、`/reset` 和 `delegate_task` 子 Agent。工作目录更改、已安装的包、`/workspace` 中的文件以及**后台进程**都会从一个工具调用延续到下一个，从一个 Hermes 进程延续到下一个。当您关闭 TUI 会话、运行 `/quit` 或启动新的 `hermes` 调用时，容器会继续运行，下一个 Hermes 进程通过标签查找重新使用它。有关确切的销毁规则，请参见下面的**容器生命周期**。
...
terminal:
  ...
  # 跨进程容器重用（默认值与“跨会话共享的单个长期运行容器”约定匹配——参见容器生命周期）。
  ...
  lifetime_seconds: 300            # 空闲回收窗口；也作为 2× 孤儿回收器的阈值输入
```

**`docker_env`** 与 **`docker_forward_env`**：前者注入您在配置中指定的字面量 `KEY=value` 对（值存在于您的 `config.yaml` 中或通过 `TERMINAL_DOCKER_ENV='{"DEBUG":"1"}'` 作为 JSON 字典传递）。后者从您的 shell 或 `~/.hermes/.env` 转发值，因此实际的密钥永远不会出现在配置文件中。对 Token 使用 `docker_forward_env`，对容器需要的静态开关使用 `docker_env`。

**`terminal.docker_extra_args`**（也可通过 `TERMINAL_DOCKER_EXTRA_ARGS='["--gpus=all"]'` 覆盖）允许您传递 Hermes 未作为一等键公开的任意 `docker run` 标志——`--gpus`、`--network`、`--add-host`、替代的 `--security-opt` 覆盖等。每个条目必须是字符串；该列表最后附加到组装的 `docker run` 调用中，以便在需要时可以覆盖 Hermes 的默认值。请谨慎使用——与沙盒加固（能力丢弃、`--user`、工作空间绑定挂载）冲突的标志会静默地削弱隔离性。

**要求：** 已安装并运行 Docker Desktop 或 Docker Engine。Hermes 会探测 `$PATH` 加上常见的 macOS 安装位置（`/usr/local/bin/docker`、`/opt/homebrew/bin/docker`、Docker Desktop 应用程序包）。Podman 开箱即用：当两者都安装时，设置 `HERMES_DOCKER_BINARY=podman`（或完整路径）以强制使用它。

#### 容器生命周期

每个由 Hermes 管理的容器都标有三个标签，以便后续进程（以及孤儿回收器）能够识别它：

- `hermes-agent=1` —— 标记为由 Hermes 管理
- `hermes-task-id=<sanitized task_id>` —— 作为按任务重用探测的键
- `hermes-profile=<sanitized profile name>` —— 将重用和回收范围限定到活动的 Hermes 配置文件

启动时，Hermes 运行 `docker ps --filter label=hermes-task-id=<id> --filter label=hermes-profile=<profile>`，并在找到一个容器时**附加到现有的容器**。如果容器处于 `exited` 状态（例如，在 Docker 守护进程重启后），它会通过 `docker start` 启动并被重用——文件系统状态和任何已安装的包会保留，但容器内的后台进程不会。

当 Hermes 进程退出时——无论是通过 `/quit`、关闭 TUI 会话、消息网关关闭，甚至是 SIGKILL——在默认模式下，容器的清理路径是**无操作的**。容器继续运行。下一个 Hermes 进程通过标签探测在几毫秒内附加到它。这是“跨会话共享的单个长期运行容器”约定所要求的行为：这是后台进程（npm 监视器、开发服务器、长时间运行的 pytest）能够跨会话存活的唯一方式。
...
| 空闲回收器 (`lifetime_seconds`，默认 300 秒) | 仅当执行环境设置为 `persist_across_processes=false` 时触发。持久化模式的执行环境会被忽略；容器在空闲扫描中存活。 |
| 下次启动时的孤儿回收器 | 扫描**已退出**的、标记为 hermes 的、且存活时间超过 `2 × lifetime_seconds`（默认 600 秒 = 10 分钟）的容器，范围限定在当前配置文件中。**正在运行的容器永远不会被触碰**——确保兄弟进程安全。设置 `docker_orphan_reaper: false` 可禁用此功能。 |
| 用户直接操作 | `docker rm -f`、`docker system prune`、Docker Desktop 重启。我们不设置 `--restart=always`，因此主机重启会使容器变为 `Exited` 状态（其 CoW 层会保留并在下次启动时重用，但后台进程会丢失）。 |

值得了解的边缘情况：

- **容器内 PID 1 进程因 OOM 被杀死**会使容器状态变为 `Exited`。下次重用时将执行 `docker start`；文件系统状态保留，后台进程丢失。
- **切换配置文件**会使容器彼此隔离——标记为 `hermes-profile=work` 的容器对于运行在 `hermes-profile=research` 下的 Hermes 进程是不可见的。孤儿回收器也限定在配置文件范围内，因此跨配置文件的容器不会被意外回收，但它们也不会自动清理，除非你在其原始配置文件下再次启动 Hermes。
...
通过 SSH 在远程服务器上运行命令。使用 ControlMaster 实现连接复用（5 分钟空闲保活）。默认启用持久化 shell——状态（当前工作目录、环境变量）在命令之间保留。
...
**持久化：** 启用后，沙盒文件系统在清理时进行快照，并在下次会话时恢复。快照信息记录在 `~/.hermes/modal_snapshots.json` 中。这仅保留文件系统状态，不保留活动进程、PID 空间或后台作业。
...
**持久性：** 启用后，沙盒在清理时会被停止（而非删除），并在下次会话时恢复。沙盒名称遵循 `hermes-{task_id}` 模式。

**磁盘限制：** Daytona 强制最大 10 GiB。超过此限制的请求会被警告并限制。
...
- **SSH** — 必须同时设置 `TERMINAL_SSH_HOST` 和 `TERMINAL_SSH_USER`。如果缺少任一，Hermes 会记录清晰的错误。
...
对于 **SSH**、**Modal** 和 **Daytona** 后端（任何 Agent 的工作树所在机器与运行 Hermes 的主机不同的情况），Hermes 会跟踪 Agent 在远程沙盒内接触过的文件，并在会话拆除 / 沙盒清理时，**将修改过的文件同步回主机**，位置在 `~/.hermes/cache/remote-syncs/<session-id>/` 下。

- 触发时机：会话关闭、`/new`、`/reset`、消息网关消息超时、`delegate_task` 子 Agent 完成且子 Agent 使用了远程后端时。
- 覆盖 Agent 修改的整个树，而不仅仅是它显式打开的文件。新增、编辑和删除都会被捕获。
- 当您去查找时，远程沙盒可能已被拆除；本地的 `~/.hermes/cache/remote-syncs/…` 副本是 Agent 所做更改的权威记录。
...
这就是您如何从会话结束后被销毁的临时云沙盒中恢复结果，而无需告诉 Agent 显式地 `scp` 或 `modal volume put` 每个产物。
...
如果您使用消息网关，并希望 Agent 通过 `MEDIA:/...` 发送生成的文件，建议使用专用的主机可见导出挂载，例如 `/home/user/.hermes/cache/documents:/output`。

- 在 Docker 内将文件写入 `/output/...`
- 在 `MEDIA:` 中发出**主机路径**，例如：
  `MEDIA:/home/user/.hermes/cache/documents/report.txt`
- **不要**发出 `/workspace/...` 或 `/output/...`，除非该确切路径也存在于主机上的消息网关进程中
...
默认情况下，Docker 容器以 `root`（UID 0）身份运行。在 `/workspace` 或其他绑定挂载目录内创建的文件在主机上最终归 root 所有，因此在会话结束后，您必须使用 `sudo chown` 命令更改所有权，才能从主机编辑器编辑它们。`terminal.docker_run_as_host_user` 标志可以解决此问题：
...
启用后，Hermes 会在 `docker run` 命令后附加 `--user $(id -u):$(id -g)`，这样写入绑定挂载目录（`/workspace`、`/root` 以及 `docker_volumes` 中的任何目录）的文件将由您的主机用户拥有，而不是 root。代价是：容器将无法再执行 `apt install` 或写入 root 拥有的路径（如 `/root/.npm`）——如果您两者都需要，请使用其 `HOME` 目录归非 root 用户拥有的基础镜像（或在构建镜像时添加所需的工具）。

为了保持向后兼容性，请将此选项保留为 `false`（默认值）。当您的工作流主要是“编辑挂载的主机文件”并且厌倦了 `sudo chown -R` 时，请将其打开。
...
禁用时，除非您通过 `docker_volumes` 显式挂载某些内容，否则 `/workspace` 将保持沙盒所有。

安全权衡：
- `false` 保留沙盒边界
- `true` 使沙盒可以直接访问您启动 Hermes 的目录
...
当 Agent 使用 `skill_manage` 创建、编辑、修补或删除技能时，Hermes 可以选择性地扫描新/更新的内容，查找危险的关键字模式（凭据窃取、明显的提示词注入、外泄指令）。扫描器**默认关闭**——因为合法的 Agent 工作流（确实会触及 `~/.ssh/` 或提及 `$OPENAI_API_KEY`）过去经常触发启发式规则。如果您希望扫描器在 Agent 的技能写入生效之前提示您，请重新将其打开：
...
启用后，任何被标记的 `skill_manage` 写入操作都会显示为带有扫描器理由的批准提示。接受的写入会生效；拒绝的写入会向 Agent 返回解释性错误。
...
控制单个 `read_file` 调用可以返回多少内容。超过限制的读取会被拒绝，并返回错误，告知 Agent 使用 `offset` 和 `limit` 来读取更小的范围。这可以防止读取一个压缩的 JS 包或大型数据文件时，单个读取操作就淹没上下文窗口。
...
- **`max_bytes`** — 当 `terminal` 命令产生的 stdout/stderr 组合字符数超过此值时，Hermes 会保留前 40% 和后 60%，并在它们之间插入一个 `[OUTPUT TRUNCATED]` 通知。默认值为 `50000`（根据典型的分词器，大约相当于 12-15K Token）。
...
这会在**每平台工具配置**（由 `hermes tools` 写入的 `platform_toolsets`）**之后**应用，因此此处列出的工具集总是会被移除——即使某个平台的保存配置仍然列出了它。当您想要一个单一的开关来实现“在所有地方关闭 X”，而不是在 `hermes tools` UI 中编辑 15+ 个平台行时，请使用此功能。
...
:::warning 摘要模型上下文长度要求
摘要模型**必须**拥有至少与您的主 Agent 模型一样大的上下文窗口。压缩器会将对话的完整中间部分发送给摘要模型——如果该模型的上下文窗口小于主模型的，摘要调用将因上下文长度错误而失败。发生这种情况时，中间轮次将被**丢弃而不进行摘要**，从而静默地丢失对话上下文。如果您覆盖了模型，请验证其上下文长度是否达到或超过您的主模型。
:::
...
agent:
  ...
  api_max_retries: 3           # 在触发备用机制前，每个提供商的重试次数（默认值：3）
```
...
`agent.api_max_retries` 控制 Hermes 在触发备用提供商切换**之前**，对瞬态错误（速率限制、连接中断、5xx 错误）重试提供商 API 调用的次数。默认值为 `3`——总共尝试四次。如果您配置了[备用提供商](/user-guide/features/fallback-providers)并希望更快地故障转移，请将此值降至 `0`，这样主提供商上的第一个瞬态错误会立即切换到备用提供商，而不是对不稳定的端点进行重试。
...
**Socket 读取超时**控制 httpx 等待来自提供商的下一个数据块的时间。本地 LLM 在产生第一个 Token 之前，可能需要几分钟的时间来对大型上下文进行预填充，因此当 Hermes 检测到本地端点时，会将此值提升至 30 分钟。如果您显式设置了 `HERMES_STREAM_READ_TIMEOUT`，则无论端点检测结果如何，都将始终使用该值。

**陈旧流检测**会终止接收 SSE 保持活动 ping 但没有实际内容的连接。对于本地提供商，此功能完全禁用，因为它们在预填充期间不发送保持活动 ping。
...
当您为同一个提供商拥有多个 API 密钥或 OAuth 令牌时，可以配置轮换策略：
...
没有开关可以禁用此功能——缓存始终开启，即使在单轮对话中也能节省成本，因为仅系统提示词就占输入 Token 数量的相当一部分。
...
早期版本将聚合器用户（OpenRouter、Nous Portal）分流到提供商端默认的廉价模型上。这令人困惑——付费订阅聚合器的用户会看到不同的模型处理他们的辅助流量。现在 `auto` 为所有人使用主模型，并且 `config.yaml` 中的每任务覆盖配置仍然优先（请参阅下面的[完整辅助配置参考](#full-auxiliary-config-reference)）。
:::
...
:::tip xAI Grok OAuth
`xai-oauth` 通过浏览器 OAuth 登录，适用于 SuperGrok 和 X Premium+ 订阅者（无需 API 密钥）。运行 `hermes model` 并选择 **xAI Grok OAuth (SuperGrok / Premium+)** 进行身份验证。相同的 OAuth Token 会复用于每个直接面向 xAI 的界面（聊天、辅助任务、TTS、图像生成、视频生成、转录）。请参阅 [xAI Grok OAuth 指南](../guides/xai-grok-oauth.md)，如果 Hermes 在远程主机上，请参阅 [通过 SSH / 远程主机进行 OAuth](../guides/oauth-over-ssh.md)。
:::

:::warning `"main"` 仅用于辅助任务
`"main"` 提供商选项意味着“使用我的主 Agent 使用的任何提供商” — 它仅在 `auxiliary:`、`compression:` 和主备选条目（`fallback_providers:` 或旧的 `fallback_model:`）内部有效。它**不是**顶层 `model.provider` 设置的有效值。如果你使用自定义的 OpenAI 兼容端点，请在 `model:` 部分设置 `provider: custom`。所有主模型提供商选项请参阅 [AI 提供商](/integrations/providers)。
:::
...
auxiliary:
  ...
  compression:
    timeout: 120               # 秒 — 压缩功能需要总结长对话，需要更多时间
...
当一个辅助任务解析到 OpenRouter（无论是显式设置还是通过 `provider: "main"` 而你的主 Agent 正在使用 OpenRouter），主 Agent 的 `provider_routing` 和 `openrouter.min_coding_score` 设置**不会传播** — 根据设计，每个辅助任务是独立的。要为特定的辅助任务设置 OpenRouter 提供商偏好或使用 [Pareto Code 路由器](/integrations/providers#openrouter-pareto-code-router)，请通过 `extra_body` 按任务设置：
...
某些模型偶尔会将预期操作描述为文本，而不是实际调用工具（例如“我会运行测试...”而不是实际调用终端）。工具使用强制功能会注入系统提示词指导，引导模型回归到实际调用工具的行为。
...
2.  **OpenAI 执行纪律**（仅限 GPT 和 Codex 模型）—— 针对 GPT 特定失败模式的额外指导：放弃对部分结果的工作、跳过先决条件查找、产生幻觉而不是使用工具，以及在未经验证的情况下声明“完成”。
...
**速度回退层次结构：** 特定提供商的速度（例如 `tts.edge.speed`）→ 全局 `tts.speed` → `1.0` 默认值。设置全局 `tts.speed` 以在所有提供商间应用统一速度，或按提供商覆盖以进行细粒度控制。
...
display:
  ...
  interim_assistant_messages: true  # 消息网关：将自然的回合中助手更新作为单独消息发送
  ...
  timestamps: false       # 为 true 时，在 CLI / TUI 记录中为 user 和 assistant 标签添加 [HH:MM] 时间戳前缀
  tool_preview_length: 0  # 工具调用预览的最大字符数（0 = 无限制，显示完整路径/命令）
  runtime_footer:         # 消息网关：在最终回复后附加运行时上下文页脚
    enabled: false
    fields: ["model", "context_pct", "cwd"]
  file_mutation_verifier: true    # 当本回合中 write_file/patch 调用失败时，附加一个提示性页脚
  language: en            # 静态消息的 UI 语言（批准提示、某些网关回复）。en | zh | zh-hant | ja | de | es | fr | tr | uk | af | ko | it | ga | pt | ru | hu
```

### 文件修改验证器

当 `display.file_mutation_verifier` 为 `true`（默认）时，如果在一个回合中 `write_file` 或 `patch` 调用失败，并且从未被对同一路径的成功写入所取代，Hermes 会在助手的最终响应后附加一行提示。这可以捕捉“一批并行补丁，一半静默失败，模型总结成功”这类过度声称的情况，而无需您在每次编辑后手动运行 `git status`。
示例页脚：
...
设置 `file_mutation_verifier: false`（或 `HERMES_FILE_MUTATION_VERIFIER=0`）以隐藏此页脚。验证器仅在回合结束时仍有实际失败时触发——在同一回合内重试并成功修补失败的模型不会触发该文件的验证器。

### 静态消息的 UI 语言

`display.language` 设置翻译一小部分面向用户的静态消息——CLI 批准提示、少数几个消息网关斜杠命令回复（例如重启-排空通知、“批准已过期”、“目标已清除”）。它**不**翻译 Agent 回复、日志行、工具输出、错误回溯或斜杠命令描述——这些内容保持英文。如果你希望 Agent 本身用另一种语言回复，只需在你的提示词或系统消息中告诉它。
...
你也可以通过 `HERMES_LANGUAGE` 环境变量按会话设置此值，它会覆盖配置文件中的值。
...
工具进度需要一个能够安全显示进度更新的网关适配器。不支持消息编辑的平台（包括 Signal）即使 `/verbose` 保存了非 `off` 模式，也会抑制工具进度气泡的显示。

### 运行时元数据页脚（仅限网关）

当 `display.runtime_footer.enabled: true` 时，Hermes 会在每个网关回合的**最终**消息后附加一个小的运行时上下文页脚。当前的页脚可以显示模型、上下文窗口百分比和当前工作目录。默认关闭；如果你的团队希望每个回复都包含此来源信息，请按网关选择启用。
...
没有覆盖设置的平台将回退到全局的 `tool_progress` 值。有效的平台键：`telegram`、`discord`、`slack`、`signal`、`whatsapp`、`matrix`、`mattermost`、`email`、`sms`、`homeassistant`、`dingtalk`、`feishu`、`wecom`、`weixin`、`bluebubbles`、`qqbot`。遗留的 `display.tool_progress_overrides` 键仍会加载以保持向后兼容性，但已弃用，并在首次加载时迁移到 `display.platforms`。

Signal 被列为有效的平台键，因为该设置可以按平台保存，但当前的 Signal 适配器无法编辑已发送的消息，并且不渲染工具进度气泡。请将 Signal 的 `tool_progress` 设置为 `off`；如果你需要实时查看每个工具调用，请使用 CLI 或支持编辑的消息平台。

`interim_assistant_messages` 仅限网关使用。启用后，Hermes 会将回合中已完成的中间助手更新作为单独的聊天消息发送。这独立于 `tool_progress`，并且不需要网关流式传输。
...
当 `redact_pii` 为 `true` 时，网关会在将系统提示词发送给 LLM 之前，从支持的平台中剥离个人可识别信息：
...
**平台支持：** 重写适用于 WhatsApp、Signal 和 Telegram。Discord 和 Slack 被排除在外，因为它们的提及系统（`<@user_id>`）需要在 LLM 上下文中使用真实的 ID。

哈希是确定性的——同一用户始终映射到同一哈希，因此模型仍能区分群聊中的用户。路由和传递在内部使用原始值。
...
streaming:
  ...
  cursor: " ▉"            # 流式传输期间显示的光标
  fresh_final_after_seconds: 60   # 当预览消息达到此时间后发送全新的最终消息（Telegram）；0 = 始终原地编辑
```

启用后，机器人会在第一个 Token 到达时发送一条消息，然后随着更多 Token 到达渐进式地编辑它。不支持消息编辑的平台（Signal、Email、Home Assistant）会在首次尝试时自动检测到——流式传输会优雅地在该会话中禁用，不会导致消息泛滥。
...
**全新最终消息（Telegram）：** Telegram 的 `editMessageText` 会保留原始消息的时间戳，因此一个长时间运行的流式回复即使在完成后也会保持第一个 Token 的时间戳。当 `fresh_final_after_seconds > 0`（默认为 `60`）时，完成的回复会作为一条全新的消息发送（并尽力删除过时的预览消息），这样 Telegram 显示的时间戳就能反映完成时间。简短的预览消息仍会原地最终确定。设置为 `0` 则始终原地编辑。

:::note 各平台流式传输默认值
主开关 `streaming.enabled` 默认为 `false`——在您打开它之前，不会进行任何流式传输。启用后，流式传输的启用与否**按平台决定**：Telegram 默认配置为 `display.platforms.telegram.streaming: true`（启用流式传输），而 Discord 默认配置为 `display.platforms.discord.streaming: false`（不启用）。因此，启用流式传输后，Telegram 会立即启用流式传输，而 Discord 则保持完整消息回复模式，直到您更改其开关。您可以通过仪表板的 **Channels** 开关或直接在 `~/.hermes/config.yaml` 中调整这些按平台的开关。
:::

## 群聊会话隔离

限制在 CLI、TUI/仪表板和消息网关中可以同时打开的聊天会话数量：

```yaml
max_concurrent_sessions: null  # null/0 = 无限制；正整数 = 活动会话上限
```

当达到上限时，Hermes 会为新会话返回一条直接的限制消息。现有的活动会话保持其正常行为。

规范键是顶层的 `max_concurrent_sessions`。Hermes 也接受 `gateway.max_concurrent_sessions` 作为回退，但如果两者都设置了，顶层键优先。

该上限通过本地运行时租约文件强制执行，并且是尽力而为的：如果无法读取或锁定注册表，Hermes 会以失败开放的方式处理，以免用户陷入困境。它适用于单个主机/配置文件的运行时，不适用于跨多台机器共享挂载的 `$HERMES_HOME`。
...
- `false` 恢复为旧的共享房间行为。如果您明确希望 Hermes 将一个频道视为一个协作对话，这可能很有用，但这也意味着用户共享上下文、Token 成本和中断状态。
- 私信不受影响。Hermes 仍会像往常一样按聊天/私信 ID 来区分私信。
- 无论哪种设置，线程都与其父频道保持隔离；当设置为 `true` 时，每个参与者在线程内也拥有自己的会话。
...
- `ignore` 会静默丢弃未经授权的私信。
- 平台配置部分会覆盖全局默认值，因此你可以在保持广泛启用配对的同时，让某个平台保持安静。

## 快速命令

定义自定义命令，这些命令要么在不调用 LLM 的情况下运行 shell 命令，要么将一个斜杠命令别名到另一个。`exec` 类型的快速命令是零 Token 的，在消息平台（Telegram、Discord 等）上用于快速服务器检查或实用脚本非常有用。
...
用法：在 CLI 或任何消息平台中输入 `/status`、`/disk`、`/update`、`/gpu` 或 `/restart`。`exec` 命令在主机本地运行并直接返回输出 —— 无需调用 LLM，不消耗 Token。`alias` 命令会重写为配置的斜杠命令目标。
...
仅包含字符串的提示词快捷方式不是有效的快速命令。对于可重用的提示词工作流，请创建一个技能或别名到现有的斜杠命令。
...
- **`strict`** —— 脚本在临时暂存目录中运行，使用 `sys.executable`（Hermes 自身的 python）。最大程度保证可复现性，但项目依赖和相对路径将无法解析。
...
browser:
  ...
  # 可选的 CDP 覆盖 —— 设置后，Hermes 将直接附加到你自己的
  # Chromium 系列浏览器（通过 /browser connect），而不是启动无头浏览器。
  cdp_url: ""
  # 对话框监督器 —— 控制当附加了 CDP 后端（Browserbase、通过 /browser connect 的本地 Chromium 系列
  # 浏览器）时如何处理原生 JS 对话框（alert / confirm / prompt）。
  # 在 Camofox 和默认的本地 agent-browser 模式下忽略。
  dialog_policy: must_respond    # must_respond | auto_dismiss | auto_accept
  dialog_timeout_s: 300          # 在 must_respond 下的安全自动关闭时间（秒）
  camofox:
...
- `must_respond`（默认）— 捕获对话框，在 `browser_snapshot.pending_dialogs` 中显示，并等待 Agent 调用 `browser_dialog(action=...)`。如果超过 `dialog_timeout_s` 秒没有响应，对话框将自动关闭，以防止页面的 JS 线程永久阻塞。
- `auto_dismiss` — 捕获后立即关闭。事后，Agent 仍能在 `browser_snapshot.recent_dialogs` 中看到对话框记录，其 `closed_by` 字段为 `"auto_policy"`。
...
- `redact_secrets` — 当为 `true` 时，在工具输出进入对话上下文和日志之前，自动检测并脱敏看起来像 API 密钥、Token 和密码的模式。**默认开启**。仅在需要原始类凭证字符串进行调试或脱敏器开发时，才显式设置为 `false`。
...
delegation:
  ...
  max_concurrent_children: 3                # 每批并行子任务数（下限 1，无上限）。也可通过 DELEGATION_MAX_CONCURRENT_CHILDREN 环境变量设置。
...
**Subagent 提供商:模型覆盖：** 默认情况下，subagent 继承父级 Agent 的提供商和模型。设置 `delegation.provider` 和 `delegation.model` 可将 subagent 路由到不同的提供商:模型组合——例如，使用廉价/快速的模型处理范围狭窄的子任务，而主 Agent 运行昂贵的推理模型。

**直接端点覆盖：** 如果需要明显的自定义端点路径，请设置 `delegation.base_url`、`delegation.api_key` 和 `delegation.model`。这将直接把 subagent 发送到该 OpenAI 兼容端点，并优先于 `delegation.provider`。如果省略 `delegation.api_key`，Hermes 仅回退到 `OPENAI_API_KEY`。

**通信协议 (`api_mode`):** Hermes 根据 `delegation.base_url` 自动检测通信协议（例如，以 `/anthropic` 结尾的路径 → `anthropic_messages`；Codex / 原生 Anthropic / Kimi-coding 主机名保持其现有的检测）。对于启发式无法分类的端点——例如 Azure AI Foundry、MiniMax、Zhipu GLM 或前端为 Anthropic 形态后端的 LiteLLM 代理——请将 `delegation.api_mode` 显式设置为 `chat_completions`、`codex_responses` 或 `anthropic_messages` 之一。保持为空（默认）以启用自动检测。
...
**宽度和深度：** `max_concurrent_children` 限制每批并行运行的 subagent 数量（默认 `3`，下限 1，无上限）。也可以通过 `DELEGATION_MAX_CONCURRENT_CHILDREN` 环境变量设置。当模型提交的 `tasks` 数组长度超过限制时，`delegate_task` 会返回一个解释限制的工具错误，而不是静默截断。`max_spawn_depth` 控制委派树的深度（强制限制在 1-3）。在默认值 `1` 时，委派是扁平的：子级不能生成孙级，传递 `role="orchestrator"` 会静默降级为 `leaf`。提高到 `2` 以使协调器子级可以生成叶子孙级；`3` 用于三层树。Agent 通过 `role="orchestrator"` 在每次调用时选择加入协调；`orchestrator_enabled: false` 会强制每个子级回到叶子节点，无论其他设置如何。成本呈乘法级增长——在 `max_spawn_depth: 3` 且 `max_concurrent_children: 3` 时，树最多可达到 3×3×3 = 27 个并发叶子 Agent。有关使用模式，请参阅 [Subagent Delegation → Depth Limit and Nested Orchestration](features/delegation.md#depth-limit-and-nested-orchestration)。
...
`~/.hermes/.env` 中的 `MESSAGING_CWD` 和直接的 `TERMINAL_CWD` 条目是旧版兼容性回退。新配置应使用 `terminal.cwd`。
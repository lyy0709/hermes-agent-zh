...
您可以设置 `providers.<id>.request_timeout_seconds` 作为提供商范围的超时，以及 `providers.<id>.models.<model>.timeout_seconds` 作为模型特定的覆盖。适用于每个传输上的主要回合客户端（OpenAI-wire、原生 Anthropic、Anthropic 兼容）、后备链、凭据轮换后的重建，以及（对于 OpenAI-wire）每个请求的超时关键字参数 — 因此配置的值优先于旧的 `HERMES_API_TIMEOUT` 环境变量。不设置这些值将保留旧的默认值（`HERMES_API_TIMEOUT=1800`s，原生 Anthropic 900s）。目前未为 AWS Bedrock 连接（`bedrock_converse` 和 AnthropicBedrock SDK 路径都使用具有自己超时配置的 boto3）。请参阅 [`cli-config.yaml.example`](https://github.com/NousResearch/hermes-agent/blob/main/cli-config.yaml.example) 中的注释示例。

## 终端后端配置

Hermes 支持六种终端后端。每种决定了 Agent 的 shell 命令实际执行的位置 — 您的本地机器、Docker 容器、通过 SSH 的远程服务器、Modal 云沙盒、Daytona 工作区或 Singularity/Apptainer 容器。
...
对于云沙盒（如 Modal 和 Daytona），`container_persistent: true` 意味着 Hermes 将尝试在沙盒重建过程中保留文件系统状态。它不保证相同的活动沙盒、PID 空间或后台进程稍后仍在运行。
...
:::warning
Agent 拥有与您的用户帐户相同的文件系统访问权限。使用 `hermes tools` 禁用您不需要的工具，或切换到 Docker 进行沙盒化。
:::
...
**凭证转发：** `docker_forward_env` 中列出的环境变量首先从你的 shell 环境解析，然后从 `~/.hermes/.env` 解析。技能也可以声明 `required_environment_variables`，这些变量会自动合并。
...
**隔离：** 使用 `--containall --no-home` 实现完整的命名空间隔离，无需挂载主机主目录。
...
仅在您有意让容器处理主机上的实时文件时，才使用此选择加入功能。
...
控制单个 `read_file` 调用可以返回多少内容。超过限制的读取会被拒绝，并返回错误，告诉 Agent 使用 `offset` 和 `limit` 来读取较小的范围。这可以防止读取压缩的 JS 包或大型数据文件时一次性涌入上下文窗口。
```yaml
file_read_max_chars: 100000  # 默认值 — 约 25-35K Token
```

如果你的模型拥有较大的上下文窗口，并且经常读取大文件，请提高此值。对于小上下文模型，请降低此值以保持读取效率：
...
```
迭代预算压力默认启用。Agent 会自然地看到工具结果中的警告，鼓励它在迭代次数耗尽前整合工作并给出响应。
...
| API 调用（非流式） | 1800秒 | 不变 | `HERMES_API_TIMEOUT` |

**Socket 读取超时** 控制 httpx 等待提供商发送下一个数据块的时间。本地 LLM 在大型上下文上进行预填充可能需要数分钟才能产生第一个 Token，因此当 Hermes 检测到本地端点时，会将此超时提升至 30 分钟。如果你显式设置了 `HERMES_STREAM_READ_TIMEOUT`，则无论端点检测结果如何，都将始终使用该值。

**陈旧流检测** 会终止那些接收到 SSE 保活 ping 但没有实际内容的连接。对于本地提供商，此功能会完全禁用，因为它们在预填充期间不会发送保活 ping。
...
辅助任务可用的提供商：`auto`、`main`，以及[提供商注册表](/docs/reference/environment-variables)中的任何提供商——`openrouter`、`nous`、`openai-codex`、`copilot`、`copilot-acp`、`anthropic`、`gemini`、`google-gemini-cli`、`qwen-oauth`、`zai`、`kimi-coding`、`kimi-coding-cn`、`minimax`、`minimax-cn`、`deepseek`、`nvidia`、`xai`、`ollama-cloud`、`alibaba`、`bedrock`、`huggingface`、`arcee`、`xiaomi`、`kilocode`、`opencode-zen`、`opencode-go`、`ai-gateway`——或者你 `custom_providers` 列表中的任何命名自定义提供商（例如 `provider: "beans"`）。

:::warning `"main"` 仅用于辅助任务
`"main"` 提供商选项意味着“使用我的主 Agent 使用的任何提供商”——它仅在 `auxiliary:`、`compression:` 和 `fallback_model:` 配置内部有效。它**不是**顶级 `model.provider` 设置的有效值。如果你使用自定义的 OpenAI 兼容端点，请在 `model:` 部分设置 `provider: custom`。所有主模型提供商选项请参阅 [AI 提供商](/docs/integrations/providers)。
:::
...
```
:::tip
每个辅助任务都有可配置的 `timeout`（单位：秒）。默认值：vision 120秒，web_extract 360秒，approval 30秒，compression 120秒。如果为辅助任务使用较慢的本地模型，请增加这些值。Vision 还有一个单独的 `download_timeout`（默认 30秒）用于 HTTP 图片下载——对于慢速连接或自托管的图片服务器，请增加此值。
:::
...
要使用 GPT-4o 而不是 Gemini Flash 进行图片分析：
...
:::warning
**Vision 需要一个多模态模型。** 如果您设置 `provider: "main"`，请确保您的端点支持多模态/vision——否则图片分析将失败。
:::
...
当未设置时（默认），推理力度默认为“中等”——这是一个适用于大多数任务的平衡级别。设置一个值会覆盖它——更高的推理力度在复杂任务上能提供更好的结果，但代价是消耗更多 Token 和增加延迟。
...
1. **通用工具使用强制**（所有匹配的模型）——指示模型立即调用工具而非描述意图，持续工作直至任务完成，且绝不以承诺未来行动结束回合。

2. **OpenAI 执行纪律**（仅限 GPT 和 Codex 模型）——额外的指导，针对 GPT 特有的失败模式：放弃处理部分结果、跳过先决条件查询、产生幻觉而非使用工具，以及未经验证就声明“完成”。
...
tts:
  ...
  openai:
    ...
    speed: 1.0                  # 语速乘数（API 限制在 0.25–4.0 范围内）
...
不同平台对详细程度的需求不同。例如，Signal 无法编辑消息，因此每个进度更新都会成为一条单独的消息——很嘈杂。使用 `tool_progress_overrides` 设置按平台的模式：
...
`interim_assistant_messages` 仅适用于网关。启用后，Hermes 会将已完成的回合中助手更新作为单独的聊天消息发送。这独立于 `tool_progress`，并且不需要网关流式传输。
...
当 `redact_pii` 为 `true` 时，网关会在将系统提示词发送给 LLM 之前，从其中删除个人身份信息（在支持的平台上）。
| 字段 | 处理方式 |
|-------|-----------|
...
启用后，响应会在流式传输框内逐个 Token 显示。工具调用仍会被静默捕获。如果提供商不支持流式传输，它会自动回退到正常显示。
...
对于无需渐进式 Token 编辑的、独立的自然回合中助手更新，请设置 `display.interim_assistant_messages: true`。
...
- 线程无论哪种方式都与其父频道保持隔离；当设置为 `true` 时，每个参与者在线程内也拥有自己的会话。
...
定义自定义命令，这些命令运行 shell 命令而无需调用 LLM——零 Token 使用，即时执行。对于从消息平台（Telegram、Discord 等）进行快速服务器检查或实用脚本尤其有用。
...
```
用法：在 CLI 或任何消息平台上输入 `/status`、`/disk`、`/update` 或 `/gpu`。该命令在主机上本地运行并直接返回输出 —— 无需调用 LLM，不消耗 Token。
...
discord:
  ...
  free_response_channels: ""     # 逗号分隔的频道 ID 列表，在此频道中机器人无需 @提及 即可响应
  auto_thread: true              # 在频道中 @提及 时自动创建线程
```

- `require_mention` —— 当为 `true`（默认）时，机器人仅在服务器频道中被 `@BotName` 提及时才响应。私信始终无需提及即可工作。
- `free_response_channels` —— 逗号分隔的频道 ID 列表，在此频道中机器人响应每条消息，无需提及。
...
- `tirith_enabled` —— 当为 `true` 时，终端命令在执行前由 [Tirith](https://github.com/StackGuardian/tirith) 扫描，以检测潜在的危险操作。
...
**子 Agent 提供商:模型覆盖：** 默认情况下，子 Agent 继承父 Agent 的提供商和模型。设置 `delegation.provider` 和 `delegation.model` 可以将子 Agent 路由到不同的提供商:模型组合——例如，在主要 Agent 运行昂贵的推理模型时，为范围狭窄的子任务使用廉价/快速的模型。

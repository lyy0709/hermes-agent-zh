...
- **[IDE 集成 (ACP)](/docs/user-guide/features/acp)** — 在兼容 ACP 的编辑器（如 VS Code、Zed 和 JetBrains）中使用 Hermes Agent。Hermes 作为 ACP 服务器运行，在您的编辑器内渲染聊天消息、工具活动、文件差异和终端命令。
...
Hermes 作为消息网关机器人运行在 15+ 个消息平台上，所有这些都通过同一个 `gateway` 子系统配置：
...
- **[RL 训练](/docs/user-guide/features/rl-training)** — 从 Agent 会话生成轨迹数据，用于强化学习和模型微调。支持具有可定制奖励函数的 Atropos 执行环境。

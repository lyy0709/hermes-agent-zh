...
- **[MCP 服务器](/docs/user-guide/features/mcp)** — 通过 Model Context Protocol 将 Hermes 连接到外部工具服务器。无需编写原生 Hermes 工具，即可访问来自 GitHub、数据库、文件系统、浏览器栈、内部 API 等的工具。支持 stdio 和 SSE 传输、按服务器工具过滤以及支持能力感知的资源/提示词注册。
...
- **[IDE 集成 (ACP)](/docs/user-guide/features/acp)** — 在兼容 ACP 的编辑器（如 VS Code、Zed 和 JetBrains）中使用 Hermes Agent。Hermes 作为 ACP 服务器运行，在编辑器内呈现聊天消息、工具活动、文件差异和终端命令。
...
Hermes 作为消息网关机器人运行在 15+ 个消息平台上，所有平台都通过相同的 `gateway` 子系统配置：

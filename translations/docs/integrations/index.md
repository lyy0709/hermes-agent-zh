...
- **[API 服务器](/docs/user-guide/features/api-server)** — 将 Hermes 暴露为兼容 OpenAI 的 HTTP 端点。任何支持 OpenAI 格式的前端 — Open WebUI、LobeChat、LibreChat、NextChat、ChatBox — 都可以连接并将 Hermes 用作具有完整工具集的后端。
...
- **[RL 训练](/docs/user-guide/features/rl-training)** — 从 Agent 会话生成轨迹数据，用于强化学习和模型微调。支持具有可自定义奖励函数的 Atropos 执行环境。

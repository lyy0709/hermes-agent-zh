...
| **Anthropic** | `hermes model` (通过 Claude Code 授权使用 Claude Pro/Max，或使用 Anthropic API 密钥，或手动设置令牌) |
...
:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）也会使用单独的“辅助”模型——默认通过 OpenRouter 使用 Gemini Flash。设置 `OPENROUTER_API_KEY` 会自动启用这些工具。您也可以配置这些工具使用哪个模型和提供商——请参阅 [辅助模型](/docs/user-guide/configuration#auxiliary-models)。
:::
...
| **`/model`** | 在 Hermes 聊天会话内部 | 在**已配置**的提供商和模型之间快速切换 |

如果您尝试切换到尚未设置的提供商（例如，您只配置了 OpenRouter 但想使用 Anthropic），您需要 `hermes model`，而不是 `/model`。首先退出会话（`Ctrl+C` 或 `/quit`），运行 `hermes model`，完成提供商设置，然后启动新会话。
...
:::warning Token 类型
Copilot API **不**支持传统的个人访问令牌（`ghp_*`）。支持的令牌类型：
...
如果你的 `gh auth token` 返回一个 `ghp_*` 令牌，请改用 `hermes model` 通过 OAuth 进行身份验证。
:::
...
:::note Z.AI 端点自动检测
当使用 Z.AI / GLM 提供商时，Hermes 会自动探测多个端点（全球版、中国版、编程版变体）以找到接受你 API 密钥的端点。你无需手动设置 `GLM_BASE_URL` —— 可用的端点会被自动检测并缓存。
:::

### xAI (Grok) 提示词缓存

当使用 xAI 作为提供商时（任何包含 `x.ai` 的基础 URL），Hermes 会自动启用提示词缓存，方法是在每个 API 请求中发送 `x-grok-conv-id` 请求头。这会将请求路由到同一会话内的同一台服务器，允许 xAI 的基础设施重用缓存的系统提示词和对话历史。
...
:::warning 遗留环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已被**移除**。Hermes 的任何部分都不会读取它们 —— `config.yaml` 是模型和端点配置的唯一真实来源。如果你的 `.env` 中有过时的条目，它们会在下一次 `hermes setup` 或配置迁移时被自动清除。请使用 `hermes model` 或直接编辑 `config.yaml`。
:::
两种方法都会持久化到 `config.yaml`，这是模型、提供商和基础 URL 的单一事实来源。
...
**`/model`**（在活跃的 Hermes 聊天会话中键入）只能**在您已设置好的提供商和模型之间切换**。它不能添加新的提供商、运行 OAuth 或提示输入 API 密钥。如果您只配置了一个提供商（例如 OpenRouter），`/model` 将只显示该提供商的模型。

**要添加新的提供商：** 退出当前会话（`Ctrl+C` 或 `/quit`），运行 `hermes model`，设置新的提供商，然后开始一个新的会话。
:::
...
:::tip
`/model custom`（裸命令，不带模型名称）会查询您端点的 `/models` API，如果恰好加载了一个模型，则自动选择该模型。对于运行单个模型的本地服务器很有用。
:::
...
Ollama 默认**不会**使用您模型的完整上下文窗口。根据您的 VRAM，默认值如下：
...
如果你正在使用**选项 2**（使用主机 IP 的 NAT 模式），Windows 上的模型服务器必须接受来自 `127.0.0.1` 之外的连接。默认情况下，大多数服务器只监听 localhost —— NAT 模式下的 WSL2 连接来自不同的虚拟子网，会被拒绝。在镜像模式下，`localhost` 直接映射，因此默认的 `127.0.0.1` 绑定可以正常工作。
...
BlockRunAI 开发的 [ClawRouter](https://github.com/BlockRunAI/ClawRouter) 是一个本地路由代理，可根据查询复杂度自动选择模型。它根据 14 个维度对请求进行分类，并将其路由到能够处理任务的最便宜模型。支付通过 USDC 加密货币进行（无需 API 密钥）。
...
使用 `hermes model` → 自定义端点，或在 `config.yaml` 中配置以上任何一项：
...
:::tip 何时手动设置
- 您正在使用 Ollama，其自定义 `num_ctx` 低于模型的最大值
- 您希望将上下文限制在模型最大值以下（例如，在 128k 模型上限制为 8k 以节省 VRAM）
- 您正在运行一个不暴露 `/v1/models` 的代理后面
:::
---

### 命名自定义提供商

如果你使用多个自定义端点（例如本地开发服务器和远程 GPU 服务器），可以将它们定义为 `config.yaml` 中的命名自定义提供商：
...
| **企业 / Azure** | Azure OpenAI 搭配自定义端点 |
| **中文 AI 模型** | z.ai (GLM)、Kimi/Moonshot (`kimi-coding` 或 `kimi-coding-cn`)、MiniMax 或小米 MiMo（一流提供商） |

:::tip
你可以随时使用 `hermes model` 在提供商之间切换 — 无需重启。无论使用哪个提供商，你的会话历史、记忆和技能都会保留。
:::
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果你更喜欢在本地运行 Firecrawl，可以改为将 Hermes 指向自托管的实例。完整的设置说明请参阅 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。
...
配置一个备份的 provider:model，当你的主模型失败（速率限制、服务器错误、身份验证失败）时，Hermes 会自动切换到它：
...
激活后，回退模型会在会话中途切换模型和提供商，而不会丢失你的对话。**每个会话最多触发一次**。
...
:::tip
回退模型配置**仅**通过 `config.yaml` 进行 — 没有相关的环境变量。有关其触发条件、支持的提供商以及如何与辅助任务和委派交互的完整详细信息，请参阅 [回退提供商](/docs/user-guide/features/fallback-providers)。
:::
## 智能模型路由

可选的廉价与强力模型路由功能，让 Hermes 能将复杂工作保留给主模型处理，同时将非常简短/简单的对话轮次发送给更廉价的模型。
...
smart_model_routing:
  ...
  cheap_model:
    ...
    # api_key_env: MY_CUSTOM_KEY          # 可选的环境变量名称，用于该端点的 API 密钥
```

工作原理：
- 如果一个对话轮次简短、单行，且看起来不涉及大量代码/工具/调试工作，Hermes 可能会将其路由到 `cheap_model`
- 如果对话轮次看起来复杂，Hermes 会继续使用您的主模型/提供商
- 如果廉价路由无法顺利解析，Hermes 会自动回退到主模型

此功能设计上偏向保守。它适用于快速、低风险的对话轮次，例如：
- 简短的事实性问题
- 快速重写
- 轻量级摘要
...
当您希望在不完全更改默认模型的情况下降低延迟或成本时，请使用此功能。

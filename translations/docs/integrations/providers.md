...
| **xAI Grok OAuth (SuperGrok)** | `hermes model` → 选择 "xAI Grok OAuth (SuperGrok Subscription)" —— 浏览器登录，无需 API 密钥。参见[指南](../guides/xai-grok-oauth.md) |
...
:::warning 政策风险
Google 认为将 Gemini CLI OAuth 客户端与第三方软件一起使用违反了政策。一些用户报告了账户受限的情况。为了获得最低风险的体验，请通过 `gemini` 提供商使用您自己的 API 密钥。Hermes 会在 OAuth 开始前显示明确的警告并要求明确的确认。
:::
...
:::info Codex 说明
OpenAI Codex 提供商通过设备代码进行身份验证（打开一个 URL，输入一个代码）。Hermes 将生成的凭据存储在其自己的身份验证存储中，位于 `~/.hermes/auth.json`，并且可以导入现有的 Codex CLI 凭据（当 `~/.codex/auth.json` 存在时）。无需安装 Codex CLI。

如果 Token 刷新失败并出现终端错误（HTTP 4xx、`invalid_grant`、授权被撤销等），Hermes 会将刷新 Token 标记为失效并停止重放它，这样您就不会看到一连串相同的身份验证失败。下一次请求会显示一个明确的重新身份验证消息。运行 `hermes auth add codex-oauth`（或 `hermes model` → OpenAI Codex）以启动一个新的设备代码登录；隔离状态会在下一次成功交换后清除。
:::

:::warning
即使使用 Nous Portal、Codex 或自定义端点，一些工具（视觉、网页摘要、MoA）也会使用一个单独的“辅助”模型。默认情况下（`auxiliary.*.provider: "auto"`），Hermes 将这些任务路由到您的**主聊天模型**——即您在 `hermes model` 中选择的同一个模型。您可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——请参阅[辅助模型](/docs/user-guide/configuration#auxiliary-models)。
:::
...
如果您尝试切换到尚未设置的提供商（例如，您只配置了 OpenRouter 但想使用 Anthropic），您需要 `hermes model`，而不是 `/model`。首先退出您的会话（`Ctrl+C` 或 `/quit`），运行 `hermes model`，完成提供商设置，然后启动一个新会话。

### Nous Portal

通过 Nous Research 的门户，基于订阅访问 Hermes-4 模型（`Hermes-4-70B`、`Hermes-4.3-36B`、`Hermes-4-405B`）。运行 `hermes model`，选择 **Nous Portal**，通过浏览器登录——Hermes 在 `~/.hermes/auth.json` 存储一个长期有效的刷新 Token。

刷新 Token 也会通过共享的 Token 存储在所有配置文件之间共享，因此在一个配置文件上登录会延续到其他配置文件。

#### Token 处理

Hermes 在每次推理调用时，从您存储的 Nous 刷新 Token 中生成一个短期的 JWT，而不是重复使用一个长期有效的 API 密钥。Token 的生命周期是完全自动的——刷新、生成、在临时 401 错误时重试——您永远不会看到它。

如果门户使刷新 Token 失效（密码更改、手动撤销、会话过期），失效的刷新 Token 会在本地被隔离，这样 Hermes 就会停止重放它，您也不会看到一连串相同的 401 错误。下一次调用会显示清晰的“需要重新身份验证”消息。运行 `hermes auth add nous` 再次登录；隔离状态会在下一次成功登录后清除。
...
当您通过 `hermes model` → Anthropic OAuth（或通过 `hermes auth add anthropic --type oauth`）进行身份验证时，Hermes 会作为 Claude Code 路由到您的 Anthropic 账户。**这仅在您拥有 Claude Max 套餐并购买了额外使用额度时才有效。** 基础 Max 套餐额度（Claude Code 默认包含的使用量）不会被 Hermes 消耗——只有您额外添加的额度/超量额度才会被消耗。Claude Pro 订阅者无法使用此路径。
...
# 使用设置 Token 进行手动覆盖（备用 / 旧方法）
export ANTHROPIC_TOKEN=***  # 设置 Token 或手动 OAuth Token
hermes chat --provider anthropic

# 自动检测 Claude Code 凭据（如果您已经使用 Claude Code）
hermes chat --provider anthropic  # 自动读取 Claude Code 凭据文件
```
当你通过 `hermes model` 选择 Anthropic OAuth 时，Hermes 会优先使用 Claude Code 自身的凭证存储，而不是将 Token 复制到 `~/.hermes/.env` 中。这确保了可刷新的 Claude 凭证能够保持可刷新状态。
...
如果你的 `gh auth token` 返回的是 `ghp_*` Token，请改用 `hermes model` 通过 OAuth 进行认证。
:::
...
当收到 HTTP 401 响应时，Hermes 现在会在回退之前执行一次性的凭证恢复：

1. 通过正常的优先级链重新解析 Token（`COPILOT_GITHUB_TOKEN` → `GH_TOKEN` → `GITHUB_TOKEN` → `gh auth token`）
2. 使用刷新的请求头重建共享的 OpenAI 客户端
3. 重试请求一次

一些较旧的社区代理使用 `api.github.com/copilot_internal/v2/token` 交换流程。该端点可能对某些账户类型不可用（返回 404）。因此，Hermes 将直接 Token 认证作为主要路径，并依赖运行时的凭证刷新和重试来确保鲁棒性。
:::
...
:::note Z.AI 端点自动检测
当使用 Z.AI / GLM 提供商时，Hermes 会自动探测多个端点（全球版、中国版、编码变体）以找到接受你 API 密钥的端点。你无需手动设置 `GLM_BASE_URL` — 可用的端点会被自动检测并缓存。
:::
...
SuperGrok 和 X Premium+ 订阅用户可以使用浏览器 OAuth 登录，而无需使用 API 密钥——在 `hermes model` 中选择 **xAI Grok OAuth (SuperGrok Subscription)**，或运行 `hermes auth add xai-oauth`。相同的 OAuth 持有者令牌会被直接调用 xAI 的工具（TTS、图像生成、视频生成、转录）自动复用。完整流程请参阅 [xAI Grok OAuth 指南](../guides/xai-grok-oauth.md)——如果 Hermes 运行在远程主机上，还需参阅 [通过 SSH / 远程主机进行 OAuth](../guides/oauth-over-ssh.md) 了解所需的 `ssh -L` 隧道。

当使用 xAI 作为提供商（任何包含 `x.ai` 的基础 URL）时，Hermes 会通过在每个 API 请求中发送 `x-grok-conv-id` 标头来自动启用提示词缓存。这会将请求路由到同一会话中的同一服务器，允许 xAI 的基础设施复用缓存的系统提示词和对话历史。
...
模型目录会从 `ollama.com/v1/models` 动态获取并缓存一小时。`model:tag` 表示法（例如 `qwen3-coder:480b-cloud`）通过规范化得以保留——不要使用连字符。
...
通过 AWS Bedrock 使用 Anthropic Claude、Amazon Nova、DeepSeek v3.2、Meta Llama 4 及其他模型。使用 AWS SDK (`boto3`) 凭据链——无需 API 密钥，只需标准的 AWS 认证。
...
Bedrock 底层使用 **Converse API**——请求会被转换为 Bedrock 的模型无关格式，因此相同的配置适用于 Claude、Nova、DeepSeek 和 Llama 模型。仅当你调用非默认的区域端点时才设置 `BEDROCK_BASE_URL`。
...
`qwen-oauth` 使用面向消费者的 Qwen Portal 进行 OAuth 登录——适合个人用户。`alibaba` 提供商使用 Qwen Cloud (Alibaba DashScope) 并需要 `DASHSCOPE_API_KEY`——适合程序化/生产工作负载。两者都路由到 Qwen 系列模型，但位于不同的端点。
:::

### 阿里云 (Coding Plan)

如果你订阅了阿里巴巴的 **Coding Plan**（一个独立于标准 DashScope API 访问的定价 SKU），Hermes 会将其作为一个独立的一级提供商暴露：`alibaba-coding-plan`。端点：`https://coding-intl.dashscope.aliyuncs.com/v1`。它与常规的 `alibaba` 提供商一样是 OpenAI 兼容的，但具有不同的基础 URL 和计费面。
```yaml
model:
...
支持的模型：`MiniMax-M2.7`（主要）和 `MiniMax-M2.7-highspeed`（默认配置为辅助模型）。OAuth 路径忽略 `MINIMAX_API_KEY` / `MINIMAX_BASE_URL`。

:::tip MiniMax OAuth 与 API 密钥
`minimax-oauth` 使用 MiniMax 面向消费者的门户进行 OAuth 登录 —— 无需设置计费。`minimax` 和 `minimax-cn` 提供商使用 `MINIMAX_API_KEY` / `MINIMAX_CN_API_KEY` —— 用于程序化访问。完整指南请参阅 [MiniMax OAuth 指南](/docs/guides/minimax-oauth)。
:::
...
:::tip 本地 NIM
对于本地部署（DGX Spark、本地 GPU），设置 `NVIDIA_BASE_URL=http://localhost:8000/v1`。NIM 暴露与 build.nvidia.com 相同的 OpenAI 兼容的聊天补全 API，因此在云端和本地之间切换只需一行环境变量更改。
:::

Hermes 会自动在每个发往 `build.nvidia.com` 的请求上附加 NIM 计费来源标头 —— 无需配置。这会将消耗路由到 NVIDIA 计费仪表板中的正确来源。
...
:::warning 遗留环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已**移除**。Hermes 的任何部分都不会读取它们 —— `config.yaml` 是模型和端点配置的唯一真实来源。如果你的 `.env` 中有过时的条目，它们会在下一次 `hermes setup` 或配置迁移时自动清除。请使用 `hermes model` 或直接编辑 `config.yaml`。
:::
...
**`hermes model`**（从你的终端运行，在任何聊天会话之外）是**完整的提供商设置向导**。用它来添加新提供商、运行 OAuth 流程、输入 API 密钥和配置自定义端点。
**`/model`**（在活跃的 Hermes 聊天会话中输入）只能**在您已设置好的提供商和模型之间切换**。它无法添加新的提供商、运行 OAuth 或提示输入 API 密钥。如果您只配置了一个提供商（例如 OpenRouter），`/model` 将只显示该提供商的模型。

**要添加新的提供商：** 退出您的会话（`Ctrl+C` 或 `/quit`），运行 `hermes model`，设置新的提供商，然后开始一个新的会话。
:::
...
Ollama 默认**不会**使用您模型的完整上下文窗口。根据您的 VRAM，默认值为：
...
[vLLM](https://docs.vllm.ai/) 是生产环境 LLM 服务的标准。最适合：GPU 硬件上的最大吞吐量、服务大型模型、连续批处理。
...
# 跳过 API 密钥（或者如果您用 --api-key 配置了 vLLM，则输入一个）
...
**工具调用：** 使用 `--tool-call-parser` 并为您模型系列指定相应的解析器：`qwen` (Qwen 2.5)、`llama3`、`llama4`、`deepseekv3`、`mistral`、`glm`。没有此标志，工具调用将以纯文本形式返回。

:::caution SGLang 默认最大输出 Token 数为 128
如果响应看起来被截断，请在请求中添加 `max_tokens` 或在服务器上设置 `--default-max-tokens`。如果请求中未指定，SGLang 默认每个响应只有 128 个 Token。
:::
...
**上下文长度 (`-c`):** 最近的构建默认值为 `0`，这会从 GGUF 元数据中读取模型的训练上下文。对于具有 128k+ 训练上下文的模型，尝试分配完整的 KV 缓存可能会导致 OOM。请根据您的需要明确设置 `-c`（对于 Agent 使用，32k–64k 是一个不错的范围）。如果使用并行槽位 (`-np`)，总上下文将在槽位之间分配 — 使用 `-c 32768 -np 4`，每个槽位只能获得 8k。
...
这将把端点保存到 `config.yaml`，以便在会话之间持久化。

:::caution 工具调用需要 `--jinja`
没有 `--jinja`，llama-server 会完全忽略 `tools` 参数。模型将尝试通过在响应文本中写入 JSON 来调用工具，但 Hermes 不会将其识别为工具调用 — 您会看到像 `{"name": "web_search", ...}` 这样的原始 JSON 被打印为消息，而不是实际的搜索。
...
您可以使用 CLI 来估计模型是否适合：`lms load model-name --context-length 64000 --estimate-only`
...
由于 Hermes Agent 需要 Unix 环境，Windows 用户在 WSL2 内部运行它。如果您的模型服务器（Ollama、LM Studio 等）运行在 **Windows 主机**上，您需要桥接网络差距 — WSL2 使用具有自己子网的虚拟网络适配器，因此 WSL2 内部的 `localhost` 指的是 Linux 虚拟机，**而不是** Windows 主机。

:::tip 两者都在 WSL2 中？没问题。
如果您的模型服务器也运行在 WSL2 内部（对于 vLLM、SGLang 和 llama-server 很常见），`localhost` 可以按预期工作 — 它们共享相同的网络命名空间。跳过本节。
:::
...
如果你使用的是**选项 2**（使用主机 IP 的 NAT 模式），Windows 上的模型服务器必须接受来自 `127.0.0.1` 之外的连接。默认情况下，大多数服务器只监听 localhost —— NAT 模式下的 WSL2 连接来自不同的虚拟子网，会被拒绝。在镜像模式下，`localhost` 直接映射，因此默认的 `127.0.0.1` 绑定可以正常工作。
...
Windows 防火墙将 WSL2 视为一个独立的网络（在 NAT 和镜像模式下都是如此）。如果完成上述步骤后连接仍然失败，请为你的模型服务器端口添加防火墙规则：
...
如果你收到一个列出模型的 JSON 响应，说明配置成功。在 Hermes 配置的 `base_url` 中使用相同的 URL。

---

### 本地模型故障排除

这些问题在 Hermes 使用**所有**本地推理服务器时都会影响。

#### 从 WSL2 到 Windows 主机上的模型服务器出现 "Connection refused"

如果你在 WSL2 内部运行 Hermes，而你的模型服务器在 Windows 主机上，在 WSL2 默认的 NAT 网络模式下，`http://localhost:<port>` 将无法工作。请参阅上面的 [WSL2 网络](#wsl2-networking-windows-users) 部分以获取解决方案。
...
Hermes 从你服务器的 `/v1/models` 端点自动检测上下文长度。如果服务器报告的值很低（或根本不报告），Hermes 会使用模型声明的限制，这可能是错误的。
...
[LiteLLM](https://docs.litellm.ai/) 是一个兼容 OpenAI 的代理，它将 100 多个 LLM 提供商统一在一个 API 之后。最适合：无需更改配置即可在提供商之间切换、负载均衡、故障转移链、预算控制。
...
带故障转移功能的 `litellm_config.yaml` 示例：
```yaml
model_list:
  - model_name: "best"
    litellm_params:
...
[ClawRouter](https://github.com/BlockRunAI/ClawRouter) 由 BlockRunAI 开发，是一个本地路由代理，可根据查询复杂度自动选择模型。它根据 14 个维度对请求进行分类，并将其路由到能够处理任务的最便宜模型。支付使用 USDC 加密货币（无需 API 密钥）。
...
然后在 Hermes 中配置：`hermes model` → 自定义端点 → `http://localhost:8402/v1` → 模型名称 `blockrun/auto`。
...
:::note
ClawRouter 需要在 Base 或 Solana 上拥有一个 USDC 资金钱包用于支付。所有请求都通过 BlockRun 的后端 API 路由。运行 `npx @blockrun/clawrouter doctor` 以检查钱包状态。
:::
...
任何提供兼容 OpenAI API 的服务都可以使用。一些流行的选择：
...
在 `hermes model` → 自定义端点中配置以上任何服务，或在 `config.yaml` 中配置：
...
:::note 两个容易混淆的设置
**`context_length`** 是**总上下文窗口** —— 输入*和*输出 Token 的合计预算（例如 Claude Opus 4.6 为 200,000）。Hermes 使用它来决定何时压缩历史记录以及验证 API 请求。

**`model.max_tokens`** 是**输出上限** —— 模型在*单次响应*中最多可以生成的 Token 数量。它与你的对话历史记录可以有多长无关。行业标准名称 `max_tokens` 是常见的混淆来源；Anthropic 的原生 API 后来为了清晰起见将其重命名为 `max_output_tokens`。
...
如果你需要与多个自定义端点协作（例如，本地开发服务器和远程 GPU 服务器），你可以在 `config.yaml` 中将它们定义为命名自定义提供商：
...
[其他兼容提供商](#other-compatible-providers) 中列出的云提供商都使用 OpenAI 的 REST 方言，因此它们在 `custom_providers:` 下的配置方式相同。以下是三个现成的配置示例。每个都可以放入 `~/.hermes/config.yaml`，相应的 API 密钥则放入 `~/.hermes/.env`。
...
这三个示例可以组合使用 —— 同时使用它们，并通过 `/model custom:<name>:<model>` 在每次对话中切换：
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果您更倾向于在本地运行 Firecrawl，可以将 Hermes 指向一个自托管的实例。完整的设置说明请参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。

**您将获得：** 无需 API 密钥，无速率限制，无按页计费，完全的数据主权。

**您将失去：** 云版本使用 Firecrawl 专有的 "Fire-engine" 来绕过高级反机器人措施（Cloudflare、验证码、IP 轮换）。自托管版本使用基本的 fetch + Playwright，因此一些受保护的网站可能会失败。搜索使用 DuckDuckGo 而不是 Google。
...
-   `min_coding_score` **仅**在 `model.model` 为 `openrouter/pareto-code` 时发送。对于任何其他模型，该值无效。
-   设置为空字符串（或删除该行）以让 OpenRouter 选择当前可用的最强编码器 — 这是省略插件块时其文档记录的行为。
...
-   要将 Pareto 代码路由器用于特定的**辅助任务**（压缩、视觉等）而不是主 Agent，请在该任务下设置 `extra_body.plugins` — 参见 [辅助模型 → OpenRouter 路由和 Pareto 代码用于辅助任务](/docs/user-guide/configuration#openrouter-routing--pareto-code-for-auxiliary-tasks)。

## 备用提供商

配置一个备用提供商链，当主模型失败（速率限制、服务器错误、身份验证失败）时，Hermes 会按顺序尝试这些提供商。规范格式是顶层的 `fallback_providers:` 列表：
...
当激活时，备用链会在会话中途切换模型和提供商，而不会丢失您的对话。链会逐个条目尝试；每个会话的激活是一次性的。
...
:::tip
备用配置**仅**通过 `config.yaml` 进行 — 或通过 `hermes fallback` 交互式配置。有关其触发时机、链如何推进以及如何与辅助任务和委派交互的完整详细信息，请参阅 [备用提供商](/docs/user-guide/features/fallback-providers)。
:::
---

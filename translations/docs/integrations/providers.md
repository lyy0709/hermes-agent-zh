...
| **GitHub Copilot ACP** | `hermes model` (在本地生成 `copilot --acp --stdio`) |
| **Anthropic** | `hermes model` (通过 Claude Code 身份验证使用 Claude Pro/Max，或使用 Anthropic API 密钥，或手动设置令牌) |
...
:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）也会使用单独的“辅助”模型——默认情况下是通过 OpenRouter 的 Gemini Flash。设置 `OPENROUTER_API_KEY` 会自动启用这些工具。您也可以配置这些工具使用哪个模型和提供商——请参阅[辅助模型](/docs/user-guide/configuration#auxiliary-models)。
:::
...
当您通过 `hermes model` 选择 Anthropic OAuth 时，Hermes 优先使用 Claude Code 自身的凭据存储，而不是将令牌复制到 `~/.hermes/.env` 中。这使可刷新的 Claude 凭据保持可刷新状态。
...
这些提供商拥有内置支持及专用的提供商 ID。设置 API 密钥并使用 `--provider` 来选择：
...
:::note Z.AI 端点自动检测
当使用 Z.AI / GLM 提供商时，Hermes 会自动探测多个端点（全球、中国、编程变体）以找到接受你 API 密钥的端点。你无需手动设置 `GLM_BASE_URL` —— 可用的端点会被自动检测并缓存。
:::

### xAI (Grok) 提示词缓存

当使用 xAI 作为提供商（任何包含 `x.ai` 的基础 URL）时，Hermes 会自动启用提示词缓存，方法是在每个 API 请求中发送 `x-grok-conv-id` 请求头。这会将请求路由到同一会话内的同一服务器，使 xAI 的基础设施能够复用缓存的系统提示词和对话历史。
...
Hermes Agent 可与**任何 OpenAI 兼容的 API 端点**协同工作。如果服务器实现了 `/v1/chat/completions`，你就可以让 Hermes 指向它。这意味着你可以使用本地模型、GPU 推理服务器、多提供商路由器或任何第三方 API。
...
:::warning 遗留环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已被**移除**。Hermes 的任何部分都不会读取它们 —— `config.yaml` 是模型和端点配置的唯一真实来源。如果你的 `.env` 中有过时的条目，它们会在下一次运行 `hermes setup` 或配置迁移时被自动清除。请使用 `hermes model` 或直接编辑 `config.yaml`。
:::

这两种方法都会将配置持久化到 `config.yaml`，该文件是模型、提供商和基础 URL 的真实来源。
...
:::tip
`/model custom`（裸命令，无模型名称）会查询你端点的 `/models` API，如果恰好加载了一个模型，则自动选择该模型。对于运行单个模型的本地服务器很有用。
:::
...
### Ollama —— 本地模型，零配置
[Ollama](https://ollama.com/) 通过一条命令在本地运行开放权重模型。最适合：快速本地实验、隐私敏感工作、离线使用。通过 OpenAI 兼容的 API 支持工具调用。
...
**上下文长度：** vLLM 默认读取模型的 `max_position_embeddings`。如果该值超过你的 GPU 内存，它会报错并要求你将 `--max-model-len` 设置得更低。你也可以使用 `--max-model-len auto` 来自动找到适合的最大值。设置 `--gpu-memory-utilization 0.95`（默认为 0.9）以将更多上下文挤入 VRAM。
...
**上下文长度 (`-c`):** 最近的构建默认值为 `0`，这会从 GGUF 元数据中读取模型的训练上下文。对于具有 128k+ 训练上下文的模型，尝试分配完整的 KV 缓存可能会导致 OOM。将 `-c` 显式设置为你需要的值（对于 Agent 使用，32k–64k 是一个不错的范围）。如果使用并行槽位 (`-np`)，总上下文将在槽位之间分配 — 使用 `-c 32768 -np 4` 时，每个槽位只获得 8k。
然后配置 Hermes 指向它：
...
没有 `--jinja`，llama-server 会完全忽略 `tools` 参数。模型会尝试通过在响应文本中写入 JSON 来调用工具，但 Hermes 不会将其识别为工具调用 — 你会看到像 `{"name": "web_search", ...}` 这样的原始 JSON 作为消息打印出来，而不是执行实际的搜索。
...
:::note 两个容易混淆的设置
**`context_length`** 是 **总上下文窗口** —— 输入 *和* 输出 Token 的合计预算（例如 Claude Opus 4.6 的 200,000）。Hermes 使用它来决定何时压缩历史记录以及验证 API 请求。
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果您希望本地运行 Firecrawl，可以将 Hermes 指向自托管的实例。完整的设置说明请参阅 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。

**您将获得：** 无需 API 密钥，无速率限制，无按页计费，完全的数据主权。

**您将失去：** 云版本使用 Firecrawl 专有的 "Fire-engine" 进行高级反机器人绕过（Cloudflare、CAPTCHA、IP 轮换）。自托管版本使用基本的 fetch + Playwright，因此某些受保护的站点可能会失败。搜索使用 DuckDuckGo 而非 Google。
...
使用 OpenRouter 时，您可以控制请求如何在各个提供商之间路由。在 `~/.hermes/config.yaml` 中添加 `provider_routing` 部分：
...
配置一个备用 provider:model，当您的主模型失败时（速率限制、服务器错误、身份验证失败），Hermes 会自动切换到它：
...
激活后，备用模型会在会话中途切换模型和提供商，而不会丢失您的对话。**每个会话最多触发一次**。
...
可选的廉价与强大模型路由功能，让 Hermes 将复杂工作保留给主模型，同时将非常简短/简单的对话轮次发送到更便宜的模型。
...
工作原理：
- 如果对话轮次简短、单行，且看起来不涉及大量代码/工具/调试工作，Hermes 可能会将其路由到 `cheap_model`
...
当您希望降低延迟或成本，但又不想完全更改默认模型时，请使用此功能。

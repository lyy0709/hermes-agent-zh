...
:::warning 政策风险
Google 认为将 Gemini CLI OAuth 客户端与第三方软件一起使用违反了其政策。一些用户报告了账户受限的情况。为了获得风险最低的体验，请通过 `gemini` 提供商使用您自己的 API 密钥。Hermes 会在 OAuth 开始前显示明确警告并要求明确确认。
:::
...
:::info Codex 说明
OpenAI Codex 提供商通过设备码进行身份验证（打开一个 URL，输入一个代码）。Hermes 将生成的凭据存储在其自己的身份验证存储中，位于 `~/.hermes/auth.json`，并且可以导入 `~/.codex/auth.json` 中现有的 Codex CLI 凭据（如果存在）。无需安装 Codex CLI。
:::
...
:::tip Nous 工具网关
付费的 Nous Portal 订阅者还可以访问**[工具网关](/docs/user-guide/features/tool-gateway)**——通过网络搜索、图像生成、TTS 和浏览器自动化，这些功能都通过您的订阅路由。无需额外的 API 密钥。在 `hermes model` 设置期间会自动提供，或者稍后通过 `hermes tools` 启用。
:::
...
如果您尝试切换到尚未设置的提供商（例如，您只配置了 OpenRouter 并想使用 Anthropic），您需要 `hermes model`，而不是 `/model`。首先退出您的会话（`Ctrl+C` 或 `/quit`），运行 `hermes model`，完成提供商设置，然后开始一个新的会话。
...
当您通过 `hermes model` → Anthropic OAuth（或通过 `hermes auth add anthropic --type oauth`）进行身份验证时，Hermes 会以 Claude Code 的身份路由到您的 Anthropic 账户。**这仅在您拥有 Claude Max 套餐并购买了额外使用额度时才有效。** 基础 Max 套餐额度（Claude Code 默认包含的使用量）不会被 Hermes 消耗——只有您额外添加的超额额度才会被消耗。Claude Pro 订阅者无法使用此路径。
...
如果您的 `gh auth token` 返回的是 `ghp_*` 令牌，请改用 `hermes model` 通过 OAuth 进行身份验证。
:::

:::info Copilot 在 Hermes 中的身份验证行为
Hermes 将受支持的 GitHub 令牌（`gho_*`、`github_pat_*` 或 `ghu_*`）直接发送到 `api.githubcopilot.com`，并包含 Copilot 特定的请求头（`Editor-Version`、`Copilot-Integration-Id`、`Openai-Intent`、`x-initiator`）。

当收到 HTTP 401 响应时，Hermes 现在会在回退之前执行一次性的凭据恢复：

1.  通过正常的优先级链重新解析令牌（`COPILOT_GITHUB_TOKEN` → `GH_TOKEN` → `GITHUB_TOKEN` → `gh auth token`）
2.  使用刷新的请求头重建共享的 OpenAI 客户端
3.  重试请求一次

一些较旧的社区代理使用 `api.github.com/copilot_internal/v2/token` 交换流程。该端点可能对某些账户类型不可用（返回 404）。因此，Hermes 将直接令牌身份验证作为主要路径，并依赖运行时凭据刷新 + 重试机制来确保鲁棒性。
:::
...
xAI 通过 Responses API（`codex_responses` 传输）连接，以在 Grok 4 模型上自动支持推理 —— 无需 `reasoning_effort` 参数，服务器默认进行推理。在 `~/.hermes/.env` 中设置 `XAI_API_KEY`，并在 `hermes model` 中选择 xAI，或者将 `grok` 作为快捷方式放入 `/model grok-4-1-fast-reasoning`。

SuperGrok 订阅者可以使用浏览器 OAuth 登录，而不是使用 API 密钥 —— 在 `hermes model` 中选择 **xAI Grok OAuth (SuperGrok Subscription)**，或者运行 `hermes auth add xai-oauth`。相同的 OAuth 承载令牌会自动被直接调用 xAI 的工具（TTS、图像生成、视频生成、转录）复用。完整流程请参阅 [xAI Grok OAuth 指南](../guides/xai-grok-oauth.md) —— 如果 Hermes 运行在远程主机上，还请参阅 [通过 SSH / 远程主机的 OAuth](../guides/oauth-over-ssh.md) 了解所需的 `ssh -L` 隧道。

当使用 xAI 作为提供商时（任何包含 `x.ai` 的基础 URL），Hermes 会自动启用提示词缓存，方法是在每个 API 请求中发送 `x-grok-conv-id` 请求头。这会将请求路由到同一会话内的同一台服务器，允许 xAI 的基础设施复用缓存的系统提示词和对话历史。

无需配置 —— 当检测到 xAI 端点且会话 ID 可用时，缓存会自动激活。这减少了多轮对话的延迟和成本。

xAI 还提供了一个专用的 TTS 端点（`/v1/tts`）。在 `hermes tools` → Voice & TTS 中选择 **xAI TTS**，或参阅 [语音与 TTS](../user-guide/features/tts.md#text-to-speech) 页面进行配置。

### NovitaAI

[NovitaAI](https://novita.ai) 是面向构建者和 Agent 的 AI 原生云。其三条产品线分别是：提供 200+ 模型的 Model API、用于构建和运行 AI Agent 的 Agent Sandbox，以及用于可扩展计算的 GPU Cloud，所有这些都可以从一个平台获得。
```bash
# 使用任意可用模型
hermes chat --provider novita --model moonshotai/kimi-k2.5
...
[Ollama Cloud](https://ollama.com/cloud) 托管着与本地 Ollama 相同的开放权重模型目录，但无需 GPU 要求。在 `hermes model` 中选择 **Ollama Cloud**，从 [ollama.com/settings/keys](https://ollama.com/settings/keys) 粘贴你的 API 密钥，Hermes 会自动发现可用模型。
...
两者都使用相同的 OpenAI 兼容 API。Cloud 是一级提供商（`--provider ollama-cloud`, `OLLAMA_API_KEY`）；本地 Ollama 通过自定义端点流程访问（基础 URL `http://localhost:11434/v1`，无需密钥）。使用 Cloud 来运行本地无法运行的大型模型；使用本地版本用于隐私或离线工作。
:::

### AWS Bedrock

通过 AWS Bedrock 使用 Anthropic Claude、Amazon Nova、DeepSeek v3.2、Meta Llama 4 及其他模型。使用 AWS SDK (`boto3`) 凭证链 — 无需 API 密钥，只需标准 AWS 认证。
...
`qwen-oauth` 使用面向消费者的 Qwen Portal 进行 OAuth 登录 — 适合个人用户。`alibaba` 提供商使用 DashScope 的企业 API 和 `DASHSCOPE_API_KEY` — 适合程序化/生产工作负载。两者都路由到 Qwen 系列模型，但位于不同的端点。
:::

### Alibaba Coding Plan

如果你订阅了阿里巴巴的 **Coding Plan**（一个独立于标准 DashScope API 访问的定价 SKU），Hermes 将其作为自己的一级提供商公开：`alibaba-coding-plan`。端点：`https://coding-intl.dashscope.aliyuncs.com/v1`。它与常规的 `alibaba` 提供商一样是 OpenAI 兼容的，但具有不同的基础 URL 和计费面。
...
通过浏览器 OAuth 登录使用 MiniMax-M2.7 — 无需 API 密钥。在 `hermes model` 中选择 **MiniMax (OAuth)**，通过浏览器登录，Hermes 将持久化访问令牌和刷新令牌。在底层使用 Anthropic Messages 兼容端点 (`/anthropic`)。
...
支持的模型：`MiniMax-M2.7`（主模型）和 `MiniMax-M2.7-highspeed`（默认配置为辅助模型）。OAuth 路径忽略 `MINIMAX_API_KEY` / `MINIMAX_BASE_URL`。
...
**上下文长度：** vLLM 默认读取模型的 `max_position_embeddings`。如果该值超过你的 GPU 内存，它会报错并要求你将 `--max-model-len` 设置得更低。你也可以使用 `--max-model-len auto` 来自动找到适合的最大值。设置 `--gpu-memory-utilization 0.95` (默认 0.9) 以将更多上下文挤入 VRAM。
...
如果响应似乎被截断，请在请求中添加 `max_tokens` 或在服务器上设置 `--default-max-tokens`。如果请求中未指定，SGLang 的默认值仅为每个响应 128 个 tokens。
:::
...
**上下文长度 (`-c`):** 最近的构建默认值为 `0`，这会从 GGUF 元数据中读取模型的训练上下文。对于具有 128k+ 训练上下文的模型，尝试分配完整的 KV 缓存可能会导致 OOM。将 `-c` 显式设置为你需要的值 (对于 Agent 使用，32k–64k 是一个不错的范围)。如果使用并行槽 (`-np`)，总上下文会在槽之间分配 — 使用 `-c 32768 -np 4`，每个槽只获得 8k。
然后配置 Hermes 指向它：
...
这会将端点保存到 `config.yaml` 中，以便在会话之间持久化。

:::caution 工具调用需要 `--jinja`
如果没有 `--jinja`，llama-server 会完全忽略 `tools` 参数。模型会尝试在其响应文本中写入 JSON 来调用工具，但 Hermes 不会将其识别为工具调用 — 你会看到像 `{"name": "web_search", ...}` 这样的原始 JSON 被打印为消息，而不是执行实际的搜索。

原生工具调用支持（最佳性能）：Llama 3.x、Qwen 2.5（包括 Coder）、Hermes 2/3、Mistral、DeepSeek、Functionary。所有其他模型使用通用的处理程序，可以工作但效率可能较低。完整列表请参阅 [llama.cpp 函数调用文档](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md)。
...
[LM Studio](https://lmstudio.ai/) 是一个用于运行本地模型的桌面应用，带有图形界面。最适合：喜欢可视化界面的用户、快速模型测试、macOS/Windows/Linux 上的开发者。
...
1. 点击模型选择器旁边的齿轮图标
2. 将 "Context Length" 设置为至少 64000 以获得流畅的体验
3. 重新加载模型以使更改生效
...
:::tip 两者都在 WSL2 内？没问题。
如果你的模型服务器也运行在 WSL2 内（对于 vLLM、SGLang 和 llama-server 很常见），`localhost` 可以正常工作 — 它们共享相同的网络命名空间。跳过本节。
:::
...
Windows 防火墙将 WSL2 视为一个独立的网络（无论是在 NAT 模式还是镜像模式下）。如果完成上述步骤后连接仍然失败，请为你的模型服务器端口添加防火墙规则：
...
如果收到列出模型的 JSON 响应，说明配置成功。在 Hermes 配置中使用相同的 URL 作为 `base_url`。
...
如果你在 WSL2 内运行 Hermes，而模型服务器在 Windows 主机上，在 WSL2 默认的 NAT 网络模式下，`http://localhost:<port>` 将无法工作。请参阅上文的 [WSL2 网络](#wsl2-networking-windows-users) 部分以获取解决方案。
...
**修复：** 对于 Agent 使用，将上下文设置为至少 **32,768 个 Token**。请参阅上文每个服务器部分的具体标志。
...
**可能的原因：**
1.  **服务器上的输出上限（`max_tokens`）过低** — SGLang 默认每个响应为 128 个 Token。在服务器上设置 `--default-max-tokens` 或在 config.yaml 中配置 Hermes 的 `model.max_tokens`。注意：`max_tokens` 仅控制响应长度 — 它与你的对话历史可以有多长（即 `context_length`）无关。
...
[LiteLLM](https://docs.litellm.ai/) 是一个兼容 OpenAI 的代理，将 100 多个 LLM 提供商统一在一个 API 后面。最适合：无需更改配置即可在提供商之间切换、负载均衡、故障转移链、预算控制。
...
BlockRunAI 的 [ClawRouter](https://github.com/BlockRunAI/ClawRouter) 是一个本地路由代理，根据查询复杂度自动选择模型。它根据 14 个维度对请求进行分类，并将其路由到能够处理任务的最便宜模型。支付通过 USDC 加密货币进行（无需 API 密钥）。
...
:::note
ClawRouter 需要在 Base 或 Solana 上拥有 USDC 资金的钱包进行支付。所有请求都通过 BlockRun 的后端 API 路由。运行 `npx @blockrun/clawrouter doctor` 以检查钱包状态。
:::
---

### 其他兼容的提供商

任何提供 OpenAI 兼容 API 的服务均可使用。一些流行的选择：
...
:::note 两个容易混淆的设置
**`context_length`** 是 **总上下文窗口** —— 输入 *和* 输出 Token 的合计预算（例如 Claude Opus 4.6 为 200,000）。Hermes 使用此值来决定何时压缩历史记录以及验证 API 请求。
...
对于大多数设置，这可以开箱即用。该系统具有提供商感知能力 —— 同一模型根据服务提供商的不同可能具有不同的上下文限制（例如，`claude-opus-4.6` 在 Anthropic 直接访问时为 1M，但在 GitHub Copilot 上为 128K）。
...
[其他兼容的提供商](#其他兼容的提供商) 中列出的云提供商都使用 OpenAI 的 REST 协议，因此它们在 `custom_providers:` 下的配置方式相同。以下是三个配置示例。每个都可以放入 `~/.hermes/config.yaml`，匹配的 API 密钥则放入 `~/.hermes/.env`。
...
以上三种配置可以组合使用——将它们一起使用，并通过 `/model custom:<name>:<model>` 在每次对话中切换：
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网络搜索和抓取。如果你更喜欢在本地运行 Firecrawl，可以改为将 Hermes 指向自托管实例。有关完整设置说明，请参阅 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。
...
OpenRouter 提供了一个实验性的编码模型路由器 `openrouter/pareto-code`，它会自动将请求路由到满足编码质量门槛的最便宜模型（根据 [Artificial Analysis](https://artificialanalysis.ai/) 排名）。选择此模型并在 `~/.hermes/config.yaml` 中调整 `min_coding_score` 旋钮：
...
openrouter:
  min_coding_score: 0.65   # 0.0–1.0；数值越高 = 能力越强（也更昂贵）的编码模型。默认值 0.65。
```

注意：

- `min_coding_score` **仅**在 `model.model` 为 `openrouter/pareto-code` 时发送。对于任何其他模型，此值无效。
- 设置为空字符串（或删除该行）以让 OpenRouter 选择当前可用的最强编码模型——这是其文档中描述的当省略插件块时的行为。
...
- 若要将 Pareto Code 路由器用于特定的**辅助任务**（压缩、视觉等）而非主 Agent，请在该任务下设置 `extra_body.plugins`——参见 [辅助模型 → 辅助任务的 OpenRouter 路由和 Pareto Code](/docs/user-guide/configuration#openrouter-routing--pareto-code-for-auxiliary-tasks)。

## 备用提供商

配置一个备用提供商链，当主模型失败（速率限制、服务器错误、认证失败）时，Hermes 会按顺序尝试这些提供商。规范格式是一个顶层的 `fallback_providers:` 列表：

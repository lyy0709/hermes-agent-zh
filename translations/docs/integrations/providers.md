...
:::warning 政策风险
Google 认为将 Gemini CLI OAuth 客户端与第三方软件一起使用违反了政策。一些用户报告了账户受限的情况。为了获得最低风险的体验，请通过 `gemini` 提供商使用您自己的 API 密钥。Hermes 会在 OAuth 开始前显示明确的警告并要求确认。
:::
...
:::info Codex 说明
OpenAI Codex 提供商通过设备码进行身份验证（打开一个 URL，输入一个代码）。Hermes 将生成的凭据存储在其自己的身份验证存储中，位于 `~/.hermes/auth.json`，并且可以导入现有的 Codex CLI 凭据（当 `~/.codex/auth.json` 存在时）。无需安装 Codex CLI。

如果令牌刷新失败并出现终端错误（HTTP 4xx、`invalid_grant`、已撤销的授权等），Hermes 会将刷新令牌标记为失效并停止重放它，这样您就不会看到大量相同的身份验证失败。下一次请求会显示一个类型化的重新身份验证消息。运行 `hermes auth add codex-oauth`（或 `hermes model` → OpenAI Codex）以启动一个新的设备码登录；隔离状态将在下一次成功交换后清除。
:::

:::warning
即使在使用 Nous Portal、Codex 或自定义端点时，某些工具（视觉、网页摘要、MoA）会使用一个单独的“辅助”模型。默认情况下（`auxiliary.*.provider: "auto"`），Hermes 将这些任务路由到您的**主聊天模型**——即您在 `hermes model` 中选择的同一模型。您可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——请参阅[辅助模型](/docs/user-guide/configuration#auxiliary-models)。
:::
...
通过 Nous Research 的门户，基于订阅访问 Hermes-4 模型（`Hermes-4-70B`、`Hermes-4.3-36B`、`Hermes-4-405B`）。运行 `hermes model`，选择 **Nous Portal**，通过浏览器登录——Hermes 在 `~/.hermes/auth.json` 存储一个长期有效的刷新令牌。

刷新令牌也通过共享令牌存储在所有配置文件之间共享，因此在一个配置文件上登录会延续到其他配置文件。

#### 令牌处理

Hermes 在每次推理调用时从您存储的 Nous 刷新令牌中生成一个短期有效的 JWT，而不是重复使用长期有效的 API 密钥。令牌生命周期是完全自动的——刷新、生成、在临时 401 错误时重试——您永远不会看到它。

如果门户使刷新令牌失效（密码更改、手动撤销、会话过期），失效的刷新令牌会在本地被隔离，这样 Hermes 就会停止重放它，您也不会看到一连串相同的 401 错误。下一次调用会显示清晰的“需要重新身份验证”消息。运行 `hermes auth add nous` 再次登录；隔离状态将在下一次成功登录后清除。
...
当您通过 `hermes model` → Anthropic OAuth（或通过 `hermes auth add anthropic --type oauth`）进行身份验证时，Hermes 会作为 Claude Code 路由到您的 Anthropic 账户。**这仅在您拥有 Claude Max 套餐并购买了额外使用额度时才有效。** 基础 Max 套餐额度（Claude Code 默认包含的使用量）不会被 Hermes 消耗——只有您额外添加的超额额度才会被消耗。Claude Pro 订阅者无法使用此路径。

如果您没有 Max + 额外额度，请使用 `ANTHROPIC_API_KEY`——请求将按 Token 计费，费用计入该密钥所属的组织（标准 API 定价，独立于任何 Claude 订阅）。
:::
...
如果你的 `gh auth token` 返回一个 `ghp_*` Token，请改用 `hermes model` 通过 OAuth 进行认证。
:::
...
在收到 HTTP 401 响应时，Hermes 现在会在回退之前执行一次性的凭证恢复：

1. 通过正常的优先级链重新解析 Token（`COPILOT_GITHUB_TOKEN` → `GH_TOKEN` → `GITHUB_TOKEN` → `gh auth token`）
2. 使用刷新的请求头重建共享的 OpenAI 客户端
3. 重试请求一次

一些较旧的社区代理使用 `api.github.com/copilot_internal/v2/token` 交换流程。该端点可能对某些账户类型不可用（返回 404）。因此，Hermes 将直接 Token 认证作为主要路径，并依赖运行时凭证刷新 + 重试来确保鲁棒性。
:::
...
# 需要 PATH 中存在 GitHub Copilot CLI 以及现有的 `copilot login` 会话
```
...
:::note Z.AI 端点自动检测
当使用 Z.AI / GLM 提供商时，Hermes 会自动探测多个端点（全球版、中国版、编码变体）以找到接受你 API 密钥的那个。你无需手动设置 `GLM_BASE_URL` —— 可用的端点会被自动检测并缓存。
:::
...
当使用 xAI 作为提供商（任何包含 `x.ai` 的基础 URL）时，Hermes 会自动启用提示词缓存，方法是在每个 API 请求中发送 `x-grok-conv-id` 头部。这会将请求路由到同一会话中的同一服务器，允许 xAI 的基础设施复用缓存的系统提示词和对话历史。

无需配置——当检测到 xAI 端点且会话 ID 可用时，缓存会自动激活。这降低了多轮对话的延迟和成本。

xAI 还提供了一个专用的 TTS 端点 (`/v1/tts`)。在 `hermes tools` → Voice & TTS 中选择 **xAI TTS**，或查看 [Voice & TTS](../user-guide/features/tts.md#text-to-speech) 页面进行配置。
...
通过 AWS Bedrock 使用 Anthropic Claude、Amazon Nova、DeepSeek v3.2、Meta Llama 4 等模型。使用 AWS SDK (`boto3`) 凭据链——无需 API 密钥，只需标准的 AWS 身份验证。
...
如果你订阅了阿里巴巴的 **Coding Plan**（一个与标准 DashScope API 访问分开的定价 SKU），Hermes 会将其作为一个独立的一级提供商暴露出来：`alibaba-coding-plan`。端点：`https://coding-intl.dashscope.aliyuncs.com/v1`。它与常规的 `alibaba` 提供商一样是 OpenAI 兼容的，但具有不同的基础 URL 和计费面。
```yaml
model:
...
通过浏览器 OAuth 登录使用 MiniMax-M2.7 —— 无需 API 密钥。在 `hermes model` 中选择 **MiniMax (OAuth)**，通过浏览器登录，Hermes 会持久保存访问令牌和刷新令牌。底层使用与 Anthropic Messages 兼容的端点 (`/anthropic`)。
...
支持的模型：`MiniMax-M2.7`（主模型）和 `MiniMax-M2.7-highspeed`（被配置为默认辅助模型）。OAuth 路径会忽略 `MINIMAX_API_KEY` / `MINIMAX_BASE_URL`。
...
在 [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) 获取你的令牌 —— 确保启用 "Make calls to Inference Providers" 权限。包含免费层级（每月 0.10 美元额度，提供商费率无加价）。
...
**`/model`**（在活跃的 Hermes 聊天会话中输入）只能**在您已设置的提供商和模型之间切换**。它无法添加新的提供商、运行 OAuth 或提示输入 API 密钥。如果您只配置了一个提供商（例如 OpenRouter），`/model` 将只显示该提供商的模型。

**要添加新的提供商：** 退出您的会话（`Ctrl+C` 或 `/quit`），运行 `hermes model`，设置新的提供商，然后开始一个新的会话。
:::

一旦您配置了至少一个自定义端点，就可以在会话中切换模型：
...
**您无法通过 OpenAI 兼容的 API** (`/v1/chat/completions`) 设置上下文长度。它必须在服务器端或通过 Modelfile 配置。这是将 Ollama 与 Hermes 等工具集成时最常见的困惑来源。
:::
...
**上下文长度：** vLLM 默认读取模型的 `max_position_embeddings`。如果该值超过您的 GPU 内存，它会报错并要求您将 `--max-model-len` 设置得更低。您也可以使用 `--max-model-len auto` 来自动找到适合的最大值。设置 `--gpu-memory-utilization 0.95`（默认为 0.9）以将更多上下文挤入 VRAM。
...
**工具调用：** 使用 `--tool-call-parser` 并为您模型系列指定相应的解析器：`qwen` (Qwen 2.5)、`llama3`、`llama4`、`deepseekv3`、`mistral`、`glm`。没有此标志时，工具调用将以纯文本形式返回。

:::caution SGLang 默认最大输出 Token 数为 128
如果响应看起来被截断，请在您的请求中添加 `max_tokens` 或在服务器上设置 `--default-max-tokens`。如果请求中未指定，SGLang 的默认值仅为每个响应 128 个 Token。
:::
...
**上下文长度 (`-c`):** 最近的构建默认值为 `0`，这会从 GGUF 元数据中读取模型的训练上下文。对于具有 128k+ 训练上下文的模型，尝试分配完整的 KV 缓存可能会导致 OOM。请根据您的需要明确设置 `-c`（对于 Agent 使用，32k–64k 是一个不错的范围）。如果使用并行槽位 (`-np`)，总上下文将在槽位之间分配 — 使用 `-c 32768 -np 4` 时，每个槽位仅获得 8k。
...
由于 Hermes Agent 需要 Unix 环境，Windows 用户在 WSL2 内运行它。如果您的模型服务器（Ollama、LM Studio 等）运行在 **Windows 主机** 上，您需要桥接网络差距 — WSL2 使用具有自己子网的虚拟网络适配器，因此 WSL2 内的 `localhost` 指的是 Linux 虚拟机，**而不是** Windows 主机。

:::tip 两者都在 WSL2 中？没问题。
如果您的模型服务器也运行在 WSL2 内（对于 vLLM、SGLang 和 llama-server 很常见），`localhost` 可以正常工作 — 它们共享相同的网络命名空间。跳过本节。
:::
...
这些问题在 Hermes 中使用**所有**本地推理服务器时都会遇到。
...
| **LM Studio** | 更新到 0.3.6+ 版本并使用支持原生工具调用的模型 |
...
Hermes 从你服务器的 `/v1/models` 端点自动检测上下文长度。如果服务器报告的值很低（或根本没有报告），Hermes 会使用模型声明的限制，这可能是不正确的。
...
:::note
ClawRouter 需要在 Base 或 Solana 上有一个 USDC 资金的钱包用于支付。所有请求都通过 BlockRun 的后端 API 路由。运行 `npx @blockrun/clawrouter doctor` 检查钱包状态。
:::
...
任何具有兼容 OpenAI API 的服务都可以使用。一些流行的选择：
...
对于大多数设置，这可以开箱即用。该系统是提供商感知的 —— 同一个模型根据服务提供商的不同可能有不同的上下文限制（例如，`claude-opus-4.6` 在 Anthropic 直接访问时为 1M，但在 GitHub Copilot 上为 128K）。
...
`hermes model` → 自定义端点向导现在会明确提示输入 `api_mode`，并将你的答案持久化到 `config.yaml`。当该字段留空时，仍会进行基于 URL 的自动检测（例如 `/anthropic` 路径 → `anthropic_messages`）作为后备方案。

在会话中，可以使用三重语法在它们之间切换：
...
[其他兼容提供商](#other-compatible-providers) 中列出的云提供商都使用 OpenAI 的 REST 接口规范，因此它们在 `custom_providers:` 下的配置方式相同。以下是三个配置示例。每个示例都需放入 `~/.hermes/config.yaml`，对应的 API 密钥则放入 `~/.hermes/.env`。
...
超快速推理（Llama-3.3-70B 上约 500 tok/s）。模型目录较小，但对延迟敏感的交互式使用场景非常强大。
...
以上三个示例可以组合使用 —— 将它们全部放在一起，并通过 `/model custom:<name>:<model>` 在每次交互时切换：
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网络搜索和抓取。如果您更倾向于在本地运行 Firecrawl，可以将 Hermes 指向一个自托管的实例。完整的设置说明请参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。

**您将获得：** 无需 API 密钥，无速率限制，无按页计费，完全的数据主权。

**您将失去：** 云版本使用 Firecrawl 专有的 "Fire-engine" 来实现高级的反机器人绕过（Cloudflare、验证码、IP 轮换）。自托管版本使用基础的 fetch + Playwright，因此一些受保护的站点可能会失败。搜索使用 DuckDuckGo 而非 Google。
...
使用 OpenRouter 时，您可以控制请求如何在各提供商之间路由。在 `~/.hermes/config.yaml` 中添加 `provider_routing` 部分：
...
- 在给定的一天内，选择对于特定分数是确定性的，但随着帕累托前沿的移动（新模型、基准测试更新），实际选择的模型可能会发生变化。
- 有关完整的路由器行为，请参阅 OpenRouter 的 [Pareto 路由器文档](https://openrouter.ai/docs/guides/routing/routers/pareto-router)。
- 若要将 Pareto 代码路由器用于特定的**辅助任务**（压缩、视觉等）而非主 Agent，请在该任务下设置 `extra_body.plugins` — 参见 [辅助模型 → OpenRouter 路由和 Pareto 代码用于辅助任务](/docs/user-guide/configuration#openrouter-routing--pareto-code-for-auxiliary-tasks)。

## 备用提供商

配置一个备用提供商链，当主模型失败时（速率限制、服务器错误、身份验证失败），Hermes 会按顺序尝试这些提供商。规范格式是一个顶层的 `fallback_providers:` 列表：
...
当备用机制激活时，会在不丢失会话的情况下，在会话中途切换模型和提供商。链会按条目逐一尝试；每个会话的激活是一次性的。

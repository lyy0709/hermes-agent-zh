...
:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）也会使用一个单独的“辅助”模型——默认情况下是通过 OpenRouter 的 Gemini Flash。设置 `OPENROUTER_API_KEY` 可以自动启用这些工具。您也可以配置这些工具使用哪个模型和提供商——请参阅 [辅助模型](/docs/user-guide/configuration#auxiliary-models)。
:::

:::tip Nous 工具网关
付费的 Nous Portal 订阅者还可以访问 **[工具网关](/docs/user-guide/features/tool-gateway)**——通过网络搜索、图像生成、TTS 和浏览器自动化，这些功能都通过您的订阅路由。无需额外的 API 密钥。它在 `hermes model` 设置期间会自动提供，或者稍后可以通过 `hermes tools` 启用。
:::
...
当您通过 `hermes model` 选择 Anthropic OAuth 时，Hermes 优先使用 Claude Code 自身的凭据存储，而不是将令牌复制到 `~/.hermes/.env`。这样可以保持可刷新的 Claude 凭据的可刷新性。
...
# 需要 PATH 中有 GitHub Copilot CLI 并且存在现有的 `copilot login` 会话
```
...
:::note Z.AI 端点自动检测
使用 Z.AI / GLM 提供商时，Hermes 会自动探测多个端点（全球、中国、编码变体）以找到接受您 API 密钥的端点。您无需手动设置 `GLM_BASE_URL` —— 可用的端点会被自动检测并缓存。
:::
...
当使用 xAI 作为提供商（任何包含 `x.ai` 的基础 URL）时，Hermes 会自动启用提示词缓存，方法是在每个 API 请求中发送 `x-grok-conv-id` 头部。这会将请求路由到同一会话内的同一服务器，允许 xAI 的基础设施重用缓存的系统提示词和对话历史。
...
[Ollama Cloud](https://ollama.com/cloud) 托管与本地 Ollama 相同的开放权重模型目录，但无需 GPU 要求。在 `hermes model` 中选择 **Ollama Cloud**，从 [ollama.com/settings/keys](https://ollama.com/settings/keys) 粘贴您的 API 密钥，Hermes 会自动发现可用模型。
...
模型目录从 `ollama.com/v1/models` 动态获取并缓存一小时。`model:tag` 表示法（例如 `qwen3-coder:480b-cloud`）通过规范化保留 —— 不要使用短横线。

:::tip Ollama Cloud 与本地 Ollama
两者都使用相同的 OpenAI 兼容 API。Cloud 是一级提供商（`--provider ollama-cloud`, `OLLAMA_API_KEY`）；本地 Ollama 通过自定义端点流程访问（基础 URL `http://localhost:11434/v1`，无需密钥）。使用 Cloud 运行无法在本地运行的大型模型；使用本地版本用于隐私或离线工作。
:::
...
Bedrock 在底层使用 **Converse API** —— 请求被转换为 Bedrock 的模型无关格式，因此相同的配置适用于 Claude、Nova、DeepSeek 和 Llama 模型。仅当调用非默认区域端点时才设置 `BEDROCK_BASE_URL`。
...
```
:::tip 本地 NIM
对于本地部署（DGX Spark、本地 GPU），请设置 `NVIDIA_BASE_URL=http://localhost:8000/v1`。NIM 暴露了与 build.nvidia.com 相同的 OpenAI 兼容聊天补全 API，因此在云端和本地之间切换只需更改一行环境变量。
:::
...
[Ollama](https://ollama.com/) 通过一条命令在本地运行开源模型。最适合：快速本地实验、隐私敏感工作、离线使用。通过 OpenAI 兼容 API 支持工具调用。
...
**上下文长度 (`-c`):** 最近的构建默认值为 `0`，这会从 GGUF 元数据中读取模型的训练上下文。对于具有 128k+ 训练上下文的模型，尝试分配完整的 KV 缓存可能会导致 OOM。将 `-c` 明确设置为您需要的值（对于 Agent 使用，32k–64k 是一个不错的范围）。如果使用并行槽 (`-np`)，总上下文将在槽之间分配 — 使用 `-c 32768 -np 4` 时，每个槽仅获得 8k。
...
:::note Hyper-V 防火墙
在某些 Windows 11 版本上，Hyper-V 防火墙默认会阻止镜像连接。如果在启用镜像模式后 `localhost` 仍然无效，请在 **管理员 PowerShell** 中运行：
```powershell
...
如果你在 WSL2 内运行 Hermes，而你的模型服务器运行在 Windows 主机上，在 WSL2 默认的 NAT 网络模式下，`http://localhost:<端口>` 将无法工作。请参考上方的 [WSL2 网络](#wsl2-networking-windows-users) 部分以获取解决方案。

#### 工具调用显示为文本而非执行

模型输出类似 `{"name": "web_search", "arguments": {...}}` 的内容作为消息，而不是实际调用工具。

**原因：** 你的服务器未启用工具调用，或者模型不支持服务器实现的工具调用功能。
...
| **LM Studio** | 更新到 0.3.6+ 版本并使用原生支持工具调用的模型 |
...
**解决方案：** 对于 Agent 使用，将上下文设置为至少 **32,768 个 Token**。请参阅上方每个服务器章节的具体配置标志。

#### 启动时显示 "Context limit: 2048 tokens"

Hermes 会从你服务器的 `/v1/models` 端点自动检测上下文长度。如果服务器报告的值很低（或根本没有报告），Hermes 会使用模型声明的限制，这可能是不正确的。

**解决方案：** 在 `config.yaml` 中明确设置：
...
**可能原因：**
1.  **服务器上的输出上限 (`max_tokens`) 过低** —— SGLang 默认每个回应为 128 个 Token。在服务器上设置 `--default-max-tokens` 或在 Hermes 的 config.yaml 中配置 `model.max_tokens`。注意：`max_tokens` 仅控制回应长度 —— 它与你的对话历史能有多长（即 `context_length`）无关。
...
BlockRunAI 出品的 [ClawRouter](https://github.com/BlockRunAI/ClawRouter) 是一个本地路由代理，它根据查询复杂度自动选择模型。它通过 14 个维度对请求进行分类，并路由到能够处理任务的最便宜模型。支付通过 USDC 加密货币进行（无需 API 密钥）。
...
使用 `hermes model` → 自定义端点配置以上任何服务，或在 `config.yaml` 中配置：
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果你更喜欢在本地运行 Firecrawl，可以将 Hermes 指向自托管的实例。完整的设置说明请参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。
**您将获得：** 无需 API 密钥，无速率限制，无按页计费，完全的数据主权。

**您将失去：** 云端版本使用 Firecrawl 专有的 "Fire-engine" 进行高级反机器人绕过（Cloudflare、验证码、IP 轮换）。自托管版本使用基本的 fetch + Playwright，因此某些受保护的站点可能会失败。搜索使用 DuckDuckGo 而非 Google。
...
使用 OpenRouter 时，您可以控制请求如何在不同提供商之间路由。在 `~/.hermes/config.yaml` 中添加 `provider_routing` 部分：
...
配置一个备用提供商：模型，当您的主模型失败时（速率限制、服务器错误、身份验证失败），Hermes 会自动切换到它：
...
激活后，备用模型会在会话中途切换模型和提供商，而不会丢失您的对话。它在**每个会话中最多触发一次**。
...
可选的廉价与强力模型路由功能，让 Hermes 将复杂工作保留给主模型，同时将非常简短/简单的轮次发送到更便宜的模型。
...
工作原理：
- 如果一个轮次简短、单行，并且看起来不涉及大量代码/工具/调试，Hermes 可能会将其路由到 `cheap_model`
- 如果轮次看起来复杂，Hermes 会继续使用您的主模型/提供商
- 如果廉价路由无法干净地解析，Hermes 会自动回退到主模型

此功能设计上是保守的。它适用于快速、低风险的轮次，例如：
- 简短的事实性问题
- 快速重写
- 轻量级摘要

...
| **阿里云 Coding Plan** | `DASHSCOPE_API_KEY` (提供商：`alibaba-coding-plan`，别名：`alibaba_coding`) —— 独立的计费 SKU，不同的端点 |
...
关于官方 API 密钥路径，请参阅专门的 [Google Gemini 指南](/docs/guides/google-gemini)。
...
Hermes 默认附带 Google 的**公开** `gemini-cli` 桌面 OAuth 客户端——与 Google 在其开源 `gemini-cli` 中包含的凭证相同。桌面 OAuth 客户端不是机密的（PKCE 提供安全性）。您无需安装 `gemini-cli` 或注册自己的 GCP OAuth 客户端。
...
**推理工作原理：**
- 流量发送到 `https://cloudcode-pa.googleapis.com/v1internal:generateContent` (流式传输时发送到 `:streamGenerateContent?alt=sse`)，而不是付费的 `v1beta/openai` 端点
...
:::warning 政策风险
Google 认为在第三方软件中使用 Gemini CLI OAuth 客户端违反政策。一些用户报告了账户受限的情况。为了获得最低风险的体验，请通过 `gemini` 提供商使用您自己的 API 密钥。Hermes 会在 OAuth 开始前显示明确警告并要求确认。
:::

**自定义 OAuth 客户端（可选）：**

如果您更愿意注册自己的 Google OAuth 客户端——例如，将配额和授权范围限定在您自己的 GCP 项目中——请设置：
...
:::info Codex 说明
OpenAI Codex 提供商通过设备码进行身份验证（打开 URL，输入代码）。Hermes 将生成的凭据存储在其自己的身份验证存储中，位于 `~/.hermes/auth.json`，并且可以导入 `~/.codex/auth.json` 中现有的 Codex CLI 凭据（如果存在）。无需安装 Codex CLI。
:::

:::warning
即使使用 Nous Portal、Codex 或自定义端点，一些工具（视觉、网页摘要、MoA）也会使用单独的“辅助”模型。默认情况下（`auxiliary.*.provider: "auto"`），Hermes 将这些任务路由到您的**主聊天模型**——即您在 `hermes model` 中选择的同一模型。您可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——请参阅[辅助模型](/docs/user-guide/configuration#auxiliary-models)。
:::
...
当您通过 `hermes model` → Anthropic OAuth（或通过 `hermes auth add anthropic --type oauth`）进行身份验证时，Hermes 会作为 Claude Code 路由到您的 Anthropic 账户。**这仅在您使用 Claude Max 套餐并购买了额外使用额度时才有效。** 基础 Max 套餐额度（Claude Code 默认包含的使用量）不会被 Hermes 消耗——只有您额外添加的超额额度才会被消耗。Claude Pro 订阅者无法使用此路径。
...
当您通过 `hermes model` 选择 Anthropic OAuth 时，Hermes 优先使用 Claude Code 自身的凭据存储，而不是将令牌复制到 `~/.hermes/.env`。这使可刷新的 Claude 凭据保持可刷新状态。
...
:::info Copilot 在 Hermes 中的身份验证行为
Hermes 将受支持的 GitHub Token（`gho_*`、`github_pat_*` 或 `ghu_*`）直接发送到 `api.githubcopilot.com`，并包含 Copilot 特定的请求头（`Editor-Version`、`Copilot-Integration-Id`、`Openai-Intent`、`x-initiator`）。

在收到 HTTP 401 响应时，Hermes 现在会在回退之前执行一次性的凭据恢复：

1.  通过正常的优先级链重新解析 Token（`COPILOT_GITHUB_TOKEN` → `GH_TOKEN` → `GITHUB_TOKEN` → `gh auth token`）
2.  使用刷新的请求头重建共享的 OpenAI 客户端
3.  重试请求一次

一些较旧的社区代理使用 `api.github.com/copilot_internal/v2/token` 交换流程。该端点可能对某些账户类型不可用（返回 404）。因此，Hermes 将直接使用 Token 的身份验证作为主要路径，并依赖运行时的凭据刷新和重试机制来确保鲁棒性。
:::
...
这些提供商具有内置支持，并拥有专用的提供商 ID。设置 API 密钥并使用 `--provider` 来选择：
...
基础 URL 可以通过环境变量 `NOVITA_BASE_URL`、`GLM_BASE_URL`、`KIMI_BASE_URL`、`MINIMAX_BASE_URL`、`MINIMAX_CN_BASE_URL`、`DASHSCOPE_BASE_URL`、`XIAOMI_BASE_URL`、`GMI_BASE_URL` 或 `TOKENHUB_BASE_URL` 来覆盖。
...
xAI 通过 Responses API（`codex_responses` 传输）连接，以在 Grok 4 模型上自动支持推理 —— 无需 `reasoning_effort` 参数，服务器默认进行推理。在 `~/.hermes/.env` 中设置 `XAI_API_KEY`，并在 `hermes model` 中选择 xAI，或者使用快捷方式 `/model grok-4-1-fast-reasoning`。

当使用 xAI 作为提供商时（任何包含 `x.ai` 的基础 URL），Hermes 会自动启用提示词缓存，方法是在每个 API 请求中发送 `x-grok-conv-id` 请求头。这会将请求路由到同一会话内的同一台服务器，使 xAI 的基础设施能够重用缓存的系统提示词和会话历史记录。
...
[NovitaAI](https://novita.ai) 是面向开发者和 Agent 的 AI 原生云。其三条产品线分别是：提供 200+ 模型的 Model API、用于构建和运行 AI Agent 的 Agent Sandbox，以及用于可扩展计算的 GPU Cloud，所有这些都可以从一个平台获得。
...
### Ollama Cloud — 托管版 Ollama 模型，支持 OAuth + API 密钥

[Ollama Cloud](https://ollama.com/cloud) 托管了与本地 Ollama 相同的开放权重模型目录，但无需 GPU。在 `hermes model` 中选择 **Ollama Cloud**，从 [ollama.com/settings/keys](https://ollama.com/settings/keys) 粘贴你的 API 密钥，Hermes 会自动发现可用模型。
...
两者都使用相同的 OpenAI 兼容 API。Cloud 是一级提供商（`--provider ollama-cloud`, `OLLAMA_API_KEY`）；本地 Ollama 通过自定义端点流程访问（基础 URL `http://localhost:11434/v1`，无需密钥）。对于无法在本地运行的大型模型使用 Cloud；对于隐私或离线工作使用本地。
:::

### AWS Bedrock

通过 AWS Bedrock 使用 Anthropic Claude、Amazon Nova、DeepSeek v3.2、Meta Llama 4 等模型。使用 AWS SDK (`boto3`) 凭证链——无需 API 密钥，只需标准的 AWS 认证。
...
Bedrock 底层使用 **Converse API**——请求被转换为 Bedrock 的模型无关格式，因此相同的配置适用于 Claude、Nova、DeepSeek 和 Llama 模型。仅当你调用非默认的区域端点时才设置 `BEDROCK_BASE_URL`。

有关 IAM 设置、区域选择和跨区域推理的详细步骤，请参阅 [AWS Bedrock 指南](/docs/guides/aws-bedrock)。
...
仅当门户端点变更时才设置 `HERMES_QWEN_BASE_URL`（默认：`https://portal.qwen.ai/v1`）。
...
如果你订阅了阿里巴巴的 **Coding Plan**（一个独立于标准 DashScope API 访问的定价 SKU），Hermes 会将其作为一个独立的一级提供商暴露：`alibaba-coding-plan`。端点：`https://coding-intl.dashscope.aliyuncs.com/v1`。它与常规的 `alibaba` 提供商一样兼容 OpenAI，但具有不同的基础 URL 和计费面。
...
`minimax-oauth` 使用 MiniMax 面向消费者的门户进行 OAuth 登录——无需设置计费。`minimax` 和 `minimax-cn` 提供商使用 `MINIMAX_API_KEY` / `MINIMAX_CN_API_KEY`——用于编程访问。完整步骤请参阅 [MiniMax OAuth 指南](/docs/guides/minimax-oauth)。
:::
...
:::tip 本地 NIM
对于本地部署（DGX Spark、本地 GPU），请设置 `NVIDIA_BASE_URL=http://localhost:8000/v1`。NIM 暴露与 build.nvidia.com 相同的 OpenAI 兼容的聊天补全 API，因此在云端和本地之间切换只需更改一行环境变量。
:::
...
在 [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) 获取你的 Token —— 确保启用 "Make calls to Inference Providers" 权限。包含免费套餐（每月 $0.10 额度，提供商费率无加价）。
...
:::warning 遗留环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已**移除**。Hermes 的任何部分都不会读取它们 —— `config.yaml` 是模型和端点配置的唯一真实来源。如果你的 `.env` 中有过时的条目，它们会在下次运行 `hermes setup` 或配置迁移时自动清除。请使用 `hermes model` 或直接编辑 `config.yaml`。
:::

两种方法都会将配置持久化到 `config.yaml`，这是模型、提供商和基础 URL 的真实来源。
...
**`/model`**（在活动的 Hermes 聊天会话中键入）只能**在你已设置好的提供商和模型之间切换**。它不能添加新提供商、运行 OAuth 或提示输入 API 密钥。如果你只配置了一个提供商（例如 OpenRouter），`/model` 将只显示该提供商的模型。
...
[Ollama](https://ollama.com/) 只需一个命令即可在本地运行开源模型。最适合：快速本地实验、隐私敏感的工作、离线使用。通过 OpenAI 兼容的 API 支持工具调用。
...
Ollama 默认**不会**使用模型的完整上下文窗口。根据你的 VRAM，默认值如下：
...
对于使用工具的 Agent，**你至少需要 16k–32k 的上下文**。在 4k 的情况下，仅系统提示词 + 工具模式就可能填满整个窗口，没有空间进行对话。
...
**你无法通过 OpenAI 兼容的 API** (`/v1/chat/completions`) 设置上下文长度。必须在服务器端或通过 Modelfile 进行配置。这是将 Ollama 与 Hermes 等工具集成时最常见的困惑来源。
:::
...
# 跳过 API 密钥（如果为 vLLM 配置了 --api-key 则输入）
# 输入模型名称：meta-llama/Llama-3.1-70B-Instruct
```

**上下文长度：** vLLM 默认读取模型的 `max_position_embeddings`。如果该值超过你的 GPU 内存，它会报错并要求你将 `--max-model-len` 设置得更低。你也可以使用 `--max-model-len auto` 来自动寻找适合的最大值。设置 `--gpu-memory-utilization 0.95`（默认 0.9）以将更多上下文挤入 VRAM。
...
**上下文长度：** SGLang 默认从模型配置中读取。使用 `--context-length` 来覆盖。如果你需要超过模型声明的最大值，请设置 `SGLANG_ALLOW_OVERWRITE_LONGER_CONTEXT_LEN=1`。

**工具调用：** 使用 `--tool-call-parser` 并为你模型系列选择合适的解析器：`qwen` (Qwen 2.5), `llama3`, `llama4`, `deepseekv3`, `mistral`, `glm`。没有此标志，工具调用将以纯文本形式返回。
...
**上下文长度 (`-c`):** 最近的构建默认值为 `0`，这会从 GGUF 元数据中读取模型的训练上下文。对于具有 128k+ 训练上下文的模型，尝试分配完整的 KV 缓存可能会导致内存不足 (OOM)。请根据你的需要显式设置 `-c`（对于 Agent 使用，32k–64k 是一个不错的范围）。如果使用并行槽 (`-np`)，总上下文会在槽之间分配 — 使用 `-c 32768 -np 4` 时，每个槽只获得 8k。
...
如果没有 `--jinja` 参数，llama-server 会完全忽略 `tools` 参数。模型会尝试通过在响应文本中写入 JSON 来调用工具，但 Hermes 不会将其识别为工具调用——你会看到像 `{"name": "web_search", ...}` 这样的原始 JSON 被作为消息打印出来，而不是执行实际的搜索。
...
[LM Studio](https://lmstudio.ai/) 是一个用于运行本地模型的桌面应用，带有图形界面。最适合：喜欢可视化界面、快速测试模型、在 macOS/Windows/Linux 上的开发者。
...
**工具调用：** 自 LM Studio 0.3.6 起支持。具有原生工具调用训练的模型（Qwen 2.5、Llama 3.x、Mistral、Hermes）会被自动检测并显示工具徽章。其他模型使用一个可能不太可靠的通用后备方案。

---

### WSL2 网络（Windows 用户）

由于 Hermes Agent 需要 Unix 环境，Windows 用户在 WSL2 内运行它。如果你的模型服务器（Ollama、LM Studio 等）运行在 **Windows 主机** 上，你需要桥接网络差距——WSL2 使用具有自己子网的虚拟网络适配器，因此 WSL2 内的 `localhost` 指的是 Linux 虚拟机，**而不是** Windows 主机。
...
如果你使用 **选项 2**（使用主机 IP 的 NAT 模式），Windows 上的模型服务器必须接受来自 `127.0.0.1` 之外的连接。默认情况下，大多数服务器只监听 localhost——NAT 模式下的 WSL2 连接来自不同的虚拟子网，将被拒绝。在镜像模式下，`localhost` 直接映射，因此默认的 `127.0.0.1` 绑定可以正常工作。
...
如果你收到一个列出模型的 JSON 响应，那就没问题。在 Hermes 配置中使用相同的 URL 作为 `base_url`。
...
如果你在 WSL2 内运行 Hermes，而你的模型服务器在 Windows 主机上，在 WSL2 默认的 NAT 网络模式下，`http://localhost:<port>` 将无法工作。请参阅上面的 [WSL2 网络](#wsl2-networking-windows-users) 部分以获取解决方案。
...
| **Ollama** | 默认启用工具调用 — 确保你的模型支持它（使用 `ollama show model-name` 检查） |
...
**修复：** 对于 Agent 使用，将上下文设置为至少 **32,768 个 Token**。请参阅上面每个服务器部分的特定标志。

#### 启动时显示"Context limit: 2048 tokens"

Hermes 从你服务器的 `/v1/models` 端点自动检测上下文长度。如果服务器报告的值较低（或根本没有报告），Hermes 会使用模型声明的限制，这可能是错误的。
...
**可能的原因：**
1.  **服务器上的输出上限（`max_tokens`）过低** — SGLang 默认每个响应为 128 个 Token。在服务器上设置 `--default-max-tokens` 或在 config.yaml 中配置 Hermes 的 `model.max_tokens`。注意：`max_tokens` 仅控制响应长度 — 它与你的对话历史记录可以有多长（即 `context_length`）无关。
...
然后配置 Hermes：`hermes model` → 自定义端点 → `http://localhost:4000/v1`。
...
BlockRunAI 的 [ClawRouter](https://github.com/BlockRunAI/ClawRouter) 是一个本地路由代理，它根据查询复杂度自动选择模型。它根据 14 个维度对请求进行分类，并将其路由到能够处理任务的最便宜的模型。支付通过 USDC 加密货币进行（无需 API 密钥）。
...
任何提供 OpenAI 兼容 API 的服务都可以使用。一些流行的选择：
...
可以通过 `hermes model` → 自定义端点，或在 `config.yaml` 中配置以上任何服务：
...
当自动检测得到的窗口大小不正确时，设置 `context_length`。
...
对于大多数设置，这可以开箱即用。该系统是提供商感知的 — 同一个模型根据服务提供商的不同可能具有不同的上下文限制（例如，`claude-opus-4.6` 在 Anthropic 直接访问时为 1M，但在 GitHub Copilot 上为 128K）。
...
:::tip 何时手动设置
- 你正在使用 Ollama，其自定义的 `num_ctx` 低于模型的最大值
...
[其他兼容的提供商](#其他兼容的提供商) 中列出的云提供商都使用 OpenAI 的 REST 方言，因此它们在 `custom_providers:` 下的配置方式相同。以下是三个现成的配置示例。每个都放入 `~/.hermes/config.yaml`，匹配的 API 密钥放入 `~/.hermes/.env`。

#### Together AI

托管开源模型（Llama、MiniMax、Gemma、DeepSeek、Qwen），价格显著低于第一方 API。是多模型集群的良好默认选择。
...
超快速推理（Llama-3.3-70B 上约 500 tok/s）。目录较小，但对于延迟敏感的交互式使用非常强大。
...
以上三种配置可以组合使用——将它们一起使用，并通过 `/model custom:<name>:<model>` 在每次交互时切换：
...
model:
  ...
  provider: custom:together      # 启动到 Together；之后可自由切换
```

:::tip 故障排除
- 在 CLI 验证器修复 #15083 之后，`hermes doctor` 对于这些名称中的任何一个都不应打印 `Unknown provider` 警告。
...
| 功能 | 提供商 | 环境变量 |
|---------|----------|--------------|
| 网络爬取 | [Firecrawl](https://firecrawl.dev/) | `FIRECRAWL_API_KEY`, `FIRECRAWL_API_URL` |
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网络搜索和爬取。如果你更喜欢在本地运行 Firecrawl，可以改为将 Hermes 指向自托管的实例。有关完整的设置说明，请参阅 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。
...
openrouter:
  min_coding_score: 0.65   # 0.0–1.0；值越高 = 编码能力越强（也更昂贵）。默认值 0.65。
```

注意：

- `min_coding_score` **仅**在 `model.model` 为 `openrouter/pareto-code` 时发送。对于任何其他模型，此值无效。
- 设置为空字符串（或删除该行）以让 OpenRouter 选择可用的最强编码器——这是文档中描述的当省略 plugins 块时的行为。
- 在给定的一天内，选择对于特定分数是确定性的，但实际选择的模型可能会随着帕累托前沿的移动（新模型、基准测试更新）而变化。
- 有关完整的路由器行为，请参阅 OpenRouter 的 [Pareto Router 文档](https://openrouter.ai/docs/guides/routing/routers/pareto-router)。
- 要将 Pareto Code 路由器用于特定的**辅助任务**（压缩、视觉等）而非主 Agent，请在该任务下设置 `extra_body.plugins`——参见 [辅助模型 → 用于辅助任务的 OpenRouter 路由和 Pareto Code](/docs/user-guide/configuration#openrouter-routing--pareto-code-for-auxiliary-tasks)。
...
当激活时，备用方案会在会话中途切换模型和提供商，而不会丢失您的对话。链会按条目逐一尝试；每个会话的激活是一次性的。
...
:::tip
备用配置**仅**通过 `config.yaml` 进行配置——或通过 `hermes fallback` 交互式配置。有关其触发时机、链如何推进以及如何与辅助任务和委派交互的完整详细信息，请参阅 [备用提供商](/docs/user-guide/features/fallback-providers)。
:::

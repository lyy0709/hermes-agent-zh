...
```
:::warning 政策风险
Google 认为在第三方软件中使用 Gemini CLI OAuth 客户端违反政策。部分用户报告了账户受限情况。为获得最低风险体验，请通过 `gemini` 提供商使用您自己的 API 密钥。Hermes 会在 OAuth 开始前显示明确警告并要求确认。
:::

**自定义 OAuth 客户端（可选）：**

如果您希望注册自己的 Google OAuth 客户端（例如，将配额和授权范围限定在您自己的 GCP 项目中），请设置：
...
在 [console.cloud.google.com/apis/credentials](https://console.cloud.google.com/apis/credentials) 注册一个**桌面应用** OAuth 客户端，并确保已启用 Generative Language API。

:::info Codex 说明
OpenAI Codex 提供商通过设备码进行身份验证（打开 URL，输入代码）。Hermes 将生成的凭据存储在其自己的身份验证存储 `~/.hermes/auth.json` 中，并且可以导入 `~/.codex/auth.json` 中现有的 Codex CLI 凭据（如果存在）。无需安装 Codex CLI。
:::

:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）也会使用单独的“辅助”模型——默认通过 OpenRouter 使用 Gemini Flash。设置 `OPENROUTER_API_KEY` 可自动启用这些工具。您也可以配置这些工具使用的模型和提供商——请参阅[辅助模型](/docs/user-guide/configuration#auxiliary-models)。
:::

:::tip Nous 工具网关
付费的 Nous Portal 订阅者还可以访问**[工具网关](/docs/user-guide/features/tool-gateway)**——通过网络搜索、图像生成、TTS 和浏览器自动化，这些功能通过您的订阅路由。无需额外的 API 密钥。在 `hermes model` 设置期间会自动提供，或稍后通过 `hermes tools` 启用。
:::
...
如果您尝试切换到尚未设置的提供商（例如，您只配置了 OpenRouter 但想使用 Anthropic），您需要 `hermes model`，而不是 `/model`。先退出当前会话（`Ctrl+C` 或 `/quit`），运行 `hermes model`，完成提供商设置，然后开始新的会话。
...
**API 路由**：GPT-5+ 模型（除 `gpt-5-mini` 外）自动使用 Responses API。所有其他模型（GPT-4o、Claude、Gemini 等）使用 Chat Completions。模型从实时 Copilot 目录中自动检测。
...
这些提供商拥有内置支持，并配有专用的提供商 ID。设置 API 密钥并使用 `--provider` 来选择：
...
:::note Z.AI 端点自动检测
使用 Z.AI / GLM 提供商时，Hermes 会自动探测多个端点（全球版、中国版、编程版变体）以找到接受你 API 密钥的那个。你无需手动设置 `GLM_BASE_URL` —— 可用的端点会被自动检测并缓存。
:::

### xAI (Grok) — Responses API + 提示词缓存

xAI 通过 Responses API（`codex_responses` 传输）连接，以支持 Grok 4 模型的自动推理 —— 无需 `reasoning_effort` 参数，服务器默认进行推理。在 `~/.hermes/.env` 中设置 `XAI_API_KEY`，并在 `hermes model` 中选择 xAI，或者将 `grok` 作为快捷方式输入 `/model grok-4-1-fast-reasoning`。
...
[Ollama Cloud](https://ollama.com/cloud) 托管着与本地 Ollama 相同的开源模型目录，但无需 GPU 要求。在 `hermes model` 中选择 **Ollama Cloud**，从 [ollama.com/settings/keys](https://ollama.com/settings/keys) 粘贴你的 API 密钥，Hermes 会自动发现可用模型。
...
两者都使用相同的 OpenAI 兼容 API。Cloud 是原生提供商（`--provider ollama-cloud`，`OLLAMA_API_KEY`）；本地 Ollama 通过自定义端点流程访问（基础 URL `http://localhost:11434/v1`，无需密钥）。使用 Cloud 来运行无法在本地运行的大型模型；使用本地版用于隐私或离线工作。
:::
...
认证使用标准的 boto3 链：明确的 `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`、来自 `~/.aws/credentials` 的 `AWS_PROFILE`、EC2/ECS/Lambda 上的 IAM 角色、IMDS 或 SSO。如果你已经通过 AWS CLI 认证，则无需环境变量。
Bedrock 底层使用 **Converse API** —— 请求会被转换为 Bedrock 的模型无关格式，因此同一配置适用于 Claude、Nova、DeepSeek 和 Llama 模型。仅在调用非默认区域端点时才需要设置 `BEDROCK_BASE_URL`。

关于 IAM 设置、区域选择和跨区域推理的详细指南，请参阅 [AWS Bedrock 指南](/docs/guides/aws-bedrock)。

### Qwen Portal (OAuth)

阿里巴巴的 Qwen Portal，支持基于浏览器的 OAuth 登录。在 `hermes model` 中选择 **Qwen OAuth (Portal)**，通过浏览器登录，Hermes 会持久化刷新 Token。
...
仅在 Portal 端点地址变更时才需要设置 `HERMES_QWEN_BASE_URL`（默认：`https://portal.qwen.ai/v1`）。
...
在 [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) 获取您的 Token —— 请确保启用 "Make calls to Inference Providers" 权限。包含免费层级（每月 $0.10 额度，提供商费率无加价）。
...
:::warning 遗留环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已被 **移除**。Hermes 的任何部分都不会读取它们 —— `config.yaml` 是模型和端点配置的唯一真实来源。如果您的 `.env` 中有过时的条目，它们将在下次运行 `hermes setup` 或配置迁移时自动清除。请使用 `hermes model` 或直接编辑 `config.yaml`。
:::
...
:::tip
`/model custom`（裸命令，不带模型名称）会查询你的端点的 `/models` API，如果恰好加载了一个模型，则自动选择它。对于运行单个模型的本地服务器很有用。
:::
...
```
**上下文长度（`-c`）：** 最近的构建版本默认值为 `0`，这会从 GGUF 元数据中读取模型的训练上下文。对于训练上下文为 128k+ 的模型，尝试分配完整的 KV 缓存可能会导致内存溢出（OOM）。请根据你的需求显式设置 `-c`（对于 Agent 使用，32k–64k 是一个不错的范围）。如果使用并行槽位（`-np`），总上下文长度会在槽位间分配 —— 使用 `-c 32768 -np 4` 时，每个槽位只获得 8k。
...
没有 `--jinja`，llama-server 会完全忽略 `tools` 参数。模型会尝试通过在响应文本中写入 JSON 来调用工具，但 Hermes 不会将其识别为工具调用 —— 你会看到像 `{"name": "web_search", ...}` 这样的原始 JSON 被打印为消息，而不是执行实际的搜索。
...
[LM Studio](https://lmstudio.ai/) 是一个用于运行本地模型的桌面应用，带有图形界面。最适合：偏好可视化界面、快速模型测试、macOS/Windows/Linux 上的开发者。
...
:::note Hyper-V 防火墙
在某些 Windows 11 构建版本上，Hyper-V 防火墙默认会阻止镜像连接。如果在启用镜像模式后 `localhost` 仍然无法工作，请在 **管理员 PowerShell** 中运行此命令：
```powershell
...
| 服务器 | 默认绑定地址 | 如何修复 |
|--------|-------------|------------|
| **Ollama** | `127.0.0.1` | 在启动 Ollama 前设置环境变量 `OLLAMA_HOST=0.0.0.0`（Windows 上在系统设置 → 环境变量中设置，或编辑 Ollama 服务） |
...
这些问题在 Hermes 中使用**所有**本地推理服务器时都可能出现。

#### 从 WSL2 到 Windows 主机上的模型服务器出现 "Connection refused"

如果你在 WSL2 内运行 Hermes，而你的模型服务器在 Windows 主机上，在 WSL2 默认的 NAT 网络模式下，`http://localhost:<port>` 将无法工作。请参考上面的 [WSL2 网络](#wsl2-networking-windows-users) 部分进行修复。
...
BlockRunAI 出品的 [ClawRouter](https://github.com/BlockRunAI/ClawRouter) 是一个本地路由代理，它根据查询复杂度自动选择模型。它根据 14 个维度对请求进行分类，并将请求路由到能够处理任务的最便宜的模型。支付通过 USDC 加密货币进行（无需 API 密钥）。

```bash
# 安装并启动
npx @blockrun/clawrouter    # 在端口 8402 上启动
```
然后使用 `hermes model` 配置 Hermes → 自定义端点 → `http://localhost:8402/v1` → 模型名称 `blockrun/auto`。
...
:::note
ClawRouter 需要一个在 Base 或 Solana 上注资 USDC 的钱包进行支付。所有请求都通过 BlockRun 的后端 API 路由。运行 `npx @blockrun/clawrouter doctor` 来检查钱包状态。
:::
...
:::note 两个容易混淆的设置
**`context_length`** 是 **总上下文窗口** —— 输入 *和* 输出 Token 的合并预算（例如，Claude Opus 4.6 是 200,000）。Hermes 使用它来决定何时压缩历史记录以及验证 API 请求。

**`model.max_tokens`** 是 **输出上限** —— 模型在 *单次响应* 中可以生成的最大 Token 数。它与你的对话历史记录可以有多长无关。行业标准名称 `max_tokens` 是一个常见的混淆来源；Anthropic 的原生 API 后来为了清晰起见将其重命名为 `max_output_tokens`。
...
对于大多数设置，这可以开箱即用。该系统是提供商感知的 —— 同一个模型根据服务提供商的不同可能有不同的上下文限制（例如，`claude-opus-4.6` 在 Anthropic 直接访问时是 1M，但在 GitHub Copilot 上是 128K）。
...
:::tip
你可以随时使用 `hermes model` 命令切换提供商，无需重启。无论使用哪个提供商，你的对话历史、记忆和技能都会保留。
:::
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果你更倾向于在本地运行 Firecrawl，可以配置 Hermes 指向自托管的实例。完整的设置说明请参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。

**优点：** 无需 API 密钥，无速率限制，无按页计费，完全的数据主权。

**缺点：** 云版本使用 Firecrawl 专有的 "Fire-engine" 来绕过高级反机器人机制（Cloudflare、验证码、IP 轮换）。自托管版本使用基础的 fetch + Playwright，因此一些受保护的网站可能会失败。搜索使用 DuckDuckGo 而非 Google。
...
配置一个备用提供商:模型，当你的主模型失败时（速率限制、服务器错误、认证失败），Hermes 会自动切换到它：
...
:::tip
备用模型配置**仅**通过 `config.yaml` 进行，没有对应的环境变量。关于其触发条件、支持的提供商以及如何与辅助任务和委派交互的完整细节，请参阅 [备用提供商](/docs/user-guide/features/fallback-providers)。
:::

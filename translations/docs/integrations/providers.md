...
本页面介绍如何为 Hermes Agent 设置推理提供商——从 OpenRouter 和 Anthropic 等云端 API，到 Ollama 和 vLLM 等自托管端点，再到高级路由和故障转移配置。您需要至少配置一个提供商才能使用 Hermes。
...
| **Anthropic** | `hermes model` (Claude Max + 通过 OAuth 获得额外使用额度；也支持 Anthropic API 密钥或手动设置令牌——参见下方说明) |
...
| **NVIDIA Build** | 在 `~/.hermes/.env` 中设置 `NVIDIA_API_KEY` (提供商：`nvidia`；build.nvidia.com 上的 NIM 托管模型) |
| **Ollama Cloud** | `hermes model` → 选择 "Ollama Cloud" (提供商：`ollama-cloud`；云端托管的 Ollama API) |
...
**客户端标识。** Hermes Agent 发出的每个 Portal 请求都携带一个 `client=hermes-client-v<version>` 标签（例如 `client=hermes-client-v0.13.0`），该标签会自动与您安装的版本对齐。此标签在所有 Portal 路径上发送——主聊天循环、辅助调用、压缩摘要器、网页提取——并允许 Portal 端遥测区分 Hermes 流量与其他客户端流量。无需配置；当您执行 `hermes update` 时，标签会自动更新。
**JWT 认证（自动）。** Hermes 在处理 Portal 请求时，优先使用限定范围的 `inference:invoke` JWT，并将传统的非透明会话密钥路径作为备用方案。无需配置——凭证由 OAuth 流程管理并透明地轮换。被撤销的刷新令牌会被隔离，以避免重放循环。

:::info Codex 说明
OpenAI Codex 提供商通过设备码（打开 URL，输入代码）进行认证。Hermes 将生成的凭证存储在其自己的认证存储中，位于 `~/.hermes/auth.json`，并且当存在时，可以从 `~/.codex/auth.json` 导入现有的 Codex CLI 凭证。无需安装 Codex CLI。

如果令牌刷新因终端错误（HTTP 4xx、`invalid_grant`、已撤销的授权等）而失败，Hermes 会将刷新令牌标记为失效并停止重放它，这样你就不会看到大量相同的认证失败。下一次请求会显示一个类型化的重新认证消息。运行 `hermes auth add codex-oauth`（或 `hermes model` → OpenAI Codex）以启动一个新的设备码登录；隔离状态会在下一次成功的交换后清除。
:::

:::warning
即使在使用 Nous Portal、Codex 或自定义端点时，某些工具（视觉、网页摘要、MoA）会使用一个独立的“辅助”模型。默认情况下（`auxiliary.*.provider: "auto"`），Hermes 将这些任务路由到你的**主聊天模型**——即你在 `hermes model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::

:::tip Nous 工具网关
付费的 Nous Portal 订阅者还可以访问**[工具网关](/user-guide/features/tool-gateway)**——网络搜索、图像生成、TTS 和浏览器自动化都通过你的订阅路由。无需额外的 API 密钥。在新安装时，`hermes setup --portal` 会登录，将 Nous 设置为你的提供商，并在一个命令中开启网关。现有用户可以通过 `hermes model` 启用它，或通过 `hermes tools` 为每个工具单独启用。随时使用 `hermes portal info` 检查路由。
:::
...
如果你试图切换到一个尚未设置的提供商（例如，你只配置了 OpenRouter 但想使用 Anthropic），你需要 `hermes model`，而不是 `/model`。先退出你的会话（`Ctrl+C` 或 `/quit`），运行 `hermes model`，完成提供商设置，然后启动一个新会话。
...
如果你没有 Max + 额外积分，请改用 `ANTHROPIC_API_KEY`——请求将按 Token 计费，费用计入该密钥所属的组织（标准 API 定价，独立于任何 Claude 订阅）。
:::
...
当你通过 `hermes model` 选择 Anthropic OAuth 时，Hermes 优先使用 Claude Code 自身的凭证存储，而不是将令牌复制到 `~/.hermes/.env`。这使得可刷新的 Claude 凭证保持可刷新状态。
...
# 要求 GitHub Copilot CLI 在 PATH 中，并且存在现有的 `copilot login` 会话
```
...
可以使用环境变量 `NOVITA_BASE_URL`、`GLM_BASE_URL`、`KIMI_BASE_URL`、`MINIMAX_BASE_URL`、`MINIMAX_CN_BASE_URL`、`DASHSCOPE_BASE_URL`、`XIAOMI_BASE_URL`、`GMI_BASE_URL` 或 `TOKENHUB_BASE_URL` 覆盖基础 URL。

:::note Z.AI 端点自动检测
使用 Z.AI / GLM 提供商时，Hermes 会自动探测多个端点（全球版、中国版、编码变体）以找到接受您 API 密钥的端点。您无需手动设置 `GLM_BASE_URL` —— 可用的端点会被自动检测并缓存。
:::
...
SuperGrok 和 X Premium+ 订阅者可以使用浏览器 OAuth 登录，而不是使用 API 密钥 —— 在 `hermes model` 中选择 **xAI Grok OAuth (SuperGrok / Premium+)**，或者运行 `hermes auth add xai-oauth`。相同的 OAuth Bearer Token 会自动被直接调用 xAI 的工具（TTS、图像生成、视频生成、转录）复用。完整流程请参阅 [xAI Grok OAuth 指南](../guides/xai-grok-oauth.md) —— 如果 Hermes 在远程主机上运行，还请参阅 [通过 SSH / 远程主机进行 OAuth](../guides/oauth-over-ssh.md) 了解所需的 `ssh -L` 隧道。

当使用 xAI 作为提供商时（任何包含 `x.ai` 的基础 URL），Hermes 会自动启用提示词缓存，方法是在每个 API 请求中发送 `x-grok-conv-id` 头部。这会将请求路由到同一会话内的同一台服务器，允许 xAI 的基础设施重用缓存的系统提示词和对话历史记录。

无需配置 —— 当检测到 xAI 端点且会话 ID 可用时，缓存会自动激活。这减少了多轮对话的延迟和成本。

xAI 还提供了一个专用的 TTS 端点（`/v1/tts`）。在 `hermes tools` → Voice & TTS 中选择 **xAI TTS**，或查看 [语音与 TTS](../user-guide/features/tts.md#text-to-speech) 页面进行配置。

**已停用的 xAI 模型迁移（2026年5月15日）：** xAI 将于 2026-05-15 停用 `grok-4*`、`grok-3`、`grok-code-fast-1` 和 `grok-imagine-image-pro`。`hermes doctor` 和 `hermes chat` 启动时都会检测配置中是否仍指向已停用的引用，并打印推荐的替换项。使用 `hermes migrate xai` 进行一次性配置重写 —— 默认是试运行，添加 `--apply` 以写入更改（会自动创建一个带时间戳的 `config.yaml.bak-pre-migrate-xai-*` 备份）。
...
[NovitaAI](https://novita.ai) 是面向构建者和 Agent 的 AI 原生云。其三条产品线分别是：涵盖 200+ 模型的 Model API、用于构建和运行 AI Agent 的 Agent Sandbox，以及用于可扩展计算的 GPU Cloud，所有这些都可以在一个平台上获得。
...
[Ollama Cloud](https://ollama.com/cloud) 托管着与本地 Ollama 相同的开放权重模型目录，但无需 GPU 要求。在 `hermes model` 中选择 **Ollama Cloud**，粘贴来自 [ollama.com/settings/keys](https://ollama.com/settings/keys) 的 API 密钥，Hermes 会自动发现可用模型。
...
如果你订阅了阿里巴巴的 **Coding Plan**（一个与标准 DashScope API 访问分开的定价 SKU），Hermes 会将其作为一个独立的一级提供商暴露：`alibaba-coding-plan`。端点：`https://coding-intl.dashscope.aliyuncs.com/v1`。它与常规的 `alibaba` 提供商一样是 OpenAI 兼容的，但具有不同的基础 URL 和计费面。
...
`alibaba_coding` 使用与你的 `alibaba` 条目相同的 `DASHSCOPE_API_KEY`——无需单独的密钥，只需不同的路由目标。在该提供商注册之前，在 `config.yaml` 中设置 `provider: alibaba_coding` 的用户会静默地回退到 OpenRouter 路由。
...
`minimax-oauth` 使用 MiniMax 面向消费者的门户网站进行 OAuth 登录 —— 无需设置计费。`minimax` 和 `minimax-cn` 提供商使用 `MINIMAX_API_KEY` / `MINIMAX_CN_API_KEY` —— 用于程序化访问。完整操作指南请参阅 [MiniMax OAuth 指南](/guides/minimax-oauth)。
:::
...
Hermes 默认附带 Google 的**公开** `gemini-cli` 桌面 OAuth 客户端 —— 与 Google 在其开源 `gemini-cli` 中包含的凭证相同。桌面 OAuth 客户端不是机密的（PKCE 提供安全性）。你无需安装 `gemini-cli` 或注册自己的 GCP OAuth 客户端。
...
- `invalid_grant`（刷新 Token 被撤销）→ 凭证文件被清除，提示用户重新登录
...
:::warning 政策风险
Google 认为在第三方软件中使用 Gemini CLI OAuth 客户端违反政策。一些用户报告了账户限制。为了获得最低风险的体验，请通过 `gemini` 提供商使用您自己的 API 密钥。Hermes 会在 OAuth 开始前显示明确警告并要求明确确认。
:::
...
:::warning 遗留环境变量
`.env` 中的 `LLM_MODEL` 已被 **移除** —— `config.yaml` 是模型和端点配置的唯一真实来源。`OPENAI_BASE_URL` 仍然有效，但 **仅** 适用于 `openai-api` 提供商（它会覆盖直接使用 API 密钥访问的 OpenAI 端点）。对于其他提供商和自定义端点，请使用 `hermes model` 或直接在 `config.yaml` 中设置 `model.base_url`。如果您的 `.env` 中有过时的条目，它们将在下一次运行 `hermes setup` 或配置迁移时自动清除。
:::
...
**`hermes model`**（从终端运行，在任何聊天会话之外）是 **完整的提供商设置向导**。使用它来添加新提供商、运行 OAuth 流程、输入 API 密钥以及配置自定义端点。
...
:::tip
`/model custom`（不带模型名称）会查询您端点的 `/models` API，如果恰好加载了一个模型，则自动选择该模型。对于运行单个模型的本地服务器很有用。
:::
...
Hermes Agent 需要至少 **64,000 个 Token** 的上下文才能与工具一起进行代理使用。较小的窗口在启动时会被拒绝，因为系统提示词、工具架构和工作对话状态需要足够的空间来支持可靠的多步骤工作流。
...
如果响应似乎被截断，请在您的请求中添加 `max_tokens` 或在服务器上设置 `--default-max-tokens`。如果请求中未指定，SGLang 的默认值仅为每个响应 128 个 Token。
:::
...
**上下文长度 (`-c`):** 最近的构建默认值为 `0`，这会从 GGUF 元数据中读取模型的训练上下文。对于具有 128k+ 训练上下文的模型，尝试分配完整的 KV 缓存可能会导致 OOM。为 Hermes 显式设置 `-c` 至少为 64,000 个 Token。如果使用并行槽位 (`-np`)，总上下文将在槽位间分配 — 使用 `-c 64000 -np 4` 时，每个槽位仅获得 16k，这低于 Hermes 每个活动会话的最低要求。
...
您可以使用 CLI 来估算模型是否适合：`lms load model-name --context-length 64000 --estimate-only`

要设置持久的每模型默认值：我的模型选项卡 → 模型上的齿轮图标 → 设置上下文大小。
:::

**工具调用：** 自 LM Studio 0.3.6 起支持。具有原生工具调用训练的模型（Qwen 2.5、Llama 3.x、Mistral、Hermes）会自动检测并显示工具徽章。其他模型使用可能不太可靠的通用后备方案。

---

### WSL2 网络（Windows 用户）

由于 Hermes Agent 需要 Unix 环境，Windows 用户在 WSL2 内运行它。如果您的模型服务器（Ollama、LM Studio 等）运行在 **Windows 主机** 上，您需要桥接网络间隙 —— WSL2 使用具有自己子网的虚拟网络适配器，因此 WSL2 内的 `localhost` 指的是 Linux 虚拟机，**而不是** Windows 主机。

:::tip 两者都在 WSL2 中？没问题。
如果您的模型服务器也运行在 WSL2 内（对于 vLLM、SGLang 和 llama-server 很常见），`localhost` 会按预期工作 —— 它们共享相同的网络命名空间。跳过本节。
:::
...
如果您无法使用镜像模式，请从 WSL2 内部找到 Windows 主机 IP，并使用它代替 `localhost`：
...
如果您使用 **选项 2**（使用主机 IP 的 NAT 模式），Windows 上的模型服务器必须接受来自 `127.0.0.1` 之外的连接。默认情况下，大多数服务器只监听 localhost —— NAT 模式下的 WSL2 连接来自不同的虚拟子网，将被拒绝。在镜像模式下，`localhost` 直接映射，因此默认的 `127.0.0.1` 绑定工作正常。
...
Windows 防火墙将 WSL2 视为一个单独的网络（在 NAT 和镜像模式下都是如此）。如果按照上述步骤后连接仍然失败，请为您的模型服务器端口添加防火墙规则：
...
如果您收到列出模型的 JSON 响应，那就没问题。在您的 Hermes 配置中将相同的 URL 用作 `base_url`。
...
**修复：** 对于 Agent 使用，将上下文设置为至少 **64,000 个 Token**。请参阅上文每个服务器的部分以获取具体的标志。

#### 启动时显示“Context limit: 2048 tokens”

Hermes 从你服务器的 `/v1/models` 端点自动检测上下文长度。如果服务器报告的值较低（或根本不报告），Hermes 会使用模型声明的限制，这可能是错误的。
...
**可能的原因：**
1.  **服务器上的输出上限（`max_tokens`）过低** —— SGLang 默认每个响应 128 个 Token。在服务器上设置 `--default-max-tokens` 或在 config.yaml 中配置 Hermes 的 `model.max_tokens`。注意：`max_tokens` 仅控制响应长度 —— 它与你的对话历史记录能有多长（即 `context_length`）无关。
...
:::note
ClawRouter 需要在 Base 或 Solana 上有一个 USDC 资金的钱包用于支付。所有请求都通过 BlockRun 的后端 API 路由。运行 `npx @blockrun/clawrouter doctor` 以检查钱包状态。
:::
...
**`model.max_tokens`** 是**输出上限**——模型在*单次响应*中可生成的最大 Token 数。它与对话历史记录的长度无关。行业标准名称 `max_tokens` 是常见的混淆来源；Anthropic 的原生 API 已将其重命名为 `max_output_tokens` 以更清晰。

当自动检测获取的窗口大小错误时，设置 `context_length`。
仅当需要限制单个响应的长度时，设置 `model.max_tokens`。
:::
...
对于大多数设置，这可以开箱即用。该系统具有提供商感知能力——同一模型根据服务提供商的不同可能具有不同的上下文限制（例如，`claude-opus-4.6` 在 Anthropic 直接服务时为 1M，但在 GitHub Copilot 上为 128K）。
...
使用您的服务器文档中描述的格式。例如，vLLM Gemma 部署和一些 NVIDIA NIM 端点期望 `enable_thinking` 位于 `chat_template_kwargs` 下，而不是作为顶层的 `extra_body` 字段：
...
`hermes model` → 自定义端点向导现在会显式提示 `api_mode`，并将您的答案持久化到 `config.yaml`。当该字段留空时，基于 URL 的自动检测（例如 `/anthropic` 路径 → `anthropic_messages`）仍会作为后备方案进行。

**自定义提供商模型的原生视觉支持。** 如果您的自定义端点提供支持视觉的模型，但该模型不在 models.dev 中，请设置 `model.supports_vision: true`，以便 Hermes 将附加的图像原生路由（作为 `image_url` 部分），而不是通过 `vision_analyze` 进行预处理。单一开关——无需同时设置 `agent.image_input_mode: native`。
...
:::tip 故障排除
- 在 CLI 验证器修复 #15083 后，`hermes doctor` 不应为这些名称中的任何一个打印 `Unknown provider` 警告。
- 如果提供商的 `/v1/models` 端点无法访问（Perplexity 是常见情况），`hermes model` 将保留模型并发出警告，而不是硬性拒绝 — 参见 #15136。
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网络搜索和抓取。如果你希望本地运行 Firecrawl，可以将 Hermes 指向自托管的实例。完整的设置说明请参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。

**你将获得：** 无需 API 密钥，无速率限制，无每页成本，完全的数据主权。

**你将失去：** 云版本使用 Firecrawl 专有的 "Fire-engine" 进行高级反机器人绕过（Cloudflare、CAPTCHA、IP 轮换）。自托管版本使用基本的 fetch + Playwright，因此一些受保护的站点可能会失败。搜索使用 DuckDuckGo 而不是 Google。
...
- 设置为空字符串（或删除该行）以让 OpenRouter 选择可用的最强编码器——这是其文档中描述的当省略 plugins 块时的行为。
...
当激活时，备用机制会在会话中途切换模型和提供商，而不会丢失您的对话。链会按条目逐一尝试；每个会话的激活是一次性的。
...
:::tip
备用配置**仅**通过 `config.yaml` 进行配置——或通过 `hermes fallback` 交互式配置。有关其触发时机、链如何推进以及如何与辅助任务和委派交互的完整详细信息，请参阅 [备用提供商](/user-guide/features/fallback-providers)。
:::

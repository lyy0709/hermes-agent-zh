...
description: "逐步指南：设置一个可供整个团队使用的 Telegram 机器人，用于代码帮助、研究、系统管理等"
---

# 设置团队 Telegram 助手

本教程将引导您设置一个由 Hermes Agent 驱动的 Telegram 机器人，供多名团队成员使用。完成后，您的团队将拥有一个共享的 AI 助手，可以发送消息寻求代码、研究、系统管理等方面的帮助——并通过每用户授权确保安全。
...
- **在您的服务器上运行**，拥有完整的工具访问权限——终端、文件编辑、网络搜索、代码执行
- **每用户会话**——每个人都有自己的对话上下文
...
- **Hermes Agent 已安装** 在服务器或 VPS 上（不是您的笔记本电脑——机器人需要保持运行）。如果尚未安装，请遵循 [安装指南](/docs/getting-started/installation)。
- **您自己的 Telegram 帐户**（机器人所有者）
- **已配置 LLM 提供商**——至少需要在 `~/.hermes/.env` 中配置 OpenAI、Anthropic 或其他受支持提供商的 API 密钥
...
:::warning
请妥善保管您的机器人令牌。任何拥有该令牌的人都可以控制机器人。如果令牌泄露，请在 BotFather 中使用 `/revoke` 生成新令牌。
:::
...
这将通过箭头键选择引导您完成所有步骤。选择 **Telegram**，粘贴您的机器人令牌，并在提示时输入您的用户 ID。
...
您的 Telegram 用户 ID 是一个数字值（不是您的用户名）。查找方法：
...
:::info
Telegram 用户 ID 是像 `123456789` 这样的永久数字。它们与您的 `@username` 不同，后者可以更改。在允许列表中始终使用数字 ID。
:::
...
打开 Telegram，找到您的机器人，并向其发送消息。如果它回复了，说明一切正常。按 `Ctrl+C` 停止。
...
这将创建一个后台服务：默认情况下，在 Linux 上创建用户级 **systemd** 服务，在 macOS 上创建 **launchd** 服务，如果传递 `--system` 参数，则创建启动时 Linux 系统服务。
...
:::tip macOS PATH
launchd plist 在安装时捕获您的 shell PATH，以便消息网关子进程可以找到 Node.js 和 ffmpeg 等工具。如果您后来安装了新工具，请重新运行 `hermes gateway install` 以更新 plist。
:::
...
私聊配对更灵活——你不需要预先收集用户 ID。工作原理如下：

1.  **队友向机器人发送私聊消息** —— 由于他们不在允许列表中，机器人会回复一个一次性配对码：
    ```
    🔐 配对码：XKGH5N7P
    将此代码发送给机器人所有者以获取批准。
    ```
...
这样，即使有人要求机器人运行破坏性操作，你的主机系统也会受到保护。

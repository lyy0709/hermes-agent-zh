...
云 LLM API 按 Token 收费。一次繁重的编码会话可能花费 5-20 美元。对于个人项目、学习或隐私敏感的工作，这会累积起来 — 而且您将每次对话都发送给了第三方。
...
| **存储空间** | 5 GB 可用空间 | 30+ GB（用于多个模型） |
| **CPU** | 4 核 | 8+ 核（AMD EPYC, Ryzen, Intel Xeon） |
| **GPU** | 非必需 | 具有 8+ GB 显存的 NVIDIA GPU 可显著提升速度 |

:::tip 仅 CPU 也能工作，但响应会较慢
Ollama 可以在仅 CPU 的服务器上运行。现代 8 核 CPU 上的 9B 模型提供约 10 个 Token/秒。CPU 上的 31B 模型更慢（约 2–5 个 Token/秒）— 每次响应需要 30–120 秒，但可以工作。GPU 能显著改善这一点。对于仅 CPU 的设置，请通过环境变量（它不是 `config.yaml` 中的键）增加 API 超时时间：
...
:::info 多个模型
您可以拉取多个模型，并在 Hermes 内部使用 `/model` 命令在它们之间切换。Ollama 会按需将活动模型加载到内存中，并自动卸载空闲模型。
:::
...
```
你：列出此目录中的所有 Python 文件并统计每个文件的行数

你：读取 README.md 并总结这个项目是做什么的
...
一旦 Hermes 在 CLI 中本地工作，您可以将其暴露为 Telegram 或 Discord 机器人 — 仍然完全在您的硬件上运行。
...
现在在 Telegram 上给您的机器人发消息 — 它将使用您的本地模型进行回复。
...
默认的 Ollama 上下文（2048 个 Token）对于代理式工作来说太小了。请参阅 [步骤 6](#step-6-optimize-for-speed) 以增加它。

## 成本对比

以下是与云 API 相比，本地运行节省的成本，基于典型的编码会话（约 100K 输入 Token，约 20K 输出 Token）：
...
- **长上下文窗口** — 云模型提供 100K–1M 个 Token；本地模型通常为 8K–32K

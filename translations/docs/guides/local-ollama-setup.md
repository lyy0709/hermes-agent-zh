...
description: "使用 Ollama 和 Gemma 4 等开源模型，在您自己的机器上完全本地运行 Hermes Agent 的逐步指南，无需云 API 密钥或付费订阅"
---
...
云 LLM API 按 Token 收费。一次繁重的编码会话可能花费 5-20 美元。对于个人项目、学习或隐私敏感的工作来说，这会累积起来 — 而且您将每次对话都发送给了第三方。
...
:::tip 仅使用 CPU 也可以工作，但响应会较慢
Ollama 可以在仅使用 CPU 的服务器上运行。在现代 8 核 CPU 上，9B 模型提供约 10 个 Token/秒。31B 模型在 CPU 上更慢（约 2–5 个 Token/秒）— 每个响应需要 30–120 秒，但可以工作。GPU 可以显著改善这一点。对于仅使用 CPU 的设置，请通过环境变量（它不是 `config.yaml` 中的键）增加 API 超时时间：

```bash
# ~/.hermes/.env
HERMES_API_TIMEOUT=1800   # 30 分钟 — 为慢速本地模型留出充足时间
```
:::
...
默认情况下，Ollama 使用 2048 个 Token 的上下文。Hermes 在使用工具的代理式工作中至少需要 64,000 个 Token：
...
然后将您的 Hermes 配置更新为使用 `gemma4-64k` 作为模型名称。
...
如果您有 NVIDIA GPU，Ollama 会自动将模型层卸载到 GPU。使用以下命令检查：
...
一旦 Hermes 在 CLI 中本地工作，您可以将其公开为 Telegram 或 Discord 机器人 — 仍然完全在您的硬件上运行。
...
本地模型在处理复杂任务时可能会遇到困难。设置一个云回退，仅在本地模型失败时激活：
...
- **检查模型大小与内存：** 如果您的模型所需内存超过可用内存，则会交换到磁盘。使用较小的模型或增加内存。
- **检查 `ollama ps`：** 如果没有 GPU 层被卸载，则响应受 CPU 限制。这对于仅使用 CPU 的服务器是正常的。
...
默认的 Ollama 上下文（2048 个 Token）对于代理式工作来说太小。请参阅 [步骤 6](#step-6-optimize-for-speed) 以增加它。

## 成本对比

以下是在典型编码会话（约 100K 个输入 Token，约 20K 个输出 Token）中，本地运行与云 API 相比节省的成本：
...
- **终端命令** — Hermes 包装命令、运行它、读取输出，与模型无关
- **网页浏览** — 浏览器工具负责获取；模型只是解释结果
- **定时任务和计划任务** — 与云设置工作方式相同
- **多平台消息网关** — Telegram、Discord、Slack 都可以与本地模型一起工作

## 云模型表现更好的方面

- **非常复杂的多步推理** — 70B+ 或像 Claude Opus 这样的云模型明显更好
- **长上下文窗口** — 云模型提供 100K–1M 个 Token；本地运行时通常默认低于 Hermes 的 64K 最低要求，除非您进行配置
- **大型响应的速度** — 对于长文本生成，云推理比仅使用 CPU 的本地推理更快

最佳方案：日常任务使用本地模型，困难任务设置云回退。
...
sidebar_position: 10
title: "从 OpenClaw 迁移"
description: "将 OpenClaw / Clawdbot 设置迁移到 Hermes Agent 的完整指南 —— 迁移内容、配置映射关系以及迁移后需要检查的事项。"
---

# 从 OpenClaw 迁移

`hermes claw migrate` 命令将你的 OpenClaw（或旧版 Clawdbot/Moldbot）设置导入到 Hermes。本指南详细说明了迁移的具体内容、配置键的映射关系以及迁移后需要验证的事项。

:::tip
如果你的 OpenClaw 设置是多提供商的，`hermes setup --portal` 会将其整合为一次 OAuth 登录 —— 300+ 模型加上 Tool Gateway，一次登录即可使用。请参阅 [Nous Portal](/integrations/nous-portal)。
:::
...
迁移总是会在进行任何更改之前，显示将要导入内容的完整预览。请查看列表，然后确认以继续。

默认从 `~/.openclaw/` 读取。系统会自动检测旧的 `~/.clawdbot/` 或 `~/.moltbot/` 目录。对于旧的配置文件（`clawdbot.json`, `moltbot.json`）也是如此。
...
| `--no-backup` | 跳过迁移前对 `~/.hermes/` 的 zip 快照（默认情况下，在应用前会写入一个单独的还原点存档，位于 `~/.hermes/backups/pre-migration-*.zip`；可通过 `hermes import` 恢复）。 |
...
| 工作空间指令 | `workspace/AGENTS.md` | `AGENTS.md` 在 `--workspace-target` 中 | 需要 `--workspace-target` 标志 |
...
| 提供商 API 密钥 | `models.providers.*.apiKey` | `~/.hermes/.env` | 需要 `--migrate-secrets`。请参阅下面的 [API 密钥解析](#api-key-resolution)。 |
...
| Telegram | `credentials/telegram-default-allowFrom.json` | `TELEGRAM_ALLOWED_USERS` | 由 `allowFrom[]` 数组连接而成，以逗号分隔 |
...
| 消息网关认证 Token | `gateway.auth.token` | `.env` → `HERMES_GATEWAY_TOKEN` | 需要 `--migrate-secrets` |
| 工作目录 | `agents.defaults.workspace` | `config.yaml` → `terminal.cwd` | 旧版迁移可能仍会输出 `MESSAGING_CWD` 作为兼容性回退 |

### 已归档（无直接 Hermes 对应项）

这些内容保存到 `~/.hermes/migration/openclaw/<timestamp>/archive/` 供手动审查：
...
| 复杂通道配置 | `archive/channels-deep-config.json` | 手动平台配置 |

## API 密钥解析

当启用 `--migrate-secrets` 时，API 密钥按以下优先级从 **四个来源** 收集：
1. **配置值** — `openclaw.json` 中的 `models.providers.*.apiKey` 和 TTS 提供商密钥
2. **环境文件** — `~/.openclaw/.env`（包含 `OPENROUTER_API_KEY`、`ANTHROPIC_API_KEY` 等密钥）
3. **配置中的 env 子对象** — `openclaw.json` → `"env"` 或 `"env"."vars"`（某些设置将密钥存储在此处而非单独的 `.env` 文件中）
4. **认证配置文件** — `~/.openclaw/agents/main/agent/auth-profiles.json`（每个 Agent 的凭据）

配置值具有最高优先级。后续的每个来源会填补任何剩余的空白。
...
OpenClaw 配置中用于 Token 和 API 密钥的值可以有以下三种格式：
...
迁移过程会解析所有三种格式。对于环境变量模板和 `source: "env"` 的 SecretRef 对象，它会在 `~/.openclaw/.env` 和 `openclaw.json` 的 env 子对象中查找对应的值。对于 `source: "file"` 或 `source: "exec"` 的 SecretRef 对象，则无法自动解析——迁移过程会对此发出警告，这些值必须通过 `hermes config set` 手动添加到 Hermes。

## 迁移后

1.  **检查迁移报告** — 完成后会打印报告，包含已迁移、已跳过和存在冲突的项目数量。
2.  **审查归档文件** — `~/.hermes/migration/openclaw/<timestamp>/archive/` 目录下的任何内容都需要手动处理。
3.  **启动新会话** — 导入的技能和记忆条目会在新会话中生效，而非当前会话。
4.  **验证 API 密钥** — 运行 `hermes status` 以检查提供商认证状态。
...
根据你的 OpenClaw 版本，密钥可能存储在多个地方：直接存储在 `openclaw.json` 的 `models.providers.*.apiKey` 下、存储在 `~/.openclaw/.env` 中、存储在 `openclaw.json` 的 `"env"` 子对象中，或者存储在 `agents/main/agent/auth-profiles.json` 中。迁移过程会检查所有四个位置。如果密钥使用了 `source: "file"` 或 `source: "exec"` 的 SecretRef，则无法自动解析——请通过 `hermes config set` 添加它们。

### 迁移后技能未出现

导入的技能位于 `~/.hermes/skills/openclaw-imports/`。启动新会话以使它们生效，或者运行 `/skills` 来验证它们是否已加载。

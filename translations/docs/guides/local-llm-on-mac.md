...
本指南针对配备 Apple Silicon（M1 及更新型号）的 Mac。Intel Mac 可以使用 llama.cpp，但没有 GPU 加速——性能会显著降低。
:::
...
对于更大的模型（27B, 35B），您将需要 32 GB+ 的统一内存。9B 模型是 8-16 GB 机器的理想选择。
...
| `--cache-type-v q4_0` | 将 Value 缓存量化为 4 位。与上述一起，这使 KV 缓存内存比 f16 减少约 75%。 |
...
如果仍然内存不足，首先减少上下文大小（`-c`），然后尝试更小的量化（使用 Q3_K_M 而不是 Q4_K_M）。
...
omlx 默认在 `http://127.0.0.1:8000` 上提供模型服务。从应用程序 UI 开始服务，或者使用 CLI（如果可用）。
...
- **llama.cpp** 在提示处理方面表现出色——其 flash attention + 量化 KV 缓存流水线使您在大约 66 毫秒内获得第一个 Token。如果您正在构建感知响应性很重要的交互式应用程序（聊天机器人、自动完成），这是一个有意义的优势。
...
选择 **自定义端点** 并按照提示操作。它将询问基础 URL 和模型名称——使用您上面设置的任一后端的值。
...
如果您仍然遇到超时错误（例如，在慢速硬件上处理非常大的上下文），可以覆盖流式读取超时：
...
|---------|---------|----------------------|------------------|
| 流式读取（套接字级别） | 120秒 | 提高到 1800秒 | `HERMES_STREAM_READ_TIMEOUT` |
| 陈旧流检测 | 180秒 | 完全禁用 | `HERMES_STREAM_STALE_TIMEOUT` |
| API 调用（非流式） | 1800秒 | 无需更改 | `HERMES_API_TIMEOUT` |

流式读取超时是最可能引起问题的——它是接收下一个数据块的套接字级截止时间。在处理大型上下文的预填充阶段，本地模型在处理提示时可能几分钟没有输出。自动检测会透明地处理这种情况。
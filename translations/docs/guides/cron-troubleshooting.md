...
如果任务触发一次后从列表中消失，则它是一个一次性时间表（`30m`、`1d` 或 ISO 时间戳）——这是预期行为。
...
其他支持的平台包括 `mattermost`、`homeassistant`、`dingtalk`、`feishu`、`wecom`、`weixin`、`bluebubbles` 和 `webhook`。你也可以使用 `platform:chat_id` 语法（例如 `telegram:-1001234567890`）定位到特定的聊天。

如果交付失败，任务仍会运行——只是不会发送到任何地方。检查 `hermes cron list` 中更新的 `last_error` 字段（如果可用）。

### 检查 2：检查 `[SILENT]` 的使用

如果你的定时任务没有产生任何输出，或者 Agent 以 `[SILENT]` 响应，则交付会被抑制。这对于监控任务是故意的——但要确保你的提示词没有意外地抑制所有内容。

提示词说“如果没有变化，请用 [SILENT] 响应”也会静默地吞掉非空响应。检查你的条件逻辑。
...
- **Slack**：机器人必须已添加到工作区并具有 `chat:write` 作用域

### 检查 4：响应包装

默认情况下，定时任务响应会带有页眉和页脚包装（`config.yaml` 中的 `cron.wrap_response: true`）。某些平台或集成可能无法很好地处理此问题。要禁用：
...
技能名称区分大小写，并且必须与已安装技能的文件夹名称匹配。如果你的任务指定了 `ai-funding-daily-report`，但技能文件夹是 `ai-funding-daily-report`，请从 `hermes skills list` 确认确切名称。
...
**任务运行但未交付任何内容**
可能是交付目标问题（见上文“交付失败”）或静默抑制的响应（`[SILENT]`）。

**任务挂起或超时**
调度程序使用基于不活动的超时（默认为 600 秒，可通过 `HERMES_CRON_TIMEOUT` 环境变量配置，`0` 表示无限制）。只要 Agent 在主动调用工具，它就可以运行——计时器仅在持续不活动后触发。长时间运行的任务应使用脚本来处理数据收集，并且只交付结果。

### 检查 3：锁争用

调度程序使用基于文件的锁定来防止重叠计时。如果两个消息网关实例正在运行（或者 CLI 会话与消息网关冲突），任务可能会被延迟或跳过。
...
每个定时任务都会创建一个新的 AIAgent 会话，这可能涉及提供商身份验证和模型加载。对于时间敏感的时间表，请添加缓冲时间（例如，使用 `0 8 * * *` 而不是 `0 9 * * *`）。
...
输出数兆字节数据的脚本会减慢 Agent 速度，并可能达到 Token 限制。在脚本级别进行过滤/总结——只发出 Agent 需要推理的内容。
...
1. 使用 `hermes cron run <job_id>` 运行任务（在下一次消息网关计时时触发），并观察聊天输出中的错误

...
如果你期望任务自动触发，则需要一个正在运行的消息网关（`hermes gateway` 用于前台运行，或 `hermes gateway start` 用于已安装的服务）。对于一次性调试，你可以使用 `hermes cron tick` 手动触发计时。
...
输出数兆字节数据的脚本会减慢 Agent 的速度，并可能达到 Token 限制。在脚本级别进行过滤/总结——只输出 Agent 需要推理的内容。

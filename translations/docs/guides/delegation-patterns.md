...
sidebar_position: 13
title: "委派与并行工作"
description: "何时以及如何使用子Agent委派——用于并行研究、代码审查和多文件工作的模式"
---
...
delegate_task(tasks=[
    ...
    },
    {
        ...
        "context": "重点关注：已发货的服务器芯片、云提供商的采用情况、软件生态系统",
        "toolsets": ["web"]
    },
    {
        "goal": "研究实用的量子计算应用",
...
每个子Agent独立研究一个选项。由于它们是隔离的，不存在交叉影响——每个评估都基于其自身的优点。父Agent获得所有三个摘要并进行比较。
...
:::tip
每个子Agent都有自己的终端会话。只要它们编辑的是不同的文件，它们就可以在同一个项目目录中工作而不会相互干扰。如果两个子Agent可能修改同一个文件，请在并行工作完成后自己处理该文件。
:::
...
这通常是最有效的模式：`execute_code` 廉价地处理 10 多个顺序工具调用，然后一个子Agent在一个干净的上下文中执行单个昂贵的推理任务。
...
- **最多 3 个并行任务** —— 批次限制为 3 个并发子Agent
- **无嵌套** —— 子Agent不能调用 `delegate_task`、`clarify`、`memory`、`send_message` 或 `execute_code`
- **独立的终端** —— 每个子Agent都有自己的终端会话，具有独立的工作目录和状态
- **无对话历史** —— 子Agent只能看到你放在 `goal` 和 `context` 中的内容
...
**目标要具体。** “修复这个bug”太模糊。“修复 api/handlers.py 第 47 行中 process_request() 从 parse_body() 接收到 None 导致的 TypeError”则给子Agent提供了足够的工作信息。

**包含文件路径。** 子Agent不知道你的项目结构。始终包含相关文件、项目根目录和测试命令的绝对路径。

**利用委派实现上下文隔离。** 有时你需要一个新的视角。委派迫使你清晰地阐述问题，而子Agent会不带你在对话中积累的假设来处理它。

**检查结果。** 子Agent的摘要仅仅是摘要。如果子Agent说“修复了bug并且测试通过”，请通过自己运行测试或阅读差异来验证。

...
sidebar_position: 13
title: "委派与并行工作"
description: "何时以及如何使用子 Agent 委派——用于并行研究、代码审查和多文件工作的模式"
---

# 委派与并行工作

Hermes 可以生成隔离的子 Agent 来并行处理任务。每个子 Agent 都拥有自己的对话、终端会话和工具集。只有最终摘要会返回——中间的工具调用永远不会进入你的上下文窗口。
...
- 会因中间数据而淹没你上下文的任务
- 并行的独立工作流（同时进行研究 A 和研究 B）
- 需要全新上下文、希望 Agent 不带偏见处理的任务
...
三者同时运行。每个子 Agent 独立搜索网络并返回摘要。然后父 Agent 将它们综合成一份连贯的简报。

---

## 模式：代码审查

将安全审查委派给一个具有全新上下文的子 Agent，使其能够不带先入之见地审查代码：
...
关键是 `context` 字段——它必须包含子 Agent 所需的一切信息：
...
:::warning 上下文问题
子 Agent **完全不知道**你的对话内容。它们从零开始。如果你委派“修复我们讨论过的那个 bug”，子 Agent 根本不知道你指的是哪个 bug。务必明确传递文件路径、错误信息、项目结构和约束条件。
:::
...
针对每种方法：评估设置复杂性、查询能力、资源需求和维护开销。比较它们并推荐一种。
```
...
delegate_task(tasks=[
    ...
    },
    {
        ...
        将所有响应示例从旧格式更新为新格式。
        在 docs/api/overview.md 中添加“响应格式”部分来解释模式。""",
        "toolsets": ["terminal", "file"]
    }
])
```
...
:::tip
每个子 Agent 都有自己的终端会话。只要它们编辑的是不同的文件，它们就可以在同一个项目目录中工作而不会相互干扰。如果两个子 Agent 可能修改同一个文件，请在并行工作完成后自己处理该文件。
:::
...
这通常是最有效的模式：`execute_code` 廉价地处理 10 多个顺序工具调用，然后子 Agent 在干净的上下文中执行单个昂贵的推理任务。
...
限制工具集可以保持子 Agent 专注，并防止意外的副作用（例如研究子 Agent 运行 shell 命令）。

---

## 约束

- **默认 3 个并行任务** — 批处理默认使用 3 个并发子 Agent（可通过 config.yaml 中的 `delegation.max_concurrent_children` 配置）
- **无嵌套** — 子 Agent 不能调用 `delegate_task`、`clarify`、`memory`、`send_message` 或 `execute_code`
- **独立的终端** — 每个子 Agent 都有自己的终端会话，具有独立的工作目录和状态
- **无对话历史** — 子 Agent 只能看到你放在 `goal` 和 `context` 中的内容
...
**目标要具体。** “修复 bug”太模糊。“修复 api/handlers.py 第 47 行中 process_request() 从 parse_body() 接收到 None 导致的 TypeError”则给子 Agent 提供了足够的工作信息。

**包含文件路径。** 子 Agent 不知道你的项目结构。始终包含相关文件、项目根目录和测试命令的绝对路径。

**利用委派实现上下文隔离。** 有时你需要一个全新的视角。委派迫使你清晰地阐述问题，而子 Agent 会不带对话中积累的假设来处理它。

**检查结果。** 子 Agent 的摘要仅仅是摘要。如果子 Agent 说“修复了 bug 并且测试通过”，请通过自己运行测试或阅读差异来验证。

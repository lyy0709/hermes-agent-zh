...
完整功能参考，请参阅[子Agent委派](/docs/user-guide/features/delegation)。
...
三者同时运行。每个子Agent独立搜索网络并返回摘要。然后，父Agent将它们综合成一份连贯的简报。
...
:::warning 上下文问题
子Agent对你的对话**一无所知**。它们完全从头开始。如果你委派“修复我们正在讨论的bug”，子Agent根本不知道你指的是哪个bug。务必明确传递文件路径、错误消息、项目结构和约束条件。
:::
...
:::tip
每个子Agent都有自己的终端会话。只要它们编辑的是不同的文件，它们就可以在同一项目目录中工作而不会相互干扰。如果两个子Agent可能触及同一个文件，请在并行工作完成后自己处理该文件。
:::
...
这通常是最有效的模式：`execute_code` 廉价地处理 10 多个顺序工具调用，然后子Agent在干净的上下文中执行单个昂贵的推理任务。
...
- **嵌套委派需手动启用**：叶子子Agent（默认）不能调用 `delegate_task`、`clarify`、`memory`、`send_message` 或 `execute_code`。编排器子Agent（`role="orchestrator"`）保留 `delegate_task` 以进行进一步委派，但仅在 `delegation.max_spawn_depth` 提高到默认值 1 以上时（支持 1-3）；其他四个调用仍被阻止。可通过 `delegation.orchestrator_enabled: false` 全局禁用。
...
| `max_spawn_depth` | 1 | 1-3 | 可以生成多少层委派 |
...
**目标要具体。** “修复 bug”太模糊。“修复 api/handlers.py 第 47 行中 process_request() 从 parse_body() 接收到 None 导致的 TypeError”为子Agent提供了足够的工作信息。

**包含文件路径。** 子Agent不知道你的项目结构。始终包含相关文件的绝对路径、项目根目录和测试命令。

**利用委派实现上下文隔离。** 有时你需要一个新的视角。委派迫使你清晰地阐述问题，并且子Agent会不带对话中积累的假设来处理它。

**检查结果。** 子Agent的摘要仅仅是摘要。如果子Agent说“修复了 bug 并且测试通过”，请通过自己运行测试或阅读差异来验证。

---

*完整的委派参考——所有参数、ACP 集成和高级配置——请参阅[子Agent委派](/docs/user-guide/features/delegation)。*
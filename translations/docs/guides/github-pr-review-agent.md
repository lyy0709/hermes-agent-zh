...
**问题所在：** 你的团队创建 PR 的速度比你审查的速度还快。PR 等待审查的时间长达数天。初级开发者合并了有问题的代码，因为没人有时间检查。你每天早晨都在追赶代码差异，而不是构建新功能。

**解决方案：** 一个全天候监控你代码库的 AI Agent，审查每个新 PR 的缺陷、安全问题和代码质量，并发送摘要给你——这样你只需将时间花在真正需要人工判断的 PR 上。
...
- **已安装并认证 GitHub CLI (`gh`)**：
  ...
  gh auth login
  ```
- **配置消息传递**（可选）— [Telegram](/user-guide/messaging/telegram) 或 [Discord](/user-guide/messaging/discord)

:::tip 没有消息传递？没问题
使用 `deliver: "local"` 将审查结果保存到 `~/.hermes/cron/output/`。在配置通知之前进行测试非常有用。
:::
...
```
审查这个拉取请求。阅读差异，检查缺陷、安全问题和代码质量。具体说明行号并引用有问题的代码。
...
---
name: code-review
description: 审查拉取请求的缺陷、安全问题和代码质量
---
...
- 如果 PR 看起来不错，就说出来。不要无中生有。
- 以以下内容结尾：APPROVE / REQUEST_CHANGES / COMMENT
```

验证它是否已加载——启动 `hermes`，你应该在启动时的技能列表中看到 `code-review`。

---

## 步骤 4：教它你的约定

这是让审查员真正有用的地方。启动一个会话，教 Hermes 你团队的标准：
...
这些记忆将永久保存——审查员将强制执行你的约定，而无需每次都被告知。
...
不将结果发送到 Telegram，而是让 Agent 在 PR 本身发表评论：
...
- **[每日简报机器人](/guides/daily-briefing-bot)** — 将 PR 审查与你的早晨新闻摘要结合起来
- **[构建一个插件](/guides/build-a-hermes-plugin)** — 将审查逻辑封装成可共享的插件
- **[配置文件](/user-guide/profiles)** — 运行一个具有自己记忆和配置的专用审查员配置文件
- **[备用提供商](/user-guide/features/fallback-providers)** — 确保即使一个提供商宕机，审查也能运行
...
```yaml
author: Your Name
requires_env:          # 根据环境变量控制加载；安装期间提示
...
:::tip 旧模式
旧的 `shutil.copy2` 模式（将技能复制到 `~/.hermes/skills/`）仍然有效，但存在与内置技能名称冲突的风险。对于新插件，建议优先使用 `ctx.register_skill()`。
:::
...
- **临时性** — 注入仅在 API 调用时发生。对话历史中的原始用户消息永远不会被修改，并且不会持久化到会话数据库。
...
**活跃提供商门控：** 记忆插件 CLI 命令仅在其提供商是配置中活跃的 `memory.provider` 时才会出现。如果用户没有设置你的提供商，你的 CLI 命令将不会使帮助输出变得混乱。
...
def _handle_status(raw_args: str) -> str:
    """/mystatus 命令的处理函数 — 调用时传入命令名之后的所有内容。"""
...
需要编排工具（通过 `delegate_task` 生成子 Agent、调用 `file_edit` 等）的斜杠命令处理函数，应使用 `ctx.dispatch_tool()`，而不是直接访问框架内部。父 Agent 上下文（工作区提示、加载动画、模型继承）会自动连接。
...
**完整指南：** [技能中心](/docs/user-guide/features/skills#skills-hub) · [发布自定义技能源](/docs/user-guide/features/skills#publishing-a-custom-skill-tap)（仓库布局、最小示例、非默认路径、信任级别）。

...
当 `model_tools.py` 被导入时，它会调用 `tools/registry.py` 中的 `discover_builtin_tools()`。此函数使用 AST 解析扫描每个 `tools/*.py` 文件，查找包含顶层 `registry.register()` 调用的模块，然后导入它们：
...
这种自动发现意味着新的工具文件会被自动拾取——无需维护手动列表。AST 检查只匹配顶层的 `registry.register()` 调用（不匹配函数内部的调用），因此 `tools/` 中的辅助模块不会被导入。
...
在运行时，工具通过中央注册表分发，但某些 Agent 级工具（如记忆/待办事项/会话搜索处理）在 Agent 循环中有例外。
...
所有工具执行都在两个级别上包装了错误处理：

1.  **`registry.dispatch()`** —— 捕获来自处理程序的任何异常，并以 JSON 格式返回 `{"error": "Tool execution failed: ExceptionType: message"}`。
...
-   **工作线程（并行工具）** —— 使用存储在线程本地存储中的每个线程的持久化循环

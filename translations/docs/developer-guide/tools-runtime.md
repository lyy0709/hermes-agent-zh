...
这种自动发现意味着新的工具文件会被自动拾取——无需维护手动列表。AST 检查仅匹配顶层的 `registry.register()` 调用（不包括函数内部的调用），因此 `tools/` 中的辅助模块不会被导入。
...
5.  **动态模式修补** —— 过滤后，`execute_code` 和 `browser_navigate` 模式会被动态调整，仅引用实际通过过滤的工具（防止模型对不可用工具产生幻觉）。
...
在运行时，工具通过中央注册表分发，但某些 Agent 级工具（如 memory/todo/session-search 处理）在 Agent 循环中有例外。

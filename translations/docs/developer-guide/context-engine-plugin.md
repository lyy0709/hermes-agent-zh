...
上下文引擎插件用于替换内置的 `ContextCompressor`，提供替代的对话上下文管理策略。例如，一个无损上下文管理（LCM）引擎，它构建知识有向无环图（DAG）而非进行有损的摘要。
...
class LCMEngine(ContextEngine):
    ...
    def compress(self, messages: list, current_tokens: int = None,
        ...
        支持引导压缩的引擎应优先保留与其相关的信息，其他引擎可以忽略它。
        """
```
...
| `update_model(model, context_length, ...)` | 更新 context_length 和 threshold | 在模型切换时需要重新计算预算时 |
...
`on_session_reset()` 在 `/new` 或 `/reset` 时被调用，用于清除会话状态而不完全关闭。
...
`compression` 配置块（`compression.threshold`、`compression.protect_last_n` 等）是内置 `ContextCompressor` 特有的。如果需要，你的引擎应定义自己的配置格式，在初始化时从 `config.yaml` 读取。
...
- [记忆提供商插件](/developer-guide/memory-provider-plugin) — 类似的用于记忆的单选插件系统

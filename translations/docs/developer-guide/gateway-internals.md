...
| `gateway/builtin_hooks/` | 始终注册的 Hook 的扩展点（默认不包含任何 Hook） |
...
消息网关钩子是响应生命周期事件的 Python 模块：

### 消息网关钩子事件

| 事件 | 触发时机 |
|-------|-----------|
| `gateway:startup` | 消息网关进程启动 |
...
钩子从 `gateway/builtin_hooks/`（一个扩展点——当前在分发的版本中为空；`_register_builtin_hooks()` 是一个空操作存根）和 `~/.hermes/hooks/`（用户安装的）中发现。每个钩子都是一个包含 `HOOK.yaml` 清单和 `handler.py` 的目录。

## 记忆提供商集成

当启用记忆提供商插件（例如，Honcho）时：

1. 消息网关为每条消息创建一个带有会话 ID 的 `AIAgent`
...
**配置文件作用域与全局**：`start_gateway()` 使用配置文件作用域的 PID 文件。`hermes gateway stop` 仅停止当前配置文件的消息网关。`hermes gateway stop --all` 使用全局 `ps aux` 扫描来终止所有消息网关进程（在更新期间使用）。

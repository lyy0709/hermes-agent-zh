...
Hermes Agent 包含一个完整的环境框架，可将其工具调用能力与 [Atropos](https://github.com/NousResearch/atropos) 强化学习训练框架连接起来。这支持三种工作流：
...
-   **想进行强化学习训练？** 关于 Agent 驱动的界面，请参阅 [强化学习训练工具](/user-guide/features/rl-training)；关于手动执行，请参阅 [运行环境](#运行环境)
...
-   **终端后端配置** — 为沙盒化执行设置 `TERMINAL_ENV`（本地、Docker、Modal、Daytona、SSH、Singularity）
...
class AgentResult:
    ...
    tool_errors: List[ToolError]          # 工具分派期间遇到的错误
    managed_state: Optional[Dict]         # VLLM ManagedServer 状态（阶段 2）
```

### 工具上下文

`ToolContext` (`environments/tool_context.py`) 让奖励函数能够直接访问模型在其运行轨迹期间使用的**同一个沙盒**。`task_id` 作用域意味着所有状态（文件、进程、浏览器标签页）都被保留。
...
对于**第二阶段**（VLLM ManagedServer），服务器返回的是原始文本，没有结构化的工具调用。客户端解析器位于 `environments/tool_call_parsers/` 目录下，用于从原始输出中提取 `tool_calls`：
...
TBLite 是 TerminalBench2 的一个轻量级子类 —— 仅数据集和超时时间不同。由 OpenThoughts Agent 团队（Snorkel AI + Bespoke Labs）创建。数据集：[NousResearch/openthoughts-tblite](https://huggingface.co/datasets/NousResearch/openthoughts-tblite)。
...
一个最小化的自包含环境，带有内联任务（无需外部数据集）。用于**端到端验证完整技术栈**。每个任务要求模型在已知路径创建文件；验证器检查内容。
...
python environments/hermes_swe_env/hermes_swe_env.py serve \
    --openai.model_name YourModel
```
执行环境从 Atropos 接收任务项，运行 Agent 的 rollout，计算奖励，并将评分后的轨迹发送回训练流程。

## 两阶段操作

### 第一阶段：OpenAI 服务器（评估 / SFT）

使用 `server.chat_completion()` 并传入 `tools=` 参数。服务器（VLLM、SGLang、OpenRouter、OpenAI）原生处理工具调用解析。返回包含结构化 `tool_calls` 的 `ChatCompletion` 对象。

- **用于**：评估、SFT 数据生成、基准测试、功能测试
- 为 Atropos 流水线创建**占位符 Token**（因为 OpenAI API 不提供真实的 Token ID）

### 第二阶段：VLLM ManagedServer（完整 RL）

使用 ManagedServer 通过 `/generate` 端点获取精确的 Token ID 和 logprobs。客户端的[工具调用解析器](#tool-call-parsers)从原始输出中重建结构化的 `tool_calls`。

- **用于**：使用 GRPO/PPO 进行完整的 RL 训练
...
1.  **在** `environments/benchmarks/your-benchmark/` **目录下创建**
2.  **设置仅评估配置**：`eval_handling=STOP_TRAIN`，`steps_per_eval=1`，`total_steps=1`
3.  **存根化训练方法**：`collect_trajectories()` 返回 `(None, [])`，`score()` 返回 `None`
...
| `system_prompt` | `str` | `None` | 给 Agent 的系统消息 |
| `terminal_backend` | `str` | `"local"` | `local`、`docker`、`modal`、`daytona`、`ssh`、`singularity` |
| `terminal_timeout` | `int` | `120` | 每个终端命令的超时时间（秒） |
| `terminal_lifetime` | `int` | `3600` | 沙盒最大生命周期（秒） |
...
YAML 值会覆盖 `config_init()` 中的默认值。CLI 参数会覆盖 YAML 值：
...
### 对于使用 Modal 沙盒的基准测试（TB2、TBLite）

- [Modal](https://modal.com) 账户和 CLI：`pip install "hermes-agent[modal]"`
- 设置 `MODAL_TOKEN_ID` 和 `MODAL_TOKEN_SECRET` 环境变量

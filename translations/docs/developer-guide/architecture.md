...
本页面是 Hermes Agent 内部结构的顶层地图。您可以通过它来了解代码库的整体布局，然后深入特定子系统的文档以获取实现细节。
...
中央工具注册表（`tools/registry.py`），包含 20 个工具集中的 47 个已注册工具。每个工具文件在导入时自行注册。注册表处理模式收集、分发、可用性检查和错误包装。终端工具支持 6 个后端（本地、Docker、SSH、Daytona、Modal、Singularity）。

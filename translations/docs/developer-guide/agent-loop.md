...
核心编排引擎是 `run_agent.py` 中的 `AIAgent` 类——大约 9,200 行代码，负责处理从提示词组装到工具调度再到提供商故障转移的一切事务。
...
`AIAgent` 支持特定于平台的回调，这些回调在 CLI、消息网关和 ACP 集成中实现实时进度：
...
- 通过 `_get_budget_warning()` 实现两级预算压力：
  - 使用率达到 70%+（注意层级）：将 `[BUDGET: 迭代 X/Y。剩余 N 次迭代。开始整合你的工作。]` 追加到最后一条工具结果
  - 使用率达到 90%+（警告层级）：将 `[BUDGET WARNING: 迭代 X/Y。仅剩 N 次迭代。立即提供你的最终响应。]` 追加到最后一条工具结果

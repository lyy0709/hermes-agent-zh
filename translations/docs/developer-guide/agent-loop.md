...
核心编排引擎是 `run_agent.py` 中的 `AIAgent` 类——大约 9200 行代码，负责处理从提示词组装到工具调度再到提供商故障转移的一切事务。
...
`AIAgent` 支持特定于平台的回调，这些回调在 CLI、消息网关和 ACP 集成中实现实时进度显示：
...
- 在父 Agent 和子 Agent 之间共享——子 Agent 消耗父 Agent 的预算
- 通过 `_get_budget_warning()` 实现两级预算压力：
  - 使用率达到 70%+（谨慎层级）：将 `[BUDGET: Iteration X/Y. N iterations left. Start consolidating your work.]` 追加到最后一条工具结果中
  - 使用率达到 90%+（警告层级）：将 `[BUDGET WARNING: Iteration X/Y. Only N iteration(s) left. Provide your final response NOW.]` 追加到最后一条工具结果中
...
回退系统也独立覆盖辅助任务——视觉、压缩、网页提取和会话搜索各自都有通过 `auxiliary.*` 配置部分可配置的回退链。
...
| 文件 | 用途 |
|------|---------|
| `run_agent.py` | AIAgent 类——完整的 Agent 循环（约 9200 行） |

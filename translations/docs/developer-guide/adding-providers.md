...
sidebar_position: 5
title: "添加提供商"
description: "如何向 Hermes Agent 添加新的推理提供商 — 身份验证、运行时解析、CLI 流程、适配器、测试和文档"
---
...
- 提供商特定的身份验证或 Token 刷新
- 精选的模型目录
- 设置菜单或 `hermes model` 菜单条目
...
如果这些文件中的 ID 不同，提供商将感觉像是半连接的：身份验证可能有效，而 `/model`、设置或运行时解析会静默地忽略它。
...
如果提供商公开实时模型列表，优先使用该列表，并将 `_PROVIDER_MODELS` 作为静态回退。
...
一个提供商只有在交互式 `hermes model` 流程中出现时才是可发现的。
...
- 一个 `_model_flow_<provider>()` 函数，如果合适也可以复用 `_model_flow_api_key_provider()`
...
如果提供商没有合适的辅助模型默认值，辅助任务可能会回退失败，或者意外地使用昂贵的主模型。
...
对于原生提供商，至少验证一次工具调用，而不仅仅是纯文本响应。

## 步骤 10：更新面向用户的文档

如果该提供商打算作为一等选项发布，也请更新用户文档：
...
主聊天路径可能正常工作，但摘要、记忆刷新或视觉助手会失败，因为辅助路由从未更新。

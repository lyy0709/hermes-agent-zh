...
关键设计决策：
- **WAL 模式** 用于支持并发读取 + 一个写入者（消息网关多平台）
- **FTS5 虚拟表** 用于在所有会话消息中进行快速文本搜索
- **会话谱系** 通过 `parent_session_id` 链实现（由上下文压缩触发的拆分）
...
`schema_version` 表存储一个整数。在初始化时，`_init_schema()` 检查当前版本并按顺序应用迁移：
...
每个迁移都使用 `ALTER TABLE ADD COLUMN` 包装在 try/except 中，以处理列已存在的情况（幂等）。每个成功的迁移块之后，版本号都会递增。


## 写入竞争处理

多个 hermes 进程（消息网关 + CLI 会话 + 工作树 Agent）共享一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：

- **短 SQLite 超时**（1 秒）而不是默认的 30 秒
- **应用级重试** 并带有随机抖动（20-150 毫秒，最多 15 次重试）
- **BEGIN IMMEDIATE** 事务以在事务开始时暴露锁竞争
- **定期 WAL 检查点** 每 50 次成功写入后执行一次（PASSIVE 模式）

这避免了“车队效应”，即 SQLite 确定性的内部退避导致所有竞争写入者在相同的时间间隔重试。
...
- `context` — 匹配消息之前和之后的 1 条消息（内容截断为 200 个字符）
...
会话可以通过 `parent_session_id` 形成链。这发生在上下文压缩触发消息网关中的会话拆分时。

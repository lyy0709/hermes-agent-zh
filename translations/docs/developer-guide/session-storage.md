...
├── messages_fts_trigram  — 使用 trigram 分词器的 FTS5 虚拟表（CJK / 子字符串搜索）
├── state_meta            — 键/值元数据表
...
- 通过 `parent_session_id` 链实现**会话谱系**（由压缩触发的分割）
...
- `reasoning` 存储暴露原始推理文本的提供商提供的推理文本
...
`schema_version` 表存储一个整数。简单的列添加由 `_reconcile_columns()` 以声明式方式处理（它比较实际列与 `SCHEMA_SQL` 的差异，并 ADD 任何缺失的列）。版本控制的迁移链则保留给无法以声明式表达的数据迁移和索引/FTS 更改：
...
| 10 | 添加 `messages_fts_trigram` 虚拟表（用于 CJK / 子字符串搜索的 trigram 分词器）并回填现有行 |
| 11 | 重新索引 `messages_fts` 和 `messages_fts_trigram` 以覆盖 `tool_name` + `tool_calls`，并从外部内容模式切换到内联模式；删除旧触发器并回填每条消息行 |

声明式列添加使用 `ALTER TABLE ADD COLUMN` 包装在 try/except 中，以处理列已存在的情况（幂等操作）。版本号在每个成功的迁移块之后递增。


## 写入竞争处理

多个 hermes 进程（消息网关 + CLI 会话 + worktree agents）共享一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：
- **缩短 SQLite 超时时间**（1 秒），而非默认的 30 秒
- **应用级重试**，带有随机抖动（20-150 毫秒，最多重试 15 次）
- **BEGIN IMMEDIATE** 事务，以便在事务开始时暴露锁争用情况
- **定期 WAL 检查点**，每成功写入 50 次执行一次（PASSIVE 模式）

这避免了 SQLite 确定性的内部退避机制导致所有竞争写入者以相同间隔重试的“车队效应”。
...
db.create_session(
    ...
    user_id="user_1",
    parent_session_id=None,  # 或用于表示谱系的上一个会话 ID
)
...
| 布尔 OR | `docker OR kubernetes` | 任意一个词 |
...
- `context` — 匹配消息前后各 1 条消息（内容截断至 200 个字符）
...
会话可以通过 `parent_session_id` 形成链。这发生在消息网关中的上下文压缩触发会话拆分时。
...
这源自 `hermes_constants.get_hermes_home()`，默认解析为 `~/.hermes/`，或环境变量 `HERMES_HOME` 的值。

...
sidebar_position: 12
title: "Web 搜索提供商插件"
description: "如何为 Hermes Agent 构建 web-search/extract/crawl 后端插件"
---
...
| `supports_extract()` | — | `False` | `web_extract` 路由的功能标志 |
| `supports_crawl()` | — | `False` | 深度爬取模式的功能标志 |
...
如果您的提供商仅支持一个功能，请将其他标志保留为默认值（`False`），注册表将跳过该工具 —— 当用户仅使用 X 进行搜索并要求 Agent 提取时，他们不会看到误导性的“提供商 X 失败”错误。

...
和 `result.audit` 中，以便操作员查看哪个插件进行了哪个
调用。对于任何频繁触发的操作，它是可选的但建议使用。
...
宿主向提供商请求 JSON 输出，在本地解析作为
后备方案，如果安装了 `jsonschema` 则根据你的模式进行验证，
并在 `result.parsed` 上返回一个 Python 对象。如果
模型无法生成有效的 JSON，`result.parsed` 为 `None`，
`result.text` 则携带原始响应。
...
* **宿主管理的凭证。** OAuth Token、刷新流程、
  ...
  概念都适用。插件永远看不到
  Token；宿主通过 `result.audit` 将调用溯源。
* **有界。** 单次同步或异步调用。无流式传输，无工具
  循环，无会话状态需要管理。陈述输入，获取
  结果，返回。
* **故障关闭信任。** 一个你从未配置过的插件无法
...
计数；`result.provider` 和 `result.model` 携带溯源信息。
...
仓库（参考插件的配套仓库 — 不随 hermes-agent 本身捆绑）。关于异步接口（`acomplete()` / `acomplete_structured()` 与 `asyncio.gather()`），请查看同一仓库中的
...
`provider=` 和 `model=` 是独立的，并且遵循与宿主主配置（`model.provider` + `model.model`）相同的格式。仅设置 `model=` 将使用用户当前活动的提供商，但使用该提供商上的不同模型。同时设置两者将完全切换提供商。任何未经操作员选择启用的参数都会引发 `PluginLlmTrustError`。
...
输入是类型化的文本或图像块（原始字节会自动编码为 `data:` URL）。当提供了 `json_schema` 或 `json_mode=True` 时，宿主通过 `response_format` 请求 JSON 输出，在本地解析作为回退，并在安装了 `jsonschema` 的情况下根据你的模式进行验证。
...
**大多数插件永远不需要此部分。** 一个只调用 `ctx.llm.complete(messages=...)` 且不进行任何覆盖的插件，将针对用户当前活动的任何配置运行，并且无需配置即可工作。下面的配置块仅当插件特别希望固定使用与用户不同的模型或提供商时才相关。
  ...
  entries:
    my-plugin:
      llm:
        ...
        # （必须是 Hermes 已经知道的 — 与 `hermes model` 和 config.yaml 中的 model.provider 名称相同）。
        allow_provider_override: true

        # 可选地限制允许哪些提供商。使用 ["*"] 表示任何。
        allowed_providers:
          - openrouter
          - anthropic
        ...
        # 可选地限制允许哪些模型。使用 ["*"] 表示任何。
        # 模型与插件发送的任何字符串进行字面匹配 — Hermes 不进行任何查找。
        allowed_models:
          - openai/gpt-4o-mini
...
```
插件 ID 对于扁平插件是清单中的 `name:` 字段，对于嵌套插件则是基于路径派生的键（例如 `image_gen/openai`、`memory/honcho` 等）。
...
每个覆盖项都是独立控制的。授予 `allow_model_override` **不会**同时授予 `allow_provider_override` —— 一个被信任可以选取模型的插件，除非也获得了 provider 权限，否则仍然会被限制使用用户当前激活的 provider。

### 权限控制门无需强制执行的内容

*   请求整形参数 —— `temperature`、`max_tokens`、`timeout`、`system_prompt`、`purpose`、`messages`、`instructions`、`input`、`json_schema`、`schema_name`、`json_mode` —— 始终被允许；它们不涉及选择凭据或路由。
*   默认拒绝的立场意味着未配置的插件仍然可以完成有用的工作 —— 它只是针对当前激活的 provider 和 model 运行。操作员只需要为那些希望进行更精细路由的插件考虑 `plugins.entries`。

## 宿主负责的内容

以下是 `ctx.llm` 为插件处理的所有事情，你无需操心：

*   **Provider 解析。** 从用户配置（或在受信任时从显式覆盖）中读取 `model.provider` + `model.model`。
*   **认证。** 从 `~/.hermes/auth.json` / 环境变量中提取 API 密钥、OAuth Token 或刷新 Token，包括配置了凭据池的情况。插件永远不会看到它们。
*   **视觉模型路由。** 当提供了图像输入且用户当前激活的文本模型仅支持文本时，宿主会自动回退到配置的视觉模型。
*   **回退链。** 如果用户的主 provider 返回 5xx 或 429 错误，请求会在向插件返回错误之前，经过 Hermes 通常的、支持聚合器的回退流程。
*   **超时。** 遵守你的 `timeout=` 参数，回退到 `auxiliary.<task>.timeout` 配置或全局辅助默认值。
*   **JSON 整形。** 当你请求 JSON 时，向 provider 发送 `response_format`，如果 provider 返回了代码块形式的响应，则在本地重新解析。
*   **模式验证。** 当安装了 `jsonschema` 时，根据你的 `json_schema` 进行验证；否则记录调试信息并跳过严格验证。
*   **审计日志。** 每次调用都会向 `agent.log` 写入一行 INFO 级别的日志，包含插件 ID、provider/model、目的和 Token 总数。

## 插件负责的内容

*   **请求结构。** 聊天使用 `messages`，结构化任务使用 `instructions` + `input`。插件构建提示词；宿主运行它。
*   **模式。** 你希望返回的任何结构。宿主不会为你推断。
*   **错误处理。** `complete_structured()` 在输入为空或模式验证失败时引发 `ValueError`。当信任门拒绝覆盖时，会触发 `PluginLlmTrustError`。其他任何情况（provider 5xx 错误、未配置凭据、超时）都会引发 `auxiliary_client.call_llm()` 所引发的任何异常。
*   **成本。** 每次调用都针对用户付费的 provider 运行。不要不加思考地针对每条网关消息循环调用 `complete()`，而不考虑 Token 开销。

## 这在插件接口中的定位

现有的 `ctx.*` 方法扩展了 Hermes 的现有子系统：
...
`ctx.llm` 是第一个允许插件**带外**运行用户正在与之对话的同一模型的接口，而无需依赖上述任何方法。这是它唯一的工作。如果你的插件需要注册一个供 Agent 调用的工具，请使用 `register_tool`。如果它需要对生命周期事件做出反应，请使用 `register_hook`。如果它需要出于任何原因（无论是否结构化）进行自己的模型调用 —— 请使用 `ctx.llm`。
...
*   辅助客户端（底层引擎）：请参阅 [Provider Runtime](/developer-guide/provider-runtime)。
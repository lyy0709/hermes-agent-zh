...
面向模型的接口是一个单一的 `cronjob` 工具，具有类似操作的动作：`create`、`list`、`update`、`pause`、`resume`、`run`、`remove`。

## 任务存储

任务存储在 `~/.hermes/cron/jobs.json` 中，具有原子写入语义（先写入临时文件，然后重命名）。每个任务记录包含：
...
在消息网关模式下，调度器触发被集成到消息网关的主事件循环中。消息网关在其定期维护周期中调用 `scheduler.tick()`，该周期与消息处理并行运行。
...
这反映了消息网关的行为——没有它，定时任务 Agent 在遇到速率限制时将无法尝试恢复。
...
定时任务响应中的 `[SILENT]` 前缀会完全抑制交付——对于只需要写入文件或执行副作用的任务很有用。
...
调度器使用基于文件的锁定来防止重叠的触发执行同一批到期任务两次。这在消息网关模式下很重要，因为如果前一次触发花费的时间超过触发间隔，多个维护周期可能会重叠。

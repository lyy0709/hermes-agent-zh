...
| `gateway/run.py` | 消息网关集成——在长运行循环中进行定时任务滴答 |
...
| **时间间隔** | `every 2h`, `every 30m` | 重复性，按固定时间间隔触发 |
...
定时任务交付**不会**镜像到消息网关会话的对话历史中。它们仅存在于定时任务自身的会话中。这防止了目标聊天对话中出现消息交替违规。
...
调度器使用基于文件的锁定，以防止重叠的滴答两次执行同一批到期任务。这在消息网关模式下很重要，如果前一个滴答花费的时间超过滴答间隔，多个维护周期可能会重叠。

...
当能力可以表示为指令 + shell 命令 + 现有工具（arXiv 搜索、git 工作流、Docker 管理、PDF 处理）时，将其设为**技能**。

当需要端到端集成 API 密钥、自定义处理逻辑、二进制数据处理或流式处理（浏览器自动化、TTS、视觉分析）时，将其设为**工具**。
...
- `check_fn` 在构建工具定义时被调用 —— 如果它返回 `False`，工具会被静默排除
...
一些工具（`todo`、`memory`、`session_search`、`delegate_task`）需要访问每个会话的 Agent 状态。这些工具在到达注册表之前会被 `run_agent.py` 拦截。注册表仍然保存它们的模式，但如果绕过拦截，`dispatch()` 会返回一个回退错误。

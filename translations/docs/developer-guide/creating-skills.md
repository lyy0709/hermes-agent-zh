...
**`fallback_for_*` 的用例：** 创建一个在主工具不可用时的备用技能。例如，一个带有 `fallback_for_tools: [web_search]` 的 `duckduckgo-search` 技能，仅在 web 搜索工具（需要 API 密钥）未配置时显示。
...
当技能需要 API 密钥或 Token 时，使用 `required_environment_variables`。缺失的值**不会**在发现时隐藏该技能。相反，Hermes 会在本地 CLI 加载该技能时安全地提示用户输入。
...
用户可以跳过设置并继续加载技能。Hermes 永远不会向模型暴露原始的秘密值。消息网关和消息会话会显示本地设置指引，而不是在会话内收集秘密。
...
技能可以声明非秘密的设置，这些设置存储在 `config.yaml` 中的 `skills.config` 命名空间下。与环境变量（作为秘密存储在 `.env` 中）不同，配置设置用于路径、偏好设置和其他非敏感值。
...
:::tip 何时使用哪个
使用 `required_environment_variables` 处理 API 密钥、Token 和其他**秘密**（存储在 `~/.hermes/.env` 中，从不显示给模型）。使用 `config` 处理**路径、偏好设置和非敏感设置**（存储在 `config.yaml` 中，在 config show 中可见）。
:::
...
- **同步到 Modal** 沙盒中（在创建时 + 每条命令执行前，因此会话中的 OAuth 可以工作）
- 在**本地**后端上可用，无需任何特殊处理
...
| `${HERMES_SESSION_ID}` | 活动会话 ID（如果没有会话，则保持原样） |
...
Agent 看到替换后的绝对路径，并使用一个可以直接运行的命令调用 `terminal` 工具——无需路径计算，无需额外的 `skill_view` 往返。可以在 `config.yaml` 中通过设置 `skills.template_vars: false` 全局禁用替换。
#### 内联 Shell 代码片段（可选）

技能也可以在 SKILL.md 正文中嵌入以内联 Shell 代码片段形式编写的 `` !`cmd` ``。启用后，每个代码片段的 stdout 会在 Agent 读取消息之前内联到消息中，因此技能可以注入动态上下文：
...
如果您的技能是官方的且有用，但并非普遍需要（例如，付费服务集成、重量级依赖项），请将其放在 **`optional-skills/`** 中——它随仓库一起提供，可通过 `hermes skills browse` 发现（标记为 "official"），并以内置信任安装。

如果您的技能是专业化的、社区贡献的或小众的，则更适合放在 **Skills Hub** 中——将其上传到注册表并通过 `hermes skills install` 分享。
...
所有通过 Hub 安装的技能都会经过安全扫描器检查，检查内容包括：
...
如果您希望您的技能无需特定于 GitHub 的安装程序即可被发现，除了在仓库或市场中发布之外，还可以考虑从知名端点提供服务。
...
如果你试图添加一个新的第一方推理提供商，请结合本页阅读 [添加提供商](./adding-providers.md)。
...
- 无需重新运行设置即可切换提供商
- 即使当前 shell 中未导出 `OPENAI_BASE_URL`，也应继续工作的、保存在配置中的自定义端点
...
当辅助任务配置了提供商 `main` 时，Hermes 会通过与正常聊天相同的共享运行时路径来解析它。实际上这意味着：
...
参见 `tests/test_fallback_model.py` 以获取涵盖所有支持的提供商、一次性语义和边缘情况的全面测试。

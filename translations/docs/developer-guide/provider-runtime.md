...
如果你试图添加一个新的第一方推理 Provider，请阅读[添加 Provider](./adding-providers.md) 以及本页内容。
...
这个顺序很重要，因为 Hermes 将保存的模型/Provider 选择视为常规运行的唯一真实来源。这可以防止过时的 shell 导出变量静默覆盖用户在 `hermes model` 中最后选择的端点。
...
- 保存在配置中的自定义端点，即使当前 shell 中没有导出 `OPENAI_BASE_URL`，也应继续工作
...
原生 Anthropic 的凭证解析现在优先使用可刷新的 Claude Code 凭证，而不是复制的环境变量 Token。实际上这意味着：

- 当 Claude Code 凭证文件包含可刷新的认证信息时，它们被视为首选来源
- 手动的 `ANTHROPIC_TOKEN` / `CLAUDE_CODE_OAUTH_TOKEN` 值仍然可以作为显式覆盖使用
- Hermes 在调用原生 Messages API 之前，会预先检查 Anthropic 凭证的刷新状态
...
当辅助任务配置为使用 `main` Provider 时，Hermes 会通过与正常聊天相同的共享运行时路径来解析它。实际上这意味着：
...
-   **定时任务** (`cron/`)：使用固定的 Provider 运行，没有后备机制
-   **辅助任务**：使用它们自己独立的 Provider 自动检测链（参见上面的辅助模型路由）

### 测试覆盖

请参阅 `tests/test_fallback_model.py` 以获取全面的测试，涵盖所有支持的 Provider、一次性语义和边缘情况。

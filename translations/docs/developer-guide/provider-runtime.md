...
如果你正在尝试添加一个新的第一方推理 provider，请阅读[添加 Providers](./adding-providers.md) 和 [Model Provider Plugin 指南](./model-provider-plugin.md)以及本页内容。
...
这个顺序很重要，因为 Hermes 将保存的模型/provider 选择视为正常运行的真相来源。这可以防止过时的 shell 导出静默覆盖用户在 `hermes model` 中最后选择的端点。
...
定时任务**确实**支持后备：`run_job()` 从 `config.yaml` 读取 `fallback_providers`（或旧的 `fallback_model`）并将其传递给 `AIAgent(fallback_model=...)`，与消息网关的 `_load_fallback_model()` 模式匹配。请参阅[定时任务内部机制](./cron-internals.md)。
...
- `tests/gateway/test_fallback_eviction.py` — 消息网关对失败 providers 的驱逐

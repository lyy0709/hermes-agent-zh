...
Hermes Agent 采用双压缩系统和 Anthropic 提示词缓存机制，以高效管理长对话中的上下文窗口使用。
...
上下文管理基于 `ContextEngine` 抽象基类（`agent/context_engine.py`）。内置的 `ContextCompressor` 是默认实现，但插件可以用其他引擎（例如无损上下文管理）替换它。
...
- 跟踪 API 响应的 Token 使用量

通过 `config.yaml` 中的 `context.engine` 进行配置驱动选择。解析顺序为：
...
位于 `gateway/run.py` 中（搜索 `Session hygiene: auto-compress`）。这是一个**安全网**，在 Agent 处理消息之前运行。它防止会话在轮次之间（例如，在 Telegram/Discord 中过夜累积）变得过大而导致 API 失败。
...
消息网关清理阈值有意设置得比 Agent 的压缩器更高。将其设置为 50%（与 Agent 相同）会导致在长消息网关会话中每一轮都过早压缩。
...
compression:
  ...
  codex_gpt55_autoraise: true  # Codex OAuth 上的 gpt-5.5：将触发点提高到 85%（默认：true）
...
| `codex_gpt55_autoraise` | `true` | bool | 对于 ChatGPT Codex OAuth 路由上的 gpt-5.5，将触发点提高到 85%（见下文）。设置为 `false` 以保持全局 `threshold` |

### Codex gpt-5.5 阈值自动提升

ChatGPT Codex OAuth 后端将 gpt-5.5 的上下文窗口硬性限制在 **272K**（相同的模型在 OpenAI 的直接 API 和 OpenRouter 上暴露 1.05M，在 GitHub Copilot 上暴露 400K）。在默认的 50% 触发点，压缩将在约 136K 时触发 — 这是模型实际可用窗口的一半。当活动路由是 Codex OAuth（`provider: openai-codex`）且模型是 gpt-5.5 时，Hermes 将触发点提高到 **85%**（约 231K）并打印一次性通知及退出命令。仅此确切路由受影响；在任何其他提供商上的 gpt-5.5 保持您的全局 `threshold`。要退回到全局值：

```bash
hermes config set compression.codex_gpt55_autoraise false
```

### 计算值（以默认设置下 200K 上下文模型为例）
...
:::note 阈值源自 MAIN 模型的上下文窗口
`threshold_tokens` 始终是 `threshold × context_length`，其中 `context_length` 是**主 Agent 模型**的上下文窗口 — 绝不是辅助/摘要模型的。在默认 `0.50` 的 262,144 Token 模型上，阈值是 `262,144 × 0.50 = 131,072`。这个数字接近常见的 "128K 上下文" 是百分比巧合，而不是辅助模型窗口是触发点的标志。辅助模型的上下文窗口是另一个问题 — 请参阅下面的“摘要模型上下文长度”警告，了解它如何影响能否生成摘要，而不是何时触发压缩。
:::
## 压缩算法

`ContextCompressor.compress()` 方法遵循 4 阶段算法：

### 阶段 1：清理旧工具结果（低成本，无需调用 LLM）
...
这是一个低成本的前置处理，可以从冗长的工具输出（文件内容、终端输出、搜索结果）中节省大量 Token。
...
│  [3..N]  ← 中间轮次 → 被总结                                 │
│  [N..end] ← 尾部（基于 Token 预算 OR protect_last_n）       │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

尾部保护是**基于 Token 预算的**：从末尾向前遍历，累积 Token 直到预算耗尽。如果预算保护的消息数量较少，则回退到固定的 `protect_last_n` 计数。

边界会对齐以避免拆分 tool_call/tool_result 组。`_align_boundary_backward()` 方法会跳过连续的工具结果以找到父级助手消息，从而保持组的完整性。

### 阶段 3：生成结构化摘要

:::warning 摘要模型的上下文长度
摘要模型的上下文窗口**必须至少与**主 Agent 模型的一样大。整个中间部分会在一次 `call_llm(task="compression")` 调用中发送给摘要模型。如果摘要模型的上下文较小，API 将返回上下文长度错误 —— `_generate_summary()` 会捕获该错误，记录警告并返回 `None`。然后压缩器将**在没有摘要的情况下**丢弃中间轮次，悄无声息地丢失会话上下文。这是导致压缩质量下降的最常见原因。
:::

中间轮次使用辅助 LLM 和结构化模板进行总结：
...
在后续压缩中，之前的摘要会传递给 LLM，并附上**更新**它的指令，而不是从头开始总结。这可以在多次压缩中保留信息 —— 项目从“进行中”移动到“已完成”，添加新的进展，并移除过时的信息。
...
通过在多次对话轮次中缓存对话前缀，将输入 Token 成本降低约 75%。使用 Anthropic 的 `cache_control` 断点。
...
1.  **稳定的系统提示词**：系统提示词是断点 1，在所有轮次中被缓存。避免在对话中途修改它（压缩仅在第一次压缩时附加一个注释）。
2.  **消息顺序很重要**：缓存命中需要前缀匹配。在中间添加或删除消息会使之后的所有内容缓存失效。
3.  **压缩缓存交互**：压缩后，压缩区域的缓存失效，但系统提示词的缓存保留。滚动的 3 条消息窗口会在 1-2 轮内重新建立缓存。
4.  **TTL 选择**：默认是 `5m`（5 分钟）。对于用户在轮次之间休息的长时间运行会话，使用 `1h`。
...
中间上下文压力警告已被移除（参见 `run_agent.py` 中的 iteration-budget 块，其中注明："没有中间压力警告——它们导致模型在复杂任务上过早'放弃'"）。当提示词 Token 达到配置的 `compression.threshold`（默认为 50%）时，压缩会触发，没有预先的警告步骤；消息网关会话清理作为二级安全网，在模型上下文窗口的 85% 时触发。
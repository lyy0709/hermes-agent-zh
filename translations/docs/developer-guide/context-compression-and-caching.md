...
Hermes Agent 采用双重压缩系统和 Anthropic 提示词缓存，以在长对话中高效管理上下文窗口使用。
...
位于 `gateway/run.py`（搜索 `_maybe_compress_session`）。这是一个**安全网**，在 Agent 处理消息之前运行。它防止会话在回合之间（例如，在 Telegram/Discord 中过夜累积）变得过大而导致 API 失败。
...
消息网关清理阈值有意设置得比 Agent 的压缩器更高。将其设置为 50%（与 Agent 相同）会导致在长网关会话中每一回合都过早压缩。
...
compression:
  ...
  target_ratio: 0.20         # 将多少阈值作为尾部保留（默认：0.20）
  protect_last_n: 20         # 最小受保护的尾部消息数（默认：20）
  summary_model: null        # 覆盖摘要使用的模型（默认：使用辅助模型）
```
...
这是一个廉价的预扫描，可以从冗长的工具输出（文件内容、终端输出、搜索结果）中节省大量 Token。
...
1.  **稳定的系统提示词**：系统提示词是断点 1，并在所有轮次中被缓存。避免在对话中途修改它（压缩仅在首次压缩时附加一个说明）。
2. **消息顺序很重要**：缓存命中需要前缀匹配。在中间添加或删除消息会使之后的所有内容缓存失效。

3. **压缩缓存交互**：压缩后，被压缩区域的缓存会失效，但系统提示词的缓存会保留。滚动的 3 条消息窗口会在 1-2 轮对话内重新建立缓存。

4. **TTL 选择**：默认是 `5m`（5 分钟）。对于用户在两轮对话之间有长时间休息的长时间运行的会话，请使用 `1h`。
...
压缩后，如果使用量降至阈值 85% 以下，警告状态会被清除。如果压缩未能将使用量降低到警告水平以下（对话内容过于密集），警告会持续存在，但压缩不会再次触发，直到再次超过阈值。
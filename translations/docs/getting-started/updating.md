...
5.  **消息网关自动重启** — 更新完成后，正在运行的消息网关会被刷新，以便新代码立即生效。由服务管理的消息网关（Linux 上的 systemd，macOS 上的 launchd）通过服务管理器重启。当 Hermes 可以将正在运行的 PID 映射回某个配置文件时，手动启动的消息网关会自动重新启动。
...
对于高价值的配置文件（生产环境消息网关、共享团队安装），你可以选择对 `HERMES_HOME`（配置、认证、会话、技能、配对）进行更新前的完整备份：

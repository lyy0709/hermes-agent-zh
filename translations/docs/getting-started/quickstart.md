...
:::tip Android / Termux
如果您在手机上安装，请参阅专门的 [Termux 指南](./termux.md)，了解经过测试的手动路径、支持的附加功能以及当前 Android 特定的限制。
:::

:::tip Windows 用户
请先安装 [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install)，然后在您的 WSL2 终端内运行上述命令。
:::
...
Hermes Agent 需要一个至少具有 **64,000 Token** 上下文的模型。上下文窗口较小的模型无法为多步骤工具调用工作流保持足够的工作记忆，将在启动时被拒绝。大多数托管模型 (Claude, GPT, Gemini, Qwen, DeepSeek) 都轻松满足此要求。如果您运行本地模型，请将其上下文大小设置为至少 64K (例如，对于 llama.cpp 使用 `--ctx-size 65536`，对于 Ollama 使用 `-c 65536`)。
:::
...
就这样！您将看到一个欢迎横幅，显示您的模型、可用工具和技能。输入消息并按 Enter 键。
...
通过 Telegram、Discord、Slack、WhatsApp、Signal、电子邮件或 Home Assistant 从手机或其他界面与 Hermes 聊天：
...
按 `Ctrl+B` 进行录音，或使用 `/voice tts` 让 Hermes 说出它的回复。有关 CLI、Telegram、Discord 和 Discord 语音通道的完整设置，请参阅 [语音模式](../user-guide/features/voice-mode.md)。
...
Hermes 也可以作为 ACP 服务器运行，用于与 VS Code、Zed 和 JetBrains 等 ACP 兼容的编辑器：

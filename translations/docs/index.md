...
由 [Nous Research](https://nousresearch.com) 构建的自我改进 AI Agent。唯一内置学习循环的 Agent —— 它从经验中创建技能，在使用中改进它们，推动自身持久化知识，并在跨会话过程中构建一个不断深化的关于你的模型。
...
它不是一个绑定在 IDE 上的编码助手，也不是围绕单一 API 的聊天机器人包装器。它是一个**自主 Agent**，运行时间越长，能力越强。它存在于你放置它的任何地方 —— 一台 5 美元的 VPS、一个 GPU 集群，或是闲置时几乎不花钱的无服务器基础设施（Daytona, Modal）。当它在你从未亲自 SSH 登录的云 VM 上工作时，你可以从 Telegram 与它对话。它不绑定在你的笔记本电脑上。
...
- **委派与并行化** —— 生成隔离的子 Agent 以进行并行工作流。通过 `execute_code` 的程序化工具调用可将多步骤流水线压缩为单次推理调用

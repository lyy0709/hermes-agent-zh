...
description: "由 Nous Research 构建的自我改进 AI Agent。内置学习循环，可从经验中创建技能，在使用中改进它们，并跨会话记忆。"
...
由 [Nous Research](https://nousresearch.com) 构建的自我改进 AI Agent。唯一内置学习循环的 Agent —— 它从经验中创建技能，在使用中改进它们，促使自身持久化知识，并在跨会话中构建一个关于你的不断深化的模型。
...
查看完整的 **[安装指南](/docs/getting-started/installation)** 了解安装程序的作用、用户级与 root 级布局，以及 Windows 特定说明。

## 什么是 Hermes Agent？

它不是绑定在 IDE 上的编码助手，也不是围绕单个 API 的聊天机器人包装器。它是一个**自主 Agent**，运行时间越长，能力越强。它存在于你放置的任何地方 —— 一台 5 美元的 VPS、一个 GPU 集群，或几乎在空闲时不花费任何成本的 Serverless 基础设施（Daytona, Modal）。当它在你从未亲自 SSH 登录的云 VM 上工作时，你可以从 Telegram 与它交谈。它不绑定在你的笔记本电脑上。

## 快速链接

| | |
|---|---|
| 🚀 **[安装](/docs/getting-started/installation)** | 在 Linux、macOS、WSL2 或原生 Windows（早期测试版）上 60 秒内安装 |
...
| 🔧 **[工具与工具集](/docs/user-guide/features/tools)** | 70+ 内置工具及其配置方法 |
...
- **闭环学习循环** — Agent 管理的记忆，带有周期性提示、自主技能创建、使用中的技能自我改进、带 LLM 摘要的 FTS5 跨会话召回，以及 [Honcho](https://github.com/plastic-labs/honcho) 辩证用户建模
- **随处运行，不仅限于你的笔记本电脑** — 6 种终端后端：本地、Docker、SSH、Daytona、Singularity、Modal。Daytona 和 Modal 提供 Serverless 持久性 —— 你的执行环境在空闲时休眠，几乎不产生成本
- **存在于你所在之处** — CLI、Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Mattermost、Email、SMS、钉钉、飞书、企业微信、微信、QQ 机器人、元宝、BlueBubbles、Home Assistant、Microsoft Teams、Google Chat 等 —— 20+ 个平台，一个消息网关
- **由模型训练师构建** — 由 [Nous Research](https://nousresearch.com) 创建，该实验室是 Hermes、Nomos 和 Psyche 模型背后的团队。可与 [Nous Portal](https://portal.nousresearch.com)、[OpenRouter](https://openrouter.ai)、OpenAI 或任何端点配合使用
- **定时任务自动化** — 内置定时任务，可交付到任何平台
- **委派与并行化** — 生成隔离的子 Agent 以进行并行工作流。通过 `execute_code` 进行程序化工具调用，将多步骤流水线压缩为单次推理调用
...
- **[`/llms-full.txt`](/llms-full.txt)** — 每个文档页面连接成一个单独的 Markdown 文件，用于一次性摄取。约 1.8 MB。

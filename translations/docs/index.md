...
它不是一个绑定在 IDE 上的编码助手，也不是围绕单一 API 的聊天机器人包装器。它是一个**自主 Agent**，运行时间越长，能力越强。它可以存在于你放置的任何地方 —— 一台 5 美元的 VPS、一个 GPU 集群，或几乎在空闲时不产生成本的 Serverless 基础设施（Daytona, Modal）。当它在你从未亲自 SSH 登录的云 VM 上工作时，你可以通过 Telegram 与它交谈。它不绑定在你的笔记本电脑上。
...
- **闭环学习循环** —— Agent 管理的记忆，包含定期提示、自主技能创建、使用中的技能自我改进、带 LLM 摘要的 FTS5 跨会话召回，以及 [Honcho](https://github.com/plastic-labs/honcho) 辩证用户建模
...
- **委派与并行化** —— 生成隔离的子 Agent 以进行并行工作流。通过 `execute_code` 的程序化工具调用将多步流水线折叠为单次推理调用

...
查看完整的 **[安装指南](/docs/getting-started/installation)** 了解安装程序的功能、用户级与根目录布局以及 Windows 特定说明。

## 什么是 Hermes Agent？

它不是绑定在 IDE 上的编码助手，也不是围绕单一 API 的聊天机器人包装器。它是一个**自主 Agent**，运行时间越长，能力越强。它存在于你放置的任何地方 —— 一台 5 美元的 VPS、一个 GPU 集群，或是闲置时几乎不花钱的无服务器基础设施（Daytona、Modal）。当它在你从未亲自 SSH 登录的云 VM 上工作时，你可以从 Telegram 与它对话。它不绑定在你的笔记本电脑上。

## 快速链接

| | |
|---|---|
| 🚀 **[安装](/docs/getting-started/installation)** | 在 Linux、macOS、WSL2 或原生 Windows（早期测试版）上 60 秒内完成安装 |
...
| 📚 **[技能系统](/docs/user-guide/features/skills)** | Agent 创建并重用的程序性记忆 |
...
- **闭环学习循环** — Agent 管理的记忆，带有周期性提示、自主技能创建、使用中的技能自我改进、带 LLM 摘要的 FTS5 跨会话回忆，以及 [Honcho](https://github.com/plastic-labs/honcho) 辩证用户建模
- **随处运行，不限于你的笔记本电脑** — 6 种终端后端：本地、Docker、SSH、Daytona、Singularity、Modal。Daytona 和 Modal 提供无服务器持久化 —— 你的执行环境在闲置时休眠，几乎不产生费用
- **存在于你所在之处** — CLI、Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Mattermost、Email、SMS、钉钉、飞书、企业微信、微信、QQ 机器人、元宝、BlueBubbles、Home Assistant、Microsoft Teams、Google Chat 等 —— 一个消息网关支持 20+ 个平台
...
- **定时自动化** — 内置定时任务，可交付到任何平台
- **委派与并行化** — 生成隔离的子 Agent 以进行并行工作流。通过 `execute_code` 进行编程式工具调用，将多步流水线折叠为单次推理调用

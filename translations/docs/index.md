...
由 [Nous Research](https://nousresearch.com) 构建的自我改进 AI Agent。唯一内置学习循环的 Agent —— 它从经验中创建技能，在使用中改进它们，推动自身持久化知识，并在会话间构建一个不断深化的关于你的模型。
...
它不是绑定在 IDE 上的编码助手，也不是围绕单一 API 的聊天机器人包装器。它是一个**自主 Agent**，运行时间越长，能力越强。它存在于你放置的任何地方 —— 一台 5 美元的 VPS、一个 GPU 集群，或几乎在空闲时不花费任何成本的 Serverless 基础设施（Daytona, Modal）。当它在你从未亲自 SSH 进入的云 VM 上工作时，你可以从 Telegram 与它对话。它不绑定在你的笔记本电脑上。
...
- **随处运行，不限于你的笔记本电脑** —— 6 种终端后端：本地、Docker、SSH、Daytona、Singularity、Modal。Daytona 和 Modal 提供 Serverless 持久性 —— 你的执行环境在空闲时休眠，几乎不产生成本
- **存在于你所在之处** —— CLI、Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Mattermost、Email、SMS、钉钉、飞书、企业微信、BlueBubbles、Home Assistant、Microsoft Teams —— 通过一个消息网关支持 15+ 个平台
...
- **定时任务自动化** —— 内置 cron，可向任何平台交付结果
- **委派与并行化** —— 生成隔离的子 Agent 以进行并行工作流。通过 `execute_code` 进行编程式工具调用，将多步流水线压缩为单次推理调用

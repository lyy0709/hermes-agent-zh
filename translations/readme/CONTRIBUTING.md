...
感谢您为 Hermes Agent 贡献代码！本指南涵盖了您需要了解的一切：设置开发环境、理解架构、决定构建什么内容，以及让您的 PR 被合并。
...
如果您的技能是官方的且有用，但并非普遍需要（例如，付费服务集成、重量级依赖项），请将其放入 **`optional-skills/`** — 它随仓库一起发布，但默认不激活。用户可以通过 `hermes skills browse`（标记为“官方”）发现它，并使用 `hermes skills install` 安装（没有第三方警告，内置信任）。
...
**我们不再接受新的记忆提供商加入此仓库。** `plugins/memory/` 下的内置提供商集合（honcho、mem0、supermemory、byterover、hindsight、holographic、openviking、retaindb）已经关闭。如果您想添加新的记忆后端，请将其发布为**独立的插件仓库**，供用户安装到 `~/.hermes/plugins/`（或通过 pip entry point）。

独立的记忆插件：

- 实现相同的 `MemoryProvider` ABC（`agent/memory_provider.py`）— `sync_turn`、`prefetch`、`shutdown`，以及可选的 `post_setup(hermes_home, config)` 用于设置向导集成
...
在 `plugins/memory/` 下添加新目录的 PR 将被关闭，并会提示将提供商作为自己的仓库发布。现有的内置提供商保留；欢迎对它们进行错误修复。
...
# 替代方案（首先激活 venv）。在您打开 PR 之前，仍然建议使用包装器以与 GitHub Actions 保持一致：
pytest tests/ -v
```
...
│   ├── commands.py               # 中央斜杠命令注册表（CommandDef）、自动补全、消息网关助手
...
**连接到工具集（必需）**：内置工具是自动发现的：任何包含顶层 `registry.register(...)` 调用的 `tools/*.py` 文件，在 `model_tools` 加载时，都会由 `tools/registry.py` 中的 `discover_builtin_tools()` 导入。`model_tools.py` 中**没有**需要维护的手动导入列表。
...
消息网关和消息会话从不通过带内方式收集密钥；它们会指示用户本地运行 `hermes setup` 或更新 `~/.hermes/.env`。
...
2.  **SKILL.md 正文中引用的工具必须是原生 Hermes 工具或技能明确期望的 MCP 服务器。** 当技能需要某项功能时，请用反引号指出正确的工具名称：`` `terminal` ``、`` `web_extract` ``、`` `web_search` ``、`` `read_file` ``、`` `write_file` ``、`` `patch` ``、`` `search_files` ``、`` `vision_analyze` ``、`` `browser_navigate` ``、`` `delegate_task` ``、`` `image_generate` ``、`` `text_to_speech` ``、`` `cronjob` ``、`` `memory` ``、`` `skill_view` ``、`` `todo` ``、`` `execute_code` ``。
...
---
title: 技能贡献指南
description: 如何为 Hermes 添加新技能

---

## 技能提交清单

在提交 PR 之前，请对照此清单进行检查。

1. **`skill.yaml` 遵循标准格式。** 使用 `scripts/validate_skill_yaml.py` 进行验证。确保 `name` 字段与目录名匹配（例如 `skill_name` 对应 `skills/skill_name`）。`description` 应简明扼要（1-2 句话）。`tags` 必须包含 `skill` 和至少一个其他标签（例如 `web`、`dev`、`admin`）。`toolsets` 必须包含 `skills`。`platforms` 默认为 `["linux", "macos", "windows"]`，仅在必要时缩小范围。

2. **`## Prerequisites` 部分包含所有外部依赖项。** 如果技能依赖于 MCP 服务器，请命名该 MCP 服务器并在 `## Prerequisites` 中记录其设置。第三方 CLI（例如 `ffmpeg`、`gh`、特定 SDK）可以在脚本文件中调用，但描述中应将其交互表述为“通过 `terminal` 工具调用”，而不是手动 shell 会话。

3. **`platforms:` 根据实际脚本导入进行门控。** 使用仅限 POSIX 的原语（`fcntl`、`termios`、`os.setsid`、用于存活性检查的 `os.kill(pid, 0)`、`/proc`、硬编码的 `/tmp` 路径、`signal.SIGKILL`、bash heredocs、`osascript`、`apt`、`systemctl`）的技能必须通过 `platforms:` frontmatter 声明其支持的平台。默认立场是首先进行跨平台修复——使用 `tempfile.gettempdir()`、`pathlib.Path`、`psutil.pid_exists()`、Python 级别的过滤而非 `grep`。仅当依赖项真正受平台限制时才将门控范围缩小到更窄的集合（例如 `osascript` 仅限 macOS，`/proc` 仅限 Linux）。
...
5. **SKILL.md 正文使用现代章节顺序。** `# <技能> 技能` 标题，2-3 句话介绍说明其功能和不做什么，然后：
   - `## 何时使用` —— 触发条件
   - `## 先决条件` —— 环境变量、安装步骤、MCP 设置、API 密钥来源
   - `## 如何运行` —— 通过 `terminal` 工具的规范调用方式
   - `## 快速参考` —— 扁平化的命令/API 参考
   - `## 步骤` —— 带有可复制粘贴命令的编号步骤
   - `## 常见问题` —— 已知限制、速率限制、看似已损坏但实际正常的情况
   - `## 验证` —— 证明技能有效的单个命令

   复杂技能目标约 200 行，简单技能约 100 行。删除冗余的介绍性内容、营销性文字以及对已在 `## 先决条件` 中记录的环境变量的重复解释。
...
7. **测试位于 `tests/skills/test_<技能>_skill.py`** 并且仅使用标准库 + pytest + `unittest.mock`。不进行实时网络调用。通过 `scripts/run_tests.sh tests/skills/test_<skill>_skill.py -q` 运行。必须在封闭的 CI 环境下通过（没有 API 密钥泄漏）。对任何环境变量或文件系统依赖项使用 `monkeypatch` 和 `tmp_path`。

8. **`.env.example` 的添加内容被隔离到一个清晰分隔的块中。** 不要修改周围文件——贡献者提供的 `.env.example` 版本通常是过时的，并且在技能自身块之外的编辑将在抢救过程中被丢弃。用 `#` 注释所有值（这是文档，不是实时配置）。
...
1. **切勿调用 `os.kill(pid, 0)` 进行存活性检查。** `os.kill(pid, 0)` 是检查“此 PID 是否存活”的标准 POSIX 惯用法——信号 0 是无操作权限检查。**在 Windows 上，它并非无操作。** Python 的 Windows `os.kill` 将 `sig=0` 映射到 `CTRL_C_EVENT`（它们在整数值 0 处冲突），并通过 `GenerateConsoleCtrlEvent(0, pid)` 路由，该函数将 Ctrl+C 广播到包含目标 PID 的**整个控制台进程组**。“探测是否存活”会悄无声息地变成“杀死目标进程以及通常与其共享控制台的不相关进程”。参见 [bpo-14484](https://bugs.python.org/issue14484)（自 2012 年开放——出于兼容性原因永远不会修复）。
...
审计 grep 查找新的调用点：`rg "os\.kill\([^,]+,\s*0\s*\)"`。任何在非测试代码中的匹配，都可能是 Windows 静默终止 bug。
...
10. **Windows 上分离的后台守护进程需要 `pythonw.exe`，而不是 `python.exe`。** `python.exe` 总是分配或附加到一个控制台，这使其容易受到来自任何兄弟进程的 `CTRL_C_EVENT` 广播的影响。`pythonw.exe` 是无控制台的变体。结合 `subprocess.Popen(creationflags=...)` 中的 `CREATE_NO_WINDOW | DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP | CREATE_BREAKAWAY_FROM_JOB` 使用。参考实现见 `hermes_cli/gateway_windows.py::_spawn_detached`。

11. **使用 `subprocess.Popen` 调用 `.cmd` 或 `.bat` 包装器时，需要用 `shutil.which` 来解析。** 在 Windows 上向 `Popen` 传递 `"agent-browser"` 会找到 `node_modules/.bin/` 中无扩展名的 POSIX shebang 包装器，而 `CreateProcessW` 无法执行它 — 你会得到 `WinError 193 "not a valid Win32 application"`。使用 `shutil.which("agent-browser", path=local_bin)`，它会遵循 PATHEXT 并在 Windows 上选择 `.CMD` 变体。
...
14. **Windows 上已知被 OneDrive 重定向的路径：** 桌面、文档、图片、视频。当 OneDrive 备份启用时，“真实”路径是 `%USERPROFILE%\OneDrive\Desktop`（等等），而不是 `%USERPROFILE%\Desktop`（后者作为一个空壳存在）。通过 `ctypes` + `SHGetKnownFolderPath` 或读取 `Shell Folders` 注册表项来解析真实位置 — 永远不要假设 `~/Desktop`。
...
如果你为跨平台测试对 `sys.platform` 进行猴子补丁，也请同时补丁 `platform.system()` / `platform.release()` / `platform.mac_ver()` — 每个函数都会独立地重新读取真实的操作系统信息，因此半补丁的测试在 Windows 运行器上仍然会走错分支。
...
| **危险命令检测** | `tools/approval.py` 中的正则表达式模式，配合用户批准流程 |
...
在 2026 年 3 月的 [litellm 供应链攻击](https://github.com/BerriAI/litellm/issues/24512) 和 2026 年 5 月的 [Mini Shai-Hulud 蠕虫攻击活动](https://socket.dev/blog/tanstack-npm-packages-compromised-mini-shai-hulud-supply-chain-attack) 之后，所有依赖项必须遵循以下规则：
...
| **仅 CI 使用的 pip 安装** | `==exact` | 封闭的 CI 构建；可以接受变更。 |

**PR 中每个新的 PyPI 依赖项都必须有 `<next_major` 上限。** 添加无上限 `>=X.Y.Z` 规范的 PR 将被审阅者拒绝。`supply-chain-audit.yml` CI 工作流也会标记依赖清单的更改以供人工审阅。

...
- 它需要每次都必须精确执行的自定义处理逻辑（而不是依赖 LLM 解释的“尽力而为”）
...
如果您的技能是官方的且有用，但并非普遍需要（例如，付费服务集成、重量级依赖项），请将其放入 **`optional-skills/`** 目录 — 它随仓库一起发布，但默认不激活。用户可以通过 `hermes skills browse`（标记为“official”）发现它，并使用 `hermes skills install` 安装（没有第三方警告，内置信任）。
...
在 `plugins/memory/` 下添加新目录的 PR 将被关闭，并会提示您将提供商作为自己的仓库发布。现有的内置提供商保留；欢迎对它们进行错误修复。
...
- **临时注入**：系统提示词和预填充消息在 API 调用时注入，从不持久化到数据库或日志。
...
技能可以声明一些条件，这些条件基于当前会话中可用的工具和工具集来控制技能何时出现在系统提示词中。这主要用于**备用技能**——即仅当主要工具不可用时才应显示的替代方案。
...
# 本地浏览器备用方案 —— 当 Browserbase 不可用时显示
metadata:
  hermes:
...
2.  **SKILL.md 正文中引用的工具必须是原生 Hermes 工具或技能明确期望的 MCP 服务器。** 当技能需要某项功能时，请使用反引号指出正确的工具名称：`` `terminal` ``、`` `web_extract` ``、`` `web_search` ``、`` `read_file` ``、`` `write_file` ``、`` `patch` ``、`` `search_files` ``、`` `vision_analyze` ``、`` `browser_navigate` ``、`` `delegate_task` ``、`` `image_generate` ``、`` `text_to_speech` ``、`` `cronjob` ``、`` `memory` ``、`` `skill_view` ``、`` `todo` ``、`` `execute_code` ``。
...
- [ ] 技能目录位于 `skills/` 下，命名格式为 `skill_<name>`
- [ ] 包含 `SKILL.md` 文档
- [ ] 包含 `__init__.py` 文件
- [ ] 包含 `tests/` 目录
- [ ] 包含 `.env.example` 更新（如果需要）
- [ ] 代码通过 `scripts/check-windows-footguns.py` 检查
- [ ] 测试通过 `scripts/run_tests.sh tests/skills/test_<skill>_skill.py -q` 运行
- [ ] 技能通过 `hermes --toolsets skills -q "使用 X 技能执行 Y"` 验证
...
1. **`__init__.py` 必须定义 `skill` 变量。** 技能定义必须导出名为 `skill` 的变量，类型为 `Skill`。使用 `@skill` 装饰器或直接实例化 `Skill` 类。技能名称必须与目录名匹配（例如 `skill_foo` 目录中的技能必须命名为 `foo`）。技能描述应简明扼要，说明其用途。

2. **`SKILL.md` 是面向用户的文档。** 假设读者是终端用户，而非开发者。使用第二人称（"你"）和祈使语气（"运行此命令"）。如果技能依赖 MCP 服务器，请在 `## 前提条件` 中命名该 MCP 服务器并记录其设置。第三方 CLI（例如 `ffmpeg`、`gh`、特定 SDK）可以在脚本文件中调用，但文档中应描述为"通过 `terminal` 工具调用"，而非手动 shell 会话。

3. **`platforms:` 根据实际脚本导入进行门控。** 使用仅限 POSIX 的原语（`fcntl`、`termios`、`os.setsid`、用于存活性检查的 `os.kill(pid, 0)`、`/proc`、硬编码的 `/tmp` 路径、`signal.SIGKILL`、bash heredocs、`osascript`、`apt`、`systemctl`）的技能必须通过 `platforms:` frontmatter 声明其支持的平台。默认立场是首先进行跨平台修复——使用 `tempfile.gettempdir()`、`pathlib.Path`、`psutil.pid_exists()`、Python 级别的过滤而非 `grep`。仅当依赖项真正受平台限制时才限制到较窄的平台集（例如 `osascript` 仅限 macOS，`/proc` 仅限 Linux）。

4. **`author` 首先注明人类贡献者。** 对于外部贡献，贡献者的真实姓名 + GitHub 句柄放在首位（`Jane Doe (jane-doe)`）；"Hermes Agent" 是次要协作者。如果贡献者的提交显示 "Hermes Agent" 为作者（因为他们使用 Hermes 起草了技能），请将其替换为他们的真实姓名——功劳归于人类，而非工具。
...
6. **脚本放在 `scripts/` 中，参考文件放在 `references/` 中，模板放在 `templates/` 中。** 不要期望模型每次调用都内联编写解析器、XML 遍历器或非平凡逻辑——提供一个辅助脚本。在 SKILL.md 中通过相对于技能目录的路径引用脚本。
...
8. **`.env.example` 的添加内容被隔离到一个明确分隔的块中。** 不要修改周围文件——贡献者提供的 `.env.example` 版本通常已过时，在技能自身块之外的编辑将在清理过程中被丢弃。用 `#` 注释所有值（这是文档，不是实时配置）。

### 技能指南

- **除非绝对必要，否则不要使用外部依赖。** 优先使用标准库 Python、curl 和现有的 Hermes 工具（`web_extract`、`terminal`、`read_file`）。
- **渐进式披露。** 将最常见的工作流程放在前面。边缘情况和高级用法放在底部。
- **包含辅助脚本** 用于 XML/JSON 解析或复杂逻辑——不要期望 LLM 每次都能内联编写解析器。
...
> **提交 PR 前：** 运行 `scripts/check-windows-footguns.py` 以捕获你的差异中常见的 Windows 不安全模式。它是基于 grep 的，成本低廉；CI 也会在每个 PR 上运行它。

### 关键规则

1. **切勿使用 `os.kill(pid, 0)` 进行存活性检查。** `os.kill(pid, 0)` 是检查"此 PID 是否存活"的标准 POSIX 惯用法——信号 0 是无操作权限检查。**在 Windows 上，它并非无操作。** Python 的 Windows `os.kill` 将 `sig=0` 映射到 `CTRL_C_EVENT`（它们在整数值 0 处冲突）并通过 `GenerateConsoleCtrlEvent(0, pid)` 路由，这将 Ctrl+C 广播到包含目标 PID 的**整个控制台进程组**。"探测是否存活"会悄无声息地变成"杀死目标进程以及通常与其共享控制台的不相关进程"。参见 [bpo-14484](https://bugs.python.org/issue14484)（自 2012 年开放——出于兼容性原因永远不会修复）。
...
如果你特别需要使用 hermes 包装器（它在 pip install 完成前的脚手架阶段导入时有一个标准库回退），请使用 `gateway.status._pid_exists(pid)`。它会先调用 `psutil.pid_exists`，仅在 Windows 上且 psutil 因故缺失时，回退到手动实现的 `OpenProcess + WaitForSingleObject` 操作。

审计 grep 查找新的调用点：`rg "os\.kill\([^,]+,\s*0\s*\)"`。任何在非测试代码中的匹配，都可能是 Windows 静默终止 bug。

2. **在调用 shell 命令前使用 `shutil.which()` — 不要假设 Windows 拥有 Linux 拥有的工具。** `wmic` 在 Windows 10 21H1 及更高版本中已被移除。`ps`、`kill`、`grep`、`awk`、`fuser`、`lsof`、`pgrep` 以及大多数 POSIX CLI 工具在 Windows 上根本不存在。使用 `shutil.which("tool")` 测试可用性，并回退到 Windows 原生等效工具 — 通常是通过 `subprocess.run(["powershell", "-NoProfile", "-Command", ...])` 使用 PowerShell。
...
14. **Windows 上已知被 OneDrive 重定向的路径：** 桌面、文档、图片、视频。当启用 OneDrive 备份时，“真实”路径是 `%USERPROFILE%\OneDrive\Desktop`（等等），而不是 `%USERPROFILE%\Desktop`（后者作为一个空壳存在）。通过 `ctypes` + `SHGetKnownFolderPath` 或读取 `Shell Folders` 注册表项来解析真实位置 — 永远不要假设 `~/Desktop`。
15. **生成脚本中的 CRLF 与 LF 问题。** Windows 的 `cmd.exe` 和 `schtasks` 会逐行解析；混合或仅使用 LF 的行尾符可能会破坏多行的 `.cmd` / `.bat` 文件。在生成 Windows 将要执行的脚本时，请使用 `open(path, "w", encoding="utf-8", newline="\r\n")` —— 或者 `open(path, "wb")` + 显式的字节。

16. **命令行中的两种不同引用方案。** `subprocess.run(["schtasks", "/TR", some_cmd])` → schtasks 本身会解析 `/TR`，并且当任务触发时，`some_cmd` 字符串会被 `cmd.exe` 重新解析。不同的解析器，不同的转义规则。请使用两个独立的引用辅助函数，切勿混用。参考 `hermes_cli/gateway_windows.py::_quote_cmd_script_arg` 和 `_quote_schtasks_arg` 这一对函数。
...
如果你为了跨平台测试而修改了 `sys.platform`，也请一并修改 `platform.system()` / `platform.release()` / `platform.mac_ver()` —— 每个函数都会独立地重新读取真实的操作系统信息，因此半修改的测试在 Windows 运行器上仍然会走错分支。
...
| **仅限 CI 的 pip 安装** | `==exact` | 封闭的 CI 构建；变动是可接受的。 |
...
示例：
```
fix(cli): 修复当 model 为字符串时 save_config_value 中的崩溃问题

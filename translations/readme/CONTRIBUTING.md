...
感谢您为 Hermes Agent 贡献代码！本指南涵盖了您所需的一切：设置开发环境、理解架构、决定构建什么以及让您的 PR 被合并。
...
捆绑技能（位于 `skills/` 中）随每个 Hermes 安装一起发布。它们应该**对大多数用户广泛有用**：

- 文档处理、网络研究、常见的开发工作流、系统管理
- 被广泛人群定期使用

如果您的技能是官方的且有用，但并非普遍需要（例如，付费服务集成、重量级依赖项），请将其放入 **`optional-skills/`** 中 — 它随仓库一起发布，但默认不激活。用户可以通过 `hermes skills browse`（标记为“official”）发现它，并使用 `hermes skills install` 安装（没有第三方警告，内置信任）。
...
**我们不再接受将新的记忆提供商加入此仓库。** `plugins/memory/` 下的内置提供商集合（honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb）已经关闭。如果您想添加新的记忆后端，请将其发布为**独立的插件仓库**，供用户安装到 `~/.hermes/plugins/`（或通过 pip entry point）。
...
- 使用相同的发现系统 — `discover_memory_providers()` 从用户/项目插件目录和 pip entry point 中获取它们
- 通过 `post_setup()` 与 `hermes memory setup` 集成 — 无需接触核心代码
- 可以通过 `cli.py` 文件中的 `register_cli(subparser)` 注册自己的 CLI 子命令
- 获得与内置提供商相同的所有生命周期钩子和配置管道

在 `plugins/memory/` 下添加新目录的 PR 将被关闭，并会指引您将提供商作为自己的仓库发布。现有的内置提供商保留；欢迎对它们进行 Bug 修复。

这不是一个质量标准问题 — 这是一个耦合和维护决策。记忆提供商是最常见的插件类型，它们不应该都放在这个代码树中。
...
```bash
# 首选 — 与 CI 匹配（封闭环境，4 个 xdist worker）；参见 AGENTS.md
scripts/run_tests.sh

# 替代方案（首先激活虚拟环境）。在您打开 PR 之前，仍然建议使用包装器以确保与 GitHub Actions 一致：
pytest tests/ -v
```
...
├── model_tools.py            # 工具编排（tools/registry.py 的薄层）
...
| `~/.hermes/sessions/` | 消息网关路由索引（`sessions.json`）、请求转储面包屑、消息网关 `*.jsonl` 记录，以及（可选）当设置 `sessions.write_json_snapshots: true` 时的每个会话 JSON 快照。每个会话的快照默认关闭；state.db 是规范存储。 |
| `~/.hermes/cron/` | 定时任务数据 |
...
- **会话持久化**：所有对话都存储在 SQLite（`hermes_state.py`）中，支持全文搜索和唯一的会话标题。`~/.hermes/sessions/` 中的每个会话 JSON 快照已被 SQLite 存储取代，默认关闭；如果你有直接使用 JSON 文件的外部工具，可以通过设置 `sessions.write_json_snapshots: true` 重新启用。
- **临时注入**：系统提示词和预填充消息在 API 调用时注入，永远不会持久化到数据库或日志中。
- **提供商抽象**：Agent 可与任何 OpenAI 兼容的 API 配合工作。提供商解析在初始化时进行（Nous Portal OAuth、OpenRouter API 密钥或自定义端点）。
...
你仍然必须将工具名称添加到 `toolsets.py` 中的相应列表（例如 `_HERMES_CORE_TOOLS` 或专用的工具集）；否则工具会注册但永远不会暴露给 Agent。如果你引入了一个新的工具集，请在 `toolsets.py` 中添加它，并将其连接到相关的平台预设中。
...
**语义：**
- `fallback_for_*`：该技能是备用方案。当列出的工具/工具集可用时**隐藏**，当它们不可用时**显示**。将此用于高级工具的免费替代方案。
- `requires_*`：该技能需要某些工具才能运行。当列出的工具/工具集不可用时**隐藏**。将此用于依赖特定功能的技能（例如，仅在具有终端访问权限时才有意义的技能）。
- 如果两者都指定，则必须同时满足两个条件，技能才会出现。
- 如果两者都未指定，则始终显示该技能（向后兼容）。
...
每个新的或现代化的技能——无论是捆绑的、可选的还是贡献的——在合并前都必须满足这些标准。审阅者会拒绝违反这些标准的 PR。

1.  **`description` ≤ 60 个字符，一句话，以句号结尾。** 过长的描述会使技能列表 UI 臃肿，并且在加载许多技能时会分散模型的注意力。说明功能，而非实现。不要使用营销词汇（"强大"、"全面"、"无缝"、"先进"）。不要重复技能名称。使用以下代码验证：
    ```python
    import re, pathlib
...
2.  **SKILL.md 正文中引用的工具必须是原生的 Hermes 工具或技能明确期望的 MCP 服务器。** 当技能需要某项功能时，请用反引号指出正确的工具名称：`` `terminal` ``、`` `web_extract` ``、`` `web_search` ``、`` `read_file` ``、`` `write_file` ``、`` `patch` ``、`` `search_files` ``、`` `vision_analyze` ``、`` `browser_navigate` ``、`` `delegate_task` ``、`` `image_generate` ``、`` `text_to_speech` ``、`` `cronjob` ``、`` `memory` ``、`` `skill_view` ``、`` `todo` ``、`` `execute_code` ``。

    不要命名 Agent 已经封装好的 shell 实用程序：
    ...
    | `sed`, `awk` | `patch` |
    | `find`, `ls` | `search_files` (带 `target='files'`) |
...
1.  **`skill.yaml` 必须通过 `hermes validate` 的验证。** 使用 `hermes validate --skill-dir path/to/skill` 进行验证。如果验证失败，PR 将无法合并。确保 `skill.yaml` 包含 `name`、`description`、`author` 和 `toolsets` 字段。

2.  **技能必须通过 `terminal` 工具运行。** 技能描述应指导用户通过 `terminal` 工具调用脚本，而不是手动执行 shell 会话。如果技能依赖于 MCP 服务器，请在 `## Prerequisites` 部分命名该 MCP 服务器并记录其设置。第三方 CLI（例如 `ffmpeg`、`gh`、特定 SDK）可以在脚本文件中调用，但描述性文字应将其交互方式表述为“通过 `terminal` 工具调用”，而不是手动 shell 会话。

3.  **`platforms:` 字段需根据实际脚本导入进行审核。** 使用仅限 POSIX 的原语（`fcntl`、`termios`、`os.setsid`、用于存活性检查的 `os.kill(pid, 0)`、`/proc`、硬编码的 `/tmp` 路径、`signal.SIGKILL`、bash heredocs、`osascript`、`apt`、`systemctl`）的技能必须通过 `platforms:` frontmatter 字段声明其支持的平台。默认立场是首先进行跨平台修复——使用 `tempfile.gettempdir()`、`pathlib.Path`、`psutil.pid_exists()`、Python 级别的过滤而非 `grep`。仅当依赖项确实受平台限制时（例如 `osascript` 仅限 macOS，`/proc` 仅限 Linux），才将平台范围限制在较窄的集合。

4.  **`author` 字段首先注明人类贡献者。** 对于外部贡献，贡献者的真实姓名 + GitHub 用户名放在首位（`Jane Doe (jane-doe)`）；“Hermes Agent”作为次要协作者。如果贡献者的提交显示“Hermes Agent”为作者（因为他们使用 Hermes 来起草技能），请将其替换为他们的真实姓名——功劳归于人类，而非工具。

5.  **SKILL.md 正文使用现代章节顺序。** `# <技能> Skill` 标题，2-3 句话介绍说明其功能和不做什么，然后：
    - `## When to Use` — 触发条件
    - `## Prerequisites` — 环境变量、安装步骤、MCP 设置、API 密钥获取
    - `## How to Run` — 通过 `terminal` 工具的规范调用方式
    - `## Quick Reference` — 扁平化的命令/API 参考
    - `## Procedure` — 带有可复制粘贴命令的编号步骤
    - `## Pitfalls` — 已知限制、速率限制、看似有问题但实际正常的情况
    - `## Verification` — 证明技能有效的单一命令

    复杂技能目标约 200 行，简单技能约 100 行。删除冗余的介绍性内容、营销文案以及对已在 `## Prerequisites` 中记录的环境变量的重复解释。

6.  **脚本放在 `scripts/` 目录，参考文件放在 `references/` 目录，模板放在 `templates/` 目录。** 不要期望模型每次调用都内联编写解析器、XML 遍历器或非平凡逻辑——提供一个辅助脚本。在 SKILL.md 中通过相对于技能目录的路径引用脚本。

7.  **测试位于 `tests/skills/test_<skill>_skill.py`** 并且仅使用标准库 + pytest + `unittest.mock`。不进行实时的网络调用。通过 `scripts/run_tests.sh tests/skills/test_<skill>_skill.py -q` 运行。必须在封闭的 CI 环境中通过（没有 API 密钥泄露）。对任何环境变量或文件系统依赖使用 `monkeypatch` 和 `tmp_path`。

8.  **`.env.example` 的添加内容被隔离在一个清晰分隔的块中。** 不要修改周围文件——贡献者提供的 `.env.example` 版本通常是过时的，并且在抢救过程中，技能自身块之外的编辑将被丢弃。用 `#` 注释所有值（这是文档，不是实时配置）。
...
- **测试它。** 运行 `hermes --toolsets skills -q "Use the X skill to do Y"` 并验证 Agent 是否正确遵循指令。
...
如果你特别需要使用 hermes 包装器（它在 pip install 完成前的脚手架阶段导入时有一个标准库回退方案），请使用 `gateway.status._pid_exists(pid)`。它会先调用 `psutil.pid_exists`，并且仅在 psutil 因故缺失时，在 Windows 上回退到手动实现的 `OpenProcess + WaitForSingleObject` 操作。

审计 grep 查找新的调用点：`rg "os\.kill\([^,]+,\s*0\s*\)"`。在非测试代码中的任何匹配点都推定为 Windows 静默终止 bug。

2. **在执行 shell 命令前使用 `shutil.which()` — 不要假设 Windows 拥有 Linux 拥有的工具。** `wmic` 在 Windows 10 21H1 及更高版本中已被移除。`ps`、`kill`、`grep`、`awk`、`fuser`、`lsof`、`pgrep` 以及大多数 POSIX CLI 工具在 Windows 上根本不存在。使用 `shutil.which("tool")` 测试可用性，并回退到 Windows 原生等效方案 — 通常是通过 `subprocess.run(["powershell", "-NoProfile", "-Command", ...])` 使用 PowerShell。

对于进程枚举：PowerShell 的 `Get-CimInstance Win32_Process` 是 `wmic process` 的现代替代品。请参考 `hermes_cli/gateway.py::_scan_gateway_pids` 中的模式。
...
5. **进程管理。** `os.setsid()`、`os.killpg()`、`os.fork()`、`os.getuid()` 和 POSIX 信号处理在 Windows 上有所不同。使用 `platform.system()`、`sys.platform` 或 `hasattr(os, "setsid")` 进行防护：
   ...
   **首选：** 要终止一个进程及其子进程（即 `os.killpg` 在 POSIX 上的功能），请使用 `psutil` — 它在所有平台上都有效：
   ```python
   import psutil
   try:
...
6. **Windows 上不存在的信号：`SIGALRM`、`SIGCHLD`、`SIGHUP`、`SIGUSR1`、`SIGUSR2`、`SIGPIPE`、`SIGQUIT`、`SIGKILL`。** 如果你在 Windows 上引用它们，Python 的 `signal` 模块会在导入时引发 `AttributeError`。使用 `getattr(signal, "SIGKILL", signal.SIGTERM)` 或将整个代码块放在平台检查之后。`loop.add_signal_handler` 在 Windows 上会引发 `NotImplementedError` — 始终捕获它。
...
9. **POSIX 文件模式（0o600、0o644 等）在 NTFS 上默认不被强制执行。** 断言 `stat().st_mode & 0o777` 的测试必须在 Windows 上跳过 — 这个概念不适用。如果需要 Windows 机密文件保护，请使用 ACL（`icacls`、`pywin32`）。

10. **Windows 上分离的后台守护进程需要 `pythonw.exe`，而不是 `python.exe`。** `python.exe` 总是分配或附加到一个控制台，这使得它容易受到来自任何兄弟进程的 `CTRL_C_EVENT` 广播的影响。`pythonw.exe` 是无控制台的变体。结合 `subprocess.Popen(creationflags=...)` 中的 `CREATE_NO_WINDOW | DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP | CREATE_BREAKAWAY_FROM_JOB` 使用。请参考 `hermes_cli/gateway_windows.py::_spawn_detached` 中的参考实现。

11. **使用 `.cmd` 或 `.bat` 包装器的 `subprocess.Popen` 需要 `shutil.which` 来解析。** 在 Windows 上向 `Popen` 传递 `"agent-browser"` 会找到 `node_modules/.bin/` 中无扩展名的 POSIX shebang 包装器，而 `CreateProcessW` 无法执行它 — 你会得到 `WinError 193 "not a valid Win32 application"`。使用 `shutil.which("agent-browser", path=local_bin)`，它会遵循 PATHEXT 并在 Windows 上选择 `.CMD` 变体。

12. **不要使用 shell shebang 作为运行 Python 的方式。** `#!/usr/bin/env python` 仅在文件通过 Unix shell 执行时有效。在 Windows 上，即使文件有 shebang 行，`subprocess.run(["./myscript.py"])` 也会失败。始终显式调用 Python：`[sys.executable, "myscript.py"]`。
...
14. **Windows 上已知被 OneDrive 重定向的路径：** 桌面、文档、图片、视频。当 OneDrive 备份启用时，“真实”路径是 `%USERPROFILE%\OneDrive\Desktop`（等等），而不是 `%USERPROFILE%\Desktop`（它作为一个空壳存在）。通过 `ctypes` + `SHGetKnownFolderPath` 或读取 `Shell Folders` 注册表项来解析真实位置 — 永远不要假设 `~/Desktop`。
15. **生成脚本中的 CRLF 与 LF 问题。** Windows 的 `cmd.exe` 和 `schtasks` 会逐行解析；混合或仅使用 LF 的行尾可能会破坏多行的 `.cmd` / `.bat` 文件。在生成 Windows 将执行的脚本时，请使用 `open(path, "w", encoding="utf-8", newline="\r\n")` —— 或者 `open(path, "wb")` + 显式的字节。

16. **命令行中的两种不同引用方案。** `subprocess.run(["schtasks", "/TR", some_cmd])` → schtasks 本身会解析 `/TR`，并且当任务触发时，`some_cmd` 字符串会被 `cmd.exe` 重新解析。不同的解析器，不同的转义规则。使用两个独立的引用辅助函数，切勿混用。参考 `hermes_cli/gateway_windows.py::_quote_cmd_script_arg` 和 `_quote_schtasks_arg` 这一对函数。
...
如果为了跨平台测试而修改 `sys.platform`，请同时修改 `platform.system()` / `platform.release()` / `platform.mac_ver()` —— 每个函数都会独立地重新读取真实的操作系统信息，因此半修改的测试在 Windows 运行器上仍然会走错分支。
...
- **使用 `os.path.realpath()` 解析符号链接**，然后再进行基于路径的访问控制检查
...
在 2026 年 3 月的 [litellm 供应链攻击](https://github.com/BerriAI/litellm/issues/24512) 和 2026 年 5 月的 [Mini Shai-Hulud 蠕虫攻击](https://socket.dev/blog/tanstack-npm-packages-compromised-mini-shai-hulud-supply-chain-attack) 之后，所有依赖必须遵循以下规则：
...
| **仅限 CI 的 pip 安装** | `==exact` | 封闭的 CI 构建；可以接受变更。 |

**PR 中每个新的 PyPI 依赖都必须有 `<next_major` 上限。** 添加无上限 `>=X.Y.Z` 规范的 PR 将被审阅者拒绝。`supply-chain-audit.yml` CI 工作流也会标记依赖清单的更改以供人工审阅。
...
# ❌ 被拒绝 —— 对于 1.0 之前版本太宽松（允许 80 个次要版本）
"some-package>=0.20,<1"
```

**参考 PR：** #2796 (litellm 移除), #2810 (上限传递), #9801 (SHA 固定 + supply-chain-audit CI)。
...
示例：
```
fix(cli): 修复当模型为字符串时 save_config_value 中的崩溃问题

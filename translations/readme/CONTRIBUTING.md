...
- 它不需要自定义的 Python 集成或内置于 Agent 框架中的 API 密钥管理
...
技能可以声明条件，根据当前会话中可用的工具和工具集来控制它们何时出现在系统提示词中。这主要用于**后备技能**——即仅当主要工具不可用时才应显示的替代方案。
...
技能可以通过 `required_environment_variables` frontmatter 字段声明安全的加载时设置元数据。缺少值不会将技能从发现中隐藏；它们只会在技能实际加载时触发仅限 CLI 的安全提示。
...
1.  **`termios` 和 `fcntl` 仅适用于 Unix 系统。** 务必捕获 `ImportError` 和 `NotImplementedError`：
    ```python
    try:
...
2.  **文件编码。** Windows 可能以 `cp1252` 编码保存 `.env` 文件。务必处理编码错误：
    ```python
    try:
...
- **捕获工具执行周围的宽泛异常**，以免单个故障导致 Agent 循环崩溃

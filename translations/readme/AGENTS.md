...
适用于在 hermes-agent 代码库上工作的 AI 编程助手和开发人员的说明。
...
仪表板嵌入的是真实的 `hermes --tui` — **并非**重写。参见 `hermes_cli/pty_bridge.py` 以及 `hermes_cli/web_server.py` 中的 `@app.websocket("/api/pty")` 端点。
...
- 帧：双向原始 PTY 字节；通过 `\x1b[RESIZE:<cols>;<rows>]` 调整大小，该指令在服务器端被拦截并通过 `TIOCSWINSZ` 应用。

**不要在 React 中重新实现主要的聊天体验。** 主要的对话记录、输入框/输入流（包括斜杠命令行为）以及基于 PTY 的终端都属于嵌入的 `hermes --tui` — 你在 Ink 中添加的任何新内容都会自动显示在仪表板中。如果你发现自己正在为仪表板重建对话记录或输入框，请停止并改为扩展 Ink。
...
仅当用户明确贡献一个应包含在基础系统中的新核心 Hermes 工具时，才使用下面的内置路径。
...
**Agent 级工具**（todo, memory）：在 `handle_function_call()` 之前被 `run_agent.py` 拦截。参见 `tools/todo_tool.py` 中的模式。
...
2. 增加 `_config_version`（在 `DEFAULT_CONFIG` 顶部查看当前值）
   **仅当**你需要主动迁移/转换现有用户配置时（例如重命名键、更改结构）。向现有部分添加新键由深度合并自动处理，**不需要**增加版本号。
...
provider/model/base_url/max_tokens/reasoning_effort。参见
...
非机密设置（超时、阈值、功能标志、路径、显示偏好）应放在 `config.yaml` 中，而不是 `.env` 中。如果内部代码为了向后兼容需要一个环境变量镜像，请在代码中将 `config.yaml` 的值桥接到环境变量（参见 `gateway_timeout`, `terminal.cwd` → `TERMINAL_CWD`）。
...
Hermes 有两个插件接口。它们都位于仓库的 `plugins/` 目录下，这样仓库自带的插件可以与用户安装在 `~/.hermes/plugins/` 的插件以及通过 pip 安装的入口点插件一起被发现。
...
**2026年5月起，不再新增内置记忆提供商（政策）：** `plugins/memory/` 下的内置记忆提供商集合已关闭。新的记忆后端必须作为**独立的插件仓库**发布，由用户安装到 `~/.hermes/plugins/`（或通过 pip entry points）——它们实现相同的 `MemoryProvider` ABC，通过相同的发现路径注册，并通过 `hermes memory setup` / `post_setup()` 集成，而无需进入此代码树。在 `plugins/memory/` 下添加新目录的 PR 将被关闭，并会指引将提供商作为自己的仓库发布。现有的内置提供商保留；欢迎为其提供错误修复。
...
每个新的或现代化的技能——无论是捆绑的、可选的还是贡献的——在合并前都必须满足这些标准。审查者将拒绝违反这些标准的 PR。

1.  **`description` ≤ 60 个字符，一句话，以句号结尾。**
    过长的描述会使技能列表臃肿，并且在加载许多技能时分散模型的注意力。说明能力，而非实现。不要使用营销词汇（"强大"、"全面"、"无缝"、"先进"）。不要重复技能名称。使用以下代码验证：
    ```python
    import re, pathlib
...
2.  **SKILL.md 正文中引用的工具必须是原生的 Hermes 工具或技能明确期望的 MCP 服务器。** 当技能需要某项能力时，用反引号指出正确的工具名称（`` `terminal` ``、`` `web_extract` ``、`` `read_file` ``、`` `patch` ``、`` `search_files` ``、`` `vision_analyze` ``、`` `browser_navigate` ``、`` `delegate_task` `` 等）。**不要**命名 Agent 已经封装好的 shell 工具——`grep` → `search_files`、`cat`/`head`/`tail` → `read_file`、`sed`/`awk` → `patch`、`find`/`ls` → `search_files target='files'`。如果技能依赖于 MCP 服务器，请命名该 MCP 服务器并在 `## 先决条件` 中记录预期的设置。其他任何内容（第三方 CLI、shell 管道等）在脚本文件中是允许的，但不应成为正文中的主要交互界面。

3.  **`platforms:` 限制需根据实际脚本导入进行审核。**
    使用仅限 POSIX 的原语（`fcntl`、`termios`、`os.setsid`、用于存活性检查的 `os.kill(pid, 0)`、`/proc`、硬编码的 `/tmp`、`signal.SIGKILL`、bash heredocs、`osascript`、`apt`、`systemctl`）的技能必须声明其支持的平台。默认立场：首先尝试跨平台修复——使用 `tempfile.gettempdir`、`pathlib.Path`、`psutil.pid_exists`、Python 级别的过滤而非 `grep`。仅当依赖项确实受平台限制时，才将其限制在更窄的集合中。
...
- `role="orchestrator"` —— 保留 `delegate_task` 以便它可以生成自己的工作者。
  受 `delegation.orchestrator_enabled`（默认 true）控制，
...
同步性规则：delegate_task **不是** 持久性的。对于必须比当前轮次存活更长时间的长时间运行工作，
请改用 `cronjob` 或 `terminal(background=True, notify_on_complete=True)`。
...
- **核心：** `agent/curator.py`（审查循环、自动转换、LLM 审查提示词）
  + `agent/curator_backup.py`（运行前 tar.gz 快照）。
...
- **遥测：** `tools/skill_usage.py` 管理侧车文件
  `~/.hermes/skills/.usage.json` —— 记录每个技能的 `use_count`、`view_count`、
...
- `skill_manage(action="delete")` 拒绝已固定的技能；补丁/编辑/
  写文件/删除文件操作允许通过，以便 Agent 可以继续改进已固定的技能。
...
`provider` 覆盖、`script`（运行前数据收集脚本，其标准输出被注入到提示词中；
...
- 定时任务会话默认传递 `skip_memory=True`；记忆提供商
  在定时任务期间故意不运行。
定时任务的交付**不会**镜像到目标消息网关会话中——它们会落在自己的定时任务会话中，并带有页眉/页脚帧，这样主对话的消息角色交替就能保持完整。

---

## 看板（多 Agent 工作队列）

这是一个持久化的、基于 SQLite 的看板，允许多个配置文件/工作者在共享任务上协作。用户通过 `hermes kanban <动词>` 来驱动它；由调度器生成的工作者通过专用的 `kanban_*` 工具集来驱动它，这样当它们不在看板任务中时，其模式占用为零。
...
- **工作者工具集：** `tools/kanban_tools.py` 暴露 `kanban_show`、`kanban_complete`、`kanban_block`、`kanban_heartbeat`、`kanban_comment`、`kanban_create`、`kanban_link`——由 `HERMES_KANBAN_TASK` 控制，因此该模式仅出现在实际作为工作者运行的进程中。
- **调度器：** 一个长期运行的循环（默认每 60 秒），它会回收过期的认领、提升就绪任务、原子性地认领并生成分配好的配置文件。默认情况下，通过 `kanban.dispatch_in_gateway: true` **在消息网关内部**运行。
...
- **租户**是看板*内部*的一个软命名空间——一个专家团队可以通过工作空间路径 + 记忆键隔离来服务多个业务。
- 在同一任务上连续约 5 次生成失败后，调度器会自动阻塞该任务，以防止循环旋转。
...
Hermes-Agent 确保缓存在整个对话期间保持有效。**请勿实施会破坏缓存的更改：**
- 在对话中途更改过去的上下文
- 在对话中途更改工具集
- 在对话中途重新加载记忆或重建系统提示词
...
5.  **消息网关平台适配器应使用令牌锁**——如果适配器使用唯一凭据（机器人令牌、API 密钥）连接，请在 `connect()`/`start()` 方法中调用 `gateway.status` 中的 `acquire_scoped_lock()`，并在 `disconnect()`/`stop()` 中调用 `release_scoped_lock()`。这可以防止两个配置文件使用相同的凭据。请参阅 `gateway/platforms/telegram.py` 作为规范模式。

6.  **配置文件操作是基于 HOME 锚定的，而不是基于 HERMES_HOME 锚定的**——`_get_profiles_root()` 返回 `Path.home() / ".hermes" / "profiles"`，**而不是** `get_hermes_home() / "profiles"`。这是有意为之——它允许 `hermes -p coder profile list` 查看所有配置文件，无论哪个配置文件是活动的。
...
首选的 UI 是 curses（标准库），因为 `simple_term_menu` 在 tmux/iTerm2 中使用方向键时存在幽灵重复渲染的 bug。新的交互式菜单必须使用 `hermes_cli/curses_ui.py` — 请参考 `hermes_cli/tools_config.py` 中的规范模式。
...
### 禁止在模式描述中硬编码跨工具引用
工具模式描述中不得按名称提及来自其他工具集的工具（例如，`browser_navigate` 说“优先使用 web_search”）。这些工具可能不可用（缺少 API 密钥、工具集被禁用），导致模型幻觉式地调用不存在的工具。如果需要跨引用，请在 `model_tools.py` 的 `get_tool_definitions()` 中动态添加 — 请参考 `browser_navigate` / `execute_code` 后处理块的模式。
...
在压缩合并一个 PR 之前，请确保该分支与 `main` 保持同步（在工作目录中执行 `git fetch origin main && git reset --hard origin/main`，然后重新应用 PR 的提交）。一个过时分支中不相关文件的版本在压缩时会静默地覆盖 main 分支上的最新修复。合并后使用 `git diff HEAD~1..HEAD` 进行验证 — 意外的删除是危险信号。

### 不要在没有端到端验证的情况下接入死代码
从未发布过的未使用代码之所以是死的，是有原因的。在将未使用的模块接入到活动代码路径之前，请使用实际的导入（而非模拟）针对临时的 `HERMES_HOME` 对真实的解析链进行端到端测试。

### 测试不得写入 `~/.hermes/`
`tests/conftest.py` 中的 `_isolate_hermes_home` 自动使用夹具会将 `HERMES_HOME` 重定向到一个临时目录。切勿在测试中硬编码 `~/.hermes/` 路径。

**配置文件测试**：当测试配置文件功能时，同时模拟 `Path.home()`，以便 `_get_profiles_root()` 和 `_get_default_hermes_home()` 在临时目录内解析。
...
**始终使用 `scripts/run_tests.sh`** — 不要直接调用 `pytest`。该脚本强制执行与 CI 一致的无副作用的隔离环境（取消设置凭证变量、TZ=UTC、LANG=C.UTF-8、4 个 xdist 工作进程以匹配 GHA ubuntu-latest）。在设置了 API 密钥的 16+ 核开发机器上直接运行 `pytest` 会与 CI 环境产生差异，这已导致多起“本地工作正常，CI 失败”的事件（反之亦然）。
...
`tests/conftest.py` 也通过一个自动使用的夹具强制执行第 1-4 点，因此**任何** pytest 调用（包括 IDE 集成）都会获得隔离行为 — 但包装脚本是双重保险。

### 不使用包装脚本运行（仅在必须时）

如果你无法使用包装脚本（例如在 Windows 上或在直接 shell 执行 pytest 的 IDE 内），至少激活虚拟环境并传递 `-n 4`：
...
如果一个测试在**预期会变化**的数据更新时失败 — 例如模型目录、配置版本号、枚举计数、硬编码的提供商模型列表 — 那么它就是一个**变更检测器**。这些测试不提供任何行为覆盖；它们只是保证常规的源代码更新会破坏 CI，并耗费工程时间来“修复”。
...
规则：如果测试读起来像是当前数据的快照，则删除它。如果它读起来像是关于两段数据必须如何关联的契约，则保留它。当 PR 添加新的提供商/模型并且你需要测试时，让测试断言关系（例如“所有目录条目都有上下文长度”），而不是具体的名称。

...
├── hermes_logging.py     # setup_logging() — agent.log（INFO+），errors.log（WARNING+），gateway.log（当运行消息网关时）
├── batch_runner.py       # 并行批处理
├── agent/                # Agent 内部（提供商适配器、缓存、压缩等）
├── hermes_cli/           # Python JSON-RPC 后端用于 TUI
├── acp_adapter/          # ACP 服务器（VS Code / Zed / JetBrains 集成）
├── cron/                 # 调度器 — jobs.py，scheduler.py
├── scripts/              # run_tests.sh，release.py，辅助脚本
├── website/              # Docusaurus 文档站点
└── tests/                # Pytest 测试套件（截至 2026 年 5 月，约 900 个文件中的约 17k 测试）
```

**用户配置：** `~/.hermes/config.yaml`（设置），`~/.hermes/.env`（仅 API 密钥）。
**日志：** `~/.hermes/logs/` — `agent.log`（INFO+），`errors.log`（WARNING+），
运行消息网关时的 `gateway.log`。通过 `get_hermes_home()` 进行配置文件感知。
...
会话上下文、预算、凭据池等）。下面的签名是
您通常需要处理的最小子集 — 完整列表请阅读 `run_agent.py`。
...
class AIAgent:
    def __init__(self,
        ...
        max_iterations: int = 90,          # 工具调用迭代（与子代理共享）
        ...
        # ... 加上回调、线程/用户/聊天 ID、iteration_budget、prefill_messages、
        # service_tier、reasoning_config 等。
    ): ...
...
核心循环在 `run_conversation()` 内部，完全同步，包含中断检查、预算跟踪和一次宽限调用：
...
**围绕 TUI 构建结构化的 React UI 是允许的，只要它不是第二个聊天界面。** 侧边栏小部件、检查器、摘要、状态面板以及类似的辅助视图（例如 `ChatSidebar`、`ModelPickerDialog`、`ToolCall`）是可以的，只要它们是对嵌入式 TUI 的补充，而不是替代对话记录/输入框/终端。保持它们的状态独立于 PTY 子进程的会话，并以非破坏性的方式呈现它们的故障，以便终端窗格能继续正常工作。

---

## 添加新工具

对于大多数自定义或仅限本地的工具，**不要**编辑 Hermes 核心。请使用插件方式：创建 `~/.hermes/plugins/<name>/plugin.yaml` 和 `~/.hermes/plugins/<name>/__init__.py`，然后使用 `ctx.register_tool(...)` 注册工具。插件工具集会被自动发现，并且可以在不接触 `tools/` 或 `toolsets.py` 的情况下启用或禁用。

仅当用户明确贡献一个应该包含在基础系统中的新核心 Hermes 工具时，才使用下面描述的内置路径。
...
所有依赖项都必须有上限，以限制供应链攻击面。该策略在 litellm 安全事件（PR #2796, #2810）后建立，并在 Mini Shai-Hulud 蠕虫攻击活动（2026年5月）后得到加强。
...
2. 对于 1.0 之前的包，使用 `<0.(当前次版本号 + 2)`（例如 `>=0.29,<0.32`）。
3. 切勿提交没有上限的裸 `>=X.Y.Z` —— CI 和审阅者会拒绝它。
...
2. **仅当**你需要主动迁移/转换现有用户配置（重命名键、更改结构）时，才增加 `_config_version`（在 `DEFAULT_CONFIG` 顶部查看当前值）。向现有部分添加新键由深度合并自动处理，**不需要**增加版本号。
...
非机密设置（超时、阈值、功能标志、路径、显示偏好）属于 `config.yaml`，而不是 `.env`。如果内部代码为了向后兼容需要一个环境变量镜像，请在代码中将 `config.yaml` 的值桥接到环境变量（参见 `gateway_timeout`, `terminal.cwd` → `TERMINAL_CWD`）。
### 配置加载器（三条路径——请清楚你处于哪条路径）：
...
如果你添加了一个新键，CLI 能看到但网关看不到（或反之），说明你使用了错误的加载器。请检查 `DEFAULT_CONFIG` 的覆盖范围。

### 工作目录：
- **CLI** — 使用进程的当前目录 (`os.getcwd()`)。
- **消息传递** — 使用 `config.yaml` 中的 `terminal.cwd`。消息网关会将其桥接为 `TERMINAL_CWD` 环境变量供子工具使用。**`MESSAGING_CWD` 已被移除** — 如果它在 `.env` 中设置，配置加载器会打印弃用警告。`.env` 中的 `TERMINAL_CWD` 同理；规范设置是 `config.yaml` 中的 `terminal.cwd`。
...
钩子从 `model_tools.py`（工具调用前/后）和 `run_agent.py`（生命周期）调用。**发现时机陷阱：** `discover_plugins()` 仅在导入 `model_tools.py` 的副作用中运行。在导入 `model_tools.py` 之前读取插件状态的代码路径必须显式调用 `discover_plugins()`（它是幂等的）。
...
每个提供者都实现了 `MemoryProvider` 抽象基类（参见 `agent/memory_provider.py`），并由 `agent/memory_manager.py` 编排。生命周期钩子包括 `sync_turn(turn_messages)`、`prefetch(query)`、`shutdown()`，以及可选的 `post_setup(hermes_home, config)` 用于设置向导集成。

**通过 `plugins/memory/<name>/cli.py` 的 CLI 命令：** 如果一个记忆插件定义了 `register_cli(subparser)`，`discover_plugin_cli_commands()` 会在 argparse 设置时找到它，并将其连接到 `hermes <plugin>`。该框架仅暴露**当前激活的**记忆提供者的 CLI 命令（从 config.yaml 中的 `memory.provider` 读取），因此禁用的提供者不会使 `hermes --help` 变得杂乱。
...
**不再接受树内新增记忆提供商（政策，2026年5月起）：** `plugins/memory/` 下的内置记忆提供商集合已关闭。新的记忆后端必须作为**独立的插件仓库**发布，由用户安装到 `~/.hermes/plugins/`（或通过 pip entry points）——它们实现相同的 `MemoryProvider` ABC，通过相同的发现路径注册，并通过 `hermes memory setup` / `post_setup()` 集成，而无需进入本代码树。在 `plugins/memory/` 下添加新目录的 PR 将被关闭，并会指引将提供商作为自己的仓库发布。现有的树内提供商保留；欢迎为其提交错误修复。

### 模型提供商插件 (`plugins/model-providers/<name>/`)

每个推理后端（openrouter、anthropic、gmi、deepseek、nvidia 等）都作为一个插件放在这里。每个插件的 `__init__.py` 在模块加载时调用 `providers.register_provider(ProviderProfile(...))`。`providers/__init__.py._discover_providers()` 是一个**惰性的、独立的发现系统**——在首次调用 `get_provider_profile()` 或 `list_providers()` 时扫描，而不是由通用的 PluginManager 扫描。
...
审查技能 PR 时，请检查它们的目标目录——依赖较重或小众的技能应属于 `optional-skills/`。

### SKILL.md frontmatter

标准字段：`name`、`description`、`version`、`author`、`license`、`platforms`（操作系统限制列表：`[macos]`、`[linux, macos]`、...）、`metadata.hermes.tags`、`metadata.hermes.category`、`metadata.hermes.related_skills`、`metadata.hermes.config`（技能需要的 config.yaml 设置——存储在 `skills.config.<key>` 下，在设置期间提示，在加载时注入）。
...
1.  **`description` 不超过 60 个字符，一句话，以句号结尾。**
    过长的描述会使技能列表臃肿，并在加载许多技能时分散模型的注意力。说明能力，而非实现。不要使用营销词汇（"强大"、"全面"、"无缝"、"先进"）。不要重复技能名称。使用以下代码验证：
    ```python
    import re, pathlib
...
2.  **SKILL.md 正文中引用的工具必须是原生的 Hermes 工具或技能明确期望的 MCP 服务器。** 当技能需要某个能力时，用反引号指向正确的工具名称（`` `terminal` ``、`` `web_extract` ``、`` `read_file` ``、`` `patch` ``、`` `search_files` ``、`` `vision_analyze` ``、`` `browser_navigate` ``、`` `delegate_task` `` 等）。**不要**命名 Agent 已经封装了的 shell 工具——`grep` → `search_files`、`cat`/`head`/`tail` → `read_file`、`sed`/`awk` → `patch`、`find`/`ls` → `search_files target='files'`。如果技能依赖于 MCP 服务器，请命名该 MCP 服务器，并在 `## Prerequisites` 中记录预期的设置。其他任何内容（第三方 CLI、shell 管道等）在脚本文件中是允许的，但不应成为正文中的主要交互界面。

3.  **`platforms:` 限制需根据实际脚本导入进行审核。** 使用仅限 POSIX 的原语（`fcntl`、`termios`、`os.setsid`、用于存活性检查的 `os.kill(pid, 0)`、`/proc`、硬编码的 `/tmp`、`signal.SIGKILL`、bash heredocs、`osascript`、`apt`、`systemctl`）的技能必须声明其支持的平台。默认立场：首先尝试跨平台修复——使用 `tempfile.gettempdir`、`pathlib.Path`、`psutil.pid_exists`、Python 级别的过滤而非 `grep`。仅当依赖项确实受平台限制时，才将其限制在更窄的集合中。
...
6. **脚本放在 `scripts/` 目录，参考资料放在 `references/` 目录，
   模板放在 `templates/` 目录。** 不要期望模型每次调用都内联编写
   解析器、XML 遍历器或非平凡逻辑——提供一个辅助脚本。
   在 SKILL.md 中通过相对于技能目录的路径引用它。

7. **测试位于 `tests/skills/test_<skill>_skill.py`** 并且仅使用
   标准库 + pytest + `unittest.mock`。不进行实时网络调用。通过
   `scripts/run_tests.sh tests/skills/test_<skill>_skill.py -q` 运行。

8. **`.env.example` 的添加内容被隔离在一个清晰分隔的块中。**
   不要改动周围文件——贡献者提供的 `.env.example` 版本通常已过时，
   在抢救过程中必须丢弃对技能自身块之外的编辑。
...
每个平台的适配器选择一个基础工具集（例如 Telegram 使用 `"messaging"`）；
`_HERMES_CORE_TOOLS` 是大多数平台继承的默认捆绑包。
...
`tools/delegate_tool.py` 生成一个具有独立上下文 + 终端会话的子 Agent。
同步：父 Agent 在继续其自身循环之前等待子 Agent 的摘要——如果父 Agent 被中断，子 Agent 将被取消。
...
- `role="orchestrator"` —— 保留 `delegate_task` 以便它可以生成自己的工作者。受 `delegation.orchestrator_enabled`（默认 true）控制，
  并受 `delegation.max_spawn_depth`（默认 2）限制。

关键配置项（位于 `config.yaml` 的 `delegation:` 下）：
`max_concurrent_children`、`max_spawn_depth`、`child_timeout_seconds`、
`orchestrator_enabled`、`subagent_auto_approve`、`inherit_mcp_toolsets`、
`max_iterations`。

同步性规则：delegate_task **不是** 持久性的。对于必须比当前轮次存活更长时间的长时间运行工作，请改用 `cronjob` 或
...
后台技能维护系统，跟踪 Agent 创建技能的用法并自动归档过时的技能。
用户永远不会丢失技能；归档文件存放在 `~/.hermes/skills/.archive/` 并可恢复。

- **核心：** `agent/curator.py`（审查循环、自动状态转换、LLM 审查提示词）
  + `agent/curator_backup.py`（运行前的 tar.gz 快照）。
- **CLI：** `hermes_cli/curator.py` 连接 `hermes curator <动词>`，其中
  动词有：`status`、`run`、`pause`、`resume`、`pin`、`unpin`、
  `archive`、`restore`、`prune`、`backup`、`rollback`。
- **遥测：** `tools/skill_usage.py` 管理侧车文件
...
- 策展器只处理具有 `created_by: "agent"` 来源的技能 ——
  捆绑的 + 从 Hub 安装的技能是禁区。
- 永不删除；最具破坏性的操作是归档。
- 已固定的技能免于所有自动状态转换和 LLM 审查过程。
- `skill_manage(action="delete")` 拒绝已固定的技能；补丁/编辑/
  write_file/remove_file 可以通过，以便 Agent 可以继续改进已固定的技能。
...
`cron/jobs.py`（作业存储） + `cron/scheduler.py`（滴答循环）。Agent
通过 `cronjob` 工具调度作业；用户通过 `hermes cron <动词>`
...
每个作业的字段包括 `skills`（加载特定技能）、`model` /
`provider` 覆盖、`script`（运行前的数据收集脚本，其标准输出被注入到提示词中；
`no_agent=True` 将脚本变成整个作业）、`context_from`（将作业 A 的最后输出链接到
作业 B 的提示词中）、`workdir`（在特定目录中运行，并加载其 `AGENTS.md`/`CLAUDE.md`），
以及多平台交付。

强化不变性：
- 定时任务会话有 **3 分钟硬中断** —— 失控的 Agent 循环
  不能独占调度器。
- 追赶窗口：作业周期的一半，限制在 120 秒到 2 小时之间。
- 宽限窗口：对于错过触发时间的一次性作业，有 120 秒。
- `~/.hermes/cron/.tick.lock` 处的文件锁防止跨进程的重复滴答。
- 定时任务会话默认传递 `skip_memory=True`；记忆提供商
  在定时任务期间故意不运行。
定时任务的交付内容**不会**镜像到目标消息网关会话中——它们会进入自己的定时任务会话，并带有页眉/页脚帧，以保持主对话中消息角色的交替完整性。

---

## 看板（多 Agent 工作队列）

这是一个持久化的、基于 SQLite 的看板，允许多个配置/工作者在共享任务上进行协作。用户通过 `hermes kanban <动词>` 来驱动它；由调度器生成的工作者通过专用的 `kanban_*` 工具集来驱动它，这样当它们不在看板任务中时，其架构占用为零。
...
- **工作者工具集：** `tools/kanban_tools.py` 暴露 `kanban_show`、`kanban_complete`、`kanban_block`、`kanban_heartbeat`、`kanban_comment`、`kanban_create`、`kanban_link` —— 由 `HERMES_KANBAN_TASK` 控制，因此该架构仅对实际作为工作者运行的进程出现。
- **调度器：** 一个长期运行的循环（默认每 60 秒），负责回收过期的认领、提升就绪任务、原子化认领并生成分配好的配置。默认通过 `kanban.dispatch_in_gateway: true` **在消息网关内部**运行。
- **插件资源：** `plugins/kanban/dashboard/`（Web UI）+ `plugins/kanban/systemd/`（用于独立调度器部署的 `hermes-kanban-dispatcher.service`）。

隔离模型：
- **看板**是硬边界 —— 工作者在生成时，其环境变量中固定设置了 `HERMES_KANBAN_BOARD`，因此它们无法看到其他看板。
- **租户**是看板*内部*的软命名空间 —— 一个专家团队可以通过工作空间路径 + 记忆键隔离来服务多个业务。
- 在同一任务上连续约 5 次生成失败后，调度器会自动阻塞该任务以防止循环自旋。
...
会改变系统提示词状态（技能、工具、记忆等）的斜杠命令必须是**缓存感知的**：默认采用延迟失效（更改在下一次会话生效），并提供一个可选的 `--now` 标志用于立即失效。请参考 `/skills install --now` 作为规范模式。
...
Hermes 支持**配置** —— 多个完全隔离的实例，每个实例都有自己的 `HERMES_HOME` 目录（配置、API 密钥、记忆、会话、技能、消息网关等）。

核心机制：`hermes_cli/main.py` 中的 `_apply_profile_override()` 在任何模块导入之前设置 `HERMES_HOME`。所有 `get_hermes_home()` 引用都会自动限定到当前活动的配置。
...
2.  **对面向用户的消息使用 `display_hermes_home()`。** 从 `hermes_constants` 导入。对于默认配置，它返回 `~/.hermes`；对于其他配置，它返回 `~/.hermes/profiles/<名称>`。
    ```python
    # 正确
...
3.  **模块级常量是可以的** —— 它们在导入时缓存 `get_hermes_home()`，这是在 `_apply_profile_override()` 设置环境变量**之后**。只需使用 `get_hermes_home()`，而不是 `Path.home() / ".hermes"`。
...
5.  **消息网关平台适配器应使用令牌锁** —— 如果适配器使用唯一凭据（机器人令牌、API 密钥）连接，请在 `connect()`/`start()` 方法中调用 `gateway.status` 中的 `acquire_scoped_lock()`，并在 `disconnect()`/`stop()` 中调用 `release_scoped_lock()`。这可以防止两个配置使用相同的凭据。请参考 `gateway/platforms/telegram.py` 作为规范模式。

6.  **配置操作是基于 HOME 锚定的，而不是基于 HERMES_HOME 锚定的** —— `_get_profiles_root()` 返回 `Path.home() / ".hermes" / "profiles"`，**而不是** `get_hermes_home() / "profiles"`。这是有意为之 —— 它允许 `hermes -p coder profile list` 查看所有配置，无论哪个配置是活动的。

## 已知陷阱

### 请勿硬编码 `~/.hermes` 路径
对于代码路径，请使用 `hermes_constants` 中的 `get_hermes_home()`。对于面向用户的打印/日志消息，请使用 `display_hermes_home()`。硬编码 `~/.hermes` 会破坏配置 —— 每个配置都有自己的 `HERMES_HOME` 目录。这是在 PR #3575 中修复的 5 个错误的根源。
### 禁止引入新的 `simple_term_menu` 使用方式
`hermes_cli/main.py` 中现有的调用点仅作为遗留回退方案保留；
首选 UI 是 curses（标准库），因为 `simple_term_menu` 在 tmux/iTerm2 中使用方向键时存在幽灵重复渲染的 bug。新的交互式菜单必须使用 `hermes_cli/curses_ui.py` — 请参考 `hermes_cli/tools_config.py` 中的规范模式。

### 禁止在 spinner/display 代码中使用 `\033[K`（ANSI 清除至行尾）
...
### 从陈旧分支进行的压缩合并会静默地回滚最近的修复
在压缩合并 PR 之前，请确保分支与 `main` 保持同步（在工作树中执行 `git fetch origin main && git reset --hard origin/main`，然后重新应用 PR 的提交）。一个陈旧分支中不相关文件的版本在压缩时会静默地覆盖 `main` 上的最新修复。合并后使用 `git diff HEAD~1..HEAD` 验证 — 意外的删除是危险信号。
...
**始终使用 `scripts/run_tests.sh`** — 不要直接调用 `pytest`。该脚本强制执行与 CI 一致的无干扰环境（取消设置凭据变量、TZ=UTC、LANG=C.UTF-8、4 个 xdist 工作进程以匹配 GHA ubuntu-latest）。在设置了 API 密钥的 16+ 核开发机器上直接运行 `pytest` 会与 CI 产生差异，这已导致多起“本地工作正常，CI 失败”的事件（反之亦然）。
...
`tests/conftest.py` 还通过一个自动使用的夹具强制执行第 1-4 点，因此**任何** pytest 调用（包括 IDE 集成）都会获得无干扰行为 — 但包装脚本是双保险。

### 不使用包装脚本运行（仅在必须时）

如果无法使用包装脚本（例如在 Windows 上或在直接 shell 执行 pytest 的 IDE 内），至少激活虚拟环境并传递 `-n 4`：
...
工作进程数超过 4 会暴露 CI 从未遇到的测试顺序不稳定性问题。

推送更改前务必运行完整测试套件。

### 不要编写变更检测器测试

如果一个测试在**预期会变化**的数据更新时失败 — 例如模型目录、配置版本号、枚举计数、硬编码的提供商模型列表 — 那么它就是一个**变更检测器**。这些测试不提供行为覆盖；它们只是保证常规的源代码更新会破坏 CI，并耗费工程时间来“修复”。
...
# 不变性：目录中的每个模型都有一个上下文长度条目
for m in _PROVIDER_MODELS["huggingface"]:
    assert m.lower() in DEFAULT_CONTEXT_LENGTHS_LOWER
```

规则：如果测试读起来像是当前数据的快照，则删除它。如果读起来像是关于两段数据必须如何关联的契约，则保留它。当 PR 添加新的提供商/模型并且你想要一个测试时，让测试断言关系（例如“所有目录条目都有上下文长度”），而不是具体的名称。

审阅者应拒绝新的变更检测器测试；作者应在重新请求审阅前将其转换为不变性测试。
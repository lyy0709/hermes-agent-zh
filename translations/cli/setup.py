...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if disabled_tools:
        ...
        print_warning(f"或直接编辑 {_dhh()}/.env 文件以添加缺失的API密钥。")
        print()
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "这可以保留您的主提供商，同时减少因配额问题导致的中断。"
            )
            print()
            if auto_count > 0:
                print_info(
                    f"当前 {selected_provider} 的池化凭据：{entry_count} 个"
                    f"（{manual_count} 个手动添加，{auto_count} 个从环境/共享认证自动检测）"
                )
            else:
                print_info(f"当前 {selected_provider} 的池化凭据：{entry_count} 个")
    ...
    if selected_provider == "nous" and nous_subscription_selected:
        ...
        if "tts" in changed_defaults:
            print_success("TTS 提供商已设置为：通过您的 Nous 订阅使用 OpenAI TTS")
        else:
            print_info(f"保留您现有的 TTS 提供商：{current_tts}")
...
def setup_terminal_backend(config: dict):
    ...
    next_idx = 5
    if is_linux:
        terminal_choices.append("Singularity/Apptainer - HPC 友好型容器")
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  会话重置: 空闲 (1440 分钟) + 每日 (4:00)")
...
def setup_agent_settings(config: dict):
    ...
    print_info("默认值为 90，适用于大多数任务。进行开放式探索时使用 150+。")
    ...
    if reset_idx == 0:  # Both
        ...
        idle_str = prompt("  空闲超时（分钟）", str(current_idle))
        try:
        ...
        try:
            ...
            pass
        print_success(
            f"会话将在空闲 {config['session_reset'].get('idle_minutes', 1440)} 分钟后或每日 {config['session_reset'].get('at_hour', 4)}:00 重置"
        )
        ...
        try:
            ...
            pass
        print_success(
            f"会话将在空闲 {config['session_reset'].get('idle_minutes', 1440)} 分钟后重置"
        )
...
def _setup_telegram():
    ...
    if existing:
        ...
        if not prompt_yes_no("重新配置 Telegram?", False):
            ...
            if not get_env_value("TELEGRAM_ALLOWED_USERS"):
                print_info("⚠️  Telegram 未设置用户允许列表 - 任何人都可以使用你的机器人!")
    ...
    if allowed_users:
        save_env_value("TELEGRAM_ALLOWED_USERS", allowed_users.replace(" ", ""))
        print_success("Telegram 允许列表已配置 - 只有列出的用户可以使用该机器人")
    else:
        print_info("⚠️  未设置允许列表 - 任何发现你机器的人都可以使用它!")
...
def _setup_discord():
    ...
    if existing:
        ...
        if not prompt_yes_no("重新配置 Discord?", False):
            if not get_env_value("DISCORD_ALLOWED_USERS"):
                ...
                if prompt_yes_no("现在添加允许的用户?", True):
                    print_info("   要查找 Discord ID: 启用开发者模式, 右键点击用户名 → 复制 ID")
                    allowed_users = prompt("允许的用户 ID (逗号分隔)")
                    if allowed_users:
    ...
    print()
    print_info("🔒 安全: 限制谁可以使用你的机器人")
    print_info("   要查找你的 Discord 用户 ID:")
    ...
    if allowed_users:
        ...
        print_info("⚠️  未设置允许列表 - 任何在你机器人所在服务器中的人都可以使用它!")

    print()
    print_info("📬 主频道: Hermes 在此发送定时任务结果,")
    print_info("   跨平台消息和通知。")
    print_info("   要获取频道 ID: 右键点击一个频道 → 复制频道 ID")
...
def _setup_slack():
    ...
    if allowed_users:
        ...
        print_warning("⚠️  未设置 Slack 允许名单 - 默认将拒绝所有未配对的用户。")
...
def _setup_bluebubbles():
    ...
    print()
    print_info("📬 主频道: 用于定时任务交付和通知的电话或邮箱。")
...
_HIGH_IMPACT_KIND_KEYWORDS = {
    ...
    "config": "⚠ 配置值 — OpenClaw 设置可能与 Hermes 的对应项不是 1:1 映射",
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    if conflicts:
        print_info(f"跳过了 {conflicts} 个已存在于 Hermes 的项目（使用 hermes claw migrate --overwrite 强制覆盖）。")
    if skipped:

...
# 每个提供商的默认模型列表 — 当无法访问实时 /models 端点时用作回退。
...
def prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    print_info(f"  按 Enter 键使用默认值 ({default + 1})  按 Ctrl+C 退出")
...
def _prompt_vercel_sandbox_settings(config: dict):
    ...
    print_info("  快照仅恢复文件；沙盒重建后，活动进程不会继续。")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Hermes 可以为同一提供商保存多个凭证，并在凭证耗尽或受到速率限制时在它们之间轮换。"
            )
            print_info(
                "这可以保留您的主提供商，同时减少因配额问题造成的中断。"
            )
            print()
            if auto_count > 0:
                print_info(
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            if _oai_key:
                ...
                if _is_native_openai:
                    ...
                    _selected_vision_model = prompt("  视觉模型 (留空 = 使用主/自定义默认)").strip()
    ...
    if selected == "neutts":
        ...
        if oauth_logged_in:
            ...
            choice_idx = prompt_choice(
                "您希望 xAI TTS 如何认证?",
                choices=[
                    "使用 xAI Grok OAuth (SuperGrok 订阅) 登录 — 浏览器登录",
...
def setup_agent_settings(config: dict):
    ...
    print_info("  new     — 仅当工具变更时显示其名称（减少噪音）")
    ...
    if reset_idx == 0:  # Both
        ...
        try:
            ...
            pass
        print_success(
            f"会话将在闲置 {config['session_reset'].get('idle_minutes', 1440)} 分钟后或每日 {config['session_reset'].get('at_hour', 4)}:00 重置"
        )
        ...
        try:
            ...
            pass
        print_success(
            f"会话将在闲置 {config['session_reset'].get('idle_minutes', 1440)} 分钟后重置"
        )
        ...
        try:
            ...
            pass
        print_success(
            f"会话将每日在 {config['session_reset'].get('at_hour', 4)}:00 重置"
        )
...
def _setup_bluebubbles():
    ...
    if allowed_users:
        ...
        print_info("⚠️  未设置允许列表 — 任何可以给你发 iMessage 的人都可以使用这个 bot!")

...
TIPS = [
    ...
    "/queue <提示词> 为下一轮排队一条消息，而不中断当前轮次。",
    "/undo 从对话中移除最后一条用户/助手交换记录。",
    ...
    "/personality pirate 设置一个有趣的个性 — 14 个内置选项，从可爱到莎士比亚风格。",
    ...
    "/paste 检查您的剪贴板中是否有图像并将其附加到您的下一条消息。",
    ...
    "hermes profile create coder 创建一个隔离的配置文件，该文件成为其自己的命令。",
    ...
    "设置 agent.max_turns: 200 以允许 Agent 每轮采取更多工具调用步骤。",
    ...
    "在消息网关会话重置之前，Hermes 会自动将重要事实刷新到后台的记忆中。",

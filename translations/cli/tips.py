...
TIPS = [
    ...
    "/queue <prompt> 为下一轮排队消息，而不中断当前轮次。",
    ...
    "hermes -p work chat 在特定配置文件下运行，而不更改您的默认配置。",
    ...
    "hermes sessions stats 按平台显示会话计数和数据库大小。",
    ...
    "hermes auth add 允许您添加多个 API 密钥以进行凭证池轮换。",
    ...
    "设置 display.busy_input_mode: queue 以排队消息而不是中断 Agent，或设置为 steer 以通过 /steer 在运行中注入消息。",
    ...
    "API 服务器支持 Chat Completions 和 Responses API 与服务器端状态。",
    ...
    "config.yaml 中的 prefill_messages_file 将少样本示例注入每个 API 调用，永远不会保存到历史。",
    ...
    "在第二次及以后的压缩中，压缩器更新先前的摘要而不是从头开始。",
    "在消息网关会话重置之前，Hermes 在后台自动将重要事实刷新到记忆。",
    "config.yaml 中的 network.force_ipv4: true 修复具有损坏 IPv6 的服务器上的挂起 — 猴子补丁 socket。",
    "终端工具注释常见退出代码：grep 返回 1 = '未找到匹配项（非错误）'。",
    "失败的前台终端命令自动重试最多 3 次，使用指数退避（2s、4s、8s）。",
    "裸 sudo 命令自动重写以从 .env 管道 SUDO_PASSWORD — 无需交互式提示。",
    ...
    "读取同一文件区域 3 次以上会触发警告。4 次以上，硬阻止以防止循环。",
    ...
    "agent.api_max_retries（默认 3）控制 Agent 在抛出错误之前重试失败 API 调用的次数 — 降低它以快速故障转移。",
    ...
    "长危险命令（>70 字符）在批准提示中获得 'view' 选项以首先查看完整文本。",
    ...
    "上下文文件支持 10 种以上提示词注入模式 — 不可见 Unicode、'忽略指令'、泄露尝试。",

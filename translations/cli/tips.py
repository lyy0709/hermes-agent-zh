...
TIPS = [
    ...
    "@staged 将您已暂存的 git 更改注入到消息中 (git diff --staged)。",
    ...
    "hermes memory setup 允许您配置外部记忆提供程序 (Honcho、Mem0 等)。",
    ...
    "delegate_task 生成最多 3 个具有隔离上下文的并发子 Agent 以进行并行工作。",
    ...
    "send_message 可以从会话中到达任何已连接的消息传递平台。",
    ...
    "记忆条目会自动扫描提示词注入和泄露模式。",
    "Agent 有两个记忆存储：个人笔记 (~2200 字符) 和用户档案 (~1375 字符)。",
    ...
    "Discord 语音频道模式：机器人加入 VC，转录语音，并回复。",
    ...
    # --- 插件 ---
    "三种插件类型：通用 (工具/钩子)、记忆提供程序和上下文引擎。",
    "hermes plugins install owner/repo 直接从 GitHub 安装插件。",
    "8 个外部记忆提供程序可用：Honcho、OpenViking、Mem0、Hindsight 等。",
    "插件钩子包括 pre_tool_call、post_tool_call、pre_llm_call 和 post_llm_call。",
    ...
    "API 服务器支持 Chat Completions 和 Responses API 以及服务器端状态。",
    "config 中的 tool_preview_length: 0 在旋转器的活动源中显示完整文件路径。",
    "hermes status --deep 在所有组件上运行更深入的诊断检查。",

    # --- 隐藏宝石与高级用户技巧 ---
    "~/.hermes/BOOT.md 下的 BOOT.md 在每次消息网关启动时自动运行 — 用于启动检查。",
    ...
    "/compress <聚焦主题> 将 60-70% 的摘要预算分配给您的主题，并积极修剪其余部分。",
    ...
    "压缩器进行廉价预传递：超过 200 个字符的工具输出在 LLM 运行之前被替换为占位符。",
    "当压缩失败时，进一步尝试暂停 10 分钟以避免 API 冲击。",
    ...
    "智能路由可以将简单查询自动路由到更便宜的模型 — 设置 smart_model_routing.enabled: true。",

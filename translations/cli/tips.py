...
TIPS = [
    ...
    "设置 display.compact: true 以减少输出中的空白，获得更密集的信息。",
    ...
    "plan 技能将 markdown 计划保存在活动工作空间的 .hermes/plans/ 下。",
    ...
    "browser_vision 与 annotate=true 在交互元素上叠加带编号的标签。",
    ...
    "config.yaml 中的 tool_preview_length: 0 在旋转器的活动源中显示完整文件路径。",
    "hermes status --deep 在所有组件上运行更深入的诊断检查。",

    # --- 隐藏宝石与高级用户技巧 ---
    "~/.hermes/BOOT.md 中的 BOOT.md 在每次消息网关启动时自动运行 — 用于启动检查。",
    ...
    "agent.api_max_retries（默认 3）控制 Agent 在将错误暴露给用户之前重试失败 API 调用的次数 — 降低它以快速故障转移。",
    "消息网关按会话缓存 AIAgent 实例 — 销毁此缓存会破坏 Anthropic 提示词缓存。",
    "任何网站都可以通过 /.well-known/skills/index.json 暴露技能 — 技能中心会自动发现它们。",
    "~/.hermes/skills/.hub/audit.log 中的技能审计日志跟踪每次安装和移除操作。",
    ...
    "当压缩失败时，进一步尝试会暂停 10 分钟以避免 API 滥用。",
    ...
    "~/.hermes/skills/.hub/quarantine/ 中的技能隔离区保存待安全审查的技能。",
]

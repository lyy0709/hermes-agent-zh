...
TIPS = [
    ...
    "/branch 分支当前会话，以便你可以探索不同方向而不会丢失进度。",
    ...
    "/retry 重新发送你的最后一条消息 — 当 Agent 的响应不太准确时很有用。",
    ...
    "/model 让你在会话中途切换模型 — 试试 /model sonnet 或 /model gpt-5。",
    ...
    "/statusbar 切换一个持久状态栏，显示模型、Token、上下文填充百分比、成本和持续时间。",
    ...
    "hermes -p work chat 在特定配置文件下运行而不更改你的默认配置。",
    ...
    "hermes auth add 让你添加多个 API 密钥以进行凭证池轮换。",
    ...
    "hermes webhook subscribe 创建事件驱动的 Webhook 路由并支持 HMAC 验证。",
    ...
    "设置 agent.max_turns: 200 以让 Agent 每轮采取更多工具调用步骤。",
    ...
    "设置 security.website_blocklist.enabled: true 以阻止特定域名的 Web 工具访问。",
    ...
    "send_message 可以从会话中到达任何已连接的消息平台。",
    ...
    "退出时，Hermes 打印一个恢复命令，包含会话 ID 和统计信息。",
    ...
    "~/.hermes/SOUL.md 是 Agent 的主要身份 — 自定义它以塑造行为。",
    ...
    "Camofox 是一个反检测浏览器 — 具有 C++ 指纹欺骗的 Firefox 分支。",
    ...
    "MCP OAuth 支持：auth: oauth 启用基于浏览器的授权，支持 PKCE。",
    ...
    # --- 隐藏宝石与高级用户技巧 ---
    "~/.hermes/BOOT.md 在每次消息网关启动时自动运行 — 用它进行启动检查。",
    ...
    "/compress <focus topic> 将 60-70% 的总结预算分配给你的主题，并积极修剪其余部分。",
    ...
    "失败的前台终端命令自动重试最多 3 次，并带有指数退避 (2s、4s、8s)。",
    ...
    "读取同一文件区域 3+ 次会触发警告。在 4+ 次时，会被硬阻止以防止循环。",
    ...
    "过时的 git 工作树会自动清理：在启动时清理 24-72 小时旧且没有未推送提交的工作树。",
    ...
    "在中断模式中，在 Agent 执行期间输入的斜杠命令绕过中断逻辑并立即运行。",

...
TIPS = [
    ...
    "/browser connect 通过 CDP 将浏览器工具附加到你正在运行的 Chrome 实例。",
    ...
    "当 Agent 正在工作时输入新消息以中断并重定向它。",
    "Alt+V 从剪贴板粘贴图像到对话中。",
    "粘贴 5 行以上内容会自动保存到文件并插入紧凑的引用。",
    ...
    "hermes sessions browse 打开一个带有搜索功能的交互式会话选择器。",
    ...
    "hermes profile create work --clone 将你当前的配置和密钥复制到新的配置文件。",
    ...
    "config.yaml 中的快速命令可立即运行 shell 命令，零 Token 使用量。",
    ...
    "退出时，Hermes 会打印一个带有会话 ID 和统计信息的恢复命令。",
    ...
    "hermes gateway install 将其设置为在启动时启动的系统服务。",
    ...
    "~/.hermes/SOUL.md 处的 SOUL.md 是 Agent 的主要身份 — 自定义它以塑造行为。",
    ...
    "browser_vision 与 annotate=true 在交互式元素上叠加带编号的标签。",
    ...
    "你的仓库根目录中的 .worktreeinclude 列出了要复制到工作树中的 gitignored 文件。",
    ...
    "credential_pool_strategies 支持 fill_first、round_robin、least_used 和随机轮换。",
    ...
    "SOUL.md 完全取代 Agent 的默认身份 — 重写它以让 Hermes 成为你自己的。",
    "SOUL.md 在首次运行时自动植入默认个性。编辑 ~/.hermes/SOUL.md 以自定义。",
    "/compress <焦点主题> 将 60-70% 的摘要预算分配给你的主题，并积极修剪其余部分。",
    "在第二次及以后的压缩时，压缩器会更新先前的摘要，而不是从头开始。",
    "在消息网关会话重置之前，Hermes 会在后台自动将重要事实刷新到记忆中。",
    "config.yaml 中的 network.force_ipv4: true 修复了在具有损坏 IPv6 的服务器上的挂起 — 猴子补丁 socket。",
    "终端工具注释常见的退出代码：grep 返回 1 = '未找到匹配项（不是错误）'。",
    "失败的前台终端命令会自动重试最多 3 次，并采用指数退避（2s、4s、8s）。",
    "裸 sudo 命令会自动重写以从 .env 管道传输 SUDO_PASSWORD — 无需交互式提示。",
    "execute_code 具有内置助手：用于容错解析的 json_parse()、shell_quote() 和带有退避的 retry()。",
    "execute_code 的 7 个沙盒工具（web_search、terminal、read/write/search/patch）使用 RPC — 永远不会进入上下文。",
    "读取同一文件区域 3 次以上会触发警告。4 次以上时，会被硬阻止以防止循环。",
    ...
    "agent.tool_use_enforcement 引导描述操作而不是调用工具的模型 — 对 GPT/Codex 自动执行。",
    "agent.restart_drain_timeout（默认 60 秒）让正在运行的 Agent 在消息网关重启生效之前完成。",
    "agent.api_max_retries（默认 3 次）控制 Agent 在将错误暴露给用户之前重试失败 API 调用的次数 — 降低它以快速故障转移。",
    ...
    "陈旧的 git 工作树会自动清理：在启动时修剪 24-72 小时旧且没有未推送提交的工作树。",
    "每个配置文件在 HERMES_HOME/home/ 处都有自己的子进程 HOME — 隔离的 git、ssh、npm、gh 配置。",
    ...
    "长危险命令（>70 个字符）在批准提示中获得一个“查看”选项，以便首先查看完整文本。",
    ...
    # --- 高级斜杠命令 ---
    '/steer <prompt> 在下一次工具调用后注入一个注释 — 在不中断的情况下中途调整方向。',
    ...
    '放置一个 ~/.hermes/BOOT.md 清单，gateway:startup 钩子会在每次启动时将其作为一次性 Agent 运行。',

    # --- 策展人 ---
    'hermes curator run --dry-run 预览策展人将归档或合并的内容，而不进行任何更改。',
    ...
    'use_gateway: true 每工具将 Web、图像、TTS 或浏览器路由到你的 Nous 订阅 — 无需额外密钥。',
    ...
    # --- TUI 与仪表板 ---
    'HERMES_TUI_RESUME=1 在启动时自动重新附加到最近的 TUI 会话 — 在 SSH 断开后很方便。',
    ...
    '在 ~/.hermes/dashboard-themes/ 中放置一个包含两个调色板颜色的 YAML 以重新设计整个仪表板的外观。',
    ...
    'HERMES_BACKGROUND_NOTIFICATIONS=result 仅在后台任务完成时 ping（相对于 all/error/off）。',
    ...
    '/image <path> 为你的下一个提示词附加本地图像文件，而无需粘贴或拖放。',
    ...
    '/gquota 在 Google Gemini Code Assist 提供商处于活动状态时，显示其配额使用情况并带有进度条。',
    '/voice tts 切换仅 TTS 模式 — Agent 回复大声说出，但你仍然输入提示词。',
    '/reload-skills 重新扫描 ~/.hermes/skills/，以便即插即用技能出现而无需重启会话。',
    '/indicator kaomoji|emoji|unicode|ascii 选择在 Agent 运行期间显示的 TUI 忙碌指示器样式。',
    '/debug 上传支持包（系统信息 + 日志）并返回可共享链接 — 在聊天中也有效。',

    # --- CLI 子命令与标志 ---
    'hermes -z "<prompt>" 是最纯粹的一次性：最终答案在 stdout 上，没有其他内容 — 非常适合在脚本中管道传输。',
    'hermes chat --pass-session-id 将会话 ID 注入系统提示词，以便 Agent 可以自我引用它。',
    'hermes chat --image path/to/pic.png 将本地图像附加到单个 -q 查询，而无需单独的上传步骤。',
    ...
    'hermes dump --show-keys 包含经过编辑的 API 密钥指纹，以进行更深层次的支持调试。',
    ...
    # --- Agent 行为环境变量 ---
    'HERMES_AGENT_TIMEOUT=0 禁用消息网关对正在运行的 Agent 的不活动终止 — 用于长时间研究运行。',
    ...
    # --- 消息网关行为环境变量 ---
    'HERMES_GATEWAY_BUSY_ACK_ENABLED=false 在用户向繁忙的 Agent 发送消息时静默 ⚡/⏳/⏩ 确认消息。',
    ...
    'image_gen.provider 通过插件（OpenAI Images、Codex、FAL）路由图像生成，而不是默认值。',
    'AUXILIARY_VISION_BASE_URL + AUXILIARY_VISION_API_KEY 将视觉分析指向任何 OpenAI 兼容端点。',
    'auxiliary.session_search.max_concurrency 限制并行总结多少个匹配的会话（默认 3）。',
    'auxiliary.session_search.extra_body 在总结调用上转发提供商特定的 OpenAI 兼容字段。',
    ...
    '会话 ID 带有时间戳前缀（20250305_091523_abcd），因此在 ls 和 jq 中排序自然有效。',

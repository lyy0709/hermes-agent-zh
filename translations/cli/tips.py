...
TIPS = [
    ...
    "/model 允许你在会话中途切换模型 — 尝试 /model sonnet 或 /model gpt-5。",
    ...
    "hermes chat --source telegram 标记会话，以便在 hermes sessions list 中过滤。",
    ...
    "hermes auth add 允许你添加多个 API 密钥以进行凭证池轮换。",
    ...
    "config.yaml 中的快速命令可立即运行 shell 命令，Token 使用量为零。",
    ...
    "delegate_task 默认生成最多 3 个并发子 Agent（delegation.max_concurrent_children），具有隔离的上下文以进行并行工作。",
    ...
    "send_message 可以从会话内到达任何已连接的消息平台。",
    ...
    # --- 消息网关与消息传递 ---
    "Hermes 在 21 个消息平台上运行：Telegram、Discord、Slack、WhatsApp、Signal、Matrix、IRC、Microsoft Teams、电子邮件等。",
    ...
    "hermes chat -Q 为编程使用启用安静模式 — 抑制横幅和旋转器。",
    ...
    "hermes login 支持基于 OAuth 的 Nous 和 OpenAI Codex 提供商认证。",
    "API 服务器支持 Chat Completions 和 Responses API，并具有服务器端状态。",
    "config.yaml 中的 tool_preview_length: 0 在旋转器的活动提要中显示完整的文件路径。",
    ...
    "SOUL.md 完全替换了 Agent 的默认身份 — 重写它以让 Hermes 成为你自己的。",
    ...
    "config.yaml 中的 network.force_ipv4: true 修复了在 IPv6 损坏的服务器上的挂起 — 猴子补丁 socket。",
    ...
    "write_file 和 patch 检测文件自上次读取后是否被外部修改，并警告过时。",
    ...
    "agent.tool_use_enforcement 引导描述动作而不是调用工具的模型 — auto 用于 GPT/Codex。",
    "agent.restart_drain_timeout（默认 60 秒）让正在运行的 Agent 在消息网关重启生效前完成。",
    "agent.api_max_retries（默认 3 次）控制 Agent 在将错误浮出水面之前重试失败 API 调用的次数 — 降低它以快速故障转移。",
    ...
    "容器模式：在 HERMES_HOME 中放置 .container-mode，主机 CLI 会自动执行到容器中。",
    ...
    "长危险命令（>70 个字符）在批准提示中获得一个 'view' 选项，以便先查看完整文本。",
    ...
    '/redraw 强制完整的 UI 重绘，修复 tmux 调整大小或鼠标选择伪影后的终端漂移。',
    ...
    '/kanban boards switch <slug> 从聊天内部更改活动的多项目看板板。',
    ...
    "HERMES_CRON_MAX_PARALLEL（默认 4）限制每个滴答运行多少个定时任务，以免突发流量饱和你的密钥。",
    ...
    # --- 凭证池与路由 ---
    'hermes auth reset <provider> 清除凭证池上的所有冷却和耗尽标志。',
    ...
    'Ctrl+G 或 Ctrl+X Ctrl+E 在 TUI 中在 $EDITOR 中打开输入缓冲区，用于长多行提示词。',
    ...
    # --- 环境变量与配置门控 ---
    "display.tool_progress_command: true 在消息平台上暴露 /verbose；默认情况下它仅限 CLI。",
    ...
    # --- API 服务器与代理 ---
    'API_SERVER_ENABLED=true 在消息网关旁边运行一个 OpenAI 兼容端点，用于 Open WebUI 和 LibreChat。',
    'GATEWAY_PROXY_URL 运行拆分设置：平台 I/O 本地，Agent 工作委托给远程 API 服务器。',

    # --- 平台特定 ---
    'MATRIX_DEVICE_ID 为 E2EE 固定一个稳定的设备 ID — 没有它，密钥每次启动都会轮换，历史解密会中断。',
    ...
    '/history 内联打印当前对话而不离开 CLI — 用于快速重读很有用。',
    '/save 将会话写入磁盘而不结束会话。',
    '/status 一目了然地显示会话信息：ID、标题、模型、Token 使用情况和经过时间。',
    '/image <path> 为你的下一个提示词附加本地图像文件，无需粘贴或拖放。',
    '/platforms 直接从聊天内部显示消息网关和消息平台连接状态。',
    ...
    '/debug 上传支持包（系统信息 + 日志）并返回可共享的链接 — 在聊天中也有效。',

    # --- CLI 子命令与标志 ---
    'hermes -z "<prompt>" 是最纯粹的一次性：最终答案在 stdout 上，没有其他内容 — 非常适合在脚本中管道传输。',
    'hermes chat --pass-session-id 将会话 ID 注入到系统提示词中，以便 Agent 可以自我引用它。',
    ...
    "hermes chat --source tool 标记程序化聊天，以便它们不会在 hermes sessions list 中杂乱显示。",
    'hermes dump --show-keys 包括经过编辑的 API 密钥指纹，用于更深层次的支持调试。',
    ...
    'HERMES_OAUTH_TRACE=1 记录经过编辑的 OAuth 令牌交换和刷新尝试，用于调试提供商认证。',
    'HERMES_STREAM_RETRIES（默认 3 次）控制中间流重连尝试以应对瞬态网络错误。',

    # --- 消息网关行为环境变量 ---
    'HERMES_GATEWAY_BUSY_ACK_ENABLED=false 当用户向繁忙的 Agent 发送消息时，静默 ⚡/⏳/⏩ 确认消息。',
    'HERMES_AGENT_NOTIFY_INTERVAL（默认 180 秒）设置消息网关在长轮次中 ping 进度的频率。',
    ...
    # --- 会话与源标签 ---
    '--source tool 聊天默认从 hermes sessions list 中排除 — 设置 --source 以查看它们。',

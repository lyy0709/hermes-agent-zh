...
TIPS = [
    # --- 斜杠命令 ---
    "/background <prompt>（别名 /bg 或 /btw）在单独的会话中运行任务，同时保持当前会话空闲。",
    "/branch 分支当前会话，以便您可以探索不同方向而不会丢失进度。",
    ...
    "/browser connect 通过 CDP 将浏览器工具附加到您正在运行的 Chromium 系列浏览器。",
    "/plugins 列出已安装的插件及其状态。",
    "/cron 管理定时任务 — 设置定期提示并交付到任何平台。",
    ...
    "@folder:src/ 注入目录树列表。",
    "@diff 将您未暂存的 git 更改注入到消息中。",
    "@staged 将您已暂存的 git 更改（git diff --staged）注入到消息中。",
    "@git:5 注入最后 5 次提交及其完整补丁。",
    ...
    "hermes -p work chat 在特定配置文件下运行而不更改您的默认配置。",
    ...
    "hermes config edit 在您的默认编辑器中打开 config.yaml。",
    ...
    "hermes models 将视觉、压缩和辅助任务路由到更便宜的模型 — 在不降低主聊天模型的情况下将后台 Token 成本降低 85% 以上。",
    ...
    "设置 display.busy_input_mode: queue 以排队消息而不是中断 Agent，或设置为 steer 以通过 /steer 在运行中注入它们。",
    ...
    "config.yaml 中的快速命令以零 Token 使用量即时运行 shell 命令。",
    ...
    "终端命令支持带有 notify_on_complete 的后台模式，用于长时间运行的任务。",
    ...
    # --- 消息网关与消息传递 ---
    "Hermes 在 21 个消息传递平台上运行：Telegram、Discord、Slack、WhatsApp、Signal、Matrix、IRC、Microsoft Teams、email 等。",
    "hermes gateway install 将其设置为启动时启动的系统服务。",
    ...
    "您的仓库根目录中的 .worktreeinclude 列出要复制到工作树中的 gitignored 文件。",
    ...
    "agent.tool_use_enforcement 引导描述操作而不是调用工具的模型 — 对 GPT/Codex 自动。",
    "agent.restart_drain_timeout（默认 60s）让正在运行的 Agent 在消息网关重启生效之前完成。",
    ...
    "~/.hermes/skills/.hub/audit.log 处的技能审计日志跟踪每次安装和移除操作。",
    "过时的 git 工作树会自动清理：在启动时修剪 24-72 小时旧且没有未推送提交的工作树。",
    ...
    "每个任务的委派模型：config 中的 delegation.model 和 delegation.provider 将子 Agent 路由到更便宜的模型。",
    ...
    "当压缩失败时，进一步尝试会暂停 10 分钟以避免 API 冲击。",
    "长危险命令（>70 个字符）在批准提示中获得“查看”选项，以便首先查看完整文本。",
    ...
    "每个任务的辅助覆盖：config.yaml 中的 auxiliary.vision.provider、auxiliary.compression.model 等。",
    ...
    "~/.hermes/skills/.hub/quarantine/ 处的技能隔离区保存待安全审查的技能。",

    # --- 高级斜杠命令 ---
    '/steer <prompt> 在下一次工具调用后注入注释 — 在任务中途微调方向而不中断。',
    '/goal <text> 设置一个持续的 Ralph 循环目标 — Hermes 在一轮又一轮后自动继续，直到判断者说完成。',
    '/snapshot create [label] 保存 Hermes 配置的完整状态快照；/snapshot restore <id> 稍后恢复。',
    '/copy [N] 将最后的助手响应复制到剪贴板，或使用数字将倒数第 N 个复制到剪贴板。',
    ...
    '/busy queue|steer|interrupt 控制在 Hermes 工作时按 Enter 的作用。',
    '/topic 在 Telegram 私信中启用用户管理的多会话主题模式 — /topic <id> 内联恢复过去的会话。',
    '/approve session|always 使用您选择的信任范围运行待处理的危险命令；/deny 拒绝它。',
    '/restart 在排空活动运行后优雅地重启消息网关，然后在恢复时 ping 请求者。',
    '/kanban boards switch <slug> 从聊天内部更改活动的多项目看板。',
    '/reload 将 ~/.hermes/.env 重新加载到正在运行的会话中 — 获取新的 API 密钥而无需重启。',

    # --- 定时任务（无 Agent 和脚本） ---
    '带有 no_agent=True 的定时任务按计划运行脚本并直接发送其 stdout — 零 Token，零 LLM。',
    '空的定时任务脚本 stdout 意味着静默滴答 — 不交付任何内容，非常适合阈值看门狗。',
    "HERMES_CRON_MAX_PARALLEL（默认 4）限制每个滴答运行多少个定时任务，以免突发流量使您的密钥饱和。",
    ...
    '放置一个 ~/.hermes/BOOT.md 清单，每次启动时 gateway:startup 钩子都会将其作为一次性 Agent 运行。',

    # --- 策展人 ---
    'hermes curator run --dry-run 预览策展人将归档或合并的内容，而不改变任何内容。',
    "hermes curator pin <skill> 硬隔离一个技能，防止自动归档和 Agent 的 skill_manage 工具。",
    'hermes curator rollback 从预运行快照恢复技能 — 备份位于 skills/.curator_backups/ 下。',

    # --- 凭据池与路由 ---
    'hermes auth reset <provider> 清除凭据池上的所有冷却和耗尽标志。',
    'credential_pool_strategies.<provider>: round_robin 均匀轮换密钥，而不是默认的 fill_first。',
    'use_gateway: true 每个工具将 Web、图像、tts 或浏览器通过您的 Nous 订阅路由 — 无需额外密钥。',
    'provider_routing.data_collection: deny 在 OpenRouter 上排除存储数据的提供商。',
    'provider_routing.require_parameters: true 仅路由到支持您请求中每个参数的提供商。',
    ...
    'TUI 内联渲染 LaTeX — $E=mc^2$ 变为 Unicode 数学而不是原始 TeX。',
    'hermes dashboard 在 127.0.0.1:9119 启动本地 Web UI — 零数据离开本地主机。',
    'hermes dashboard --tui 通过 xterm.js 和 WebSocket PTY 将完整的 Hermes TUI 嵌入到您的浏览器中。',
    '在 ~/.hermes/dashboard-themes/ 中放置一个带有两种调色板颜色的 YAML 以重新设计整个仪表板的外观。',
    '仪表板插件是即插即用的：~/.hermes/dashboard-plugins/ 中的 manifest.json + JS 包 — 无需 npm 构建。',
    ...
    'HERMES_BACKGROUND_NOTIFICATIONS=result 仅在后台任务完成时 ping（对比 all/error/off）。',
    'HERMES_WRITE_SAFE_ROOT 将 write_file 和 patch 限制在目录前缀；外部写入需要批准。',
    ...
    'auxiliary.goal_judge.model 将 /goal 判断者路由到廉价快速模型，以将循环成本保持在接近零。',
    ...
    'GATEWAY_PROXY_URL 运行拆分设置：平台 I/O 本地，Agent 工作委托给远程 API 服务器。',

    # --- 平台特定 ---
    'MATRIX_DEVICE_ID 固定一个稳定的设备 ID 用于 E2EE — 没有它，密钥每次启动都会轮换，历史解密会中断。',
    'TELEGRAM_WEBHOOK_SECRET 是设置 TELEGRAM_WEBHOOK_URL 时的必需项 — 使用 openssl rand -hex 32 生成。',
    ...
    '/image <path> 为您的下一个提示词附加本地图像文件，无需粘贴或拖放。',
    '/platforms 直接从聊天内部显示消息网关和消息传递平台连接状态。',
    '/commands 对完整的斜杠命令 + 已安装技能列表进行分页 — 在没有 Tab 补全的平台上很有用。',
    '/toolsets 列出每个可用的工具集，以便您知道 -t/--toolsets 接受什么。',
    '/gquota 显示 Google Gemini Code Assist 配额使用情况，并在该提供商处于活动状态时显示进度条。',
    '/voice tts 切换仅 TTS 模式 — Agent 回复大声说出，但您仍然输入提示词。',
    ...
    # --- CLI 子命令与标志 ---
    'hermes -z "<prompt>" 是最纯粹的一次性：stdout 上的最终答案，没有其他内容 — 非常适合在脚本中管道传输。',
    'hermes chat --pass-session-id 将会话 ID 注入系统提示词，以便 Agent 可以自我引用它。',
    'hermes chat --image path/to/pic.png 将本地图像附加到单个 -q 查询，无需单独的上传步骤。',
    'hermes chat --ignore-user-config 跳过活动用户配置 — 可重现的错误报告和 CI 运行。',
    "hermes chat --source tool 标记程序化聊天，以免它们弄乱 hermes sessions list。",
    'hermes dump --show-keys 包含经过编辑的 API 密钥指纹以进行更深层次的支持调试。',
    ...
    'hermes pairing 轮换私信配对令牌 — 轮换后的第一个消息发送者获得对机器人的访问权限。',
    'hermes setup 引导首次用户在一个交互式流程中完成提供商、密钥和平台连接。',
    ...
    "HERMES_DISABLE_FILE_STATE_GUARD=1 关闭 patch 和 write_file 上的“自您读取以来文件已更改”防护。",
    ...
    'HERMES_OAUTH_TRACE=1 记录经过编辑的 OAuth 令牌交换和刷新尝试以调试提供商授权。',
    'HERMES_STREAM_RETRIES（默认 3）控制瞬态网络错误上的流中重新连接尝试。',

    # --- 消息网关行为环境变量 ---
    'HERMES_GATEWAY_BUSY_ACK_ENABLED=false 当用户向繁忙的 Agent 发送消息时，静默 ⚡/⏳/⏩ 确认消息。',
    'HERMES_AGENT_NOTIFY_INTERVAL（默认 180s）设置消息网关在长轮次中多久 ping 一次进度。',
    'HERMES_RESTART_DRAIN_TIMEOUT（默认 900s）限制 /restart 在强制之前等待正在运行的运行的时间。',
    'HERMES_CHECKPOINT_TIMEOUT（默认 30s）限制文件系统检查点创建 — 在大型单体仓库上提高它。',
    ...
    # --- 会话与源标签 ---
    '默认情况下，--source tool 聊天被排除在 hermes sessions list 之外 — 设置 --source 以查看它们。',
    '会话 ID 带有时间戳前缀（20250305_091523_abcd），因此在 ls 和 jq 中自然排序有效。',

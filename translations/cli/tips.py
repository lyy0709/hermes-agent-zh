...
TIPS = [
    ...
    "/queue <prompt> 将消息排队等待下一轮，而不中断当前轮次。",
    "/undo 从对话中移除最后一条用户/助手交换。",
    "/retry 重新发送您的最后一条消息 — 当 Agent 的响应不太准确时很有用。",
    ...
    "/stop 终止由 Agent 启动的所有正在运行的后台进程。",

    # --- @ 上下文引用 ---
    "@file:path/to/file.py 将文件内容直接注入您的消息。",
    "@file:main.py:10-50 仅注入文件的第 10-50 行。",
    "@folder:src/ 注入目录树列表。",
    "@diff 将您未暂存的 git 更改注入消息。",
    "@staged 将您已暂存的 git 更改（git diff --staged）注入消息。",
    "@git:5 注入最近 5 次提交的完整补丁。",
    ...
    "hermes profile create coder 创建一个隔离的配置文件，该配置文件成为其自己的命令。",
    ...
    "hermes memory setup 让您配置外部记忆提供商（Honcho、Mem0 等）。",
    ...
    # --- 配置 ---
    "在 config.yaml 中设置 display.bell_on_complete: true，以便在长任务完成时听到提示音。",
    ...
    "设置 security.website_blocklist.enabled: true 以阻止特定域访问 Web 工具。",
    ...
    "search_files 基于 ripgrep，比 grep 更快 — 使用它代替终端 grep。",
    ...
    "browser_vision 截取屏幕截图并使用 AI 分析 — 适用于 CAPTCHA 和视觉内容。",
    ...
    "如果两个配置文件意外共享一个 bot token，第二个消息网关会被阻止并显示清晰的错误。",
    ...
    "API 服务器暴露与 Open WebUI 和 LibreChat 兼容的 OpenAI 兼容端点。",
    ...
    "子目录 AGENTS.md 文件在 Agent 导航到文件夹时逐步发现。",
    ...
    "browser_vision 带有 annotate=true 在交互元素上叠加编号标签。",
    ...
    "HERMES_EPHEMERAL_SYSTEM_PROMPT 注入一个永远不会持久化到历史的系统提示词。",
    ...
    # --- 隐藏宝石与高级用户技巧 ---
    "定时任务可以附加 Python 脚本（--script），其 stdout 作为上下文注入提示词。",
    "定时任务脚本位于 ~/.hermes/scripts/ 中，并在 Agent 之前运行 — 非常适合数据收集流水线。",
    "config.yaml 中的 prefill_messages_file 将少样本示例注入每个 API 调用，永远不会保存到历史记录。",
    "SOUL.md 完全取代 Agent 的默认身份 — 重写它以让 Hermes 成为您自己的。",
    "SOUL.md 在首次运行时自动植入默认个性。编辑 ~/.hermes/SOUL.md 以自定义。",
    "/compress <焦点主题> 将 60-70% 的摘要预算分配给您的主题，并积极修剪其余部分。",
    "在第二次及以后的压缩中，压缩器会更新先前的摘要而不是从头开始。",
    ...
    "delegate_task 带有 acp_command: 'claude' 从任何平台生成 Claude Code 作为子 Agent。",
    ...
    "config.yaml 中的 context.engine 可以设置为插件名称以使用替代的上下文管理策略。",
    "超过 8000 个 Token 的浏览器页面在返回给 Agent 之前由辅助 LLM 自动总结。",
    "压缩器进行廉价预传递：超过 200 字符的工具输出在 LLM 运行前被占位符替换。",
